...
> 2.LauncherActivity#intentForPosition()，这个是获取包名和类名。隐式启动，跨进程一般用intent隐式启动activity
...
在横竖屏切换的过程中，会发生Activity被销毁并重建的过程。两个回调：**onSaveInstanceState和onRestoreInstanceState**。通过onSaveInstanceState来保存当前Activity的状态，通过onRestoreInstanceState方法来恢复Activity的状态。
...
- 栈顶复用模式（singleTop）：如果要启动的Activity已经位于任务栈的顶部，不会创建新的实例，而是直接调用其onNewIntent()方法。适用于需要频繁切换到同一Activity的场景。
- 栈内复用模式（singleTask）：如果要启动的Activity已经存在于任务栈中，会将该Activity上面的其他Activity全部出栈，使其成为栈顶，并调用其onNewIntent()方法。适用于需要保持单一实例的界面，如主界面。
- 单例模式（singleInstance）：每次启动该Activity都会创建一个新的任务栈，并且该任务栈中只有一个实例。适用于需要独立于其他任务栈的特殊界面，如应用程序的设置界面。
