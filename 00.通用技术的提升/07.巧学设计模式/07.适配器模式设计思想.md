# 07.适配器模式设计思想
#### 目录介绍
- [01.适配器模式基础](#01适配器模式基础)
    - [1.1 适配器模式由来](#11-适配器模式由来)
    - [1.2 适配器模式定义](#12-适配器模式定义)
    - [1.3 适配器模式场景](#13-适配器模式场景)
    - [1.4 适配器模式思考](#14-适配器模式思考)
- [02.适配器模式实现](#02适配器模式实现)
    - [2.1 罗列一个场景](#21-罗列一个场景)
    - [2.2 用例子理解适配器](#22-用例子理解适配器)
    - [2.3 适配器基本实现](#23-适配器基本实现)
    - [2.4 如何选择适配器](#24-如何选择适配器)
- [03.适配器模式分析](#03适配器模式分析)
    - [3.1 类适配器案例](#31-类适配器案例)
    - [3.2 对象适配器案例](#32-对象适配器案例)
    - [3.3 适配器模式结构图](#33-适配器模式结构图)
    - [3.4 适配器模式时序图](#34-适配器模式时序图)
- [04.适配器应用解析](#04适配器应用解析)
    - [4.1 读卡器适配案例【类】](#41-读卡器适配案例类)
    - [4.2 类适配器优缺点](#42-类适配器优缺点)
    - [4.3 播放器适配器案例【对象】](#43-播放器适配器案例对象)
    - [4.4 对象适配器优缺点](#44-对象适配器优缺点)
- [05.实际场景演变](#05实际场景演变)
    - [5.1 封装有缺陷的接口设计](#51-封装有缺陷的接口设计)
    - [5.2 设计统一多个类的接口](#52-设计统一多个类的接口)
    - [5.3 替换依赖的外部系统](#53-替换依赖的外部系统)
    - [5.4 兼容老版本接口](#54-兼容老版本接口)
    - [5.5 适配不同格式的数据](#55-适配不同格式的数据)
    - [5.6 Java适配器模式使用](#56-java适配器模式使用)
    - [5.7 数据库适配器](#57-数据库适配器)
    - [5.8 日志框架适配器](#58-日志框架适配器)
    - [5.9 第三方库适配器](#59-第三方库适配器)
- [06.适配器总结一下](#06适配器总结一下)
    - [6.1 适配器使用环境](#61-适配器使用环境)
    - [6.2 适配器模式拓展](#62-适配器模式拓展)
    - [6.3 跟其他模式对比](#63-跟其他模式对比)
- [07.动态代理总结](#07动态代理总结)
    - [7.1 总结一下学习](#71-总结一下学习)
    - [7.2 更多内容推荐](#72-更多内容推荐)
