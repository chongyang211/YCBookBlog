...
几乎每个项目都会有一个"读配置"的工具类——比如读 `application.properties`、读环境变量、读数据库连接字符串。一个新人入职，第一反应往往是这样写：
...
整个调用链路看上去也挺正常，但只要画一张对象关系图，就会发现问题：
...
每个调用方都拿到一份"自己的副本"，互不感知。

### 1.2 它哪里不舒服

- ❌ **重复 I/O**：每 `new` 一次都重新读文件、重新解析，明明配置加载后就不变；
- ❌ **状态分裂**：如果有人调了 `cfg.set("xxx")` 改了内存值，`cfg2` 完全不知道，bug 当场上演；
- ❌ **资源浪费**：底层的连接池、缓存、线程池如果也跟着多份，内存直接翻倍。

> 这类 "**对象只该有一个**、并且**全局都该看到同一个**" 的场景，就是单例模式登场的时刻。
...
下面 6 种实现方式各有侧重，可以先看这张选型图，再带着问题进入每一节：
