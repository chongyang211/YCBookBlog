...
- [01.整体概述介绍](#01整体概述介绍)
    ...
    - [1.3 设计目标](#13-设计目标)
    - [1.4 收益分析](#14-收益分析)
- [02.市面抓包的分析](#02市面抓包的分析)
    - [2.1 Https三要素](#21-https三要素)
    - [2.2 抓包核心原理](#22-抓包核心原理)
    - [2.3 搞定CA证书](#23-搞定ca证书)
    - [2.4 突破CA证书校验](#24-突破ca证书校验)
    - [2.5 如何搞定加解密](#25-如何搞定加解密)
    - [2.6 Charles原理](#26-charles原理)
    - [2.7 抓包原理图](#27-抓包原理图)
    - [2.8 抓包核心流程](#28-抓包核心流程)
- [03.防止抓包思路](#03防止抓包思路)
    - [3.1 先看如何抓包](#31-先看如何抓包)
    - [3.2 设置配置文件](#32-设置配置文件)
    - [3.3 数据加密处理](#33-数据加密处理)
    - [3.4 避免黑科技抓包](#34-避免黑科技抓包)
- [04.防抓包实践开发](#04防抓包实践开发)
  - [4.1 App安全配置](#41-app安全配置)
  - [4.2 关闭代理](#42-关闭代理)
  - [4.3 证书校验](#43-证书校验)
  - [4.4 双向认证](#44-双向认证)
  - [4.5 防止挂载抓包](#45-防止挂载抓包)
  - [4.6 数据加解密](#46-数据加解密)
  - [4.7 证书锁定](#47-证书锁定)
  - [4.8 Sign签名](#48-sign签名)
  - [4.9 其他的方式](#49-其他的方式)
- [05.架构设计说明](#05架构设计说明)
    ...
    - [5.3 稳定性设计](#53-稳定性设计)
    - [5.4 降级设计](#54-降级设计)
    - [5.5 异常设计说明](#55-异常设计说明)
- [06.防抓包功能自测](#06防抓包功能自测)
    - [6.1 网络请求测试](#61-网络请求测试)
    - [6.2 抓包测试](#62-抓包测试)
    - [6.3 黑科技挂载测试](#63-黑科技挂载测试)
    - [6.4 逆向破解测试](#64-逆向破解测试)
...
**在写开放的API接口时是如何保证数据的安全性的？** 请求来源(身份)是否合法？请求参数被篡改？请求的唯一性(不可复制)？

问题的解决方案设想：为了保证数据在通信时的安全性，我们可以采用参数签名的方式来进行相关验证。

最终决定的解决方案：调用接口之前需要验证签名和有效时间，要生成一个sign签名。先拼接-后转码-再加密-再发请求！

sign签名校验实践

1. 需要对请求参数进行签名验证，签名方式如下：key1=value1&key2=value2&key3=value3&secret=yc 。对这个字符串进行md5一下。
2. 然后被sign后的接口就变成了：http://api.test.com/getbanner?key1=value1&key2=value2&key3=value3&sign=xxx
3. 为什么在获取sign的时候建议使用secret参数？secret仅作加密使用，添加在参数中主要是md5，为了保证数据安全请不要在请求参数中使用。

服务端对sign校验。这样请求的时候就需要合法正确签名sign才可以获取数据。这样就解决了身份验证和防止参数篡改问题，如果请求参数被人拿走，没事，他们永远也拿不到secret，因为secret是不传递的。再也无法伪造合法的请求。
...
Sign签名安全性分析：通过上面的案例，安全的关键在于参与签名的secret，整个过程中secret是不参与通信的，所以只要保证secret不泄露，请求就不会被伪造。
