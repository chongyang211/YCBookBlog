...
数据结构在计算机科学中扮演着至关重要的角色：

1. **提高程序效率**：合适的数据结构能显著提升算法性能
2. **简化程序设计**：良好的数据组织使程序逻辑更清晰
3. **节省存储空间**：优化的存储方式减少内存占用
4. **增强代码可维护性**：结构化的数据便于理解和修改
...
```python
# 数据的不同形式示例
integer_data = 42                    # 整数数据
string_data = "Hello World"          # 字符串数据
float_data = 3.14159                 # 浮点数据
boolean_data = True                  # 布尔数据
list_data = [1, 2, 3, 4, 5]         # 列表数据
dict_data = {"name": "Alice", "age": 25}  # 字典数据
```
...
```python
# 学生信息中的数据项
student_id = "20230001"      # 学号数据项
student_name = "张三"        # 姓名数据项
student_age = 20             # 年龄数据项
student_gender = "男"        # 性别数据项
student_score = 85.5         # 成绩数据项
```

#### 2.2.3 数据元素（Data Element）
...
class Student:
    def __init__(self, student_id, name, age, gender, score):
        self.student_id = student_id    # 数据项
        self.name = name                # 数据项
        self.age = age                  # 数据项
        self.gender = gender            # 数据项
        self.score = score              # 数据项
    
    def __str__(self):
        return f"Student({self.student_id}, {self.name}, {self.age})"

# 创建数据元素
student1 = Student("20230001", "张三", 20, "男", 85.5)
student2 = Student("20230002", "李四", 19, "女", 92.0)
```

#### 2.2.4 数据结构（Data Structure）
