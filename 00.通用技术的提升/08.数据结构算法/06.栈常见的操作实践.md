...
假设你是 Chrome 浏览器的开发工程师，你会如何实现这个功能呢？答案是使用**两个栈**：

- **后退栈（Back Stack）**：每访问一个新页面，将当前页面压入后退栈
- **前进栈（Forward Stack）**：每次后退时，将当前页面压入前进栈；访问新页面时清空前进栈

这就是"栈"这种数据结构的经典应用——**用LIFO（后进先出）特性天然匹配"撤销/恢复"的语义**。
...
class BrowserHistory {
    ...
    void visit(String url) {
        ...
        currentPage = url;
        forwardStack.clear();  // 访问新页面，清空前进栈
    }
...
| 场景 | 栈的作用 | LIFO体现 |
|------|---------|---------|
| 函数调用 | 调用栈 | 最后调用的函数最先返回 |
| 括号匹配 | 操作数栈 | 最后打开的括号最先关闭 |
| 表达式求值 | 操作数+运算符栈 | 先处理优先级最高的运算 |
| 撤销操作 | 操作历史栈 | 撤销最近的操作 |
| DFS遍历 | 路径栈 | 回溯到最近的分支点 |
| 编译器语法分析 | 分析栈 | 递归下降中最内层最先归约 |

## 02.什么是栈

### 2.1 栈的定义

栈（Stack）是一种线性数据结构，它遵循**后进先出（LIFO - Last In First Out）** 的原则。
...
**关键特征**：所有操作都是 O(1)——这是栈的设计目标。栈通过限制操作（只允许在一端操作）来换取极致的性能。
...
|------|----------------|----------------|
| 内存 | 连续，预分配 | 离散，按需分配 |
| 缓存性能 | 优秀（连续访问） | 较差（指针跳转） |
| 扩容 | 需要搬移数据 | 天然动态 |
| 空间开销 | 可能有空闲位置 | 每个节点有指针开销 |
| 实现难度 | 较简单 | 稍复杂（指针管理） |
| 工业选择 | Java Stack、Python list | 函数式语言的栈 |

**各语言标准库的栈实现选择**：

- **Java**：`Stack` 类基于 `Vector`（数组），`ArrayDeque` 也基于数组（推荐）
- **C++ STL**：`std::stack` 默认基于 `std::deque`（分段连续内存）
- **Python**：直接使用 `list`（动态数组），append/pop 操作栈
- **Go**：使用 `slice`（动态数组），没有内置栈类型
- **Rust**：使用 `Vec`（动态数组），push/pop 操作栈

### 3.2 数组实现栈

数组实现栈是最常用的方式，核心思路是用一个数组存储数据，用一个 `top` 变量指示栈顶位置。
...
链表栈以链表头部作为栈顶，Push/Pop 都在链表头部操作，天然 O(1)，且无需担心容量问题。
...
void stack_push(LinkedStack* s, int value) {
...
int stack_pop(LinkedStack* s, int* out) {
...
**链表栈的优势场景**：当无法预估栈的最大深度，或者需要频繁创建/销毁栈时（如多线程环境下每个线程独立的栈），链表栈避免了数组扩容的开销。
...
**为什么选择 1/4 作为缩容阈值而不是 1/2？** 如果用 1/2 作为阈值，当元素数量在容量的一半附近反复增删时，会不断触发扩容→缩容→扩容的"乒乓效应"。用 1/4 留出了足够的缓冲区间。
...
**1. 基础理解**：用数组实现的栈和用链表实现的栈，在 `push`/`pop` 操作上时间复杂度都是 O(1)。那它们在实际性能上有差异吗？（提示：考虑内存分配频率和缓存局部性）

**2. 经典应用**：如何用两个栈实现一个队列？请写出 `enqueue` 和 `dequeue` 的代码，并分析均摊复杂度。反过来，如何用两个队列实现一个栈？哪种实现更"自然"？

**3. 深入思考**：函数调用栈的大小通常是固定的（如 Linux 默认 8MB，JVM 默认 512KB~1MB）。为什么操作系统不把调用栈设计成可以自动扩展的？如果允许无限递归，会带来什么安全问题？Go 语言的 goroutine 栈是可以动态增长的（初始仅 2KB），它是怎么做到的？

**4. 算法设计**：设计一个支持 `push`、`pop`、`getMin` 三个操作都是 O(1) 的"最小栈"。空间上最优的方案是什么？如果进一步要求支持 `getMax`（同时获取最大值和最小值），该怎么设计？

**5. 工程实践**：在表达式求值中，中缀表达式 `3 + 4 * 2 / (1 - 5)` 需要考虑运算符优先级和括号。请完整描述"中缀转后缀（逆波兰表达式）+ 后缀求值"的双栈算法流程，并思考：为什么编译器更喜欢处理后缀表达式而不是中缀表达式？
