...
**1. 基础理解**：ArrayList 的默认初始容量是 10（JDK 8+ 采用延迟初始化，`new ArrayList()` 时实际为空数组）。请问：第一次 `add` 时扩容到 10，之后按 1.5 倍增长。如果要插入 100 个元素，会触发几次扩容？每次扩容后的容量分别是多少？

**2. 浅拷贝陷阱**：`Arrays.copyOf()` 和 `System.arraycopy()` 都是浅拷贝。请构造一个例子，展示 ArrayList 扩容后新旧数组共享引用类型元素的问题。在实际开发中，这可能导致什么难以排查的 bug？

**3. Fail-Fast 机制**：ArrayList 的迭代器通过 `modCount` 实现 fail-fast。请思考：(1) 为什么 `modCount` 不用 `volatile` 修饰？(2) 在多线程环境下，fail-fast 能保证一定抛出 `ConcurrentModificationException` 吗？(3) 为什么说 fail-fast 是"尽力而为"的检测而非严格保证？

**4. 性能陷阱**：在 ArrayList 的头部（index=0）频繁插入元素，每次都需要移动所有已有元素。请计算插入 N 个元素到头部的总复制次数（提示：1+2+3+...+N）。如果必须在头部频繁插入，应该用什么替代方案？`LinkedList` 是好的选择吗？为什么？

**5. 设计决策**：ArrayList 在 `remove` 后不会自动缩容（除非手动调用 `trimToSize()`）。请分析这个设计决策的原因。如果自动缩容，应该在什么时机触发？负载因子低于多少时缩容？缩容到原来的多少？Go 的 slice 和 Rust 的 Vec 在缩容策略上有什么不同？
