...
树形结构（BST）：每次操作排除一半元素
  查找过程：
        50
       /  \
     30    70      50 > 35? 往左走，排除右半边
    /  \   / \
  20  40 60  80    30 < 35? 往右走，排除左半边
              
  N个元素查找：最多log₂N次比较 → O(log N)
```

**量化对比——为什么log N如此强大**：

| 数据量N | 线性查找O(N) | 树查找O(log N) | 加速比 |
|---------|------------|----------------|--------|
| 1,000 | 500次 | 10次 | 50倍 |
| 100万 | 50万次 | 20次 | 25000倍 |
| 10亿 | 5亿次 | 30次 | 1667万倍 |

**树结构的其他优势**：
- **天然表达层级关系**：文件系统目录、公司组织架构、XML/JSON/HTML DOM
- **支持范围查询**：BST中序遍历天然有序，可快速查找">=30且<=70"的所有元素
- **动态性好**：不像有序数组那样插入需要O(N)搬移，BST插入只需O(log N)
...
├── 需要按排序顺序处理
│   └── 中序遍历（In-order）
│   └── 场景：BST排序输出、验证BST合法性、查找第K小
│
├── 需要先处理完子节点，才能处理父节点
│   └── 后序遍历（Post-order）
│   └── 场景：删除树（先删子再删父）、计算子树大小、
│            表达式树求值（先算操作数再算运算符）
│
├── 需要逐层处理
│   └── 层序遍历（Level-order / BFS）
│   └── 场景：求最小深度、按层打印、序列化（LeetCode格式）
│
└── 需要找最近公共祖先/路径
    └── 后序遍历（自底向上回溯）
```
...
| 遍历 | 最好空间 | 最坏空间 | 平衡树空间 |
|------|---------|---------|----------|
| 前/中/后序（递归） | O(log N) | O(N)（退化链表） | O(log N) |
| 层序（BFS） | O(1)（斜树） | O(N/2)（满二叉树最后一层） | O(N/2) |

层序遍历在**满二叉树**时空间最大（最后一层有N/2个节点），而DFS在**链状退化树**时空间最大（递归深度=N）。
...
有序数组二分查找的过程：
  ...
  第1次：取中间 50 → 目标比50大？往右看
  第2次：取中间 70 → 目标比70小？往左看  
  第3次：取中间 60 → 找到了！
...
| 插入 | O(N)搬移 | O(1) | O(log N) |
| 删除 | O(N)搬移 | O(1)*已定位 | O(log N) |
| 范围查询 | O(log N + K) | O(N) | O(log N + K) |
| 有序遍历 | O(N) | O(N·log N)需排序 | O(N)中序遍历 |

BST完美结合了数组的查找效率和链表的插入灵活性——前提是树保持平衡。
...
数组存储：[90, 70, 80, 50, 60, 40, 30]
索引关系：父节点 i → 左子 2i+1, 右子 2i+2
         子节点 i → 父节点 (i-1)/2
...
| 问题 | 选择 | 原因 |
|------|------|------|
| 找前K大的元素 | **小顶堆**（容量K） | 堆顶是第K大，比堆顶小的直接丢弃 |
| 找前K小的元素 | **大顶堆**（容量K） | 堆顶是第K小，比堆顶大的直接丢弃 |
| 优先级调度 | **大顶堆** | 优先级最高的在堆顶 |
| 合并K个有序链表 | **小顶堆** | 每次取最小的元素 |
| 中位数维护 | 大顶堆+小顶堆 | 左半部分大顶堆，右半部分小顶堆 |

```java
// Top-K问题：从10亿个数中找最大的100个
// 错误思路：排序全部数据 O(N log N)，内存放不下
// 正确思路：维护大小为100的小顶堆 O(N log K)
...
// minHeap中就是最大的100个数，空间O(K)，时间O(N·logK)
```
...
**1. 基础理解**：二叉树的前序、中序、后序遍历都可以用递归轻松实现。请分别写出这三种遍历的**非递归（迭代）**实现。其中后序遍历的非递归实现最复杂，请思考为什么。

**2. 平衡的代价**：AVL 树要求任意节点左右子树高度差不超过 1，红黑树只要求最长路径不超过最短路径的 2 倍。请思考：为什么"更严格的平衡"反而导致 AVL 树在频繁插入/删除场景下性能不如红黑树？具体来说，AVL 树的旋转频率和红黑树相比如何？

**3. 堆的应用**：Top-K 问题（从 N 个元素中找出最大的 K 个）有两种堆解法：(1) 大顶堆放全部元素然后取 K 次；(2) 维护大小为 K 的小顶堆。请分析这两种方案的时间/空间复杂度，并说明为什么方案 2 在 N 远大于 K 时更优。

**4. 序列化与唯一性**：已知前序遍历和中序遍历可以唯一确定一棵二叉树，但前序遍历和后序遍历不能。请给出一个反例。如果是满二叉树（每个节点要么有 0 个子节点要么有 2 个），前序 + 后序能唯一确定吗？

**5. 工业级思考**：B 树和 B+ 树是数据库索引的核心数据结构。请思考：为什么数据库不直接用红黑树做索引？B+ 树相比 B 树有什么优势？为什么 B+ 树的叶子节点要用链表连起来？（提示：从磁盘 IO 次数和范围查询两个角度分析）
