...
**1. 基础理解**：循环队列用数组实现时，如何区分"队列满"和"队列空"（都是 `front == rear`）？请列出至少两种解决方案，并分析各自的优缺点。

**2. 设计分析**：Java 的 `ArrayBlockingQueue` 和 `LinkedBlockingQueue` 都是阻塞队列，但内部实现完全不同。前者用一把锁，后者用两把锁（putLock 和 takeLock）。请分析为什么 `LinkedBlockingQueue` 可以用两把锁而 `ArrayBlockingQueue` 不行？这对并发性能有什么影响？

**3. 无锁原理**：CAS（Compare-And-Swap）是无锁队列的核心。请解释 ABA 问题：线程 A 读到值 X，线程 B 把 X 改成 Y 再改回 X，线程 A 的 CAS 仍然成功——这会导致什么 bug？Java 的 `AtomicStampedReference` 如何解决 ABA 问题？

**4. 性能极致**：Disruptor 号称比 `ArrayBlockingQueue` 快 100 倍。请分析 Disruptor 的三个核心优化：(1) 环形数组预分配避免 GC；(2) 缓存行填充避免伪共享；(3) 无锁序号发布。其中"伪共享"是什么？为什么 64 字节的 padding 可以消除它？

**5. 实战设计**：设计一个"延迟队列"（消息到达指定时间后才可消费）。请思考至少两种实现方案（如堆 + 轮询、时间轮），并分析各自的时间复杂度和适用场景。Kafka、RocketMQ 是如何实现延迟消息的？
