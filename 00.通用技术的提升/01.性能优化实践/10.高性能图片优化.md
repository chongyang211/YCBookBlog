# 高性能图片优化
#### 目录介绍
- [01.图片基础概念介绍](#01图片基础概念介绍)
  - [1.1 图片占用内存介绍](#11-图片占用内存介绍)
  - [1.2 网络图片加载流程](#12-网络图片加载流程)
  - [1.3 三方库加载图片逻辑](#13-三方库加载图片逻辑)
  - [1.4 BitmapFactory](#14-bitmapfactory)
  - [1.5 图片大小VS内存](#15-图片大小vs内存)
  - [1.6 Bitmap能直接存储吗](#16-bitmap能直接存储吗)
  - [1.7 Bitmap创建流程](#17-bitmap创建流程)
  - [1.8 图片框架如何设计](#18-图片框架如何设计)
- [02.图片内存计算方式](#02图片内存计算方式)
  - [2.1 如何计算占用内存](#21-如何计算占用内存)
  - [2.2 上面计算内存对吗](#22-上面计算内存对吗)
  - [2.3 一个像素占用内存](#23-一个像素占用内存)
  - [2.4 使用API获取内存](#24-使用api获取内存)
  - [2.5 影响Bitmap内存因素](#25-影响bitmap内存因素)
  - [2.6 加载xhdpi和xxhdpi图片](#26-加载xhdpi和xxhdpi图片)
  - [2.7 图片一些注意事项](#27-图片一些注意事项)
- [03.大图的内存优化](#03大图的内存优化)
  - [3.0 图片压缩核心思想](#30-图片压缩核心思想)
  - [3.1 常见图片压缩](#31-常见图片压缩)
  - [3.2 图片尺寸压缩](#32-图片尺寸压缩)
  - [3.3 图片质量压缩](#33-图片质量压缩)
  - [3.4 双线性采样压缩](#34-双线性采样压缩)
  - [3.5 高清图分片加载](#35-高清图分片加载)
  - [3.6 鲁班图片综合压缩](#36-鲁班图片综合压缩)
- [04.色彩格式及内存优化](#04色彩格式及内存优化)
  - [4.1 RGB颜色种类](#41-rgb颜色种类)
  - [4.2 ARGB色彩模式](#42-argb色彩模式)
  - [4.3 改变色彩格式优化](#43-改变色彩格式优化)
- [05.图片内存缓存设计](#05图片内存缓存设计)
  - [5.1 图片内存缓存思想](#51-图片内存缓存思想)
  - [5.2 Lru内存缓存](#52-lru内存缓存)
  - [5.3 Lru缓存注意事项](#53-lru缓存注意事项)
  - [5.4 使用Lru磁盘缓存](#54-使用lru磁盘缓存)
- [06.不同版本对Bitmap管理](#06不同版本对bitmap管理)
  - [6.1 演变进程](#61-演变进程)
  - [6.2 管理Bitmap内存](#62-管理bitmap内存)
  - [6.3 提高Bitmap复用](#63-提高bitmap复用)
- [07.图片其他方面优化](#07图片其他方面优化)
  - [7.1 减少PNG图片使用](#71-减少png图片使用)
  - [7.2 控件切割圆角优化](#72-控件切割圆角优化)
  - [7.3 如何给图片置灰色](#73-如何给图片置灰色)
  - [7.4 如何处理图片旋转呢](#74-如何处理图片旋转呢)
  - [7.5 保存图片且刷相册](#75-保存图片且刷相册)
  - [7.6 统一图片域名优化](#76-统一图片域名优化)
  - [7.7 优化H5图片加载](#77-优化h5图片加载)
  - [7.8 优化图片阴影效果](#78-优化图片阴影效果)
  - [7.9 图片资源的压缩](#79-图片资源的压缩)
...
**疑惑**：图片格式那么多（JPEG、PNG、WebP、AVIF、HEIF），到底该怎么选？

**答疑**：每种格式的设计目标不同，需要根据场景选择。

**论证**：

| 格式 | 有损/无损 | 透明度 | 动画 | 压缩率 | 解码速度 | 平台支持 |
|------|---------|-------|------|--------|---------|---------|
| JPEG | 有损 | 不支持 | 不支持 | 中等 | 快 | 全平台 |
| PNG | 无损 | 支持 | 不支持 | 低 | 快 | 全平台 |
| WebP | 有损+无损 | 支持 | 支持 | 高（比JPEG小25-35%） | 中等 | Android 4.0+/iOS 14+/所有现代浏览器 |
| AVIF | 有损+无损 | 支持 | 支持 | 极高（比WebP小20%） | 慢 | Android 12+/iOS 16+/Chrome 85+ |
| HEIF | 有损+无损 | 支持 | 支持 | 高 | 中等 | iOS 11+/Android 10+/浏览器不支持 |

选择策略：
- **照片类内容**：优先WebP有损，降级JPEG
- **图标/Logo**：优先WebP无损，降级PNG
- **需要透明度**：WebP > PNG（WebP无损比PNG小26%）
- **极致压缩**：AVIF > WebP（但需考虑兼容性和解码性能）
- **iOS原生拍照**：HEIF（系统默认格式，空间占用小）

**结果**：WebP是目前最佳的通用选择，兼顾压缩率和平台支持。AVIF是未来趋势，但解码性能和兼容性仍需时间完善。

### 8.2 跨平台图片加载架构对比

| 维度 | Android | iOS | Web |
|------|---------|-----|-----|
| 主流框架 | Glide/Coil | SDWebImage/Kingfisher | 浏览器原生/<img> + lazy |
| 内存缓存 | LruCache | NSCache | 浏览器Cache API |
| 磁盘缓存 | DiskLruCache | 文件系统缓存 | Cache Storage/HTTP Cache |
| 解码方式 | BitmapFactory/ImageDecoder | UIImage/CGImage | 浏览器自动解码 |
| 采样压缩 | inSampleSize/inTargetDensity | downsamplingRatio | CSS object-fit/srcset |
| 大图加载 | BitmapRegionDecoder | CATiledLayer | CSS background-size |
| 像素格式 | ARGB_8888/RGB_565/HARDWARE | RGBA/RGB/Alpha | 浏览器自动管理 |
| 内存位置 | Native堆(8.0+)/Java堆 | 系统管理 | 浏览器进程堆 |
...
```java
// Android高性能图片加载优化器
public class AndroidImageOptimizer {
    
    // 1. 智能采样——根据控件大小计算最优inSampleSize
    public static int calculateInSampleSize(BitmapFactory.Options options, 
        ...
        if (height > reqHeight || width > reqWidth) {
            ...
            // 找到最大的inSampleSize值，使得图片尺寸仍大于目标尺寸
    ...
    public static Bitmap decodeSampledBitmap(String path, int reqWidth, int reqHeight) {
        ...
        // 选择合适的像素格式
        // ARGB_8888: 4字节/像素（默认，最高质量）
        // RGB_565:   2字节/像素（无透明度，适合照片）
        // HARDWARE:  GPU纹理（Android 8.0+，最省内存）
    ...
    // 3. Bitmap内存复用（减少GC压力）
    public static Bitmap decodeBitmapWithReuse(String path, Bitmap reuseBitmap) {
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inMutable = true;
        
        // 复用已有的Bitmap内存
        // 要求：reuseBitmap的字节大小 >= 新图片解码后的字节大小
    ...
    // 4. 大图分片加载（如长图、高清地图）
    public static Bitmap decodeRegion(String path, Rect region) {
        try {
    ...
    // 5. 图片内存监控
    public long calculateBitmapMemory(Bitmap bitmap) {
...
```objectivec
// iOS高性能图片加载优化器
@interface IOSImageOptimizer : NSObject
@end

@implementation IOSImageOptimizer

// 1. 降采样加载——避免将全尺寸图片加载到内存
+ (UIImage *)downsampleImage:(NSURL *)imageURL 
                  toPointSize:(CGSize)pointSize 
                        scale:(CGFloat)scale {
    
    NSDictionary *options = @{
        (NSString *)kCGImageSourceShouldCache: @NO  // 不缓存原始数据
    };
    
    CGImageSourceRef source = CGImageSourceCreateWithURL(
        (__bridge CFURLRef)imageURL, (__bridge CFDictionaryRef)options);
    ...
    NSDictionary *downsampleOptions = @{
        ...
        (NSString *)kCGImageSourceShouldCacheImmediately: @YES,  // 立即解码
...
// 3. 图片内存计算
+ (uint64_t)calculateImageMemory:(UIImage *)image {
    CGImageRef imageRef = image.CGImage;
...
<picture>
    ...
    <img src="image.jpg" alt="格式降级" loading="lazy">
</picture>
```
...
class WebImageOptimizer {
    ...
    // 4. 图片解码性能优化
    async optimizedImageLoad(url) {
        const img = new Image();
        img.src = url;
        
        // 使用decode()在后台线程解码，避免阻塞主线程
        try {
            await img.decode();
            // 解码完成后再添加到DOM，避免解码引起的布局抖动
        ...
        const observer = new PerformanceObserver((list) => {
            for (const entry of list.getEntries()) {
                if (entry.initiatorType === 'img') {
                    const metrics = {
                        url: entry.name,
                        transferSize: entry.transferSize,  // 传输大小(字节)
                        decodedSize: entry.decodedBodySize, // 解码后大小
                        duration: entry.duration,          // 总加载时间(ms)
                        fromCache: entry.transferSize === 0
                    };
                    
                    // 图片加载超过3秒告警
                    if (metrics.duration > 3000) {
                        this.reportSlowImage(metrics);
                    }
                    
                    // 图片过大告警（超过500KB）
                    if (metrics.transferSize > 500 * 1024) {
                        this.reportLargeImage(metrics);
                    }
                }
            }
        });
...
**答疑**：图片优化涉及格式、尺寸、压缩、缓存、解码等多个环节，容易陷入片面优化。

**论证**：

**误区一：质量压缩可以减少内存占用**

质量压缩（JPEG quality参数）只影响文件大小（磁盘/网络），不影响内存占用。一张1000x1000的JPEG图片，无论quality是100还是10，加载到内存后都是 1000 × 1000 × 4 = 4MB（ARGB_8888）。要减少内存占用，必须降低分辨率（尺寸压缩）。

**误区二：所有图片都应该用WebP**

WebP的解码速度比JPEG慢约20-30%。对于需要快速滚动展示大量图片的场景（如图片瀑布流），JPEG可能是更好的选择。应该根据场景权衡压缩率和解码性能。

**误区三：加大内存缓存就能提升性能**

内存缓存过大会挤占应用其他模块的内存空间，增加被系统杀死的风险。通常建议内存缓存大小为可用内存的1/8到1/4。

**结果**：图片优化需要从文件大小（网络传输）和内存占用（运行时）两个维度分别考虑。分辨率匹配、格式选择、缓存策略、解码优化缺一不可。
