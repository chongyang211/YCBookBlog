...
- 非静态内部类创建静态实例造成内存泄漏，有的时候我们可能会在启动频繁的Activity中，为了避免重复创建相同的数据资源，可能会出现这种写法
- 问题代码
    ```
...
- 内存溢出（Out Of Memory，简称OOM）
  - 指程序在申请内存时，没有足够的内存空间供其使用。通俗来说，就是"要的太多，给的不够"。
  - 在Java/Android中，当JVM的堆内存不足以分配新的对象时，会抛出`java.lang.OutOfMemoryError`。
- 内存溢出和内存泄漏的关系
  - 内存泄漏积累到一定程度就会导致内存溢出，但内存溢出不一定都是由内存泄漏引起的。
  - 例如，一次性加载一张 8000x8000 像素的图片到内存，即使没有内存泄漏，也可能直接OOM。

### 3.5 内存溢出后果

- 应用直接崩溃
  - OOM是致命错误，一旦发生，应用进程会被系统终止，用户看到的就是"应用停止运行"。
- 数据丢失
  - 用户正在编辑的内容、未保存的状态都可能丢失，严重影响用户体验和数据完整性。
- 连锁反应
  - 在多进程架构下，一个进程OOM可能导致其他进程通过Binder通信超时，进而引发更大范围的异常。
...
- 常见的内存溢出场景
  ...
  - 无限递归或深度递归：方法调用栈溢出（StackOverflowError，严格来说是栈溢出而非堆溢出）。
  - WebView长期使用：WebView会占用大量内存且难以完全释放，特别是加载复杂网页时。
  - 大量字符串拼接：在循环中使用"+"进行字符串拼接，每次都产生新的String对象。
- 跨平台对比
  - iOS中没有Java堆的概念，但也存在内存溢出问题，系统通过`didReceiveMemoryWarning`通知应用清理内存。
  - Web端通过浏览器的垃圾回收机制管理内存，但JavaScript闭包、DOM节点泄漏等也会导致内存持续增长。
...
内存衡量标准是什么？App内存可以用工具查看总的消耗内存，比如java内存，native内存。但衡量app内存是否健康的标准是什么，要建立一套衡量准则……

内存怎么判断泄漏？因为内存泄漏是在堆内存中，所以对我们来说并不是可见的。通常我们可以借助MAT、LeakCanary等工具来检测应用程序是否存在内存泄漏。
...
- 典型的Activity泄漏案例
  - 在Activity中创建了一个内部类Handler，Handler持有Activity的引用。当Activity销毁时，如果MessageQueue中还有未处理的消息，Handler无法被回收，从而导致Activity也无法被回收。
  - 另一个常见案例是单例持有Activity的Context。单例的生命周期等于应用的生命周期，如果单例持有Activity的引用，Activity就永远不会被释放。
- 集合类泄漏案例
  - 使用static修饰的List或Map，不断往里面添加对象但从不清除，这些对象就会一直留在内存中。

### 5.2 内存为何会泄漏

- 根本原因：对象的生命周期不匹配
  - 长生命周期的对象持有了短生命周期对象的引用。GC判断一个对象是否可回收的标准是"是否存在从GC Root到该对象的引用链"。
- GC Root有哪些
  - 虚拟机栈中的局部变量引用的对象、方法区中静态变量引用的对象、方法区中常量引用的对象、Native方法栈中JNI引用的对象。
  - 只要存在从任意GC Root到目标对象的引用路径，该对象就不会被回收。

### 5.3 内存捕获核心思想

- 内存泄漏检测的核心思想
  - 在对象应该被销毁的时候（如Activity.onDestroy），将该对象用弱引用包装，并关联到一个ReferenceQueue中。
  - 触发一次GC，然后检查ReferenceQueue，如果弱引用没有出现在队列中，说明该对象仍然存活——即发生了泄漏。
- 这就是LeakCanary的核心原理
  - LeakCanary通过Application.registerActivityLifecycleCallbacks监听Activity的onDestroy，在销毁时将Activity实例用WeakReference包装并注册到ReferenceQueue。延迟5秒后触发GC，再检查引用是否被回收。

### 5.4 内存泄漏引用链

- 引用链的作用
  - 仅知道某个对象泄漏了是不够的，开发者还需要知道"谁持有了它"，即引用链。
  - 通过dump堆内存（hprof文件），分析从GC Root到泄漏对象的最短引用路径，就能定位到具体的泄漏原因。
- 引用链分析工具
  - MAT（Memory Analyzer Tool）：Eclipse出品的堆分析工具，功能强大，适合深度分析。
  - Android Studio Profiler：集成在IDE中，可以直接dump堆并查看对象引用关系。
  - LeakCanary：自动dump堆并分析引用链，以通知的形式展示给开发者。

## 06.内存如何治理优化

内存治理背景，在所有的App中，内存泄露问题一直存在，泄露或多或少而已，对于使用时间比较小的应用来说，即便存在内存泄露也没那么危险，因为出现OOM的概率较低，但是内存泄露问题对于长时间运行的App来说是致命的，如何解决内存泄露就是是我们工作的重点方向。
...
- 具体实施方向
  - 图片优化：使用合适的采样率加载Bitmap，及时回收不需要的图片资源，使用图片加载框架（如Glide、Coil）的内存缓存和磁盘缓存策略。
  - 数据结构优化：Android中可使用SparseArray替代HashMap<Integer, Object>，避免自动装箱带来的额外内存开销。
  - 布局优化：减少布局层级，避免过度绘制，使用ViewStub延迟加载不必要的视图。
  - 后台内存释放：在Application中重写onTrimMemory方法，根据不同的级别释放相应的缓存资源。
...
- 什么是对象池
  - 对象池（Object Pool）是一种设计模式，预先创建一组对象放入池中，需要使用时从池中获取，使用完毕后归还到池中，而不是每次都new一个新对象。
  - 其核心目标是减少频繁创建和销毁对象带来的GC压力和性能开销。
- Android中的对象池实践
  - `Message.obtain()`：Android的Message类就使用了对象池，通过一个链表维护回收的Message实例，下次获取Message时优先从池中复用。
  - `Pools.SimplePool`：Android SDK提供了`android.util.Pools`工具类，支持简单的对象池管理。
  - RecyclerView的ViewHolder复用机制本质上也是对象池思想的体现。
- 对象池的适用场景
  - 对象创建成本高（如需要复杂初始化的对象）。
  - 对象使用频率高且生命周期短（如自定义View的onDraw中使用的Paint、Rect对象）。
  - 短时间内需要大量相同类型的对象（如粒子效果中的粒子对象）。
- 注意事项
  - 对象池的大小需要合理控制，过大会占用过多内存，过小则起不到复用效果。
  - 归还对象前需要重置对象状态，避免脏数据问题。
...
#### 3.3.4 
- 错误使用静态变量导致引用后无法销毁，在平时开发中，有时候我们创建了一个工具类。
...
RSS (Resident Set Size): 进程在物理内存中的实际占用

VSZ (Virtual Size): 进程虚拟地址空间大小

为什么需要两个指标：

- RSS: 反映真实的物理内存消耗，用于内存泄漏检测
- VSZ: 反映进程的内存映射总量，用于虚拟内存管理分析
