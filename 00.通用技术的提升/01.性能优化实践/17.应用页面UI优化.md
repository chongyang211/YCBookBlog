...
- [02.UI绘制概念](#02ui绘制概念)
  ...
  - [2.6 VSYNC信号](#26-vsync信号)
  - [2.7 三缓冲机制](#27-三缓冲机制)
- [05.布局优化策略](#05布局优化策略)
  - [5.1 布局检测工具](#51-布局检测工具)
  - [5.2 布局卡顿优化](#52-布局卡顿优化)
- [06.UI渲染测量](#06ui渲染测量)
  - [6.1 gfxinfo](#61-gfxinfo)
...
- [11.跨平台UI渲染伪代码](#11跨平台ui渲染伪代码)
  - [11.1 Android渲染性能监控](#111-android渲染性能监控)
  - [11.2 iOS UI渲染优化](#112-ios-ui渲染优化)
  - [11.3 Web UI渲染优化](#113-web-ui渲染优化)
  - [11.4 跨平台UI优化对比](#114-跨平台ui优化对比)
...
- Project Butter（黄油计划）的核心设计思想
    ```
    Android 4.1 之前的渲染问题（无VSYNC同步）：
    
    时间轴:  ──T0────T1────T2────T3────T4──
    Display:  [帧A]  [帧A]  [帧B]  [帧B]  [帧C]
    CPU:      ──[帧B处理]──────[帧C处理]──────
    GPU:              ──[帧B渲染]──[帧C渲染]──
    问题: CPU/GPU没有对齐VSYNC信号，导致：
    1. 画面撕裂（Tearing）：上半屏显示旧帧，下半屏显示新帧
    2. 掉帧（Jank）：CPU和GPU错过VSYNC截止时间
    
    ═══════════════════════════════════════════
    
    Project Butter 的解决方案：
    
    VSYNC:    ↓        ↓        ↓        ↓
    时间轴:  ──T0────T1────T2────T3────T4──
    Display:  [帧A]  [帧B]  [帧C]  [帧D]  [帧E]
    CPU:      [帧B]  [帧C]  [帧D]  [帧E]
    GPU:       [帧B]  [帧C]  [帧D]  [帧E]
    
    关键改进：
    ① VSYNC信号触发CPU立即开始处理下一帧
    ② Triple Buffering: 当GPU还在渲染帧B时，CPU可以用第三个Buffer处理帧C
    ③ Choreographer: 统一调度input/animation/draw，保证时序正确
    ```
- Project Butter 三大技术支柱
    | 技术 | 作用 | 原理 |
    |------|------|------|
    | VSYNC同步 | 统一渲染节拍 | 硬件中断信号(HW_VSYNC)驱动软件VSYNC，CPU/GPU在同一时刻开始工作 |
    | Triple Buffering | 消除GPU阻塞CPU | 增加第三个缓冲区，CPU不必等待GPU释放Buffer即可开始下一帧 |
    | Choreographer | 统一帧调度 | 将Input、Animation、Traversal(measure/layout/draw)按序执行 |
...
### 7.3 RenderThread 与 RenderScript
