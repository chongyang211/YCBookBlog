...
- [08.跨平台FPS监控伪代码](#08跨平台fps监控伪代码)
  - [8.1 Android FPS监控](#81-android-fps监控)
  - [8.2 iOS FPS监控](#82-ios-fps监控)
  - [8.3 Web FPS监控](#83-web-fps监控)
  - [8.4 FPS监控跨平台对比](#84-fps监控跨平台对比)
...
- FPS监控在性能优化体系中的定位
    ...
    │  ┌──────┐  ┌───────┐  ┌────────┐  ┌──────┐     │
    │  │ FPS  │  │帧耗时 │  │掉帧率  │  │冻帧率│     │
    │  │均值  │  │P50/P99│  │>16ms帧 │  │>700ms│     │
    │  └──────┘  └───────┘  └────────┘  └──────┘     │
    │       ↕          ↕           ↕          ↕       │
    │  反映整体流畅  定位具体卡帧  衡量轻微卡顿  衡量严重卡顿 │
    ├─────────────────────────────────────────────────┤
    │                根因分析层                          │
    │  主线程耗时 │ 布局复杂度 │ 过度绘制 │ GC频繁     │
    └─────────────────────────────────────────────────┘
    ```
- 为什么FPS不能直接代表流畅度
    - 静止不动时FPS为0（没有绘制需求），但用户不会觉得卡。FPS=50时如果每帧均匀20ms，体感其实还不错；但FPS=55时如果有一帧耗时200ms，用户会明显感知卡顿。因此，**掉帧分布比FPS均值更有价值**
