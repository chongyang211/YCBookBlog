...
#### 目录介绍
- [01.阅读说明](#01阅读说明)
- [02.贯穿案例：某出行 App"半数崩溃看不见"事件](#02贯穿案例某出行-app半数崩溃看不见事件)
- [03.问题域定义](#03问题域定义)
  - [3.1 现象与代价](#31-现象与代价)
  - [3.2 度量准则](#32-度量准则)
...
- [05.度量与采集](#05度量与采集)
  - [5.1 三类捕获方案的本质](#51-三类捕获方案的本质)
  - [5.2 各方案的可见盲区](#52-各方案的可见盲区)
  - [5.3 跨平台采集对照表](#53-跨平台采集对照表)
  - [5.4 数据可信度评估](#54-数据可信度评估)
- [06.Android Java/Kotlin 崩溃全链路 ⭐](#06android-javakotlin-崩溃全链路)
  - [6.1 崩溃如何产生：从语言语义到 JVM 中止](#61-崩溃如何产生从语言语义到-jvm-中止)
  - [6.2 投递流程：JVM 栈展开与 handler 查找链](#62-投递流程jvm-栈展开与-handler-查找链)
  - [6.3 监听点设计：为什么必须替换并回调原 handler](#63-监听点设计为什么必须替换并回调原-handler)
  - [6.4 堆栈采集：从单线程栈到全线程 + cause 链](#64-堆栈采集从单线程栈到全线程--cause-链)
  - [6.5 符号化原理：混淆为什么能逆向](#65-符号化原理混淆为什么能逆向)
- [07.Android Native 崩溃全链路 ⭐](#07android-native-崩溃全链路)
  - [7.1 崩溃如何产生：从硬件异常到 POSIX 信号](#71-崩溃如何产生从硬件异常到-posix-信号)
  - [7.2 投递流程：内核 page fault → signal → debuggerd](#72-投递流程内核-page-fault--signal--debuggerd)
  - [7.3 监听点设计：sigaction 与 async-signal-safe 边界](#73-监听点设计sigaction-与-async-signal-safe-边界)
  - [7.4 堆栈采集：unwind 算法与跨语言栈的难点](#74-堆栈采集unwind-算法与跨语言栈的难点)
  - [7.5 符号化原理：从 PC 地址到源代码行](#75-符号化原理从-pc-地址到源代码行)
- [08.Android ANR 全链路 ⭐](#08android-anr-全链路)
  - [8.1 ANR 如何产生：消息循环与超时哲学](#81-anr-如何产生消息循环与超时哲学)
  - [8.2 投递流程：system_server 的 SIGQUIT 与 ART dump](#82-投递流程system_server-的-sigquit-与-art-dump)
  - [8.3 监听点设计：三种监听方式的优劣选型](#83-监听点设计三种监听方式的优劣选型)
  - [8.4 堆栈采集：为什么必须抓全线程栈](#84-堆栈采集为什么必须抓全线程栈)
  - [8.5 归因思路：从 BLOCKED 状态反向追锁](#85-归因思路从-blocked-状态反向追锁)
- [09.iOS NSException 崩溃全链路 ⭐](#09ios-nsexception-崩溃全链路)
  - [9.1 OC 语义异常与 Swift trap 的本质差异](#91-oc-语义异常与-swift-trap-的本质差异)
  - [9.2 投递流程：objc_exception_throw 与 abort 的衔接](#92-投递流程objc_exception_throw-与-abort-的衔接)
  - [9.3 监听点设计：NSSetUncaughtExceptionHandler 的局限](#93-监听点设计nssetuncaughtexceptionhandler-的局限)
  - [9.4 堆栈采集：callStackSymbols 与 dSYM 的协作](#94-堆栈采集callstacksymbols-与-dsym-的协作)
- [10.iOS Mach Exception 崩溃全链路 ⭐](#10ios-mach-exception-崩溃全链路)
  - [10.1 双层异常机制：Mach 微内核 vs BSD 信号](#101-双层异常机制mach-微内核-vs-bsd-信号)
  - [10.2 投递流程：exception port 与回退到 signal](#102-投递流程exception-port-与回退到-signal)
  - [10.3 监听点设计：为什么 KSCrash 选 Mach 而非 signal](#103-监听点设计为什么-kscrash-选-mach-而非-signal)
  - [10.4 堆栈采集：thread_get_state 与 fp 链回溯](#104-堆栈采集thread_get_state-与-fp-链回溯)
- [11.iOS Watchdog 与 OOM 崩溃全链路 ⭐](#11ios-watchdog-与-oom-崩溃全链路)
  - [11.1 SIGKILL 不可捕获的根本原因](#111-sigkill-不可捕获的根本原因)
  - [11.2 Watchdog 与 Jetsam 的投递机制](#112-watchdog-与-jetsam-的投递机制)
  - [11.3 MetricKit 设计思想：让系统替你抓](#113-metrickit-设计思想让系统替你抓)
  - [11.4 老版本系统的"异常退出排除法"](#114-老版本系统的异常退出排除法)
- [12.Web 崩溃与错误全链路 ⭐](#12web-崩溃与错误全链路)
  - [12.1 Web 没有"崩溃"：四类故障的本质](#121-web-没有崩溃四类故障的本质)
  - [12.2 投递流程：V8 异常分发与事件机制](#122-投递流程v8-异常分发与事件机制)
  - [12.3 监听点设计：三个 listener 与 capture 阶段陷阱](#123-监听点设计三个-listener-与-capture-阶段陷阱)
  - [12.4 堆栈采集与符号化：Error.stack 与 sourcemap](#124-堆栈采集与符号化errorstack-与-sourcemap)
- [13.跨端流程对照与堆栈采集对比](#13跨端流程对照与堆栈采集对比)
...
- [15.求证实验 ⭐](#15求证实验)
  ...
  - [15.4 实验四：Tombstone / MetricKit 补全静默崩溃](#154-实验四tombstone--metrickit-补全静默崩溃)
  - [15.5 实验五：符号化自动化对治理时长的影响](#155-实验五符号化自动化对治理时长的影响)
  - [15.6 五大实验启示](#156-五大实验启示)
- [16.优化策略深化](#16优化策略深化)
  - [16.1 第一层：捕获覆盖（消灭盲区）](#161-第一层捕获覆盖消灭盲区)
  - [16.2 第二层：现场质量（让定位高效）](#162-第二层现场质量让定位高效)
  - [16.3 第三层：上报可靠性（让数据到家）](#163-第三层上报可靠性让数据到家)
  - [16.4 第四层：治理闭环（让 Top 5 持续下降）](#164-第四层治理闭环让-top-5-持续下降)
...
- [18.防劣化与长效治理](#18防劣化与长效治理)
  ...
  - [18.3 CI 卡口与线上 SLO](#183-ci-卡口与线上-slo)
...
### 7.2 投递流程：内核 page fault → signal → debuggerd
...
### 7.3 监听点设计：sigaction 与 async-signal-safe 边界
...
### 8.2 投递流程：system_server 的 SIGQUIT 与 ART dump
...
> 崩溃是"某行代码错了"——栈顶就是凶手。ANR 是"系统整体响应慢了"——可能是 10 个原因的组合：主线程任务多 + 后台线程抢 CPU + 系统资源紧张 + Binder 远端慢。所以 ANR 治理本质是**性能治理**，不是简单的 bug 修复。详细治理思路见 [`卷三·04 ANR 监控与治理`](../03.流水线专项/04.ANR监控与治理.md)。

---

## 09.iOS NSException 崩溃全链路

> 本章拆解 iOS OC 异常路径，重点回答：**OC 异常和 Swift fatalError 为什么走不同路径 / 为什么仅接 NSSetUncaughtExceptionHandler 完全不够**。

### 9.1 OC 语义异常与 Swift trap 的本质差异
...
### 9.2 投递流程：objc_exception_throw 与 abort 的衔接
...
### 9.3 监听点设计：NSSetUncaughtExceptionHandler 的局限
...
### 9.4 堆栈采集：callStackSymbols 与 dSYM 的协作
...
### 10.2 投递流程：exception port 与回退到 signal
...
### 10.3 监听点设计：为什么 KSCrash 选 Mach 而非 signal
...
### 10.4 堆栈采集：thread_get_state 与 fp 链回溯
...
### 12.3 监听点设计：三个 listener 与 capture 阶段陷阱
...
### 12.4 堆栈采集与符号化：Error.stack 与 sourcemap
...
- **"看不见的崩溃"路径**：Android LMK/ANR 系统历史 + iOS Watchdog/Jetsam MetricKit 是补全 [§02 案例](#02贯穿案例某出行-app半数崩溃看不见事件) 中"60% 漏报"的唯一答案。
...
### 15.4 实验四：Tombstone / MetricKit 补全静默崩溃

**Step 1 — 原始观察**：[§02 案例](#02贯穿案例某出行-app半数崩溃看不见事件) 中应用 SDK 捕获不到 LMK 杀、jetsam 杀、启动 hang。**系统级 API 能补全多少？**
...
**核心命题**：[§02 案例](#02贯穿案例某出行-app半数崩溃看不见事件) 证明单一 SDK 漏 60% 崩溃。本层目标：**三方案组合 + 系统 API 补全，覆盖率 99%**。

**策略 1.1：Android 三件套（Java + Native + 系统历史）**

- **机理**：[§15.1](#151-实验一捕获完整性) + [§15.4](#154-实验四tombstone--metrickit-补全静默崩溃) 双重证据。
...
- **收益**：[§02 案例](#02贯穿案例某出行-app半数崩溃看不见事件) 可见崩溃率 0.04% → 0.48%（暴露真相）。
...
- **机理**：[§02 案例](#02贯穿案例某出行-app半数崩溃看不见事件) 启动早期崩溃漏 70%。
...
- **机理**：[§15.5 实验](#155-实验五符号化自动化对治理时长的影响) 修复时长 6-10×。
...
- **同步网络上报**：成功率反而低。
- **未上传符号就发版**：上线后崩溃无法定位。
- **只接 Java/OC 异常**：[§02 案例](#02贯穿案例某出行-app半数崩溃看不见事件) 漏 60% 的根因。
...
1. **三方案组合 + 系统 API 补全**：异常 + 信号 + 历史，从 60% 提到 99%（[§15.1](#151-实验一捕获完整性) + [§15.4](#154-实验四tombstone--metrickit-补全静默崩溃)）。
2. **现场必须完整**：[§15.3](#153-实验三现场抓取价值) 让定位 -75%。
3. **本地优先 + 启动重试**：上报率 60% → 95%+。
4. **自动符号化**：[§15.5](#155-实验五符号化自动化对治理时长的影响) 修复时长 6-10×。
5. **Top 5 持续治理**：80/20 法则不是口号是制度。

### 20.2 五个常见误区

1. **"接了 Crashlytics 就够了"**：错（[§02 案例](#02贯穿案例某出行-app半数崩溃看不见事件) 漏 60% 崩溃）。
2. **"崩溃时同步上报"**：错（成功率低）。
3. **"只采 stack 够用"**：错（定位效率差 4×）。
4. **"忽略系统 API"**：错（[§15.4](#154-实验四tombstone--metrickit-补全静默崩溃) MetricKit/getHistoricalProcessExitReasons 补全 60-98% 静默崩溃）。
5. **"出问题再补符号"**：错（[§15.5](#155-实验五符号化自动化对治理时长的影响) 修复时长 6-10×；必须 CI 自动化）。
