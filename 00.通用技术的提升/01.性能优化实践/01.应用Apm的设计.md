...
| 维度 | 传统日志 | APM |
|------|---------|-----|
| 数据结构 | 非结构化文本 | 结构化指标 + Trace |
| 分析方式 | 人工搜索 | 自动聚合 + 告警 |
| 覆盖范围 | 仅后端 | 全端（客户端 + 后端 + 网络） |
| 实时性 | 事后分析 | 实时/准实时 |
| 关联能力 | 单一维度 | 多维关联（用户/设备/版本/地域） |
...
```
T0: 进程创建 → T1: Application/AppDelegate初始化 → T2: 首页创建
→ T3: 首帧渲染完成 → T4: 数据加载完成（用户可交互）
