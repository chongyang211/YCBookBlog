...
**Android**：Hook `Activity.onCreate/onStart/onResume/onWindowFocusChanged`，使用 `ViewTreeObserver.OnPreDrawListener` 检测绘制完成。

**iOS**：Swizzle `UIViewController` 的 `viewDidLoad/viewWillAppear/viewDidAppear`，页面耗时 = viewDidAppear - viewDidLoad。

**Web SPA**：Hook `pushState/replaceState/popstate`，使用 `MutationObserver` 检测 DOM 更新完成。
...
| 平台 | 获取方式 |
|------|---------|
| Android | 读取 `/proc/[pid]/stat` 的 utime + stime |
| iOS | `thread_info` 遍历所有线程累加 `cpu_usage` |
| Web | Long Tasks API 间接反映（无直接 CPU API） |
...
| 平台 | 监控方式 | 建议阈值 |
|------|---------|---------|
| Android | 遍历 View 树计算深度 | 层级 > 10 告警 |
| iOS | 递归遍历 `view.subviews` | 层级 > 10 告警 |
| Web | 递归计算 DOM 深度 | 节点 > 1500 告警 |
...
**Android — OkHttp Interceptor**：通过标准拦截器机制，在 `chain.proceed()` 前后记录耗时。

**iOS — NSURLProtocol + NSURLSessionTaskMetrics**：自定义 `NSURLProtocol` 拦截请求，通过 `didFinishCollecting:metrics` 获取 DNS/TCP/TLS/TTFB 各阶段耗时。

**Web — fetch/XHR Monkey Patch + Resource Timing API**：重写 `window.fetch` 和 `XMLHttpRequest.prototype`，结合 `PerformanceObserver` 监听 `resource` 条目获取详细阶段耗时。
