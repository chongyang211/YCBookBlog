# 网络分析和优化
#### 目录介绍
- [01.整体概述介绍](#01整体概述介绍)
  - [1.1 项目背景介绍](#11-项目背景介绍)
  - [1.2 遇到问题](#12-遇到问题)
  - [1.3 网络优化概念](#13-网络优化概念)
  - [1.4 网络优化维度](#14-网络优化维度)
  - [1.5 监控网络思路](#15-监控网络思路)
- [02.网络基础知识](#02网络基础知识)
  - [2.1 curl分析网络](#21-curl分析网络)
  - [2.2 弱网模拟思路](#22-弱网模拟思路)
  - [2.3 抓包拦截思路](#23-抓包拦截思路)
  - [2.4 流量统计分析](#24-流量统计分析)
  - [2.5 耗时统计思路](#25-耗时统计思路)
  - [2.6 Glide耗时思路](#26-glide耗时思路)
  - [2.7 网络速度测试](#27-网络速度测试)
- [03.网络优化策略](#03网络优化策略)
  - [3.1 HTTPDNS优化](#31-httpdns优化)
  - [3.2 连接复用](#32-连接复用)
  - [3.3 传输数据压缩](#33-传输数据压缩)
  - [3.7 数据缓存](#37-数据缓存)
- [04.一些常见优化](#04一些常见优化)
  - [4.1 网络数据加密](#41-网络数据加密)
  - [4.2 防止域名劫持](#42-防止域名劫持)
  - [4.4 数据劫持优化](#44-数据劫持优化)
  - [4.6 图片网络优化](#46-图片网络优化)
  - [4.9 网络数据优化](#49-网络数据优化)
- [05.如何衡量优化成果](#05如何衡量优化成果)
  - [5.1 流量消耗分析](#51-流量消耗分析)
  - [5.2 异常率统计](#52-异常率统计)
  - [5.3 稳定性统计](#53-稳定性统计)
- [06.网络优化设计总结](#06网络优化设计总结)
  - [6.1 网络优化核心原则](#61-网络优化核心原则)
  - [6.2 跨平台网络优化对比](#62-跨平台网络优化对比)
  - [6.3 网络优化方法论](#63-网络优化方法论)
  - [6.4 网络优化常见误区](#64-网络优化常见误区)
- [07.跨平台网络监控伪代码](#07跨平台网络监控伪代码)
  - [7.1 Android网络监控](#71-android网络监控)
  - [7.2 iOS网络监控](#72-ios网络监控)
  - [7.3 Web网络监控](#73-web网络监控)
  - [7.4 网络优化技术演进](#74-网络优化技术演进)
- [08.网络监控设计总结](#08网络监控设计总结)
  - [8.1 跨平台网络监控对比](#81-跨平台网络监控对比)
  - [8.2 网络优化核心策略](#82-网络优化核心策略)






## 01.整体概述介绍
### 1.1 项目背景介绍

网络性能直接影响用户体验。在移动端，网络环境复杂多变——WiFi、4G、5G、弱网、无网等场景频繁切换，这对网络优化提出了极高的要求。

一个典型的网络请求包含以下阶段，每个阶段都可能成为性能瓶颈：

```
HTTP请求的完整生命周期（各阶段耗时对比）：

4G网络(正常)：
DNS解析 → TCP连接 → TLS握手 → 发送请求 → 等待响应 → 接收数据
  ~50ms    ~50ms    ~100ms    ~10ms     ~200ms     ~100ms   = ~510ms

WiFi网络(优质)：
  ~5ms     ~10ms    ~30ms     ~5ms      ~100ms     ~50ms    = ~200ms

弱网(2G/信号差)：
  ~500ms   ~1000ms  ~2000ms   ~100ms    ~3000ms    ~2000ms  = ~8600ms!

首次请求 vs 复用连接：
首次:  DNS + TCP + TLS + 请求 = 全流程
复用:  直接发送请求 = 省去 DNS/TCP/TLS 约200ms
```

**网络优化的核心价值量化**：

| 优化项 | 优化前 | 优化后 | 用户感知提升 |
|-------|--------|--------|------------|
| DNS优化(HTTPDNS) | P99=500ms | P99=50ms | 首次加载快450ms |
| 连接复用(keep-alive) | 每次新建连接 | 复用已有连接 | 省去200ms建连 |
| 数据压缩(gzip) | 响应体100KB | 响应体30KB | 3G下省2秒传输 |
| HTTP/2多路复用 | 串行请求 | 并行请求 | 页面加载快50% |
| 离线缓存 | 无网=空白 | 展示缓存数据 | 弱网体验极大提升 |

网络请求的总耗时约500ms（4G网络），但在弱网环境下可能超过数秒甚至超时。网络优化的目标就是减少每个阶段的耗时，提高成功率，降低流量消耗。


### 1.2 遇到问题

在实际项目中，网络相关的问题通常有以下几类：

1. **请求耗时长**：DNS解析慢、TCP连接慢、服务器响应慢
2. **请求失败率高**：弱网环境下超时、DNS劫持、服务器异常
3. **流量消耗大**：接口数据量大、图片资源未压缩、重复请求
4. **安全问题**：数据明文传输、域名劫持、中间人攻击
5. **用户感知差**：加载等待时间长、频繁显示加载失败

**真实场景问题分析**：

```
案例：电商App商品详情页加载慢

用户反馈：打开商品详情页需要等3-5秒

问题排查流程：
1. 抓包分析 → 发现页面发起了12个串行请求
   ├── 商品信息接口   300ms
   ├── 评价列表接口   400ms  
   ├── 推荐商品接口   350ms
   ├── 优惠券接口     200ms
   ├── 商品图片×8    500ms×8(串行!)
   └── 总计: 300+400+350+200+4000 = 5250ms

优化方案：
1. 接口合并 → 商品信息+评价+推荐合为1个接口  = 400ms
2. 图片并行加载 → 8张图片并行             = 500ms
3. HTTP/2多路复用 → 所有请求复用1个连接     = 省200ms
4. 预加载 → 列表页滑动时预加载下一页数据    = 0ms(已在缓存)

优化后：400+500 = 900ms (提升83%!)
```


### 1.3 网络优化概念
- Android 网络优化方法
    - 主要讲的就是线下网络测试工具、线上网络监控方案、流量优化方案和质量优化方案。
- 网络测试工具
    - Network Profiler、Charles和Stetho。
- 线上网络监控方案
    - 有OkHttp 的 EventListener、NetworkStatsManager 和 TrafficStats。
- 流量优化方案
    - 数据缓存、数据压缩和图片压缩。
- 质量优化方案
    - HttpDns 优化、协议版本优化以及资本优化。


### 1.4 网络优化维度

**网络优化的双维度模型**

- 网络优化时要考虑的维度
    - 流量维度和质量维度，然后根据这两个维度的数据减少流量消耗以及提升请求速度。

```
网络优化双维度体系：

┌─────────────────────────────────────────────────┐
│                网络优化全景                        │
├───────────────────────┬─────────────────────────┤
│      流量维度          │       质量维度           │
│                       │                         │
│  ┌─ 数据压缩          │  ┌─ DNS优化(HTTPDNS)     │
│  ├─ 图片压缩/WebP     │  ├─ 连接复用(keep-alive)  │
│  ├─ 缓存策略          │  ├─ 协议升级(HTTP/2/3)    │
│  ├─ 增量更新          │  ├─ 请求合并             │
│  ├─ 预判断(304)       │  ├─ 预连接/预加载         │
│  └─ 请求去重          │  ├─ 重试策略             │
│                       │  └─ 弱网适配             │
├───────────────────────┼─────────────────────────┤
│  监控指标:             │  监控指标:               │
│  · 日均流量(MB/天)     │  · 请求成功率(>99.9%)    │
│  · 前后台流量占比      │  · P50/P90/P99耗时       │
│  · 接口维度流量排名    │  · DNS/TCP/TLS分段耗时    │
│  · 流量异常告警        │  · 重试率               │
└───────────────────────┴─────────────────────────┘
```

- 流量维度
    - App 在一段时间内流量消耗的精准度量，流量消耗可以区分为网络类型、前后台以及请求和响应等维度。
- 质量维度
    - 可以从请求时长、请求成功率和Top失败接口等数据上进行区分，以便后续能快速定位和解决问题。



### 1.5 监控网络思路
- 建立完善的监控体系
    - 指的是做网络优化要监控网络请求成功率和网络请求异常，比如流量消耗过多、请求次数过多以及下载文件过大等异常。




## 02.网络基础知识
### 2.1 curl分析网络
- curl如何分析接口请求说明
    - curl（全称Client URL）是一个在命令行下运行的网络工具，用于与服务器进行通信。
    - 它支持多种协议，包括HTTP、HTTPS、FTP、SMTP、TELNET等，并提供了丰富的选项和功能，使其成为一个强大的网络调试和测试工具。
- curl命令的基本语法
    - 基本语法如下：curl 【选项】 【URL】 比如，我要调试某个接口查看请求详细信息：curl -v https://www.wanandroid.com/article/list/0/json
- curl测试接口案例分析
    ```
    yangchongdeMacBook-Pro:~ yangchong$ curl -v https://www.wanandroid.com/article/list/0/json
    *   Trying 47.104.74.169:443...
    * Connected to www.wanandroid.com (47.104.74.169) port 443 (#0)
    * ALPN: offers h2,http/1.1
    * (304) (OUT), TLS handshake, Client hello (1):
    *  CAfile: /etc/ssl/cert.pem
    *  CApath: none
    * (304) (IN), TLS handshake, Server hello (2):
    * TLSv1.2 (IN), TLS handshake, Certificate (11):
    * TLSv1.2 (IN), TLS handshake, Server key exchange (12):
    * TLSv1.2 (IN), TLS handshake, Server finished (14):
    * TLSv1.2 (OUT), TLS handshake, Client key exchange (16):
    * TLSv1.2 (OUT), TLS change cipher, Change cipher spec (1):
    * TLSv1.2 (OUT), TLS handshake, Finished (20):
    * TLSv1.2 (IN), TLS change cipher, Change cipher spec (1):
    * TLSv1.2 (IN), TLS handshake, Finished (20):
    * SSL connection using TLSv1.2 / ECDHE-RSA-AES256-SHA384
    * ALPN: server did not agree on a protocol. Uses default.
    * Server certificate:
    *  subject: CN=wanandroid.com
    *  start date: Jan  2 00:00:00 2023 GMT
    *  expire date: Jan  2 23:59:59 2024 GMT
    *  subjectAltName: host "www.wanandroid.com" matched cert's "www.wanandroid.com"
    *  issuer: C=CN; O=TrustAsia Technologies, Inc.; CN=TrustAsia RSA DV TLS CA G2
    *  SSL certificate verify ok.
    * using HTTP/1.x
    > GET /article/list/0/json HTTP/1.1
    > Host: www.wanandroid.com
    > User-Agent: curl/7.88.1
    > Accept: */*
    > 
    < HTTP/1.1 200 OK
    < Server: Apache-Coyote/1.1
    < Cache-Control: private
    < Expires: Thu, 01 Jan 1970 08:00:00 CST
    < Set-Cookie: JSESSIONID=34447AAA7368A824F67DE25710B89326; Path=/; Secure; HttpOnly
    < Content-Type: application/json;charset=UTF-8
    < Transfer-Encoding: chunked
    < Date: Fri, 04 Aug 2023 02:14:56 GMT
    < 
    {"data":{"curPage":1,"datas":""} ……此处省略一堆的响应数据}
    ```
- 一些核心要点分析：
    - Connected to www.wanandroid.com (47.104.74.169) port 443 (#0) ：这段话表示解析域名后得到的ip地址和端口号port443
    - (OUT),TLS handshake, Client hello (1)：客户端开始新的握手( ClientHello)，将自身支持的加密功能（Cipher Suites）提交给服务器。
    - (IN), TLS handshake, Server hello (2)：服务器( ServerHello)，选择Cipher Suite。
    - (IN), TLS handshake, Certificate (11)：服务器发送其证书(Certificate)（当需要服务器身份验证时）。
    - (IN), TLS handshake, Server key exchange (12)：根据选择的密钥交换方式，服务器发送生成主密钥的额外信息。
    - (IN), TLS handshake, Server finished (14)：服务器通知客户端自己完成了协商过程。
    - (OUT), TLS handshake, Client key exchange (16)：客户端发送生成主密钥所需的额外信息（ClientKeyExchange）。
    - (OUT), TLS change cipher, Change cipher spec (1)：客户端切换到加密方式并通知服务器。
    - (OUT), TLS handshake, Finished (20)：客户端计算发送和接收到的握手消息的MAC并发送。
    - (IN), TLS change cipher, Change cipher spec (1)：服务器切换加密方式并通知客户端。
    - (IN), TLS handshake, Finished (20)：服务器计算发送和接收到的握手消息的MAC并发送。
    - 详细讲解一下每个步骤，这里主要是TLS协议中握手协议流程。更多可以看：[TLS协议中的握手协议](https://blog.csdn.net/zhbgreat/article/details/87867380)
- 通过curl可以快速分析网络
    ```
    //GET请求
    //要发送 GET 请求，只需要在 curl 命令中指定 -X GET 参数，后面跟随请求的 URL。
    curl -X GET 'https://www.example.com/api/v1/resource'
    curl -X GET https://www.example.com/api/v1/resource?param1=value1&param2=value2
    //下面两个作用等同
    curl -X GET -v https://www.wanandroid.com/article/list/0/json
    curl -v https://www.wanandroid.com/article/list/0/json
    
    
    //POST请求
    //发送表单数据。在许多情况下，你可能需要发送表单数据。使用-d或--data参数可以实现这一目标：
    curl -X POST -d "username=yangchong&password=123456" https://www.wanandroid.com/user/login
    curl -X POST -v "username=yangchong&password=123456" https://www.wanandroid.com/user/login
    //发送JSON数据。发送JSON数据稍微复杂一点，需要使用-H参数指定Content-Type为application/json，然后用-d参数传递JSON字符串：
    curl -X POST -H "Content-Type: application/json" -d '{"key1":"value1", "key2":"value2"}' http://example.com
    ```





### 2.2 弱网模拟思路
- 如何开发模拟网络异常工具
    - 整体设计思路是，给OkHttpClient添加拦截器，然后在拦截器中拿到request和respond数据做处理即可。
- 如何模拟网络超时
    - 网络超时分为两种：请求超时和响应超时。请求超时是指客户端发送request到服务端过程超时，响应超时是指客户端在接收服务端respond数据过程超时。
    - 模拟请求超时需要在chain.request()代码前面sleep时间，模拟响应超时需要在chain.proceed(request)代码之后sleep时间即可。
- 如何模拟重定向或服务端异常
    - 通过拦截器拿到Chain对象，先发送request，请求成功后会拿到真是respond数据。那么这个时候可以针对这个respond处理，重新设置数据和code码信息
- 如何模拟弱网环境
    - 模拟弱网环境相对复杂一点，发送request和接收respond数据都会有io流读写操作。其核心思路就是对io读写速度做出限制，比如每秒读取1k数据，那么读取10k的接口数据需要10秒
    - 如何对RequestBody数据限制？请求数据write的时候，第一限制字节流传输(每次写设置byteCount字节长度)，第二在一秒内如果超出限制就sleep一下
    - 如何对ResponseBody数据限制？
- 弱网络模拟工具可以看
    - MonitorInterceptor



### 2.3 抓包拦截思路
- Stetho库，Facebook 开源的一个 Android 调试工具，可以测试网络，用语言来描述应该是这样子：
    - 1、安装了stetho插件的app启动之后，会启动一个本地server1（LocalSocketServer），这个本地server1等待着app(client)的连接。
    - 2、同时，这个本地server1会与另外一个本地server2（ChromeDevtoolsServer）连接着。
    - 3、本地app一旦连接上，数据将会不停的被发送到本地server1，然后转由server2.
    - 4、然后Chrome Developer Tools，想访问网站一样的，访问了ChromeDevtoolsServer，随之将数据友好的展示给了开发者，这么一个过程就此完结。
- 那么既然网络请求添加StethoInterceptor，既可以拦截网络请求和响应信息，发送给Chrome。那么能不能自己拿来用……
    - StethoInterceptor大概流程，可以简化为：发送请求时，给Chrome发了条消息，收到请求时，再给Chrome发条消息（具体怎么发的可以看NetworkEventReporterImpl的实现）
    - 两条消息通过EventID联系起来，它们的类型分别是OkHttpInspectorRequest 和 OkHttpInspectorResponse，两者分别继承自NetworkEventReporter.InspectorRequest和NetworkEventReporter.InspectorResponse。
    - 我们只要也继承自这两个类，在自己的网络库发送和收到请求时，构造一个Request和Response并发送给Chrome即可。
- 如何拿来用
    - 既然Android中使用到facebook的stetho库，可以拦截手机请求请求，然后去Chrome浏览器，在浏览器地址栏输入：chrome://inspect 。即可查看请求信息。
    - 那么能不能把这个拿到的请求信息(请求信息和相应信息)，放到集合中，然后在Android的页面中展示呢？这样方便开发和测试查看网络请求信息，以及请求流程中的消耗时间（比如dns解析时间，请求时间，响应时间，共耗时等等）
- 如何统计网络请求和相应信息，大概流程如下所示
    - 第一步，找到入口，给OkHttpClient添加拦截器，该自定义拦截器继承Interceptor然后重写intercept拦截方法。
    - 第二步，构建一个独特的eventID，一对网络事件（请求和回包）对应一个eventID(使用AtomicInteger保证异步下数据独一)。这个是在创建NetworkEventReporter对象重写nextRequestId方法获取。
    - 第三步，创建一个记录请求的bean，通过eventID创建对象，然后将请求链接，链接时间，读写时间等都设置进来。这个用来记录基本信息
    - 第四步，在准备发送请求时，构造一个OkHttpInspectorRequest，报告给Chrome，此时Network会显示一条请求，处于Pending状态
    - 第五步，发送请求，获取数据返回包信息，在构造一个OkHttpInspectorResponse，发送给Chrome用于展示，
- 如何统计各个请求阶段的时间差，大概流程如下所示
    - 第一步，找到入口，给OkHttpClient添加eventListenerFactory事件监听，这个时候创建一个继承EventListener的对象；
    - 第二步，在最开始调用方法中，获取eventID（请求和回包网络事件id），然后通过这个id拿到之前创建的bean(存在Map中，key就是构建的id)。
    - 第三步，在各个方法中打上时间戳，举个例子，在dnsStart记录时间戳，在dnsEnd记录时间戳，时间差就是两个的差值。
    - 第四步，在执行监听最后的方法callEnd中，做出最后的计算，主要是记录每个阶段的差值，存到对应的bean中(注意一个完整的请求和回包流程id是唯一的)。
- 抓包拦截工具可以看
    - MonitorNetLib



### 2.4 流量统计分析
- 流量监测的实现相对简单
    - 利用系统提供的 TrafficStats.getUidRxBytes方法，配合Activity生命周期，即可获取每个Activity的流量消耗。
    - 具体做法：在Activity start的时候记录起点，在pause的时候累加，最后在Destroyed的时候统计整个Activity的流量消耗，如果想要做到Fragment维度，就要具体业务具体分析了
- TrafficStats 缺点
    - 只能统计到手机上次重启后到现在的流量消耗。
- 流量统计工具可以看
    - MonitorFlow




### 2.5 耗时统计思路
- OkHttp如何进行各个请求环节的耗时统计呢？
    - OkHttp 版本提供了EventListener接口，可以让调用者接收一系列网络请求过程中的事件，例如DNS解析、TSL/SSL连接、Response接收等。
    - 通过继承此接口，调用者可以监视整个应用中网络请求次数、流量大小、耗时(比如dns解析时间，请求时间，响应时间等等)情况。
- 如何消耗记录时间
    - 在OkHttp库中有一个EventListener类。该类是网络事件的侦听器。扩展这个类以监视应用程序的HTTP调用的数量、大小和持续时间。
    - 所有启动/连接/获取事件最终将接收到匹配的结束/释放事件，要么成功(非空参数)，要么失败(非空可抛出)。
    - 比如，可以在开始链接记录时间；dns开始，结束等方法解析记录时间，可以计算dns的解析时间。
    - 比如，可以在开始请求记录时间，记录connectStart，connectEnd等方法时间，则可以计算出connect连接时间。
- 遇到的问题记录说明
    - 客户端请求从发起到网关实际接收到，中间有很复杂的链路，发起请求，dns解析，开始链接，发送请求头，发送请求body，返回数据等很多个阶段。
- 基于OkHttp的网络监控
    - 是不是可以考虑把整个api发起到结束进行监控，从而可以方便线上去监控一个Api真实的发起到结束的状况呢？
- 基于OKHttp提供的EventListener
    - 可以对于一个请求发起到最后的各个节点进行监控，之后上报日志数据，针对网络请求每个阶段的耗时情况都可以用数据说明。
- 网络请求耗时统计已经有了现成的库
    - 针对网络请求每个阶段统计了消耗时间，[网络请求耗时统计库](https://github.com/yangchong211/YCAndroidTool/tree/master/MonitorNetLib)；
    - 关于EventListener的原理解读可以看我另一篇博客：[OkHttp请求耗时统计和实践](https://juejin.cn/post/6875626117265358855)


### 2.6 Glide耗时思路
- EventListener 还能和 Glide 搭配使用，以获取图片请求的各个步骤的数据。



### 2.7 网络速度测试

网络速度测试可以帮助判断当前网络环境质量，从而实施不同的加载策略（如弱网下加载缩略图）。

**测速原理**：下载一个已知大小的数据包，测量下载时间，计算带宽。

```java
// 简单的网络测速实现
public class NetworkSpeedTester {
    private static final String SPEED_TEST_URL = "https://example.com/speed-test-file";
    private static final int TEST_SIZE = 100 * 1024;  // 100KB测试文件
    
    public interface SpeedCallback {
        void onResult(float speedKBps, NetworkQuality quality);
    }
    
    public static void testSpeed(SpeedCallback callback) {
        new Thread(() -> {
            try {
                long start = SystemClock.elapsedRealtime();
                URL url = new URL(SPEED_TEST_URL);
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setConnectTimeout(5000);
                conn.setReadTimeout(10000);
                
                InputStream is = conn.getInputStream();
                byte[] buffer = new byte[4096];
                int totalRead = 0;
                while (totalRead < TEST_SIZE) {
                    int read = is.read(buffer);
                    if (read == -1) break;
                    totalRead += read;
                }
                is.close();
                
                long duration = SystemClock.elapsedRealtime() - start;
                float speedKBps = totalRead / 1024f / (duration / 1000f);
                
                NetworkQuality quality;
                if (speedKBps > 500) quality = NetworkQuality.EXCELLENT;
                else if (speedKBps > 150) quality = NetworkQuality.GOOD;
                else if (speedKBps > 50) quality = NetworkQuality.MODERATE;
                else quality = NetworkQuality.POOR;
                
                callback.onResult(speedKBps, quality);
            } catch (Exception e) {
                callback.onResult(0, NetworkQuality.POOR);
            }
        }).start();
    }
    
    public enum NetworkQuality {
        EXCELLENT,  // >500KB/s
        GOOD,       // 150-500KB/s
        MODERATE,   // 50-150KB/s
        POOR        // <50KB/s
    }
}



## 03.网络优化策略
### 3.1 HTTPDNS优化
- DNS 的解析是我们网络请求的第一项工作，默认我们使用运营商的 LocalDNS 服务。这块耗时在 3G 网络下可能是 200～300ms，4G 网络也需要 100ms。解析慢并不是默认 LocalDNS 最大的“原罪”，它还存在一些其他问题：
    - 稳定性。UDP 协议，无状态，容易域名劫持（难复现、难定位、难解决），每天至少几百万个域名被劫持，一年至少十次大规模事件。
    - 准确性。LocalDNS 调度经常出现不准确，比如北京的用户调度到广东 IP，移动的运营商调度到电信的 IP，跨运营商调度会导致访问慢，甚至访问不了。
    - 及时性。运营商可能会修改 DNS 的 TTL，导致 DNS 修改生效延迟。不同运营商的服务实现不一致，我们也很难保证 DNS 解析的耗时。
- 为了解决这些问题，就有了 HTTPDNS。
    - 简单来说自己做域名解析的工作，通过 HTTP 请求后台去拿到域名对应的 IP 地址，直接解决上述所有问题。具体可以参考：[百度DNS优化](https://mp.weixin.qq.com/s/iaPtSF-twWz-AN66UJUBDg)



### 3.2 连接复用
- 在 DNS 解析之后，我们来到了创建连接这个环节。创建连接要经过 TCP 三次握手、TLS 密钥协商，连接建立的代价是非常大的。这里我们主要的优化思路是复用连接，这样不用每次请求都重新建立连接。
- 连接管理，网络库并不会立刻把连接释放，而是放到连接池中。这时如果有另一个请求的域名和端口是一样的，就直接拿出连接池中的连接进行发送和接收数据，少了建立连接的耗时。
- 这里我们利用 HTTP 协议里的 keep-alive，而 HTTP/2.0 的多路复用则可以进一步的提升连接复用率。它复用的这条连接支持同时处理多条请求，所有请求都可以并发在这条连接上进行。
- 虽然 H2 十分强大，不过这里还有两个问题需要解决。一个是同一条 H2 连接只支持同一个域名，一个是后端支持 HTTP/2.0 需要额外的改造。这个时候我们只需要在统一接入层做改造，接入层将数据转换到 HTTP/1.1 再转发到对应域名的服务器。
- 这样所有的服务都不用做任何改造就可以享受 HTTP/2.0 的所有优化，不过这里需要注意的是 H2 的多路复用在本质上依然是同一条 TCP 连接，如果所有的域名的请求都集中在某一条连接中，在网络拥塞的时候容易出现 TCP 队首阻塞问题。
- 对于客户端网络库来说，无论 OkHttp 还是 Chromium 网络库对于 HTTP/2.0 的连接，同一个域名只会保留一条连接。对于一些第三方请求，特别是文件下载以及视频播放这些场景可能会遇到对方服务器单连接限速的问题。在这种情况下我们可以通过修改网络库实现，也可以简单的通过禁用 HTTP/2.0 协议解决。



### 3.3 传输数据压缩
- 传输数据压缩
    - 具体有 开启 Gzip 压缩、压缩请求头以及合并请求三个方案。
    - 开启 Gzip 压缩就是把内容编码 Content-Encoding 首部设为 gzip，网络框架用的是 OkHttp ，那么在没有设置 Content-Encoding 请求头的情况下，OkHttp 默认会设置为 gzip。
    - 压缩请求头指的是在请求头不变的情况下，让服务端缓存请求头，在需要请求头的某些信息时，根据之前的请求头的 MD5 从之前的缓存中取。
    - 合并请求指的是把可以合并的请求合并起来，因为每一个网络请求头会有冗余信息。
- 第一时间想到的还是减少传输的数据量，也就是我们常说的数据压缩。首先对于 HTTP 请求来说，数据主要包括三个部分：
    - 请求 URL
    - 请求 header
    - 请求 body
- 具体对数据量压缩如何理解
    - 对于 header 来说，如果使用 HTTP/2.0 连接本身的头部压缩技术，因此需要压缩的主要是请求 URL 和请求 body。
    - 对于请求 URL 来说，一般会带很多的公共参数，这些参数大部分都是不变的。这样不变的参数客户端只需要上传一次即可，其他请求我们可以在接入层中进行参数扩展。
    - 对于请求 body 来说，一方面是数据通信协议的选择，在网络传输中目前最流行的两种数据序列化方式是 JSON 和 Protocol Buffers。正如我之前所说的一样，Protocol Buffers 使用起来更加复杂一些，但在数据压缩率、序列化与反序列化速度上面都有很大的优势。



### 3.7 数据缓存
- 客户端数据缓存
    - 可以分为 OkHttp 的 Cache 和 无网拦截器两种方案，我们项目中一般会有一些对实时性要求不高的接口，比如省市区列表，这时就可以考虑缓存这些接口的数据。
    - OkHttp 默认是不会缓存响应数据的。Cache 内部使用了 DiskLruCache ，创建 Cache 只需要指定缓存目录和缓存大小，然后设置给 OkHttpClient 就可以了。使用 Cache 要注意的是，默认缓存拦截器只会缓存 GET 和 HEAD 等获取资源的请求方法的响应。



## 04.一些常见优化
### 4.1 网络数据加密

网络数据加密是保障数据安全的基础：

**传输层加密（TLS/SSL）**：
- 所有网络请求必须使用HTTPS，禁止使用HTTP明文传输
- 使用TLS 1.2及以上版本，禁用不安全的加密套件
- 配置证书固定（Certificate Pinning）防止中间人攻击

```java
// OkHttp配置证书固定
CertificatePinner pinner = new CertificatePinner.Builder()
    .add("api.example.com", "sha256/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=")
    .build();

OkHttpClient client = new OkHttpClient.Builder()
    .certificatePinner(pinner)
    .build();
```

**应用层加密**：
- 敏感数据（密码、token等）在传输前进行额外加密
- 使用非对称加密传输对称密钥，对称加密传输业务数据
- 对请求参数进行签名，防止篡改



### 4.2 防止域名劫持
- 什么叫做域名劫持
    - 指在劫持的网络范围内拦截域名解析的请求，分析请求的域名，返回假的IP地址或者什么都不做使请求失去响应，其效果就是对特定的网络不能访问或访问的是假网址。
- 先说下域名劫持的现象
    - 广告劫持：用户正常页面指向到广告页面。这个背景估计很多人遇到过……
    - 恶意劫持：域名指向IP被改变，将用户访问流量引到挂马，盗号等对用户有害页面的劫持。
    - local DNS缓存：为了降低跨网流量及用户访问速度进行的一种劫持，导致域名解析结果不能按时更新。
- 域名劫持原因分析
    - 缓存是DNS被劫持的根本原因，在DNS解析过程的各个缓存中均有可能被劫持。主要包括本机的hosts篡改劫持，和运营商的Local DNS劫持等。
- 解决办法该怎么做
    - 方案1：HttpDNS使用HTTP协议向DNS服务器的80端口进行请求，代替传统DNS协议向DNS服务器的53端口进行请求。也就是使用Http协议去进行DNS解析请求，将服务器返回的解析结果，直接向该IP发起对应的API服务请求，代替使用域名，域名解析请求直接发送到HTTPDNS服务器，绕过运营商Local DNS，避免域名劫持问题。
    - 方案2：如果知道该域名的真实IP地址，则可以直接用此IP代替域名后进行访问。比如访问谷歌网站，可以把访问改为https://216.239.XX.XX/ ，从而绕开域名劫持。


### 4.4 数据劫持优化
- 先说下数据劫持的现象
    - 数据劫持针对明文传输的内容发生。用户发起HTTP请求，服务器返回页面内容时，经过中间的运营商网络，页面内容被篡改或加塞内容，强行插入弹窗或者广告。
- 劫持数据的大概原理
    - 数据劫持的核心原理是中间人攻击（MITM）。当数据以HTTP明文传输时，经过的任何网络节点（路由器、运营商设备等）都可以读取和修改数据内容。常见的劫持方式：
    - 1. **内容注入**：在HTML页面中注入JavaScript代码或iframe广告
    - 2. **302重定向**：将原始请求重定向到广告页面
    - 3. **DNS劫持+内容替换**：先劫持DNS解析，再返回修改后的内容
    - 4. **TCP劫持**：在TCP数据流中直接注入数据
- 如何预防数据被劫持
    - 解决方案一：对内容进行HTTPS加密，实现密文传输，彻底避免劫持问题。
    - 解决方案二：对于不方便升级HTTPS的接口，可以在应用层对响应数据做完整性校验（如MD5/SHA签名验证），发现数据被篡改时丢弃并重试。
    - 解决方案三：使用HTTPDNS绕过运营商LocalDNS，避免DNS劫持。
    - 解决方案四：使用QUIC协议（基于UDP），由于运营商劫持设备通常只针对TCP流量，QUIC可以有效避免部分劫持场景。

**数据完整性校验示例**：

```java
// 响应数据完整性校验
public class IntegrityCheckInterceptor implements Interceptor {
    @Override
    public Response intercept(Chain chain) throws IOException {
        Request request = chain.request();
        Response response = chain.proceed(request);
        
        // 检查响应头中的签名
        String serverSign = response.header("X-Data-Sign");
        if (serverSign != null) {
            ResponseBody body = response.body();
            byte[] bodyBytes = body.bytes();
            
            // 计算body的MD5
            String clientSign = md5(bodyBytes);
            
            if (!serverSign.equals(clientSign)) {
                // 数据被篡改，丢弃并重试
                throw new IOException("Data integrity check failed, possible hijacking");
            }
            
            // 重新构建Response
            return response.newBuilder()
                .body(ResponseBody.create(body.contentType(), bodyBytes))
                .build();
        }
        
        return response;
    }
}
```



### 4.6 图片网络优化
- 比如我之前看到豆瓣接口，提供一种加载图片方式特别好。
    - 接口返回图片的数据有三种，一种是高清大图，一种是正常图片，一种是缩略小图。
    - 当用户处于wifi下给控件设置高清大图，当4g或者3g模式下加载正常图片，当弱网条件下加载缩略图【也称与加载图】。
    - 简单来说根据用户的当前的网络质量来判断下载什么质量的图片（电商用的比较多）。豆瓣开源接口可以参考一下！




### 4.7 弱网模拟优化


### 4.9 网络数据优化
- 连接复用：节省连接建立时间，如开启 keep-alive。
    - 对于Android来说默认情况下HttpURLConnection和HttpClient都开启了keep-alive。只是2.2之前HttpURLConnection存在影响连接池的Bug，具体可见：Android HttpURLConnection及HttpClient选择
- 请求合并：即将多个请求合并为一个进行请求，比较常见的就是网页中的CSS Image Sprites。如果某个页面内请求过多，也可以考虑做一定的请求合并。
- 减少请求数据的大小：对于post请求，body可以做gzip压缩的，header也可以做数据压缩(不过只支持http)
    - 返回数据的body也可以做gzip压缩，body数据体积可以缩小到原来的30%左右。（也可以考虑压缩返回的json数据的key数据的体积，尤其是针对返回数据格式变化不大的情况，支付宝聊天返回的数据用到了）


## 相关demo
- 弱网络模拟工具可以看
    - MonitorInterceptor
- 抓包拦截工具可以看
    - MonitorNetLib
- 流量统计工具可以看
    - MonitorFlow
- 网络测速工具可以看
    - MonitorSpeed

**弱网模拟的关键参数配置**：

| 网络环境 | 下行带宽 | 上行带宽 | 延迟 | 丢包率 |
|---------|---------|---------|------|-------|
| 4G优质 | 20Mbps | 5Mbps | 30ms | 0% |
| 4G一般 | 5Mbps | 1Mbps | 80ms | 1% |
| 3G | 1Mbps | 256Kbps | 200ms | 3% |
| 弱网 | 256Kbps | 64Kbps | 500ms | 5% |
| 极弱网 | 64Kbps | 16Kbps | 2000ms | 10% |

**弱网测试检查清单**：

1. 弱网下是否有合理的超时提示？
2. 超时后是否有重试机制？
3. 大图片是否有降级为缩略图的策略？
4. 关键接口是否有本地缓存兜底？
5. 网络恢复后是否能自动重新加载？
6. 弱网下是否会出现ANR（主线程网络请求）？
7. 请求队列是否在弱网下出现堆积导致OOM？
8. 是否有网络状态变化的监听和对应策略调整？

**网络优化效果对比模板**：

| 指标 | 优化前 | 优化后 | 改善幅度 |
|------|-------|--------|---------|
| DNS解析P90 | 120ms | 5ms | 95.8% |
| 请求成功率 | 98.5% | 99.7% | +1.2% |
| 平均请求耗时 | 650ms | 380ms | 41.5% |
| 月均流量消耗 | 150MB | 95MB | 36.7% |


## 05.如何衡量优化成果

### 5.1 流量消耗分析

流量消耗是网络优化最直接的指标之一。衡量流量消耗需要从以下维度进行：

**按场景统计**：
- 应用启动阶段的流量消耗
- 单个页面的流量消耗
- 后台运行时的流量消耗
- 图片/视频等多媒体的流量消耗

**按网络类型统计**：

| 网络类型 | 流量敏感度 | 优化策略 |
|---------|-----------|---------|
| WiFi | 低 | 可预加载、高清图片 |
| 5G | 中 | 正常加载策略 |
| 4G | 中 | 按需加载、图片压缩 |
| 3G | 高 | 缩略图、延迟加载 |
| 2G/弱网 | 极高 | 最低画质、核心数据优先 |

**流量消耗监控代码**：

```java
// 使用TrafficStats统计应用流量
public class TrafficMonitor {
    private long startRxBytes;
    private long startTxBytes;
    
    public void startMonitor() {
        int uid = android.os.Process.myUid();
        startRxBytes = TrafficStats.getUidRxBytes(uid);  // 接收字节数
        startTxBytes = TrafficStats.getUidTxBytes(uid);  // 发送字节数
    }
    
    public TrafficResult stopMonitor() {
        int uid = android.os.Process.myUid();
        long rxBytes = TrafficStats.getUidRxBytes(uid) - startRxBytes;
        long txBytes = TrafficStats.getUidTxBytes(uid) - startTxBytes;
        return new TrafficResult(rxBytes, txBytes);
    }
}
```

### 5.2 异常率统计

网络异常率是衡量网络质量的关键指标：

**关键指标定义**：
- 请求成功率 = 成功请求数 / 总请求数 × 100%
- DNS解析成功率 = DNS解析成功数 / DNS解析总数 × 100%
- 连接成功率 = TCP连接成功数 / TCP连接总数 × 100%
- 超时率 = 超时请求数 / 总请求数 × 100%

**健康标准**：
- 请求成功率 > 99.5%（优秀）
- DNS解析成功率 > 99.9%
- 连接成功率 > 99%
- 超时率 < 0.5%

**异常分类**：

| 异常类型 | 可能原因 | 排查方向 |
|---------|---------|---------|
| DNS解析失败 | DNS服务器异常、域名配置错误 | 切换DNS服务器、HTTPDNS |
| 连接超时 | 网络不通、服务器拒绝 | 检查网络连通性 |
| 读写超时 | 服务器响应慢、数据量大 | 优化接口、分页加载 |
| SSL错误 | 证书过期、证书不匹配 | 检查证书配置 |
| HTTP 4xx | 客户端请求错误 | 检查请求参数 |
| HTTP 5xx | 服务器内部错误 | 通知后端排查 |

### 5.3 稳定性统计

网络稳定性关注的是长期表现和一致性：

1. **P50/P90/P99请求耗时趋势**：持续跟踪各分位数的变化
2. **失败率变化趋势**：版本迭代前后的对比
3. **重试成功率**：评估重试策略的有效性
4. **缓存命中率**：评估缓存策略的效果


## 06.网络优化设计总结

### 6.1 网络优化核心原则

**原则一：减少请求次数**

每一次网络请求都有固定开销（DNS、TCP、TLS），减少请求次数是最直接的优化手段：
- 合并小请求为批量请求
- 合理使用缓存减少重复请求
- 避免不必要的轮询

**原则二：减少传输数据量**

- 请求和响应数据使用gzip压缩
- 使用Protocol Buffers替代JSON（数据量减少30%-70%）
- 图片按需加载不同清晰度的版本
- 去除冗余的请求头和公共参数

**原则三：加速请求过程**

- 使用HTTPDNS减少DNS解析时间
- HTTP/2多路复用减少连接建立次数
- 连接预建立（预热）
- CDN就近接入

**原则四：提高可靠性**

- 合理的重试策略（指数退避）
- 备用域名/IP切换
- 弱网环境降级策略
- 离线缓存兜底

### 6.2 跨平台网络优化对比

**疑惑**：不同平台的网络优化有什么差异？Android和iOS在网络优化上各有什么特点？

**答疑**：各平台的网络底层实现不同，但优化思路是相通的。

**论证**：

| 维度 | Android | iOS | Web | Flutter |
|------|---------|-----|-----|---------|
| 网络库 | OkHttp | NSURLSession | fetch/XMLHttpRequest | dio/http |
| HTTP/2 | OkHttp原生支持 | NSURLSession原生支持 | 浏览器自动协商 | 依赖底层库 |
| DNS优化 | HTTPDNS SDK | HTTPDNS SDK | 浏览器管理 | 自定义DNS |
| 连接复用 | OkHttp ConnectionPool | NSURLSession | 浏览器管理 | 依赖底层库 |
| 缓存策略 | OkHttp Cache | NSURLCache | Service Worker | 自定义 |
| 监控 | EventListener | URLSessionTaskMetrics | Performance API | HttpClientAdapter |
| 证书固定 | CertificatePinner | URLSessionDelegate | 不支持 | SecurityContext |

**iOS网络优化特点**：

```swift
// iOS使用URLSessionTaskMetrics统计网络各阶段耗时
let session = URLSession(configuration: config, delegate: self, delegateQueue: nil)

func urlSession(_ session: URLSession, task: URLSessionTask, 
                didFinishCollecting metrics: URLSessionTaskMetrics) {
    for metric in metrics.transactionMetrics {
        let dnsTime = metric.domainLookupEndDate?.timeIntervalSince(
            metric.domainLookupStartDate ?? Date()) ?? 0
        let connectTime = metric.connectEndDate?.timeIntervalSince(
            metric.connectStartDate ?? Date()) ?? 0
        let tlsTime = metric.secureConnectionEndDate?.timeIntervalSince(
            metric.secureConnectionStartDate ?? Date()) ?? 0
        let requestTime = metric.requestEndDate?.timeIntervalSince(
            metric.requestStartDate ?? Date()) ?? 0
        let responseTime = metric.responseEndDate?.timeIntervalSince(
            metric.responseStartDate ?? Date()) ?? 0
        
        print("DNS: \(dnsTime)s, Connect: \(connectTime)s, TLS: \(tlsTime)s")
    }
}
```

**Web端网络优化特点**：

```javascript
// Web使用Performance API和Resource Timing API
const observer = new PerformanceObserver((list) => {
    for (const entry of list.getEntries()) {
        if (entry.entryType === 'resource') {
            console.log(`${entry.name}:`);
            console.log(`  DNS: ${entry.domainLookupEnd - entry.domainLookupStart}ms`);
            console.log(`  TCP: ${entry.connectEnd - entry.connectStart}ms`);
            console.log(`  TLS: ${entry.secureConnectionStart > 0 ? 
                entry.connectEnd - entry.secureConnectionStart : 0}ms`);
            console.log(`  TTFB: ${entry.responseStart - entry.requestStart}ms`);
            console.log(`  Download: ${entry.responseEnd - entry.responseStart}ms`);
        }
    }
});
observer.observe({ type: 'resource', buffered: true });

// Service Worker缓存策略
self.addEventListener('fetch', (event) => {
    event.respondWith(
        caches.match(event.request).then((response) => {
            // 缓存优先，缓存未命中时请求网络
            return response || fetch(event.request).then((networkResponse) => {
                const cache = caches.open('v1');
                cache.then(c => c.put(event.request, networkResponse.clone()));
                return networkResponse;
            });
        })
    );
});
```

**结果**：无论哪个平台，网络优化的核心都是减少请求次数、减少数据量、加速请求过程、提高可靠性。关键差异在于各平台提供的底层API和可优化的粒度不同。

### 6.3 网络优化方法论

网络优化遵循"监控-分析-优化-验证"的循环方法论：

**第一步：建立监控体系**

全面采集网络请求的各阶段耗时数据：
- DNS解析耗时
- TCP连接耗时
- TLS握手耗时
- 首字节时间（TTFB）
- 数据传输耗时
- 总请求耗时
- 请求成功率/失败率

**第二步：分析性能瓶颈**

根据监控数据找出主要的性能瓶颈：
- P90耗时超过阈值的接口
- 失败率Top 10接口
- 流量消耗Top 10接口
- 弱网环境下的特殊问题

**第三步：针对性优化**

按照影响面和收益进行优先级排序：
- DNS优化（HTTPDNS）→ 解决DNS劫持和解析慢问题
- 连接优化（HTTP/2、连接复用）→ 减少连接建立开销
- 数据优化（压缩、缓存）→ 减少传输量
- 协议优化（QUIC）→ 解决TCP队首阻塞

**第四步：效果验证**

- A/B测试对比优化前后的各项指标
- 关注不同网络环境下的改善程度
- 确保优化没有引入新的问题

### 6.4 网络优化常见误区

**疑惑**：网络优化中有哪些容易踩的坑？

**答疑**：网络优化涉及客户端、服务端、网络链路多个环节，容易出现片面优化的问题。

**论证**：

**误区一：只关注客户端不关注服务端**

很多时候网络请求慢不是客户端的问题，而是服务端接口性能差。需要端到端地分析请求耗时，明确瓶颈在哪一端。

**误区二：在WiFi环境下做网络优化测试**

WiFi环境下DNS快、带宽大、延迟低，掩盖了很多问题。网络优化必须在4G、3G、弱网等真实移动网络环境下测试。

**误区三：缓存策略过于激进**

缓存可以减少网络请求，但过于激进的缓存会导致数据时效性问题。需要根据数据的更新频率合理设置缓存时间和刷新策略。

**误区四：忽略请求重试的副作用**

盲目的请求重试在弱网环境下会加重网络拥塞，并且可能导致服务端重复处理（幂等性问题）。重试策略需要设置合理的间隔（指数退避）、最大次数、和幂等保障。

**误区五：HTTP/2一定比HTTP/1.1快**

HTTP/2的多路复用确实减少了连接数，但在高丢包率的网络环境下，TCP的队首阻塞问题会导致所有请求都被阻塞。这种场景下QUIC（HTTP/3）是更好的选择。

**结果**：网络优化需要全链路的视角，从DNS、连接、传输、数据到业务逻辑，每个环节都可能是瓶颈。建立完善的监控体系，在真实网络环境下测试验证，才能取得实际的优化效果。


## 07.跨平台网络监控伪代码

### 7.1 Android网络监控

```java
// Android网络性能监控器
public class AndroidNetworkMonitor {
    
    private OkHttpClient client;
    
    // 基于OkHttp EventListener的全链路网络监控
    public void setupNetworkMonitor() {
        client = new OkHttpClient.Builder()
            .eventListenerFactory(NetworkEventListener::new)
            .addInterceptor(new NetworkStatsInterceptor())
            .dns(new MonitoredDns(Dns.SYSTEM))  // DNS监控
            .build();
    }
    
    // 网络事件监听器——精确记录每个阶段耗时
    static class NetworkEventListener extends EventListener {
        private long dnsStartTime;
        private long connectStartTime;
        private long tlsStartTime;
        private long requestStartTime;
        private long responseStartTime;
        
        @Override
        public void dnsStart(Call call, String domainName) {
            dnsStartTime = System.nanoTime();
        }
        
        @Override
        public void dnsEnd(Call call, String domainName, List<InetAddress> addresses) {
            long dnsTime = (System.nanoTime() - dnsStartTime) / 1_000_000;
            // DNS解析耗时（ms）
            NetworkStats.recordDns(domainName, dnsTime, addresses.size());
        }
        
        @Override
        public void connectStart(Call call, InetSocketAddress addr, Proxy proxy) {
            connectStartTime = System.nanoTime();
        }
        
        @Override
        public void secureConnectStart(Call call) {
            tlsStartTime = System.nanoTime();
        }
        
        @Override
        public void secureConnectEnd(Call call, Handshake handshake) {
            long tlsTime = (System.nanoTime() - tlsStartTime) / 1_000_000;
            NetworkStats.recordTls(tlsTime, handshake.tlsVersion().javaName());
        }
        
        @Override
        public void connectEnd(Call call, InetSocketAddress addr, 
                              Proxy proxy, Protocol protocol) {
            long connectTime = (System.nanoTime() - connectStartTime) / 1_000_000;
            NetworkStats.recordConnect(connectTime, protocol.toString());
        }
        
        @Override
        public void requestBodyEnd(Call call, long byteCount) {
            NetworkStats.recordRequestSize(byteCount);
        }
        
        @Override
        public void responseBodyEnd(Call call, long byteCount) {
            NetworkStats.recordResponseSize(byteCount);
        }
        
        @Override
        public void callFailed(Call call, IOException e) {
            // 记录失败信息：网络类型、错误类型、重试次数
            NetworkStats.recordFailure(call.request().url().toString(), 
                e.getClass().getSimpleName(), e.getMessage());
        }
    }
    
    // 网络状态检测
    public NetworkInfo getNetworkInfo(Context context) {
        ConnectivityManager cm = (ConnectivityManager) 
            context.getSystemService(Context.CONNECTIVITY_SERVICE);
        
        NetworkInfo info = new NetworkInfo();
        
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            Network network = cm.getActiveNetwork();
            NetworkCapabilities caps = cm.getNetworkCapabilities(network);
            
            if (caps != null) {
                info.hasWifi = caps.hasTransport(NetworkCapabilities.TRANSPORT_WIFI);
                info.hasCellular = caps.hasTransport(NetworkCapabilities.TRANSPORT_CELLULAR);
                // 获取下行带宽估计（Kbps）
                info.downstreamBandwidth = caps.getLinkDownstreamBandwidthKbps();
                info.upstreamBandwidth = caps.getLinkUpstreamBandwidthKbps();
            }
        }
        
        // 通过TelephonyManager获取移动网络类型
        TelephonyManager tm = (TelephonyManager) 
            context.getSystemService(Context.TELEPHONY_SERVICE);
        info.networkType = getNetworkTypeName(tm.getDataNetworkType());
        
        return info;
    }
    
    // HTTPDNS实现（避免Local DNS劫持）
    static class MonitoredDns implements Dns {
        private final Dns systemDns;
        private final Map<String, List<InetAddress>> dnsCache = new ConcurrentHashMap<>();
        
        MonitoredDns(Dns systemDns) {
            this.systemDns = systemDns;
        }
        
        @Override
        public List<InetAddress> lookup(String hostname) throws UnknownHostException {
            // 1. 优先使用HTTPDNS
            List<InetAddress> httpDnsResult = queryHttpDns(hostname);
            if (httpDnsResult != null && !httpDnsResult.isEmpty()) {
                dnsCache.put(hostname, httpDnsResult);
                return httpDnsResult;
            }
            
            // 2. 降级到本地缓存
            List<InetAddress> cached = dnsCache.get(hostname);
            if (cached != null) {
                return cached;
            }
            
            // 3. 降级到系统DNS
            return systemDns.lookup(hostname);
        }
        
        private List<InetAddress> queryHttpDns(String hostname) {
            // 调用HTTPDNS服务（如阿里云/腾讯云HTTPDNS）
            // 返回IP地址列表
            return null;
        }
    }
}
```

### 7.2 iOS网络监控

```objectivec
// iOS网络性能监控器
@interface IOSNetworkMonitor : NSObject <NSURLSessionTaskDelegate>

@property (nonatomic, strong) NSURLSession *monitoredSession;

@end

@implementation IOSNetworkMonitor

- (void)setupMonitoring {
    NSURLSessionConfiguration *config = [NSURLSessionConfiguration defaultSessionConfiguration];
    // 设置超时
    config.timeoutIntervalForRequest = 30;
    config.timeoutIntervalForResource = 60;
    
    self.monitoredSession = [NSURLSession sessionWithConfiguration:config 
                                                          delegate:self 
                                                     delegateQueue:nil];
}

// 基于NSURLSessionTaskMetrics的全链路监控（iOS 10+）
- (void)URLSession:(NSURLSession *)session 
              task:(NSURLSessionTask *)task 
didFinishCollectingMetrics:(NSURLSessionTaskMetrics *)metrics {
    
    for (NSURLSessionTaskTransactionMetrics *transaction in metrics.transactionMetrics) {
        NSMutableDictionary *timing = [NSMutableDictionary dictionary];
        
        // DNS解析耗时
        if (transaction.domainLookupStartDate && transaction.domainLookupEndDate) {
            NSTimeInterval dnsTime = [transaction.domainLookupEndDate 
                timeIntervalSinceDate:transaction.domainLookupStartDate] * 1000;
            timing[@"dnsTime"] = @(dnsTime);
        }
        
        // TCP连接耗时
        if (transaction.connectStartDate && transaction.connectEndDate) {
            NSTimeInterval connectTime = [transaction.connectEndDate 
                timeIntervalSinceDate:transaction.connectStartDate] * 1000;
            timing[@"connectTime"] = @(connectTime);
        }
        
        // TLS握手耗时
        if (transaction.secureConnectionStartDate && transaction.secureConnectionEndDate) {
            NSTimeInterval tlsTime = [transaction.secureConnectionEndDate 
                timeIntervalSinceDate:transaction.secureConnectionStartDate] * 1000;
            timing[@"tlsTime"] = @(tlsTime);
        }
        
        // 请求发送耗时
        if (transaction.requestStartDate && transaction.requestEndDate) {
            NSTimeInterval requestTime = [transaction.requestEndDate 
                timeIntervalSinceDate:transaction.requestStartDate] * 1000;
            timing[@"requestTime"] = @(requestTime);
        }
        
        // 响应接收耗时（首字节）
        if (transaction.requestEndDate && transaction.responseStartDate) {
            NSTimeInterval ttfb = [transaction.responseStartDate 
                timeIntervalSinceDate:transaction.requestEndDate] * 1000;
            timing[@"ttfb"] = @(ttfb);  // Time To First Byte
        }
        
        // 总耗时
        if (transaction.fetchStartDate && transaction.responseEndDate) {
            NSTimeInterval totalTime = [transaction.responseEndDate 
                timeIntervalSinceDate:transaction.fetchStartDate] * 1000;
            timing[@"totalTime"] = @(totalTime);
        }
        
        // 协议信息
        timing[@"protocol"] = transaction.networkProtocolName ?: @"unknown";
        timing[@"isReused"] = @(transaction.isReusedConnection);
        timing[@"resourceType"] = transaction.resourceFetchType == 
            NSURLSessionTaskMetricsResourceFetchTypeLocalCache ? @"cache" : @"network";
        
        [self reportNetworkMetrics:timing forURL:task.originalRequest.URL.absoluteString];
    }
}

// 网络质量检测
- (void)detectNetworkQuality {
    // 使用NWPathMonitor（iOS 12+）
    NWPathMonitor *monitor = [[NWPathMonitor alloc] init];
    monitor.pathUpdateHandler = ^(nw_path_t path) {
        if (nw_path_get_status(path) == nw_path_status_satisfied) {
            // 网络可用
            BOOL isExpensive = nw_path_is_expensive(path);      // 是否计费网络
            BOOL isConstrained = nw_path_is_constrained(path);  // 是否受限（低数据模式）
            
            // 检查可用的网络接口
            nw_path_enumerate_interfaces(path, ^bool(nw_interface_t interface) {
                nw_interface_type_t type = nw_interface_get_type(interface);
                // wifi / cellular / wiredEthernet / loopback / other
                return true;  // 继续枚举
            });
        }
    };
    [monitor startWithQueue:dispatch_get_global_queue(QOS_CLASS_UTILITY, 0)];
}

@end
```

### 7.3 Web网络监控

```javascript
// Web网络性能监控器
class WebNetworkMonitor {
    constructor() {
        this.metrics = [];
    }
    
    startMonitoring() {
        this.monitorResourceTiming();
        this.monitorFetch();
        this.monitorNetworkInfo();
        this.monitorNavigationTiming();
    }
    
    // 1. Resource Timing API——自动采集所有资源的网络耗时
    monitorResourceTiming() {
        const observer = new PerformanceObserver((list) => {
            for (const entry of list.getEntries()) {
                // 只关注XMLHttpRequest和Fetch请求
                if (entry.initiatorType === 'xmlhttprequest' || 
                    entry.initiatorType === 'fetch') {
                    const timing = {
                        url: entry.name,
                        // DNS解析
                        dnsTime: entry.domainLookupEnd - entry.domainLookupStart,
                        // TCP连接
                        connectTime: entry.connectEnd - entry.connectStart,
                        // TLS握手
                        tlsTime: entry.secureConnectionStart > 0 
                            ? entry.connectEnd - entry.secureConnectionStart : 0,
                        // TTFB（首字节时间）
                        ttfb: entry.responseStart - entry.requestStart,
                        // 响应下载
                        downloadTime: entry.responseEnd - entry.responseStart,
                        // 总耗时
                        totalTime: entry.responseEnd - entry.startTime,
                        // 传输大小
                        transferSize: entry.transferSize,
                        encodedSize: entry.encodedBodySize,
                        decodedSize: entry.decodedBodySize,
                        // 协议
                        protocol: entry.nextHopProtocol,  // h2, h3, http/1.1
                        // 缓存状态
                        fromCache: entry.transferSize === 0
                    };
                    this.reportMetrics(timing);
                }
            }
        });
        observer.observe({ entryTypes: ['resource'] });
    }
    
    // 2. Fetch拦截——监控请求成功率和错误类型
    monitorFetch() {
        const originalFetch = window.fetch;
        window.fetch = async (input, init) => {
            const startTime = performance.now();
            const url = typeof input === 'string' ? input : input.url;
            
            try {
                const response = await originalFetch(input, init);
                const duration = performance.now() - startTime;
                
                // 记录请求结果
                this.recordRequest({
                    url: url,
                    method: init?.method || 'GET',
                    status: response.status,
                    duration: duration,
                    success: response.ok,
                    contentLength: response.headers.get('content-length')
                });
                
                return response;
            } catch (error) {
                const duration = performance.now() - startTime;
                
                // 记录失败请求
                this.recordRequest({
                    url: url,
                    method: init?.method || 'GET',
                    status: 0,
                    duration: duration,
                    success: false,
                    error: error.message
                });
                
                throw error;
            }
        };
    }
    
    // 3. Network Information API——感知网络质量变化
    monitorNetworkInfo() {
        if (!navigator.connection) {
            console.log('Network Information API不可用');
            return;
        }
        
        const connection = navigator.connection;
        
        // 初始网络信息
        this.logNetworkInfo(connection);
        
        // 监听网络变化
        connection.addEventListener('change', () => {
            this.logNetworkInfo(connection);
        });
    }
    
    logNetworkInfo(connection) {
        const info = {
            // 有效连接类型：slow-2g, 2g, 3g, 4g
            effectiveType: connection.effectiveType,
            // RTT估计（ms）
            rtt: connection.rtt,
            // 下行带宽估计（Mbps）
            downlink: connection.downlink,
            // 是否处于省流量模式
            saveData: connection.saveData,
            // 实际连接类型（wifi, cellular等）
            type: connection.type
        };
        
        // 根据网络质量调整请求策略
        if (info.effectiveType === 'slow-2g' || info.effectiveType === '2g') {
            // 弱网策略：降低图片质量、减少并发请求、增加超时时间
            this.applyWeakNetworkStrategy();
        } else if (info.effectiveType === '4g') {
            // 强网策略：预加载、高质量图片
            this.applyStrongNetworkStrategy();
        }
    }
    
    // 4. Navigation Timing——页面加载的网络耗时
    monitorNavigationTiming() {
        window.addEventListener('load', () => {
            setTimeout(() => {
                const nav = performance.getEntriesByType('navigation')[0];
                if (!nav) return;
                
                const pageLoadMetrics = {
                    // 重定向耗时
                    redirectTime: nav.redirectEnd - nav.redirectStart,
                    // DNS耗时
                    dnsTime: nav.domainLookupEnd - nav.domainLookupStart,
                    // TCP连接耗时
                    connectTime: nav.connectEnd - nav.connectStart,
                    // TLS耗时
                    tlsTime: nav.secureConnectionStart > 0 
                        ? nav.connectEnd - nav.secureConnectionStart : 0,
                    // TTFB
                    ttfb: nav.responseStart - nav.requestStart,
                    // 响应耗时
                    responseTime: nav.responseEnd - nav.responseStart,
                    // DOM解析耗时
                    domParseTime: nav.domInteractive - nav.responseEnd,
                    // 总加载时间
                    totalLoadTime: nav.loadEventEnd - nav.startTime,
                    // 协议
                    protocol: nav.nextHopProtocol
                };
                
                this.reportPageLoadMetrics(pageLoadMetrics);
            }, 0);
        });
    }
}
```

### 7.4 网络优化技术演进

**疑惑**：从HTTP/1.0到HTTP/3，网络协议的演进解决了哪些核心问题？

**答疑**：每一代协议都是对前一代核心痛点的针对性解决。

**论证**：

| 协议版本 | 核心改进 | 解决的问题 | 遗留问题 |
|---------|---------|-----------|---------|
| HTTP/1.0 | 首个标准化HTTP | 无标准协议 | 每个请求新建TCP连接 |
| HTTP/1.1 | 持久连接、管线化 | 连接复用 | 队首阻塞（HOL Blocking） |
| HTTP/2 | 多路复用、头部压缩、服务器推送 | HTTP层队首阻塞 | TCP层队首阻塞、握手延迟 |
| HTTP/3(QUIC) | 基于UDP、0-RTT、独立流 | TCP队首阻塞、连接迁移 | 部署普及度、中间件支持 |

```
HTTP/1.1 的队首阻塞问题：
┌─────────────┐     ┌──────────┐
│  Request A   │────▶│ Server   │  A阻塞后，B、C必须等待
│  Request B   │  ×  │          │
│  Request C   │  ×  │          │
└─────────────┘     └──────────┘

HTTP/2 多路复用：
┌─────────────┐     ┌──────────┐
│ Stream 1 (A) │────▶│          │  多个Stream在一个TCP连接上
│ Stream 2 (B) │────▶│ Server   │  HTTP层不再阻塞
│ Stream 3 (C) │────▶│          │  但TCP丢包会阻塞所有Stream
└─────────────┘     └──────────┘

HTTP/3(QUIC) 独立流：
┌─────────────┐     ┌──────────┐
│ Stream 1 (A) │────▶│          │  基于UDP，每个Stream独立
│ Stream 2 (B) │────▶│ Server   │  Stream 1丢包不影响2和3
│ Stream 3 (C) │────▶│          │  支持0-RTT和连接迁移
└─────────────┘     └──────────┘
```

**结果**：在实际网络优化中，协议选择需要结合业务场景。HTTP/2适合大多数场景，HTTP/3在高丢包率的移动网络环境下优势明显。但协议升级只是优化的一部分，DNS优化、连接复用、数据压缩、缓存策略等同样重要。


## 08.网络监控设计总结

### 8.1 跨平台网络监控对比

| 维度 | Android | iOS | Web |
|------|---------|-----|-----|
| 全链路监控 | OkHttp EventListener | NSURLSessionTaskMetrics | Resource Timing API |
| DNS监控 | 自定义Dns接口 | CFHost/NSURLSessionTaskMetrics | Resource Timing API |
| 网络状态 | ConnectivityManager/NetworkCallback | NWPathMonitor | Network Information API |
| 流量统计 | TrafficStats(uid级别) | NetworkExtension | Transfer-Size (Resource Timing) |
| 弱网模拟 | Charles/Fiddler代理 | Network Link Conditioner | Chrome DevTools Throttling |
| HTTPDNS | OkHttp自定义Dns | CFHost/NSURLConnection | 不适用（浏览器控制DNS） |
| 抓包工具 | Charles/Wireshark/OkHttp Interceptor | Charles/Wireshark | Chrome DevTools Network |

### 8.2 网络优化核心策略

- **减少请求次数**：合并请求、接口聚合、GraphQL按需查询
- **减少传输数据量**：gzip/br压缩、Protocol Buffers替代JSON、图片格式优化（WebP/AVIF）
- **加速连接建立**：HTTPDNS预解析、连接预建立、HTTP/2多路复用、QUIC 0-RTT
- **智能缓存策略**：本地缓存+增量更新、ETag/Last-Modified条件请求、离线优先架构
- **弱网适配**：自适应超时策略、请求优先级队列、降级方案（低质量图片、精简数据）

- 探索 Android 网络优化方法
    - https://www.jianshu.com/p/41bbf2f9c2a9
- 移动端网络监控实践
    - https://juejin.cn/post/7018212919439523847
- Android网络性能监控方案
    - https://zhuanlan.zhihu.com/p/285191490
- 货拉拉移动端网络优化——协议升级篇
  - https://juejin.cn/post/7194627379656917047#heading-11




