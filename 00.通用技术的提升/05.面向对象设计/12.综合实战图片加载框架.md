...
#### 目录介绍
- [00.一场图片引发的崩溃](#00一场图片引发的崩溃)
  - [0.1 大促夜的事故现场](#01-大促夜的事故现场)
  - [0.2 排查路径与最终归因](#02-排查路径与最终归因)
  - [0.3 八个灵魂拷问](#03-八个灵魂拷问)
  - [0.4 本篇路线图](#04-本篇路线图)
- [01.V0版本能跑就行](#01v0版本能跑就行)
  - [1.1 三百行的屎山](#11-三百行的屎山)
  - [1.2 找出十七处坏味道](#12-找出十七处坏味道)
  - [1.3 屎山的代价清单](#13-屎山的代价清单)
- [02.封装救场状态浮现](#02封装救场状态浮现)
  - [2.1 静态地狱的根源](#21-静态地狱的根源)
  - [2.2 抽出请求对象](#22-抽出请求对象)
  - [2.3 引入状态机守卫](#23-引入状态机守卫)
  - [2.4 值对象与缓存键](#24-值对象与缓存键)
- [03.抽象与接口分离](#03抽象与接口分离)
  - [3.1 识别变化轴](#31-识别变化轴)
  - [3.2 三个端口的浮现](#32-三个端口的浮现)
  - [3.3 抽象类共享骨架](#33-抽象类共享骨架)
  - [3.4 与Glide的对照](#34-与glide的对照)
- [04.组合优于继承的洋葱](#04组合优于继承的洋葱)
  - [4.1 继承式方案的爆炸](#41-继承式方案的爆炸)
  - [4.2 拦截器链的诞生](#42-拦截器链的诞生)
  - [4.3 能力可插拔实战](#43-能力可插拔实战)
  - [4.4 与OkHttp的同构](#44-与okhttp的同构)
- [05.V1阶段小结](#05v1阶段小结)
- [06.SOLID五原则精修](#06solid五原则精修)
  - [6.1 SRP拆解请求职责](#61-srp拆解请求职责)
  - [6.2 OCP扩展WebP不改旧码](#62-ocp扩展webp不改旧码)
  - [6.3 LSP前置后置条件](#63-lsp前置后置条件)
  - [6.4 ISP拆分缓存接口](#64-isp拆分缓存接口)
  - [6.5 DIP解OkHttp耦合](#65-dip解okhttp耦合)
- [07.重构十二式实战](#07重构十二式实战)
  - [7.1 提取方法解码逻辑](#71-提取方法解码逻辑)
  - [7.2 引入参数对象LoadOptions](#72-引入参数对象loadoptions)
  - [7.3 用多态替换条件](#73-用多态替换条件)
  - [7.4 引入策略变换族](#74-引入策略变换族)
  - [7.5 守卫子句替换嵌套](#75-守卫子句替换嵌套)
  - [7.6 替换魔法数](#76-替换魔法数)
  - [7.7 分解上帝类](#77-分解上帝类)
  - [7.8 查找表替换switch](#78-查找表替换switch)
- [08.可测试性改造](#08可测试性改造)
  - [8.1 注入Clock抽象时间](#81-注入clock抽象时间)
  - [8.2 注入Scheduler抽象线程](#82-注入scheduler抽象线程)
  - [8.3 注入Network抽象网络](#83-注入network抽象网络)
  - [8.4 五个高价值单测](#84-五个高价值单测)
  - [8.5 录制式调度器](#85-录制式调度器)
- [09.V4阶段小结](#09v4阶段小结)
- [10.V5用DDD重塑边界](#10v5用ddd重塑边界)
  - [10.1 为何还要再切一刀](#101-为何还要再切一刀)
  - [10.2 五个限界上下文](#102-五个限界上下文)
  - [10.3 上下文映射与防腐](#103-上下文映射与防腐)
  - [10.4 领域事件串联协作](#104-领域事件串联协作)
  - [10.5 三大库的对照分析](#105-三大库的对照分析)
- [11.全景类图与篇章映射](#11全景类图与篇章映射)
  - [11.1 V5终极架构全景](#111-v5终极架构全景)
  - [11.2 按图索骥十一篇映射](#112-按图索骥十一篇映射)
  - [11.3 演进路径回望](#113-演进路径回望)
- [12.终极思考题](#12终极思考题)
  - [12.1 跨平台抽象题](#121-跨平台抽象题)
  - [12.2 画质降级题](#122-画质降级题)
  - [12.3 百万QPS改造题](#123-百万qps改造题)
- [13.写在最后](#13写在最后)
