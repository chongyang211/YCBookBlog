...
> 3. **第 03 节 · 原理与实现** — 类适配器（继承）vs 对象适配器（组合）两种姿势
...
>
> - **18:30**：商户后台退款接口 500，原因 `BigDecimal` × 100 转 `long cents` 时调用点忘了粘贴 `setScale(2, ROUND_HALF_UP)`，**一笔 99.999 元退款变成了 9999 分出了甚么问题你们猜**；
> - **22:15**：对账 Job 跱了，原因商户后台代码中有些地方走老 SDK 、有些走新 SDK，**订单号字段一个叫 `orderId` 一个叫 `bizNo`，双表联不上**；
