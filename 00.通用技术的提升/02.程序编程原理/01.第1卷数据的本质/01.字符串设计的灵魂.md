...
**反直觉案例**：1998 年 C++98 标准化 `std::string` 时，引发了一场 **ABI 大战**——GCC 和微软 MSVC 的 `std::string` 内部布局**完全不兼容**，导致同一份头文件编译出的二进制不能互相链接。原因是双方在做同一个优化：**SSO（Small String Optimization，短字符串优化）**，但实现方式不同。
...
**推理链：如果让你来设计一门新语言的字符串，你会怎么做？**

让我们沿着前面四节累积的实证数据，一步步推导。
