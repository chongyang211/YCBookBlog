...
> 🔗 **延伸阅读**：← [2.7 反射与元编程核心设计](./2.7.反射与元编程核心设计.md) · ← [2.4 函数调用栈与栈帧设计](./2.4.函数调用栈与栈帧设计.md) · → [3.x 并发错误处理](../03.第3卷并发之道/) · → [5.x RPC 错误传播](../05.第5卷交互与系统/)
...
- [04.Checked vs Unchecked：Java 的争议设计](#04checked-vs-uncheckedjava-的争议设计)
...
### 1.3 范式 3：Result 类型（Rust/Haskell 风格）
...
## 04.Checked vs Unchecked：Java 的争议设计
...
### 5.2 Go 的 panic / recover：保留的"逃生通道"
...
### 5.4 Rust 的 Result vs Go 的 error
...
### 7.1 陷阱一：catch (Exception e) {} 吞异常（§0.1）
...
### 7.6 陷阱六：try-with-resources 不用是耻辱
...
### 7.7 陷阱七：在 catch 中吞掉 InterruptedException
...
**InterruptedException 是 Java 多线程编程最易错的点之一**——吞掉它会让上游线程池的关闭机制失效。详见 [3.x 线程池设计](../03.第3卷并发之道/)。
...
下一卷 [第 3 卷 并发之道](../03.第3卷并发之道/) 我们会进入更复杂的领域——**当多个执行线索同时存在时，世界变得多么不同**。从"一个 CPU 核心顺序执行"到"多核并发竞争"，这是过去 20 年最大的复杂度跃迁。
...
- 下卷开篇：[3.x 并发模型基础](../03.第3卷并发之道/)
- 工程层面：[5.x RPC 错误传播与重试](../05.第5卷交互与系统/)
- 经典文献：
