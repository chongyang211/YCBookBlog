...
#### 目录介绍
- [00.从一次「列表里的按钮点不动」事故说起](#00从一次列表里的按钮点不动事故说起)
  - [0.1 用户反馈：「我点不到收藏按钮」](#01-用户反馈我点不到收藏按钮)
...
### 6.2 Android 解法：requestDisallowInterceptTouchEvent
...
### 6.3 iOS 解法：require(toFail:) + delegate
...
### 7.2 Flutter 的 Gesture Arena：最优雅的设计
...
### 7.3 Web 的 touch-action：用 CSS 表达手势意图
...
### 8.1 陷阱一：onTouchEvent 返回 false 后后续事件不再收到
...
### 8.2 陷阱二：不处理 ACTION_CANCEL（§0 事故根因）
...
### 8.3 陷阱三：多点触控下只看 ACTION_DOWN 不看 ACTION_POINTER_DOWN
...
### 8.4 陷阱四：在 dispatchTouchEvent 打印日志导致 ANR
...
### 8.5 陷阱五：Web 的 click 事件 300ms 延迟
...
### 8.6 陷阱六：iOS 嵌套 ScrollView 内部手势不响应
...
### 8.7 陷阱七：Flutter 在 GestureDetector 中嵌套 InkWell
