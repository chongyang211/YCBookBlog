...
> 🎯 **核心矛盾**：**多个执行流隔离运行 vs 必须协作完成任务** —— 不通信就没意义，乱通信就出 bug
> 🧭 **设计灵魂**：线程通信只有两大流派——**共享内存 + 同步原语**（Java/C++ 主流）vs **消息传递**（Go/Erlang）；前者快但易错，后者慢但易写
> 🌐 **跨语言覆盖**：Java(wait/notify + Condition + 共享变量) · POSIX(pthread_cond + 信号量) · Go(channel) · C++(condition_variable + future) · JS(SharedArrayBuffer + postMessage)
> 🔗 **延伸阅读**：← [11.线程前世今生探索](./11.线程前世今生探索.md) · → [15.并发编程设计思想](./15.并发编程设计思想.md) · → [24.Actor与CSP并发模型](./24.Actor与CSP并发模型.md)

```mermaid
flowchart TB
    A[多个执行流<br/>需要协作] --> B{怎么传递信息}
    B --> C1[共享内存派<br/>变量 + 锁/条件变量]
    B --> C2[消息传递派<br/>channel / mailbox]
    C1 --> D1[优势：性能<br/>劣势：易死锁/数据竞争]
...
- [1.线程通信背景](#1线程通信背景)
  - [1.1 通信的根因](#11-通信的根因)
  - [1.2 线程通信本质](#12-线程通信本质)
  - [1.3 没有通信状况](#13-没有通信状况)
  - [1.4 解决什么问题](#14-解决什么问题)
- [2.线程通信框架](#2线程通信框架)
  - [2.1 两大通信模型](#21-两大通信模型)
  - [2.2 共享内存模型](#22-共享内存模型)
  - [2.3 消息传递模型](#23-消息传递模型)
  - [2.4 硬件层设计](#24-硬件层设计)
  - [2.5 软件层设计](#25-软件层设计)
- [3.线程通信原理](#3线程通信原理)
  - [3.1 通信数学本质](#31-通信数学本质)
  - [3.2 硬件两工具](#32-硬件两工具)
  - [3.3 同步原语](#33-同步原语)
  - [3.4 抽象本质分类](#34-抽象本质分类)
  - [3.5 设计权衡](#35-设计权衡)
- [5.Java线程通信](#5java线程通信)
  - [5.1 wait和notify](#51-wait和notify)
  - [5.2 Condition](#52-condition)
  - [5.3 BlockingQueue](#53-blockingqueue)
  - [5.4 CountDownLatch](#54-countdownlatch)
  - [5.5 CyclicBarrier](#55-cyclicbarrier)
  - [5.6 Semaphore](#56-semaphore)
  - [5.7 Future](#57-future)
  - [5.8 通信场景总结](#58-通信场景总结)
- [6.C++线程通信](#6c线程通信)
  - [6.1 互斥锁+条件变量](#61-互斥锁条件变量)
  - [6.2 原子操作](#62-原子操作)
  - [6.3 信号量](#63-信号量)
  - [6.4 屏障](#64-屏障)
  - [6.5 消息队列](#65-消息队列)
...
- [11.线程前世今生探索](./11.线程前世今生探索.md)：通信前置条件——什么是线程
- [13.线程异常设计原理](./13.线程异常设计原理.md)：通信失败时如何安全终止
- [18.锁核心设计和思想](./18.锁核心设计和思想.md)：mutex / synchronized / ReentrantLock 的实现差异
- [19.AQS核心思想揭秘](./19.AQS核心思想揭秘.md)：理解 Doug Lea 的并发框架基石
- [23.协程核心设计思想](./23.协程核心设计思想.md)：协程时代的通信范式
