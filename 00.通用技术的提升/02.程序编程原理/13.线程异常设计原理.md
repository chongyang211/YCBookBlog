...
> 📍 **本篇位置**：第 3 卷 · 并发之道 · 第 3 篇
> 🎯 **核心矛盾**：**异常天然属于调用栈 vs 线程各自有独立栈** —— 子线程崩了，谁来收尸？
> 🧭 **设计灵魂**：线程异常处理走过三代——**默认杀进程** → **每线程 UncaughtHandler** → **结构化并发统一汇集**；本质是从"放任"到"父对子负责"
> 🌐 **跨语言覆盖**：Java(Thread.UncaughtExceptionHandler) · C++(std::terminate / 异常逃出 thread = 程序终止) · Kotlin(CoroutineExceptionHandler + SupervisorJob) · Swift(Task 取消传播) · Go(panic + recover 仅本 goroutine)
> 🔗 **延伸阅读**：← [12.线程通信设计思想](./12.线程通信设计思想.md) · → [25.线程池的设计思想](./25.线程池的设计思想.md) · → [23.协程核心设计思想](./23.协程核心设计思想.md)
...
flowchart LR
    ...
    B -->|有| C1[正常处理]
    B -->|没有| C2[逃出栈顶]
    C2 --> D1[Java: 调 UncaughtHandler]
    C2 --> D2[C++: terminate 整个进程]
    C2 --> D3[Go: panic 传到 goroutine 顶]
...
- [1.线程异常由来](#1线程异常由来)
  - [1.1 无异常世界](#11-无异常世界)
  - [1.2 无异常推演](#12-无异常推演)
  - [1.3 为何设计异常](#13-为何设计异常)
  - [1.4 异常设计思想](#14-异常设计思想)
  - [1.5 核心哲学](#15-核心哲学)
  - [1.6 异常的本质](#16-异常的本质)
- [2.异常如何引起](#2异常如何引起)
  - [2.1 两种触发源](#21-两种触发源)
  - [2.2 硬件触发异常](#22-硬件触发异常)
  - [2.3 软件触发异常](#23-软件触发异常)
  - [2.4 分层模型](#24-分层模型)
- [3.异常如何处理](#3异常如何处理)
  - [3.1 通用处理模型](#31-通用处理模型)
  - [3.2 异常处理本质](#32-异常处理本质)
  - [3.3 底层的机制](#33-底层的机制)
  - [3.4 各语言实现](#34-各语言实现)
  - [3.5 异常匹配规则](#35-异常匹配规则)
  - [3.6 跨线程异常传递](#36-跨线程异常传递)
  - [3.7 各语言处理差异](#37-各语言处理差异)
- [4.异常整体框架](#4异常整体框架)
  - [4.1 Java异常体系](#41-java异常体系)
  - [4.2 C++异常体系](#42-c异常体系)
  - [4.3 设计哲学对比](#43-设计哲学对比)
- [5.异常监听处理](#5异常监听处理)
  - [5.1 各语言异常监听](#51-各语言异常监听)
- [6.异常捕获原理](#6异常捕获原理)
  - [6.1 两大底层实现](#61-两大底层实现)
  - [6.2 各语言捕获对比](#62-各语言捕获对比)
- [8.各语言异常](#8各语言异常)
  - [8.1 Java异常](#81-java异常)
  - [8.2 C++异常](#82-c异常)
...
**所有异常，无论哪种语言，最终都归结为两种触发源：硬件触发 和 软件触发。** 异常的引起归根结底：
...
软件异常是程序**主动**检测到错误条件后，通过语言机制触发的。 throw / raise 的底层实现
...
**核心原理就一句话**：**沿栈逆向查找 + 逐帧清理 + 跳转恢复。** 所有语言的差异只是这三步的具体实现方式不同。
...
| 不要做的事 | ❌ `catch(Throwable t) { /* ignore */ }` ❌ 在 finally 里抛异常覆盖原异常 ❌ 在线程的 run 里漏 try-catch |
| 跨语言迁移 | C/Go/Rust 项目接触 Java 时，记住"异常 = 隐式 return"，别再到处嵌 try-catch |
| 调试技巧 | OOM/StackOverflow 时优先看 jstack + jmap，而不是盲目加内存 |
