...
**场景设定**：你写了一个**订单处理后台服务**，有一个专门的"订单处理线程"在后台默默工作，从队列取订单、扣库存、调用支付。代码看起来无懈可击：
...
**这段代码有个致命问题——你可能根本意识不到**：

某天凌晨 3 点，有一个订单的 `order.userId` 是 `null`（数据库脏数据），`chargePayment` 内部 `user.getCard()` 抛出了 NPE。**这个异常去了哪里？**
...
第二天早上，你的老板冲过来问："为什么昨晚 3 点之后 8 小时的订单一个都没处理？"——你打开日志，干干净净。**这就是线程异常设计中最痛的一类 bug**。
