...
> 🔗 **延伸阅读**：← [3.11 异步和同步的设计](./3.11.异步和同步的设计.md) · ← [3.12 单线程模型的思想](./3.12.单线程模型的思想.md) · → [3.14 Actor 与 CSP 并发模型](./3.14.Actor与CSP并发模型.md) · → [3.18 结构化并发设计思想](./3.18.结构化并发设计思想.md) · → [2.4 函数调用栈与栈帧设计](../02.第2卷运行模型/2.4.函数调用栈与栈帧设计.md)
...
**为什么 Go 能做到，C 做不到？**（详见 [2.4 函数调用栈与栈帧设计](../02.第2卷运行模型/2.4.函数调用栈与栈帧设计.md)）
...
- 第 2 卷视角：[2.4 函数调用栈与栈帧设计](../02.第2卷运行模型/2.4.函数调用栈与栈帧设计.md)（理解协程栈的物理基础）
- 经典文献：
