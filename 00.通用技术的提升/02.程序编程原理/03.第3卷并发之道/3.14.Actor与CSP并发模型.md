...
> 🔗 **延伸阅读**：← [3.13 协程核心设计思想](./3.13.协程核心设计思想.md) · ← [3.9 锁核心设计和思想](./3.9.锁核心设计和思想.md) · → [3.15 线程池的设计思想](./3.15.线程池的设计思想.md) · → [3.18 结构化并发设计思想](./3.18.结构化并发设计思想.md) · → [5.x 消息机制设计思想](../05.第5卷交互与系统/)
...
#### 目录介绍
- [00.真实事故引入](#00真实事故引入)
- [01.共享内存为什么走到了尽头](#01共享内存为什么走到了尽头)
- [02.两位巨人的 50 年：Actor 与 CSP 的演进](#02两位巨人的-50-年actor-与-csp-的演进)
- [03.Actor 模型：从公理到工业化](#03actor-模型从公理到工业化)
- [04.CSP 模型：会合即同步的反直觉之美](#04csp-模型会合即同步的反直觉之美)
- [05.六种语言的横向对比](#05六种语言的横向对比)
- [06.两大模型的物理实现](#06两大模型的物理实现)
...
**这就是 §0.5 第二题的部分答案**——BEAM 是为 Actor 量身打造的 VM，进程切换、消息传递、调度都做了极致优化。
...
**§0.5 第三题的答案**：WhatsApp 的成功不只是"语言选得好"——是**OTP 的监督树重新定义了"可靠系统"的构建方式**。即使代码有 bug、即使硬件偶发故障，监督树都能在毫秒级恢复。

**对比 Java**：JVM 一个线程 OOM → 整个进程崩；Erlang 一个 Actor OOM → 只死自己，监督者立即重启。**这是 quality of resilience 的代际差异**。

### 3.5 Actor 的进一步演化：分布式透明
...
- 第 5 卷视角：[5.x 消息机制设计思想](../05.第5卷交互与系统/)（事件循环 + 消息队列的实战）
- 经典文献：
