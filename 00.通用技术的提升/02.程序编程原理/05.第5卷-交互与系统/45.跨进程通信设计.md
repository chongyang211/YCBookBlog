...
> 🔗 **延伸阅读**：← [44.消息机制设计思想](./44.消息机制设计思想.md) · ← [40.窗口核心设计思想](./40.窗口核心设计思想.md) · ← [03.第3卷-并发之道](../03.第3卷-并发之道/)
...
#### 目录介绍
- [00.从一次"App 启动 8 秒"事故说起](#00从一次app-启动-8-秒事故说起)
...
- [01.为什么需要 IPC：进程隔离的代价](#01为什么需要-ipc进程隔离的代价)
...
- [03.管道：最古老的 IPC](#03管道最古老的-ipc)
  ...
  - [3.3 管道的内核实现：环形缓冲区](#33-管道的内核实现环形缓冲区)
  - [3.4 管道的硬伤](#34-管道的硬伤)
...
- [05.Socket：万能但偏重的 IPC](#05socket万能但偏重的-ipc)
  - [5.1 Unix Domain Socket：本机也走 socket](#51-unix-domain-socket本机也走-socket)
  - [5.2 网络 Socket：跨主机的代价](#52-网络-socket跨主机的代价)
  - [5.3 Socket 的拷贝路径](#53-socket-的拷贝路径)
- [06.Binder：Android 的天才设计](#06binderandroid-的天才设计)
  - [6.1 为什么 Android 不用现成的 IPC](#61-为什么-android-不用现成的-ipc)
  - [6.2 一次拷贝的秘密](#62-一次拷贝的秘密)
  - [6.3 Binder 驱动 + Service Manager + Proxy/Stub](#63-binder-驱动--service-manager--proxystub)
  - [6.4 AIDL：让 IPC 像本地调用](#64-aidl让-ipc-像本地调用)
  - [6.5 Binder 的边界：1MB 限制的来历](#65-binder-的边界1mb-限制的来历)
- [07.Mach Port / XPC：苹果的优雅方案](#07mach-port--xpc苹果的优雅方案)
  - [7.1 Mach Port：内核对象 + 端口语义](#71-mach-port内核对象--端口语义)
  - [7.2 XPC：进程沙盒的协作之道](#72-xpc进程沙盒的协作之道)
- [08.信号 / 消息队列 / 信号量](#08信号--消息队列--信号量)
  - [8.1 信号：异步通知的最小单位](#81-信号异步通知的最小单位)
  - [8.2 System V / POSIX 消息队列](#82-system-v--posix-消息队列)
  - [8.3 信号量：同步而非通信](#83-信号量同步而非通信)
- [09.分布式 IPC：从单机走向集群](#09分布式-ipc从单机走向集群)
  - [9.1 RPC 的本质：让远程调用看起来像本地](#91-rpc-的本质让远程调用看起来像本地)
  - [9.2 序列化的抉择：JSON / XML / Protobuf](#92-序列化的抉择json--xml--protobuf)
...
- [10.IPC 选型决策与设计哲学](#10ipc-选型决策与设计哲学)
  ...
  - [10.4 一句话总结：IPC 的设计哲学](#104-一句话总结ipc-的设计哲学)

---

## 00.从一次"App 启动 8 秒"事故说起
...
### 5.1 Unix Domain Socket：本机也走 socket
...
## 06.Binder：Android 的天才设计

### 6.1 为什么 Android 不用现成的 IPC
...
### 6.3 Binder 驱动 + Service Manager + Proxy/Stub
