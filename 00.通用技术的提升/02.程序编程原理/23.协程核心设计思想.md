...
这是无栈协程的核心黑魔法——**CPS (Continuation-Passing Style) 变换**。

**先讲清楚 Continuation 是什么**

Continuation 直译为"续延"，指的是**"程序后续要做的所有事"**——一段被打包成一等公民对象的"未来"。

```text
普通函数视角：
    foo() 调完返回值，控制权流回调用方  —— 隐式的"调用方就是 continuation"

CPS 视角：
    foo(x, k) 不"返回"，而是把结果传给 k(result)
    k 就是 "foo 完成后程序要做什么" 的对象化表达
```

**直观看个例子**：

```javascript
// 普通风格（直接风格 Direct Style）
const a = readA();
const b = readB();
return a + b;

// CPS 风格 —— 把"后续"显式作为最后一个参数
readA((a) =>
    readB((b) =>
        callback(a + b)));
```

CPS 风格其实就是**回调地狱的形式化定义**。任何看上去线性的代码，都能机械地翻译成嵌套回调。这个"机械翻译"过程，**就是编译器在帮你做的事**。
...
```
方案A：让程序员手写状态机
    → 早期 Boost.Asio 的协程库就是这风格
    → 噩梦级心智负担、代码可读性极低

方案B：让程序员手写 CPS（嵌套回调）
    → 2010s JavaScript 的回调地狱
    → 代码缩进逐层加深、错误处理灾难

方案C（现代选择）：让程序员写线性代码、编译器做 CPS 变换
    → Kotlin / C# / Rust / C++20 / Swift 都走这条路
    → 程序员心智 = 同步、性能 = 异步、零运行时代价
```

**这是工程史上"语言+编译器"对"程序员心智"的一次彻底解放。**
