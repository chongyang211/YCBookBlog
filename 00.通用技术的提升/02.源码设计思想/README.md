...
**根因**：两个非常接近的数相减，高位相消，结果只剩低位的舍入噪声。有效精度从 15 位暴跌到 1-2 位。

---

### 六、各语言的差异处理

虽然底层都是 IEEE 754，但各语言在**语义封装**上有设计差异：

| 特性 | C/C++ | Java | JavaScript | Python |
|------|-------|------|------------|--------|
| 默认类型 | `double` (64位) | `double` (64位) | **只有** `number` (64位 double) | `float` (64位) |
| 32位浮点 | `float` | `float` | 无（TypedArray 除外） | 无（numpy 有） |
| NaN 检测 | `std::isnan()` | `Double.isNaN()` | `Number.isNaN()` | `math.isnan()` |
| 严格模式 | 平台相关，可能用80位扩展精度 | **严格 IEEE 754**，`strictfp` | 严格 IEEE 754 | 严格 IEEE 754 |
| 整数精度 | 整数和浮点分离 | 整数和浮点分离 | **大整数用 double**→超过 2⁵³ 丢精度 | 整数无限精度 |
| 十进制浮点 | 无（需库） | `BigDecimal` | 无（需库） | `decimal.Decimal` |
...
当精确结果无法用浮点数表示时，必须**舍入**。IEEE 754 定义了 4 种模式：
...
**为什么默认是"就近舍入到偶数"而不是"四舍五入"？**

四舍五入在恰好是 0.5 时总是向上，造成系统性偏差（银行家问题）。舍入到偶数在 0.5 时交替向上/向下，统计上无偏。
