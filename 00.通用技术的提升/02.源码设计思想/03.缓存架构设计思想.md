...
graph LR
    subgraph "写入策略对比"
        A[Write-Through] --> A1[同步写入缓存和数据源]
        A --> A2[保证强一致性]
        A --> A3[写入延迟较高]
        
        B[Write-Behind] --> B1[异步写入数据源]
        B --> B2[写入性能高]
        B --> B3[可能数据丢失]
        
        C[Write-Around] --> C1[直接写入数据源]
        C --> C2[避免缓存污染]
        C --> C3[读取可能未命中]
    end
...
graph TD
    subgraph "一致性实现方案"
        A[Cache-Aside] --> A1[应用管理缓存]
        A --> A2[更新数据库后删除缓存]
        A --> A3[简单但可能不一致]
        
        B[Write-Through] --> B1[同时更新缓存和数据库]
        B --> B2[保证强一致性]
        B --> B3[写入性能较差]
        
        C[Write-Behind] --> C1[先更新缓存]
        C --> C2[异步更新数据库]
        C --> C3[性能好但可能丢数据]
        
        D[Refresh-Ahead] --> D1[预测性刷新]
        D --> D2[在过期前更新]
        D --> D3[避免缓存失效]
    end
