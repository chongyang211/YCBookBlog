...
| 场景 | 语言/系统 | 说明 |
|------|----------|------|
| `fork()` 系统调用 | Linux内核 | 子进程共享父进程内存页，写入时才复制页面 |
| `std::string`（旧版） | C++ (GCC < 5) | libstdc++曾使用CoW实现string，C++11后因多线程安全问题废弃 |
| `String` 的 `substring` | Java (JDK < 7u6) | substring共享底层char[]，JDK 7u6后改为独立拷贝 |
| Swift值类型 | Swift | Array、Dictionary等值类型底层使用CoW优化 |
| `Rc<T>` / `Arc<T>` | Rust | 引用计数智能指针，clone时增加计数，修改时deep copy |

CoW的局限性：引用计数本身有开销，多线程下需要原子操作保护计数器。当修改操作频繁时，CoW反而比直接深拷贝更慢。
...
不同编程语言对数据拷贝有着截然不同的设计哲学：

| 语言 | 赋值行为 | 深拷贝方式 | 设计哲学 |
|------|---------|-----------|---------|
| **Java** | 基本类型值拷贝，引用类型拷贝引用 | clone()、序列化、手动构造 | 一切皆对象引用 |
| **C++** | 默认值语义（调用拷贝构造函数） | 自定义拷贝构造函数 | 值语义优先 |
| **Python** | 赋值只拷贝引用 | `copy.deepcopy()` | 一切皆引用 |
| **JavaScript** | 基本类型值拷贝，对象拷贝引用 | `structuredClone()`、JSON序列化 | 原型链继承 |
| **Rust** | 默认移动语义(`move`) | 实现`Clone` trait | 所有权系统 |
| **Go** | 值类型深拷贝，引用类型浅拷贝 | 手动遍历或反射 | 简单直接 |
| **Swift** | 值类型深拷贝(CoW优化)，引用类型浅拷贝 | `NSCopying`协议 | 值类型优先 |

**Rust的独特设计**：Rust通过所有权系统从根本上重新定义了"拷贝"的语义。默认赋值是`move`（所有权转移），原变量不再有效。只有实现了`Copy` trait的类型（如基本类型）才执行位拷贝。需要显式调用`.clone()`才会深拷贝。这种设计在编译期就避免了悬垂指针和双重释放问题。
