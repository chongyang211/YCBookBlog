...
字符串作为编程语言中最基础且最重要的数据类型，其设计理念直接影响着程序的性能、安全性和可维护性。
...
**第二阶段：面向对象封装（C++/Java早期）**

面向对象语言引入了字符串类，封装了内存管理和基本操作：
...
**1. 字符串重复性高**
在实际应用中，相同的字符串字面量经常出现多次，如配置项、错误消息、SQL语句等。

**2. 字符串不可变性**
由于字符串不可变，相同内容的字符串可以安全地共享同一个对象。

**3. 内存使用优化**
通过共享相同的字符串对象，可以显著减少内存使用。
...
public class LeakProofStringPool {
    ...
    public String intern(String str) {
        ...
        return pool.compute(str, (key, existingRef) -> {
            if (existingRef != null) {
                String existing = existingRef.get();
                if (existing != null) {
                    return existingRef; // 返回现有引用
                }
            }
            // 创建新的弱引用
            return new WeakReference<>(new String(str));
        }).get();
    }
    
    private void cleanup() {
        int removedCount = 0;
        Iterator<Map.Entry<String, WeakReference<String>>> iterator = 
        ...
        if (removedCount > 0) {
            System.out.println("清理了 " + removedCount + " 个过期字符串引用");
        }
    }
...
public class ConstructorCreation {
    ...
    public static void performanceComparison() {
        String template = "Template String for Performance Test";
        int iterations = 100000;
        
        // 测试字面量赋值
        long start1 = System.nanoTime();
        for (int i = 0; i < iterations; i++) {
            String s = template; // 直接引用，无额外开销
        }
        ...
        for (int i = 0; i < iterations; i++) {
            String s = new String(template); // 每次创建新对象
        }
        long end2 = System.nanoTime();
        
        System.out.println("字面量赋值时间: " + (end1 - start1) + "ns");
        System.out.println("构造函数创建时间: " + (end2 - start2) + "ns");
        System.out.println("性能差异: " + ((end2 - start2) / (double)(end1 - start1)) + "倍");
    }
}
```
