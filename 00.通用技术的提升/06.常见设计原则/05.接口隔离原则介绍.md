...
- [02.学习接口隔离目标](#02学习接口隔离目标)
- [03.理解接口隔离原则](#03理解接口隔离原则)
- [04.接口隔离原则思想](#04接口隔离原则思想)
- [05.一组API接口集合案例](#05一组api接口集合案例)
- [06.单个API接口或函数](#06单个api接口或函数)
- [07.总结接口隔离原则](#07总结接口隔离原则)
- [08.思考一道课后题](#08思考一道课后题)
- [09.接口隔离原则总结](#09接口隔离原则总结)
...
### 11.1 ISP 的本质：角色接口 vs 头接口

> **疑惑**：接口拆得越小越好吗？拆到什么程度？

> **答疑**：ISP 的核心不是"越小越好"，而是从**客户端角色**的角度来设计接口。Martin Fowler 将接口分为两类：

```
头接口（Header Interface）                角色接口（Role Interface）
┌────────────────────────┐     ┌─────────┐  ┌──────────┐  ┌──────────┐
│     IAnimal            │     │ IFlyable │  │ ISwimmable│  │ IRunnable │
│  + eat()               │     │ + fly()  │  │ + swim() │  │ + run()  │
│  + sleep()             │  →  └─────────┘  └──────────┘  └──────────┘
│  + fly()               │
│  + swim()              │     class Eagle implements IFlyable, IRunnable
│  + run()               │     class Fish implements ISwimmable
└────────────────────────┘     class Duck implements IFlyable, ISwimmable, IRunnable
```

**角色接口**才是ISP推荐的方式——每个接口代表一个"角色"或"能力"，类根据自己具备的能力来实现对应的接口。

### 11.2 多语言实现 ISP

**Python 实现：多媒体播放器的接口隔离**
...
class MediaPlayer(ABC):
    @abstractmethod
    def play_audio(self, file: str): pass
    @abstractmethod
    def play_video(self, file: str): pass
    @abstractmethod
    def stream_online(self, url: str): pass
    @abstractmethod
...
class SimpleAudioPlayer(MediaPlayer):
    ...
    def play_video(self, file: str):
        raise NotImplementedError("不支持视频")  # 被迫实现！
    def stream_online(self, url: str):
        raise NotImplementedError("不支持在线流")  # 被迫实现！
    def record(self):
        raise NotImplementedError("不支持录制")  # 被迫实现！
...
# 每个类只实现自己需要的接口
class SimpleAudioPlayer(AudioPlayable):
    def play_audio(self, file: str):
        print(f"播放音频: {file}")

class FullMediaPlayer(AudioPlayable, VideoPlayable, Streamable, Recordable):
    def play_audio(self, file: str):
        print(f"播放音频: {file}")
    def play_video(self, file: str):
        print(f"播放视频: {file}")
...
> **重要洞察**：Go 语言的接口设计哲学就是 ISP 的最佳实践——小接口、隐式实现、按需组合。Go 标准库中最大的接口也不过3-4个方法。

### 11.3 ISP 在 API 设计中的应用

ISP 不仅适用于面向对象的接口，也适用于 REST API 和微服务设计：

```
❌ 违反ISP的API设计：一个大而全的端点
...
结果：客户端只请求需要的数据，减少网络传输，降低耦合
（这也是 GraphQL 受欢迎的原因之一——客户端决定查什么）
```
