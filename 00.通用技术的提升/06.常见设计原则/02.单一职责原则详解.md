...
#### 目录介绍
- [01.问题思考分析](#01问题思考分析)
- [02.单一职责原则介绍](#02单一职责原则介绍)
- [03.如何理解单一指责](#03如何理解单一指责)
- [04.用例子理解单一职责](#04用例子理解单一职责)
- [05.为何遵守单一原则](#05为何遵守单一原则)
- [06.方法层面单一职责](#06方法层面单一职责)
- [07.接口层面单一职责](#07接口层面单一职责)
- [08.类层面单一职责](#08类层面单一职责)
- [09.单一职责判断模糊](#09单一职责判断模糊)
- [10.单一职责判断原则](#10单一职责判断原则)
- [11.最后总结一下](#11最后总结一下)
- [12.更多内容推荐](#12更多内容推荐)
...
### 13.1 SRP 的数学本质：内聚度与耦合度

> **疑惑**：单一职责到底"单一"到什么程度？有没有量化标准？

> **答疑**：软件工程中有两个度量指标可以辅助判断——

**内聚度（Cohesion）**：衡量一个模块内部各元素之间关联程度的指标。
...
| 内聚类型 | 描述 | 是否满足SRP |
|---------|------|-----------|
| 偶然内聚 | 模块内元素之间毫无关系 | 严重违反 |
| 逻辑内聚 | 模块内执行逻辑上相似的功能（如"处理所有输入"） | 通常违反 |
| 时间内聚 | 模块内元素因在同一时间执行而聚合（如"初始化模块"） | 可能违反 |
| 功能内聚 | 模块内所有元素共同完成一个且仅一个功能 | **满足** |

> **论证**：SRP 追求的就是**功能内聚**——模块内的所有代码都服务于同一个职责。

**耦合度（Coupling）**：衡量模块之间依赖程度的指标。SRP做得好，自然耦合度就低。

### 13.2 "变化原因"的深层含义

Robert C. Martin 对SRP的精确定义是：

> "A class should have only one reason to change."（一个类应该只有一个引起它变化的原因。）
...
```
                        ...
                        │  Employee  │  ← 一个类
                        ├────────────┤
                        │ calcPay()  │  → 变化原因1: CFO/财务部门
                        │ reportHrs()│  → 变化原因2: COO/运营部门  
                        │ save()     │  → 变化原因3: CTO/技术部门
                        └────────────┘
                              ↓
                        违反了 SRP！
         ...
         │  calcPay()   │ reportHrs()  │   save()     │
         │ (财务职责)    │ (运营职责)    │ (持久化职责)  │
         └──────────────┴──────────────┴──────────────┘
```

> **关键洞察**：不是看"功能数量"，而是看"变化来源数量"。如果一个类只有一个角色会要求它改变，那即使它有多个方法，也可能满足SRP。

### 13.3 多语言视角的 SRP 实践

**Python 实现：日志系统的SRP拆分**

```python
# ❌ 违反SRP：一个类同时负责日志的格式化、过滤和输出
class Logger:
    ...
    def format(self, message):
        ...
        return f"[{datetime.now()}] {self.level}: {message}"
...
class LogWriter:
    """职责：将日志写入目标"""
    def write(self, destination: str, message: str):
...
class Logger:
    """组合器：协调各个单一职责的组件"""
    def __init__(self, formatter: LogFormatter, log_filter: LogFilter, writer: LogWriter):
        self._formatter = formatter
        self._filter = log_filter
        self._writer = writer
    
    def log(self, level: str, message: str, destination: str = "console"):
        if self._filter.should_log(level):
            formatted = self._formatter.format(level, message)
            self._writer.write(destination, formatted)
```
...
// ❌ 违反SRP：用户服务既管验证、又管持久化、又管通知
// type UserService struct { ... }
// func (u *UserService) Register(name, email, pwd string) { 校验+存库+发邮件 }
...
func (v *UserValidator) Validate(name, email, pwd string) error {
    if len(name) < 2 {
        return fmt.Errorf("用户名过短")
    }
...
func (r *UserRepository) Save(name, email, pwdHash string) (int64, error) {
    // 存储到数据库
...
func (n *UserNotifier) SendWelcome(email string) error {
...
func (s *UserService) Register(name, email, pwd string) error {
    if err := s.validator.Validate(name, email, pwd); err != nil {
        return err
    }
    _, err := s.repo.Save(name, email, hashPassword(pwd))
    if err != nil {
        return err
    }
...
### 13.4 SRP 的"度"：何时拆分，何时合并

> **疑惑**：是不是把每个方法都拆成一个类就最好了？

> **答疑**：绝对不是！过度拆分会导致"类爆炸"问题——

```
过度拆分的信号：
├── UserNameValidator.java          ← 太细了
├── UserEmailValidator.java         ← 太细了
├── UserPasswordValidator.java      ← 太细了
├── UserNameFormatter.java          ← 太细了
└── UserEmailNormalizer.java        ← 太细了

合理的拆分：
├── UserValidator.java              ← 验证职责
├── UserFormatter.java              ← 格式化职责
└── UserRepository.java             ← 持久化职责
```

**判断拆分粒度的三个准则**：

1. **变化频率**：如果两个功能总是一起变化，放在一起；如果独立变化，拆开
2. **复用需求**：如果某个功能需要在其他地方复用，拆出来
3. **团队边界**：如果不同的人/团队负责不同的功能，按团队边界拆分
