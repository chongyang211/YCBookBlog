...
- [02.学习依赖倒置目标](#02学习依赖倒置目标)
- [03.理解依赖倒置原则](#03理解依赖倒置原则)
- [04.依赖倒置原则思想](#04依赖倒置原则思想)
- [05.多数据库操作案例](#05多数据库操作案例)
- [06.用户购买家电案例](#06用户购买家电案例)
- [07.发送消息的案例](#07发送消息的案例)
- [08.依赖倒置VS依赖注入](#08依赖倒置vs依赖注入)
- [09.依赖倒置的思考](#09依赖倒置的思考)
- [10.依赖倒置原则总结](#10依赖倒置原则总结)
...
> **论证**：关键的"倒置"发生在——抽象接口由高层模块定义（而非低层模块定义）。低层模块"反过来"依赖高层定义的抽象，这就是"倒置"。

### 12.2 DIP 的三个层次

```
层次1：代码层面 —— 变量声明用接口类型而非具体类型
    List<String> list = new ArrayList<>();  // ✅ 依赖 List 接口
    ArrayList<String> list = new ArrayList<>();  // ❌ 依赖具体实现

层次2：模块层面 —— 模块间通过接口通信
    Service层 → Repository接口 ← MySQL实现
                               ← Redis实现

层次3：架构层面 —— 整洁架构 / 六边形架构
    ┌──────────────────────────────────┐
    │         Frameworks & Drivers     │ ← 最外层：数据库、Web框架
    │  ┌──────────────────────────┐    │
    │  │    Interface Adapters    │    │ ← 适配层：Controller、Gateway
    │  │  ┌──────────────────┐   │    │
    │  │  │   Use Cases      │   │    │ ← 用例层：业务规则
    │  │  │  ┌──────────┐   │   │    │
    │  │  │  │ Entities  │   │   │    │ ← 核心层：领域实体
...
class FileCache(CacheStore):
    ...
    def get(self, key: str) -> Optional[Any]:
        try:
            with open(f"{self._dir}/{key}.json", 'r') as f:
                return json.load(f)
    ...
    def set(self, key: str, value: Any, ttl: int = 0) -> None:
        ...
        with open(f"{self._dir}/{key}.json", 'w') as f:
            json.dump(value, f)
    
    def delete(self, key: str) -> None:
        import os
        try:
            os.remove(f"{self._dir}/{key}.json")
        except FileNotFoundError:
            pass

# 高层模块：只依赖抽象
class UserService:
    def __init__(self, cache: CacheStore):  # 依赖注入
        self._cache = cache
...
# 使用：自由切换缓存实现
service_dev = UserService(MemoryCache())         # 开发环境用内存
service_prod = UserService(FileCache("/var/cache")) # 生产环境用文件
```
...
func main() {
    // 生产环境
    prodHandler := NewUserHandler(&MySQLUserRepo{db: db})
    
    // 测试环境
    testRepo := &InMemoryUserRepo{users: map[string]*User{
        "1": {ID: "1", Name: "测试用户"},
    }}
    testHandler := NewUserHandler(testRepo)
}
```

### 12.4 IoC 容器：DIP 的工业实现

> **疑惑**：每次都手动 new 并传入依赖，项目大了怎么办？

> **答疑**：这就是 IoC（控制反转）容器的用武之地。

```
手动依赖注入：
  repo = MySQLUserRepo(db)
  service = UserService(repo)
  handler = UserHandler(service)
  ↑ 需要手动管理所有对象的创建和组装

IoC 容器自动管理：
  container.register(UserRepository, MySQLUserRepo)
  container.register(UserService)
  container.register(UserHandler)
  handler = container.resolve(UserHandler)  // 自动注入所有依赖
```
...
### 12.5 DIP 的边界：何时不需要倒置

| 场景 | 是否需要DIP | 原因 |
|------|-----------|------|
| 稳定的标准库（如 `String`、`List`） | 不需要 | 极少变化，无需抽象 |
| 只有一种实现且未来不会变 | 谨慎使用 | YAGNI原则 |
| 原型/MVP阶段 | 不需要 | 先跑通再优化 |
| 外部依赖（数据库、网络、文件系统） | **需要** | 易变，且测试需要mock |
| 多团队协作的模块边界 | **需要** | 面向接口定义契约 |
