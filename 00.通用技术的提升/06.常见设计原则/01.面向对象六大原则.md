# 01.面向对象六大原则
#### 目录介绍
- [00.面向对象六大原则](#00面向对象六大原则)
- [01.代码单一职责原则](#01代码单一职责原则)
- [02.代码开放封闭原则](#02代码开放封闭原则)
- [03.代码里氏替换原则](#03代码里氏替换原则)
- [04.代码依赖倒置原则](#04代码依赖倒置原则)
- [05.代码接口隔离原则](#05代码接口隔离原则)
- [06.代码迪米特原则](#06代码迪米特原则)
...
### 7.1 从混沌到秩序：设计原则的诞生背景

> **疑惑**：为什么需要设计原则？写代码能跑不就行了？

> **答疑**：20世纪60-70年代，随着软件规模急剧膨胀，出现了"软件危机"（Software Crisis）。项目延期、预算超支、Bug难以修复成为常态。1968年NATO软件工程会议首次正式提出"软件工程"概念，标志着人们开始系统性地思考如何写出"好"代码。

> **论证**：从结构化编程（Dijkstra, 1968）→ 面向对象编程（Simula, 1967; Smalltalk, 1972）→ 设计原则（1980s-2000s），软件工程经历了一条从"关注计算"到"关注结构"再到"关注变化"的演进路径。

**关键历史节点**：

```
1967  Simula 诞生 ——— 面向对象思想萌芽（类、继承、多态）
  |
1972  Smalltalk ——— 纯面向对象语言，消息传递机制
  |
1979  Bertrand Meyer ——— 提出"契约式设计"（Design by Contract）
  |
1986  Barbara Liskov ——— 提出里氏替换原则（LSP）
  |
1988  Bertrand Meyer ——— 在《OOSC》中正式提出开闭原则（OCP）
  |
1996  Robert C. Martin ——— 整理提出依赖倒置原则（DIP）
  |
2000  Robert C. Martin ——— 在论文中首次将五大原则合称为 SOLID
  |
2003  《敏捷软件开发：原则、模式与实践》出版 ——— SOLID 广泛传播
  |
2004  Martin Fowler ——— 《重构》第一版广泛流行，设计原则落地实践
```

### 7.2 SOLID 的命名由来

SOLID 是五个设计原则英文首字母的缩写，由 Robert C. Martin（"Uncle Bob"）在2000年左右整合提出：

| 字母 | 原则 | 英文全称 | 提出者 | 年份 |
|------|------|---------|--------|------|
| **S** | 单一职责原则 | Single Responsibility Principle | Robert C. Martin | 2003 |
| **O** | 开闭原则 | Open-Closed Principle | Bertrand Meyer | 1988 |
| **L** | 里氏替换原则 | Liskov Substitution Principle | Barbara Liskov | 1986 |
| **I** | 接口隔离原则 | Interface Segregation Principle | Robert C. Martin | 1996 |
| **D** | 依赖倒置原则 | Dependency Inversion Principle | Robert C. Martin | 1996 |

> **注意**：迪米特法则（Law of Demeter, 1987）虽然不属于SOLID五原则，但同样是面向对象设计中极其重要的原则，通常与SOLID并称为"六大原则"。
...
**疑惑**：这六个原则之间是什么关系？为什么学了一个还不够？

**答疑**：它们形成一条完整的设计逻辑链——

1. **SRP（单一职责）** 是基石：先把类拆"小"，每个类只做一件事
2. **OCP（开闭原则）** 是目标：让系统能够通过扩展而非修改来演进
3. **LSP（里氏替换）** 是手段：保证子类能正确替换父类，这是多态安全性的保障
4. **ISP（接口隔离）** 是约束：保证接口精简，不强迫依赖不需要的方法
5. **DIP（依赖倒置）** 是架构：高层不依赖低层，都依赖抽象
6. **LOD（迪米特法则）** 是纪律：模块间最小知识，降低耦合

**结果**：六个原则共同构成了一套"高内聚、低耦合、易扩展"的设计方法论体系。
...
六大设计原则虽然起源于面向对象语言（Java、C++、Smalltalk），但其核心思想在任何编程范式中都适用。下面用不同语言展示**依赖倒置原则**的核心思想：
...
class NotificationService:
    def __init__(self, sender: MessageSender):
        self._sender = sender    # 依赖注入

    def notify(self, to: str, content: str):
        self._sender.send(to, content)

# 使用：轻松替换发送方式
service = NotificationService(EmailSender())
service.notify("user@example.com", "你好")

service = NotificationService(SmsSender())
service.notify("13800138000", "验证码：1234")
```
...
type NotificationService struct {
    sender MessageSender  // 依赖注入
}

func (n *NotificationService) Notify(to, content string) {
    n.sender.Send(to, content)
}
...
func main() {
    ...
    service = &NotificationService{sender: &SmsSender{}}
    service.Notify("13800138000", "验证码")
}
```
...
class NotificationService {
    ...
    async notify(to: string, content: string): Promise<void> {
...
emailService.notify("user@example.com", "你好");
```

### 9.2 函数式编程中的设计原则

> **疑惑**：我用函数式编程，不用类和继承，设计原则还有用吗？

> **答疑**：完全有用！原则的本质不是"类和继承"，而是"关注点分离"和"依赖管理"。

| SOLID原则 | 函数式编程中的对应 |
|-----------|------------------|
| SRP | 一个函数只做一件事（Unix哲学） |
| OCP | 高阶函数 + 函数组合实现扩展 |
| LSP | 函数签名一致性（相同输入→相同类型输出） |
| ISP | 函数参数最小化，只传必要参数 |
| DIP | 将依赖作为参数传入（依赖注入的函数式版本） |

**函数式风格的依赖注入**（Python）：

```python
# 不依赖具体实现，而是将"发送行为"作为参数传入
def notify(send_fn, to: str, content: str):
    """高层函数依赖于抽象的发送函数，而不是具体类"""
    send_fn(to, content)

# 具体实现是普通函数
def send_email(to: str, content: str):
    print(f"邮件 -> {to}: {content}")

def send_sms(to: str, content: str):
    print(f"短信 -> {to}: {content}")

# 使用
notify(send_email, "user@example.com", "你好")
notify(send_sms, "13800138000", "验证码")
```
...
### 10.1 如何判断代码是否违反了设计原则

| 代码坏味道 | 可能违反的原则 | 改进方向 |
|-----------|-------------|---------|
| 一个类超过500行 | SRP | 拆分为多个职责单一的类 |
| 新增功能需要改多处旧代码 | OCP | 引入抽象层和扩展点 |
| 子类覆写父类方法后行为异常 | LSP | 重新设计继承关系 |
| 实现接口时有大量空方法 | ISP | 将大接口拆分为小接口 |
| 高层类直接 new 低层类 | DIP | 引入接口和依赖注入 |
| 一个类需要了解多个不相关类的细节 | LOD | 引入中介者或封装门面 |

### 10.2 过度设计的信号

> **疑惑**：设计原则是不是用得越多越好？

> **答疑**：绝对不是！过度设计和不设计一样有害。

**过度设计的典型信号**：
1. 一个简单功能拆成了十几个类，看代码需要来回跳转
2. 所有地方都抽了接口，但每个接口只有一个实现
3. 用了很多设计模式，但说不清"为什么要用"
4. 代码"很优雅"但新人完全看不懂
...
```
SRP 要求拆分类    ←→   拆分过多类会增加复杂度
OCP 要求不修改    ←→   有时简单修改比抽象扩展更合理
ISP 要求拆分接口  ←→   拆分过多接口增加维护成本
DIP 要求依赖抽象  ←→   简单场景直接依赖实现更清晰
```

**权衡的核心原则**：

> 设计原则是**指导方针**，不是**法律条文**。在具体场景中，要根据项目规模、团队能力、业务复杂度来灵活取舍。**最好的设计是刚刚好的设计**。



## 11.六大原则的工业实践案例

### 11.1 Spring Framework 中的 SOLID 体现

| 原则 | Spring 中的体现 |
|------|----------------|
| SRP | `@Controller`、`@Service`、`@Repository` 分层，每层职责单一 |
| OCP | `BeanPostProcessor` 提供扩展点，不修改框架核心代码就能扩展功能 |
| LSP | 任何 `ApplicationContext` 子类都可以替换父类使用 |
| ISP | `BeanFactory`（基础容器）vs `ApplicationContext`（增强容器）分层接口 |
| DIP | 核心机制就是依赖注入（`@Autowired`），高层依赖抽象接口 |
| LOD | 通过 IoC 容器管理对象关系，组件之间不直接创建依赖 |

### 11.2 Linux/Unix 中的设计原则

Unix 哲学本身就是设计原则的最佳实践：

```
"Do one thing and do it well"   → SRP 单一职责
"Write programs to work together" → ISP 接口隔离 + LOD 迪米特
"Everything is a file"          → DIP 依赖抽象（统一的文件接口）
管道机制 (ls | grep | sort)      → OCP 开闭原则（通过组合扩展功能）
```
