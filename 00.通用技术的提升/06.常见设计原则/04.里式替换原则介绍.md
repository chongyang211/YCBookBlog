...
- [4.如何理解里式替换原则](#4如何理解里式替换原则)
- [5.支付案例演变过程](#5支付案例演变过程)
- [6.鸟类飞行演变过程](#6鸟类飞行演变过程)
- [7.LSP 的理论基础：契约式设计](#7lsp-的理论基础契约式设计)
- [8.LSP 的四条核心规则](#8lsp-的四条核心规则)
- [9.经典反例：正方形不是长方形](#9经典反例正方形不是长方形)
- [10.类型系统中的 LSP：协变与逆变](#10类型系统中的-lsp协变与逆变)
- [11.LSP 检查清单](#11lsp-检查清单)
- [12.里氏替换优缺点](#12里氏替换优缺点)
- [13.里氏替换与多态的区别](#13里氏替换与多态的区别)
- [14.回到开篇的控件继承案例](#14回到开篇的控件继承案例)
- [15.本篇收获](#15本篇收获)
- [16.思考题](#16思考题)
- [17.课后作业](#17课后作业)
- [18.更多内容推荐](#18更多内容推荐)

里式替换原则（LSP）是面向对象设计的重要原则之一，确保子类可以无缝替换父类而不破坏程序功能。本文由一个"子类静默吞事件"的真实事故切入，展开 LSP 的定义、背景、理解方法与落地规则，再通过支付、鸟类飞行、正方形三个经典案例层层推进，最后回到开篇问题、给出可操作的修复方向。

## 1.工作中的一个真实案例

做过终端 UI 框架的同学一定体会过这个坑：

> 项目里有一个自研的基础控件 `BaseInput`，负责"接受输入 + 校验 + 派发变更事件"。某次业务同学写了一个 `PhoneInput` 继承它，在内部重写了"文本变更"逻辑——遇到非数字就直接把 `text` 置空，并**静默吞掉变更事件**。
>
