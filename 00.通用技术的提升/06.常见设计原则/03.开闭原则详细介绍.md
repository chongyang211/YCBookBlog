# 03.开闭原则详细介绍
#### 目录介绍
- [01.问题思考的分析](#01问题思考的分析)
- [02.如何理解开闭原则](#02如何理解开闭原则)
- [03.开闭原则的背景](#03开闭原则的背景)
- [04.开闭原则比较难学](#04开闭原则比较难学)
- [05.实现开闭原则方式](#05实现开闭原则方式)
- [06.画图形案例分析](#06画图形案例分析)
- [07.银行业务案例分析](#07银行业务案例分析)
- [08.开闭原则优缺点](#08开闭原则优缺点)
- [09.开闭原则的总结](#09开闭原则的总结)
...
> **疑惑**：开闭原则说"对修改关闭"，但实际开发中不可能完全不修改代码啊？

> **答疑**：OCP 的真正含义不是"永远不改代码"，而是**识别变化点，将变化封装在扩展中**。关键在于：哪些代码是稳定的（关闭修改），哪些是易变的（开放扩展）。
...
### 11.2 Bertrand Meyer 的原始定义 vs 现代理解

OCP 最初由 Bertrand Meyer 在1988年《Object-Oriented Software Construction》中提出。但他的定义和今天通用的理解有重要区别：

| 维度 | Meyer 的 OCP（1988） | Martin 的 OCP（2000s） |
|------|---------------------|----------------------|
| 扩展方式 | 通过**继承**实现扩展 | 通过**抽象+多态+组合**实现扩展 |
| 关闭含义 | 发布后的模块不再修改源码 | 核心逻辑不因新增功能而改动 |
| 实现手段 | 继承为主 | 接口、策略模式、插件机制等 |

> **关键演进**：现代OCP更推荐**组合优于继承**。过度使用继承会导致类层次膨胀，应更多地使用接口+组合。
...
class FullReductionDiscount(DiscountStrategy):
    ...
    def calculate(self, price: float) -> float:
        if price >= self._threshold:
            return price - self._reduction
        return price
    
    def name(self) -> str:
        return f"满{self._threshold}减{self._reduction}"

# 核心逻辑：订单计算器（关闭区，不修改）
class OrderCalculator:
    """无论新增多少种折扣策略，这个类都不需要修改"""
    def calculate_total(self, price: float, discount: DiscountStrategy) -> float:
        final_price = discount.calculate(price)
        print(f"原价: {price}, 策略: {discount.name()}, 折后: {final_price}")
        return final_price

# 新增一种折扣？只需新建一个类，不修改任何已有代码！
...
calc.calculate_total(100, NoDiscount())
calc.calculate_total(100, PercentageDiscount(20))
calc.calculate_total(200, FullReductionDiscount(150, 30))
calc.calculate_total(100, VipDiscount())  # 新增的策略无缝接入
```
...
type LogHandler interface {
    Handle(level, message string) error
}

// 实现1：控制台输出
type ConsoleHandler struct{}
func (h *ConsoleHandler) Handle(level, message string) error {
...
func (h *FileHandler) Handle(level, message string) error {
...
func NewLogger(handlers ...LogHandler) *Logger {
...
// 新增：远程日志（只需新增实现，不改Logger）
type RemoteHandler struct {
    Endpoint string
}
func (h *RemoteHandler) Handle(level, message string) error {
    fmt.Printf("发送到远程 %s: [%s] %s\n", h.Endpoint, level, message)
    return nil
}
...
func main() {
    logger := NewLogger(
        &ConsoleHandler{},
        &FileHandler{FilePath: "app.log"},
        &RemoteHandler{Endpoint: "http://log-server.com"},
    )
...
class LengthValidator implements Validator<string> {
    ...
    validate(value: string): ValidationResult {
        ...
        return {
            isValid,
            errors: isValid ? [] : [`长度应在${this.min}-${this.max}之间`]
        };
    }
}
...
// 新增验证器：邮箱格式（只需新增类，不改引擎）
class EmailValidator implements Validator<string> {
    validate(value: string): ValidationResult {
        const isValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value);
        return { isValid, errors: isValid ? [] : ['邮箱格式不正确'] };
    }
}
```
...
│
├── 2. 设计模式
│   ├── 策略模式（Strategy）——— 封装算法变化
│   ├── 模板方法（Template Method）——— 封装流程中的步骤变化
│   ├── 观察者模式（Observer）——— 封装通知机制的变化
│   ├── 装饰器模式（Decorator）——— 动态添加职责
│   └── 工厂模式（Factory）——— 封装对象创建的变化
│
├── 3. 扩展点机制
│   ├── 钩子方法（Hook Method）
│   ├── 回调函数（Callback）
...
| Django | 不修改框架，通过 Middleware 扩展 | `MIDDLEWARE` 配置列表 |
...
> **结论**：优秀的框架设计本身就是OCP的最佳实践——它们提供稳定的核心 + 丰富的扩展点，用户通过扩展而非修改来满足需求。
