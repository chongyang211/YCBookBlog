...
- [02.学习迪米特原则目标](#02学习迪米特原则目标)
- [03.理解迪米特原则](#03理解迪米特原则)
- [04.迪米特原则思想](#04迪米特原则思想)
- [05.迪米特原则案例1](#05迪米特原则案例1)
- [06.迪米特原则案例2](#06迪米特原则案例2)
- [07.迪米特原则思考](#07迪米特原则思考)
- [08.迪米特原则的总结](#08迪米特原则的总结)
...
> **答疑**：对于一个对象 O 中的方法 M，M 只能调用以下对象的方法——
...
❌ 不是朋友：
├── 朋友的朋友（链式调用: a.getB().getC().doSomething()）
├── 方法返回值的内部对象
└── 集合中元素的内部细节
```
...
> **答疑**：这被称为"火车残骸"（Train Wreck）或"链式调用"反模式，严重违反迪米特法则。

```python
# ❌ 火车残骸：调用者了解了太多内部结构
city_name = order.get_customer().get_address().get_city().get_name()

# 问题：
# 1. order 需要知道 Customer 有 Address
# 2. 需要知道 Address 有 City
# 3. 需要知道 City 有 Name
# 4. 任何一层结构变化，调用者都要修改
...
class Order:
    ...
    def get_shipping_city(self) -> str:
        """Order 只暴露业务需要的信息"""
        return self._customer.get_city_name()
...
class Customer:
    ...
    def get_city_name(self) -> str:
        """Customer 只暴露必要的信息"""
        return self._address.get_city_name()
...
func (s *InventoryService) CheckStock(productID string) bool {
    fmt.Printf("检查库存: %s\n", productID)
    return true
}

type PaymentService struct{}
func (s *PaymentService) Charge(userID string, amount float64) bool {
    fmt.Printf("扣费: 用户%s, 金额%.2f\n", userID, amount)
    return true
}

type ShippingService struct{}
func (s *ShippingService) Arrange(orderID, address string) bool {
    fmt.Printf("安排发货: 订单%s -> %s\n", orderID, address)
    return true
}
...
func (f *OrderFacade) PlaceOrder(userID, productID, address string, amount float64) bool {
    // 客户端只需要调用一个方法，不需要知道内部子系统
    ...
    if !f.shipping.Arrange(productID, address) {
        fmt.Println("发货失败")
        return false
    }
    fmt.Println("下单成功！")
...
func main() {
    ...
    facade.PlaceOrder("user001", "prod001", "北京市朝阳区", 99.9)
}
```

**TypeScript 实现：事件总线解耦**
...
class OrderModule {
    ...
    createOrder(orderId: string, amount: number): void {
        ...
        // 不直接调用其他模块，只发事件
        this.bus.emit('order:created', { orderId, amount });
    }
}
...
class InventoryModule {
    constructor(bus: EventBus) {
        ...
        bus.on('order:created', (data) => {
            console.log(`扣减库存: 订单${data.orderId}`);
        });
    }
}
...
class NotificationModule {
    constructor(bus: EventBus) {
        bus.on('order:created', (data) => {
            console.log(`发送通知: 订单${data.orderId}, 金额${data.amount}`);
        });
    }
}

// 各模块只知道 EventBus，不知道其他模块的存在
const bus = new EventBus();
const inventory = new InventoryModule(bus);
const notification = new NotificationModule(bus);
const order = new OrderModule(bus);
order.createOrder("ORD-001", 199.9);
```
...
┌─────────────────────────────┐
│     表示层 (Controller)      │  ← 只和 Service 层对话
└──────────────┬──────────────┘
               │ ↕ 直接朋友
┌──────────────┴──────────────┐
│     业务层 (Service)         │  ← 只和 Repository 层对话
└──────────────┬──────────────┘
               │ ↕ 直接朋友
┌──────────────┴──────────────┐
│     数据层 (Repository)      │  ← 只和数据库对话
└──────────────┬──────────────┘
               │
           [Database]

规则：每一层只和相邻层通信
Controller ❌→ Repository  （跨层调用违反迪米特法则）
...
| 跨层调用 | Controller 直接访问 DAO | 补充 Service 层 |
| 频繁的 getter 链 | 获取对象内部的内部 | "告诉，不要问"原则 |

> **"Tell, Don't Ask"原则**：不要向对象"询问"信息然后自己做决定，而是直接"告诉"对象你想要的结果。这是迪米特法则在方法设计层面的具体体现。
