...
### **2. 使用 `SoundEffect` 播放短音频**

`SoundEffect` 适用于播放短音频（如音效），支持低延迟播放。
...
在 QML 中，播放视频可以通过 `MediaPlayer` 和 `VideoOutput` 组件实现。以下是快速入门 QML 视频播放的指南，包含常用示例。

---

### **1. 使用 `MediaPlayer` 和 `VideoOutput` 播放视频**

`MediaPlayer` 用于控制视频的播放，`VideoOutput` 用于显示视频内容。
...
可以通过 `position` 和 `duration` 属性控制播放进度。
