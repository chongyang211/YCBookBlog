...
`Dialog` 是一个非常灵活的控件，可以满足大多数对话框的需求。

1. 打开和关闭事件：`Dialog` 提供了以下信号来监听打开和关闭事件：**`onOpened`**：当对话框打开时触发。**`onClosed`**：当对话框关闭时触发。
2. 显示和隐藏事件：`Dialog` 继承自 `Popup`，因此可以使用 `Popup` 的信号来监听显示和隐藏事件：**`onVisibleChanged`**：当对话框的可见性发生变化时触发。
3. 对话框状态监听：`Dialog` 的状态可以通过以下属性监听：**`visible`**：对话框是否可见。**`opened`**：对话框是否打开。
4. 完全自定义生命周期：如果需要更复杂的生命周期管理，可以结合 `Component.onCompleted` 和 `Component.onDestruction` 来监听对话框的创建和销毁事件。
