...
- **QDataStream**：二进制序列化
                                    ...
                                    Qt 提供了丰富的网络协议支持类，用于处理 URL、主机信息、网络接口等。以下是 `QUrl`、`QUrlQuery`、`QHostInfo` 和 `QNetworkInterface` 的详细说明和用法：

---

### **1. QUrl/QUrlQuery：URL 解析与构建**
`QUrl` 用于解析和构建 URL，`QUrlQuery` 用于处理 URL 中的查询参数。
...
### **总结**
- **`QUrl` / `QUrlQuery`**：用于解析和构建 URL，处理查询参数。
- **`QHostInfo`**：用于查询主机名和 IP 地址。
- **`QNetworkInterface`**：用于获取网络接口的详细信息。

这些类为 Qt 应用程序提供了强大的网络协议支持。如果还有其他问题，请随时告诉我！
