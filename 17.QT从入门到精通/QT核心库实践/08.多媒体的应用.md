...
#### **(1) 媒体加载**
- 使用 `setMedia()` 方法加载媒体文件或 URL。
- 媒体内容可以是本地文件、网络资源或内存数据。

#### **(2) 解码与播放**
- `QMediaPlayer` 依赖于底层的多媒体框架（如 GStreamer、DirectShow、AVFoundation 等）进行媒体解码。
- 解码后的音频数据通过音频设备输出，视频数据通过 `QVideoWidget` 或自定义渲染器显示。

#### **(3) 状态管理**
- `QMediaPlayer` 维护一个内部状态机，包括以下状态：
    - `StoppedState`：停止状态。
    - `PlayingState`：播放中。
    - `PausedState`：暂停状态。
- 通过 `state()` 方法获取当前状态。
...
#### **(4) 信号与槽**
- `QMediaPlayer` 通过信号与槽机制通知状态变化和事件，例如：
    - `stateChanged()`：播放状态变化时触发。
    - `positionChanged()`：播放位置变化时触发。
    - `durationChanged()`：媒体时长变化时触发。
    - `errorOccurred()`：播放出错时触发。

#### **(5) 资源管理**
- 播放完成后，调用 `stop()` 方法释放资源。
- 使用 `setMedia(nullptr)` 清除当前媒体内容。
