...
`Q_INVOKABLE` 宏，用于标记成员函数为可调用的，可以通过 `QMetaObject::invokeMethod()` 动态调用。
                        ...
                        Qt 提供了一些关键宏和类，用于扩展类的功能，支持元对象系统、信号与槽机制、属性系统等。以下是 `Q_OBJECT`、`QMetaObject`、`Q_PROPERTY` 和 `Q_INVOKABLE` 的详细说明和用法：

---

### **1. Q_OBJECT**
`Q_OBJECT` 是一个宏，用于启用 Qt 的元对象系统功能，包括信号与槽机制、动态属性、对象树管理等。
...
{
    MyClass obj;
    QObject::connect(&obj, &MyClass::mySignal, &obj, &MyClass::mySlot); // 连接信号与槽
    obj.doSomething(); // 触发信号
...
### **2. QMetaObject**
`QMetaObject` 是 Qt 元对象系统的核心类，提供了对类元信息的访问。
