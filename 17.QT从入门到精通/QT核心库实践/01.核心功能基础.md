# 01.核心功能基础
#### 目录介绍
- 1.1 基础类概念
  - 1.1.1 对象模型
  - 1.1.2 元对象系统



## 1.1 基础类概念

Qt Core 是 Qt 框架的基础模块，提供了跨平台的 C++ 核心功能，不依赖于 GUI 组件。

### 1.1.1 对象模型


### 1.1.2 元对象系统



## 一、核心功能与基础类

### 1. 对象模型与元对象系统
- **QObject**：所有 Qt 对象的基类，提供信号槽机制、对象树管理、事件处理等核心功能
- **QMetaObject**：提供运行时类型信息（RTTI）和反射能力
- **Q_GADGET**：为非 QObject 类添加元对象能力
- **Q_PROPERTY**：声明属性系统
- **Q_INVOKABLE**：使方法可通过元对象系统调用


### **1. 元对象系统（Meta-Object System）**
- **功能**：提供运行时类型信息（RTTI）、信号与槽机制、动态属性系统等。
- **核心类**：
    - `QObject`：所有 Qt 对象的基类。
    - `QMetaObject`：存储类的元信息（如类名、信号、槽、属性等）。
    - `QMetaMethod`、`QMetaProperty`：用于访问方法和属性信息。

### **4. 对象模型（Object Model）**
- **功能**：提供对象树管理、父子关系、对象生命周期管理。
- **核心类**：
    - `QObject`：支持对象树和父子关系。
    - `QObject::parent()`、`QObject::children()`：访问父对象和子对象。





            Qt 的对象模型和元对象系统是 Qt 框架的核心特性之一，它们为 Qt 提供了强大的功能，如信号与槽机制、动态属性、反射等。以下是关于 Qt 对象模型和元对象系统的详细说明：

---

### **1. Qt 对象模型**
Qt 对象模型是 Qt 框架的基础，它扩展了 C++ 的对象模型，提供了以下特性：

#### **1.1 对象树与父子关系**
- Qt 对象可以组织成树形结构，每个对象可以有一个父对象和多个子对象。
- 当父对象被销毁时，其所有子对象也会自动销毁。
- 通过 `QObject::setParent()` 设置父对象，或通过 `QObject::children()` 获取子对象。

#### **1.2 对象名称与查找**
- 每个 `QObject` 对象可以有一个唯一的名称（`objectName`），用于在对象树中查找。
- 使用 `QObject::findChild()` 或 `QObject::findChildren()` 查找对象。

#### **1.3 动态属性**
- Qt 对象可以动态添加属性，这些属性可以在运行时查询和修改。
- 使用 `QObject::setProperty()` 设置属性，`QObject::property()` 获取属性。

#### **1.4 信号与槽**
- Qt 的信号与槽机制是对象模型的核心特性，用于对象之间的通信。
- 信号是事件的通知，槽是事件的响应函数。
- 使用 `QObject::connect()` 连接信号与槽。

---

### **2. 元对象系统**
元对象系统（Meta-Object System）是 Qt 实现对象模型的基础，它提供了以下功能：

#### **2.1 元对象（Meta-Object）**
- 每个 `QObject` 派生类都有一个对应的元对象，用于描述类的元信息（如类名、信号、槽、属性等）。
- 元对象由 `moc`（Meta-Object Compiler）生成，存储在 `moc_*.cpp` 文件中。

#### **2.2 元对象编译器（moc）**
- `moc` 是 Qt 的工具，用于处理 `Q_OBJECT` 宏，生成元对象代码。
- 在编译 Qt 项目时，`moc` 会自动处理包含 `Q_OBJECT` 宏的头文件。

#### **2.3 反射（Reflection）**
- 元对象系统支持反射，可以在运行时查询对象的类信息、信号、槽、属性等。
- 使用 `QMetaObject` 类访问元对象信息。

#### **2.4 动态调用**
- 通过元对象系统，可以在运行时动态调用对象的信号、槽和属性。
- 使用 `QMetaObject::invokeMethod()` 动态调用槽函数。

---

### **3. 关键宏与类**
#### **3.1 `Q_OBJECT` 宏**
- 所有使用元对象系统的类必须包含 `Q_OBJECT` 宏。
- 该宏会触发 `moc` 生成元对象代码。

#### **3.2 `QMetaObject` 类**
- 提供对元对象信息的访问，如类名、信号、槽、属性等。

#### **3.3 `Q_PROPERTY` 宏**
- 用于声明属性，支持读写、通知信号等特性。

#### **3.4 `Q_INVOKABLE` 宏**
- 用于标记成员函数为可调用的，可以通过 `QMetaObject::invokeMethod()` 动态调用。

---

### **4. 示例代码**
以下是一个简单的示例，展示了 Qt 对象模型和元对象系统的使用：

```cpp
#include <QObject>
#include <QDebug>

class MyClass : public QObject
{
    Q_OBJECT
    Q_PROPERTY(int value READ value WRITE setValue NOTIFY valueChanged)

public:
    explicit MyClass(QObject *parent = nullptr) : QObject(parent), m_value(0) {}

    int value() const { return m_value; }
    void setValue(int value) {
        if (m_value != value) {
            m_value = value;
            emit valueChanged(m_value);
        }
    }

signals:
    void valueChanged(int value);

private:
    int m_value;
};

int main(int argc, char *argv[])
{
    QCoreApplication app(argc, argv);

    MyClass obj;
    obj.setObjectName("MyObject");

    // 使用元对象系统动态调用槽函数
    QMetaObject::invokeMethod(&obj, "setValue", Q_ARG(int, 42));

    // 使用反射获取属性
    qDebug() << "Value:" << obj.property("value").toInt();

    return app.exec();
}

#include "main.moc"
```

---

### **5. 总结**
- Qt 对象模型通过父子关系、信号与槽、动态属性等特性，提供了强大的对象管理能力。
- 元对象系统通过 `moc` 和 `QMetaObject` 实现了反射和动态调用，是 Qt 框架的核心机制。
- 理解对象模型和元对象系统是掌握 Qt 开发的关键。

希望以上内容能帮助你更好地理解 Qt 的对象模型和元对象系统！如果还有其他问题，请随时提问。




                        `QObject` 是 Qt 框架中最基础的类，几乎所有 Qt 类都直接或间接继承自 `QObject`。它提供了 Qt 对象模型的核心功能，如对象树管理、信号与槽机制、事件处理、动态属性等。以下是关于 `QObject` 的详细介绍：

---

### **1. `QObject` 的核心功能**

#### **1.1 对象树与父子关系**
- `QObject` 支持对象树结构，每个对象可以有一个父对象和多个子对象。
- 当父对象被销毁时，其所有子对象也会自动销毁。
- 通过 `QObject::setParent()` 设置父对象，或通过 `QObject::children()` 获取子对象。

#### **1.2 信号与槽**
- `QObject` 提供了信号与槽机制，用于对象之间的通信。
- 信号是事件的通知，槽是事件的响应函数。
- 使用 `QObject::connect()` 连接信号与槽。

#### **1.3 动态属性**
- `QObject` 支持动态添加属性，这些属性可以在运行时查询和修改。
- 使用 `QObject::setProperty()` 设置属性，`QObject::property()` 获取属性。

#### **1.4 对象名称**
- 每个 `QObject` 对象可以有一个唯一的名称（`objectName`），用于在对象树中查找。
- 使用 `QObject::findChild()` 或 `QObject::findChildren()` 查找对象。

#### **1.5 事件处理**
- `QObject` 提供了事件处理机制，可以重写 `QObject::event()` 或特定事件处理函数（如 `QObject::timerEvent()`）来处理事件。

#### **1.6 线程支持**
- `QObject` 可以在多线程中使用，但需要注意线程安全性。
- 使用 `QObject::moveToThread()` 将对象移动到另一个线程。

---

### **2. `QObject` 的常用方法**

#### **2.1 构造函数**
```cpp
QObject(QObject *parent = nullptr);
```
- 创建一个 `QObject` 对象，`parent` 指定父对象。

#### **2.2 设置和获取父对象**
```cpp
void setParent(QObject *parent);
QObject *parent() const;
```

#### **2.3 设置和获取对象名称**
```cpp
void setObjectName(const QString &name);
QString objectName() const;
```

#### **2.4 查找子对象**
```cpp
QObject *findChild(const QString &name = QString(), Qt::FindChildOptions options = Qt::FindChildrenRecursively) const;
QList<QObject *> findChildren(const QString &name = QString(), Qt::FindChildOptions options = Qt::FindChildrenRecursively) const;
```

#### **2.5 动态属性**
```cpp
bool setProperty(const char *name, const QVariant &value);
QVariant property(const char *name) const;
```

#### **2.6 信号与槽**
```cpp
static bool connect(const QObject *sender, const char *signal, const QObject *receiver, const char *method, Qt::ConnectionType type = Qt::AutoConnection);
static bool disconnect(const QObject *sender, const char *signal, const QObject *receiver, const char *method);
```

#### **2.7 线程支持**
```cpp
void moveToThread(QThread *targetThread);
```

#### **2.8 事件处理**
```cpp
virtual bool event(QEvent *event);
virtual void timerEvent(QTimerEvent *event);
```

---

### **3. `QObject` 的使用示例**

以下是一个简单的示例，展示了 `QObject` 的基本用法：

```cpp
#include <QObject>
#include <QDebug>

class MyObject : public QObject
{
    Q_OBJECT

public:
    explicit MyObject(QObject *parent = nullptr) : QObject(parent) {}

signals:
    void mySignal();

public slots:
    void mySlot() {
        qDebug() << "Slot called!";
    }
};

int main(int argc, char *argv[])
{
    QCoreApplication app(argc, argv);

    MyObject obj;
    obj.setObjectName("MyObject");

    // 连接信号与槽
    QObject::connect(&obj, &MyObject::mySignal, &obj, &MyObject::mySlot);

    // 发射信号
    emit obj.mySignal();

    return app.exec();
}

#include "main.moc"
```

---

### **4. `QObject` 的注意事项**

1. **`Q_OBJECT` 宏**：
    - 所有使用信号与槽、动态属性等特性的类必须包含 `Q_OBJECT` 宏。
    - 该宏会触发 `moc`（元对象编译器）生成元对象代码。

2. **对象树管理**：
    - 使用对象树管理父子关系，避免内存泄漏。
    - 注意不要在栈上创建带有父对象的 `QObject`，因为父对象销毁时会尝试删除子对象。

3. **线程安全**：
    - `QObject` 默认是线程不安全的，跨线程操作时需要使用 `QMetaObject::invokeMethod()` 或信号与槽。

4. **信号与槽**：
    - 信号与槽的参数类型必须完全匹配，否则连接会失败。
    - 使用 `Qt::QueuedConnection` 实现跨线程的信号与槽连接。

---

### **5. 总结**
`QObject` 是 Qt 框架的基石，提供了对象管理、信号与槽、动态属性等核心功能。理解 `QObject` 的使用和特性是掌握 Qt 开发的关键。如果还有其他问题，请随时提问！