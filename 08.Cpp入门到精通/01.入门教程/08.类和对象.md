...
#### 目录介绍
- [10.1 类的介绍](#101-类的介绍)
  - [10.1.1 类的含义](#1011-类的含义)
  - [10.1.2 struct和class区别](#1012-struct和class区别)
  - [10.1.3 综合案例与思考](#1013-综合案例与思考)
- [10.2 类定义和对象](#102-类定义和对象)
  - [10.2.1 类的定义](#1021-类的定义)
  - [10.2.2 创建对象](#1022-创建对象)
  - [10.2.3 访问类成员](#1023-访问类成员)
  - [10.2.4 对象指针](#1024-对象指针)
  - [10.2.6 案例实践](#1026-案例实践)
  - [10.2.6 综合案例与思考](#1026-综合案例与思考)
- [10.3 类成员属性](#103-类成员属性)
  - [10.3.1 成员变量](#1031-成员变量)
  - [10.3.2 成员函数](#1032-成员函数)
  - [10.3.3 访问权限](#1033-访问权限)
  - [10.3.4 成员设置私有](#1034-成员设置私有)
  - [10.3.5 explicit](#1035-explicit)
  - [10.3.6 案例实践](#1036-案例实践)
  - [10.3.6 综合案例与思考](#1036-综合案例与思考)
- [10.4 对象初始化](#104-对象初始化)
  - [10.4.1 构造和析构函数](#1041-构造和析构函数)
  - [10.4.2 构造函数分类](#1042-构造函数分类)
  - [10.4.3 拷贝构造函数](#1043-拷贝构造函数)
  - [10.4.4 构造调用规则](#1044-构造调用规则)
  - [10.4.5 初始化列表](#1045-初始化列表)
  - [10.4.7 委托构造函数](#1047-委托构造函数)
  - [10.4.10 深拷贝与浅拷贝](#10410-深拷贝与浅拷贝)
  - [10.4.11 对象生命周期](#10411-对象生命周期)
  - [10.4.12 综合案例与思考](#10412-综合案例与思考)
- [10.5 类对象使用](#105-类对象使用)
  - [10.5.1 类对象作为类成员](#1051-类对象作为类成员)
  - [10.5.2 综合案例与思考](#1052-综合案例与思考)
- [10.6 对象模型和this](#106-对象模型和this)
  - [10.6.1 变量和函数存储](#1061-变量和函数存储)
  - [10.6.2 this指针概念](#1062-this指针概念)
  - [10.6.3 空指针访问成员函数](#1063-空指针访问成员函数)
  - [10.6.4 const修饰成员函数](#1064-const修饰成员函数)
  - [10.6.5 综合案例与思考](#1065-综合案例与思考)
- [10.7 友元类和函数](#107-友元类和函数)
  - [10.7.1 友元三种实现](#1071-友元三种实现)
  - [10.7.2 全局函数做友元](#1072-全局函数做友元)
  - [10.7.3 类做友元](#1073-类做友元)
  - [10.7.4 成员函数做友元](#1074-成员函数做友元)
  - [10.7.5 综合案例与思考](#1075-综合案例与思考)
- [10.9 综合案例练习](#109-综合案例练习)
  - [10.9.1 指针VS对象](#1091-指针vs对象)
  - [10.9.2 创建对象方式](#1092-创建对象方式)
- [10.10 类和对象底层原理](#1010-类和对象底层原理)
  - [10.10.1 对象内存布局](#10101-对象内存布局)
  - [10.10.2 构造函数与析构函数的调用机制](#10102-构造函数与析构函数的调用机制)
  - [10.10.3 this指针的底层实现](#10103-this指针的底层实现)
  - [10.10.4 访问控制的编译期本质](#10104-访问控制的编译期本质)
- [10.11 类和对象训练题](#1011-类和对象训练题)
...
## 10.13 卷一改造增补：零法则、三法则、五法则与 `=default`/`=delete`

> 本节为卷一新增。这是 C++ 类设计中**最容易写出 BUG 也最难调试**的一块知识。掌握后你写的类会在拷贝、赋值、移动、销毁四个场合都不出问题。

### 10.13.1 编译器自动合成的「特殊成员函数」

每个类都默默拥有 6 个特殊成员函数（编译器自动合成）：

| # | 函数 | 何时合成 |
|---|------|---------|
| 1 | 默认构造 `T()` | 用户没写任何构造函数时自动合成 |
| 2 | 析构 `~T()` | 总是合成（除非删除） |
| 3 | 拷贝构造 `T(const T&)` | 用户没写时合成 |
| 4 | 拷贝赋值 `T& operator=(const T&)` | 用户没写时合成 |
| 5 | 移动构造 `T(T&&)` | C++11 起，**且没有自定义拷贝/析构**才合成 |
| 6 | 移动赋值 `T& operator=(T&&)` | 同上 |

**核心规则**：自定义了其中任何一个，编译器对其他几个的合成策略就会改变——这是无数 BUG 的源头。

### 10.13.2 三法则（Rule of Three, C++98 起）

**如果你需要自定义析构、拷贝构造、拷贝赋值中的任何一个，那么三个都要写。**
...
class StringBad {
    char* data_;
public:
    StringBad(const char* s) { data_ = new char[strlen(s)+1]; strcpy(data_, s); }
    ~StringBad() { delete[] data_; }                       // 自定义了析构
    // 没写拷贝构造！编译器合成的是浅拷贝
};

StringBad a("hello");
StringBad b = a;        // 浅拷贝：a.data_ 与 b.data_ 指向同一块内存
// 析构时 double-free，程序崩溃
```

### 10.13.3 五法则（Rule of Five, C++11 起）

C++11 加入了移动语义，三法则升级为五法则：
...
public:
    ...
    ~String() noexcept;                               // 1. 析构
    String(const String& o);                          // 2. 拷贝构造
    String(String&& o) noexcept;                      // 3. 移动构造
    String& operator=(const String& o);               // 4. 拷贝赋值
    String& operator=(String&& o) noexcept;           // 5. 移动赋值
};
```

**关键：移动构造、移动赋值要标 `noexcept`**——否则 `std::vector` 在扩容时为了异常安全会退化为「拷贝」，性能直接打回原形。

### 10.13.4 零法则（Rule of Zero, 现代 C++ 推荐）

**最佳实践：让用户类用 `std::vector`、`std::string`、`std::unique_ptr` 等智能容器/指针管理资源，让编译器合成所有特殊成员函数。**

```cpp
class GoodString {
    std::string data_;        // 让 string 自己管理内存
    std::vector<int> idx_;    // 让 vector 自己管理内存
    // 不写析构、不写拷贝、不写移动——编译器合成的版本就是对的
};
```

> 90% 的业务类应该走零法则。只有写**资源句柄包装类**（文件、socket、GPU 内存、互斥锁）时才需要五法则。

### 10.13.5 `= default` 与 `= delete`（C++11）
...
private:
    Singleton() = default;                            // 显式要求合成默认构造
    ~Singleton() = default;
};
```

`= delete` 比「写成 private 不实现」更优雅——错误在编译期就报出，而非链接期。

### 10.13.6 决策表

| 场景 | 应遵循 |
|------|-------|
| 类只持有 `int` / `string` / `vector` 等 | **零法则** |
| 类管理 `new` 出来的裸内存 | **五法则**（或换成 `unique_ptr`）|
| 类是文件/锁/socket 包装器 | **五法则** + 移动专属语义 |
| 单例、不可拷贝的资源持有者 | `= delete` 拷贝、`= default` 移动（视情况）|

### 10.13.7 推荐阅读

- 卷一第 10 章 [`A3.内存模型与作用域基础`](./A3.内存模型与作用域基础.md) —— 对象生命周期与构造/析构时机
- 卷一第 10 章 [`10.动态内存.md`](./10.动态内存.md) —— 智能指针与 RAII
- 卷一 [`A4.Cpp11至Cpp23特性速览图谱`](./A4.Cpp11至Cpp23特性速览图谱.md) §C++11 移动语义部分
