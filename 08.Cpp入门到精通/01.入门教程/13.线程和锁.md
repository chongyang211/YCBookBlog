...
- [13.1 线程管理](#131-线程管理)
  ...
  - [13.1.2 创建线程](#1312-创建线程)
  - [13.1.3 等待线程结束](#1313-等待线程结束)
  - [13.1.4 分离线程](#1314-分离线程)
  - [13.1.5 检查线程](#1315-检查线程)
  - [13.1.6 线程休眠](#1316-线程休眠)
  - [13.1.9 线程管理总结](#1319-线程管理总结)
  - [13.1.10 综合案例与思考](#13110-综合案例与思考)
- [13.2 多线程](#132-多线程)
  - [13.2.1 多线程头文件](#1321-多线程头文件)
  - [13.2.2 创建多线程](#1322-创建多线程)
  - [13.2.3 多线程数据竞争](#1323-多线程数据竞争)
  - [13.2.4 死锁](#1324-死锁)
  - [13.2.5 虚假唤醒](#1325-虚假唤醒)
  - [13.2.6 线程局部存储](#1326-线程局部存储)
  - [13.2.7 C++锁机制发展](#1327-c锁机制发展)
  - [13.2.8 综合案例与思考](#1328-综合案例与思考)
- [13.3 mutex](#133-mutex)
  - [13.3.1 mutex作用](#1331-mutex作用)
  - [13.3.2 lock和unlock](#1332-lock和unlock)
  - [13.3.3 lock_guard](#1333-lock_guard)
  - [13.3.4 unique_lock](#1334-unique_lock)
  - [13.3.5 adopt_lock](#1335-adopt_lock)
  - [13.3.6 recursive_mutex](#1336-recursive_mutex)
  - [13.3.7 mutex总结](#1337-mutex总结)
  - [13.3.8 综合案例与思考](#1338-综合案例与思考)
- [13.4 condition_variable](#134-condition_variable)
  - [13.4.1 条件变量](#1341-条件变量)
  - [13.4.2 核心方法](#1342-核心方法)
  - [13.4.3 案例展示](#1343-案例展示)
  - [13.4.4 条件变量总结](#1344-条件变量总结)
  - [13.4.5 综合案例与思考](#1345-综合案例与思考)
- [13.5 atomic](#135-atomic)
  - [13.5.1 基本概念](#1351-基本概念)
  - [13.5.2 常用操作](#1352-常用操作)
  - [13.5.3 使用示例](#1353-使用示例)
  - [13.5.4 核心原理](#1354-核心原理)
  - [13.5.5 注意事项](#1355-注意事项)
  - [13.5.6 综合案例与思考](#1356-综合案例与思考)
- [13.6 async和future](#136-async和future)
  - [13.6.1 异步概念](#1361-异步概念)
  - [13.6.2 核心API](#1362-核心api)
  - [13.6.3 简单案例](#1363-简单案例)
  - [13.6.4 多个异步任务](#1364-多个异步任务)
- [13.7 shared_mutex](#137-shared_mutex)
  - [13.7.1 读写锁机制](#1371-读写锁机制)
  - [13.7.2 简单案例](#1372-简单案例)
  - [13.7.3 应用场景](#1373-应用场景)
  - [13.7.4 实现原理](#1374-实现原理)
  - [13.7.5 综合案例与思考](#1375-综合案例与思考)
- [13.9 综合案例](#139-综合案例)
  - [13.9.1 死锁与避免](#1391-死锁与避免)
  - [13.9.2 延迟任务](#1392-延迟任务)
  - [13.9.3 轮训定时器](#1393-轮训定时器)
  - [13.9.4 提前唤醒休眠](#1394-提前唤醒休眠)
- [13.10 线程和锁底层原理](#1310-线程和锁底层原理)
  - [13.10.1 线程的内核实现](#13101-线程的内核实现)
  - [13.10.2 mutex的底层机制](#13102-mutex的底层机制)
  - [13.10.3 原子操作与内存序](#13103-原子操作与内存序)
- [13.11 线程和锁训练题](#1311-线程和锁训练题)
- [13.12 综合思考题](#1312-综合思考题)
  - [13.9.2 延迟任务](#1392-延迟任务)
  - [13.9.3 轮训定时器](#1393-轮训定时器)
  - [13.9.4 提前唤醒休眠](#1394-提前唤醒休眠)
...
> 本节为卷一新增。掌握 `jthread`、`scoped_lock`、`atomic` 与 C++20 新原语后，你能写出更短、更安全的并发代码。

### 13.X.1 `std::jthread`（C++20）：自动 join 的线程

```cpp
{
    std::jthread t([]{ /* 工作 */ });
    // 离开作用域自动 join，不再有「线程未 join 直接 terminate」
}

// 还内置了 stop_token，支持协作式取消
...
std::lock_guard<std::mutex>     lk1(m);          // C++11：最简单
std::unique_lock<std::mutex>    lk2(m);          // C++11：可手动 unlock/lock，配 condition_variable
std::scoped_lock                lk3(m1, m2, m3); // C++17：一次锁多个，避免死锁
std::shared_lock<std::shared_mutex> sk(rwm);     // C++17：读写锁的读端
```

**口诀**：单锁用 `lock_guard`；多锁用 `scoped_lock`；条件变量必须 `unique_lock`；读多写少用 `shared_mutex`。

### 13.X.3 `std::atomic` 的内存序

```cpp
std::atomic<int> counter{0};

// 默认 memory_order_seq_cst：最强、最慢、最安全
counter.fetch_add(1);

// 计数场景：用 relaxed 即可
counter.fetch_add(1, std::memory_order_relaxed);

// 标志位 / 发布订阅：release / acquire
flag.store(true, std::memory_order_release);
if (flag.load(std::memory_order_acquire)) { /* ... */ }
```

### 13.X.4 异步任务：`std::async` / `std::future`

```cpp
auto fut = std::async(std::launch::async, []{ return heavy_calc(); });
do_other_work();
int v = fut.get();         // 阻塞等待结果
```

C++20 起还有 `std::latch` / `std::barrier` / `std::counting_semaphore`，覆盖几乎所有同步原语。

### 13.X.5 推荐阅读

- 卷一 [`A4.Cpp11至Cpp23特性速览图谱`](./A4.Cpp11至Cpp23特性速览图谱.md) §C++20 并发新增
- 卷四《工程实战卷》—— 高并发实战与协程
- cppreference: [`<thread>`](https://en.cppreference.com/w/cpp/thread)、[`<atomic>`](https://en.cppreference.com/w/cpp/atomic)
...
| # | 陷阱 | 说明 |
|---|------|------|
| 1 | 用 `std::thread` 没 join | 析构时直接 `std::terminate`；用 `jthread` |
| 2 | 同时锁多个 mutex 死锁 | 用 `std::scoped_lock` 或固定加锁顺序 |
| 3 | 读取共享变量没用 atomic / 锁 | 数据竞争是 UB；编译器可能优化掉读取 |
| 4 | `condition_variable` 没循环判定 | 必须 `while(!cond) cv.wait(lk);` 防虚假唤醒 |
| 5 | 把 `unique_ptr` 传给多个线程 | 移动后只有一个线程持有；考虑 `shared_ptr` |
