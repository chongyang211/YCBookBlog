...
- [10.4 抽象类](#104-抽象类)
  ...
  - [10.4.8 看个案例](#1048-看个案例)
  - [10.4.9 综合案例与思考](#1049-综合案例与思考)
- [10.5 综合案例](#105-综合案例)
  - [10.5.1 计算器类](#1051-计算器类)
  - [10.5.2 制作饮品](#1052-制作饮品)
  - [10.5.3 组装电脑](#1053-组装电脑)
- [10.6 继承和多态底层原理](#106-继承和多态底层原理)
  - [10.6.1 继承的内存模型](#1061-继承的内存模型)
  - [10.6.2 虚函数表与动态绑定](#1062-虚函数表与动态绑定)
  - [10.6.3 虚继承的底层实现](#1063-虚继承的底层实现)
  - [10.6.4 RTTI与dynamic_cast的代价](#1064-rtti与dynamic_cast的代价)
...
## 10.X 卷一改造增补：现代多态四种形态对比

> 本节为卷一新增。多态不只是「虚函数」，现代 C++ 提供了四种各有取舍的多态形式。

### 10.X.1 四种多态形态

| 形态 | 时机 | 代价 | 典型用途 |
|------|------|------|---------|
| 虚函数（动态多态） | 运行期 | vptr/vtable，无法内联 | 异质对象统一接口 |
| 函数重载 | 编译期 | 零开销 | 同名不同参 |
| 模板（参数化多态） | 编译期 | 零开销，二进制膨胀 | 容器、算法 |
| Concepts（C++20） | 编译期约束 | 零开销，错误信息友好 | 现代模板设计 |

### 10.X.2 必须懂的 5 个虚函数关键字

```cpp
class Base {
public:
    virtual void f();                    // 1. virtual：声明虚函数
    virtual void g() = 0;                // 2. =0：纯虚（抽象类）
    virtual ~Base() = default;           // 3. 基类析构必须 virtual
};

class Derived : public Base {
public:
    void f() override;                   // 4. override：明确声明覆盖
    // void g() override final;          // 5. final：禁止再被覆盖
};
class Last final : public Derived {};    // final：禁止再被继承
```

**强烈推荐**：所有覆盖基类虚函数的地方都加 `override`。如果哪天基类签名变了，编译器会立刻报错。

### 10.X.3 静态多态：CRTP（编译期多态）
...
零开销、可内联，但失去运行期动态分派能力——适合性能敏感、类型集合编译期已知的场景。

### 10.X.4 C++20 Concepts：用约束代替继承

```cpp
template <class T>
concept Drawable = requires(T t) { t.draw(); };

void render(Drawable auto& obj) { obj.draw(); }   // 不需要继承基类，鸭子类型
```

### 10.X.5 推荐阅读

- 卷一 [`18.特性图谱`](./18.特性图谱.md) §C++20 Concepts
- 卷一第 9 章 [`09.类和对象.md`](./09.类和对象.md) §9.13 五法则与零法则
- 卷三《模板与泛型卷》—— Concepts 与 CRTP 深度展开
