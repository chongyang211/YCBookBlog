...
## 1.X 卷一改造增补：从 Hello World 到现代 C++ 工程

> 本节为卷一新增。本章带你迈出第一步，但要写出能在团队、面试、生产环境立得住的现代 C++，还要补这几块。

### 1.X.1 现代 C++ 的 Hello World
...
int main() {
    ...
    std::cout << std::format("Hello, modern C++! Year={}\n", v);
    return 0;
}
```
...
# 或更现代：clang++ -std=c++23 -fexperimental-library hello.cpp
```

### 1.X.2 工程化最小骨架（CMake 模板）
...
set(CMAKE_EXPORT_COMPILE_COMMANDS ON)   # clangd 必备
...
- 卷一 [`A1.Cpp简史与版本演进`](./A1.Cpp简史与版本演进.md) —— 选哪个 C++ 版本上车
- 卷一 [`A2.开发环境完整搭建`](./A2.开发环境完整搭建.md) —— 编译器/IDE/调试器全套
- 卷一 [`A4.Cpp11至Cpp23特性速览图谱`](./A4.Cpp11至Cpp23特性速览图谱.md) —— 学习路径地图
...
| # | 陷阱 | 说明 |
|---|------|------|
| 1 | `using namespace std;` 写在头文件 | 污染所有引入者；只允许在 cpp 文件局部使用 |
| 2 | 编译期版本不一致 | 命令行 `-std=c++20`、CMake、IDE 三处必须一致 |
| 3 | 缺 `-Wall -Wextra` | 警告就是免费的代码审查，必开 |
| 4 | 不区分 `\n` 与 `std::endl` | `endl` 会强制 flush，循环里慢几十倍 |
| 5 | UTF-8 源文件没指定编码 | MSVC 需 `/utf-8`，否则中文字符串乱码 |
