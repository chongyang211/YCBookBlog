...
## 8.X 卷一改造增补：现代指针守则——`nullptr`、智能指针、`span`、`reference_wrapper`

> 本节为卷一新增。**写新代码时，绝大多数 `T*` 都应该用其他东西替代**。

### 8.X.1 用 `nullptr` 替代 `NULL` / `0`（C++11）
...
> 详见卷一第 12 章 §12.11 《RAII 主线》。

### 8.X.3 用 `std::span<T>`（C++20）替代 `T*` + `size`
...
- 卷一第 12 章 [`12.动态内存.md`](./12.动态内存.md) §12.11 RAII 主线
- 卷一第 11 章 [`11.内存模型`](./11.内存模型.md) —— 指针的内存视角
