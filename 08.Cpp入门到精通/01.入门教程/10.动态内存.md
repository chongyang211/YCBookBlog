...
在异步编程中，一个任务可能需要保活自己，直到任务完成后再自行销毁。如果任务在异步执行过程中被外部释放，可能会导致未定义行为（例如访问已释放的内存）。

`std::enable_shared_from_this` 提供了一种机制，让对象可以在内部安全地持有自己的 `std::shared_ptr`，从而延长生命周期，直到任务完成。
...
int main() {
  {
    ...
    std::cout << "use_count before Start: " << task.use_count() << "\n";  // 1
    task->Start();
    std::cout << "use_count after Start: " << task.use_count() << "\n";   // 2
  }
  // 出了作用域，外部的 shared_ptr 已销毁，但对象还活着（self_ 保活）
  std::cout << "outer shared_ptr gone, but task is still alive\n";
...
```text
use_count before Start: 1
use_count after Start: 2
outer shared_ptr gone, but task is still alive
[MyTask] working...
...
- 继承 `std::enable_shared_from_this<AsyncTask>`，使得 `AsyncTask` 对象可以安全地获取指向自身的 `std::shared_ptr`。
- 通过工厂方法 `Create` 创建对象，确保对象始终由 `std::shared_ptr` 管理。
- 在 `Start` 方法中，使用 `shared_from_this()` 获取 `std::shared_ptr`，并将其存储在 `self_` 中，从而保活对象。
- 在异步任务完成后，调用 `self_.reset()` 释放自引用，允许对象被销毁。

2）`main` 函数

- 创建 `AsyncTask` 对象，并调用 `Start` 方法启动异步任务。
- 在 `Start` 方法中，`self_` 持有对象的 `std::shared_ptr`，即使外部的 `std::shared_ptr` 被释放，对象仍然存活。
- 异步任务完成后，`self_.reset()` 释放自引用，对象被销毁。
...
> **如果没有 `std::enable_shared_from_this` 会发生什么？**

如果直接通过 `this` 创建 `std::shared_ptr`，会导致多个独立的 `std::shared_ptr` 管理同一个对象，从而引发未定义行为（通常是双重释放）。

```cpp
void Start() {
    self_ = std::shared_ptr<AsyncTask>(this);  // 错误：直接通过 this 创建 shared_ptr
