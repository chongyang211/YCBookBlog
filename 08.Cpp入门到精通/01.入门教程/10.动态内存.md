# 10.内存模型
#### 目录介绍
- [10.1 动态内存概念](#101-动态内存概念)
  - [10.1.1 何为动态内存](#1011-何为动态内存)
  - [10.1.2 动态内存特点](#1012-动态内存特点)
  - [10.1.3 综合案例与思考](#1013-综合案例与思考)
- [10.2 动态内存操作](#102-动态内存操作)
  - [10.2.1 new分配内存](#1021-new分配内存)
  - [10.2.2 delete释放内存](#1022-delete释放内存)
  - [10.2.3 动态内存示例](#1023-动态内存示例)
  - [10.2.4 动态内存最佳实践](#1024-动态内存最佳实践)
  - [10.2.5 综合案例与思考](#1025-综合案例与思考)
- [10.3 动态内存问题](#103-动态内存问题)
  - [10.3.1 内存泄漏](#1031-内存泄漏)
  - [10.3.2 野指针](#1032-野指针)
  - [10.3.3 悬空指针问题](#1033-悬空指针问题)
  - [10.3.4 重复释放](#1034-重复释放)
  - [10.3.5 空指针](#1035-空指针)
  - [10.3.6 综合案例与思考](#1036-综合案例与思考)
- [10.4 智能指针](#104-智能指针)
  - [10.4.1 unique_ptr](#1041-unique_ptr)
  - [10.4.2 shared_ptr](#1042-shared_ptr)
  - [10.4.3 weak_ptr](#1043-weak_ptr)
  - [10.4.4 综合案例与思考](#1044-综合案例与思考)
- [10.5 内存管理函数](#105-内存管理函数)
  - [10.5.1 malloc和free](#1051-malloc和free)
  - [10.5.2 calloc](#1052-calloc)
  - [10.5.3 realloc](#1053-realloc)
  - [10.5.4 综合案例与思考](#1054-综合案例与思考)
- [10.6 内存池](#106-内存池)
  - [10.6.1 内存池概念](#1061-内存池概念)
  - [10.6.2 内存池核心思想](#1062-内存池核心思想)
  - [10.6.3 内存池的优点](#1063-内存池的优点)
  - [10.6.4 内存池的实现](#1064-内存池的实现)
  - [10.6.5 内存池优化](#1065-内存池优化)
  - [10.6.6 综合案例与思考](#1066-综合案例与思考)
- [10.7 高级用法](#107-高级用法)
  - [10.7.1 enable_shared_from_this](#1071-enable_shared_from_this)
- [10.8 动态内存底层原理](#108-动态内存底层原理)
  - [10.8.1 堆内存分配器原理](#1081-堆内存分配器原理)
  - [10.8.2 new/delete的底层过程](#1082-newdelete的底层过程)
  - [10.8.3 智能指针的底层实现](#1083-智能指针的底层实现)
- [10.9 动态内存训练题](#109-动态内存训练题)
- [10.10 综合思考题](#1010-综合思考题)
...
## 10.11 卷一改造增补：RAII 主线 —— 用智能指针告别裸 `new`/`delete`

> 本节为卷一新增。学完本节后，**你的代码里不应再出现裸 `new`/`delete`**。这是从「能写 C++」到「写好 C++」的分水岭。

### 10.11.1 为什么要用 RAII？看一段会泄漏的代码

```cpp
void buy_item() {
    Resource* r = new Resource();        // 裸 new
    if (!check_user()) return;           // 早返回 → 内存泄漏
    do_something();                      // 可能抛异常 → 内存泄漏
    delete r;
}
```

只要中途任何一处 `return`、`throw`、未来同事新加的 `goto`，`delete` 就执行不到。

### 10.11.2 RAII 的核心思想

**Resource Acquisition Is Initialization**：资源的获取 = 对象的初始化；资源的释放 = 对象的析构。  
这样，**只要对象正常进入作用域**，无论函数走哪条路退出（正常返回、`return`、异常），栈上对象的析构函数总会被调用——资源就一定被释放。

### 10.11.3 三大智能指针对比

| 智能指针 | 所有权 | 用途 | 开销 |
|---------|-------|------|------|
| `std::unique_ptr<T>` | **独占**所有权（不可拷贝、可移动） | 90% 场景的首选 | 零开销，与裸指针等价 |
| `std::shared_ptr<T>` | **共享**所有权（引用计数） | 多个对象共享同一资源 | 有引用计数原子操作 |
| `std::weak_ptr<T>` | **观察**（不影响计数） | 打破 `shared_ptr` 循环引用 | 同 `shared_ptr` |

### 10.11.4 `unique_ptr`：默认就该用它

```cpp
#include <memory>
auto p = std::make_unique<Resource>();   // C++14 起，强烈推荐
// p 离开作用域时自动 delete
```
...
auto p2 = std::move(p1);    // OK：所有权转移到 p2，p1 变空
```

返回 `unique_ptr` 是工厂函数的标准签名：

```cpp
std::unique_ptr<Connection> connect(const Config& c) {
    return std::make_unique<Connection>(c);
}
```

### 10.11.5 `shared_ptr`：引用计数管理

```cpp
auto sp1 = std::make_shared<Image>("a.png");   // 引用计数 = 1
auto sp2 = sp1;                                // 引用计数 = 2
// sp1、sp2 都析构后，引用计数 = 0，资源释放
```

**注意**：`shared_ptr` 不是免费的，每次拷贝都有原子加减。能用 `unique_ptr` 就别用 `shared_ptr`。

### 10.11.6 `weak_ptr` 打破循环引用

```cpp
struct Node {
    std::shared_ptr<Node> next;
    std::weak_ptr<Node>   prev;     // 反向必须用 weak，否则永远不释放
};
```

### 10.11.7 RAII 的应用远不止智能指针

**任何成对获取/释放的资源都该 RAII 化**：

```cpp
std::lock_guard<std::mutex> lk(mu);              // 互斥锁
std::ifstream f("a.txt");                        // 文件
std::unique_ptr<FILE, decltype(&fclose)> fp{fopen("b","r"), fclose};   // C 资源
```

C++17 起还可以用 `std::scoped_lock`、`std::shared_lock`，C++20 起 `std::jthread` 自动 join。

### 10.11.8 「不要用裸 `new`」的少数例外

1. 标准库或框架内部实现智能指针、容器时；
2. 与 C API 边界交互、对方明确接管所有权时；
3. 用 placement new 在指定地址构造对象时（极少见）。

业务代码 99% 的场景都应该用 `make_unique` / `make_shared`。

### 10.11.9 推荐阅读

- 卷一第 10 章 [`A3.内存模型与作用域基础`](./A3.内存模型与作用域基础.md) —— 栈帧析构与 RAII 的底层机制
- 卷一第 8 章 [`08.类和对象.md`](./08.类和对象.md) §10.13 —— 零法则与五法则
- 卷四《工程实战卷》[`04.开发技巧/01.从一次BusError排查Cpp崩溃问题`](../04.开发技巧/01.从一次BusError排查Cpp崩溃问题.md) —— 不用 RAII 的真实代价
