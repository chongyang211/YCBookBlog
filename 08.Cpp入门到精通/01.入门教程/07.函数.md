...
## 7.X 卷一改造增补：现代函数语法（Lambda、`auto` 返回、尾置返回、`constexpr`）

> 本节为卷一新增。掌握这五个特性，你的函数代码会发生质变。

### 7.X.1 Lambda 表达式（C++11/14/17/20 持续增强）
...
### 7.X.2 尾置返回类型 与 `auto` 返回（C++11/14）
...
- 卷一 [`18.特性图谱`](./18.特性图谱.md) §Lambda 演进
- 卷一第 10 章 [`10.继承多态.md`](./10.继承多态.md) —— 多态 vs Lambda 选型
