#### 目录介绍
- [18.1 函数模版](#181-函数模版)
  - [18.1.1 函数模版概念](#1811-函数模版概念)
  - [18.1.2 函数模板语法](#1812-函数模板语法)
  - [18.1.3 函数模板示例](#1813-函数模板示例)
  - [18.1.4 多类型参数函数模板](#1814-多类型参数函数模板)
  - [18.1.5 显式指定模板参数](#1815-显式指定模板参数)
  - [18.1.6 函数模板的特化](#1816-函数模板的特化)
  - [18.1.7 函数模板与重载](#1817-函数模板与重载)
  - [18.1.8 综合案例与思考](#1818-综合案例与思考)
- [18.2 类模版](#182-类模版)
  - [18.2.1 类模版概念](#1821-类模版概念)
  - [18.2.2 类模板语法](#1822-类模板语法)
  - [18.2.3 类模板示例](#1823-类模板示例)
  - [18.2.4 多类型参数类模板](#1824-多类型参数类模板)
  - [18.2.5 类模板的特化](#1825-类模板的特化)
  - [18.2.6 类模板默认模板参数](#1826-类模板默认模板参数)
  - [18.2.7 类模板嵌套](#1827-类模板嵌套)
  - [18.2.9 类模板注意事项](#1829-类模板注意事项)
- [18.3 STL标准库模版](#183-stl标准库模版)
  - [18.3.1 STL核心组件](#1831-stl核心组件)
  - [18.3.2 容器（Containers）](#1832-容器containers)
  - [18.3.3 迭代器（Iterators）](#1833-迭代器iterators)
  - [18.3.4 算法（Algorithms）](#1834-算法algorithms)
  - [18.3.5 函数对象（Function Objects）](#1835-函数对象function-objects)
  - [18.3.6 综合案例与思考](#1836-综合案例与思考)
- [18.4 STL算法模版](#184-stl算法模版)
  - [18.4.1 非修改序列算法](#1841-非修改序列算法)
  - [18.4.2 修改序列算法](#1842-修改序列算法)
  - [18.4.3 排序算法](#1843-排序算法)
  - [18.4.4 查找算法](#1844-查找算法)
  - [18.4.5 数值算法](#1845-数值算法)
  - [18.4.6 总结一下](#1846-总结一下)
  - [18.4.7 综合案例与思考](#1847-综合案例与思考)
- [18.5 STL迭代器](#185-stl迭代器)
  - [18.5.1 迭代器的分类](#1851-迭代器的分类)
  - [18.5.2 常用迭代器操作](#1852-常用迭代器操作)
  - [18.5.3 迭代器示例](#1853-迭代器示例)
  - [18.5.4 特殊迭代器](#1854-特殊迭代器)
  - [18.5.5 迭代器适配器](#1855-迭代器适配器)
- [18.6 STL适配器](#186-stl适配器)
- [18.7 STL函数对象](#187-stl函数对象)
- [18.8 模板与STL底层原理](#188-模板与stl底层原理)
  - [18.8.1 模板的编译期展开](#1881-模板的编译期展开)
  - [18.8.2 STL容器的内存布局](#1882-stl容器的内存布局)
  - [18.8.3 迭代器失效问题](#1883-迭代器失效问题)
- [18.9 模板与STL训练题](#189-模板与stl训练题)
- [18.10 综合思考题](#1810-综合思考题)
...
## 18.X 卷一改造增补：STL 选型决策表 + Ranges 速览

> 本节为卷一新增。先把容器选型刻进肌肉记忆，再用 Ranges 让算法链路更优雅。

### 18.X.1 容器选型决策表

| 需求 | 首选容器 | 备选 |
|------|---------|------|
| 默认情况、随机访问、尾部增删 | `std::vector` | — |
| 频繁中间插入删除 | `std::list`（链表）；多数情况仍 `vector` 更快 | `std::deque` |
| 键值查找：有序 + 范围遍历 | `std::map` / `std::set`（红黑树，O(log n)） | — |
| 键值查找：无序 + 极致速度 | `std::unordered_map` / `std::unordered_set`（哈希，O(1)） | — |
| 头尾增删 | `std::deque` | — |
| 优先队列 | `std::priority_queue` | — |
| 不可变小集合 | `std::array<T, N>` | — |
| 字符串 | `std::string` | `std::string_view`（只读视图）|

> 90% 的场景请直接用 `std::vector`——缓存友好、访问最快，理论复杂度的劣势在实测中往往被常数项抹平。

### 18.X.2 必会的 10 个算法

```cpp
#include <algorithm>
std::ranges::sort(v);                                      // 排序（C++20 ranges）
std::ranges::find(v, 42);                                  // 查找
std::ranges::count_if(v, [](int x){ return x > 0; });      // 计数
std::ranges::any_of  / all_of  / none_of(v, pred);
std::ranges::transform(v, out.begin(), [](int x){ return x*2; });
std::ranges::for_each(v, print);
std::ranges::min_element / max_element(v);
std::accumulate(v.begin(), v.end(), 0);                    // 求和（在 <numeric>）
std::ranges::unique(v);                                    // 去重（需先排序）
std::ranges::remove_if(v, pred);                           // 标记删除（配 erase）
```

### 18.X.3 C++20 Ranges 与管道
...
**惰性求值**：管道中的元素只在迭代时按需生成，不构造中间容器；可读性接近 Python/Rust。

### 18.X.4 emplace 与 move 的小性能贴士

```cpp
v.emplace_back(arg1, arg2);   // 直接在尾部就地构造，避免临时对象
v.push_back(std::move(big));  // 已有对象就用 move，避免拷贝
v.reserve(N);                 // 已知容量提前 reserve，避免多次扩容
```

### 18.X.5 推荐阅读

- 卷一 [`A4.Cpp11至Cpp23特性速览图谱`](./A4.Cpp11至Cpp23特性速览图谱.md) §C++20 Ranges
- 卷三《模板与泛型卷》—— STL 内部实现剖析
- cppreference: [`Algorithms`](https://en.cppreference.com/w/cpp/algorithm)、[`Ranges`](https://en.cppreference.com/w/cpp/ranges)
...
| # | 陷阱 | 说明 |
|---|------|------|
| 1 | 迭代器失效 | `vector::push_back` 扩容后旧迭代器全部失效 |
| 2 | `remove_if` 不删除 | 它只移动、不缩短；要 `v.erase(remove_if(...), v.end())` 或 C++20 `std::erase_if` |
| 3 | `unordered_map` 在循环里访问不存在的键 | `m[k]` 会插入默认值；用 `find` 或 `contains`（C++20） |
| 4 | `vector<bool>` 不是普通 vector | 它是位压缩的特化，引用语义不同；用 `vector<char>` |
| 5 | 不 reserve 导致反复扩容 | 已知规模时永远 reserve |
