# 17.异常处理
#### 目录介绍
- [17.1 异常入门介绍](#171-异常入门介绍)
  - [17.1.1 异常概念解释](#1711-异常概念解释)
  - [17.1.2 运行时异常案例](#1712-运行时异常案例)
  - [17.1.3 异常基本语法](#1713-异常基本语法)
  - [17.1.4 捕获异常](#1714-捕获异常)
  - [17.1.5 发生异常位置](#1715-发生异常位置)
  - [17.1.6 综合案例与思考](#1716-综合案例与思考)
- [17.2 多级catch匹配](#172-多级catch匹配)
  - [17.2.1 多级catch使用](#1721-多级catch使用)
  - [17.2.2 匹配中类型转换](#1722-匹配中类型转换)
  - [17.2.3 综合案例与思考](#1723-综合案例与思考)
- [17.3 throw抛出异常](#173-throw抛出异常)
  - [17.3.1 异常抛出概念](#1731-异常抛出概念)
  - [17.3.2 throw抛异常用法](#1732-throw抛异常用法)
  - [17.3.3 throw异常规范](#1733-throw异常规范)
  - [17.3.4 综合案例与思考](#1734-综合案例与思考)
- [17.4 exception异常](#174-exception异常)
  - [17.4.1 exception类介绍](#1741-exception类介绍)
  - [17.4.2 exception派生类](#1742-exception派生类)
  - [17.4.3 定义新的异常](#1743-定义新的异常)
  - [17.4.4 noexcept关键字](#1744-noexcept关键字)
  - [17.4.5 综合案例与思考](#1745-综合案例与思考)
- [17.5 异常原理探索](#175-异常原理探索)
  - [17.5.1 try工作原理](#1751-try工作原理)
  - [17.5.2 catch工作原理](#1752-catch工作原理)
  - [17.5.3 throw工作原理](#1753-throw工作原理)
- [17.6 异常处理底层原理](#176-异常处理底层原理)
  - [17.6.1 异常的零成本模型](#1761-异常的零成本模型)
  - [17.6.2 栈展开的过程](#1762-栈展开的过程)
  - [17.6.3 noexcept的优化意义](#1763-noexcept的优化意义)
- [17.7 异常处理训练题](#177-异常处理训练题)
- [17.8 综合思考题](#178-综合思考题)
...
## 12.X 卷一改造增补：现代错误处理 —— `noexcept`、错误码 vs 异常、`std::expected`

> 本节为卷一新增。错误处理是 C++ 工程化最有争议的话题；本节给出现代 C++ 项目的主流抉择。

### 12.X.1 `noexcept`（C++11）：编译期承诺「我不抛异常」

```cpp
void swap(int& a, int& b) noexcept {       // 承诺不抛
    int t = a; a = b; b = t;
}
```
...
1. **`std::vector` 扩容选择**：移动构造若是 `noexcept`，扩容时直接移动；否则退化为拷贝，性能差几倍。
2. **代码生成更优**：编译器无需生成异常展开表，体积更小、速度更快。
3. **向调用方传递语义**：`noexcept` 函数失败只能 `std::terminate`，调用方无须 try/catch。

**强烈建议标 `noexcept` 的函数**：

- 移动构造、移动赋值
- 析构函数（C++11 起默认就是 `noexcept`）
- `swap` 操作
- 所有不可能抛的简单 getter

### 12.X.2 错误码 vs 异常：什么时候用哪个？

| 维度 | 错误码 | 异常 |
|------|-------|------|
| 性能 | 几乎零开销 | 抛出时较慢；不抛时几乎零开销 |
| 强制处理 | 易被忽略 | 不处理就崩溃，更安全 |
| 控制流可读性 | `if (err)` 充斥 | 主流程清爽 |
| 跨 ABI 边界 | 安全 | 不能跨 dll/so 边界 |
| 嵌入式/实时 | 适合 | 谨慎（异常路径不可预测）|

**实践共识**：

- **业务逻辑错误**（用户输入错、文件不存在）→ 错误码 / `std::optional` / `std::expected`
- **真正异常的情况**（内存耗尽、不变量被破坏）→ 异常
- **跨语言/跨 ABI 接口**（C API、DLL/SO 边界）→ 错误码
- **构造函数失败**（无法返回错误码）→ 异常

### 12.X.3 `std::expected<T, E>`（C++23）：错误码与异常的最优解

C++23 终于带来了 Rust `Result<T, E>` 的等价物——同时拥有异常的强制处理与错误码的低开销：
...
// 链式：C++23 提供 .and_then / .or_else / .transform
auto v = parse("42")
    .transform([](int x) { return x * 2; })
    .value_or(-1);
```

**优势**：返回值含错误信息、类型安全、强制检查、零异常开销。

### 12.X.4 异常安全的三个级别

| 级别 | 含义 | 示例 |
|------|------|------|
| 不抛保证（noexcept） | 承诺不抛 | `swap`、移动构造、析构 |
| 强保证 | 要么成功，要么状态完全回滚 | `vector::push_back`（用 copy-and-swap）|
| 基本保证 | 不泄漏资源、不变量保持 | 大多数业务函数的兜底 |

写库时优先达到强保证；写应用层时基本保证够用。

### 12.X.5 推荐阅读

- 卷一 [`A4.Cpp11至Cpp23特性速览图谱`](./A4.Cpp11至Cpp23特性速览图谱.md) §C++23 部分
- 卷一第 10 章 [`10.动态内存.md`](./10.动态内存.md) §10.11 RAII 主线 —— 异常安全的基础
- 卷四《工程实战卷》[`01.从一次BusError排查Cpp崩溃问题`](../04.开发技巧/01.从一次BusError排查Cpp崩溃问题.md)
