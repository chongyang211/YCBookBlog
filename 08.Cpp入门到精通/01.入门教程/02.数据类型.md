...
- [2.1 基本数据](#21-基本数据)
  ...
  - [2.1.2 固定宽度类型](#212-固定宽度类型)
  - [2.1.3 空类型 (void)](#213-空类型-void)
  - [2.1.4 综合案例与思考](#214-综合案例与思考)
- [2.2 整数类型](#22-整数类型)
  - [2.2.1 有符号类型](#221-有符号类型)
  - [2.2.2 无符号类型](#222-无符号类型)
  - [2.2.3 固定宽度整数类型](#223-固定宽度整数类型)
  - [2.2.4 综合案例与思考](#224-综合案例与思考)
- [2.3 字符类型](#23-字符类型)
  - [2.3.1 字符类型列表](#231-字符类型列表)
  - [2.3.2 字符类型特点](#232-字符类型特点)
  - [2.3.3 字符使用场景](#233-字符使用场景)
  - [2.3.4 字符注意事项](#234-字符注意事项)
  - [2.3.5 综合案例与思考](#235-综合案例与思考)
- [2.4 浮点类型](#24-浮点类型)
  - [2.4.1 浮点类型列表](#241-浮点类型列表)
  - [2.4.2 浮点类型特点](#242-浮点类型特点)
  - [2.4.3 使用场景](#243-使用场景)
  - [2.4.4 注意事项](#244-注意事项)
  - [2.4.5 浮点数特殊值](#245-浮点数特殊值)
  - [2.4.6 综合案例与思考](#246-综合案例与思考)
- [2.5 布尔类型](#25-布尔类型)
  - [2.5.1 布尔类型使用](#251-布尔类型使用)
  - [2.5.2 布尔类型底层](#252-布尔类型底层)
  - [2.5.3 综合案例与思考](#253-综合案例与思考)
- [2.6 变量和常量](#26-变量和常量)
  - [2.6.1 变量](#261-变量)
  - [2.6.2 常量](#262-常量)
  - [2.6.3 综合案例与思考](#263-综合案例与思考)
- [2.7 类型转换](#27-类型转换)
  - [2.7.1 隐式类型转换](#271-隐式类型转换)
  - [2.7.2 显式类型转换](#272-显式类型转换)
    - [2.7.2.1 static_cast](#2721-static_cast)
    - [2.7.2.2 dynamic_cast](#2722-dynamic_cast)
    - [2.7.2.3 const_cast](#2723-const_cast)
    - [2.7.3.4 reinterpret_cast](#2734-reinterpret_cast)
  - [2.7.3 类型转换建议](#273-类型转换建议)
  - [2.7.4 类型转换总结](#274-类型转换总结)
  - [2.7.5 综合案例与思考](#275-综合案例与思考)
- [2.8 练习题](#28-练习题)
  - [2.8.1 类型转换案例](#281-类型转换案例)
  - [2.8.2 键盘数据输入](#282-键盘数据输入)
...
## 2.9 卷一改造增补：现代 C++ 类型补充（C++11~C++23）

> 本节为卷一新增。前 8 节讲的是「类 C 时代」的传统类型；本节补齐现代 C++ 中真正常用、几乎每个项目都会用到的类型工具。

### 2.9.1 `auto` 与 `decltype`：让编译器替你写类型
...
### 2.9.2 `std::byte`（C++17）：取代 `unsigned char` 表示「裸字节」
...
### 2.9.3 `std::optional<T>`（C++17）：表达「可能没有值」
...
### 2.9.4 `std::variant<T...>`（C++17）：类型安全的联合体
...
### 2.9.6 `std::string_view`（C++17）：零拷贝字符串视图
...
- 卷一第 1 章 [`A1.Cpp简史与版本演进`](./A1.Cpp简史与版本演进.md) ——了解每个新类型对应的标准版本
- 卷一第 10 章 [`A3.内存模型与作用域基础`](./A3.内存模型与作用域基础.md) ——这些类型在内存中的分布
- 卷一末章 [`A4.Cpp11至Cpp23特性速览图谱`](./A4.Cpp11至Cpp23特性速览图谱.md) ——类型相关的特性矩阵
- cppreference: [`<optional>`](https://en.cppreference.com/w/cpp/utility/optional) / [`<variant>`](https://en.cppreference.com/w/cpp/utility/variant) / [`<string_view>`](https://en.cppreference.com/w/cpp/string/basic_string_view)
