...
#### 目录介绍
- [7.1 指针概念](#71-指针概念)
  - [7.1.1 什么是指针](#711-什么是指针)
  - [7.1.2 指针声明](#712-指针声明)
  - [7.1.3 指针初始化](#713-指针初始化)
  - [7.1.4 指针占用空间](#714-指针占用空间)
  - [7.1.5 综合案例与思考](#715-综合案例与思考)
- [7.2 指针操作](#72-指针操作)
  - [7.2.1 取地址运算符](#721-取地址运算符)
  - [7.2.2 解引用运算符](#722-解引用运算符)
  - [7.2.3 综合案例与思考](#723-综合案例与思考)
- [7.3 指针使用](#73-指针使用)
  - [7.3.1 指针和数组](#731-指针和数组)
  - [7.3.2 指针和函数](#732-指针和函数)
  - [7.3.3 指针与常量](#733-指针与常量)
  - [7.3.4 指针和结构体](#734-指针和结构体)
  - [7.3.5 综合案例与思考](#735-综合案例与思考)
- [7.4 指针高级用法](#74-指针高级用法)
  - [7.4.1 指针的指针](#741-指针的指针)
  - [7.4.2 函数指针](#742-函数指针)
  - [7.4.3 综合案例与思考](#743-综合案例与思考)
- [7.5 引用](#75-引用)
  - [7.5.1 什么是引用](#751-什么是引用)
  - [7.5.2 引用声明](#752-引用声明)
  - [7.5.3 引用必须初始化](#753-引用必须初始化)
  - [7.5.4 引用访问数据](#754-引用访问数据)
  - [7.5.5 引用和函数](#755-引用和函数)
  - [7.5.6 常量引用](#756-常量引用)
  - [7.5.7 避免返回局部变量引用](#757-避免返回局部变量引用)
  - [7.5.8 综合案例与思考](#758-综合案例与思考)
- [7.6 指针VS引用](#76-指针vs引用)
  - [7.6.1 两者对比](#761-两者对比)
  - [7.6.2 两者转换](#762-两者转换)
  - [7.6.3 综合案例与思考](#763-综合案例与思考)
- [7.7 总结](#77-总结)
- [7.8 综合案例](#78-综合案例)
  - [7.8.1 遍历数组案例](#781-遍历数组案例)
  - [7.8.2 指针vs普通对象](#782-指针vs普通对象)
  - [7.8.3 交换两个变量值](#783-交换两个变量值)
- [7.9 指针与引用底层原理](#79-指针与引用底层原理)
  - [7.9.1 指针的内存模型与寻址](#791-指针的内存模型与寻址)
  - [7.9.2 指针运算的汇编实现](#792-指针运算的汇编实现)
  - [7.9.3 引用的底层本质](#793-引用的底层本质)
  - [7.9.4 const指针与const引用的编译器处理](#794-const指针与const引用的编译器处理)
- [7.10 指针与引用训练题](#710-指针与引用训练题)
- [7.11 综合思考题](#711-综合思考题)
...
## 7.X 卷一改造增补：现代指针守则——`nullptr`、智能指针、`span`、`reference_wrapper`

> 本节为卷一新增。**写新代码时，绝大多数 `T*` 都应该用其他东西替代**。

### 7.X.1 用 `nullptr` 替代 `NULL` / `0`（C++11）

```cpp
void f(int);
void f(char*);
f(NULL);     // 历史上 NULL 是 0，调用 f(int) — 出乎意料！
f(nullptr);  // 类型 std::nullptr_t，明确调用 f(char*)
```

### 7.X.2 用智能指针替代裸 `new`/`delete`

| 旧写法 | 现代写法 |
|--------|---------|
| `T* p = new T(...);` | `auto p = std::make_unique<T>(...);` |
| 共享所有权 | `std::shared_ptr<T>` + `std::weak_ptr<T>` |
| 不拥有的观察者指针 | 裸指针 `T*`（仅观察，不释放）|

> 详见卷一第 10 章 §10.11 《RAII 主线》。

### 7.X.3 用 `std::span<T>`（C++20）替代 `T*` + `size`

```cpp
// 老写法：散落的指针 + 长度
void process(int* data, size_t n);

// 现代写法：span 把指针和长度打包，自动支持 array/vector/C 数组
...
| 场景 | 选型 |
|------|------|
| 必有值，不会换绑 | **引用** `T&` / `const T&` |
| 可能为空 | 指针 / `std::optional` |
| 需要重新指向不同对象 | 指针 |
| 容器里要存「引用」 | `std::reference_wrapper<T>`（容器不能直接放引用） |

```cpp
std::vector<std::reference_wrapper<Logger>> loggers;
loggers.push_back(std::ref(g_logger));
```

### 7.X.5 推荐阅读

- 卷一第 10 章 [`10.动态内存.md`](./10.动态内存.md) §10.11 RAII 主线
- 卷一第 10 章 [`A3.内存模型与作用域基础`](./A3.内存模型与作用域基础.md) —— 指针的内存视角
- 卷四《工程实战卷》[`04.开发技巧/01.从一次BusError排查Cpp崩溃问题`](../04.开发技巧/01.从一次BusError排查Cpp崩溃问题.md) —— 悬挂指针实战
