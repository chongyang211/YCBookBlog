...
- [6.1 概述和定义](#61-概述和定义)
  - [6.1.1 函数概念](#611-函数概念)
  - [6.1.2 函数声明和定义](#612-函数声明和定义)
  - [6.1.3 函数分类](#613-函数分类)
  - [6.1.4 Lambda函数](#614-lambda函数)
  - [6.1.5 综合案例与思考](#615-综合案例与思考)
- [6.2 函数参数](#62-函数参数)
  - [6.2.1 值传递](#621-值传递)
  - [6.2.2 引用传递](#622-引用传递)
  - [6.2.3 指针传递](#623-指针传递)
...
- [6.3 函数返回值](#63-函数返回值)
  - [6.3.1 综合案例与思考](#631-综合案例与思考)
- [6.4 函数重载](#64-函数重载)
  - [6.4.1 函数重载案例](#641-函数重载案例)
  - [6.4.2 重载注意事项](#642-重载注意事项)
  - [6.4.3 综合案例与思考](#643-综合案例与思考)
- [6.5 内联函数](#65-内联函数)
  - [6.5.1 内联函数是什么](#651-内联函数是什么)
  - [6.5.2 内联函数](#652-内联函数)
  - [6.5.3 内联函数特点](#653-内联函数特点)
  - [6.5.4 注意事项](#654-注意事项)
  - [6.5.5 内联函数 vs 宏](#655-内联函数-vs-宏)
  - [6.5.6 内联函数场景](#656-内联函数场景)
  - [6.5.7 综合案例与思考](#657-综合案例与思考)
- [6.6 默认参数](#66-默认参数)
  - [6.6.1 默认参数示例](#661-默认参数示例)
  - [6.6.2 默认参数注意点](#662-默认参数注意点)
  - [6.6.3 综合案例与思考](#663-综合案例与思考)
- [6.7 结构体与函数](#67-结构体与函数)
  - [6.7.1 结构体做参数](#671-结构体做参数)
  - [6.7.2 结构体做返回值](#672-结构体做返回值)
  - [6.7.3 结构体const场景](#673-结构体const场景)
  - [6.7.4 综合案例与思考](#674-综合案例与思考)
- [6.8 函数底层原理](#68-函数底层原理)
  - [6.8.1 函数调用的栈帧机制](#681-函数调用的栈帧机制)
  - [6.8.2 参数传递的底层差异](#682-参数传递的底层差异)
  - [6.8.3 名称修饰与函数重载](#683-名称修饰与函数重载)
- [6.9 函数训练题](#69-函数训练题)
- [6.10 综合思考题](#610-综合思考题)
...
## 6.X 卷一改造增补：现代函数语法（Lambda、`auto` 返回、尾置返回、`constexpr`）

> 本节为卷一新增。掌握这五个特性，你的函数代码会发生质变。

### 6.X.1 Lambda 表达式（C++11/14/17/20 持续增强）
...
// 捕获列表
int x = 10;
auto f1 = [x](int n)        { return x + n; };   // 按值捕获
auto f2 = [&x](int n)       { x += n; };         // 按引用捕获（注意生命周期）
auto f3 = [=](int n)        { /* 全部按值 */ };
auto f4 = [&]()             { /* 全部按引用 */ };
auto f5 = [x = std::move(big)](){ use(x); };     // C++14 初始化捕获
...
// 模板 lambda（C++20）
auto first = []<typename T>(std::vector<T>& v) { return v.front(); };
```

### 6.X.2 尾置返回类型 与 `auto` 返回（C++11/14）
...
// auto 返回（C++14）：编译器从函数体推导
auto multiply(int a, int b) { return a * b; }
```

### 6.X.3 `constexpr` / `consteval` 函数

```cpp
constexpr int factorial(int n) {                  // C++11/14：可在编译期或运行期
    return n <= 1 ? 1 : n * factorial(n - 1);
}
consteval int compile_only(int n) { return n*2; } // C++20：必须编译期

constexpr int v = factorial(10);                  // 编译期算出 3628800
```

### 6.X.4 `[[nodiscard]]`、默认参数、`= delete`

```cpp
[[nodiscard]] int parse();                        // 调用方忽略返回值会警告
void log(std::string_view msg, int level = 0);   // 默认参数
void dangerous(double) = delete;                  // 禁止隐式转换
```

### 6.X.5 推荐阅读

- 卷一 [`A4.Cpp11至Cpp23特性速览图谱`](./A4.Cpp11至Cpp23特性速览图谱.md) §Lambda 演进
- 卷一第 9 章 [`09.继承和多态.md`](./09.继承和多态.md) —— 多态 vs Lambda 选型
- 卷三《模板与泛型卷》—— Concepts 约束模板函数
