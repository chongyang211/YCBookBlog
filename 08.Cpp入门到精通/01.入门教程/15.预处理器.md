#### 目录介绍
- [19.1 头文件](#191-头文件)
  - [19.1.1 头文件作用](#1911-头文件作用)
  - [19.1.2 基本结构](#1912-基本结构)
  - [19.1.3 头文件包含](#1913-头文件包含)
  - [19.1.4 综合案例与思考](#1914-综合案例与思考)
- [19.2 宏定义](#192-宏定义)
  - [19.2.1 什么是宏](#1921-什么是宏)
  - [19.2.2 基本语法](#1922-基本语法)
  - [19.2.3 宏的使用](#1923-宏的使用)
  - [19.2.4 宏的作用域](#1924-宏的作用域)
  - [19.2.5 宏与函数区别](#1925-宏与函数区别)
  - [19.2.6 宏的优缺点](#1926-宏的优缺点)
  - [19.2.7 综合案例与思考](#1927-综合案例与思考)
- [19.3 头文件保护](#193-头文件保护)
  - [19.3.1 理解头文件保护](#1931-理解头文件保护)
  - [19.3.2 头文件保护作用](#1932-头文件保护作用)
  - [19.3.3 示例说明](#1933-示例说明)
  - [19.3.4 不用头文件保护](#1934-不用头文件保护)
  - [19.3.5 使用场景](#1935-使用场景)
  - [19.3.6 头文件保护原理](#1936-头文件保护原理)
  - [19.3.7 头文件保护总结](#1937-头文件保护总结)
  - [19.3.8 综合案例与思考](#1938-综合案例与思考)
- [19.4 预定义宏](#194-预定义宏)
  - [19.4.1 标准预定义宏](#1941-标准预定义宏)
  - [19.4.2 编译器相关宏](#1942-编译器相关宏)
  - [19.4.3 操作系统相关宏](#1943-操作系统相关宏)
  - [19.4.4 标准库相关宏](#1944-标准库相关宏)
  - [19.4.5 其他常用宏](#1945-其他常用宏)
  - [19.4.6 预定义宏案例](#1946-预定义宏案例)
  - [19.4.7 综合案例与思考](#1947-综合案例与思考)
- [19.5 条件编译](#195-条件编译)
  - [19.5.1 基本语法](#1951-基本语法)
  - [19.5.2 常用指令](#1952-常用指令)
  - [19.5.3 示例代码案例](#1953-示例代码案例)
- [19.6 错误/警告指令](#196-错误警告指令)
  - [19.6.1 错误指令](#1961-错误指令)
  - [19.6.2 警告指令](#1962-警告指令)
  - [19.6.3 使用的场景](#1963-使用的场景)
  - [19.6.4 注意事项](#1964-注意事项)
  - [19.6.5 综合案例与思考](#1965-综合案例与思考)
- [19.7 行控制](#197-行控制)
- [19.8 空指令](#198-空指令)
- [19.9 综合宏案例](#199-综合宏案例)
  - [19.9.1 日志记录宏](#1991-日志记录宏)
- [19.10 预处理器底层原理](#1910-预处理器底层原理)
  - [19.10.1 预处理的编译阶段](#19101-预处理的编译阶段)
  - [19.10.2 宏展开的规则](#19102-宏展开的规则)
  - [19.10.3 头文件包含的实现](#19103-头文件包含的实现)
- [19.11 预处理器训练题](#1911-预处理器训练题)
- [19.12 综合思考题](#1912-综合思考题)
...
- `"file"`：先在当前文件所在目录搜索，再搜索系统路径
- `<file>`：只在系统路径（`-I`指定的路径和标准库路径）搜索
...
## 19.X 卷一改造增补：用现代特性替代宏 + C++20 Modules 展望

> 本节为卷一新增。这是预处理器的「劝退篇」——告诉你哪些宏可以扔掉。
...
| `#define MAX(a,b) ((a)>(b)?(a):(b))` | `template<class T> constexpr auto max(T a, T b){ return a>b?a:b; }` |
| `#define DEBUG_LOG(x) ...` | `inline` 函数 / `consteval` 函数 / Lambda |
| 头文件保护 `#ifndef X #define X ... #endif` | `#pragma once`（多数编译器支持），未来 → Modules |
| 条件编译选择类型 | `if constexpr` |
| 配置开关 `#ifdef DEBUG` | `if constexpr (kDebug)` 或 `[[maybe_unused]]` |

**结论**：日常业务代码里宏只剩三类合法用法——`__FILE__/__LINE__`、字符串化 `#`、跨平台 ABI 适配。

### 19.X.2 必须保留的「最小宏集」

```cpp
// 1. 调试日志：必须 __FILE__/__LINE__
#define LOG_HERE() std::printf("[%s:%d]\n", __FILE__, __LINE__)
...
### 19.X.3 C++20 Modules：宏与头文件的终结者（展望）
...
1. **编译速度数量级提升**——再不需要重复展开几万行头文件；
2. **完美隔离宏**——模块内部宏不会泄漏到引入者；
3. **真正的 C++ 模块系统**——不再依赖文本预处理。

> 当前 C++20 Modules 在 GCC 14+/Clang 17+/MSVC 已基本可用，但生态（CMake、IDE、第三方库）还在追赶。**生产项目仍以 `#include` 为主**，新项目可在 2025-2026 年逐步引入。

### 19.X.4 推荐阅读

- 卷一 [`A4.Cpp11至Cpp23特性速览图谱`](./A4.Cpp11至Cpp23特性速览图谱.md) §C++20 Modules
- 卷一 [`A1.Cpp简史与版本演进`](./A1.Cpp简史与版本演进.md) §C++20 四大支柱
- 卷四《工程实战卷》—— 大型项目的预编译头与编译加速
...
| # | 陷阱 | 说明 |
|---|------|------|
| 1 | 宏副作用 | `#define SQ(x) (x)*(x)`，`SQ(a+1)` 展开成 `a+1*a+1` |
| 2 | 宏没加括号 | 参数与返回值都要加括号；考虑改 `constexpr` 函数 |
| 3 | 头文件没用 `#pragma once` 或 include guard | 多重包含报「重定义」 |
| 4 | 在头文件里 `using namespace` | 污染所有引入方 |
| 5 | 用宏定义常量 | 没有类型检查、不能调试；用 `constexpr` |
