...
## 12.11 卷一改造增补：RAII 主线 —— 用智能指针告别裸 `new`/`delete`

> 本节为卷一新增。学完本节后，**你的代码里不应再出现裸 `new`/`delete`**。这是从「能写 C++」到「写好 C++」的分水岭。

### 12.11.1 为什么要用 RAII？看一段会泄漏的代码
...
- 卷一第 11 章 [`11.内存模型`](./11.内存模型.md) —— 栈帧析构与 RAII 的底层机制
- 卷一第 9 章 [`09.类和对象.md`](./09.类和对象.md) §9.13 —— 零法则与五法则
