...
| `04.开发技巧/` | 1 篇 | 调试与工程实战（首篇为 *从一次 Bus error 排查 C++ 崩溃问题*） | 10% |
...
```
第一篇  开篇与环境
  第 1 章  C++ 简史与现代演进（C++98 → C++23）
  第 2 章  开发环境搭建（GCC / Clang / MSVC / CMake）

第二篇  基础语法
  第 3 章  基础语法与第一个程序           ← 01.基础语法.md
  第 4 章  数据类型与字面量               ← 02.数据类型.md
  第 5 章  运算符与表达式                 ← 03.运算符.md
  第 6 章  复合类型：数组、字符串、结构体 ← 04.复合类型.md
  第 7 章  流程控制语句                   ← 05.流程语句.md

第三篇  函数与作用域
  第 8 章  函数：参数、返回值、重载       ← 06.函数.md
  第 9 章  指针与引用                     ← 07.指针&引用.md
  第 10 章 内存模型与作用域【新增】

第四篇  面向对象初步
  第 11 章 类与对象                       ← 08.类和对象.md
  第 12 章 继承与多态                     ← 09.继承和多态.md
  第 13 章 动态内存与 RAII 入门           ← 10.动态内存.md

第五篇  库与外围
  第 14 章 IO 与文件操作                  ← 11.IO流和File.md
  第 15 章 异常处理                       ← 12.异常处理.md
  第 16 章 STL 模板入门                   ← 14.STL模版.md
  第 17 章 预处理器                       ← 15.预处理器.md

第六篇  现代 C++ 速览
  第 18 章 C++11 至 C++23 关键特性速览【新增】
```
...
| 原文件 | 新章节 | 改造动作 |
|--------|--------|----------|
| 01.基础语法.md | 第 3 章 | 删除环境部分（已并入第 2 章），补 `auto` / `nullptr` 等现代写法 |
| 02.数据类型.md | 第 4 章 | 新增 `std::byte` / `std::optional` 类型介绍 |
| 03.运算符.md | 第 5 章 | 新增 `<=>` 三路比较运算符（C++20） |
| 04.复合类型.md | 第 6 章 | 数组小节增加"为什么不该用裸数组" |
| 05.流程语句.md | 第 7 章 | 增加结构化绑定 / `if constexpr` |
| 06.函数.md | 第 8 章 | 增加 `[[nodiscard]]` / `consteval` |
| 07.指针&引用.md | 第 9 章 | 引用部分前置智能指针概念 |
| 08.类和对象.md | 第 11 章 | 增加"五法则 / 零法则"小节 |
| 09.继承和多态.md | 第 12 章 | 增加 `override` / `final` |
| 10.动态内存.md | 第 13 章 | 显著重构：以 RAII 为主线，裸指针为反例 |
| 11.IO流和File.md | 第 14 章 | 补 `std::filesystem` |
| 12.异常处理.md | 第 15 章 | 补 `noexcept` / 错误码 vs 异常的取舍 |
| 13.线程和锁.md | 移到卷三 | 因深度需求高，挪到《底层卷》 |
| 14.STL模版.md | 第 16 章 | 仅做使用层面，源码留给卷三 |
| 15.预处理器.md | 第 17 章 | 增加 modules 简介（C++20） |

### 3.3 待补章节清单

| 序号 | 待补内容 | 预估字数 | 优先级 |
|------|----------|----------|--------|
| A1 | C++ 简史与版本演进时间线 | 5000 | ★★★★★ |
| A2 | 开发环境完整搭建（macOS / Linux / Windows，含 CMake / VSCode） | 8000 | ★★★★★ |
| A3 | 内存模型基础（栈 / 堆 / 静态 / 全局，与卷三呼应） | 7000 | ★★★★ |
| A4 | C++11 至 C++23 关键特性速览图谱 | 12000 | ★★★★★ |
| A5 | 学习路径与避坑清单（每章末附"新手陷阱 Top 5"） | 分散 | ★★★★ |
...
```
第一篇  内存与对象
  第 1 章  C++ 内存模型与对象内存布局     ← 01.C++内存模型与对象内存布局.md
  第 2 章  引用与指针的底层原理           ← 02.引用与指针的底层原理.md
  第 3 章  类与面向对象的底层机制         ← 03.类与面向对象的底层机制.md
  第 4 章  构造析构与对象生命周期         ← 04.构造析构与对象生命周期.md
  第 5 章  继承与多态的底层实现           ← 05.继承与多态的底层实现.md

第二篇  模板与泛型
  第 6 章  模板元编程与泛型原理           ← 06.模板元编程与泛型原理.md
  第 7 章  STL 容器设计原理与源码分析     ← 07.STL容器设计原理与源码分析.md
  第 8 章  STL 算法与迭代器设计原理       ← 08.STL算法与迭代器设计原理.md
  第 9 章  Concepts 与 Ranges 原理【新增】

第三篇  资源与现代特性
  第 10 章 智能指针与资源管理 RAII        ← 09.智能指针与资源管理RAII.md
  第 11 章 右值引用与移动语义原理         ← 10.右值引用与移动语义原理.md
  第 12 章 Lambda 表达式与函数式编程      ← 11.Lambda表达式与函数式编程.md

第四篇  错误与类型
  第 13 章 异常处理与错误设计原理         ← 12.异常处理与错误设计原理.md
  第 14 章 类型系统与类型推导原理         ← 14.类型系统与类型推导原理.md
  第 15 章 RTTI 与 dynamic_cast 实现【新增】

第五篇  并发与编译期
  第 16 章 并发编程与内存序原理           ← 13.并发编程与内存序原理.md
  第 17 章 std::thread 与锁的实现【新增，原 13.线程和锁.md 提升版】
  第 18 章 编译期计算与 constexpr 原理    ← 15.编译期计算与constexpr原理.md
  第 19 章 协程（Coroutine）原理【新增】

第六篇  编译与链接
  第 20 章 C++ 编译链接模型与现代特性演进 ← 16.C++编译链接模型与现代特性演进.md
...
```
第一篇  崩溃与调试
  第 1 章  从一次 Bus error 排查 C++ 崩溃问题  ← 04.开发技巧/01...md（已完成）
  第 2 章  崩溃流程和捕获原理                  ← 03.专栏博客/崩溃流程和捕获原理.md
  第 3 章  调试技巧和原理分析                  ← 03.专栏博客/调试技巧和原理分析.md
  第 4 章  GDB / LLDB 实战手册【新增】

第二篇  内存与安全
  第 5 章  内存泄漏排查实战【新增】
  第 6 章  AddressSanitizer / UBSan / TSan 全家桶【新增】
  第 7 章  Valgrind 深度使用【新增】
  第 8 章  常见安全漏洞与防御（栈溢出 / 整数溢出 / UAF）【新增】

第三篇  性能优化
  第 9 章  性能优化方法论【新增】
  第 10 章 perf / Instruments 与火焰图【新增】
  第 11 章 缓存友好编程：从 cache line 到 SIMD【新增】
  第 12 章 编译优化与 PGO / LTO【新增】

第四篇  工程化与编译链接
  第 13 章 C++ 编译和链接原理                  ← 03.专栏博客/Cpp编译和链接原理.md
  第 14 章 静态分析工具链（clang-tidy / cppcheck）【新增】
  第 15 章 代码规范与 Code Review 标准【新增】

第五篇  生产环境
  第 16 章 日志、监控与告警体系【新增】
  第 17 章 灰度发布与稳定性保障【新增】
  第 18 章 一次完整的线上故障复盘【新增】
```
...
| 原文件 | 改造动作 |
|--------|----------|
| 04.开发技巧/01.从一次 BusError | 直接成为本卷开篇，引出整套调试方法论 |
| 03.专栏博客/崩溃流程和捕获原理 | 与第 1 章形成"案例 → 原理"对照 |
| 03.专栏博客/调试技巧和原理分析 | 升级为"完整调试手册"，加入更多场景 |
| 03.专栏博客/Cpp 编译和链接原理 | 移到本卷第 13 章，作为构建工具的前置原理 |

### 6.3 待补章节清单

| 序号 | 待补内容 | 预估字数 | 优先级 |
|------|----------|----------|--------|
| D1 | GDB / LLDB 实战手册（断点、watchpoint、core 分析、远程调试） | 15000 | ★★★★★ |
| D2 | 内存泄漏排查实战（多个真实案例） | 12000 | ★★★★★ |
| D3 | Sanitizer 全家桶（ASan / UBSan / TSan / MSan） | 12000 | ★★★★★ |
| D4 | Valgrind 深度使用（memcheck / callgrind / helgrind） | 10000 | ★★★★ |
| D5 | C++ 安全漏洞图鉴与防御 | 12000 | ★★★★ |
| D6 | 性能优化方法论（先量后改、Amdahl 定律、热点定位） | 10000 | ★★★★★ |
| D7 | perf / Instruments / 火焰图实战 | 12000 | ★★★★★ |
| D8 | 缓存友好编程（cache line / 数据布局 / SIMD） | 10000 | ★★★★ |
| D9 | PGO / LTO / `__attribute__` 编译优化 | 8000 | ★★★ |
| D10 | clang-tidy / cppcheck 静态分析工具链 | 8000 | ★★★★ |
| D11 | 代码规范与 Code Review 标准（基于 Google C++ Style + ISO Core Guidelines） | 10000 | ★★★★ |
| D12 | 日志 / 监控 / 告警（spdlog / glog / Prometheus 思想） | 10000 | ★★★ |
| D13 | 灰度发布与稳定性保障 | 8000 | ★★★ |
| D14 | 完整线上故障复盘案例（与卷一第 1 章呼应，形成首尾闭环） | 12000 | ★★★★★ |
