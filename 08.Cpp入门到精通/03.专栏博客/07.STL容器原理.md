...
代码和面试题能让人"会用"，但要真正理解 STL，必须看清 Stepanov 当年在容器设计上做的几个关键取舍。这些取舍直接塑造了今天我们写 C++ 的方式。

### 15.1 为什么 STL 容器按"价值语义"设计，而不是"引用语义"

Java 的 `ArrayList<Integer>` 装的是对象引用；Python 的 `list` 装的也是对象引用。但 `std::vector<T>` 装的是 `T` 本身——一份真正的拷贝。

这个决定影响深远：

- **好处**：内存连续、cache 友好、可以 `memcpy`、可以 SIMD；对象析构时机精确、没有 GC 抖动；支持 `T` 是栈对象、裸值、甚至位域。
- **代价**：拷贝贵、类型必须可拷贝/可移动、`std::vector<Base>` 无法多态（要用 `std::vector<std::unique_ptr<Base>>`）。

移动语义（C++11）之所以是 STL 的"第二春"，就在于它让价值语义的"拷贝代价"降到和引用语义一样低，同时保留了缓存友好的优势。

### 15.2 为什么 vector 扩容用 2 倍（或 1.5 倍）

几何扩容保证 `push_back` 的**摊还复杂度 O(1)**。但"几何因子"选多少是工程问题：

- **2 倍（libstdc++、libc++）**：实现最简，但有一个被 Andrei Alexandrescu 多次吐槽的缺点——**已分配过的内存永远无法复用**。因为每次新分配 2× 旧大小，等于前面所有已释放块加起来也装不下新块。
- **1.5 倍（MSVC）**：有可能复用前面释放的块（当 `1.5^n ≈ 1.5^(n-1) + 1.5^(n-2) + ...` 累积容量够时），但摊还常数略差。

此外"增长时选择移动还是拷贝"也是 C++11 的细节：编译器只有在移动构造 `noexcept` 时才允许用移动（否则走不了强异常安全保证）。这一点是 10 章右值引用里会深入展开的。

### 15.3 deque 的"分段连续"是在解什么问题

`deque` 是 STL 里最容易被误解的容器。它不是"双端 vector"，而是"在两端 O(1) 插入同时保证随机访问仍是 O(1)"的独特产物。核心结构：

- **map（索引表）**：一个指针数组，每个指针指向一段固定大小的连续缓冲区（libstdc++ 的策略是 512 B 或 `sizeof(T)` 的较大值）。
- **访问元素**：下标运算符先算 `map[i/chunk_size][i%chunk_size]`，因此仍是 O(1)，但常数项比 vector 大。
- **前后插入**：只要 map 两端有空槽，就在对应段内 push，map 不够再扩 map（而 map 扩展时不移动元素段，只移动"指针数组"）。

这也是为什么 **`deque` 的迭代器不在 push_front/push_back 时失效（除非 map 扩容）**——元素本身的地址没动，只是多了个索引项。你很难用 `vector` 和 `list` 的组合优雅地达到这个效果，这就是"分段连续"这个设计的价值。

### 15.4 map 为什么是红黑树，不是 AVL 树

两者都是自平衡二叉搜索树，最坏 O(log n)。但 STL 选红黑树的理由是**写入密集场景下的旋转次数**：

| | 查找 | 插入旋转 | 删除旋转 |
|---|---|---|---|
| AVL | 更严格平衡，略快 | O(log n) 次旋转 | O(log n) 次旋转 |
| RB Tree | 稍松（高度 ≤ 2 log n） | 最多 2 次旋转 | 最多 3 次旋转 |

在实际 C++ 程序里，写/读比例大多数时候接近 1:1 甚至更高（`map[key] = value` 每次都是写），红黑树的"平均写代价低"赢得了这场比赛。

另一个冷知识：GCC/Clang/MSVC 三家实现的红黑树节点布局几乎完全不同，所以**直接跨编译器传递 `std::map` 的底层指针是不可行的**——这也是为什么 STL 容器从不应该出现在 ABI 边界上。

### 15.5 unordered_map 为什么慢于"大多数人的想象"

"哈希表 O(1)"是教科书的理想。真实的 `unordered_map` 有几个隐藏开销：

1. **节点是独立堆分配的**：标准要求指针稳定（不能在 rehash 时失效），所以 bucket 里挂的是**链表节点**，而不是像 Google `dense_hash_map` 那样的开放地址数组。每次 `insert` 都有一次 `malloc`。
2. **默认 hash 对 `int` 是 `identity`**：如果 key 是顺序递增的整数（很常见），低位冲突集中，性能暴跌。
3. **load_factor 默认 1.0**：意味着平均每个 bucket 都有元素，cache miss 率高。

这也是为什么高性能 C++ 代码里常见 `absl::flat_hash_map`、`boost::unordered_flat_map` 这种"开放地址 + SIMD 探测"的替代品——它们比标准 `unordered_map` 能快 2-5 倍。标准库之所以不这么做，是因为标准规定了**迭代器稳定性**（rehash 不能使迭代器失效的情况下，flat 布局无法满足）。

### 15.6 Allocator 是个接口还是个对象？为什么 C++17 又加了 pmr

原始设计里，`Allocator` 是模板参数：`std::vector<int, MyAlloc>` 和 `std::vector<int, std::allocator<int>>` 是两个不同的类型。这带来了好处（静态分派、inline）和坏处（两个 vector 不能互相赋值/swap）。

C++17 的 `std::pmr::polymorphic_allocator` 引入了**运行时多态的分配器**——所有 `pmr::vector` 都是同一个类型，内部持有一个 `memory_resource*`。代价是多一次虚调用，收益是：

- 可以在运行时切换策略（线性分配器、池分配器、栈上分配器）。
- 函数边界不用把 allocator 作为模板参数传递。
- 可以把"数据结构"和"内存策略"彻底解耦。

对游戏引擎、嵌入式、自定义池的用户来说，`pmr` 是现代 C++ 最被低估的组件之一。理解它也解释了为什么 STL 早期的 Allocator 设计"在概念上非常漂亮、在工程上非常难用"——静态多态的代价是 ABI 僵化。

读完这六个取舍，你再去看 STL 的任何一个容器、任何一个新提案，都会从"怎么用"升级到"它为什么是这样"。这也是从使用者到设计者的关键跨越。
