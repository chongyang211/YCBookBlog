...
STL（Standard Template Library）是C++标准库的核心，其容器设计体现了"零开销抽象"和"泛型编程"的精髓。理解STL容器的底层数据结构和实现细节，是写出高性能C++代码的关键。
...
└── 容器适配器 (Container Adaptors)
    ├── stack         栈（默认deque）
    ├── queue         队列（默认deque）
    └── priority_queue 优先队列（默认vector+堆）
```
