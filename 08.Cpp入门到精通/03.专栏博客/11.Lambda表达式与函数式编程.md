...
Lambda表达式是C++11引入的最受欢迎的特性之一。它本质上是编译器自动生成的匿名函数对象（仿函数），让你能在需要的地方就地定义可调用对象，极大简化了回调、算法和异步编程。

### 1.1 Lambda的演进

```
C++11: 基础lambda
C++14: 泛型lambda(auto参数)、初始化捕获
C++17: constexpr lambda、*this捕获
C++20: 模板lambda、可默认构造的无状态lambda
C++23: 递归lambda简化
```
...
> Lambda表达式将函数式编程的核心理念带入了C++，使得高阶函数、闭包、惰性求值等模式成为日常工具，同时保持了C++零开销抽象的承诺。
