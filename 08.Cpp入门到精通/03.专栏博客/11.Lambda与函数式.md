...
## 附录：深挖——Lambda 与函数式编程的机制拾遗

### Z.1 Lambda 的本质：一个你看不到的类

你写的每一个 Lambda，编译器都悄悄生成一个**匿名 class**：
...
auto f = __lambda_123(x);
```

这就解释了 Lambda 的几个规则：
- **为什么 Lambda 不能拷贝自己到 `std::function`**：`std::function` 有固定类型，但每个 Lambda 是一个**唯一的匿名类**。
- **为什么 `auto f = []{};` 可以**：`auto` 能自动推导任意类型。
- **为什么可以取 Lambda 的 `sizeof`**：它就是个对象，大小由捕获决定（无捕获的 Lambda 大小通常为 1）。
- **为什么无捕获的 Lambda 可以转函数指针**：因为没有状态，`operator()` 可以是普通函数。

### Z.2 捕获的五种姿势，各自的陷阱

| 捕获方式 | 含义 | 陷阱 |
|---|---|---|
| `[=]` | 值捕获所有 | 捕获了自己不知道的东西；在成员函数里捕获 `this`（C++20 起 deprecated） |
| `[&]` | 引用捕获所有 | 离开作用域后悬空引用 |
| `[x]` | 值捕获 x | 拷贝成本 |
| `[&x]` | 引用捕获 x | 悬空风险 |
| `[x = std::move(x)]` | 初始化捕获（C++14） | 最灵活，move 进去避免拷贝 |
...
class Widget {
    int id;
    auto get() { return [=]() { return id; }; }  // C++20 前等价于 [this]
};
```

如果对象析构后再调用返回的 Lambda → 悬空。C++17 起推荐 `[*this]`（按值捕获 `*this`）或 `[id]`（显式按值）。

### Z.3 Lambda 的"透明"不是"零成本"的绝对保证

编译器可以把 Lambda 内联到底，是因为它看得到 Lambda 的具体类型。但一旦 Lambda 被塞进 `std::function`，这个优势消失：

- `std::function` 是类型擦除容器，存储 Lambda 时要么 SBO（小对象优化，栈内存储），要么堆分配。
- `std::function::operator()` 必然是一次间接调用（vtable 或函数指针），**无法内联**。
- 所以 "`auto f = [x](int){...}`" + 直接调用 ≠ "`std::function<int(int)> f = [x](int){...}`" + 调用。前者零成本，后者可能慢 5-20 倍。

性能敏感路径应该用 `auto` 保留 Lambda 的具体类型（通过模板传递），或用 C++23 的 `std::move_only_function`（无 SBO 约束，但也无 copy）。

### Z.4 函数式模式在 C++ 里的"真落地"

函数式编程的核心概念在 C++ 里都有映射：
- **高阶函数**：接受 Lambda 为参数的模板函数（如 `std::sort`、`std::transform`）。
- **闭包**：Lambda + 捕获。
- **不可变数据**：`const` 和值语义。
- **Pure 函数**：`noexcept + constexpr + const`。
- **Monad**：C++23 的 `std::expected` 和 Ranges 里的 pipe 都是 Monadic 风格。

但 C++ 的函数式不同于 Haskell：
- 不强制纯；
- 不默认惰性；
- 不自动内存管理（闭包的生命周期需要你管理）。

这让 C++ 成为"**可以函数式，但不强制函数式**"的语言——你可以在需要的地方用函数式表达力，在性能关键处回退到命令式。这种灵活性是 C++ 的优势，也是它更难学的原因。

### Z.5 为什么 C++20 要加"泛型 Lambda"

```cpp
auto print = [](const auto& x) { std::cout << x; };  // 泛型
```

看似只是"模板 Lambda"的便利写法，但背后意义是**让 Lambda 成为真正的一等函数对象**。泛型 Lambda 让你能写：

```cpp
std::ranges::transform(v1, v2, out.begin(), [](auto a, auto b) { return a + b; });
```

不用关心容器元素类型。这种"不关心具体类型但保留编译期检查"的能力，是 C++ 泛型编程向用户端延伸的重要一步。C++20 进一步加入显式模板参数语法：

```cpp
[]<typename T>(const std::vector<T>& v) { /* 能访问 T */ };
```

让 Lambda 的表达力完全赶上普通模板函数。
