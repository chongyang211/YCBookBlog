...
- [2. 架构概览](#2-架构概览)
  - [2.1 UB 的三级分类——核心语言 UB / 库 UB / 实现定义行为](#21-ub-的三级分类核心语言-ub--库-ub--实现定义行为)
  - [2.2 UB 不是「随机结果」——是「编译器有权做任何假设」](#22-ub-不是随机结果是编译器有权做任何假设)
...
### 2.1 UB 的三级分类——核心语言 UB / 库 UB / 实现定义行为
...
> **下一篇**：59 篇——UB 图鉴——C++ 最深的地狱——完全照亮。下一篇进入终章 [60.C++设计哲学回望](60.C++设计哲学回望.md)——零开销抽象、不为不用的付费、值语义 vs 引用语义——回顾这本 60 篇专栏贯穿的核心设计理念。
