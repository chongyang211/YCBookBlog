...
- [七、高级同步原语](#七高级同步原语)
- [八、线程池实现](#八线程池实现)
- [九、并发编程中的常见问题](#九并发编程中的常见问题)
- [十、std::async与std::future](#十stdasync与stdfuture)
- [十一、协程与并发 (C++20)](#十一协程与并发-c20)
- [十二、总结](#十二总结)
...
在多核处理器时代，并发编程已经从"可选技能"变成了"必备能力"。C++11开始引入了完整的并发编程支持，包括线程、互斥量、条件变量、原子操作和内存模型。然而，并发编程中最难理解、也最容易出错的部分是**内存序（Memory Order）**。本文将从硬件层面出发，由浅入深地讲解C++并发编程的核心原理。
