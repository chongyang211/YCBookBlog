...
**测试票务系统初始化**：先验证构造函数能正确创建票务系统，并且查询接口返回的票数与初始化值一致。

```cpp
int main() {
    Logger logger("ticket_system.log");
    // 创建一个总票数为 100 的票务系统
    TicketSystem ticketSystem(100, logger);

    // 校验初始票数
    std::cout << "Total tickets: "     << ticketSystem.getTotalTickets()     << std::endl;
    std::cout << "Remaining tickets: " << ticketSystem.getRemainingTickets() << std::endl;
    return 0;
}
```
...
构造日志成功打印，且总票数 / 剩余票数都为 100，说明票务系统的初始化和查询接口都正常工作。
...
**测试配置 + 票务联动**：把配置文件读取与票务系统串起来，验证 `config.txt` 中的 `total_tickets` 能真正驱动票务系统初始化。
...
int main() {
    ...
    // 2. 初始化日志
    Logger logger("ticket_system.log");
    // 3. 用配置文件中的 total_tickets 创建票务系统
    TicketSystem ticketSystem(config.get("total_tickets"), logger);

    std::cout << "Total tickets from config: "
              << ticketSystem.getTotalTickets() << std::endl;
    return 0;
}
```
...
config key : max_tickets_per_customer , value : 5
[INFO] Ticket system created with 100 tickets
Total tickets from config: 100
```

把 `config.txt` 中的 `total_tickets` 改成 `200`，再次运行，最后一行会变成 `Total tickets from config: 200`——**不需要重新编译，配置即生效**。
...
**测试窗口创建**：验证 `TicketWindow` 能正确构造，并且初始的售票数量为 0。
...
int main() {
    ...
    // 创建一个售票窗口（注意：必须传引用！）
    TicketWindow window("Window 1", ticketSystem, logger);
    std::cout << "Initial tickets sold: " << window.getTicketsSold() << std::endl;
    return 0;
}
```
...
**测试单窗口售票线程**：让一个 `TicketWindow` 跑起来，看它是否能在后台不断卖票直到清空票仓。

```cpp
int main() {
    Logger logger("ticket_system.log");
    TicketSystem ticketSystem(20, logger);   // 20 张票
    TicketWindow window("Window 1", ticketSystem, logger);

    window.start();
    // 主线程睡 2 秒，让窗口线程有时间把票卖完
    std::this_thread::sleep_for(std::chrono::seconds(2));
    window.stop();

    std::cout << "Tickets sold by window: " << window.getTicketsSold() << std::endl;
    std::cout << "Remaining tickets: "     << ticketSystem.getRemainingTickets() << std::endl;
    return 0;
}
```

打印结果如下所示（每次运行的具体张数会因随机 1~5 而不同）：

```text
[INFO] Ticket system created with 20 tickets
[INFO] Ticket window Window 1 created
[INFO] Window 1 sold 4 tickets. Remaining tickets: 16
[INFO] Window 1 sold 3 tickets. Remaining tickets: 13
[INFO] Window 1 sold 5 tickets. Remaining tickets: 8
[INFO] Window 1 sold 2 tickets. Remaining tickets: 6
[INFO] Window 1 sold 4 tickets. Remaining tickets: 2
[WARNING] Window 1 failed to sell 5 tickets. Not enough tickets available.
[WARNING] Window 1 failed to sell tickets. Not enough tickets available.
Tickets sold by window: 18
Remaining tickets: 2
```

可以看到窗口线程一直在循环售票，剩余票数从 20 逐步下降到 2，最后因为请求张数大于剩余而退出循环——这正是 `start()` 中 `break` 分支被触发的效果。
...
**测试多窗口并发售票**：跑一个最小可运行的多窗口示例，亲眼看看多个窗口"抢票"的效果。

```cpp
int main() {
    Logger logger("ticket_system.log");
    TicketSystem ticketSystem(30, logger);   // 30 张票
    ...
    // 等 2 秒让窗口们把票分掉
    std::this_thread::sleep_for(std::chrono::seconds(2));
    for (auto& w : windows) w->stop();

    // 输出每个窗口卖了多少
    for (auto& w : windows) {
        std::cout << "tickets sold = " << w->getTicketsSold() << std::endl;
    }
    std::cout << "Remaining: " << ticketSystem.getRemainingTickets() << std::endl;
    return 0;
}
```

打印结果如下所示（每次运行各窗口分到的票数会有差异，但 3 个窗口加起来不会超过 30）：

```text
[INFO] Ticket system created with 30 tickets
[INFO] Ticket window Window 1 created
[INFO] Ticket window Window 2 created
[INFO] Ticket window Window 3 created
[INFO] Window 1 sold 4 tickets. Remaining tickets: 26
[INFO] Window 2 sold 3 tickets. Remaining tickets: 23
[INFO] Window 3 sold 5 tickets. Remaining tickets: 18
[INFO] Window 1 sold 2 tickets. Remaining tickets: 16
...
[WARNING] Window 2 failed to sell 4 tickets. Not enough tickets available.
[WARNING] Window 3 failed to sell 3 tickets. Not enough tickets available.
tickets sold = 11
tickets sold = 9
tickets sold = 8
Remaining: 2
...
> 🧠 **观察重点**：把这段代码里的 `std::lock_guard` / `std::unique_lock` 临时全部注释掉再跑——你会看到"剩余票数 + 各窗口售出 ≠ 总票数"的诡异结果，这就是**没加锁时的数据竞争**。这个反例对理解锁的必要性极有帮助。
...
**测试单顾客购票**：让一个 `Customer` 去买几张票，看看日志和剩余票数是否符合预期。

```cpp
int main() {
    Logger logger("ticket_system.log");
    TicketSystem ticketSystem(10, logger);   // 10 张票

    Customer alice("Alice", ticketSystem, logger);
    alice.buyTickets(3);   // 买 3 张
    alice.buyTickets(8);   // 还想买 8 张，应该失败（只剩 7 张）
...
```text
[INFO] Ticket system created with 10 tickets
[INFO] Customer Alice created
[INFO] Alice is trying to buy 3 tickets.
[INFO] Alice sold 3 tickets. Remaining tickets: 7
[INFO] Alice successfully bought 3 tickets.
[INFO] Alice is trying to buy 8 tickets.
... (因为没票了，cv.wait 会阻塞，直到本进程结束 / 加票后才会唤醒)
```

> ⚠️ **观察重点**：第二次 `buyTickets(8)` 看似"应该立刻失败返回"，但实际会卡住——这是因为 `sellTicket` 中的 `cv.wait` 条件是 `remainingTickets >= num || remainingTickets == 0`，剩余 7 张时**两个条件都不满足**，线程会**永远等待**。这其实是当前设计的一个潜在 Bug，§9 的错误处理与超时机制就是为了处理这类情况，读者可以提前思考解法。
...
**测试批量创建并购票**：让 5 个顾客依次买票，验证容器化管理 + 顺序购票流程都正常。
...
```text
[INFO] Ticket system created with 20 tickets
[INFO] Customer Customer 1 created
[INFO] Customer Customer 2 created
[INFO] Customer Customer 3 created
[INFO] Customer Customer 4 created
[INFO] Customer Customer 5 created
[INFO] Customer 1 is trying to buy 2 tickets.
[INFO] Customer 1 sold 2 tickets. Remaining tickets: 18
[INFO] Customer 1 successfully bought 2 tickets.
[INFO] Customer 2 is trying to buy 2 tickets.
[INFO] Customer 2 sold 2 tickets. Remaining tickets: 16
[INFO] Customer 2 successfully bought 2 tickets.
...
[INFO] Customer 5 sold 2 tickets. Remaining tickets: 10
[INFO] Customer 5 successfully bought 2 tickets.
Remaining: 10
```

5 位顾客一共买走 10 张票，剩余 10 张——结果与预期完全一致。**顾客是顺序购票的**（5.3 节 `join()` 选择的效果），日志非常工整。
...
**测试预警与动态加票**：构造一个"票数已经低于 1/4"的场景，看监控线程是否每秒都会打出 WARNING；再调用 `addTickets` 看预警是否随之消失。

```cpp
int main() {
    Logger logger("ticket_system.log");
    TicketSystem ticketSystem(40, logger);   // 总票数 40，1/4 阈值 = 10

    ticketSystem.monitorTickets();           // 启动监控线程

    // 让票数掉到阈值以下
    ticketSystem.sellTicket(35, "Test");     // 卖掉 35 张，剩 5 张 (< 10)
    std::this_thread::sleep_for(std::chrono::seconds(3));

    // 动态加票
    ticketSystem.addTickets(50);             // 加 50 张，剩 55 张 (> 10)
    std::this_thread::sleep_for(std::chrono::seconds(3));
...
```text
[INFO] Ticket system created with 40 tickets
[INFO] Test sold 35 tickets. Remaining tickets: 5
[WARNING] Warning: Only 5 tickets remaining!
[WARNING] Warning: Only 5 tickets remaining!
[WARNING] Warning: Only 5 tickets remaining!
[INFO] Added 50 tickets. Remaining tickets: 55
（之后不再有 WARNING 出现）
```

监控线程每秒检查一次，剩 5 张时连续打三次警告；调用 `addTickets(50)` 之后剩余票变 55，超过 1/4 阈值，警告就停止了。
...
**测试 VIP 通道**：让一个 VIP 窗口和一个普通窗口同时运行，验证两者会共享同一个票仓、并能按各自身份记录日志。

```cpp
int main() {
    Logger logger("ticket_system.log");
    TicketSystem ticketSystem(20, logger);

    TicketWindow normal("Window 1",   ticketSystem, logger);
    TicketWindow vip   ("VIP Window", ticketSystem, logger);

    normal.start();
    vip.start();

    // 顾客通过 VIP 窗口直接走 sellVIP
    vip.sellVIP(3, "VIPCustomerA");
    vip.sellVIP(2, "VIPCustomerB");

    std::this_thread::sleep_for(std::chrono::seconds(2));
    normal.stop();
    vip.stop();

    std::cout << "Normal window sold: " << normal.getTicketsSold() << std::endl;
    std::cout << "VIP    window sold: " << vip.getTicketsSold()    << std::endl;
    std::cout << "Remaining: " << ticketSystem.getRemainingTickets() << std::endl;
    return 0;
}
```

打印结果如下所示（每次运行的具体数值会随机变化）：

```text
[INFO] Ticket system created with 20 tickets
[INFO] Ticket window Window 1 created
[INFO] Ticket window VIP Window created
[INFO] VIP Window (VIP) sold 3 tickets. Remaining tickets: 17
[INFO] VIP customer VIPCustomerA bought 3 tickets from VIP Window
[INFO] VIP Window (VIP) sold 2 tickets. Remaining tickets: 15
[INFO] VIP customer VIPCustomerB bought 2 tickets from VIP Window
[INFO] Window 1 sold 4 tickets. Remaining tickets: 11
[INFO] VIP Window sold 5 tickets. Remaining tickets: 6
...
Normal window sold: 9
VIP    window sold: 11
Remaining: 0
```

可以看到 VIP 窗口同时支持两种售票方式：① 通过 `start()` 开启的随机循环售票；② 通过 `sellVIP()` 直接处理某位 VIP 顾客的请求。日志里出现的 `VIP Window (VIP)` 字样就是 `sellVIP` 方法在 `windowName` 后追加的标识。
...
**测试异常恢复**：手动构造一次 `recoverTickets` 调用，模拟"扣了票但出票失败"的场景，验证补偿逻辑能让票数回到正确状态。
...
int main() {
    ...
    // 1. 正常卖掉 4 张，剩余 6
    ticketSystem.sellTicket(4, "Window 1");
    std::cout << "After sell: " << ticketSystem.getRemainingTickets() << std::endl;

    // 2. 模拟"出票失败"——把 2 张票补回来
    ticketSystem.recoverTickets(2);
    std::cout << "After recover: " << ticketSystem.getRemainingTickets() << std::endl;
    return 0;
}
```
...
```text
[INFO] Ticket system created with 10 tickets
[INFO] Window 1 sold 4 tickets. Remaining tickets: 6
After sell: 6
[WARNING] Recovered 2 tickets. Total now: 8
After recover: 8
```

剩余票数从 6 → 8，准确补回了 2 张。这就是窗口线程 `catch` 块里 `ticketSystem.recoverTickets(numTickets)` 的执行效果——保证**任何异常都不会导致票"凭空消失"**。
...
**测试性能监控**：手动喂一些"成功 / 失败"事件给 `PerformanceMonitor`，看它输出的成功率和 TPS 是否符合预期。
...
int main() {
    ...
    // 模拟 7 次成功 + 3 次失败
    for (int i = 0; i < 7; ++i) monitor.recordSale(true);
    for (int i = 0; i < 3; ++i) monitor.recordSale(false);

    // 让 startTime 与 endTime 拉开 1 秒以上，便于看到非零 TPS
    std::this_thread::sleep_for(std::chrono::seconds(2));
    monitor.report();
    return 0;
}
...
成功率（7/10 = 70%）和 TPS（10 笔 / 2 秒 = 5）都被正确统计出来。把 `PerformanceMonitor::recordSale` 嵌入 `TicketSystem::sellTicket` 的 `if/else` 两个分支，就能在真实多线程跑批后拿到一份完整的性能报告。
