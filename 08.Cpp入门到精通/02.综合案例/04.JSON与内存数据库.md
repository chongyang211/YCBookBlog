# 第四章：C++ JSON 序列化器 + 内存数据库（现代 C++ 内存模型）
...
本案例**严格按照真实工程师的开发节奏推进**，不会一上来甩你 1100 行代码让你抄。我们的节奏是这样的：

```text
阶段 ① 异常体系 + JsonNode 骨架（02-03 节） · 60 min
   └ Step 1.1: 自定义 3 个异常类继承 runtime_error
   └ Step 1.2: variant<6 类型> 包成 JsonNode + 工厂函数

阶段 ② 字面量解析（04 节 4.1-4.2） · 90 min
   └ Step 2.1: 跳过空白 + 解析 null/true/false
   └ Step 2.2: 解析数字（含负数/小数/科学记数法）
   └ Step 2.3: 解析字符串（含 \" \\ \n 转义）

阶段 ③ 容器解析（04 节 4.3） · 120 min  【高峰：递归】
   └ Step 3.1: 解析 array —— 第一次写递归下降
   └ Step 3.2: 解析 object —— 复用 array 思路
   └ Step 3.3: 用 sample.json 端到端跑通

阶段 ④ 序列化输出（05 节） · 60 min
   └ Step 4.1: 无缩进版（最小可工作）
   └ Step 4.2: 加缩进与换行（pretty-print）

阶段 ⑤ KvDatabase 三大命令（06.1） · 90 min
   └ Step 5.1: get（路径查询）
   └ Step 5.2: set（创建中间路径）
   └ Step 5.3: del（递归清理空节点）

阶段 ⑥ 路径查询 a.b.c（06.2） · 60 min
   └ Step 6.1: 按 . 切分路径 token
   └ Step 6.2: 逐层下钻 + optional 表达"找不到"

阶段 ⑦ 持久化（06.3） · 60 min
   └ Step 7.1: 启动时 load + 解析失败兜底
   └ Step 7.2: 退出时 save（含格式化）

阶段 ⑧ CLI + REPL（07 节） · 60 min
   └ 把 8 条命令串到一个交互循环里
```
...
>
> ⚠️ **本案例独有的"现代 C++ 反复横跳"**：阶段 ① 你刚学会 `variant`，阶段 ② 立刻就要用 `std::get_if`；阶段 ③ 你又要回头改 ① 的工厂函数加 `std::move`。这种"**学一点 → 立刻反复用 → 强化记忆**"的设计，是新手区分 C 风格 C++ 与现代 C++ 的关键时刻。
>
> ✅ **每个阶段的结构**（你在正文里会反复看到）：
>
> ```text
...
- [01.项目需求和功能](#01项目需求和功能)
  - [1.1 业务背景](#11-业务背景)
  - [1.2 关键决策：为什么用 variant](#12-关键决策为什么用-variant) 【🔑】
  - [1.3 涉及知识点](#13-涉及知识点)
- [02.异常体系设计](#02异常体系设计) 【阶段①】
- [03.JsonNode：用 variant 表达 6 种类型](#03jsonnode用-variant-表达-6-种类型) 【阶段①·OOP现代版】
  - [3.1 类型定义](#31-类型定义)
  - [3.2 类型安全的访问](#32-类型安全的访问)
  - [3.3 工厂函数](#33-工厂函数)
- [04.递归下降解析器](#04递归下降解析器) 【阶段②③·递归高峰⭐】
  - [4.1 字面量与数字](#41-字面量与数字)
  - [4.2 字符串与转义](#42-字符串与转义)
  - [4.3 数组与对象（递归）](#43-数组与对象递归)
- [05.序列化器（缩进美化输出）](#05序列化器缩进美化输出) 【阶段④】
- [06.KvDatabase 内存数据库](#06kvdatabase-内存数据库) 【阶段⑤⑥⑦】
  - [6.1 三大命令 get/set/del](#61-三大命令-getsetdel)
  - [6.2 路径查询 a.b.c](#62-路径查询-abc)
  - [6.3 启动加载 / 退出保存](#63-启动加载--退出保存)
- [07.CLI 工具与 REPL](#07cli-工具与-repl) 【阶段⑧】
- [08.项目总结与现代 C++ 特性盘点](#08项目总结与现代-c-特性盘点)
- [09.项目技术思考](#09项目技术思考)
  - [9.1 为什么 unique_ptr 不需要拷贝构造](#91-为什么-unique_ptr-不需要拷贝构造)
  - [9.2 异常 vs 错误码](#92-异常-vs-错误码)
  - [9.3 为什么 string_view 危险](#93-为什么-string_view-危险)
- [10.衔接与延伸](#10衔接与延伸)
