...
> 本章是卷二的**集大成之作**，也是整本《C++ 入门到精通》的"毕业设计"。前 11 个案例每篇只覆盖 2-3 章，本案例**一口气串起卷一 16 章**——从基础语法到 `std::variant`，从智能指针到 `std::jthread`，从异常体系到预处理器宏，全部在同一个项目里落地。完成它之后，你会得到一个能跑、能落盘、能并发、能过期的"迷你 Redis"，更重要的是：**你会真正理解"C++ 工程"是怎样把这些散点搭成一座可维护的房子的**。
>
...
2. **`steady_clock` 不是 `system_clock`**：`system_clock` 会被用户改系统时间影响（你 SET 完用户调慢钟，TTL 全乱套），`steady_clock` 单调递增不会倒退，TTL 必须用它。
...
### 15.1 与上一案例（11.JSON 序列化器）的衔接

**11.JSON** 教会你"用 `variant` 表达异构数据"，**12.MiniKV** 教会你"用 `variant` 做业务存储"。两个案例的 Value 抽象一脉相承——你甚至可以直接把 11 章的 `mj::Value` **拿来替换** MiniKV 的 `mkv::Value`，立刻获得"存任意 JSON 到 KV"的能力。这就是良好抽象的复用价值。

留给你做的延伸：在 MiniKV 里加一条命令 `JSET key json_string`，把 JSON 字符串 parse 成 `mj::Value` 存进去；`JGET key path` 按 JSON Path（如 `user.name`）取值。
