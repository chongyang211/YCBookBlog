...
本案例采用**真实工程师的开发节奏**——**小步快跑：写一点 → 编译 → 运行 → 看到效果 → 再写下一点**。
...
**强烈建议**：每个 Step 后都**自己亲手敲键盘 → 编译 → 运行 → 看到效果**，再进入下一个 Step。**不要直接复制整段代码**——那样和直接抄答案没区别，看似学得快，实际什么也没记住。
...
- 05.多态机制实测
  ...
  - 5.3 虚函数表 vtable 原理图
  - 5.4 不加 virtual 会怎样
- 06.设计 Bank 管理类（**渐进式开发节奏：写空壳 → 跑通 → 填肉 → 再跑**）
  - 6.1 灵魂三问：Bank 类到底要管什么？
  - 6.2 Step 5.1 —— 先写最小骨架（让 main → Bank 管道通起来）
  - 6.3 Step 5.2 —— openAccount 真正生出账户
  - 6.4 Step 5.3 —— deposit 时自然催生 findAccount 辅助函数
  - 6.5 Step 5.4 —— withdraw 复用 findAccount
  - 6.6 Step 5.5 —— queryBalance 一行搞定
  - 6.7 Step 5.6 —— transfer 第一次遇到事务问题
  - 6.8 Step 5.7 —— showAll 多态的高光时刻
  - 6.9 Bank.h 完整速查（验收对照）
- 07.持久化层 FileManager
  - 7.1 为什么需要"类型标签"
  - 7.2 保存到 CSV
  - 7.3 从 CSV 读取
