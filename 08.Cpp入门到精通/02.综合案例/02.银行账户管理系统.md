...
本章是综合案例的**第二关·OOP登场**——从 [01.通讯录](./01.学生通讯录系统.md) 的"`struct Person + Person[100]` 一刀切"，跃迁到 `class + 虚函数 + std::vector<Account*>`的工业级写法。本案例会做**三件事**：
...
- [03.设计抽象基类 Account](#03设计抽象基类-account) 【阶段②OOP灵魂】
  ...
  - [3.4 关键概念：纯虚函数、virtual、虚析构](#34-关键概念纯虚函数virtual虚析构)
- [04.设计三个派生类](#04设计三个派生类) 【阶段③】
  - [4.1 NormalAccount 普通账户](#41-normalaccount-普通账户)
  - [4.2 VipAccount VIP 账户](#42-vipaccount-vip-账户)
  - [4.3 SavingAccount 储蓄账户](#43-savingaccount-储蓄账户)
  - [4.4 测试三个子类各自创建](#44-测试三个子类各自创建)
- [05.多态机制实测](#05多态机制实测) 【阶段④高光时刻⭐】
  - [5.1 用基类指针存异构对象](#51-用基类指针存异构对象)
  - [5.2 同一接口不同表现](#52-同一接口不同表现)
  - [5.3 虚函数表 vtable 原理图](#53-虚函数表-vtable-原理图)
  - [5.4 不加 virtual 会怎样](#54-不加-virtual-会怎样)
- [06.设计 Bank 管理类](#06设计-bank-管理类) 【阶段⑤业务高峰】
  - [6.1 先想清楚 Bank 要管什么](#61-先想清楚bank-类到底要管什么)
  - [6.2 Step 5.1：先写一个最小能跑的 Bank 骨架](#62-step-51先写一个最小能跑的-bank-骨架)
  - [6.3 Step 5.2：让 openaccount 真正生出一个账户](#63-step-52让-openaccount-真正生出一个账户)
  - [6.4 Step 5.3：再加存款功能 —— 但你会发现一个隐藏问题](#64-step-53再加存款功能--但你会发现一个隐藏问题)
  - [6.5 Step 5.4：取款功能 —— 复用 findaccount](#65-step-54取款功能--复用-findaccount)
  - [6.6 Step 5.5：查询余额 —— 一行就够](#66-step-55查询余额--一行就够)
  - [6.7 Step 5.6：转账功能 —— 第一次遇到"事务"问题](#67-step-56转账功能--第一次遇到事务问题)
  - [6.8 Step 5.7：显示所有账户 —— 多态的高光时刻](#68-step-57显示所有账户--多态的高光时刻)
  - [6.9 完整 Bank.h 速查（确认你的代码长这样）](#69-完整-bankh-速查确认你的代码长这样)
...
- [09.项目总结分析](#09项目总结分析)
  ...
  - [9.3 代码优缺点](#93-代码优缺点)
- [10.项目技术思考](#10项目技术思考)
  - [10.1 为什么基类析构必须是 virtual](#101-为什么基类析构必须是-virtual)
  - [10.2 为什么用 Account* 而不是 Account](#102-为什么用-account-而不是-account)
...
### 6.1 先想清楚：Bank 类到底要"管"什么？

在敲键盘之前，先停 30 秒，问自己**三个灵魂问题**：

> ❓ **Bank 类需要保存什么数据？**
> 答：一堆账户。所以需要一个容器 → `vector<Account*>`（前面 5.1 节我们已经验证了这个用法）
>
> ❓ **Bank 类对外提供什么动作？**
> 答：开户、存款、取款、查询、转账、显示……每个动作就是一个**成员函数**。
>
> ❓ **现在第一步要做哪一个？**
> 答：**开户**。因为没账户，存款、取款、查询都没意义——选**最底层的依赖项**先做。

🔑 **这就是工程师的思维**：先选**最基础的功能**，让其他功能有"可依附的根"。

### 6.2 Step 5.1：先写一个**最小能跑**的 Bank 骨架
...
### 6.3 Step 5.2：让 `openAccount` 真正"生"出一个账户
...
> "我要不要顺便把'保存到文件'也写了？"
>
> **不要！** 真实工程师每次只解决一个问题。**保存文件是另一条思路链**，混进来你的脑子会打结。

### 6.4 Step 5.3：再加"存款"功能 —— 但你会发现一个隐藏问题
...
### 6.5 Step 5.4：取款功能 —— 复用 findAccount
