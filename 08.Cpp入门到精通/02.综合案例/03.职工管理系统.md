...
1. **抽象类**：
   `Worker` 是一个抽象类（至少包含一个纯虚函数），因此不能直接实例化。

2. **非虚析构函数**：
   如果基类（`Worker`）的析构函数不是虚函数，那么通过基类指针删除派生类对象时，只会调用基类的析构函数，而不会调用派生类的析构函数。这会导致派生类的资源未被正确释放，从而引发内存泄漏或未定义行为。

3. **警告信息**：
   编译器警告你，`delete` 操作符被用于一个抽象类指针，但该类的析构函数不是虚函数。
