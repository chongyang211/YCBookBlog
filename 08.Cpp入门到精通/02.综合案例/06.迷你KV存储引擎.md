...
### Step 1.3 · 用 `std::string_view` 做零拷贝切词
...
### Step 2.1 · 用 `std::variant` 定义 5 种 Value
...
### Step 2.2 · 用 `std::visit` 写 `formatValue` 输出
...
### Step 2.3 · 写 `valueFromToken` 类型推断
...
### Step 3.2 · `IndexMap` 接进 main，让 SET/GET 真生效
...
### Step 5.4 · 把 Command 派生类接上 Store
...
### Step 7.4 · 应用守则：fail-fast + noexcept 标注 + 分层 catch
...
### Step 8.5 · ⚠️ 造 BUG → 修复：AOF 半行损坏怎么办
...
### Step 9.1 · ⚠️ 造 BUG → 修复：`cerr` 多原始 → 升级 KV_LOG
...
### Step 9.2 · 宏的几个“必须” + do-while(0) 造 BUG
...
### Step 10.1 · ⚠️ 造 BUG：计数器从 2000 变 1837
...
### Step 10.2 · `std::mutex` + `std::lock_guard` 修复
...
### Step 10.3 · `std::shared_mutex` 读写锁优化
...
### Step 11.2 · ⚠️ 造 BUG：裸 `std::thread` 忘 join → `terminate`
