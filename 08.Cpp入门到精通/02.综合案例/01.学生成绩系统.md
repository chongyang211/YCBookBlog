...
> 本章是综合案例的**第一关**——用卷一第 2-7 章（基础语法 / 结构体 / 函数 / 指针）完成一个菜单驱动的通讯录。代码故意保留 C 风格的 `struct` + 定长数组写法，是为了在第 02 案例"银行账户管理"里看到**类化后的同一问题**，体会封装与 STL 容器带来的价值。
...
> **命名说明**：本案例主线是"通讯录"（添加/显示/删除/查找/修改/清空联系人），文件名 `01.学生成绩系统.md` 是**第一卷最早的命名**，保留沿用。如果你更关心"学生"，可以把字段 `phone/address` 换成 `subjectA/subjectB/subjectC`，其他逻辑完全相同——这正是 **delta 2**（见末尾"延伸挑战"）。
...
下一案例 [02.银行账户管理](./02.银行账号管理.md) 会做三件"升级"，请留意对比：
...
| 持久化 | ❌ 程序退出即丢失 | ✅ CSV 文件 `ofstream` / `ifstream` |
| 错误处理 | 打印 `cout << "xxx"` + return | 同上 + 空指针检查 |

换句话说，**02 会让你第一次亲身体会到"封装+容器+文件"比"裸 struct+数组"多出的价值**。
...
- ➡ 下一案例：[02.银行账户管理](./02.银行账号管理.md) —— 从 `struct` 升级到 `class`，从定长数组升级到 `vector`，从内存临时数据升级到 CSV 文件持久化
