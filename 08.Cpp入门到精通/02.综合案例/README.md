...
| 04 | [JSON 序列化器与内存数据库](./04.JSON与内存数据库.md) | ★★★★☆ | 12 h | 现代内存模型：`variant` + 递归 + **智能指针** + 异常体系 + `optional`/`string_view` | **第 8、11-12、14、18 章** |
| 05 | [多线程订单系统与线程池](./05.多线程订单与线程池.md) | ★★★★★ | 14 h | 并发栈：`thread`/`mutex`/`condition_variable`/`atomic`/`future`/`jthread` + 策略模式 | **第 11-12、15-16 章** |
| 06 | [迷你 KV 存储引擎](./06.迷你KV存储引擎.md) 🎓 | ★★★★★ | 18 h | **毕业设计**：命令模式 + AOF 持久化 + 读写锁 + jthread 后台 + 宏 + 模板 + `<=>` + 内置单测 | **第 2-18 全章** |
