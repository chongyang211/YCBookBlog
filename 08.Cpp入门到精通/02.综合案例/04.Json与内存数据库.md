...
❓ **为什么要在阶段 ① 第一个写它？**
- 答：**地基性决定时机**。Parser/Node/Db 都会 throw，**先有异常类型再写业务**，可以避免"中途回头补"的反复改头文件。这和 03 案例 §5.2 先写最小 CampusSystem 骨架是同一个思想——**先打通最底层的依赖项**。

❓ **现在第一步要先做哪一个？**
- 答：**先写 JsonError 基类**——4 个子异常都从它派生，没有它子类无从下手。
...
void demo(int which) {
    switch (which) {
        case 1: throw JsonParseError("bad token", 3, 12);
        case 2: throw TypeMismatchError("string", "number");
        case 3: throw KeyNotFoundError("user.name");
        case 4: throw JsonIoError("cannot open kv.json");
    }
}
...
class JsonNode;                                    // 前置声明
using JsonNodePtr = std::unique_ptr<JsonNode>;     // 别名：节点智能指针

class JsonNode {
public:
    // 数组/对象都用 unique_ptr 装节点 → 树结构 RAII
...
并在 `JsonNode.h` 中声明 Array/Object 访问器：
