# 第五章：多线程订单系统与线程池（并发栈集大成）

> 本章是**并发编程的"全套训练"**。前 4 个案例都是单线程程序，从本章开始我们正式踏入"多核时代"——理解为什么"加锁"看起来简单，写错却会导致死锁、数据竞争、活锁等"幽灵 BUG"。
>
> 本案例**融合原 3 个案例**（票务购买 / 轮询管理 / 线程池）为一个完整的"多线程订单系统"，一次练遍 C++ 并发的全部核心：
>
...
>
> 学完本章，你将真正理解为什么"会写多线程"在工业界是个能涨薪 30% 的硬技能。
...
| 阶段 | 产物 | 时长 |
|------|------|------|
| ① BlockingQueue（生产消费基础） | 90 min |
| ② ThreadPool 组件（提交任务，自动调度） | 120 min |
| ③ OrderManager + 读写锁（多读少写） | 60 min |
| ④ Product 库存（atomic 防超卖） | 60 min |
| ⑤ 策略模式：普通/秒杀/批量下单 | 90 min |
| ⑥ Logger 线程安全日志 | 30 min |
| ⑦ CLI 交互 + benchmark 压测 | 90 min |
| ⑧ 调优：用 jthread / shared_mutex 优化 | 60 min |
