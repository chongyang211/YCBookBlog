...
阶段 ④ Product 库存 atomic（05 节） · 60 min · 3 Step  【高峰：造 BUG→修复 ⭐⭐⭐】
...
>
> 三次"踩坑→修复"流程，让你真正记住 mutex / atomic / lock_guard **为什么**存在。
>
> ✅ **每个阶段的结构**（你在正文里会反复看到）：
>
> ```text
...
### Step 1.2：双线程"作死"测试 → 看到 race condition
...
### Step 1.4：加 condition_variable 实现阻塞 pop + shutdown
...
### Step 2.1：最小骨架（无 submit，先把工人启动起来）
...
### Step 2.2：理解 packaged_task + future（先单独玩一下）
...
### Step 2.3：实现真正的模板 submit（异步返回任意类型）
...
### Step 3.3：升级 shared_mutex —— 读吞吐翻倍
...
│ 预计耗时：60 min · 3 Step（造 BUG → 看 BUG → 修 BUG）      │
└──────────────────────────────────────────────────────────┘
```
...
### Step 5.3：BatchStrategy（分批 + future 收集）
...
### Step 6.2：加 mutex + 折叠表达式实现优雅 Logger
...
### Step 7.2：benchmark.cpp 万 QPS 压测 + 一致性校验
