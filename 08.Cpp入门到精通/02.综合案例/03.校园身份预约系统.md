...
本案例**严格按照真实工程师的开发节奏推进**，不会一上来甲你 1200 行代码让你抄。我们的节奏是这样的：
...
- [01.项目需求和功能](#01项目需求和功能)
  ...
  - [1.3 身份驱动 + 容器分工](#13-设计思路身份驱动--容器分工)
  - [1.4 涉及知识点](#14-涉及知识点)
...
- [05.STL 容器选型与 CampusSystem 总管理器](#05stl-容器选型与-campussystem-总管理器) 【阶段③选型决策】
  - [5.1 为什么 map / multimap / set 各取所长](#51-为什么-map--multimap--set-各取所长)
  - [5.2 CampusSystem.h 总览](#52-campussystemh-总览)
- [06.Student 业务](#06student-业务渐进式开发节奏) 【阶段④高峰·故意造bug⭐】
  - [6.1 灵魂三问](#61-灵魂三问)
  - [6.2 Step 4.1：先把 Student 菜单和 CampusSystem 管道打通](#62-step-41先把-student-菜单和-campussystem-管道打通)
  - [6.3 Step 4.2：填 listRooms —— 范围 for + 结构化绑定首秀](#63-step-42填-listrooms--范围-for--结构化绑定首秀)
  - [6.4 Step 4.3：reserveRoom —— 第一次遇到两个容器同步的真问题](#64-step-43reserveroom--第一次遇到两个容器同步的真问题)
  - [6.5 Step 4.4：cancelReservation —— find_if + lambda 首秀](#65-step-44cancelreservation--find_if--lambda-首秀)
  - [6.6 Step 4.5：signupSpeech —— multimap 第一次登场](#66-step-45signupspeech--multimap-第一次登场)
- [07.Teacher 业务](#07teacher-业务) 【阶段⑤】
  - [7.1 审核待审预约](#71-审核待审预约)
  - [7.2 演讲评分 + lambda 排序](#72-演讲评分--lambda-排序)
  - [7.3 算法函数 find_if / count_if](#73-算法函数-find_if--count_if)
- [08.Admin 业务](#08admin-业务) 【阶段⑥】
  - [8.1 添加用户/机房](#81-添加用户机房)
  - [8.2 数据统计（accumulate + lambda）](#82-数据统计accumulate--lambda)
...
- [10.项目总结分析](#10项目总结分析)
- [11.项目技术思考](#11项目技术思考)
  - [11.1 map vs unordered_map 怎么选](#111-map-vs-unordered_map-怎么选)
  - [11.2 multimap vs map<K, vector<V>>](#112-multimap-vs-mapk-vectorv)
  - [11.3 shared_ptr 还是 raw pointer](#113-shared_ptr-还是-raw-pointer)
- [12.衔接与延伸](#12衔接与延伸)
...
### 6.2 Step 4.1：先把 Student 菜单和 CampusSystem 管道打通
...
### 6.3 Step 4.2：填 listRooms —— 范围 for + 结构化绑定首秀
...
### 6.4 Step 4.3：reserveRoom —— 第一次遇到"两个容器同步"的真问题
...
### 6.5 Step 4.4：cancelReservation —— find_if + lambda 首秀
...
### 6.6 Step 4.5：signupSpeech —— multimap 第一次登场
