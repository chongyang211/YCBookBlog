...
- 06.Student 业务（**渐进式开发节奏：每步都编译运行，故意制造 bug → 看到 → 修复**）
  - 6.1 灵魂三问：Student 类到底做什么？
  - 6.2 Step 4.1 —— 先打通多态菜单管道（空壳验证）
  - 6.3 Step 4.2 —— listRooms 范围 for + 结构化绑定首秀
  - 6.4 Step 4.3 —— reserveRoom **故意制造 bug → 修复**（vector+set 必须同步）
  - 6.5 Step 4.4 —— cancelReservation lambda + find_if 首秀
  - 6.6 Step 4.5 —— signupSpeech multimap 登场 + 选型反思点
- 07.Teacher 业务
  - 7.1 审核待审预约
  - 7.2 演讲评分 + lambda 排序
