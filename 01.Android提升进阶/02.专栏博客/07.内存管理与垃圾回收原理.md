...
#### 目录介绍
- [一、引言：Android为什么频繁OOM](#一引言android为什么频繁oom)
- [二、Android进程的内存模型](#二android进程的内存模型)
- [三、Java堆内存与ART虚拟机](#三java堆内存与art虚拟机)
- [四、ART的垃圾回收器](#四art的垃圾回收器)
- [五、GC算法原理详解](#五gc算法原理详解)
- [六、Concurrent Copying GC](#六concurrent-copying-gc)
- [七、对象的内存分配策略](#七对象的内存分配策略)
- [八、Native内存管理](#八native内存管理)
- [九、Bitmap的内存管理演进](#九bitmap的内存管理演进)
- [十、内存泄漏的原理与检测](#十内存泄漏的原理与检测)
- [十一、LowMemoryKiller与进程回收](#十一lowmemorykiller与进程回收)
- [十二、内存优化实战策略](#十二内存优化实战策略)
