# 04.Activity启动流程与生命周期原理

#### 目录介绍
- [一、引言：Activity的核心地位](#一引言activity的核心地位)
- [二、Activity启动的全链路分析](#二activity启动的全链路分析)
- [三、AMS调度Activity的核心逻辑](#三ams调度activity的核心逻辑)
- [四、进程创建与Application初始化](#四进程创建与application初始化)
- [五、Activity对象的创建过程](#五activity对象的创建过程)
- [六、生命周期的底层驱动机制](#六生命周期的底层驱动机制)
- [七、Activity的生命周期状态机](#七activity的生命周期状态机)
- [八、onSaveInstanceState与状态恢复](#八onsaveinstancestate与状态恢复)
- [九、Activity的启动模式原理](#九activity的启动模式原理)
- [十、TaskRecord与Back Stack管理](#十taskrecord与back-stack管理)
- [十一、Activity的窗口创建与关联](#十一activity的窗口创建与关联)
- [十二、Activity的销毁与内存回收](#十二activity的销毁与内存回收)
