...
- [一、引言：什么是卡顿](#一引言什么是卡顿)
- [二、渲染管线与帧率](#二渲染管线与帧率)
- [三、主线程卡顿的根因分析](#三主线程卡顿的根因分析)
- [四、卡顿监控方案原理](#四卡顿监控方案原理)
- [五、Looper Printer监控](#五looper-printer监控)
- [六、Choreographer帧监控](#六choreographer帧监控)
- [七、ANR的触发与监控](#七anr的触发与监控)
- [八、启动速度优化](#八启动速度优化)
- [九、布局渲染优化](#九布局渲染优化)
- [十、IO与线程优化](#十io与线程优化)
- [十一、Systrace与Perfetto分析](#十一systrace与perfetto分析)
- [十二、面试高频问题与深度分析](#十二面试高频问题与深度分析)
- [十三、总结](#十三总结)
...
1. 异步初始化
   // 将SDK初始化放到工作线程
   // 使用拓扑排序处理SDK依赖关系
   // 有依赖的按顺序初始化，无依赖的并行初始化
   
   class StartupTask {
       fun execute() {
           val tasks = sortByDependency(allTasks)
           
           // 必须主线程初始化的
           mainThreadTasks.forEach { it.run() }
           
           // 可以异步的，并行执行
           asyncTasks.forEach { threadPool.submit(it) }
...
2. 延迟初始化
   // 不需要在启动时初始化的SDK延迟到首次使用
   // 使用IdleHandler在主线程空闲时初始化
...
3. RecyclerView优化
   ├── DiffUtil计算差异，局部刷新
   ├── setHasFixedSize(true)避免多余measure
   ├── setItemViewCacheSize调整缓存
   ├── 预布局(prefetch)提前创建ViewHolder
   └── 嵌套RecyclerView共享RecycledViewPool

4. 异步布局填充
   └── AsyncLayoutInflater
       └── 在工作线程解析XML和创建View
       └── 注意：不能使用会访问主线程状态的View
```
...
// 方法2：使用Window.OnFrameMetricsAvailableListener (API 24+)
// 可以获取更详细的帧时间信息
```
