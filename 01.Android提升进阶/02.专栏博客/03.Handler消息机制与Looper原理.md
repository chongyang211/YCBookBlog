# 03.Handler消息机制与Looper原理

#### 目录介绍
- [一、引言：为什么需要Handler](#一引言为什么需要handler)
- [二、Handler机制的整体架构](#二handler机制的整体架构)
- [三、MessageQueue的数据结构与实现](#三messagequeue的数据结构与实现)
- [四、Looper的工作原理](#四looper的工作原理)
- [五、Handler的消息发送与处理](#五handler的消息发送与处理)
- [六、Message的复用池机制](#六message的复用池机制)
- [七、Native层的MessageQueue](#七native层的messagequeue)
- [八、epoll机制与线程休眠](#八epoll机制与线程休眠)
- [九、同步屏障与异步消息](#九同步屏障与异步消息)
- [十、IdleHandler机制](#十idlehandler机制)
- [十一、HandlerThread与IntentService](#十一handlerthread与intentservice)
- [十二、Handler导致的内存泄漏](#十二handler导致的内存泄漏)
- [十三、主线程Looper为什么不会ANR](#十三主线程looper为什么不会anr)
