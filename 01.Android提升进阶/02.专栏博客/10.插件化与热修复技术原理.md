...
#### 目录介绍
- [一、引言：动态化的需求与挑战](#一引言动态化的需求与挑战)
- [二、ClassLoader与类加载机制](#二classloader与类加载机制)
- [三、插件化的核心原理](#三插件化的核心原理)
- [四、Activity插件化方案](#四activity插件化方案)
- [五、资源加载与插件资源管理](#五资源加载与插件资源管理)
- [六、热修复的技术流派](#六热修复的技术流派)
- [七、类替换方案原理（Tinker）](#七类替换方案原理tinker)
- [八、底层替换方案原理（AndFix）](#八底层替换方案原理andfix)
- [九、Instant Run与代码热替换](#九instant-run与代码热替换)
- [十、资源热修复](#十资源热修复)
- [十一、SO库热修复](#十一so库热修复)
- [十二、动态化方案对比与选型](#十二动态化方案对比与选型)
...
学习插件化和热修复需要以下前置知识：
1. **Java ClassLoader机制**——理解类如何被加载
2. **Android组件管理**——理解AMS如何管理Activity
3. **Gradle构建系统**——理解APK的构建过程
4. **反射和动态代理**——插件化的基本手段

建议从热修复入手（原理更简单），理解dexElements的加载机制，再进阶到插件化的Activity管理和资源隔离。
