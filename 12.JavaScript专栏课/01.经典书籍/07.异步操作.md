...
**单线程**：JavaScript 引擎（如 V8）只有一个主线程来执行代码。
...
JavaScript 最初被设计为一种用于浏览器的脚本语言，主要用于处理用户交互和 DOM 操作。单线程模型的设计有以下优点：

- **简单性**：单线程模型避免了多线程编程中的复杂问题，如死锁、竞态条件等。
- **安全性**：避免了多线程操作 DOM 可能导致的冲突和不一致问题。
...
回调函数是异步操作最基本的方法。下面是两个函数`f1`和`f2`，编程的意图是`f2`必须等到`f1`执行完成，才能执行。
...
const promise = new Promise((resolve, reject) => {
    // 异步操作
    if (/* 操作成功 */) {
        resolve(value); // 将状态改为 Fulfilled，并传递结果值
    } else {
        reject(error); // 将状态改为 Rejected，并传递错误信息
    }
});
```
...
- 接收两个参数：
  - 第一个参数是成功回调函数，接收 `resolve` 传递的值。
  - 第二个参数是失败回调函数（可选），接收 `reject` 传递的错误。
