---
title: JavaScript编程代码规范指南
date: 2020-05-15 08:00:00
permalink: /pages/50efd7/
categories:
  - 技术
  - 代码规范
tags:
...
> 基于 [Airbnb JavaScript Style Guide](https://github.com/airbnb/javascript) 的 ESLint 实践规范。
...
- [01.类型与引用](#01类型与引用)
  - [1.1 基本类型](#11-基本类型)
  - [1.2 引用类型](#12-引用类型)
  - [1.3 const 优先、let 次之、禁用 var](#13-const-优先let-次之禁用-var)
- [02.对象与数组](#02对象与数组)
  - [2.1 对象字面量与简写](#21-对象字面量与简写)
  - [2.2 扩展运算符与解构](#22-扩展运算符与解构)
  - [2.3 数组操作规范](#23-数组操作规范)
- [03.字符串与模板](#03字符串与模板)
  - [3.1 单引号与模板字符串](#31-单引号与模板字符串)
- [04.函数与箭头函数](#04函数与箭头函数)
  - [4.1 函数声明与默认参数](#41-函数声明与默认参数)
  - [4.2 箭头函数的适用场景](#42-箭头函数的适用场景)
  - [4.3 rest 参数与展开调用](#43-rest-参数与展开调用)
- [05.类与模块](#05类与模块)
  - [5.1 class 语法与继承](#51-class-语法与继承)
  - [5.2 ES Module 导入导出规范](#52-es-module-导入导出规范)
- [06.变量与作用域](#06变量与作用域)
  - [6.1 变量声明与块级作用域](#61-变量声明与块级作用域)
  - [6.2 变量提升与 TDZ](#62-变量提升与-tdz)
- [07.比较与条件](#07比较与条件)
  - [7.1 严格相等 ===](#71-严格相等-)
  - [7.2 控制语句与三元表达式](#72-控制语句与三元表达式)
  - [7.3 可选链与空值合并](#73-可选链与空值合并-es2020-推荐)
- [08.Async-Await-与-Promise](#08async-await-与-promise)
  - [8.1 Promise 链式调用](#81-promise-链式调用)
  - [8.2 async-await 规范](#82-async-await-规范)
  - [8.3 错误处理与并行](#83-错误处理与并行)
- [09.注释与格式](#09注释与格式)
  - [9.1 注释规范与 JSDoc](#91-注释规范与-jsdoc)
  - [9.2 缩进、空格与换行](#92-缩进空格与换行)
- [10.类型转换与命名](#10类型转换与命名)
  - [10.1 显式类型转换](#101-显式类型转换)
  - [10.2 命名规范总则](#102-命名规范总则)
- [11.TypeScript-补充规范](#11typescript-补充规范)
  - [11.1 类型定义优先](#111-类型定义优先)
  - [11.2 any-与-unknown](#112-any-与-unknown)
  - [11.3 泛型与工具类型](#113-泛型与工具类型)
- [12.工程化实践](#12工程化实践)
  - [12.1 ESLint-与-Prettier](#121-eslint-与-prettier)
  - [12.2 pre-commit-与-CI](#122-pre-commit-与-ci)
- [13.代码审查清单](#13代码审查清单)
- [14.常见陷阱速查](#14常见陷阱速查)
  - [14.1 类型与比较](#141-类型与比较)
  - [14.2 异步陷阱](#142-异步陷阱)
  - [14.3 作用域与闭包](#143-作用域与闭包)
...
## 08.Async/Await 与 Promise

### 8.1 Promise 链式调用

```javascript
// ✅ Promise 链：每个 then 处理一个阶段
fetchUser(id)
  .then(user => fetchOrders(user.id))
  .then(orders => processOrders(orders))
  .catch(err => {
    logger.error('订单处理失败', err);
...
async function getUserOrders(userId) {
  try {
    ...
    logger.error('获取订单失败', { userId, err });
    throw err;  // 或返回降级值
  }
}

// ✅ 顶层 await（ES2022，Node 18+ / 现代浏览器）
const config = await fetch('/api/config').then(r => r.json());

// ❌ forEach 里用 async（不会等待）
[1, 2, 3].forEach(async (id) => {
  await process(id);  // ❌ forEach 不等待 async 完成！
});
...
// ✅ Promise.allSettled：并行执行，不因单个失败而中断（ES2020）
...
results.forEach((r, i) => {
  ...
  else logger.warn(`服务${i}失败`, r.reason);
});
...
// ✅ 避免忘记 return（常见坑）
async function bad() {
  await doSomething();      // ❌ 返回 undefined
}
async function good() {
  return await doSomething(); // ✅ 显式 return
}
```

### 7.3 可选链与空值合并（ES2020）【推荐】

```javascript
// ✅ 可选链 ?. —— 安全访问深层属性
const city = user?.address?.city;        // 任一层级为 null/undefined 时返回 undefined
const name = users?.[0]?.name;           // 安全访问数组元素
const result = callback?.(data);          // 安全调用可能不存在的函数

// ❌ 老写法
const city = user && user.address && user.address.city;

// ✅ 空值合并 ?? —— 只在 null/undefined 时采用默认值
const count = input ?? 0;                // input 为 null/undefined 时用 0
const name = config.name ?? '默认名称';

// ❌ || 会把 0、''、false 也当作"假值"
const count = input || 0;                // input=0 时也会用默认值！
...
value.anything.whatever();   // 编译通过，运行时炸
```
...
// ✅ Partial / Required / Readonly / Record 等工具类型
type PartialUser = Partial<User>;         // 所有字段可选
type ReadonlyUser = Readonly<User>;       // 所有字段只读
type UserMap = Record<string, User>;      // key→User 映射

// ✅ 枚举用 const enum（零运行时开销）或用字面量联合类型
type Status = 'pending' | 'active' | 'inactive';  // 推荐
// const enum Status { Pending, Active, Inactive } // 也行，但运行时有值
```
...
```markdown
## 命名与结构
- [ ] 变量/函数 camelCase，类/组件 PascalCase，常量 UPPER_CASE
- [ ] 无拼音、单字母、含义模糊的命名
- [ ] 文件名 kebab-case，与默认导出一致
...
## 对象与数组
- [ ] 字面量语法创建，属性值简写
- [ ] 用扩展运算符（...）替代 Object.assign
- [ ] 使用解构赋值提取多属性
- [ ] 数组操作用 map/filter/reduce，不用 for 循环做变换

## 异步
- [ ] Promise 异常正确处理（catch 或 try/catch）
- [ ] 优先 async/await，避免回调嵌套
- [ ] 并行请求用 Promise.all / allSettled
- [ ] 避免 forEach 内用 async

## 比较与安全
- [ ] 使用 === 而非 ==
- [ ] 用 ?. 安全访问，用 ?? 设置默认值
- [ ] 无不必要的 eval()，无用户输入直接拼 HTML
...
| # | 陷阱 | 正解 |
|---|------|------|
| 1 | `'0' == 0` 为 true | `'0' === 0` 为 false，用 `===` |
| 2 | `||` 把 0、''、false 当假值 | 用 `??` 只判 null/undefined |
| 3 | `typeof null === 'object'` | 判 null 用 `value === null` |
| 4 | `NaN !== NaN` | 用 `Number.isNaN()` 而非 `isNaN()` |
| 5 | `[1,2].sort()` 按字符串排 → `[1,10,2]` | `arr.sort((a,b)=>a-b)` |

### 14.2 异步陷阱

| # | 陷阱 | 正解 |
|---|------|------|
| 1 | forEach 里 async 不会等待 | 用 `for...of` + await |
| 2 | `await` 忘写 try/catch | 顶层 + try/catch，或 `.catch()` |
| 3 | `Promise.all` 中一个 reject 全失败 | 用 `Promise.allSettled` |
| 4 | 未处理的 Promise rejection（Node 15+ 直接崩溃） | 全局监听 `unhandledRejection` |
| 5 | 循环中串行 await → 变顺序执行 | 用 `Promise.all` 并行 |

### 14.3 作用域与闭包

| # | 陷阱 | 正解 |
|---|------|------|
| 1 | for 循环中 var → 闭包取到最后的值 | 用 `let` 块级作用域 |
| 2 | `this` 在回调中丢失 | 箭头函数自动绑定 this |
| 3 | 闭包中引用大对象→内存泄漏 | 显式置 null 或 WeakMap |
| 4 | `parseInt('08')` → 0（旧引擎八进制） | `parseInt('08', 10)` 显式指定底数 |
| 5 | `const a = b = 1` → b 变全局变量 | `const a = 1; const b = 1;`
