...
#### 目录介绍
    - [1.1 一个"卡死"的按钮](#11-一个卡死的按钮)
    - [2.1 Qt 事件系统架构](#21-qt-事件系统架构)
    - [2.2 核心类与职责](#22-核心类与职责)
    - [2.3 事件 vs 信号槽：一张图说清区别](#23-事件-vs-信号槽一张图说清区别)
    - [3.1 事件循环的工作流程](#31-事件循环的工作流程)
    - [3.2 事件循环的伪代码实现](#32-事件循环的伪代码实现)
    - [3.3 多级优先级队列](#33-多级优先级队列)
    - [3.4 平台对接：Qt 如何利用 epoll](#34-平台对接qt-如何利用-epoll)
    - [4.1 层一：事件处理函数（最精细）](#41-层一事件处理函数最精细)
    - [4.2 层二：event() 函数（通用入口）](#42-层二event-函数通用入口)
    - [4.3 层三：事件过滤器（全局拦截）](#43-层三事件过滤器全局拦截)
    - [4.4 事件传播路径总结](#44-事件传播路径总结)
    - [5.1 两种投递方式的根本区别](#51-两种投递方式的根本区别)
    - [5.2 对比表](#52-对比表)
    - [5.3 实战中的选型](#53-实战中的选型)
    - [6.1 定义自定义事件类型](#61-定义自定义事件类型)
    - [6.2 发送与接收自定义事件](#62-发送与接收自定义事件)
    - [6.3 实战案例：CAN 总线数据到 UI 的事件驱动](#63-实战案例can-总线数据到-ui-的事件驱动)
    - [7.1 常见事件类一览](#71-常见事件类一览)
    - [8.1 阻塞事件循环的常见方式](#81-阻塞事件循环的常见方式)
    - [8.2 嵌套事件循环](#82-嵌套事件循环)
    - [8.3 调试技巧](#83-调试技巧)
