...
那年公司决定把一个百万行的单体 App 拆成组件化架构——首页、商品、订单、支付、消息、我的，六个业务模块每个模块可以独立编译成AAR。**拆到第三天所有人都崩溃了**。

问题出在**页面跳转**。原来的代码到处是`startActivity(new Intent(this, OrderDetailActivity.class))`——`OrderDetailActivity` 在订单模块，商品模块想跳过去必须 `import` 它，但一import 就形成了循环依赖：商品模块依赖订单模块，订单模块的下单按钮又要跳回商品模块。**组件化的第一个死结不是构建工具，是页面跳转**。

有人提议用反射：`Class.forName("com.example.order.OrderDetailActivity")`。写了两天就放弃了——字符串写错了没编译错误、混淆之后类名变了直接 crash、参数传递全是 Bundle 手写序列化。

直到接入 ARouter 才彻底解决问题。现在的调用是这样的：
```kotlin
ARouter.getInstance()
    .build("/order/detail")
    ...
    .navigation()
```
跳转方不需要知道对方的类名，不需要 import 任何目标模块的类，甚至不需要目标模块存在（缺失时走降级）。**这一切是怎么做到的？**

深入源码后我发现，ARouter 这套看似轻描淡写的 API 背后藏着七个精妙的设计：

1. **APT 编译期生成路由表**——`@Route(path = "/order/detail")` 在编译时被扫描，为每个模块生成一个 `ARouter$$Root$$xxx` 索引类。
2. **分组懒加载**——路由表按 group 分片，`/order/xxx` 是 order 组，只有跳转 order 组时才加载这一组的路由表。
3. **两级索引架构**——第一级 `Warehouse.groupsIndex`（组 → 组类），第二级`Warehouse.routes`（路径 → RouteMeta），延迟加载。
4. **拦截器责任链**——`CancelableCountDownLatch` 支持异步拦截器（比如登录检查要弹 Dialog 等用户输入）。
5. **服务发现（IProvider）**——把接口注册到路由表，另一模块通过接口拿到实例，实现模块间调用。
6. **参数自动注入（@Autowired）**——APT生成 `Inject` 类自动从 Intent 里取参数，零手写代码。
7. **Gradle Plugin 字节码插桩**——初始化时不扫描 dex 找路由表，用 ASM 直接把注册代码插进 `init()`。

这篇文章会把它们一个一个拆开来讲，最后用一次跨模块跳转的完整链路把它们串起来。读完你会明白——**ARouter 不是"路由框架"，它是"用 APT + 组件化"重新定义了 Android 模块通信的一整套方案**。
...
前面把每个模块拆开讲了，最后用一次真实场景把它们串起来——**商品详情页跳转到订单确认页并携带 orderId，跳转前需要登录检查**。跟着这个 navigation 走完从代码调用到 Activity 启动的每一步。

**t=0ms 编译期（很久很久以前）**——`OrderConfirmActivity` 上有一个注解：
```kotlin
@Route(path = "/order/confirm")
class OrderConfirmActivity : AppCompatActivity() {
    @Autowired var orderId: String = ""
}
```
`RouteProcessor` 在编译期扫描到这个注解，生成两个类：
- `ARouter$$Group$$order`——包含 `loadInto(Map<String, RouteMeta> atlas)` 方法，把 `/order/confirm → RouteMeta(OrderConfirmActivity.class, ACTIVITY)` 写进去
- `ARouter$$Root$$app_order`——包含 `loadInto(Map<String, Class<? extends IRouteGroup>> routes)` 方法，把 `"order" → ARouter$$Group$$order.class` 写进去

同时`AutowiredProcessor` 生成：
- `OrderConfirmActivity$$ARouter$$Autowired`——包含从 Intent 取 orderId 并赋值给字段的代码

**t=0ms App 启动 init()**——`ARouter.init(app)` 被调用。传统方式会扫描所有 dex 找出以 `com.alibaba.android.arouter.routes` 为包名的类（几秒），Gradle Plugin 版本已经在编译期把这些类的注册代码用 ASM 插到了 `LogisticsCenter.init()` 里，**运行时是O(1) 的直接调用**。

结果：`Warehouse.groupsIndex` 里存了 `{"order" → ARouter$$Group$$order.class, "product" → ..., "user" → ...}`——**注意这只是第一级索引，具体的路由表还没加载**。

**t=100ms 用户点击"立即购买"**——商品详情页执行：
```kotlin
ARouter.getInstance()
    .build("/order/confirm")
    .withString("orderId", "12345")
    .navigation(this, LoginNavigationCallback())
```

**t=101ms build 构造 Postcard**——`build(path)` 返回一个 `Postcard`（继承 `RouteMeta`）。**这时 Postcard 是空的**，只有 path。`withString` 只是往 `Bundle` 里塞参数，还没做任何跳转准备。

**t=102ms navigation 触发**——`_ARouter.navigation(context, postcard, ...)` 被调用（`_ARouter` 是内部实现，`ARouter` 是门面）。第一步：`LogisticsCenter.completion(postcard)`——**给 Postcard 补齐信息**。

**t=103ms completion 补齐**——从 path `/order/confirm` 解析出 group = `"order"`。查 `Warehouse.routes` 有没有 `/order/confirm`——**没有**（因为还没加载 order 组的路由表）。查 `Warehouse.groupsIndex` 拿到 `ARouter$$Group$$order.class`，反射实例化并调用 `loadInto(Warehouse.routes)`——**此时 order 组的所有路由才被真正加载**。这就是**分组懒加载**的核心机制。

**t=104ms 第二次查 routes**——现在能查到 `/order/confirm → RouteMeta(OrderConfirmActivity.class, ACTIVITY)`。把这些信息填回 Postcard——包括destination class、type、priority、extras 等。

**t=105ms 检查绿色通道**——`postcard.isGreenChannel()` 检查是否跳过拦截器（比如登录 Activity 本身不应该被登录拦截器拦）。这次没跳过。

**t=106ms 执行拦截器链**——`InterceptorServiceImpl.doInterceptions(postcard, callback)` 触发。所有 `@Interceptor(priority = xx)` 的拦截器按优先级排序，逐个调用 `process(postcard, callback)`。

**t=107ms LoginInterceptor 起作用**——`LoginInterceptor` 发现目标需要登录（比如通过 path 前缀或注解），当前未登录。它启动 `LoginActivity` 并**不调用 callback.onContinue()**——**整个跳转被卡住**。

**t=? LoginInterceptor 用了什么魔法？**——`CancelableCountDownLatch`。调用 `latch.await(timeout)` 阻塞在工作线程等待，直到用户登录完毕再调用`callback.onContinue()`。**这就是 ARouter 能支持"异步拦截器"的关键设计**——同步的 CountDownLatch + 超时降级。

**t=10s用户登录成功**——`LoginActivity` 回调触发 `latch.countDown()`（或者相当于 onContinue），拦截器链继续往下走。所有拦截器都通过后调用 `_navigation(postcard)`。

**t=10.001s 真正启动 Activity**——根据 Postcard 的 type 分四条路径：
- ACTIVITY：`context.startActivity(intent)`，其中 intent 是根据 Postcard 构造的 `Intent(context, OrderConfirmActivity.class)`并 putExtras
- PROVIDER：从 Warehouse 里拿单例返回
- FRAGMENT：反射实例化
- BOARDCAST/CONTENT_PROVIDER：等

本次是 ACTIVITY，`startActivity` 被调用。

**t=10.002s Activity 启动**——`OrderConfirmActivity.onCreate` 里的开发者会调用：
```kotlin
ARouter.getInstance().inject(this)
```
这一句触发 `AutowiredServiceImpl.autowire(this)`——反射拿到 `OrderConfirmActivity$$ARouter$$Autowired`，调用它的 `inject` 方法，**里面的代码就是 `orderId = intent.getStringExtra("orderId")` 的直接展开**。零反射、零手写。

**t=10.003s NavigationCallback.onArrival**——`LoginNavigationCallback.onArrival(postcard)` 被调用，告诉调用方跳转成功。整个流程结束。

整个链路从调用到到达 10 秒（其中大部分是等用户登录），真正的框架开销只有几毫秒。**ARouter 用 APT + 分组懒加载 + 责任链，做到了"零反射 + 零硬编码 + 完全解耦"的三重目标**。

## 11.ARouter 的六条设计哲学

**哲学一：编译期能做的绝不放到运行时。** 反射扫描是 Android 世界最贵的开销之一——ARouter 用 APT 把扫描搬到编译期，生成的索引类是纯 Java 代码，运行时是 O(1) 的HashMap 查找。这不是"性能优化"，是"根本性的架构选择"。**任何"运行时扫描注解"的框架都应该走这条路**——ButterKnife、Dagger、Room、EventBus3 全都是。

**哲学二：解耦的本质是"名字契约"。** ARouter 用**字符串 path** 作为契约，跳转方只知道 `/order/confirm`，不需要知道类在哪个模块。**看起来"字符串是弱类型"是缺点，实际上正是这个弱类型让模块之间彻底解耦**——目标类可以随便重命名、随便换包，只要 path 不变就行。用IProvider 服务发现也是同一个哲学：接口是契约，实现在哪里由路由表决定。

**哲学三：延迟一切可以延迟的事。** 分组懒加载是这个哲学的极致——路由表按组分片，第一级索引很小（几百组的映射），只有真正跳转到某组时才加载这组的具体路由。**如果一个 App 有 500 个页面，一次跳转只需要加载其中一组的路由**——这在冷启动敏感的场景下极其关键。

**哲学四：拦截器 = 责任链 + 可中断 + 可异步。** ARouter 拦截器不只是简单的责任链，它支持**异步拦截**（登录 Dialog 等用户操作后再继续）和**超时降级**（拦截器不响应超时后自动继续或失败）。这套设计让"路由前置校验"变得极其灵活——登录、埋点、权限、AB 实验都可以做成拦截器。**责任链是骨架，可中断和可异步是让它落地的关键**。

**哲学五：门面模式让API 极简。** 用户看到的 `ARouter` 类只是一个门面（Facade），真正的实现在 `_ARouter` 里。这样做的好处是：**内部随便怎么重构，外部 API 不变**。同时 `_ARouter` 又是单例，方便统一管理状态。这个模式和 Glide、OkHttp 的 Client 类是同一个哲学——**用户 API 越简单越好，复杂性沉到内部**。

**哲学六：字节码插桩是"消灭最后一点开销"的手段。** 就算 APT 已经生成了索引类，运行时还得扫 dex 找到这些类。arouter-register Gradle Plugin 用 ASM在编译期把注册代码直接插到 `LogisticsCenter.init()` 里，**从"运行时扫描 dex 几秒"降到"直接执行几毫秒"**。**编译期字节码插桩是Android 深度优化的终极武器**——`X2C`、`Booster`、`asm-plugin` 系列都是这套思路。

## 12.速查表：常见问题一图流

| 问题 | 答案速查 |
|------|---------|
| ARouter 靠什么找到目标 Activity？ | APT 编译期扫描 @Route 生成索引类，`Warehouse.routes` 存path→ RouteMeta 的映射 |
| 分组懒加载是什么？ | 路由表按 group 分片，第一级索引存组→组类，第二级组类里存具体路由，跳转时才加载对应组|
| 服务发现如何实现？ | IProvider 接口被 APT 扫描注册进 routes，其他模块通过 `.navigation()` 拿到接口实例，Provider 默认单例缓存 |
| 参数注入的原理？ | APT 生成 `目标类$$ARouter$$Autowired`，`ARouter.inject(this)` 反射调用它的 inject 方法，内部就是`intent.getXxx` |
| 拦截器如何支持异步？ | `CancelableCountDownLatch` 阻塞工作线程，异步任务完成后countDown 或 callback.onContinue 继续 |
| 拦截器执行时机？ | Activity/Fragment 类型跳转会走拦截器链，Provider 服务发现不走拦截器 |
| 降级策略如何配置？ | 实现 `DegradeService`（IProvider）并 @Route 注册，跳转失败时全局回调；或用 `NavigationCallback.onLost`|
| Gradle Plugin 优化了什么？ | ASM 在编译期把路由注册代码直接插到 init 方法，运行时不用扫描 dex，从秒级降到毫秒级 |
| ARouter 相比反射跳转有什么优势？ | APT 零运行时开销、字符串 path 完全解耦模块、支持拦截器/降级/服务发现/参数注入等一整套能力 |
| 什么时候 ARouter 是过度设计？ | 单模块小项目、没有跨模块通信需求，此时 startActivity + Bundle 就够了 |
| 为什么现在有人推 TheRouter？|ARouter 官方停止维护、初始化性能可以更好、支持 KSP，是ARouter 的现代化替代品 |

理解了这张表，ARouter 的所有面试题基本都能拿下。但更重要的是——**这些设计可以直接迁移到其他场景**：想做一个跨进程调度器？路由表+ 拦截器就是骨架；想做一个动态化框架？path + Provider 就是模板；想在自己的项目里做模块解耦？"字符串契约 + APT 生成索引"就是范本。**读ARouter 源码不是记住 ARouter，是把"组件化通信"这块拼图彻底装进兜里**。
