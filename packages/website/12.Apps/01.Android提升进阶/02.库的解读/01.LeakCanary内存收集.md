...
三年前我接手一个已经上线两年的 App，第一周就遇到线上 OOM崩溃率 0.8%。翻看崩溃堆栈千奇百怪——有 `BitmapFactory.decodeStream` 的、有 `HashMap.resize` 的、有 `ArrayList.grow` 的。这些都是**现象**不是**原因**——真正的原因是某个 Activity 早就该被回收，却因为一条隐蔽的引用链一直活着，堆内存一路涨。

我当时的第一反应是「上LeakCanary」。集成后跑了一天，弹出十几条泄漏通知，其中一条让我至今印象深刻：

```
┬───
│ GC Root: Global variable in native code
├─ com.example.util.ImageLoader instance
│    Leaking: NO (LoadedApk↓ is not leaking)
│    ↓ staticImageLoader.INSTANCE
├─ android.content.Context instance
│    Leaking: YES
│    ↓ Activity.mDetailPage
╰→ com.example.DetailActivity instance
     Leaking: YES (Activity#mDestroyed is true)
```

一条 20 行的报告直接指到了源头——单例 `ImageLoader` 持有了 Activity 的 Context。我当时的疑问是：**LeakCanary 是怎么在几秒钟内，从几百 MB 的堆里，精准找到这条引用链的？** 更关键的是——它是怎么知道 `DetailActivity` 应该被回收但没有被回收的？

带着这个疑问深入源码后我发现，LeakCanary 短短几千行代码里藏着五个精妙的原理点：

1. **WeakReference + ReferenceQueue 的组合技**——用 JVM 自带的机制去问「这个对象死了吗」，不用轮询、不用反射，成本极低。
2. **五秒延迟 + 主动 GC 双保险**——避免把「还没来得及回收」的对象误判为泄漏。
3. **ContentProvider 零配置初始化**——利用 `ContentProvider.onCreate` 早于 `Application.onCreate` 的机制，做到接入零代码。
4. **Shark 引擎的两次遍历策略**——不把整个 hprof 加载进内存，靠索引 + seek 完成大文件分析。
5. **最短路径算法找引用链**——把整个堆当成有向图，从怀疑对象反向 BFS 找到 GC Root。

这篇文章会把这五个点一个一个拆开来讲，最后用一个 Activity 泄漏的完整生命周期把它们串起来。读完你会发现——**LeakCanary 不是一个「工具」，它是一套「用 JVM 的规则去反向审计JVM 内存」的思维方式**。
...
## 10.一个 Activity 泄漏的一生

前面把每个原理点都拆开讲了，最后用一个具体案例把它们串起来——**假设 `DetailActivity` 因为被单例 `ImageLoader` 持有而泄漏了**，我们跟着这个 Activity 走一遍它在 LeakCanary 眼里的一生。

**t=0s 应用启动时**——AAR 里的 `AppWatcherInstaller`（一个空 ContentProvider）被系统在 `Application.onCreate()` 之前实例化，`onCreate()` 里调用 `AppWatcher.manualInstall()`，把 `ActivityWatcher`、`FragmentAndViewModelWatcher`、`RootViewWatcher`、`ServiceWatcher` 注册到系统。此时开发者一行代码都没写，LeakCanary 已经开始工作。

**t=10s 用户打开 DetailActivity**——`ActivityWatcher` 通过 `Application.registerActivityLifecycleCallbacks` 收到 `onActivityCreated` 回调。`DetailActivity` 在被创建时并不会被监听，只有它被销毁时才会。

**t=30s 用户按返回**——`Activity.onDestroy()` 被调用，`ActivityWatcher.onActivityDestroyed` 触发。此时 `ObjectWatcher.expectWeaklyReachable(activity)` 被调用，做了三件事：
1. 生成一个唯一的 `key`（UUID）
2. 用 `KeyedWeakReference(activity, key, referenceQueue)` 把Activity 包起来
3. 把 `key` 存到 `watchedObjects` map 里

**t=30.001s WeakReference 的魔法开始**——`KeyedWeakReference` 是 `WeakReference` 的子类，一旦它引用的 `DetailActivity` 被 GC 回收，JVM 会**自动把这个 WeakReference 塞进 referenceQueue**。这是 JVM 层的机制，不需要 LeakCanary 做任何轮询。

**t=35s 五秒延迟检查**——LeakCanary 用一个 `Handler` 在五秒后做检查：把 `referenceQueue.poll()` 出来的所有 WeakReference 对应的 key 从 `watchedObjects` 里删掉。剩下的就是**五秒内没被回收的怀疑对象**。为什么要等五秒？因为对象在 `onDestroy` 时可能还有若干合法引用（比如动画即将结束的回调），给 GC 一点时间是合理的。

**t=35.001s 主动 GC**——如果五秒后`watchedObjects` 里还剩东西，LeakCanary 会调用 `Runtime.getRuntime().gc()` 主动触发一次 GC（注意 `System.gc()` 只是建议，`Runtime.gc()` 更可靠）。GC 后再次清理 referenceQueue，如果对象还在，才被确定为**"可能泄漏"**。

**t=35.5s Dump 堆快照**——`HeapDumper.dumpHeap()` 调用 `Debug.dumpHprofData("/sdcard/xxx.hprof")`。这一步会**冻结应用几秒**（这也是 LeakCanary 不能上线的核心原因），生成一个几十~几百 MB 的 hprof 文件。

**t=40s Shark 分析开始**——`HeapAnalyzer` 在后台线程启动 Shark 引擎。Shark 的做法是：
1. **第一次遍历**：读hprof 的 Header 和所有 Record 的 offset，建立索引（对象 ID → 文件偏移）。不加载任何对象实例。
2. **第二次遍历**：从 `watchedObjects` 里的 key 反查到 `KeyedWeakReference` 对象，通过它拿到怀疑对象的 `objectId`。
3. **反向 BFS**：从怀疑对象出发，沿着"谁引用了我"这条方向反向搜索，直到找到 GC Root（静态字段、线程栈、JNI 全局引用等）。
4. **最短路径**：如果找到多条链，选**最短的一条**——因为最短的引用链最可能是泄漏的直接原因。

**t=42s 找到根因**——Shark 输出的引用链是：`GC Root → ImageLoader.INSTANCE → mDetailPage → DetailActivity`。LeakCanary 把这条链翻译成人类可读的报告：静态单例 `ImageLoader` 有一个字段 `mDetailPage` 指向了已销毁的 `DetailActivity`。

**t=42.001s 通知开发者**——通过 Notification 弹出，点开查看完整堆栈。开发者立刻明白：**`ImageLoader` 用了 Activity 的 Context 做初始化，应该改成 `ApplicationContext`**。

整个流程从对象死亡到根因定位，只有几十秒。这里面藏着 LeakCanary 的设计哲学——**它把内存泄漏检测从"人肉分析 hprof"转成了"JVM 自己告诉你"**。

## 11.LeakCanary 的四条设计哲学

**哲学一：借力 JVM，而不是重造轮子。** 检测对象是否被回收，最笨的做法是每隔一段时间轮询、遍历、判断。LeakCanary 直接用 `WeakReference + ReferenceQueue`——这是JVM 层的官方通知机制，成本低、准确、无侵入。**优秀的库不是自己实现一切，是找到系统里已经存在的钩子挂上去**。

**哲学二：允许模糊，用时间和GC 消除误报。** 一个对象在 `onDestroy` 之后不一定马上被回收，可能还有合法的短生命周期引用。LeakCanary 没有追求"精确到毫秒"，而是用**五秒 + 主动 GC** 这种"粗糙但可靠"的策略。**在系统级软件里，"允许模糊 + 兜底机制"往往比"追求精确"更能落地**。

**哲学三：零配置，让工具消失在开发者视野里。** `AppWatcherInstaller` 是一个空 ContentProvider，用 AAR 的 manifest merge 自动注册。开发者只要加一行 gradle 依赖，什么代码都不用写，就自动开启检测。**接入体验决定工具的使用率——没人愿意为一个"调试工具"多写十行样板**。

**哲学四：不加载完整数据，用索引 + seek 处理大文件。** Shark 相比老的 haha 快 6 倍、内存低几倍，秘密就在于"两次遍历"——第一次建索引，第二次按需seek。**处理不确定大小的输入（几百 MB 的 hprof），永远优先索引，不要一次性加载**。

## 12.速查表：常见问题一图流

| 问题 | 答案速查 |
|------|---------|
| LeakCanary 靠什么发现泄漏？ | WeakReference + ReferenceQueue，对象被 GC 时JVM 自动通知 |
| 为什么要等五秒？ | 给合法的短生命周期引用一个消散时间，避免误报 |
| 为什么还要主动 GC？ | ReferenceQueue 的通知依赖 GC 发生，主动触发一次可以立即得到结论 |
| 怎么找到GC Root？ | 从怀疑对象反向 BFS，找最短的引用链 |
| 为什么不能上线？ | `Debug.dumpHprofData` 会锁堆几秒，用户会感觉卡顿甚至 ANR |
| 线上怎么办？ | KOOM 用 `fork()` + COW，父进程立即恢复，子进程慢慢分析 |
| ViewModel 怎么监控？ | 反射拿 `ViewModelStore.mMap`，遍历所有 ViewModel 交给 ObjectWatcher |
| 接入为什么不用写代码？ | AAR 里注册了`AppWatcherInstaller` ContentProvider，先于Application 初始化 |
| Shark 为什么比haha 快？ | 按需加载 + 两次遍历建索引 + Okio 高效 IO |
| 引用链为什么选最短的？ | 最短链最可能是泄漏的直接原因，长链更多是绕路|

理解了这张表，面试里所有关于 LeakCanary 的问题基本都能对答如流。但更重要的是——**这些设计思路可以迁移到其他场景**：想监控某个资源是否被正确释放？WeakReference + Queue 就是模板；想做零配置初始化的库？ContentProvider Hook 就是钥匙；想分析大文件？两次遍历建索引就是思路。**读源码不是背源码，是把这些"可迁移的原理"揣进兜里**。
