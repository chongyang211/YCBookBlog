...
阅读建议：本篇贯穿"FIFO 数据结构 → 阻塞 / 无锁 / Disruptor 容器设计实战"，一路打通从手写到选型；想速查可直接跳到对应锚点。

- [7.队列操作实践](#7队列操作实践)
- [01. 从工作案例说起](#01-从工作案例说起)
  - [1.1 同步导出打爆JVM](#11-同步导出打爆jvm)
  - [1.2 正解：入队异步化](#12-正解入队异步化)
  - [1.3 上线效果与本篇路线](#13-上线效果与本篇路线)
- [02. 队列定义与本质](#02-队列定义与本质)
  - [2.1 奶茶店排队](#21-奶茶店排队)
  - [2.2 队列核心API](#22-队列核心api)
  - [2.3 FIFO对比LIFO](#23-fifo对比lifo)
  - [2.4 队列常见变种](#24-队列常见变种)
...
- [08. 队列经典工业应用](#08-队列经典工业应用)
  - [8.1 线程池的任务队列选型](#81-线程池的任务队列选型)
  - [8.2 Kafka 的 log 其实也是队列](#82-kafka-的-log-其实也是队列)
- [09. 本篇收获与回扣](#09-本篇收获与回扣)
- [10. 思考题深度练](#10-思考题深度练)
- [11. 课后作业实战](#11-课后作业实战)
  - [作业一｜还原开篇案例，对比同步 vs 异步](#作业一还原开篇案例对比同步-vs-异步)
  - [作业二｜手写循环 + 阻塞队列](#作业二手写循环--阻塞队列)
  - [作业三｜选型决策矩阵](#作业三选型决策矩阵)
- [12. 进阶专题与延伸](#12-进阶专题与延伸)
  - [12.1 时间轮：延迟任务的最优解](#121-时间轮延迟任务的最优解)
  - [12.2 Disruptor 的核心 trick：序号 + 缓存行填充](#122-disruptor-的核心-trick序号--缓存行填充)
  - [12.3 work-stealing 队列：ForkJoinPool 的秘密](#123-work-stealing-队列forkjoinpool-的秘密)
  - [12.4 无锁环形队列的两种主流](#124-无锁环形队列的两种主流)
  - [12.5 异步 IO 与 io\_uring 中的队列](#125-异步-io-与-io_uring-中的队列)
  - [12.6 经典书与论文](#126-经典书与论文)
...
真实事故：一个订单导出服务把整个 JVM 打爆了。后台有一个 **订单导出** 功能，运营一次可勾选最多 100 万条订单导出 Excel。初期是最朴素的同步实现：

- 前端点"导出"后，直接在 Tomcat 工作线程里**同步**跑，单任务最长 30 秒；
- 用户频繁点，同一用户会开好几个同样重的任务；
- 用户按"退出"后任务没被清掉，继续跑、继续占内存。

三个问题叠加：**工作线程被占光 → 所有业务接口超时；任务失控 → 内存持续增长**。某天某运营点了 20 次，进程直接 OOM。

### 1.2 正解：入队异步化

正确的做法：**请求入队 → 工作线程按 FIFO 消费 → 同一用户最多排一个任务 → 队列满就拒新**。核心是把同步改成异步：
...
奶茶店点单就是一条活生生的队列，三个动作对应队列的全部语义：

- **入队**：新顾客只能排到**队尾**，不能插到中间；
- **出队**：店员永远先服务**队头**那位，他离开后后面的人整体前移一位；
- **FIFO**：先来的人先拿到奶茶——**First In, First Out**。

对照理解：一摞盘子是**栈（LIFO）**，总是先拿最上面那个。**排队的核心是"公平"——先来先服务**，这正是任务队列的价值：请求按到达顺序被处理，谁都不会被插队饿死。
...
> 队列讲完，线性结构详解（数组、链表、栈、队列）这条主线就完整了。下一篇《08.二叉树的操作实践》会跨到**非线性**世界——一旦节点有了\"两个孩子\"的分支，算法和复杂度的世界就彻底变天：从线性的 O(N) 降到 O(log N) 是可能的，但也出现了\"平衡与不平衡\"、\"递归与迭代\"、\"遍历顺序\" 等全新维度的博弈。
