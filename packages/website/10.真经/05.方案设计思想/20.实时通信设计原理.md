...
- [3. TCP连接本质](#3-tcp连接本质)
  ...
  - [3.6 Nagle与实时通信](#36-nagle与实时通信)
  - [3.7 延迟确认的死锁](#37-延迟确认的死锁)
  - [3.8 半开连接检测](#38-半开连接检测)
  - [3.9 保活与心跳](#39-保活与心跳)
  - [3.10 TIME_WAIT状态](#310-time_wait状态)
- [4. 消息路由本质](#4-消息路由本质)
...
- [7. MQTT解剖](#7-mqtt解剖)
  ...
  - [7.5 主题匹配算法](#75-主题匹配算法)
  - [7.6 会话生命周期](#76-会话生命周期)
  - [7.7 Broker消息路由](#77-broker消息路由)
  - [7.8 共享与集群](#78-共享与集群)
  - [7.9 安全认证](#79-安全认证)
...
**疑惑**：开篇事故里，一句话就修好了 8 秒延迟：`SetNoDelay(true)`。为什么关掉一个叫 "Nagle" 的东西就从 8 秒变成 30ms？Nagle 到底做了什么？

**论证**，Nagle 算法（RFC 896，1984 年）诞生的背景是 **Telnet 时代**——用户在终端里敲一个字就产生一个 TCP 包（1 字节 payload + 40 字节 IP+TCP 头 = 41 倍带宽浪费）。Nagle 的设计意图是**把小包攒成大包再发**：

```
Nagle 算法规则（只有两条）：
┌─────────────────────────────────────────────────────┐
│ 规则 1：如果当前有已发送但未 ACK 的数据，            │
│          新数据必须等 ACK 到了才能发。                │
│                                                      │
│ 规则 2：如果没有未 ACK 的数据，                      │
│          立刻发（不用等）。                          │
│                                                      │
│ 例外：如果累积的数据 ≥ MSS（Maximum Segment Size），  │
│         无论规则 1 是否满足，都立刻发。               │
└─────────────────────────────────────────────────────┘
```
...
```
实时通信的消息特征：
  ├─ 消息体积小（已读回执 20B、心跳 10B、IM 文本 100B）
  ├─ 消息间隔不固定（用户输入的节奏）
  └─ 要求低延迟（毫秒级）

Nagle 的"攒包"逻辑正是这些小消息的杀手：
  1. 发一条 20B 的"已读回执"
  2. Nagle：payload=20 << MSS(1460)，且上一个包还没 ACK → buffer 等着
  3. 等 200ms（等 ACK 或更多数据）
  4. 200ms 后发出去
  5. 下一条消息再等 200ms……
  → 累积延迟 = N × 200ms
```
...
| 协议 | 受害场景 | 严重程度 |
|---|---|---|
| **WebSocket** | 小帧（ping、已读回执、短文本）| ⭐⭐⭐⭐⭐ |
| **MQTT** | CONNECT/PINGREQ/PUBLISH 小 payload | ⭐⭐⭐⭐ |
| **自研 TCP 协议** | 自定义帧协议的消息体小 | ⭐⭐⭐⭐⭐ |
| **HTTP/1.1 Keep-Alive** | 分块传输的最后一个 chunk | ⭐⭐ |
| **Push (APNs 服务端侧)** | HTTP/2 帧不受 Nagle 影响 | ⭐ |

**修复方式**（所有 TCP 长连接系统都该做）：
...
**何时不该关 Nagle？**，只有一种情况：**大块数据传输**（文件上传、视频流）。此时 Nagle 的攒包是正面效果——把分散的写合并成满 MSS 的包，降低协议头占比。

**结论**：**任何实时通信场景**（WebSocket、MQTT、自研 TCP 协议），`TCP_NODELAY` 是**第一行该写的代码**。不是调优选项，是正确性前提。

### 3.7 延迟确认的死锁

**疑惑**：3.6 节讲 Nagle 会延迟 200ms，但为什么事故是 8 秒而不是 200ms × N？

**论证**，Nagle 并不孤立存在——它和 **Delayed ACK** 形成了**恶性耦合**：

**Delayed ACK（RFC 1122）**的设计意图：

```
接收方收到数据后，不立刻回 ACK，而是等：
  条件 A：积累了 2 个满 MSS 的段 → 立刻 ACK
  条件 B：200ms 超时了 → 立刻 ACK（不管有没有满）
  条件 C：有响应数据要发 → ACK 可以"搭车"（piggyback）

目的：把 ACK 和响应数据合并到一个包，减少纯 ACK 包
```

**Nagle × Delayed ACK 的死锁链条**：

```
场景：客户端连续发多个小消息（已读回执 20B → 文本 50B → emoji 10B）

  消息① 20B write
    │ Nagle: payload < MSS，但上一包已 ACK → 立刻发 ✅
    │ 
    ├──▶ 服务端收到
    │      │ Delayed ACK：等 200ms 看有没有响应数据一起回
    │      │ （消息① 本身没有响应数据，只能等超时）
    │      │
    │      ◀── ACK(200ms 后)
    │
  消息② 50B write（消息① 发出去后 10ms 到来）
    │ Nagle：消息①还没 ACK → buffer 等着 ❌
    │ 等……
    │ 等……（200ms）
    │ 消息① ACK 到了 → 消息② 立刻发
    │ 
    ├──▶ 服务端收到
    │      │ Delayed ACK：再等 200ms
    │      │
    │      ◀── ACK(又 200ms)
    │
  消息③ 10B write
    │ Nagle：消息②还没 ACK → 继续等…… ❌
    │ 
    ↓ 循环往复，每次 200+200=400ms 的"无效等待"

  总延迟 = N × 400ms（不是 200ms！）
  如果 N=20 条小消息 → 20 × 400ms = 8 秒 🎯
```

**这就是开篇事故的完整根因链**——不是 Nagle 一个人的问题，而是 Nagle + Delayed ACK **互相等待对方**的经典死锁：

```
Nagle:  "我等 ACK 到了再发下一段"
Delayed ACK: "我等响应数据到了再回 ACK"
        ↓
      死锁

每次打破死锁的成本 = 200ms 超时 × 2（发端等 ACK + 收端等数据）
```
...
| 方案 | 操作 | 副作用 |
|---|---|---|
| ① 关 Nagle（服务端） | `TCP_NODELAY = true` | 可能多发几个小包，带宽略增 |
| ② 关 Delayed ACK（服务端） | `TCP_QUICKACK = true` | 每次收数据立刻 ACK，ACK 包变多 |
| ③ 客户端合并写 | 先把多条小消息攒成一个大的再 write | 客户端逻辑复杂，延迟增加 |

**业界最佳实践**：① + ③ 组合——**服务端开 TCP_NODELAY，客户端攒 5ms 的写缓冲区**。这样既解决了死锁，又不会产生过多小包。

**结论**：Nagle + Delayed ACK 的死锁是**所有 TCP 实时通信系统的"隐藏性能杀手"**。WebSocket、MQTT、自研 TCP 协议，只要连续发小消息，必然触发。开 `TCP_NODELAY` 能解决 90% 的场景，剩下的 10% 靠客户端攒写。

### 3.8 半开连接检测

**疑惑**：TCP 连接断了，双方不就知道了吗？为什么还要心跳？

**论证**，TCP 断连分两种，**只有一种会通知对方**：

```
正常关闭（四次挥手）：
  A ──FIN──▶ B ──ACK──▶ A ──FIN──▶ B ──ACK──▶ A
  ✅ 双方都知道连接已关闭

异常断开（进程崩溃/断电/网线拔了）：
  A 崩溃了，没发 FIN，没发 RST
  B 还在内核 accept 队列里挂着，socket 状态 = ESTABLISHED
  ❌ B 完全不知道 A 已经死了
  
  → 这就是"半开连接"（Half-Open Connection）
```

**半开连接的"死亡螺旋"**：

```
B 应用层 send("你在吗？") ──▶ 内核发出去
  │ 没有 ACK 回来
  │ TCP 重传机制启动：
  │   第 1 次重传：1.5s 后
  │   第 2 次重传：3s 后  
  ...
  │   ...（指数退避）
  │   第 15 次重传：~13 分钟后
  │   最终：内核放弃，socket 关闭（ETIMEDOUT）
  │
  └── 从 B 应用层看来，send() 成功了（写到了内核缓冲区）
      但实际上消息全丢了，且 13 分钟后才知道连接死了
```

**Linux 默认重传参数**（`net.ipv4.tcp_retries2 = 15`）：

| 参数 | 默认值 | 含义 |
|---|---|---|
| `tcp_retries2` | 15 | 数据重传最大次数。15 次 ≈ 924s ≈ **15 分钟**才发现死连接 |
| 重传 RTO 初值 | 200ms | 第 1 次重传等待 200ms |
| 重传 RTO 上限 | 120s | 到达最大的单次等待 |
...
WebSocket 长连接：
  手机没电了 → 服务端 15 分钟后才知道
  这 15 分钟内所有给这个用户的推送 → 全丢，且上报告警"投递成功"

MQTT：
  传感器断电 → Broker 认为设备还在线
  Will Message 要等 15 分钟才触发 → 下游系统延迟看到"离线"状态

Push 推送：
  Token 对应的设备其实不在线了 → APNs/FCM 返回成功
  用户收不到 → 投诉
```

**解决方案只有一种：应用层心跳**。TCP 自己提供的 `SO_KEEPALIVE` 太慢（下节讲），唯一可靠的死亡检测就是"我主动发心跳，N 倍心跳周期没收到回复 → 判定死亡"。

**结论**：TCP 保证的是"如果 RST/FIN 到了，我一定通知你"，**不保证"对方死了尽快通知你"**。所有长连接系统都必须**自己实现心跳死亡检测**，时间窗口 = `2~3 × 心跳周期`，绝对不是 15 分钟。

### 3.9 保活与心跳

**疑惑**：TCP 不是有 `SO_KEEPALIVE` 吗？为什么不直接用，还要自己做心跳？

**论证**，`SO_KEEPALIVE` 和普通人的想象**差距巨大**：

**TCP Keep-Alive 的工作原理**：
...
│                                                     │
│  默认配置（Linux）：                                 │
│  ├─ 空闲多久开始探测?  tcp_keepalive_time = 7200s   │
│  │   → 2 小时！连接空闲 2 小时后才发第一个探测包     │
│  │                                                    │
│  ├─ 探测间隔?  tcp_keepalive_intvl = 75s             │
│  │   → 每 75 秒发一次                                │
│  │                                                    │
│  ├─ 探测几次宣布死亡?  tcp_keepalive_probes = 9      │
│  │   → 9 次无响应 = 死亡                             │
│  │                                                    │
│  └─ 总死亡检测时间 = 7200 + 75×9 ≈ 2h 11min ⚠️     │
│                                                     │
│  调优后（企业级最佳实践）：                           │
│  ├─ tcp_keepalive_time  = 600  (10 分钟)             │
│  ├─ tcp_keepalive_intvl = 30   (30 秒)               │
│  ├─ tcp_keepalive_probes = 3    (3 次)               │
│  └─ 总检测时间 = 600 + 30×3 = 690s ≈ 11.5 分钟       │
└────────────────────────────────────────────────────┘
```

**SO_KEEPALIVE vs 应用层心跳 对比**：

| 维度 | TCP SO_KEEPALIVE | 应用层心跳 |
|---|---|---|
| **探测速度** | 2 小时（默认）/ 10 分钟（调优） | 30~300 秒 |
| **NAT 保活** | ❌ 不能穿透（探测包无应用数据，NAT 可能不刷新表项） | ✅ 心跳包走应用层，NAT 当成正常流量 |
| **携带数据** | ❌ 纯 TCP ACK，无 payload | ✅ 可携带 seq、RTT、设备状态、电量 |
| **跨语言** | ✅ 内核级，所有语言直接可用 | ⚠️ 需应用层实现 |
| **对中间件透明** | ✅ LB/Proxy 能识别并断开死连接 | ❌ LB/Proxy 看到的是正常业务流量 |
| **适用场景** | LB 与后端之间的连接清理 | 客户端 ↔ 服务端（跨 NAT 必用） |

**对实时通信各协议的影响**：

```
WebSocket：
  ├─ SO_KEEPALIVE 太慢 → 必须用 WebSocket Ping/Pong（opcode 0x9/0xA）
  └─ 浏览器 WebSocket API 甚至没法直接开 SO_KEEPALIVE

MQTT：
  ├─ PINGREQ/PINGRESP 是规范的一部分（§7.4），IoT 设备的标准做法
  └─ Broker 侧建议开 SO_KEEPALIVE 做僵尸连接清理的兜底

Push：
  ├─ APNs/FCM 自己维护到设备的长连接，心跳是 Apple/Google 的事
  └─ 服务端到 APNs 走 HTTP/2，不需要 SO_KEEPALIVE
```

**最佳实践**：

```bash
# 调优 SO_KEEPALIVE（作为兜底，不是主力）
sysctl -w net.ipv4.tcp_keepalive_time=600   # 10 分钟空闲开始探测
sysctl -w net.ipv4.tcp_keepalive_intvl=30    # 30 秒探测间隔
sysctl -w net.ipv4.tcp_keepalive_probes=3    # 3 次无响应判定死亡
```

**结论**：**SO_KEEPALIVE 和应用层心跳不是替代关系，是互补关系**。应用层心跳负责"快"（30~300s 级死亡感知 + NAT 保活），SO_KEEPALIVE 负责"稳"（兜底清理，防止应用层心跳 bug 导致僵尸连接堆积）。长连接系统**两者都要，但主力是应用层心跳**。

### 3.10 TIME_WAIT状态

**疑惑**：连接关了，为什么端口还被占用 60 秒？"TIME_WAIT"到底在保护什么？

**论证**，TCP 关闭不是瞬间完成的：

```
主动关闭方（Active Closer）的状态机：
...
FIN_WAIT_2 (等对方 FIN)
    │ recv FIN
    ▼
TIME_WAIT ─── 等待 2MSL（Maximum Segment Lifetime = 30~60s）
    │          2MSL 在 Linux 默认 60 秒
    ▼
CLOSED
```
...
原因 1：确保最后的 ACK 被对方收到
  ...
  │     │ ◀── FIN ────────── │     │
  │     │ ── ACK ──────────▶ │     │  ← 最后一个 ACK
  │     │   (进入 TIME_WAIT)  │     │     如果这个 ACK 丢了
  │     │                     │     │     B 会重发 FIN
  └─────┘                    └─────┘
  
  如果 A 不保持 TIME_WAIT 直接 CLOSED：
  B 重发的 FIN → 端口已关闭 → A 内核回 RST → B 看到的是"异常关闭"

原因 2：让网络中残留的旧包"过期"
  如果 A 立刻复用这个五元组建新连接
  → 网络中还有一个旧的延迟数据包没到达
  → 这个旧包会被新连接错误接收
  → 2MSL 保证所有旧包在 60s 内"死光"
```

**TIME_WAIT 对长连接系统的实战影响**：

```
场景：800 万设备重连风暴，服务端主动关闭旧连接

  服务端 800 万 TIME_WAIT：
  ├─ 每个 TIME_WAIT 占用一个本地端口（对于客户端角色）
  │   对服务端来说：监听的端口不变，但每个 TIME_WAIT 的 socket
  │   占用 ~280 bytes 内存
  │   800 万 × 280B = 2.2 GB 内存被"死连接"占用
  │
  ├─ 端口耗尽风险（服务端作代理/客户端时）：
  │   本地可用端口 65535 - 1024 ≈ 64000 个
  │   如果 800 万 TIME_WAIT → 端口耗尽 → 无法建新连接
  │
  └─ 对纯服务端（listen 端口）：影响较小
      因为服务端 listen 的端口不受 TIME_WAIT 限制
      （只有在 bind 到具体端口时才受限制）
```
...
| 策略 | 操作 | 适用场景 |
|---|---|---|
| **让客户端主动关闭** | WebSocket/MQTT 交互中，客户端发 Close 帧，由它进入 TIME_WAIT | ✅ 最佳实践，服务端无 TIME_WAIT 压力 |
| `SO_REUSEADDR` | 允许 bind 到 TIME_WAIT 状态的地址 | 服务端快速重启 |
| `tcp_tw_reuse` | 允许客户端复用 TIME_WAIT 端口（前提：timestamp 开启） | 出站连接多的服务 |
| `tcp_tw_recycle` | 快速回收 TIME_WAIT（**Linux 4.12 已废弃，NAT 下有严重 bug**） | ❌ 不推荐 |
| 连接池 | 复用已建连接，避免频繁创建/销毁 | 所有场景 ✅ |

**各协议的断开语义**：

```
WebSocket：
  ├─ Close 帧（opcode 0x8）：应用层优雅关闭
  ├─ 先发 Close 帧的一方 = 主动关闭方 → 进入 TIME_WAIT
  └─ 最佳实践：客户端先发 Close 帧

MQTT：
  ├─ DISCONNECT 报文：应用层正常关闭
  ├─ 也是先发 DISCONNECT 的一方进入 TIME_WAIT
  └─ IoT 设备通常主动 DISCONNECT（省电后关闭）

Push：
  ├─ HTTP/2 连接池 → 连接复用 → TIME_WAIT 几乎不出现
  └─ 这也是 HTTP/2 相比 HTTP/1.1 的重要优势
```

**结论**：TIME_WAIT 是 TCP 正确性的代价，不是 bug。**实战第一原则：让客户端做主动关闭方**，服务端永远被动回应，这样 TIME_WAIT 的压力就压在千万台客户端上，而不是压在寥寥几台服务器上。
...
**疑惑**：7.1 节讲了 Topic 和通配符 `+` / `#`，但 Broker 收到 `sensor/+/temperature` 时怎么快速找出所有匹配的订阅者？遍历所有订阅规则 O(N) 肯定不行。

**论证**，主题匹配是 MQTT Broker 的**核心引擎**——匹配速度直接决定 Broker 吞吐上限。业界主流使用的是**字典树（Trie）**：

**Trie 订阅树结构**：

```
订阅：
  subscriber-A → sensor/temp
  subscriber-B → sensor/#
  subscriber-C → sensor/+/humidity
  subscriber-D → home/livingroom/light
...
type TrieNode struct {
    children    map[string]*TrieNode  // 精确匹配的子节点
    plusChild   *TrieNode             // '+' 单层通配
    hashChild   *TrieNode             // '#' 多层通配
    subscribers []*Subscriber         // 匹配到该节点的订阅者
}

func (n *TrieNode) Match(levels []string, depth int, result *[]*Subscriber) {
    // 1. 如果当前节点有订阅者，全部加入结果
    *result = append(*result, n.subscribers...)

    // 2. 如果已经到最后一层
    if depth >= len(levels) { return }

    // 3. '#' 通配：匹配当前及之后所有层级
    if n.hashChild != nil {
        *result = append(*result, n.hashChild.subscribers...)
        // # 之后不能继续匹配（MQTT 规范：'#' 必须是最后一级）
    }

    // 4. '+' 通配：匹配当前这一层
    if n.plusChild != nil {
        n.plusChild.Match(levels, depth+1, result)
    }

    // 5. 精确匹配
    if child, ok := n.children[levels[depth]]; ok {
        child.Match(levels, depth+1, result)
    }
}
```
...
| 方案 | 插入复杂度 | 匹配复杂度 | 删除复杂度 |
|---|---|---|---|
| 朴素遍历 | O(1) | O(N) | O(1) |
| **Trie** | O(L) | O(L) | O(L) |
| Hash 表 + 展开通配 | O(N) | O(N) | O(N) |

> 注：L = topic 层级深度（通常 ≤ 5），N = 订阅规则总数

Trie 的匹配复杂度是 O(L)，与订阅规则总数 N **无关**——这是它能支撑百万级订阅规则的核心原因。

**特殊场景优化**：

```
场景：100 万个 IoT 设备全部订阅 sensor/{deviceId}/temp
      → 100 万条 sensor/device-001/temp 精确匹配规则
      → Trie 的 children map 在 sensor 下有 100 万个孩子
      
优化：子树压缩（Path Compression）
      sensor/device-* 前缀合并为一个"前缀节点"
      → 孩子数从 100 万压缩到 O(1)

更常见做法：sensor/{deviceId}/temp → 订阅 server/+ 的模板
            匹配前先做前缀哈希，减少 Trie 宽度
```

**结论**：Trie 是 MQTT Broker 主题匹配的**唯一正解**。所有生产级 Broker（EMQX、Mosquitto、VerneMQ）都基于 Trie 或其变体（Radix Tree / Prefix Tree）。这也是为什么 Broker 能支撑"千万设备、百万 topic"的规模——**匹配复杂度与订阅数脱钩**。

### 7.6 会话生命周期

**疑惑**：设备断连后，Broker 是否还记得"你是谁、订阅了什么、哪些消息还没发完"？

**论证**，MQTT 通过 **Clean Session 标志（3.1.1）和 Session Expiry（5.0）** 控制会话生命周期：

**Clean Session = true（默认）**：

```
设备 CONNECT 时 Clean Session=true：
  ├─ 连接建立：清空该 ClientID 的所有历史会话数据
  ├─ 在连期间：新订阅 → 都在内存
  └─ 断开时：Broker 立即清空所有：
       ├─ 订阅关系（topic subscriptions）
       ├─ 未完成的 QoS1/QoS2 消息队列
       └─ Will Message（遗嘱）
       
   → 设备每次断连都像"第一次来"——无状态、干净、简单
```

**Clean Session = false（持久会话）**：

```
设备 CONNECT 时 Clean Session=false：
  ├─ 首次连接：Broker 创建新会话
  ├─ 运行期间：Broker 持久化跟踪：
  │    ├─ 订阅列表（subscribe topics）
  │    ├─ QoS1/QoS2 未完成的消息（outbound queue）
  │    │   → 设备离线期间，消息积压在 Broker 侧队列
  │    └─ 已投递但未 ACK 的消息（inflight messages）
  │
  └─ 离线后：Broker 保持会话
       ├─ 新消息攒在队列里（有上限，默认 1000 条）
       └─ 设备重新 CONNECT → 批量下发积压消息 → 恢复如初

   → 适用于"断断续续"的 IoT 设备（传感器省电周期上报）
```

**MQTT 5.0 的 Session Expiry**（更精细的控制）：

```
CONNECT 包里可以设置 Session Expiry Interval（秒）：
  ├─ 0      → 断开立刻删除会话（等价 Clean Session=true）
  ├─ N      → 断开后保留 N 秒，超时删除
  └─ 0xFFFFFFFF → 永不过期（等价 Clean Session=false）

场景：
  设备说："我可能会离线 30 分钟（省电），请帮我保留会话"
  → Session Expiry = 1800（秒）
  → 30 分钟内重连，消息不丢；超时了就别等了
```
...
type Session struct {
    ClientID     string
    CleanStart   bool                  // MQTT 5.0 改名
    ExpiryAt     time.Time             // 过期时间
    Subscriptions map[string]QoSLevel  // topic → QoS
    Inflight     *InflightWindow       // 已发出待 ACK 的消息
    OfflineQueue []*PendingMessage     // 离线积压队列
    Will         *WillMessage          // 遗嘱消息
}

type InflightWindow struct {
    maxInflight int                    // 最大并发（默认 10）
    packets     map[uint16]*Packet     // packetId → 数据
    // 收到 PUBACK/PUBREC/PUBCOMP 删除对应项
}
```
...
```
风险：设备离线 3 天 → Broker 积压了 50 万条消息
     → 设备重连时一次性推送 → Broker OOM / 对端被打爆

防护：
  ├─ 离线队列上限：MaxQueuedMessages = 1000
  ├─ 超过上限策略：
  │    ├─ DROP_OLDEST（丢弃最早的消息）
  │    └─ DROP_NEWEST（不接收新消息）
  └─ 重连后限速下发（每秒 100 条，避免冲垮客户端）
```

**结论**：会话生命周期是 MQTT "专为不可靠网络设计"的核心体现。**Clean Session 控制记忆深度，Session Expiry 控制遗忘时机**。正确理解这两个参数，是 MQTT 生产部署不丢消息的前提。

### 7.7 Broker消息路由

**疑惑**：Broker 收到一条 `PUBLISH` 后，内部链路是怎样从"一个 socket 收进来"到"N 个 socket 发出去"的？

**论证**，以 QoS 1 消息 `topic=sensor/temp, payload=25.5°C` 为例，**完整内部链路**：
...
```
MQTT 规范规定：Broker 投递给订阅者时，QoS 可以**降级但不能升级**：

Publisher QoS 2  →  Subscriber QoS 1  →  Broker 用 QoS 1 投递
Publisher QoS 0  →  Subscriber QoS 2  →  Broker 仍用 QoS 0 投递（不升级）

原因：订阅者说"我只要 QoS 1 就行"，Broker 没理由做更重的投递。
      但订阅者说"我要 QoS 2"，Publisher 只发 QoS 0，Broker 无法凭空变出可靠性。
```

**流量控制：Inflight Window**

```
Inflight Window 防止 Broker 对单个订阅者"消息轰炸"：

  maxInflight = 10（默认）
  
  窗口满的时候：
    ├─ 新消息不发给这个订阅者，入离线队列等
    └─ 等收到一个 PUBACK，窗口空出一个槽 → 发下一条

  效果：对每个订阅者，最多同时有 10 条 QoS 1/2 消息在飞
        → 限速自然生效，不用另搞限流器
```

**结论**：Broker 的核心价值不是"转发消息"——负载均衡器也能转发——而是 **QoS 降级决策 + Inflight 窗口流量控制 + 离线积压存储**。这三件事任何 TCP Proxy 都做不了，只有"懂 MQTT 协议"的 Broker 能做。这就是为什么"用 Nginx stream 转发 MQTT"只是初级运维，"用 EMQX/Mosquitto"才是生产部署。

### 7.8 共享与集群

**疑惑**：单机 Broker 扛不住怎么办？MQTT 能集群吗？

**论证**，分两种扩展思路：

**一、共享订阅（MQTT 5.0）——消息级负载均衡**：

```
场景：100 台执行器都订阅 topic=cmd/execute
     但同一条命令只需要**一台**执行器处理（不是 100 台都执行）

传统订阅：
  Pub → cmd/execute → 所有 100 台执行器都收到（广播）
  ❌ 同一条命令执行 100 次

共享订阅（Shared Subscription）：
  Pub → $share/mygroup/cmd/execute
     → Broker 从 mygroup 组里**轮询选一台**执行器
     → 只发给 1 台
  ✅ 实现了消费者组级别的负载均衡
```

```
订阅语法：$share/{GroupName}/{TopicFilter}

示例：
  执行器 1：SUBSCRIBE $share/workers/cmd/execute
  执行器 2：SUBSCRIBE $share/workers/cmd/execute
  执行器 3：SUBSCRIBE $share/workers/cmd/execute

  发一条 cmd/execute → 轮询选一台（e.g. 执行器 2）
  
  分发策略：
    ├─ 轮询（Round Robin）：最简单
    ├─ 随机（Random）
    ├─ 哈希（Sticky by ClientID）：同设备始终同执行器
    └─ 最少未 ACK（Least Inflight）：当前最空闲的
```
...
│            ▼                     ▼                      │
│    共享订阅路由表           会话广播                    │
│    topic → worker节点       ClientID → 所在节点         │
│                                                         │
│    跨节点消息投递：                                      │
│     设备 A 在 Broker-1 → pub → 匹配到 Broker-2 的订阅   │
│      → Broker-1 通过集群内 RPC 转发给 Broker-2           │
└────────────────────────────────────────────────────────┘
```
...
| 架构 | 原理 | 适用规模 | 代表 |
|---|---|---|---|
| **全连接 Mesh** | 每台 Broker 互连，订阅全量同步 | ≤ 10 节点 | Mosquitto Bridge |
| **中心化路由** | 共享 Redis/etcd 存订阅路由表 | ≤ 100 节点 | 自研 |
| **对等式** | Mnesia 分布式数据库，无中心节点 | ≤ 20 节点 | EMQX |

**EMQX 集群示例**（对等式，Erlang/OTP）：
...
设备连接：
  A 连 Broker-1，订阅 sensor/#
  B 连 Broker-2，pub sensor/temp
  → Broker-2 查路由表 → 转发给 Broker-1 → Broker-1 投给 A

节点上下线：
  节点加入 → 自动同步路由表（增量）
  节点故障 → Mnesia 自动 failover → 路由指向健康节点
```

**结论**：MQTT 集群的本质是把"N 台独立 Broker"变成"一台虚拟的大 Broker"。**共享订阅解决消息级负载均衡，集群解决连接级负载均衡**——两者正交但协同，实现"连接可达千万、消息吞吐百万"的企业级规模。

### 7.9 安全认证

**疑惑**：MQTT 协议本身不定义认证——客户端连 Broker，怎么防止"谁都能 pub/sub"？

**论证**，MQTT 安全需要在**多个层次**叠加：

**第一层：传输加密（TLS）**

```
CONNECT 前先 TLS 握手：
  客户端 ──TLS Client Hello──▶ Broker
         ◀──Server Cert─────
         验证证书（CA链）
         ──Client Cert──────▶  (mTLS 模式)
         
  ├─ 单向 TLS：客户端验证 Broker 身份（防止连到假 Broker）
  └─ 双向 mTLS：双方互验（适用于设备证书出厂烧录的场景）
```

**第二层：CONNECT 认证**

```
CONNECT 报文里携带认证信息（明文，TLS 加密后传输）：

  3.1.1 版本：
    ├─ Username + Password（最基础）
    └─ ClientID（设备唯一标识）

  5.0 版本（增强认证 Enhanced Authentication）：
    ├─ Auth Method：认证方法（SCRAM-SHA-256, Kerberos 等）
    ├─ Auth Data： 认证数据（可多次交互）
    └─ 支持 SASL 框架，可插接自定义认证插件
```
...
Client                        Broker
  ...
  │    AuthData=client-first-msg │
  │                              │
  │ ◀── AUTH (Continue)          │
  │    ReasonCode=0x18           │
  │    AuthData=server-first-msg │
  │                              │
  │ ── AUTH (Continue)           │
  │    AuthData=client-final-msg │
  │                              │
  │ ◀── CONNACK (Success)        │
  │    ← 认证通过，连接建立      │

  支持多次 AUTH 往返，适配任意挑战-响应认证协议
```

**第三层：ACL 授权**

```
Broker 连接了 10 万台设备，凭什么相信设备 A 不会
订阅 device_B/private 的话题？

ACL（Access Control List）规则示例：

  # 用户名 = sensor-01 的权限：
  allow sensor-01 pub    topic=sensor/+/data       # 只能上报自己的数据
  allow sensor-01 sub    topic=cmd/sensor-01       # 只能收自己的命令
  deny  sensor-01 sub    topic=sensor/+/data       # 不能偷看别人数据
  deny  sensor-01 pub    topic=cmd/+               # 不能伪造命令

  # 超级用户：
  allow admin     pub    topic=#                   # 可以发任意话题
  allow admin     sub    topic=#                   # 可以订阅任意话题
```
...
```
存储：Redis/MySQL/内置文件
匹配：和 topic 匹配是同一棵 Trie，只是叶子节点存的是 ACL 规则

收到一条 PUBLISH sensor-01/data 时：
  1. 查 Username 对应的 ACL 规则
  2. 匹配 topic 是否在 allow 列表
  3. 匹配 topic 是否在 deny 列表（deny 优先）
  4. pub 操作匹配 pub 规则，sub 操作匹配 sub 规则
  
  → 通过 → 继续路由投递
  → 拒绝 → 断开连接 或 静默丢弃
```

**结论**：MQTT 的安全是**分层叠加**的——**TLS 保传输、CONNECT 认证保身份、ACL 保授权**。IoT 设备出厂时烧录 Client Cert + ACL 白名单，是"零信任"在物联网领域的经典实践。没有这三层，你的 MQTT Broker 就是"全网公开的 pub/sub 公告板"。
