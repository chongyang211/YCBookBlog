...
| 检测项 | 回答的问题 | 作用（不过 → 定位到哪） |
|---|---|---|
| ① 接口存在 | 有没有网卡 / Wi-Fi 开着？ | 飞行模式 / 无 SIM / 网卡驱动未识别——连接口都没有，后面全免谈 |
| ② 接口 UP | 接口启用了吗？ | 被禁用（admin down）——在但没开，启用即恢复 |
| ③ 链路有载波 | 网线 / 信号通了吗？ | 网线松 / 对端端口 down / Wi-Fi 未关联——开了但链路没建 |
| ④ 已拿到 IP | 拿到地址了吗？ | DHCP 失败 / 静态配置错——链路通了但没地址，照样上不了网 |
