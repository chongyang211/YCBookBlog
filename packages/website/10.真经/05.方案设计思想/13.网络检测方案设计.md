...
**问题的本质**：**App 把 5 层截然不同的故障统统折叠成一个"加载失败"提示**，既不告诉用户"你连的是 Wi-Fi 需要认证"，也不告诉客服"用户所在的运营商 DNS 有问题"。
...
为什么"能不能上网"这么简单的问题，工程实现却要拆七八层？一次正常的 HTTPS 请求要经过 **7 个环节**，任何一环挂了都算"网络问题"：
...
| 层 | 常见故障 | 用户表象 | 修复动作 |
|---|---|---|---|
| 物理 | 飞行模式 / 网线松了 / Wi-Fi 关了 | 一进 App 就转圈 | 让用户开网络 |
...
| TLS | 证书过期 / 时钟错 / SNI 被封 | 提示"证书错误" | 校准设备时间 |
...
为什么客服问用户 3 个问题都问不清楚？因为用户和工程师看到的是**两个完全不重叠的世界**：
...
用户只能看到结果层（图标、进度条），看不到协议栈里的过程层，所以"你 ping 一下试试"只会换来"什么 ping？"。

**结论**：不能指望用户描述问题，必须靠**工具在用户端自动跑一遍协议栈的每一层**，把结果翻译成"你现在的问题在第 X 步"。这就是**网络检测的存在理由**——一个"用户和协议栈之间的翻译器"。

### 2.3 准确与代价

既然要检测 7 层，全跑一遍不就得了？**用户耐心的物理上限是 3 秒**（Google 研究：3 秒后 53% 用户离开），检测代价必须控制在 3~5 秒：

| 策略 | 耗时 | 问题 |
|---|---|---|
| 全量检测（7 层都查 + 多目标） | 10~30 秒 | 用户不耐烦放弃，体验比不检测还差 |
| 只查 1 层（比如只 ping） | 1 秒内 | 假阳性/假阴性率 40%，定位不到根因 |
| **分层 + 短路（失败立即返回）** | 平均 2~3 秒 | 准确且不慢，90% 场景够用 |

**数学化推导**，设第 i 层耗时 $t_i$、故障率 $p_i$：

- 全量：$T_{full} = \sum t_i \approx 15\text{s}$
- 短路：$T_{short} = \sum t_i \prod_{j<i}(1-p_j) \approx 5.6\text{s}$

**短路把 15 秒压到 5.6 秒，准确率反而更高**（不被下游异常干扰上游诊断），这就是下一章"短路返回原则"的数学根据。

### 2.4 网络检测的本质

网络检测究竟在解决什么？不是"修复网络"，而是**确定问题的边界**：

```
           问题空间
┌─────────────────────────────┐
│ 用户端：物理/本地/连通      │ ← 让用户自己解
│ 中间路径：运营商/防火墙/DNS │ ← 建议换网络
│ 服务端：接入/业务/CDN       │ ← 让 SRE 去解
└─────────────────────────────┘
              ↓
    网络检测 = 一个"边界指针"
```

**网络检测 ≠ 修复；网络检测 = 用最低代价定位"问题在第几层、责任在谁"**。明确了边界，责任划分和修复动作才有可能。
...
为什么必须"从底向上"？倒过来不行吗？假设直接检测最高层（HTTP）：请求 `https://api.example.com/health`，返回超时。此时你**根本无法区分**：
...
短路能省多少？值得设计吗？**用 2.3 节的模型量化**，假设线上真实故障分布：
...
**论证**，**图标亮 ≠ 能上网**。物理层检的**不只是"连没连"，更要检"连了以后能不能上"**——图标亮（接口 UP）只说明连上了，离"能用"还差链路与地址两态。把这四个递进状态逐项过一遍，每一态都可能是断点：
...
四态之外，物理层还会顺带采集两个**非判定**信息：**传输类型**（Wi-Fi / 移动 / 以太网，决定上层提示文案）与**信号强度**（连接质量的物理层指标，辅助弱网分级）——它们不决定"能不能用"，但决定"告诉用户什么、要不要降级"。

**Linux/终端上，同一条判定链改用底层文件逐项读**（对应 03.1 的底层原理）：`getifaddrs` 枚举真实网卡（排除 `lo`/`tun*`/`docker*`/`br-*` 虚拟口）→ 读 `/sys/class/net/<iface>/operstate` 判 UP → 读 `carrier` 判链路 → 取首个非 0 的 IPv4，四步串行、任一步全灭即 fail-fast。**两平台 API 不同，但「存在 → UP → 载波 → IP」的判定链完全同构**。
...
**疑惑**：服务层分段计时测出来的是"延迟"（各段 RTT），但"慢"到底是因为"延迟高"还是"带宽低"？两者的优化手段完全不同——延迟高要换接入点，带宽低要压缩/降码率。怎么测带宽？

**论证**，带宽测速的本质是**在采样窗口内持续下载，用「传输字节数 ÷ 采样时长」算出稳态吞吐**，而不是用建连耗时。这正是 05.1 说的"质量层内嵌进服务层"——质量层的带宽测量，落在服务层的 HTTP 下载上。业界 iperf3 / speedtest 原理都一样，区别只在"传什么、传多久"。

**终端下载测速方案**：

```text
终端 ──HTTP GET /speedtest.bin──→ 服务端（静态随机字节文件）
                                  │
                                  ↓ 采样窗口 3~5s 内持续下载
带宽(kbps) = 累计下载字节数 × 8 ÷ 采样时长(s)
RTT(ms)    = 建连耗时 或 首字节 TTFB（口径 Plan 阶段定）
```

**探测文件为什么定 ~5MB（下限 2MB）**，核心目标只有一个：**让文件大到"弱网阈值内 3~5s 下不完"，从而测到稳态吞吐，而不是把建连耗时误当成带宽**：

| 约束 | 推导过程 | 取值 |
|---|---|---|
| 下限：文件要够大 | 良好下限 2Mbps × 5s ≈ 1.25MB，上浮留余量 | 2MB |
| 上限：覆盖测速区间 | 8Mbps × 5s ≈ 5MB，覆盖约 8Mbps 以下带宽，高带宽上报值不失真 | 5MB |
| 防失真 | 随机字节 + 服务端关闭 gzip（防压缩/缓存致测速虚高） | — |

**为什么不能用现成的小文件**：小文件不到 1KB，走不完 TCP 慢启动，只能测出 RTT，测不出带宽。所以由**终端自生成 ~5MB 随机字节文件**，服务端提供**免鉴权接口**供终端下载测速。

**文件大小的权衡**：

| 文件 | 优点 | 缺点 |
|---|---|---|
| < 2MB | 省流量、快 | 走不完慢启动，测的是建连耗时而非带宽 |
| **~5MB** | 反映真实带宽、耗时可控、流量适中 | 弱网下仍可能偏慢 |
| > 10MB | 测速更稳定 | 流量消耗大、用户等待久、弱网易超时、服务端带宽成本高 |
...
**终端分级落地方案**，上面的分级是"App 视角"，落到终端/自助设备时，分级目的是**上报 + 提示**而非自动降级——终端的业务动作是确定性的（该刷掌刷掌、该上传上传），降级反而引入不确定性：

| 等级 | 阈值（编译期常量） | 等效带宽 | 业务含义 |
|---|---|---|---|
| 良好 | 带宽 ≥ 2 Mbps 且延迟 ≤ 200ms | ≥ 250 KB/s | 业务上传与交互流畅 |
| 一般 | 1~2 Mbps 或延迟 200~500ms | 125~250 KB/s | 可用，上传略慢 |
| **弱网** | **< 1 Mbps 或 > 500ms 或连续 2 次测速失败** | **< 125 KB/s** | 上传明显变慢 / 超时风险 |
| 断网 | 检测失败（连通层 / 服务层不过） | — | 无法联网 |

**判定动作**：分级标记 + 上报 + 通知上位机，**不做自动降级**。

**海外适用性**，默认阈值以国内主流商户网络为基准，海外网络基础较差地区预期弱网占比偏高。三条缓解设计：

1. **弱网分级为提示性标记，不阻断业务**——误报不影响业务可用性；
2. **推送按等级变化触发**——平稳弱网不重复推送，避免持续打扰；
3. **海外阈值校准走版本迭代**——按区域上报数据分别复盘，海外区域可单独调整阈值。
