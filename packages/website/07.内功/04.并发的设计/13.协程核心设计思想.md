...
> 🔗 **延伸阅读**：← [3.11 异步和同步的设计](https://yccoding.com/pages/5a0952/) · ← [3.12 单线程模型的思想](https://yccoding.com/pages/5069b5/) · → [3.14 Actor 与 CSP 并发模型](https://yccoding.com/pages/ff832c/) · → [3.18 结构化并发设计思想](https://yccoding.com/pages/218459/) · → [2.4 函数调用栈与栈帧设计](..../pages/dc94ae/)
...
>
> **一句话**：协程就是把"函数 + 暂停按钮 + 存档读档"焊在一起。是一种“可暂停、可恢复”的函数，它把异步流程写得像同步代码，但底层并不会阻塞线程。编译器（或运行时）负责生成状态机，调度器负责决定谁什么时候跑。不同语言叫法不同，**三件套一个不落**。
