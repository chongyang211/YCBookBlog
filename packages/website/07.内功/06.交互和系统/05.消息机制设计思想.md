...
- [5.消息机制设计思想](#5消息机制设计思想)
  ...
  - [9.性能优化](#9性能优化)
    ...
    - [9.5 典型陷阱与反模式](#95-典型陷阱与反模式)
      - [陷阱一（Android）：Handler 内存泄漏](#陷阱一androidhandler-内存泄漏)
      - [陷阱二（Android）：主线程 ANR 三宗罪](#陷阱二android主线程-anr-三宗罪)
      - [陷阱三（iOS）：RunLoop Mode 误用](#陷阱三iosrunloop-mode-误用)
      - [陷阱四（Web）：microtask 死循环饿死宏任务](#陷阱四webmicrotask-死循环饿死宏任务)
      - [陷阱五（Flutter）：单帧超时导致 jank](#陷阱五flutter单帧超时导致-jank)
      - [陷阱六（嵌入式）：定时器优先级反转](#陷阱六嵌入式定时器优先级反转)
  - [10.总结与演进](#10总结与演进)
