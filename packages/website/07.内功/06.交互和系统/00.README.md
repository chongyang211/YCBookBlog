...
| 5.10 | [数据加密和解密](./10.数据加密和解密.md) | AES/RSA/HTTPS——数据从进程到网络到磁盘全链路的保护 |
| 5.11 | [国际化适配的设计](https://yccoding.com/pages/8f7c21/) | 200+ 语言 · 40+ 书写系统 · 160+ 货币 · 200+ 时区——文化的最后一公里 |
| 5.12 | [屏幕适配设计思想](https://yccoding.com/pages/9c7a15/) | 3000+ 分辨率 · 10+ 密度 · 5+ 形态——硬件的最后一公里 |
...
flowchart TB
    ...
    subgraph 最后一公里[" 跨端最后一公里 "]
        I18N["5.11 国际化适配<br/>文化的适配"]
        SCREEN["5.12 屏幕适配<br/>硬件的适配"]
    end
    A --> B --> BB --> C --> D
    D --> A
    C --> LC
    ...
    style F fill:#d1ecf1
    style E fill:#fce4ec
    style I18N fill:#c5e1a5
    style SCREEN fill:#c5e1a5
```
...
- **国际化篇**：为什么 iOS 的 Localizable.strings、Android 的 strings.xml、Web 的 i18next JSON——**三者是不是同一件事**？为什么波兰语的复数规则要 4 条？
- **屏幕适配篇**：Android 的 dp、iOS 的 pt、Web 的 rem、Flutter 的 logical pixel——**是不是同一个抽象**？为什么 iPad 上不能"简单放大"手机布局，必须做双栏？
...
5.9 响应式数据绑定（闭合交互闭环——数据如何驱动 UI）
   ↓
5.10 数据加密和解密（数据如何被保护——从进程到网络到磁盘）
   ↓
5.11 国际化适配（跨文化最后一公里——语言/时区/货币/RTL）
   ↓
5.12 屏幕适配设计思想（跨硬件最后一公里——像素/密度/尺寸/形态）
```
