...
- 华为 Mate 60 Pro 屏幕物理像素 = 2720×1260 个真实光点

**逻辑像素（Logical Pixel / Point / dp / vp）**：代码里用的抽象单位，与物理像素之间存在一个换算因子。

- iPhone 15 Pro Max 逻辑像素 = 430×932 pt，换算因子 = 1290/430 = 3（即 @3x）
- 华为 Mate 60 Pro 逻辑像素 = 420×907 dp，换算因子 = 2720/907 ≈ 3（即 @3x）

**为什么发明这个抽象层？**

| 写法 | 按钮声明 | iPhone 4（1x） | iPhone 4S（2x） | iPhone 15 Pro（3x） |
|------|---------|--------------|----------------|--------------------|
| 写物理像素 | 200px | 200px = 61mm ✓ | 200px = 31mm ✗ 变小一半 | 200px = 21mm ✗ 更小 |
| 写逻辑像素 | 100pt | ×1 = 100px ≈ 30mm | ×2 = 200px ≈ 30mm | ×3 = 300px ≈ 30mm |

**逻辑像素的定义**：**在肉眼看起来大小一致**，无论屏幕多高清——这就是所有平台"虚拟单位"的共同底座，**把"视觉大小"从"物理像素"里解耦**。
...
| 缩写 | 全称 | 是什么 | 含义 | 场景 |
|-----|------|------|------|------|
| **PPI** | Pixels Per Inch | 屏幕硬件的物理密度 | 每英寸物理像素数（硬件参数，出厂即定） | 屏幕硬件规格表 |
| **DPI** | Dots Per Inch | 一个被借用的历史名词 | 每英寸点数（源自打印机，被 Android 借来表达密度） | Android 系统 API 沿用 |
| **DPR** | Device Pixel Ratio | 一个换算系数 | 物理像素 / 逻辑像素（1 个逻辑点背后有几个物理点） | Web / iOS 前端常用 |
...
来看具体机型的对照，感受"抽象层"的威力。**先搞懂表格里每一列是怎么来的**——有的查规格、有的算出来：

| 参数 | 本质 | 怎么得到 | iPhone 15 Pro Max 实例 |
|------|------|---------|----------------------|
| 物理像素 | 硬件参数 | 厂商规格表直接公布，出厂即定 | 1290×2796 |
| 屏幕对角线 | 硬件参数 | 厂商规格表直接公布（英寸） | 6.7 寸 |
| PPI | 推导值 | `√(物理宽² + 物理高²) ÷ 对角线英寸` | `√(1290² + 2796²) ÷ 6.7 ≈ 460` |
| 逻辑像素 | 设计值 | 厂商定，也可由 `物理像素 ÷ DPR` 反推 | 430×932 |
| DPR | 推导值 | `物理像素 ÷ 逻辑像素` | `1290 ÷ 430 = 3` |

华为 Mate 60 Pro 同法验证：PPI = `√(2720² + 1260²) ÷ 6.82 ≈ 440`，落在 Android **xxhdpi 档**（480 dpi）→ DPR = 3，逻辑像素 ≈ `2720 ÷ 3 ≈ 907`。

表格里的 DPR 就是这么算出来的：比如 iPhone 15 的 DPR = 1179 ÷ 393 = 3，Pixel 8 Pro 的 DPR = 1344 ÷ 448 = 3.0。
