...
📍 **本篇位置**：第 5 卷 · 交互与系统 · 第 12 篇（跨设备的"最后一公里"）

🎯 **核心矛盾**：**"设计师只给了一张 375×812 的稿子" vs "真机世界有 3000+ 种分辨率、10+ 种像素密度、5+ 种形态（手机/平板/折叠屏/横竖屏/异形屏/车机 IoT/嵌入式）、无数种 DPR"**——同一份代码要在 iPhone SE 5.4 寸小屏、iPad Pro 12.9 寸大屏、华为 Mate X 折叠屏、比亚迪车机 15.6 寸横屏、200×200 的智能手表、480×272 的工控屏上都"看起来合理"，本质是把「像素 · 密度 · 尺寸 · 形态 · 安全区 · 字号」六个变量从代码里彻底解耦
...
🔗 **延伸阅读**：← [5.1 窗口核心设计思想](https://yccoding.com/pages/2f009c/) · ← [5.2 视图加载渲染设计](https://yccoding.com/pages/070bb8/) · ← [5.10 国际化适配的设计](https://yccoding.com/pages/8f7c21/)

💡 **通用心智**：忘掉具体平台的 API，记住一句话——**屏幕适配 = 「基准 + 单位 + 密度 + 弹性 + 断点 + 安全区」六段流水线**。Android 的 dp、iOS 的 pt、Web 的 rem、Flutter 的 logical pixel、鸿蒙的 vp 都只是"虚拟像素单位"这同一件事在不同平台的落地。**真正难的不是让 iPhone 15 好看，是让同一份代码在 iPhone SE / iPad Pro / Galaxy Fold 内屏 / 车机 12 寸 / 智能手表 / 老年模式放大字号 下都"合理"**。屏幕适配和国际化是"跨端最后一公里"的双胞胎——一个管**文化**、一个管**硬件**。
