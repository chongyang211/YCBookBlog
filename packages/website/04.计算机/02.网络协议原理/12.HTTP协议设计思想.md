...
**为什么用标准 header 而不是塞 body？**

traceparent 是 W3C Trace Context 国际标准字段——后台网关/服务（以及任何中间件如 Nginx、网关）都认识它，能自动从 header 里取出 trace-id 归入本次请求的服务端日志，这就是“后台可按 trace-id 检索”的基础。塞 body 的话就只有业务代码自己解析了。
