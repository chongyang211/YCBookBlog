...
> 覆盖 18 篇全部思考题。基础题给结论，进阶题给推导，动手题给命令和思路框架。
...
应用数据 → +TCP头（源/目的端口、序号、确认号）→ TCP段 → +IP头（源/目的IP、TTL）→ IP包 → +MAC头+FCS → 以太网帧 → 物理层比特流。接收侧按相反顺序逐层剥离。**每层只处理本层头部，这就是分层封装。**

**Q2：本网直连 vs 跨网走网关**

用子网掩码与目标 IP 按位与：结果与本机网段相同 → 本网直连，ARP 直接请求**目标主机**的 MAC；不同 → 跨网，ARP 请求**默认网关**的 MAC，包交给网关转发。`ip route get <目标IP>` 能直接看到内核的选择结果。

**Q3：NAT 与 conntrack 的分工**

NAT 负责私网/公网地址和端口的改写（动作）；conntrack 记录每条连接的状态（五元组+状态机），让回包能找到正确的映射条目（记忆）。没有 conntrack：NAT 无法处理 FTP 这类控制通道动态协商数据端口的协议，有状态防火墙也无从谈起。**NAT 是"动作"，conntrack 是"记忆"。**

### 进阶思考题

**Q1：Keep-Alive 的本质与关闭清理**

Keep-Alive 复用的是同一个文件描述符和 socket 收发缓冲区——省掉 TCP 三次握手和 TLS 握手的 RTT。正常关闭时内核依次：应用 `close(fd)` → 发 FIN 进入四次挥手 → TIME_WAIT（2MSL，确保最后 ACK 到达 + 防止旧连接残留包误入新连接）→ 释放 socket 缓冲区和文件描述符 → conntrack 表条目超时后删除。

**Q2：家用路由器的多层角色**

三层：WAN 口路由/NAT、DHCP 分配内网 IP；二层：LAN 口交换（MAC 地址学习）；应用层：DNS 转发、管理界面。"路由器"是商业名称，实际是**路由器 + 交换机 + 微型服务器三合一**。
...
0. 一次请求从按下回车到看到页面，在内核里依次经过了哪些子系统？（路由选路 → neighbour/ARP → netfilter → conntrack → socket → epoll → 软中断收包）
...
> **答案在本总结文件的 §19（贯通四句话）到 §21（诊断表）中。答不出任一题，回到对应篇章的速通卡重读。**
