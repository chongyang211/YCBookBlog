...
**面向协议编程（POP）** 是 Swift 的核心设计范式。通过协议扩展（Protocol Extension）为协议提供默认实现。
...
extension Flyable {
    func fly() { print("正在飞行...") } // 默认实现
}

struct Bird: Flyable {}      // 自动获得默认实现
struct Plane: Flyable {
    func fly() { print("Jet engine flying!") } // 可覆盖
}
```

## 8.4 泛型入门

泛型让你写出类型无关的通用代码，是 Swift 标准库的基石。
...
func findIndex<T: Equatable>(of value: T, in array: [T]) -> Int? {
    for (index, item) in array.enumerated() where item == value { return index }
    return nil
}
```
...
拓展在 Swift 实战中有四个黄金场景：

**场景一：代码分层。** 将协议遵循放在 Extension 中，保持主类简洁。
```
...
```
extension UIView {
    func roundedCorner(radius: CGFloat = 8) {
...
```
extension UIColor {
    static let themeBlue = UIColor(red: 0.2, green: 0.5, blue: 1.0, alpha: 1.0)
}
```

**场景四：协议默认实现。** Protocol Extension 提供默认方法，遵循者自动获得能力。
...
**weak vs unowned 选择口诀**：不确定谁先释放 → `weak`（可选，自动置 nil）；确定 self 后释放 → `unowned`。99% 的场景用 `weak`，用错 `unowned` 直接 crash。

**Timer 的正确写法**：
```
// ✅ iOS 10+ 新 API，自带 weak 效果
Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
    self?.tick()
}
// ❌ 旧 API target-action 会强引用
```
