...
有一年做一个 IM 项目，一个诡异的问题困扰我们两周——**同一个接口，99% 的请求都在 200ms 以内返回，但每天总有大约 1% 的请求会卡到 30 秒然后超时**。用户抱怨"消息发不出去"，但抓包看服务端明明是即时响应的。

我先怀疑运营商 DNS，换成 HttpDNS 之后好了一点点。再怀疑 TCP 建连慢，加了埋点才发现——**大部分超时请求根本没有走到 TCP 建连**，卡在 OkHttp 内部就出不来了。

后来找到根因是 `Dispatcher` 的默认参数：`maxRequests = 64, maxRequestsPerHost = 5`。IM场景下同一个 host 会瞬间发出去很多请求，一旦排在队列里的请求超过 5 个，后面的就要等前面完成才能执行。**这不是网络慢，是被 OkHttp 自己的调度队列卡住了**。

改成 `maxRequestsPerHost = 20` 之后问题消失。这次踩坑让我意识到，一个看起来"就是发 HTTP 请求"的库，内部藏着的机制远比表面复杂：

1. **Dispatcher 双队列 + 三线程池**——`readyAsyncCalls` 排队，`runningAsyncCalls` 执行，`executorService` 是无界线程池但用 `maxRequests`卡住并发。
2. **五个内置拦截器组成责任链**——重试、桥接、缓存、连接、请求，每个只做一件事，插新功能只加一个拦截器。
3. **连接池的引用计数机制**——一条TCP 连接可以被多个请求复用，用弱引用计数管理释放时机。
4. **HTTP/2 多路复用**——一条连接上并发多个 stream，彻底解决 HTTP/1.1 的队头阻塞。
5. **Okio 的 Segment 池化**——底层 IO 用 8KB 的 Segment 链表，避免大内存分配。
6. **不可变对象 + Builder**——Request/Response/Headers 全部 immutable，拦截器链传递零锁竞争。
7. **透明的重试与重定向**——底层出错自动重连、3xx 自动跳转，对上层完全透明。

这篇文章会把这七个点一个一个拆开来讲，最后用一次完整的 HTTPS 请求把它们串起来。读完你会发现——**OkHttp 不只是"HTTP 客户端"，它是一套"如何把复杂系统拆成可插拔组件"的教科书**。
...
## 11.一次 HTTPS 请求的完整生命周期

前面把每个模块拆开讲了，最后用一次真实请求把它们串起来——**用户点击"发送消息"，App 发起 `POST https://api.example.com/msg`**，我们跟着这个请求走完从代码调用到响应返回的每一步。
...
```
`newCall` 返回一个 `RealCall`，`enqueue` 把它交给 `Dispatcher`。

**t=1ms Dispatcher 调度**——`Dispatcher` 检查两个条件：
- `runningAsyncCalls.size < maxRequests`（默认 64）
- 同 host 的正在执行数`< maxRequestsPerHost`（默认 5）

两个条件都满足则加入 `runningAsyncCalls` 并提交给 `executorService`，否则加入 `readyAsyncCalls` 等待。此时线程池是**核心线程数0、最大线程数无限、SynchronousQueue**——保证不会有 Runnable在队列里排队，只会创建新线程立即执行。

**t=2ms 拦截器链启动**——`RealCall` 在工作线程执行 `getResponseWithInterceptorChain()`，构造拦截器列表：
```
[自定义应用拦截器...]
→ RetryAndFollowUpInterceptor  // 重试+重定向
→ BridgeInterceptor            // 补齐 Header
→ CacheInterceptor             // 缓存命中判断
→ ConnectInterceptor           // 建立连接
→ [自定义网络拦截器...]
→ CallServerInterceptor        // 真正发送请求
```
每个拦截器拿到 `chain` 后可以在 `proceed()` 前后做手脚，形成一个**双向责任链**——请求向下走，响应向上回。

**t=3ms BridgeInterceptor 补 Header**——自动补齐 `Content-Length`、`Host`、`Accept-Encoding: gzip`、`User-Agent`、`Cookie`。**开发者永远不用手写这些 Header**。

**t=4ms CacheInterceptor 检查缓存**——如果配置了 `Cache`，根据 `Cache-Control` 和 `ETag` 判断是否走缓存。这次是 POST 请求，跳过。

**t=5ms ConnectInterceptor 找连接**——先从 `ConnectionPool` 里找有没有可复用的连接：
- 同 host、同端口、同协议（HTTPS）
- 空闲状态、未过期（默认 5 分钟）
- HTTP/2 场景下即使正在使用也可复用（多路复用）

假设找到了一条空闲连接，**跳过 DNS + TCP + TLS 建连，直接进入下一步**——这就是**连接复用**带来的性能收益，一次 HTTPS 建连的时间通常在 200~500ms。

**t=6ms 假设没找到连接怎么办**——按顺序执行：
1. **DNS 解析**（`Dns.lookup("api.example.com")`）——拿到 IP 列表
2. **TCP 三次握手**——`Socket.connect(inetAddress, port)`
3. **TLS 握手**——`ClientHello → ServerHello → 证书校验 → 密钥交换`
4. **HTTP/2 协商**——通过 ALPN 决定用 HTTP/1.1 还是 HTTP/2

假设协商到HTTP/2，则创建一个 `Http2Connection`，之后所有请求都在这条连接上跑 stream。

**t=8ms CallServerInterceptor 发数据**——把 Request 的 Header 和 Body 写入 socket。**Okio 的 Segment 池化**在这里发挥作用——数据被切成 8KB 的 Segment，通过 SegmentPool 复用，避免每次都 `new byte[8192]`。

**t=200ms 收到响应**——Server 返回 200，Body 是 gzip 压缩的JSON。`BridgeInterceptor` 在响应回程时自动解压（因为请求带了 `Accept-Encoding: gzip`）。**这一层透明处理让上层完全感受不到压缩的存在**。

**t=201ms 归还连接**——`Response.body().close()` 被调用（或者 `body().string()` 读完），`ConnectionPool` 里这个连接的**引用计数减1**。当计数归零且空闲超过 5 分钟时，清理线程会把它关掉。

**t=202ms Dispatcher 收尾**——`RealCall` 从 `runningAsyncCalls` 移出，检查 `readyAsyncCalls`是否有请求可以晋升（比如刚才因为 host 并发限制在等的）。

**t=203ms 回调返回**——`callback.onResponse(call, response)` 被调用，用户拿到结果。

从发起到完成 200 多毫秒，其中真正的网络 IO 只有 195 毫秒，其他都是框架内部的调度和处理。**如果连接被复用**，整个过程可能只有 30 毫秒——这就是 OkHttp 相比 HttpURLConnection 显著更快的秘密。

## 12.OkHttp 的六条设计哲学

**哲学一：拦截器链是骨架，一切都是"链上的一环"。** 重试、缓存、连接、发送——每个功能都是一个拦截器，可以自由增删。**新增功能只加拦截器，不改已有代码**，这是开闭原则的极致体现。想加日志加拦截器、想加 mock 加拦截器、想加加密加拦截器——你甚至可以整条链换成自己的实现。

**哲学二：不可变对象让并发变简单。** Request/Response/Headers 全部是immutable，拦截器链在多个线程之间传递它们不需要任何锁。**想改？newBuilder() 出一个新对象**。这是函数式编程思想在Java 世界的落地——**不可变 = 可安全共享**。

**哲学三：连接是资源，池化是必须。** 一次 HTTPS 建连 200~500ms，一次 TCP 建连也要 100ms。ConnectionPool 用**引用计数 + 空闲超时**管理连接生命周期，让一条连接可以服务成百上千个请求。**任何昂贵资源都应该池化**——线程、连接、Bitmap、ByteBuffer 都是。

**哲学四：透明处理让API 消失在开发者视野里。** GZIP、Cookie、重试、重定向——这些底层脏活OkHttp 自动做完，开发者只感知到"请求和响应"两个概念。**优秀 API 的标志是"让复杂的事变简单，让脏活变透明"**。

**哲学五：Dispatcher 用双队列做流量整形。** `readyAsyncCalls` 排队，`runningAsyncCalls` 执行。这种"入口有序、出口并发"的结构可以在流量峰值下**避免服务器被打垮**（同 host 限流）也**避免自己被打垮**（全局限流）。这个模板在消息队列、任务调度、限流器里到处能看到。

**哲学六：底层 IO 用池化 + 分段 + 零拷贝三板斧。** Okio 的 Buffer 是 Segment 链表，Segment 从 `SegmentPool` 里拿。想拼接两个 Buffer？直接改指针，**不复制字节**。这套设计让 Okio 的 IO 性能远超Java 原生 InputStream/OutputStream。**做 IO 密集型系统，永远想着"能不能不复制"**。

## 13.速查表：常见问题一图流

| 问题 | 答案速查 |
|------|---------|
| Dispatcher 为什么用双队列？ | ready 是等待区，running 是执行区，通过 maxRequests/maxRequestsPerHost 做流量整形 |
| 线程池为什么用 SynchronousQueue？ | 没有队列容量，任务来了立即创建线程执行，配合 `maxRequests` 做并发上限 |
| 五个内置拦截器的顺序为什么固定？ | 语义决定顺序——重试要包住重定向，缓存要在建连之前，Bridge 要在最外层补 Header |
| 应用拦截器和网络拦截器有什么区别？ | 应用拦截器在 Retry 之前（每个请求只执行一次），网络拦截器在 Connect 之后（重试/重定向时会多次执行）|
| ConnectionPool 靠什么复用连接？ | 引用计数 + 空闲超时（默认 5 分钟）+ 后台清理线程 |
| HTTP/2 的多路复用是什么？ | 一条 TCP 连接上并发多个 stream，彻底消除 HTTP/1.1 的队头阻塞 |
| Request 为什么设计成不可变？ | 拦截器链多线程传递，不可变 = 无需加锁 = 天然线程安全 |
| Okio 比 Java IO快在哪里？ | Segment 池化 + 分段链表 + 零拷贝拼接，减少内存分配和 GC |
| 为什么建议全局单例 OkHttpClient？ | 每 new 一个都是一套线程池 + 连接池 + 缓存，浪费资源；`newBuilder()` 可以在不建新池的前提下微调配置 |
| 请求怎么取消？ | `call.cancel()`，底层会 close socket，让阻塞在 IO 上的线程抛 IOException 提前返回 |
| 超时有几种？ | connectTimeout（建连）+ readTimeout（每个字节间隔）+ writeTimeout（写字节间隔）+ callTimeout（整个请求端到端）|
| DNS 可以换吗？ | 可以，实现 `Dns` 接口即可，HttpDNS 就是这么接入的 |

理解了这张表，OkHttp 相关的面试题基本都能拿下。但更重要的是——**这些设计可以直接拿去做别的系统**：想做一个高并发任务调度器？Dispatcher 就是模板；想做一个可扩展的框架？拦截器链就是钥匙；想做一个资源池？ConnectionPool 的引用计数就是范本。**读OkHttp 源码不是为了记住 OkHttp，是为了把"高并发 + 可扩展 + 高性能"这三张牌怎么打好，看得清清楚楚**。
