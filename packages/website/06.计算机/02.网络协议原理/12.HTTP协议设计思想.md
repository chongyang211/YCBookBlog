...
- [04.HTTP请求与响应](#04http请求与响应)
  ...
  - [4.2 请求方式（HTTP方法）](#42-请求方式http方法)
  - [4.3 请求包的设计](#43-请求包的设计)
  - [4.4 响应包的设计](#44-响应包的设计)
  - [4.5 报文空行设计](#45-报文空行设计)
  - [4.6 请求快速定位traceid](#46-请求快速定位traceid)
  - [4.7 看一个完整案例](#47-看一个完整案例)
...
- [09.综合案例 API](#09综合案例-api)
  ...
  - [9.2 随意风格版](#92-随意风格版)
  - [9.3 第二版：RESTful化——规范化](#93-第二版restful化规范化)
  - [9.4 高级特性](#94-高级特性)
  - [9.5 生产级设计](#95-生产级设计)
...
请求行的第一个字段就是**请求方法（HTTP Method）**，它告诉服务器"客户端想对资源做什么操作"。
...
HTTP/1.1 定义了 8 种标准方法，但日常开发真正高频使用的只有 4 种：

| 方法 | 语义 | 典型场景 | 是否带请求体 |
|------|------|---------|------------|
| GET | 获取资源 | 查询列表、读取详情 | 通常不带 |
| POST | 创建资源 / 提交数据 | 新增、登录、表单提交 | 带 |
| PUT | 全量更新资源 | 覆盖式更新整个资源 | 带 |
| DELETE | 删除资源 | 删除指定资源 | 通常不带 |
| PATCH | 局部更新资源 | 只改资源的某几个字段 | 带 |
| HEAD | 只取响应头 | 检查资源是否存在、拿元信息 | 不带 |
| OPTIONS | 查询支持的能力 | CORS 预检、探测允许的方法 | 不带 |

**三个容易混淆的点**：

1. **POST vs PUT**：POST 表示"创建"，PUT 表示"全量替换"。区别不只是名字——POST 通常不幂等（重复提交会创建多份），而 PUT 幂等（提交多次结果一样）。
2. **PUT vs PATCH**：PUT 是整体覆盖，PATCH 是局部打补丁。`PUT /users/123` 要传完整对象，`PATCH /users/123` 只传要改的字段。
3. **GET 带不带请求体**：规范没有明确禁止，但大多数服务器和中间件会忽略或拒绝，所以实践中 GET 不带请求体，参数走 URL 查询串。

方法的这些语义差异，背后是一套设计哲学——**安全性**（会不会改资源）和**幂等性**（重复执行结果是否一致），这部分在 07.HTTP方法的设计哲学 里专门展开。

回到踩坑②的场景：前端 `fetch` 发的是 DELETE 请求，属于"非简单请求"，浏览器会先发一个 OPTIONS 预检——这正是上面表格里 OPTIONS 方法的真实用途，详见 8.3 跨域资源共享。
...
**问题**：一次 HTTP 请求从客户端到服务器，中间可能经过网关、鉴权、业务服务、数据库、缓存……某个环节报错时，你拿到的往往只有一句"接口 500"。在上万行日志里，怎么快速捞出这一次请求相关的所有记录？

答案就是 **traceId（链路追踪 ID）**——给每一次请求分配一个全局唯一的 ID，让它在整条调用链上透传，并把所有日志都打上这个 ID。

**traceId 是怎么工作的**：

```text
① 生成：请求入口（网关或第一个服务）生成全局唯一 ID，不要在中间环节生成（否则链路会断）
② 传递：放进请求头，调用下游服务时必须把 traceId 放进请求头传下去，不能丢
③ 记录：每个服务打印日志时都带上这个 ID。每个服务的每条日志都要打印 traceId，这是能搜出来的前提。
④ 回传：响应头里带回同一个 ID，返回给调用方。注意：有的服务是没有做这一步
⑤ 排查：拿着 ID 去日志系统搜，一次请求的全链路日志全部捞出来。不用再靠时间戳、IP、接口名去"猜"是哪一次请求。
```
...
| 头部 | 来源 |
|------|------|
| `X-Request-Id` | 最通用的约定，Nginx / 网关常用 |
| `X-Trace-Id` | SkyWalking、Spring Cloud Sleuth 等 |
| `X-B3-TraceId` | Zipkin / OpenZipkin 的 B3 规范 |
| `traceparent` | W3C Trace Context 标准（新项目推荐） |

**代码视角：一段真实的 traceparent 生成实现**。理解了设计原则，再看一段 C++ 实现（W3C Trace Context 标准）：

```cpp
std::string Network::GenerateTraceparent() {
  if (current_trace_id_.empty()) {                   // ② 是否开新链
    current_trace_id_ = utils::StringUtil::GenerateRandomHexString(32); // ③ trace-id
  }
  std::string span_id = utils::StringUtil::GenerateRandomHexString(16); // ④ span-id
  return fmt::format("00-{}-{}-01", current_trace_id_, span_id);        // ⑤ 拼装
}
```

返回值 `00-{trace-id}-{span-id}-01` 四段含义：

| 段 | 值 | 含义 |
|---|---|---|
| `00` | 版本号 | W3C 当前固定为 `00` |
| `{trace-id}` | 32 个 hex（16 字节） | 整条调用链的 ID |
| `{span-id}` | 16 个 hex（8 字节） | 当前跨度的 ID |
| `01` | 采样标志 | `01`=上报，`00`=不上报 |

五个关键点：

1. **加锁**：`current_trace_id_` 是共享状态，多线程并发调用时必须加锁，否则不同请求会互相覆盖 trace-id，串到同一条链上。
2. **`if (current_trace_id_.empty())` 是核心**：只有「强制开新链」或「还没有链」时才生成新 trace-id，否则复用——这正是「同一条链上 trace-id 不变」的代码体现。
3. **trace-id 32 hex**：整条链共用，排查时一搜全出来。
4. **span-id 每次都新生成**（注意它不在 `if` 里）：即使 trace-id 复用，每个服务节点仍是不同的跨度，所以 span-id 必须每次新建。
5. **末尾 `01` 硬编码全量上报**：高并发下要改成按概率采样，否则追踪系统会被打爆。

对照前面四个设计要点：入口生成（`start_new_trace`）、全链路透传（trace-id 复用）——这段代码把前两条真正落到了实处。
...
承接 4.2 的请求方式清单：4.2 讲了"有哪些方法、各自做什么"，本节讲这些方法背后的设计哲学——安全性与幂等性。
