...
- [11.HTTP协议设计思想](#11http协议设计思想)
  ...
  - [04.HTTP请求与响应](#04http请求与响应)
    ...
    - [4.5 请求快速定位traceid](#45-请求快速定位traceid)
    - [4.6 看一个完整案例](#46-看一个完整案例)
...
```text
① 生成：请求入口（网关或第一个服务）生成全局唯一 ID
② 传递：放进请求头，调用下游服务时透传下去。
③ 记录：每个服务打印日志时都带上这个 ID
④ 回传：响应头里带回同一个 ID，返回给调用方。注意：有的服务是没有做这一步
⑤ 排查：拿着 ID 去日志系统搜，一次请求的全链路日志全部捞出来
```
...
```text
# 请求头
GET /api/order/detail?id=12345 HTTP/1.1
Host: api.example.com
X-Request-Id: 7f3a9c2e-5b81-4d6f-9e0a-1c8b7d2f4e93
...
用户报障时只要给出这个 `X-Request-Id`，就能在日志系统里精确捞出这一次请求的全部记录——不用再靠时间戳、IP、接口名去"猜"是哪一次请求。

**设计 traceId 的四个要点**：

1. **在入口生成**：网关或第一个接收请求的服务生成，不要在中间环节生成（否则链路会断）
2. **全链路透传**：调用下游服务时必须把 traceId 放进请求头传下去，不能丢
3. **日志必带**：每个服务的每条日志都要打印 traceId，这是能搜出来的前提
4. **高并发要采样**：全量上报成本很高，生产一般按比例采样（如 10%），但错误请求要全量保留

**代码视角：一段真实的 traceparent 生成实现**

理解了设计原则，再看一段 C++ 实现（W3C Trace Context 标准）：

```cpp
std::string Network::GenerateTraceparent(bool start_new_trace) {
  if (start_new_trace || current_trace_id_.empty()) {                   // ② 是否开新链
...
2. **`if (start_new_trace || current_trace_id_.empty())` 是核心**：只有「强制开新链」或「还没有链」时才生成新 trace-id，否则复用——这正是「同一条链上 trace-id 不变」的代码体现。
