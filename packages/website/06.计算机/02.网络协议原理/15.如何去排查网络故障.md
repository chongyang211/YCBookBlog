...
- [01.工作案例引入](#01工作案例引入)
  - [1.1 看一个案例](#11-看一个案例)
  - [1.2 排查的路径](#12-排查的路径)
  - [1.3 网络排查方法](#13-网络排查方法)
- [02.使用Ping工具](#02使用ping工具)
...
- [03.用ifconfig查询网络](#03用ifconfig查询网络)
  ...
  - [3.2 ifconfig 输出解读](#32-ifconfig-输出解读)
  - [3.3 排查实战](#33-排查实战)
...
网络故障排查的最高效方法是**自底向上按OSI模型逐层排查**。为什么是自底向上？因为下层故障会导致上层全部失败，但上层故障不影响下层。

检查顺序：物理层（网线插了吗？灯亮吗？）→ 链路层（ARP能解析MAC吗？）→ 网络层（`ping`能通吗？TTL正常吗？）→ 传输层（`telnet IP port`端口通吗？）→ 应用层（`curl -v`能拿到响应吗？）。

90%的网络故障在传输层以下，而你最可能一开始就跳到了应用层检查——浪费大量时间。记住这条铁律：**先ping再telnet最后curl**。
...
ifconfig（interface config）用于配置和显示网络接口信息：IP 地址、子网掩码、MAC 地址、MTU 等。它是排查链路层和网络层问题的第一站——网卡有没有起来、有没有拿到 IP、收发有没有异常，一条命令全看清。

常用命令：`ifconfig`（显示所有接口）、`ifconfig eth0`（查看特定接口）、`ifconfig eth0 up/down`（启用/禁用网卡）、`ifconfig eth0 mtu 1500`（设置 MTU）。
...
eth0: flags=4163<UP,BROADCAST,RUNNING,MULTICAST>  mtu 1500
      ...
      inet6 fe80::a00:27ff:fe8b:1234  prefixlen 64  scopeid 0x20<link>
      ether 08:00:27:8b:12:34  txqueuelen 1000  (Ethernet)
      RX packets 1048576  bytes 912345678 (870.1 MiB)
      RX errors 0  dropped 0  overruns 0  frame 0
      TX packets 678912  bytes 456789012 (435.6 MiB)
...
| 字段 | 含义 | 排查用途 |
|------|------|---------|
| `inet` | IPv4 地址 | 没有这行 = 网卡没拿到 IP（DHCP 失败） |
| `netmask` | 子网掩码 | 判断所属网段 |
| `ether` | MAC 地址 | 确认物理地址、排查 ARP |
| `mtu` | 最大传输单元 | 值不对会导致分片、影响吞吐 |
| `flags` | 接口状态标志 | 缺 `RUNNING` = 网线没插好或链路断 |
| `RX/TX errors、dropped` | 收发错误/丢包计数 | 持续增长 = 链路质量或硬件问题 |

### 3.3 排查实战

1. **网卡没 IP**：`ifconfig eth0` 没有 `inet` 行 → DHCP 没拿到地址。排查：`ifconfig eth0 up` 启动网卡 → 检查网线是否插好 → `dhclient eth0` 重新获取地址。
2. **链路丢包**：`RX/TX errors`、`dropped` 计数持续增长 → 网线老化、网卡故障或交换机端口问题。排查：换网线、换端口对比计数是否停止增长。
3. **MTU 问题**：`mtu` 不是标准值（如 PPPoE 环境是 1492）→ 大包被分片，吞吐下降。排查：`ifconfig eth0 mtu 1500` 调整后重新测速。
