...
- [25.案例FlowKit审批引擎](#25案例flowkit审批引擎)
  - [📚 渐进学习节奏](#-渐进学习节奏)
  - [FlowKit 技术全景](#flowkit-技术全景)
    - [技术栈分层详解](#技术栈分层详解)
    - [为什么审批引擎是 23 种模式的天然考场](#为什么审批引擎是-23-种模式的天然考场)
  - [案例元信息](#案例元信息)
  - [📋 目录快速导航](#-目录快速导航)
  - [01.需求说明](#01需求说明)
  ...
  - [05.审批链流转 【第2次会话·OOP高峰⭐】](#05审批链流转-第2次会话oop高峰)
  ...
  - [06.创建型五连 【第3次会话·对象怎么"生"】](#06创建型五连-第3次会话对象怎么生)
  ...
  - [07.命令与撤销 【第4次会话·毕业关上⭐】](#07命令与撤销-第4次会话毕业关上)
  ...
  - [08.条件DSL与任务树 【第4次会话·AST高峰⭐】](#08条件dsl与任务树-第4次会话ast高峰)
  ...
  - [09.多渠道通知 【第5次会话·横切高峰⭐】](#09多渠道通知-第5次会话横切高峰)
  ...
  - [10.终局六连 【第5次会话·收尾⭐】](#10终局六连-第5次会话收尾)
  ...
  - [12.项目总结](#12项目总结)
    ...
    - [12.1.1 五次会话的演变全景 【🔑】](#1211-五次会话的演变全景-)
    - [12.2 23 模式覆盖回顾 【🔑】](#122-23-模式覆盖回顾-)
    - [12.3 故意不用模式的三个地方](#123-故意不用模式的三个地方)
  - [13.技术思考](#13技术思考)
  - [14.衔接与延伸](#14衔接与延伸)
    - [14.1 与专栏 23 篇的衔接](#141-与专栏-23-篇的衔接)
    - [14.2 三个延伸挑战](#142-三个延伸挑战)
    - [14.3 配套源码包](#143-配套源码包)
  - [尾声寄语](#尾声寄语)
...
> 🔥 **模拟事故复盘 · 双 12 大促第 3 天 · "87 张被驳回的退款单，悄悄复活了"**
>
> 双 12 大促进入第三天，售后平台接到一个"诡异"的财务工单：**月度对账发现实际退款金额比审批系统记录多了 23 万**。追查 6 个小时后定位到一条链路：某外包团队两年前交付的审批模块里，`withdraw`（撤回）命令**漏判断了"已驳回"状态**——被驳回的退款单，申请人点一下"撤回"就能回到草稿、改个金额重新提交，**审批链从头到尾没有任何拦截**。87 张"复活单"里，有 31 张是故意钻空子的（同一申请人反复测试出的路径），剩下 56 张是无意的重复操作。
>
> 复盘会很尴尬：
> - 该模块核心是一个 **400 行的 GodClass**：状态判断、审批层级、审批人查找、通知发送全部长在一起；
> - 两年来 5 个团队往里塞规则，**没人能在脑内装下它全部的分支**，谁也不敢重构；
> - 测试一片空白——不是不想写，是**根本没法写**：任何单测都要先构造"半个系统"才能跑到目标分支。
>
> 复盘结论写在白板上：**不是"代码不严谨"，是结构性的"变化无处安放"**——状态变了、层级变了、审批人规则变了、通知渠道变了，四种变化在同一个方法里打架，赢了的那次，叫"碰巧没出事"。
>
> 本章要造的 FlowKit，就是把这个事故**重演一遍再修复一遍**：从同样的 if-else 骨架起步，让每类变化一个一个打过来，看 23 种模式如何逐个把它们接走。

**这个开场也回答了"为什么要从 0 到 1 造轮子"**——不是为了替代 Activiti，而是：只有亲手经历"分支腐烂 → 痛点堆积 → 结构置换"的完整周期，模式才会从"知识"变成"手感"。直接读设计好的成品代码，你只能学到"是什么"，学不到"**为什么非它不可**"——而后者才是工程能力的本体。
...
**三次失败探索**——按专栏惯例，先看三个"看起来可行"的方案怎么一个个翻车：

> **尝试一：把格子补全**。第一反应是"漏格子补格子"：在 wiki 上列出 5 状态 × 4 命令 = 20 个格子，code review 逐格核对。**翻车**：第二周产品加了一个"挂起 HOLD"状态，矩阵变 24 格；第三周又加"催办"命令，变 30 格——**文档和代码从第二周起就没再同步过**，你只是把 if-else 的混乱复制了一份到 wiki，还多了一个"哪份是真的"的新问题。

> **尝试二：查表法**。把"允许矩阵"搬进 `Map<状态, Set<允许动作>>`，运行时查表放行。**翻车**：查表只能回答"**能不能做**"，回答不了"**做完去哪**"——`withdraw` 在 `REVIEWING` 下允许（回到 DRAFT）、在 `REJECTED` 下要拒绝且提示"驳回单请重新提交"——同样的动作在不同状态下**去向和文案都不同**，转移逻辑还是得写 if-else。表只答了半张试卷，另外半张留在原地腐烂。

> **尝试三：让状态自己说话**。既然"能不能做 + 做完去哪 + 提示什么"三件事都随状态变，那就让**每个状态对象自己封装这三件事**——这就是状态模式的全部动机。下面 Step 2.2 → Step 2.3 的两步重构（enum 止血 → 状态类根治），就是这个结论的落地。
...
先说**怎么搬家不炸**——重构的顺序比结构本身更重要：

> **第 1 步**：给 `Ticket` 加 `state` 字段，但**先不删任何旧 if**——新旧两套并存，旧逻辑照跑（绞杀者模式的最小形态）；
> **第 2 步**：逐个命令（submit/approve/reject/withdraw）改成委托给状态对象，每改一个就跑一遍 §02 的 REPL 验收——四个命令就是四步，每步都可回退；
> **第 3 步**：全绿之后，删掉最后一行旧的状态 if——此时旧代码已经是"死代码"，删它不需要勇气，只需要一次 `git rm`。

这个"先并存、再切换、后删除"的节奏，比"一把梭重写"多花 20 分钟，换来的是**任何一步出问题都能回到上一步**。真实工程里的模式重构几乎全是这个形状——大爆炸重写的项目，大多死在"切到一半两套都要维护、谁也不敢再动"的半山腰。
...
> 🔥 **模拟事故复盘 · 新零售部门接入第 3 天 · "47 张单积压 6 小时，理由是'应该由 zhangsan 审批'"**
>
> 平台要接入新零售事业部。代码库里审批人是写死的（`zhangsan` / `lisi`）。接入第三天，新零售的员工提交售后单后集体卡死：所有审批请求都被打回一句 **"本节点应由 zhangsan 审批"**——但新零售根本没有叫 zhangsan 的人。47 张单积压 6 小时，客服接到 30+ 通投诉。
>
> 更深的复盘发现这不是"写错了名字"那么简单：**审批人规则至少有三种完全不同的形态**——直销部按"直属主管"、财务部按"角色池轮询"、大额单按"金额阶梯"，而且将来还会更多（外部邮箱审批、项目组审批、请假期间的"审批人代理"……）。写死任何一种，都是把"明天"焊死在"今天"。

**三次失败探索**：

> **尝试一：配置文件写名单**——把审批人挪进 properties。**翻车**：名单每周都在变（请假、离职、转岗），而且财务的"金额阶梯"根本不是名单、是**逻辑**——配置格式表达不了 if，硬塞就要发明一套配置 DSL（那已经是 §08 解释器的活了，此处先按下不表）。

> **尝试二：每个部门一个子类**——`SalesApproverNode extends ApproverNode`、`FinanceApproverNode extends ApproverNode`。**翻车**：两周后出现"财务部的大额单"——部门 × 金额阶梯两个维度**交叉**，继承是单维度的，交叉需求逼出 `FinanceHighAmountApproverNode` 这样的子类爆炸；而且子类在编译期就定死了，"按金额切策略"这种运行期才能确定的事，继承根本接不住。

> **尝试三：把所有规则塞进一个方法**——就是下面这段反面教材。它比前两次探索活得久（毕竟集中在一处），但代价是那个方法成为全系统最不敢碰的 40 行。
...
```mermaid
flowchart TD
    A{算法会被整体替换吗?} -->|不会 只有一套| B[直接写方法体<br/>别上模式]
    A -->|会 但编译期就能确定| C[子类多态 / 模板方法]
    A -->|会 且运行期才能确定| D[策略模式]
    D --> E{策略实现无状态且简单?}
    E -->|是| F[lambda / 方法引用<br/>一行就是一个策略]
    E -->|否| G[策略类 + 注册表]
    G --> H{规则要给运营改吗?}
    H -->|是| I[升级为解释器 §08]
    H -->|否| J[到此为止]
...
> 树的最后一层是本案例最想传递的判断力：**同一个"换算法"的痛点，程序员维度用策略就够了，业务维度必须升级为解释器**——分界线不是技术复杂度，是"变化权在谁手里"。
...
> 🔥 **模拟事故复盘 · "加一级财务审批"需求 · 改了 4 处，漏了 1 处**
>
> 风控新要求：所有超过 1000 元的退款单，在组长和主管之间插入"财务复核"。听起来只是"加一层"，但在 MVP 结构里它意味着：`approve()` 里插 if、`reject()` 的恢复逻辑里插 if、`withdraw()` 的重置逻辑里插 if、测试用例重写——**同一个"层级"概念被复制在 4 个方法里**。上线当晚，被驳回的单子重新提交后**跳过了财务复核直接到了主管**（`reject` 的恢复链路漏改），又流走了 3 张异常单。
>
> 根因：**"审批层级"是数据（一串节点的有序列表），却被写成了逻辑（散落各处的 if）**。数据变了逻辑就该零改动——这正是职责链模式要兑现的承诺。

### Step 4.1 审批链骨架

同样先交代搬家路线：① 新建 `ApproverNode`，但**先不删 `approve()` 里的 if**；② 让 if 的每个分支内部改为调用链节点（逻辑搬家、行为不变）；③ REPL 验收通过后，把 if 壳整个拆掉——调用方从此只认识链头。**每一步的 diff 都小到能在 code review 里活下来**，这是重构能进行到底的唯一前提。现在把 MVP 里硬编码的三层 if 改造成链。每个审批节点是职责链上的一个 Handler：
...
```mermaid
flowchart TD
    A{处理者之间是接力还是全员?} -->|接力 命中即止| B[纯职责链]
    A -->|全员 逐层穿透| C[过滤链 / 洋葱链]
    B --> D{节点顺序运行期会变吗?}
    D -->|不变| E[组装期 .then 连死]
    D -->|变 加签/灰度/插队| F[链 + 外科手术方法]
    C --> G{需要看到前后包夹吗?}
    G -->|需要 before/after| H[洋葱模型 OkHttp]
    G -->|不需要| I[Servlet Filter 式]
```

> 对照记忆锚点：审批 = 接力（B 分支），通知 = 全员（C 分支），加签 = 运行期变序（F 分支）——本案例三个真实需求，恰好把三个分支各走了一遍。
...
> 🔥 **翻车现场 · 两个团队的 case 打架**：三个月后节点种类从 2 种涨到 5 种（下面会看到），创建逻辑也从"一行 new"膨胀成"带校验、带组装"的十几行。会签组团队往 `create()` 的 `AND_SIGN` 分支里加比例校验，同一周条件节点团队重构了 `CONDITION` 的参数解析——合并时 **5 个 case 分支互相覆盖**，条件节点带着会签的校验逻辑上了线，`ratio` 参数校验把所有条件节点全部拦下。一个 switch 就是一个**单点**：所有节点类型的创建知识挤在一个方法里，改一个 = 触碰所有。
...
> 🔥 **翻车现场 · 两个相邻参数换了位置**：先看不用建造者时最经典的事故形态——`new ProcessDef("refund", 1, true, 30, nodes, edges)`。第 3 个参数 `withdrawable`（boolean）和第 4 个参数 `timeoutHours`（int）**在调用处传反了**……不对，编译器拦下了 int/boolean？那把 `timeout` 也改成 boolean 试试：`new ProcessDef("refund", 1, false, true, ...)`——**这次编译器一声不吭**。线上跑了两天才发现"退款流程不可撤回、也不超时催办"。**相邻同类型参数是构造函数的天然盲区**，而流程定义恰好有一堆开关型配置。
>
> 更深一层：流程定义的构造是**分阶段的**（节点先全部创建、再统一连线），裸构造函数表达不了"分步"——中间态（有节点没连线）就是一颗定时炸弹，谁来 new 谁负责记住"还差哪一步"。
...
> 🔥 **模拟事故复盘 · "手滑的 48 万"**
>
> 采购主管在手机上处理积压待办，列表滑动误触，把一张 48 万的采购单**点了通过**——本意是驳回。系统没有撤销功能，善后流程是：打电话给财务拦款（财务回复"款项已按流程放行"）、找申请人线下追款 3 天、内部通报 + 写检讨。
>
> 复盘结论很克制也很致命：**"审批"这个动作在真实世界里是会点错的，而点错之后"方法调用"灰飞烟灭**——`approve()` 跑完什么都不剩：没有留痕、没有快照、没有反悔药。这不是加一个"二次确认弹窗"能解决的问题（弹窗自己也会被习惯性点掉），它需要的是**把动作本身变成可撤销的数据**。
>
> 另一个更隐蔽的动因：`approve` / `reject` / `addsign` 这三个方法散落在 REPL 的三个 case 里，**"谁在何时干了什么"没有任何统一记账的地方**——审计需求（月底出报表）每次都要去数据库里"考古"。命令模式顺手把这两个问题一起解决：动作对象化之后，**记账 = 把对象存起来**。

### Step 6.1 审批动作命令化

迁移依然是绞杀者节奏：先让 `ApproveCommand` 只做"转发"（execute 内部调用原来的方法），行为零变化；验收后再把"动作语义"逐步搬进命令对象。**命令模式的重构成本几乎是所有模式里最低的——它不改变任何一行业务逻辑，只是给业务逻辑穿了一件可存储的衣服。** 把 REPL 里 `approve zhangsan` 这个**动作**本身变成对象：
...
> 🔥 **模拟事故复盘 · "改一个数字，排期 9 天"**
>
> 大促前两周，运营提出"金额路由阈值从 5000 调到 8000"——看起来只是改一个数字，但它躺在 Java 代码里：提需求 → 排期 → 改代码 → 走测试 → 发版，**全流程 9 天**，完美错过大促窗口。运营的灵魂拷问从此流传：**"为什么改一个数字要动你们的代码？"**
>
> 根因：规则是**代码**（活在程序员的世界），但规则的变化权在**业务**（活在运营的世界）。两个世界之间隔着一堵墙——解释器模式就是把规则变成**数据**、把"读懂规则"变成**引擎**的那座桥。本节的另外两位主角也有各自被"请下山"的理由：会签/或签的**嵌套分组**让扁平的任务列表撑不住了（组合）；财务/审计/运营三个团队盯上同一棵任务树要出三种报表（访问者）。
...
> 🔥 **翻车现场 · SDK 升级的地毯式轰炸**：钉钉 SDK 从 1.x 升到 2.x，`pushText` 改名为 `pushMarkdown`。FlowKit 里 **23 处调用点**直接引用旧方法名，编译期全崩。改完这一轮，企微 SDK 下个月又发版……**每次三方变更都变成全项目的地毯式轰炸**。
>
> 根因：三方接口的"知识"（webhook 怎么拼、corpSecret 从哪读、参数什么顺序）**泄漏到了业务代码的每个角落**。适配器的使命就是把这些知识**关进一个类**，让三方变更的爆炸半径从 23 处收敛到 1 处。
...
> 🔥 **翻车现场（很轻但很烦）· 漏调的那一个**：`withdraw` 只调了 `process.resetToHead()`，**忘了调 `form.unlock()`**——工单明明已撤回，申请人的表单还显示"审批中不可编辑"，连提 2 个客服工单。类似的还有：驳回时忘发通知、通过时忘记归档——**每次都是"漏调一个"，每次都藏在不同的组合里**，code review 根本防不住（reviewer 也记不全四方联动的清单）。
>
> 根因：四方联动是**编排知识**（谁先谁后、谁都不能漏），它被摊派给了**每个调用方自己记住**。而编排知识天然属于一处——就像乐队的节拍只能有一个指挥，不能靠每个乐手互相提醒。这就是中介者登台的时机。
...
> **总装前的三句话**：① 端到端跑通不代表每一步都对——它只代表"组合没有断裂"，具体正确性靠每次会话的验收点；② 如果中途某步输出和本文不符，**先别改代码，回头读该节的"为什么这样写"**——大概率是你跳过了某个重构步骤而不是写错了；③ 推荐全程开着 `git log --oneline`，每条命令执行前看一眼当前在哪个 commit——**你正在重演的不是代码，是一次架构演化史**。
...
回头看，FlowKit 的从 0 到 1 不是"设计出来"的，是**被事件推着长出来的**——每个版本都由一次外部刺激触发，每次重构都只替换"被刺激的那一层"，其余部分原封不动：

| 版本 | 触发事件 | 引入的结构 | 被替换的旧代码 | 这一版的哲学 |
|------|---------|-----------|---------------|-------------|
| v0.1 | 事故复盘：需要一个能跑的审批 | String 状态 + if-else + 写死审批人 | —— | 先让正确的功能跑起来 |
| v0.2 | typo 丢工单 / 驳回单复活 | enum + 5 个状态类 | 散落在 4 个命令里的状态 if | 让状态自己管自己 |
| v0.3 | 新部门接入卡死 / 加层漏改 | 策略注册表 + 职责链 + 链手术 | 写死的审批人 + 4 处复制的层级逻辑 | 让每个"变化的方向"各自成层 |
| v0.4 | 参数错位 / case 打架 / 模板重复铺 | 工厂 + 建造者 + 单例 + 原型 + 享元 | 裸 new + 相邻参数构造 + 手工连线 | 让"创建"本身成为架构的一部分 |
| v0.5 | 手滑的 48 万 / 附件被误改 | 命令 + 备忘录 + 命令日志 | 发完即忘的方法调用 | 让动作可撤销、可重放 |
| v0.6 | 改数字排期 9 天 / 会签嵌套 | 解释器 + 组合 + 访问者 | 硬编码规则 + 扁平任务列表 | 让规则成为数据 |
| v0.7 | SDK 升级全崩 / 渠道超时拖挂 | 适配器 + 桥接 + 异步观察者 + 动态代理 | 23 处直调三方 | 让横切归横切 |
| v1.0 | 漏调一个四方联动 | 中介者 + 外观 + 懒加载 + 装饰 + 模板 + 迭代器 | 蜘蛛网互调 + 20 行 main 编排 | 让编排归一处 |

注意一个容易被忽略的规律：**每次新结构进来，旧代码都是"被搬走"而不是"被删掉"**——if-else 版的审批逻辑被搬进职责链的节点方法，状态判断被搬进 5 个状态类，层级硬编码被搬进链的组装代码。**模式重构的实质是知识的搬家：从"散落"搬到"归位"**。而 REPL 这个从第一天就写好的回归测试器，保证了搬家途中每一步都没有丢东西——这就是为什么 §02 花一整个会话去写"注定被推翻"的 MVP。
...
诚实的复盘比漂亮的成绩更有教学价值。带着现在的眼光回看，至少有三处可以做得更好：

1. **`ApprovalContext` 应该更早成为"一等公民"**。第 2 次会话它只是个顺手放链的骨架，导致 §03 状态类的方法签名被迫带上 `ctx` 参数——如果第 1 次会话就想清楚"审批的现场"（单据+链+操作人+日志）是个独立概念，后面的命令、备忘录、中介者都能少绕弯。教训：**识别"领域概念"比识别"模式"更早发生**，模式只是概念的容器。
2. **`Attachment` 应该从第一天就是不可变对象**。§07 备忘录的坑（浅拷贝误改附件）根本解法不是"记得深拷贝"，而是"附件本就不该可变"——不可变对象天生免疫浅拷贝坑（引用共享一个不可变对象毫无风险，顺便还享元了）。教训：**可变性是欠条，越早还清利息越少**。
3. **观察者的异步化应该带"本地消息表"**。§09 的线程池方案在教学上够了，但它丢消息（进程重启时队列里的通知蒸发）。生产答案是本地消息表 + 重试——当时为了控制复杂度砍掉了，应该在"延伸挑战"里补为第四题。教训：**教学复杂度和生产可靠性的边界，值得显式标注而不是静默丢弃**。

**思考六：这个案例最大的"没做"是什么？——并发。** FlowKit 全程单线程 REPL，刻意回避了多张工单同时审批的世界。这不是遗漏而是分期：并发会把状态模式（竞态）、备忘录（快照原子性）、命令日志（写放大）全部推到下一难度档。把它留给读者，等你做完 §14.2 的三个挑战后，第四座大山就在那里。
