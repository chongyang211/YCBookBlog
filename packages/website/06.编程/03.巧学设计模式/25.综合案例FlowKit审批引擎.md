...
**学习方式**：本案例**篇幅较长**（约 2400 行 / 18-22 小时），建议**分 5 次会话读完**，每次 2-4 小时；**绝不要试图一口气吃完**。每一节都自带"为什么这样写"的思考小结——如果说前 23 篇是"单元考试"，本篇就是"毕业答辩"，它检验你能不能把模式**协同**起来，而不只是单点背诵。
...
| 代码规模 | 约 2400 行 / 15 个包 40+ 类（不含空行注释约 1700 行） |
...
│   │   ├── ProcessDef.java         # 流程定义（建造者）
│   │   ├── NodeDef.java            # 节点定义（工厂产品）
│   │   ├── NodeFactory.java        # 节点工厂
│   │   └── ProcessRegistry.java     # 注册中心（单例）
│   ├── task/                       # 任务域
│   │   ├── TaskComponent.java      # 组合模式接口
│   │   ├── SingleTask.java
...
│   │   ├── MessageSeverity.java    # 桥接维度2
│   │   └── EventBus.java           # 观察者
│   ├── stats/                      # 访问者
│   └── facade/
│       ├── WorkflowFacade.java     # 外观
│       └── FlowMediator.java      # 中介者
├── data/action.log                 # 命令日志
└── README.md
```
...
public class IconPool {
    ...
    public static Icon of(String kind) {
        return POOL.computeIfAbsent(kind, k -> new Icon(Platform.load(k + ".png")));
    }
}
```
...
public class FinanceVisitor implements TaskVisitor {
    ...
    @Override public void visit(SingleTask t)   { if (t.isApproved()) totalAmount += t.ticketAmount(); }
    @Override public void visit(AndTaskGroup g) { /* 汇总子节点 */ }
    @Override public void visit(OrTaskGroup g)  { /* 取首个通过的 */ }
}

public class AuditVisitor implements TaskVisitor { /* 数加签、记异常驳回 */ }
public class SlmVisitor implements TaskVisitor   { /* 每节点耗时 → P95 */ }
```
...
public class WorkflowFacade {
    ...
    public void approve(long ticketId, String operator) { /* 同款五连 */ }
...
需求：任意节点都可"挂"超时催办——8 小时未处理自动提醒。**不改节点类，包一层同接口**：
...
审批历史存在两个不同结构里：进行中的在**任务树**（§08），已归档的在**按月分段的文件**。上层"按时间正序输出全史"不该关心这些。自定义迭代器做**跨结构归并**（归并两个有序流——面试老朋友了）：
...
原型浅拷贝（§06）、备忘录浅拷贝（§07）、享元可变对象（§09 的反面教材）——三个模式三种形态，坑却是同一个：**引用共享 vs 对象私有**。这就是模式的另一面：**它们是同一组底层原理（组合、委托、封装变化、值语义 vs 引用语义）在不同场景的投影**。学透一个坑，免疫三个模式。
...
完整可编译源码（15 包 / 40+ 类 / 约 2400 行）与本文一一对应，每个 Step 一个 commit，可 `git log --reverse` 按学习顺序回放。仓库地址见书末附录。
