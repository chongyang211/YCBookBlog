...
## 通用必要性：为什么所有并发语言都要重新设计异常机制

> 🌐 **语言无关声明**：本篇沉淀适用于 **Java / C / C++ / Go / JS / Python / Rust / Kotlin / Swift / Erlang** 等多语言开发者阅读。先讲"为什么所有并发语言都绕不开线程异常这道坎"，再讲"每种语言怎么自圆其说"。

### 章首通用三问

读完本章你应该能回答这三道**所有语言通用**的问题：

| # | 通用母题 | 各语言映射的表现 |
|---|---|---|
| **Q1** | **一个子线程崩了，进程为什么也会崩？** | C++ 默认 `std::terminate` 整个进程死 / Java 默认线程死进程活 / Go panic 不 recover 也是整个进程死 / Rust 看 panic 策略 / JS Worker 崩了主线程还活着 |
| **Q2** | **异常如何"跨线程"传播？** | Java `Future.get()` 重抛 / C++ `std::exception_ptr` + future / Go channel 显式传错 / Kotlin CoroutineScope 自动汇集 / Rust JoinHandle 返回 Result |
| **Q3** | **结构化并发为什么会成为未来？** | "开出去的任务必须有归宿"——Kotlin Job 树 / Swift Task tree / Java 21 StructuredTaskScope / Trio nursery |

这三问对应**线程异常机制的三个必答题**——**边界（崩了谁兜底）/ 传播（错误谁来管）/ 归宿（任务谁来收尸）**。

### 通用本质：线程异常 = 单线程异常 + 三个新约束

所有并发语言对"线程异常"都要回答**三个单线程异常体系里不存在的问题**：

1. **栈不连续**：异常本质是栈展开，但**子线程的栈和父线程没有连接**——异常找不到 catch 就只能"自生自灭"
2. **生命周期错配**：父线程可能已经返回了，子线程还活着——异常往哪里抛？
3. **多个失败者**：父发出 N 个子任务，如果 3 个失败了——抛哪一个？合并？取消其他？

| 约束 | 单线程异常 | 线程异常 |
|---|---|---|
| 栈展开终点 | main 函数顶 | 子线程栈顶（无人接） |
| 处理时机 | 立刻 | 可能延迟到 join/get/await |
| 处理者 | 当前调用者 | 父线程 / 全局 Handler / 进程 |
| 取消语义 | 不存在 | 必须有（Interrupt / Cancel / Context） |

**一句话本质**：线程异常 = **跨越执行流边界传递错误信息**，这是所有并发语言都绕不过的"运行时硬约束"。

### 通用三代演进史

无论 Java/C++/Go/Kotlin，**几乎都走过下面这条路**：

```text
第一代：放任不管（C++ 早期 / pthread）
  ↓ 痛点：子线程崩了整个进程死，连日志都没有
第二代：每线程兜底 Handler（Java Thread.UncaughtExceptionHandler）
  ↓ 痛点：每个线程要自己设，遗漏一个就是事故；Future 还会吞异常
第三代：结构化并发（Kotlin / Swift / Java 21 StructuredTaskScope）
  ↓ 思想：把"开出去的任务"放进作用域，作用域负责统一收集和取消
```

**七字真言（贯穿全篇）**：**`传不出 · 救不了 · 兜得住`**

- **传不出**：异常跨线程默认就是消失的——栈展开找不到 catch
- **救不了**：等你拿到异常时，子线程往往早死了——只能记录，不能挽救
- **兜得住**：结构化并发把"任务"和"作用域"绑定，作用域就是兜底人

---

#### 目录介绍
- [1.看一个案例](#1看一个案例)
  - [1.1 子线程静默崩溃](#11-子线程静默崩溃)
  - [1.2 不处理异常的代价](#12-不处理异常的代价)
  - [1.3 try-catch为何不够](#13-try-catch为何不够)
  - [1.4 线程池吞异常陷阱](#14-线程池吞异常陷阱)
  - [1.5 引出核心矛盾](#15-引出核心矛盾)
- [2.异常设计哲学](#2异常设计哲学)
  - [2.1 无异常世界的代价](#21-无异常世界的代价)
  - [2.2 三大核心原则](#22-三大核心原则)
  - [2.3 异常机制演进](#23-异常机制演进)
  - [2.4 可恢复vs不可恢复](#24-可恢复vs不可恢复)
  - [2.5 设计哲学对比](#25-设计哲学对比)
- [3.异常如何引起](#3异常如何引起)
  - [3.1 两种触发源](#31-两种触发源)
  - [3.2 硬件触发异常](#32-硬件触发异常)
  - [3.3 软件触发异常](#33-软件触发异常)
  - [3.4 throw底层实现](#34-throw底层实现)
  - [3.5 分层模型](#35-分层模型)
- [4.异常如何处理](#4异常如何处理)
  - [4.1 通用处理模型](#41-通用处理模型)
  - [4.2 异常处理本质](#42-异常处理本质)
  - [4.3 调用栈物理结构](#43-调用栈物理结构)
  - [4.4 各语言实现机制](#44-各语言实现机制)
  - [4.5 异常匹配规则](#45-异常匹配规则)
  - [4.6 finally实现原理](#46-finally实现原理)
- [5.线程异常核心](#5线程异常核心)
  - [5.1 异常线程边界](#51-异常线程边界)
  - [5.2 跨线程异常传递](#52-跨线程异常传递)
  - [5.3 UEH全景](#53-ueh全景)
  - [5.4 中断异常地位](#54-中断异常地位)
- [6.异常整体框架](#6异常整体框架)
  - [6.1 Java异常体系](#61-java异常体系)
  - [6.2 C++异常体系](#62-c异常体系)
  - [6.3 设计哲学对比](#63-设计哲学对比)
  - [6.4 体系深度对比](#64-体系深度对比)
- [7.异常捕获原理](#7异常捕获原理)
  - [7.1 三大底层实现](#71-三大底层实现)
  - [7.2 各语言性能对比](#72-各语言性能对比)
  - [7.3 栈追踪性能](#73-栈追踪性能)
- [8.各语言线程异常实战](#8各语言线程异常实战)
  - [8.1 Java线程异常](#81-java线程异常)
  - [8.2 C++线程异常](#82-c线程异常)
  - [8.3 JavaScript异常](#83-javascript异常)
  - [8.4 Go panic/recover](#84-go-panicrecover)
  - [8.5 Rust线程异常](#85-rust线程异常)
  - [8.6 C语言异常模拟](#86-c语言异常模拟)
- [9.结构化异常设计](#9结构化异常设计)
  - [9.1 传统模型局限](#91-传统模型局限)
  - [9.2 Kotlin异常传播](#92-kotlin异常传播)
  - [9.3 Swift Task取消](#93-swift-task取消)
  - [9.4 父对子统一思想](#94-父对子统一思想)
- [10.经典陷阱与反模式](#10经典陷阱与反模式)
  - [10.1 静默吞异常](#101-静默吞异常)
  - [10.2 finally覆盖异常](#102-finally覆盖异常)
  - [10.3 中断响应错误](#103-中断响应错误)
  - [10.4 异常用作控制流](#104-异常用作控制流)
  - [10.5 调试与定位](#105-调试与定位)
- [11.一句话总结](#11一句话总结)
  - [11.1 三层认知升华](#111-三层认知升华)
  - [11.2 终极建议](#112-终极建议)
  - [11.3 四个关键收获](#113-四个关键收获)
  - [11.4 延伸阅读](#114-延伸阅读)
- [12.事故复盘](#12事故复盘)
  - [12.1 吞NPE让DB错乱](#121-吞npe让db错乱)
  - [12.2 finally覆盖原始](#122-finally覆盖原始)
  - [12.3 Go panic不传染](#123-go-panic不传染)
- [13.结构化处理未来](#13结构化处理未来)
  - [13.1 结构化自动汇集](#131-结构化自动汇集)
  - [13.2 错误代数](#132-错误代数)
  - [13.3 跨语言陷阱速查](#133-跨语言陷阱速查)
  - [13.4 七字真言映射](#134-七字真言映射)
  - [13.5 一句话总结](#135-一句话总结)
  - [13.6 异常传播三态](#136-异常传播三态)
  - [13.7 异常设计回扣](#137-异常设计回扣)
...
- ← [11.线程前世今生探索](https://yccoding.com/pages/0554f8/)：异常的边界来自线程的物理边界
- ← [12.线程通信设计思想](https://yccoding.com/pages/70930d/)：跨线程传递异常本质是一种通信
- → [14.多线程并发经典案例](https://yccoding.com/pages/3983c8/)：异常处理的实战范式
- → [22.协程异常设计思想](./22.协程异常设计思想.md)：协程时代的结构化异常传播
- → [25.线程池的设计思想](https://yccoding.com/pages/2b1ae7/)：线程池吞异常的源码级解析

## 12.事故复盘：异常吞噬的代价

### 12.1 案例：吞噬NPE让DB静默错乱
...
下面这张表是**线程异常专题**的横向陷阱清单——同一类设计错误，在不同语言里换了张脸出现，但本质都是"传不出 / 救不了 / 兜不住"：

| 语言 | 经典陷阱 | 现象 | 正确姿势 |
|---|---|---|---|
| **Java** | `ExecutorService.submit()` 吞异常 | Runnable 抛出的异常不进 UncaughtHandler，被 Future 包起来不调 `get()` 永远看不到 | 用 `execute()` 或主动 `future.get()`；或注册 `Thread.setDefaultUncaughtExceptionHandler` |
| **Java** | `InterruptedException` 被吞 | catch 完不重新 `Thread.currentThread().interrupt()` 也不抛出——中断信号永久丢失 | catch 后立即恢复中断标志或重新抛出 |
| **C++** | `std::thread` 析构未 `join`/`detach` | 程序立刻 `std::terminate` | RAII 包装（`std::jthread` C++20）或确保两条路径都处理 |
| **C++** | 异常逃出线程函数 | 默认 `std::terminate` 整个进程死 | 在线程函数顶部包 `try-catch` 或用 `std::exception_ptr` 跨线程传递 |
| **Go** | 子 goroutine `panic` 未 recover | 整个进程死，主程序救不回 | 每个 goroutine 入口 `defer recover()`；或用 errgroup 统一收集 |
| **Go** | `errgroup` 漏掉一个错误 | 多个 goroutine 同时失败，只能拿到第一个 | 用 `errors.Join` 或自定义聚合 |
| **Rust** | 线程 panic 默认只崩自己 | 主线程 `JoinHandle::join()` 拿到 `Err` 但不处理，错误静默 | 总是检查 join 返回的 `Result`；或设置 `panic = abort` 改成进程崩 |
| **Rust** | `catch_unwind` 不能跨 `UnwindSafe` 边界 | 编译失败或运行时 panic 二次抛出 | 用 `AssertUnwindSafe` 包装或重新设计数据所有权 |
| **JS** | `Promise` 未 catch 的 reject | Node 默认 `unhandledRejection` 警告→未来版本会让进程退出 | 注册 `process.on('unhandledRejection', ...)` 或全部 `await/then(..., onRej)` |
| **JS** | `async function` 抛异常 = 返回 rejected Promise | 调用方没 await，异常静默 | 顶层加 `process.on('uncaughtException')` 或用 `try { await fn() } catch{}` |
| **Python** | `concurrent.futures.Future` 异常吞咽 | 没调 `result()` 就拿不到异常 | 用 `as_completed` + 主动 `.result()` 或注册回调 |
| **Kotlin** | `GlobalScope.launch` 异常静默 | 协程异常不传给父，应用静默崩 | 用 `CoroutineScope` 配合 `SupervisorJob` + `CoroutineExceptionHandler` |
| **Swift** | `Task {}` 未捕获错误丢失 | 顶层 Task 错误无人处理 | 用结构化 `TaskGroup` 或在 Task body 顶层 `do-catch` |

> **跨语言共识**：**线程异常需要"显式接收点"**——Future/JoinHandle/CoroutineScope/TaskGroup 都是同一种"父对子负责"思想的方言。

### 13.4 七字真言 × 多语言映射

**七字真言**：**`传不出 · 救不了 · 兜得住`**

| 语言 | 传不出（边界） | 救不了（事后） | 兜得住（现代答案） |
|---|---|---|---|
| **Java** | 子线程异常默认不跨边界 | UncaughtHandler 只能记日志 | `StructuredTaskScope` (JDK 21) |
| **C++** | 异常不能跨 std::thread 抛 | terminate 已死无法救 | `std::exception_ptr` + `std::async` |
| **Go** | panic 不跨 goroutine | recover 必须在出错的 goroutine 内 | `errgroup` / Context 取消 |
| **Rust** | panic 默认 unwind 本线程 | JoinHandle 只能事后查 | `tokio::JoinSet` + Result 类型 |
| **JS** | Promise reject 不跨边界 | unhandledRejection 只能记录 | `Promise.allSettled` + AbortController |
| **Kotlin** | launch 默认隔离子异常 | 单独 Handler 仅记录 | `coroutineScope { }` + `SupervisorJob` |
| **Swift** | Task 默认隔离 | Cancellation 不算异常 | `TaskGroup` 结构化并发 |

**口诀解读**：
- **传不出**——异常天生属于栈，栈不跨线程，所以异常默认就是消失的
- **救不了**——等你通过 Handler/Future 看到异常时，子线程往往早已死亡，你能做的只有日志和告警
- **兜得住**——结构化并发把"开出去的任务"装进作用域，让作用域代替父线程兜底
...
**疑惑**：不同语言处理线程异常的策略五花八门，能不能收敛成一张"策略光谱"？

**论证**：把所有语言的"线程异常传播策略"放到一条轴上，会呈现出三个稳定的**"传播态"**：

```mermaid
flowchart LR
    S1[放任态<br/>Panic 即死<br/>C++ std::terminate / Rust panic=abort]
    S2[通道态<br/>异常打包发回父<br/>Java Future / Go errgroup / Rust JoinHandle]
    S3[结构态<br/>作用域自动汇集<br/>Kotlin coroutineScope / Swift TaskGroup / Java StructuredTaskScope]
    S1 --> S2 --> S3
...
| 态 | 传播时机 | 谁负责取消 | 兜底默认值 | 心智负担 |
|---|---|---|---|---|
| **放任态** | 立刻 | 无 | 进程死 | 低（不给你选） |
| **通道态** | 显式拿 Future/get | 调用者 | 静默丢失 | 高（容易漏拿） |
| **结构态** | 作用域退出前 | 作用域 | 自动传播 + 兄弟取消 | 低（编译器强制） |

**结论**：**三态的演化方向是"从时间散乱到边界收敛"**——放任态把异常留给"事后"，通道态把异常留给"某个 API 调用"，结构态把异常留给"离开作用域这一瞬间"。**离作用域边界越近，异常越难被吞掉**。这就是为什么 Kotlin/Swift/Java 21 都在往"结构态"上聚，而不是发明新的通道。

### 13.7 异常设计回扣

**疑惑**：如果只让你留一句话作为团队的"线程异常准则"，你会写什么？

**论证与结论**：**"任何在新执行流里抛出的异常，都必须在离开它的作用域前被显式接收，否则视为 bug"**。

这句话可以拆成三条落地规则：

1. **禁止匿名开线程**：不允许 `new Thread(...).start()` / `go func()` / `Task {}` 之后不管，必须归属到某个 Scope / Pool / Group；
2. **禁止吞异常**：`catch (Exception e) { /* ignore */ }`、`recover(); _ = err`、空的 `.catch(() => {})` 一律 CR 拒绝，必须至少落一条日志 + 决策（rethrow / fallback / cancel）；
3. **禁止裸 Future**：`executor.submit(...)` 的返回值必须被 `.get()`、被 `CompletableFuture.exceptionally`、或者被结构化 `TaskGroup` 接住——不允许"火后不问"。

**一句话背后的深意**：**并发场景下，"忘了处理"比"处理错了"更致命**——处理错了至少留下日志和堆栈；忘了处理就是深夜三点数据不一致而没人知道。**结构化并发不是新技术，是新纪律**：让"忘"这个动作在编译期就实现不了。**能被编译器管住的，就不要交给人管**。
