...
#### 目录介绍
- [01.异步的通用本质](#01异步的通用本质)
  - [1.1 章首三问](#11-章首三问)
  - [1.2 异步的通用本质](#12-异步的通用本质)
  - [1.3 四代演化](#13-四代演化)
  - [1.4 七字真言](#14-七字真言)
  - [1.5 通用收益](#15-通用收益)
- [02.基础概念](#02基础概念)
...
- [06.多语言演变](#06多语言演变)
  ...
  - [6.4 Go](#64-go)
  - [6.5 Rust](#65-rust)
  - [6.6 Kotlin](#66-kotlin)
  - [6.7 C语言](#67-c语言)
  - [6.8 跨语言对照](#68-跨语言对照)
- [07.技术架构](#07技术架构)
  - [7.1 编程模型](#71-编程模型)
  - [7.2 任务生命周期](#72-任务生命周期)
  - [7.3 线程模型](#73-线程模型)
- [08.平台实现](#08平台实现)
  - [8.1 Web端](#81-web端)
  - [8.2 Android端](#82-android端)
  - [8.3 iOS端](#83-ios端)
  - [8.4 最佳实践](#84-最佳实践)
- [09.陷阱与反模式](#09陷阱与反模式)
  - [9.1 回调异常黑洞](#91-回调异常黑洞)
  - [9.2 Promise链断裂](#92-promise链断裂)
  - [9.3 串行误用](#93-串行误用)
  - [9.4 CF静默丢异常](#94-cf静默丢异常)
  - [9.5 虚拟线程pin](#95-虚拟线程pin)
- [10.陷阱速查](#10陷阱速查)
- [11.真言映射](#11真言映射)
- [12.总结](#12总结)
  - [12.x 设计哲学回顾](#12x-设计哲学回顾)


  
## 01.异步的通用本质

> 🧭 **本节定位**：把"异步"上抬到**语言无关的通用必要性**。理解了下面三问，再回看任何一门语言的 async/await、Future、协程、回调，都只是同一个问题的"方言"。
...
**疑惑**：select/poll 为什么被淘汰？epoll 凭什么一个数据结构替换整个 O(n) 轮询？

**论证**，epoll 的创新是**控制面与数据面分离**：

```
select/poll（控制面和数据面耦合）：
  每次调用都告诉内核"我关心哪些 fd" + "哪些就绪了"
  → 10 万 fd 每次拷 10 万个 → O(n) 瓶颈

epoll（控制面和数据面分离）：
  epoll_ctl → 只注册一次（O(logN) 红黑树）
  epoll_wait → 只取就绪 fd（O(1) 就绪链表）
```

**LT vs ET 触发模式对比**：LT 只要 fd 可读每次都通知（简单可靠），ET 仅在不可读→可读时通知一次（高性能，需循环读到 EAGAIN）。

**一句话**：epoll 把"轮询所有"变成"只接收就绪事件"——这是从主动轮询到被动通知的范式转变。
...
**哲学 1：异步的本质是"等待"与"执行"的解耦**。50 年四代演化的内核从未变——callback、Promise、async/await、协程，都是把"等待 IO 时线程被浪费"这个问题用不同抽象层级解决。

**哲学 2：所有语言在向同一方向收敛**——"看着同步，跑着异步"。Java Loom 的虚拟线程、Go 的 goroutine、Kotlin 的协程，都在追求同一个终点：让程序员可以写直觉的同步代码，但底层享受异步的性能。

**哲学 3：epoll 是异步的物理基石**。没有 IO 多路复用，就没有百万并发。epoll 的控制面/数据面分离思想，在异步编程的每一层都有回响——从内核到 runtime 到应用层。

**哲学 4：异步转同步是放弃分布式红利**。让你能编异步代码跑同步结果，但要承受"线程退回挂起等待"的后果。在高并发环境里慎用。
