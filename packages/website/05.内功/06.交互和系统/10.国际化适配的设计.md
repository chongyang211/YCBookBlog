...
🔗 **延伸阅读**：← [5.2 视图加载渲染设计](https://yccoding.com/pages/070bb8/) · ← [5.3 图形渲染管线原理](https://yccoding.com/pages/318066/) · → [5.12 屏幕适配设计思想](https://yccoding.com/pages/9c7a15/)（跨端最后一公里的双胞胎，一个管文化、一个管硬件）
...
一款教育 App 翻译成阿拉伯语后直接上线，三天大量 1 星差评：界面全乱、价格显示错误、输入框光标错位。排查两天，根因是**三层都错了**：

| 层 | 错误 | 修复 |
|----|------|------|
| 资源层 | 只翻译文字，未配置 RTL | `android:supportsRtl="true"` |
| 布局层 | 用 `marginLeft/paddingLeft` 而非 `Start/End` | 全替换为 `paddingStart/End` |
| 格式化层 | `String.format` 拼货币，未走 ICU | `NumberFormat.getCurrencyInstance(locale)` |

**核心教训**：国际化不是翻译，是「资源 + 布局 + 格式化」三件套，缺一就是半成品出海。

### 1.2 德语按钮撑爆

另一个事故。跨端购物 App 的"确认支付"按钮：英文 15 字符完美居中，德语 18 字符撑爆卡片。设计师按英文给了 `120dp` 固定宽度，但德语复合词可长达 63 字符。

**解药**：设计阶段做 30-50% 膨胀预算（短词 "OK" → "Klar" 膨胀 100%），布局用 `wrap_content + minWidth + autoSize` 替代固定宽度。改完后 12 种语言全部通过。

**核心教训**：国际化不是上线前翻译，是设计稿第一笔就要考虑的约束，三方（产品/设计/工程）约定。
...
```
zh（只写语言）： 是简体？还是繁体？→ 需要 Script
zh-CN（语言 + 地区）：中国大陆，简体，能推断出 Script = Hans
zh-TW：中国台湾，繁体，能推断出 Script = Hant
zh-Hans-SG：新加坡，简体。Script 必须显式（否则会推断错）
```
...
**Unicode 之前**：GBK、Big5、Shift-JIS 各自为政，同一字节 `0xE7` 在不同编码里是不同字符。编码错 = 乱码（"锟斤拷"就是 UTF-8 被 GBK 二次解码的经典事故：`U+FFFD` ×3 → `EF BF BD` ×3 → 锟斤拷）。

**Unicode 之后**：统一编号 `U+4E2D = 中`、`U+1F600 = 😀`。编码方式：UTF-8（变长 1-4B，Web 默认）、UTF-16（Java/Windows）、UTF-32（定长 4B）。
...
**为什么这套设计跨平台通行**：1.代码只依赖"稳定的 key"，语言变了代码不动；2.资源可以由翻译团队独立维护；3.打包时按 locale 分包，用户只下自己需要的；4.便于 QA 自动化，检查有没有漏译。
...
```
用户是台湾用户（zh-TW），App 有 values-zh/（简体）但没有 values-zh-rTW/
  → 台湾用户看到简体中文。用户炸锅："你们把我当大陆人？"

修复：提供 values-zh-rTW/（繁体），或者至少提供 values-b+zh+Hant/
```
