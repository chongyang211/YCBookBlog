...
类是对象的模板，对象是类的实例——这是面向对象编程的起点。

**三个核心要点**：

1. **构造方法与 this**：构造方法没有返回类型、名与类同名。`this` 是当前对象引用，`this()` 调用本类其他构造方法——`this()` 和 `super()` 都必须放在构造方法第一行，不能同时出现
2. **封装的核心**：用 `private` 隐藏实现，通过 `getter/setter` 控制访问。这不是繁琐的规范——它让你可以随时改变内部实现而不影响调用方（比如加缓存、加校验逻辑）
3. **static vs 实例**：`static` 成员属于类，在类加载时初始化；实例成员属于对象。工具方法用 static，业务逻辑用实例方法。`static` 方法中不能用 `this`——因为它不依附于任何对象
