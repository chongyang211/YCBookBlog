...
运算符是任何编程语言的"动词"，Java 的运算符体系与 C/C++ 高度相似但有关键差异。

**三个核心要点**：

1. **短路运算**：`&&` 和 `||` 是短路的——左边能确定结果就不算右边，可用来做安全判断（`obj != null && obj.isValid()`）。`&` 和 `|` 不短路，两边都会执行
2. **位运算与移位**：`>>` 算数右移（补符号位）、`>>>` 逻辑右移（补0）、`<<` 左移（补0）。位运算比算术快，常用于 HashMap 哈希计算和权限位掩码
3. **三元运算符的类型推断**：`条件 ? A : B` 的返回类型取 A 和 B 的**最小公共类型**，可能导致隐式的自动装箱/拆箱和数值提升——这是隐蔽的类型陷阱
