...
注解和反射是 Java 元编程的两大武器——注解提供声明式编程能力，反射提供运行时动态能力。两者结合，是 Spring、JUnit、MyBatis 等框架的基石。

**三个核心要点**：

1. **注解 = 标签 + 数据**：注解本身不做任何事——需要**注解处理器**（反射读取 or APT 编译时处理）来赋予语义。`@Override` 能报错靠的是编译器内置的处理器，`@Test` 能运行靠的是 JUnit 的反射扫描
2. **反射是框架的"黑科技"**：反射打破封装访问 `private` 成员——Spring 的 DI 注入、JUnit 的测试方法调用、MyBatis 的 Mapper 代理全都依赖它。代价是性能损失（比直接调用慢 5-10 倍）和失去编译期类型安全
3. **注解 vs 反射的配合**：`getAnnotation()` 读注解信息决定行为——Spring 扫描 `@Component` 自动注册 Bean，JUnit 扫描 `@Test` 发现测试方法。这是声明式编程的 Java 实现方式
