...
接口和抽象类是 Java 实现抽象的两种方式，JDK 8 模糊了它们的边界但核心区别仍在。

**三个核心要点**：

1. **接口 = 契约，抽象类 = 模板**：接口定义"能做什么"，抽象类定义"是什么"。一个类可以实现多个接口但只能继承一个抽象类——这是设计选择的核心约束
2. **JDK 8+ 接口进化**：`default` 方法让接口可以添加新方法而不破坏已有实现，`static` 方法把工具函数和接口绑定。但接口仍然不能有实例字段和构造方法——这些是抽象类的领地
3. **面向接口编程**：`List<String> list = new ArrayList<>()` 优于 `ArrayList<String> list = new ArrayList<>()`——依赖抽象而非具体，换实现只需改一行 new
