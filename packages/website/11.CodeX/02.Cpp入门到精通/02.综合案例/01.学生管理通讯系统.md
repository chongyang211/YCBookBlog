---
title: 01.学生管理通讯系统
date: 2026-05-25 10:21:17
permalink: /pages/c34dde/
categories:
  - book
  - Cpp入门到精通
  - 综合案例
...
- [10.项目技术思考](#10项目技术思考)
  - [10.1 为何要用结构体](#101-为何要用结构体)
  - [10.2 替代方案对比](#102-替代方案对比)
  - [10.3 为何本例选结构体](#103-为何本例选结构体)
- [11.衔接与延伸](#11衔接与延伸)
  - [11.1 与下一案例的差异](#111-与下一案例的差异)
  - [11.2 三个延伸挑战](#112-三个延伸挑战)
...
**功能描述：** 用户选择功能的界面。**步骤：** 1.封装函数显示该界面  如 `void showMenu()`；2.在main函数中调用封装好的函数，注意在.h文件中添加声明。
...
🎯 **Step 2.1**：敲键盘前先想清楚要存什么。在敲键盘之前，停 1 分钟问自己**三个灵魂问题**——回答清楚了再写代码，能少走 80% 的弯路。

❓ **问题一：每个联系人有哪些字段？** 答：姓名、性别、年龄、电话、地址 → 5 个字段，需要一个结构体 `Person` 把它们打包。
...
❓ **问题三：为什么是先 `Person` 再 `AddressBooks`，不是反过来？** 来看反例：
...
联系人信息包括：姓名、性别、年龄、联系电话、家庭住址。`Person` 结构体用于存储单个联系人的信息。设计如下：
...
设计时候可以在通讯录结构体中，维护一个容量为100的存放联系人的数组，并记录当前通讯录中联系人数量。使用了一个结构体 `AddressBooks` 来存储通讯录数据，设计如下

```cpp
#define MAX 100  //最大人数（宏定义不加分号，也不能带等号）
//通讯录结构体
struct AddressBooks {
    Person personArray[MAX]; //通讯录中保存的联系人数组，长度必须是编译期常量
    int size;   //通讯录中人员人数
};
```

备注：这个数组必须要指定长度，否则报 `field has incomplete type` 错误。**为什么会报错？** 因为结构体的大小必须在**编译期**就确定下来：

- 编译器要根据结构体定义算出 `sizeof(AddressBooks)`，才能在创建变量（`AddressBooks address;`）时知道该分配多少内存、成员 `size` 该放在哪个偏移位置；
- 数组元素是连续存放的，若写成 `Person personArray[]` 不指定长度，就算不出它占多少字节，整个结构体的大小也就无法确定；
- 因此 C++ 规定：结构体成员不能是**不完整类型**（incomplete type），而未知长度的数组正是不完整类型。

💡 若确实需要"长度待定"：可改用指针（`Person* personArray;`，指针大小固定，但需手动 new/delete），或直接换成 `std::vector<Person>`（推荐，能自动扩容）。

### 4.3 创建通讯录变量

🎯 **Step 2.2**：在 main 创建通讯录。添加联系人函数封装好后，在main函数中创建一个通讯录变量，这个就是我们需要一直维护的通讯录。
...
> 🎯 **Step 2.3**：实现 addPerson 函数。添加联系人前先判断通讯录是否已满，如果满了就不再添加，未满情况将新联系人信息逐个加入到通讯录
...
为什么这里要传指针 `AddressBooks *abs`？**因为函数要改到调用方那份原始数据，而值传递只能改到副本。**

如果去掉 `*` 写成 `void addPerson(AddressBooks abs)`：调用时整个通讯录（100 个联系人）会被**完整拷贝**一份，函数里 `abs.size++`、赋值姓名改的都是这份副本；函数一结束副本就销毁，外面的 `address` 一个联系人也没多——**添加功能直接失效**，还白白拷贝了一大块内存。

传指针则是把**同一块内存的地址**交给函数，`abs->size` 改的就是外面那个 `address` 本身，所以结果能保留下来。

| 写法 | 调用方式 | 能改到原数据 | 说明 |
|------|----------|:---:|------|
| `void f(AddressBooks abs)` | `f(address)` | ❌ | 值传递：拷贝整份数据，改的是副本 |
| `void f(AddressBooks* abs)` | `f(&address)` | ✅ | 指针：本文用法，内部用 `->`（即 `(*abs).size` 的简写） |
| `void f(AddressBooks& abs)` | `f(address)` | ✅ | **引用（C++ 推荐）**：内部用 `.`，写法更简洁，也不用担心空指针 |

### 4.5 测试添加联系人 

选择界面中，如果玩家选择了 1，代表添加联系人，我们可以测试下该功能。在 switch case 语句中，case 1 里添加：
...
🎯 **Step 2.5 上**：辅助函数 isExist 诞生。设计思路：删除联系人前，我们需要先判断用户输入的联系人是否存在，如果存在删除，不存在提示用户没有要删除的联系人
...
问题1：string name 值传递 → 应改 const string& name。注意：每次调用都完整拷贝一次 string（名字长时会触发堆分配），而函数只读不写它。改用 const 引用零拷贝。

问题2：只读参数应加 const。注意：函数不修改通讯录，参数应为 const AddressBooks*——既防误改，也让调用方一眼看出"只读"。
...
🎯 **Step 2.5 下**：复用 isExist 实现 deletePerson。根据用户输入的联系人判断该通讯录中是否有此人。查找到进行删除，并提示删除成功。查不到提示查无此人。
...
这个核心数据 AddressBooks 和 Person 为何要用结构体，用结构体好处是什么。能否用其他代替？
...
下一案例 [02.银行账户管理系统](https://blog.csdn.net/m0_37700275/article/details/103463225) 会做四件"升级"，请留意对比：
...
- ➡ 下一案例：[02.银行账户管理系统](https://blog.csdn.net/m0_37700275/article/details/103463225) —— 从 `struct` 升级到 `class` + 多态三态，从定长数组升级到 `vector`，从内存临时数据升级到 CSV 文件持久化
