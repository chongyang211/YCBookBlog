...
- [第五章：C++ 订单票务购买系统](#第五章c-订单票务购买系统)
  ...
  - [03.通用线程池 ThreadPool](#03通用线程池-threadpool)
    - [灵魂三问：线程池设计要点](#灵魂三问线程池设计要点)
    - [Step 2.1 线程池最小骨架](#step-21-线程池最小骨架)
    - [Step 2.2 packaged\_task 入门](#step-22-packaged_task-入门)
    - [Step 2.3 模板 submit 实现](#step-23-模板-submit-实现)
    - [Step 2.4 千任务压测验证](#step-24-千任务压测验证)
  - [04.OrderManager 与读写锁](#04ordermanager-与读写锁)
    - [Step 3.1 Order 单线程跑通](#step-31-order-单线程跑通)
    - [Step 3.2 mutex 基线压测](#step-32-mutex-基线压测)
    - [Step 3.3 shared\_mutex 升级](#step-33-shared_mutex-升级)
  - [05.Product 防超卖](#05product-防超卖)
    - [Step 4.1 裸 int 埋 BUG](#step-41-裸-int-埋-bug)
    - [Step 4.2 千线程见超卖](#step-42-千线程见超卖)
    - [Step 4.3 atomic CAS 修复](#step-43-atomic-cas-修复)
      - [Step 4.4 故意造死锁](#step-44-故意造死锁)
      - [Step 4.5 修复方案 A：std::lock 一次锁全部](#step-45-修复方案-astdlock-一次锁全部)
      - [Step 4.6 修复方案 B：锁顺序约定（更通用）](#step-46-修复方案-b锁顺序约定更通用)
  - [06.策略模式下单](#06策略模式下单)
  ...
  - [10.项目技术思考](#10项目技术思考)
    ...
    - [10.2 cv 与 unique\_lock](#102-cv-与-unique_lock)
...
❓ **为什么要先做无锁版（Step 1.1）再加锁（Step 1.3）？不是浪费时间吗？** 答：**这是教学法刻意安排**。如果一上来就给你"完美的最终版"，你只会**机械记住代码**而不知道每一行的存在意义。**先看到无锁版的崩溃**（Step 1.2），你才会真正理解为什么 mutex / cv / shutdown 一个都不能少。**这与 04 案例 §6.3 故障演示**、**03 案例 §6.4 dirty bug** 同源，**先有问题，后有解法**。
