...
// ❌ 无锁日志（对比用）：多线程并发写时，日志会交错、甚至丢数据
class UnsafeLogger {
    vector<string> logs_;
public:
    void log(const string& level, const string& msg) {
        string entry = "[" + level + "] " + msg;
        logs_.push_back(entry);    // 无锁：vector 非线程安全 → 数据竞争
        cout << entry << endl;     // 无锁：cout 输出会交错
    }
    size_t size() { return logs_.size(); }
};
...
class Logger {
    ...
    string timestamp() {
        ...
        auto ms = chrono::duration_cast<chrono::milliseconds>(now.time_since_epoch()).count() % 10000;
...
int main() {
    // ============ 对比 1：无锁 vs 有锁（多线程并发写日志）============
    cout << "=== 无锁 UnsafeLogger：3 线程各写 3 条 ===" << endl;
    {
        UnsafeLogger unsafeLog;
        vector<thread> ts;
        for (int i = 0; i < 3; ++i) {
            ts.emplace_back([&unsafeLog, i]{
                for (int j = 0; j < 3; ++j) {
                    unsafeLog.log("INFO", "线程" + to_string(i) + "消息" + to_string(j));
                }
            });
        }
        for (auto& t : ts) t.join();
        cout << "无锁日志条数: " << unsafeLog.size() << "（应为 9，可能丢失）" << endl;
    }

    cout << "\n=== 有锁 Logger：3 线程各写 3 条 ===" << endl;
    Logger mainLogger;
    {
        vector<thread> ts;
        for (int i = 0; i < 3; ++i) {
            ts.emplace_back([&mainLogger, i]{
                for (int j = 0; j < 3; ++j) {
        ...
        for (auto& t : ts) t.join();
        cout << "有锁日志条数: " << mainLogger.size() << "（正好 9，一条不少）" << endl;
    }
...
```
=== 无锁 UnsafeLogger：3 线程各写 3 条 ===
[INFO] 线程0消息0[INFO] 线程1消息0[INFO] 线程2消息0   ← 日志交错、黏在一起
[INFO] 线程1消息1
[INFO] 线程2消息1[INFO] 线程0消息1                    ← 两条挤在一行
...
无锁日志条数: 8（应为 9，可能丢失）                    ← 数据竞争导致丢日志

=== 有锁 Logger：3 线程各写 3 条 ===
[1234ms] INFO: 线程0消息0
[1235ms] INFO: 线程1消息0
[1236ms] INFO: 线程2消息0
...                                                     ← 每条完整、一行一条、绝不交错
...
=== recursive_mutex 递归加锁 ===
递归日志[0]: 顶层消息
  递归日志[1]: 顶层消息 (子)
    递归日志[2]: 顶层消息 (子) (子)                    ← 递归 3 层不死锁
...
**对比一目了然**：同样 3 个线程并发写日志，**无锁版日志交错、黏行、甚至丢条目**；**有锁版每条完整独立、一条不少**。这就是锁存在的意义。
...
下面的例子演示 `wait_for` 的两种结果：条件提前满足就立即继续，超时则强制唤醒。`setReady` 用 1 秒完成准备，小于 2 秒超时，因此正常输出「条件达成」。
...
void waitForReady() {
    std::cout << "[等待线程] 进入 waitForReady，准备上锁等待\n";
    std::unique_lock<std::mutex> lock(mtx);
    std::cout << "[等待线程] 已持锁，调用 wait_for（最多等 2 秒）\n";
    // wait_for：最多等 2 秒；ready 提前为 true 返回 true，否则超时返回 false
    if (cv.wait_for(lock, std::chrono::seconds(2), [] { return ready; })) {
        std::cout << "[等待线程] 被唤醒，ready=true，条件达成 - 提前执行任务\n";
    } else {
        std::cout << "[等待线程] 2 秒超时，ready 仍为 false，强制唤醒执行\n";
    }
    std::cout << "[等待线程] 执行完毕，退出（unique_lock 析构自动解锁）\n";
}

void setReady() {
    std::cout << "[通知线程] 进入 setReady，开始模拟 1 秒耗时准备\n";
    std::this_thread::sleep_for(std::chrono::seconds(1)); // 模拟耗时准备
    {
        std::lock_guard<std::mutex> lock(mtx);
        std::cout << "[通知线程] 已持锁，把 ready 置为 true\n";
        ready = true;  // 修改条件必须持锁
    }
    std::cout << "[通知线程] 释放锁，调用 notify_one 唤醒等待线程\n";
    cv.notify_one();  // 唤醒一个等待线程
    std::cout << "[通知线程] 通知完成，退出\n";
}

int main() {
    std::cout << "[主线程] 启动，创建两个子线程\n";
    std::thread t1(waitForReady);
    std::thread t2(setReady);
    std::cout << "[主线程] 子线程已创建，等待它们结束（join）\n";
    t1.join();
    t2.join();
    std::cout << "[主线程] 两个子线程都结束，程序退出\n";
    return 0;
}
```
...
```text
[主线程] 启动，创建两个子线程
[主线程] 子线程已创建，等待它们结束（join）
[等待线程] 进入 waitForReady，准备上锁等待
[等待线程] 已持锁，调用 wait_for（最多等 2 秒）
[通知线程] 进入 setReady，开始模拟 1 秒耗时准备
[通知线程] 已持锁，把 ready 置为 true
[通知线程] 释放锁，调用 notify_one 唤醒等待线程
[通知线程] 通知完成，退出
[等待线程] 被唤醒，ready=true，条件达成 - 提前执行任务
[等待线程] 执行完毕，退出（unique_lock 析构自动解锁）
[主线程] 两个子线程都结束，程序退出
```
...
> 说明：等待线程先持锁后立即在 `wait_for` 里**释放锁并挂起**，所以通知线程才能拿到锁去改 `ready`。这正是「等待期间必须释放锁」的直观体现。

**为什么两种结果的日志顺序不同？** 两种输出只在前几行有差异，后半段完全一致，原因有三：

1. **前几行无同步约束**：`std::thread` 创建后立即返回，主线程、t1、t2 谁先被调度执行由操作系统决定，所以「进入 / 持锁 / 子线程已创建」这几行的先后是随机的（但不影响正确性）。
2. **后半段有因果链**：从「通知线程已持锁」起，1 秒睡眠、锁、`notify_one`、`wait_for` 返回构成确定的 happens-before 关系，顺序固定。
3. **真正被保证的只有一件事**：`ready` 的读写都在锁内，「条件达成」的结果是确定的，与日志顺序无关。

若把 `setReady` 的睡眠改成 3 秒（超过 2 秒超时），等待线程会在 2 秒处超时，输出变成：

```
[等待线程] 2 秒超时，ready 仍为 false，强制唤醒执行
```

**执行流程**：

1. `t1` 进入 `waitForReady`，持锁后调用 `wait_for`，先释放锁、进入等待。
2. `t2` 睡 1 秒后持锁把 `ready` 置 `true`，释放锁后 `notify_one` 唤醒 `t1`。
3. `t1` 被唤醒，重新持锁，谓词 `ready == true` 成立，`wait_for` 返回 `true`，输出「条件达成」。

**关键点分析**：

1. **`wait_for(lock, timeout, pred)` 的返回值是谓词结果**：被唤醒（无论通知还是超时）后都会重新持锁并求值 `pred`，返回 `bool`。因此 `true` 表示「条件满足」，`false` 表示「超时且条件仍未满足」。
2. **谓词防虚假唤醒**：线程可能被系统无端唤醒（虚假唤醒）。带谓词的 `wait`/`wait_for` 内部等价于 `while (!pred) wait(lock);`，被虚假唤醒后会再次检查谓词、继续等待，不会错误地往下执行。
3. **为什么 `wait` 必须用 `unique_lock`**：等待期间要释放锁、唤醒后再重新获取锁，`lock_guard` 不支持中途解锁，只有 `unique_lock` 可以。
4. **修改条件要持锁、再通知**：`ready = true` 在锁内完成，避免「检查—修改」间隙丢通知；`notify_one` 放在锁外可减少被唤醒线程立即抢锁失败的开销（放锁内也正确，只是多一次上下文切换）。

### 15.4.4 条件变量总结

条件变量的标准用法是「一把锁 + 一个条件变量 + 一个谓词」三件套，套用下面的骨架即可：

```cpp
// 等待方
std::unique_lock<std::mutex> lock(mtx);
cv.wait(lock, [] { return condition; });   // 或 wait_for / wait_until
// 条件满足，继续干活

// 通知方
{
    std::lock_guard<std::mutex> lock(mtx);
    condition = true;   // 改条件必须在锁内
}
cv.notify_one();        // 或 notify_all()
```

记忆口诀：**检查等待要原子，改完条件再通知，谓词防住虚假唤醒**。

- `wait`：无限等待，直到条件满足
- `wait_for` / `wait_until`：带超时，返回谓词结果（`bool`），适合「等不到就放弃」的场景
- `notify_one`：唤醒一个等待线程（适用于单消费者 / 只需一个线程处理）
- `notify_all`：唤醒所有等待线程（适用于多消费者 / 关闭时通知全部）

**为什么条件变量必须配 mutex**：检查条件与进入等待必须是原子的，否则在「检查之后、等待之前」另一线程可能改条件并通知，这条通知就丢了，线程将永远等下去。所以 `wait` 必须持锁调用，内部在原子地完成「解锁 + 挂起」。
