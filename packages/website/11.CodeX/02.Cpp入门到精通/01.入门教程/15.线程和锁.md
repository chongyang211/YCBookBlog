...
**优化案例**：上面的写法其实有三个隐患——`std::cout` 这种慢 I/O 被放进锁内拖累所有线程、手动 `lock/unlock` 遇异常会死锁、锁粒度过大。优化写法：

```cpp
std::mutex mtx;

// 优化后：RAII 加锁 + 只锁最小临界区 + 慢 I/O 移出锁外
void incrementCounter(int id) {
    for (int i = 0; i < 1000; ++i) {
        int value;
        {
            std::lock_guard<std::mutex> g(mtx);  // RAII：异常也自动解锁
            value = ++counter;                   // 只锁真正的共享资源
        }                                       // 离开作用域自动解锁
...
| 问题 | 原始写法 | 优化写法 |
|------|---------|---------|
| 锁粒度 | `++counter` + `cout` 都在锁内，慢 I/O 拖累所有线程 | 只锁 `++counter`，`cout` 移出锁外 |
| 异常安全 | 手动 `lock/unlock`，异常时锁不释放 → 死锁 | `lock_guard` RAII，异常也自动解锁 |
| 锁开销 | 临界区里夹着慢操作，线程排队等 I/O | 临界区极短，线程几乎不用等 |
