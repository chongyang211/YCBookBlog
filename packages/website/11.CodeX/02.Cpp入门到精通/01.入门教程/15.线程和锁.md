...
void t1() {
    ...
    std::lock_guard<std::mutex> lb(b);   // 等 b，但 b 被 t2 持有
}

// 线程2 — 加锁顺序相反
void t2() {
    std::lock_guard<std::mutex> lb(b);
    std::lock_guard<std::mutex> la(a);   // 等 a，但 a 被 t1 持有
}
```

解法就一条：**所有线程用同一顺序加锁**。如果必须同时锁多个，用 `std::lock()` 原子锁定：

```cpp
std::lock(a, b);                         // 要么全拿到，要么全拿不到
std::lock_guard<std::mutex> la(a, std::adopt_lock);
std::lock_guard<std::mutex> lb(b, std::adopt_lock);
```

`std::lock` 内部用死锁避免算法（try-and-back-off），`adopt_lock` 告诉 `lock_guard` "锁已经拿下了，别重复锁"。
