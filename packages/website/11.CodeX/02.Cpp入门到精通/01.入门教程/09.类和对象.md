...
### 9.3.5 静态编译时常量（`static constexpr`）

旧式 `static const` 仍需类外定义，C++11 起 `static constexpr` 可直接在类内完成定义，无需类外再写一行：

```cpp
class NetworkConfig {
public:
    static constexpr int kSoftTimeoutSeconds = 360;  // 类内完成定义，无需类外
    static constexpr int kMaxRetryCount     = 3;

    static constexpr int TimeoutMillis() { return kSoftTimeoutSeconds * 1000; }
};

// C++17 起可加 inline 省去类外定义，C++11 用 constexpr 更直接
// 旧式写法对比：
// class OldWay { static const int kTimeout = 360; };        // 类内声明
// const int OldWay::kTimeout;                               // 仍需类外定义
```

> **要点**：`static constexpr` 兼具 `static`（所有实例共享一份）和 `constexpr`（编译期确定）的优势，可用于数组大小、模板参数等编译期上下文。`k` 前缀来自 Google C++ Style Guide，表示编译时常量。
