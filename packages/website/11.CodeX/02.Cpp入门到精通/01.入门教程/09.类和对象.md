...
- [第 9 章 C++ 类和对象](#第-9-章-c-类和对象)
  ...
  - [9.3 类成员属性](#93-类成员属性)
    ...
    - [9.3.6 static constexpr](#936-static-constexpr)
    - [9.3.7 综合案例与思考](#937-综合案例与思考)
  - [9.4 对象初始化](#94-对象初始化)
  ...
  - [9.10 类和对象底层原理](#910-类和对象底层原理)
    ...
    - [9.10.5 std::move 到底做了什么](#9105-stdmove-到底做了什么)
  - [9.11 类和对象训练题](#911-类和对象训练题)
  - [9.12 综合思考题](#912-综合思考题)
  - [9.13 新手陷阱 Top 5](#913-新手陷阱-top-5)
...
struct Point1 {        // struct: 默认 public
    int x, y;
};
...
int main() {
    Point1 p1{10, 20}; // 可直接访问
    cout << p1.x << ", " << p1.y << endl;

    Point2 p2;
    p2.set(30, 40);    // 需通过公有方法访问
    p2.print();
}
```
...
创建对象时，`class` 关键字可以省略（C++ 兼容 C 风格），但通常直接使用类名：

```cpp
class Student yc1;     // C 风格，合法但不推荐
Student yc2;           // C++ 惯用写法
Student yc3[100];      // 创建对象数组
```

如果类提供了有参构造函数，则必须在创建对象时传入对应参数：
...
private:
    ...
    string name;
public:
    Student(const string& num, const string& nm)
        : number(num), name(nm) {}   // 初始化列表，效率更高
};

Student yc("2024001", "打工充");   // 调用有参构造函数
```

使用**初始化列表**而非在构造函数体内赋值，可以避免成员变量先默认初始化再被覆盖，对 `const` 和引用成员更是唯一可行的初始化方式。

### 9.2.3 访问类成员

通过点号 `.` 可以访问对象的 `public` 成员（变量或函数），语法与结构体访问一致：
...
- `.` 左侧是**对象本身**（而非指针或引用），右侧是被访问的成员名
- 只能访问 `public` 成员；`private`/`protected` 成员在类外部不可见，这是封装的体现
- 每个对象拥有一份独立的成员变量副本（如 `stu1.age` 和 `stu2.age` 互不影响），但成员函数代码只有一份，通过隐式 `this` 指针区分调用对象
...
```cpp
Student stu;
Student *pStu = &stu;          // 指向栈上已有的对象

Student *pStu2 = new Student;  // 在堆上 new 出一个匿名对象
```

**栈 vs 堆**：栈上对象有名字（如 `stu`），由编译器自动管理生命周期；堆上 `new` 出来的对象没有名字、只能通过指针访问，必须手动 `delete` 释放，否则导致内存泄漏。因此 `new` 和 `delete` 应成对使用。
...
class ServerConfig {
public:
    int timeout_;                              // ① 普通成员：每对象一份，可修改
    static const int kVersion = 2;             // ② 静态 const：C++98 风格，整型可类内初始化
    static constexpr int kSoftTimeoutSeconds = 360; // ③ 静态 constexpr：C++11 推荐，类内搞定
};

// 非整型 static const 仍需类外定义：
// class Config { static const double kRate; };
// const double Config::kRate = 0.75;
```

| 写法 | 每对象一份 | 编译期可用 | 需类外定义 | 适用场景 |
|---|---|---|---|---|
| `int timeout_` | ✅ | ❌ | 不需要 | 对象状态可变，如连接超时可按实例调整 |
| `static const int` | ❌ 全局一份 | ✅(整型) | 有时需要 | C++98 项目，非整型仍须类外定义 |
| `static constexpr int` | ❌ 全局一份 | ✅ | ❌ | C++11+ 项目首选，数组大小、模板参数 |

**选择建议**：能用 C++11 就一律用 `static constexpr`，省去类外定义、编译期 100% 可用、语义最清晰。`k` 前缀来自 Google C++ Style Guide，语义上等于"编译时常量"。
...
### 9.10.5 std::move 到底做了什么

`std::move` **不移动任何东西**。它的唯一作用是：**把左值强制转换成右值引用**，告诉编译器"这个变量之后我不再用了，你直接把它的内部资源拿走"。
...
int main() {
    ...
    std::string b = a;              // 拷贝：a 内容原样保留，两份内存
    std::string c = std::move(a);   // 转移：a 变成空壳，资源转给 c
...
**为什么函数里按值传参要加 `std::move`**：

```cpp
void SetGate(Gate fn) { 
    gate_ = std::move(fn);
}
```

`fn` 是按值传入的（不是引用），调用方传进来时已经拷贝了一份。如果不加 `move`，`gate_ = fn` 会再拷贝一次——`std::function` 内部可能持有大量捕获变量，两次拷贝开销翻倍。加了 `move`，直接把 `fn` 已拷贝好的内部资源转移给 `gate_`，`fn` 变成空壳（反正出作用域就销毁了），零额外开销。

**一句话**：`std::move` 是一个"权限转换申请"，真正移动资源的是**移动构造函数**（或移动赋值运算符）。
