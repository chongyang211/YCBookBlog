...
- [7.5 内联函数](#75-内联函数)
  ...
  - [7.5.2 核心原理](#752-核心原理)
  - [7.5.3 内联函数定义](#753-内联函数定义)
  - [7.5.4 内联函数特点](#754-内联函数特点)
  - [7.5.5 注意事项](#755-注意事项)
  - [7.5.6 内联函数 vs 宏](#756-内联函数-vs-宏)
  - [7.5.7 内联函数场景](#757-内联函数场景)
  - [7.5.8 综合案例与思考](#758-综合案例与思考)
- [7.6 默认参数](#76-默认参数)
...
- [7.11 新手陷阱 Top 5](#711-新手陷阱-top-5)
...
常见的函数样式有4种，本质上是 **参数（输入）** 和 **返回值（输出）** 两个维度的组合——参数决定函数"吃什么"，返回值决定函数"吐什么"：

| 样式 | 参数 | 返回值 | 语法骨架 | 典型场景 |
|------|:---:|:---:|----------|----------|
| 无参无返 | ✗ | ✗ | `void f()` | 横幅打印、界面刷新——只做动作，不吃数据也不产出结果 |
| 有参无返 | ✓ | ✗ | `void f(int a)` | 打印账单、写日志——接收数据去处理，调用方不需要结果 |
| 无参有返 | ✗ | ✓ | `int f()` | 读时间、生成随机数——自己拿数据，算完交出去 |
| 有参有返 | ✓ | ✓ | `int f(int a, int b)` | 加法、求绝对值、单位换算——最常用的"计算器"形态 |
...
// 2、有参无返：处理数据不回传——按单价和数量打印账单
void printBill(double price, int count) {
    cout << "账单：" << price << " x " << count
         << " = " << price * count << " 元" << endl;
}

// 3、无参有返：自己取数算完交出——生成 6 位随机验证码
int makeCode() {
    return rand() % 900000 + 100000;   // 100000 ~ 999999
}

// 4、有参有返：吃进数据吐出结果——计算折扣价
double discount(double price, double rate) {
    return price * rate;
}

int main() {
    printBanner();                                     // 无参无返：直接调用
    printBill(9.9, 3);                                 // 有参无返：只传参数，不接结果
    cout << "验证码：" << makeCode() << endl;           // 无参有返：只接结果，不传参数
    cout << "折后价：" << discount(100, 0.8) << endl;   // 有参有返：又传又接
    return 0;
}
```
...
1. **函数需要外部提供数据吗？** 需要 → 加参数；数据写死或函数内部自己生成 → 不加。
2. **调用方需要函数的执行结果吗？** 需要 → 有返回值；只是"执行个动作"（打印、通知、刷新）→ 返回 `void`。

> ⚠️ **新手提醒**：不要为了"以后可能用到"就给所有函数都加返回值——**该 `void` 就 `void`**。返回值意味着调用方要处理它，无意义的返回值是噪音；反过来，需要结果的函数也别只在函数里 `cout` 打印（否则结果没法参与后续计算）。
...
**本质：内联是"编译期抄代码"——把函数体直接复制到每个调用处，用代码体积换掉调用开销。**
...
- **省掉了什么**：`call` 跳转、返回地址压栈、参数传递、栈帧的创建与销毁（对照 7.8.1）全部消失，只剩函数体本身。
- **代价是什么**：调用 N 次就复制 N 份函数体，代码膨胀会挤占指令缓存（i-cache），反而可能变慢——所以它只适合短小、高频的函数。
- **`inline` 只是建议**：函数体过大、递归、虚函数都可能被编译器拒绝；现代编译器在 `-O2` 以上会主动内联小函数，根本不看 `inline`。
- **`inline` 真正的硬作用**：让函数定义可以写进头文件、被多个源文件包含而不违反单一定义规则（ODR）——这才是今天写 `inline` 的主要理由。
