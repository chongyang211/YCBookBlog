...
- **为什么能改到外面**：值传递（`void f(int n)`）拿到的是副本，改完就丢；传指针则是把 `num` 的地址交进去，`(*ptr)++` 顺着地址改的就是 main 里那个 `num` 本身；
- **省拷贝**：传进去的只是一个 8 字节地址，无论对象多大都不用复制——这也是传递大结构体、大对象的常用手法；
- **只想读不想改时加 `const`**：`void f(const int* ptr)` 能防止函数内误改，也把意图表达得更清楚。

**⚠️ `(*ptr)++` 的括号不能省**：`++` 的优先级高于 `*`，写成 `*ptr++` 会先让**指针自己往后挪一格**（地址 +4），而不是把值加 1。
...
- 返回的是**地址**而不是值，所以调用方拿到后还要用 `*` 解引用才能取出数据；
- ✅ **本例是安全的**：返回的 `a` 或 `b` 指向的是 main 里的 `x`、`y`，它们的生命周期比函数更长，函数返回后依然有效。
...
int* badFunc() {
    int x = 10;
    return &x;   // ❌ 大错！x 在函数结束时就被销毁了
}
```

`x` 是栈上的局部变量，函数一返回它的内存就被回收，外面拿到的就是**悬空指针**（dangling pointer），再解引用属于未定义行为——可能读到垃圾值，也可能直接崩溃。

确实需要"新造一份数据"时，用 `new` 在堆上分配（记得 `delete`），或直接返回值 / 引用。
