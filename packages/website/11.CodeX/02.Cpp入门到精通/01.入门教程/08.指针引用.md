...
可以通过指针间接访问内存。打个比方：内存是一栋大楼，每个字节都是一间房，都有门牌号（从 0 开始，用十六进制表示，如 `0x7ffd4c8e9a1c`）。

- 普通变量：房间里**装的是**数据（比如 `10`）
- 指针变量：房间里**记的是**别人的门牌号

```cpp
int  a = 10;   // a 是个盒子，里面装着 10
int* p = &a;   // p 是张纸条，上面写着 a 的门牌号
*p = 20;       // 顺着纸条找到 a，把它改成 20
```
...
|------|--------|
| `&a` | "a 住在哪儿？" → 取地址 |
| `int* p` | "p 是张纸条，专门记 int 类型变量的地址" |
| `*p` | "按纸条上的地址找过去" → 解引用 |

**为什么要用指针？** 因为拿到地址就能隔空操作那份数据——不用把数据本身搬来搬去，既快又省。
