...
- [第 12 章 C++ 动态内存](#第-12-章-c-动态内存)
  ...
  - [12.4 智能指针](#124-智能指针)
    - [12.4.1 unique\_ptr](#1241-unique_ptr)
    - [12.4.2 shared\_ptr](#1242-shared_ptr)
    - [12.4.3 weak\_ptr](#1243-weak_ptr)
  ...
  - [12.7 高级用法](#127-高级用法)
    - [12.7.1 enable\_shared\_from\_this](#1271-enable_shared_from_this)
    - [12.7.2 自定义删除器](#1272-自定义删除器)
    - [12.7.3 weak\_ptr 打破循环引用](#1273-weak_ptr-打破循环引用)
    - [12.7.4 make\_shared 的优势](#1274-make_shared-的优势)
  ...
  - [12.9 动态内存训练题](#129-动态内存训练题)
    - [12.9.1 简易内存池](#1291-简易内存池)
    - [12.9.2 不同类型对比](#1292-不同类型对比)
  - [12.10 综合思考题](#1210-综合思考题)
  - [12.11 新手陷阱 Top 5](#1211-新手陷阱-top-5)
...
通过 new 和 delete 操作符（或 C 风格的 malloc 和 free）来分配和释放内存。动态内存的使用场景包括：

- 1.需要灵活管理内存大小（如动态数组）。例如读取用户输入的文件，文件大小未知，只能用 `new char[size]` 动态分配，栈上没法提前预留。
- 2.对象的生命周期超出其作用域。例如工厂函数 `createWidget()` 在堆上 `new` 一个对象并返回指针，调用方拿到后想用多久用多久，不会随函数返回而销毁。
- 3.需要共享内存（如多线程环境）。例如多个线程共享一个任务队列，通过 `new` 在堆上分配，配合 `shared_ptr` 自动管理，线程间安全读写。

### 12.1.2 动态内存特点

1. 运行时分配：内存的分配和释放发生在程序运行时，而不是编译时。通过 `new` / `delete` 在堆上按需申请，不像栈变量在编译期就确定了大小和布局。
2. 手动管理：程序员需要显式分配和释放内存，否则会导致内存泄漏。C++ 不像 Java/C# 有 GC，`new` 出来的每一块内存必须配对 `delete`，或者用智能指针自动回收。
3. 堆区存储：动态内存从堆区分配，堆区的空间通常比栈区大。栈默认只有几 MB，放不下大对象或大数组；堆受限于物理内存 + 虚拟内存，可以分配 GB 级别。
4. 灵活性：动态内存的大小可以在运行时动态调整，适合处理不确定大小的数据。比如用户输入 n 再 `new int[n]`，编译期根本不知道 n 是多少。
...
int main() {
    ...
    // 释放动态分配的内存
    delete obj;
    MyClass* obj2 = new MyClass(30,"yc");
    delete obj2;
    return 0;
}
```
...
1. `new` 配对 `delete`，`new[]` 配对 `delete[]`——混用会导致**只析构第一个元素**，造成内存泄漏。
2. `delete nullptr` 是安全的（空操作），但**二次 delete** 同一指针是未定义行为。
3. `ptr = nullptr` 不是语法要求，但能防止"野指针"被误用，属于工程习惯。

释放单个对象：

```cpp
delete ptr;     // 调用一次析构，释放一块内存
ptr = nullptr;  // 切断悬挂指针
```
...
```cpp
delete[] arr;   // 逐个析构每个元素，再释放整块内存
arr = nullptr;
```

### 12.2.3 动态内存最佳实践

刚接触 `new`/`delete` 容易掉的几个坑，先知道、后面慢慢填：

1. **`new` 和 `delete` 一对一**：每次 `new` 在逻辑上都要有一个对应的 `delete`。最容易漏的是"提前 return 或抛异常"的那条分支。
2. **释放后置 `nullptr`**：`delete` 之后指针仍然存着旧地址，再次访问就是未定义行为——工程上习惯立刻清空，防止后续误用。
3. **别混用 `new`/`delete[]`**：用 `new[]` 开数组就必须用 `delete[]` 释放，否则只析构第一个元素。
4. **容器代替裸数组**：动态数组优先用 `std::vector`，它自动管理内存、自动扩容，不需要你写 `new int[n]`。
5. **先学会裸指针，再学智能指针**：`unique_ptr` 和 `shared_ptr` 确实是更安全的替代方案（见 §12.4），但前提是理解了 `new`/`delete` 的手动模型——否则看到 `std::move` 和引用计数照样晕。
...
有一个 `AsyncTask` 类需要在内部将自身传给异步线程保活。如果直接传 `this` 构造 `shared_ptr`，会创建独立控制块导致双重释放：
...
public:
    ...
    ~AsyncTask() { std::cout << "[" << name_ << "] destroyed\n"; }
private:
...
int main() {
    {
        ...
        task->Start();
    } // 外部 shared_ptr 销毁，对象被析构
    std::cout << "outer gone\n";
    std::this_thread::sleep_for(std::chrono::seconds(2));
}
```

**运行结果**（双重释放导致崩溃）：`shared_ptr<AsyncTask>(this)` 创建了独立控制块，与 `Create` 中的控制块互不知晓，同一块内存被释放两次。

```text
[MyTask] destroyed
outer gone
[] working...   // 对象已析构，name_ 是空字符串
[] destroyed    // 双重释放 → crash
Process finished with exit code 133 (interrupted by signal 5:SIGTRAP)   // 报错日志
```

**对象生命周期管理的问题**：在异步任务中，任务可能需要在后台运行一段时间，而外部的 `std::shared_ptr` 可能已经释放。如果没有保活机制，对象可能会被提前销毁，导致未定义行为。

**安全地持有自身引用的问题**：直接通过 `this` 创建 `std::shared_ptr` 会导致多个独立的 `std::shared_ptr` 管理同一个对象，从而引发双重释放问题。`std::enable_shared_from_this` 解决了这个问题。

**解决方案**：C++11 提供了 `std::enable_shared_from_this`，一个 CRTP 模板基类。继承它后，对象内部调用 `shared_from_this()` 即可安全获取共享同一控制块的 `shared_ptr`，彻底避免双重释放。适用于异步回调、事件处理等需要对象"自保活"的场景：

```cpp
// enable_shared_from_this 必须指定类型，内部需要知道你最终的类型才能构造出正确类型的 shared_ptr 和 weak_ptr。
...
public:
    ...
    void Start() {
        auto self = shared_from_this();  // ✅ 同一个控制块
        std::thread([self] {
            std::cout << "[" << self->name_ << "] working...\n";
            std::this_thread::sleep_for(std::chrono::seconds(1));
...
int main() {
    {
        auto task = AsyncTask::Create("MyTask");
        task->Start();
    } // 外部 shared_ptr 销毁，但线程中的 self 保活，对象延迟析构
    std::cout << "outer gone\n";
    std::this_thread::sleep_for(std::chrono::seconds(2));
}
```
...
`shared_ptr` 和 `unique_ptr` 都支持自定义删除器，用于管理非内存资源（文件句柄、socket、锁等）：
...
// 场景：用智能指针自动关闭文件
{
    ...
    std::shared_ptr<FILE> filePtr(f, [](FILE* p) {
        printf("自动关闭文件\n");
        fclose(p);
    });
    // 离开作用域时自动调用 fclose，不会忘记
}

// unique_ptr 的自定义删除器需要传递删除器类型
{
    auto deleter = [](FILE* p) { fclose(p); };
    std::unique_ptr<FILE, decltype(deleter)> filePtr(fopen("log.txt", "w"), deleter);
}

// 经典场景：数据库连接池
class DBConn {
    void disconnect() { /* 归还到连接池 */ }
};
std::shared_ptr<DBConn> conn(new DBConn, [](DBConn* c) {
    c->disconnect();  // 不 delete，而是归还连接池
});
```

**原理简析**：`shared_ptr` 的删除器以类型擦除方式存入控制块，同一类型的 `shared_ptr` 可以持有不同删除器.`unique_ptr` 的删除器是模板参数的一部分，零额外开销但不支持不同删除器的混用。

### 12.7.3 weak_ptr 打破循环引用

当两个对象相互持有 `shared_ptr` 时，引用计数永远无法归零，导致内存泄漏：
...
struct B {
    std::shared_ptr<A> a_ptr; // ❌ 如果改成 weak_ptr<A> 就解决了
    ~B() { std::cout << "B 被销毁\n"; }
};
...
void test() {
    ...
    a->b_ptr = b;
    b->a_ptr = a; // 循环引用：两个都删不掉
}
// 离开作用域后 A、B 的析构函数都不会被调用——内存泄漏
```

**解决方案**：将其中一个改为 `std::weak_ptr`，`weak_ptr` 不增加引用计数，打破循环：

```cpp
struct B {
    std::weak_ptr<A> a_ptr; // ✅ 不增加引用计数
    void doSomething() {
        if (auto a = a_ptr.lock()) { // 使用前先尝试提升
            // 安全使用 a
        } else {
            std::cout << "A 已销毁\n";
        }
...
| 方法 | 作用 | 返回值 |
|---|---|---|
| `lock()` | 尝试获取 `shared_ptr` | 对象存在返回有效 `shared_ptr`，否则返回空 |
| `expired()` | 检查对象是否已销毁 | `true` = 已销毁 |
| `use_count()` | 查看引用计数 | 不增加计数，仅查看 |

### 12.7.4 make_shared 的优势

`std::make_shared<T>(args...)` 比 `std::shared_ptr<T>(new T(args...))` 更高效、更安全：

```cpp
// 方式一：两次内存分配（对象 + 控制块分开）
std::shared_ptr<Widget> p1(new Widget(42));

// 方式二：一次内存分配（对象和控制块紧邻分配）
auto p2 = std::make_shared<Widget>(42); // ✅ 推荐
```

**核心优势**：

1. **性能更好**：`make_shared` 只分配一次内存，对象和控制块在连续内存中，缓存友好
2. **异常安全**：函数调用中 `new` 和 `shared_ptr` 构造之间的间隙可能因异常导致泄漏
3. **代码更简洁**：不需要显式 `new`，配合 `auto` 一目了然

```cpp
// 异常风险场景
void process(std::shared_ptr<Widget> p, int priority);
// ❌ 编译器可能先执行 new、再执行 computePriority()、再构造 shared_ptr
// 如果 computePriority() 抛异常，new 出来的内存就泄漏了
process(std::shared_ptr<Widget>(new Widget), computePriority());
// ✅ make_shared 保证原子性
process(std::make_shared<Widget>(), computePriority());
```

> **注意**：`make_shared` 分配的内存要等所有 `shared_ptr` 和 `weak_ptr` 都销毁后才释放。如果 `weak_ptr` 长期存在，用 `new shared_ptr` 能更早释放对象内存。
