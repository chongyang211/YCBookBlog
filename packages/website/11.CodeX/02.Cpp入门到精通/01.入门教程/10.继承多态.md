...
public:
    Base() { a = 100; }
    void func()     { std::cout << "Base - func()调用" << a << std::endl; }
    void func(int a)  { std::cout << "Base - func(int)调用" << a << std::endl; }
    int a;
};
...
public:
    Son() { a = 200; }
    // 子类同名函数会隐藏父类所有重载版本，加作用域可访问被隐藏者
    void func() { std::cout << "Son - func()调用" << a << std::endl; }
    int a;
};
...
原理：子类与父类同名成员在各自作用域内共存，默认就近解析到子类成员；加 `Base::` 作用域限定后，编译器才跳转到基类成员。对函数而言，子类声明了同名函数就会「隐藏」基类中所有重载版本（包括不同参数的），这是名字隐藏而非重载。

总结：

1. 子类对象直接访问到的是子类同名成员
2. 加作用域 `Base::` 可访问父类同名成员
3. 子类同名函数会隐藏父类所有重载版本，加作用域才能调用父类同名函数

### 10.1.7 继承同名静态成员处理

问题：同名静态成员在子类上如何访问？静态成员与非静态成员出现同名时，处理方式一致——访问子类直接访问，访问父类加作用域。

```cpp
class Base {
public:
    static void func()    { cout << "Base - func()" << a << endl; }
    static void func(int) { cout << "Base - func(int)" << endl; }
    static int a;
};
int Base::a = 100;
...
class Son : public Base {
public:
    static void func() { cout << "Son - func()" << a << endl; }
    static int a;
};
int Son::a = 200;
...
int main() {
    ...
    // 函数：对象 / 类名访问
    s.func();  s.Base::func();
    Son::func();  Son::Base::func();
    Son::Base::func(100);  // 同名函数被隐藏，需加作用域
    return 0;
}
```

原理：静态成员属于类本身，但同样受名字隐藏规则约束——子类声明同名成员后，通过子类名或子类对象默认解析到子类成员；加 `Base::` 作用域限定（`Son::Base::a` 即「以 Son 进入 Base 的作用域」）才能访问基类的静态成员。函数的重载版本同样被整体隐藏。
