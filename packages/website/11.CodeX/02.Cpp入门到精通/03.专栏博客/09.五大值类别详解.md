...
> **下一篇**：本篇讲清了"什么是 rvalue、什么是 xvalue"——为下一篇打好了语法基底。下一篇 [10.右值引用与移动语义](https://yccoding.com/pages/226137/) 进入实战：**`std::move` 为什么是空操作而又改变重载决议？移动构造的"零开销"承诺如何兑现？为什么 `noexcept` 在移动构造上是性能命门？为什么 `std::vector` 扩容时会因为没标 noexcept 而退化成拷贝？unique_ptr 为什么必须删除拷贝、保留移动？** 这些问题贯穿移动语义的工程命脉。本篇从"语法层"理解了值类别，下一篇就要把这些类别"用起来"。
