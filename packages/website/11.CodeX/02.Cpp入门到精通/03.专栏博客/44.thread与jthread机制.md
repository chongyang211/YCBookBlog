...
> **下一篇**：线程创建和协调说了。下一篇进入 [45.异步编程future家族](https://yccoding.com/pages/f24c45/)——future/promise/packaged_task 三件套、std::async 启动策略陷阱（`std::launch::async` vs `deferred`）、shared_future 的广播语义、C++20 的 `std::jthread` 与 future 的协调模式。
