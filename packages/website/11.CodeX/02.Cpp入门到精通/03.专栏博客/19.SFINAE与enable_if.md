...
> **下一篇**：[20.可变参数模板原理](https://yccoding.com/pages/6c7c8b/) 将把模板的能力推到边界——**parameter pack 在编译器内部到底如何展开？折叠表达式凭什么从 C++17 开始能写一行替代递归？`std::tuple` 的多重继承式实现为何选「结构性递归」而不是数组？`std::apply` 在不知道实参类型的前提下如何精准转发？**——可变参数模板是 C++ 模板系统通向元编程的最后一公里。
