...
> **下一篇**：本篇剖析了"以哪种可变性穿越对象"的全部规则——但还有一个 cv 之外的运行时类型机制：**RTTI 与 dynamic_cast**。下一篇 [15.RTTI与dynamic_cast](https://yccoding.com/pages/dae6fc/) 揭晓：**`typeid` 运算符到底返回什么？`type_info` 的内存布局长什么样？`dynamic_cast` 怎么在运行时找到正确的子类？为什么有些项目编译时加 `-fno-rtti` 关掉它？关闭 RTTI 后 `dynamic_cast` 与 `typeid` 还能用吗？跨动态库边界 `dynamic_cast` 为什么会失败？多继承的 vptr 偏移调整在 dynamic_cast 里怎么处理？** 第 13 篇讲了 dynamic_cast 的"用法"，第 15 篇深入它的"机制"——RTTI 是 C++ 类型系统从静态走向运行时的最后一块拼图。
