...
- [7. 共享库与mmap映射](#7-共享库与mmap映射)
  - [7.1 so的载入机制](#71-so的载入机制)
  - [7.2 PLT与GOT绑定](#72-plt与got延迟绑定)
  - [7.3 dlopen运行时加载](#73-dlopen运行时加载)
...
崩溃地址落在 **`---p`（无任何权限）的守护页（guard page）** 内——典型的"**栈溢出**"，不是缓冲区越界，是**整个调用栈用光了**。
...
看似"递归就递归"的代码，没毛病在算法，**毛病在没有意识到栈是有边界的**——这条代码碰到的不是 C 语言的坑，是**进程地址空间布局**的坑。
...
这个事故就是本篇的**主线案例**。我们带着上面 7 个问号往下走，每讲完一段原理就解开一两个；最后在第 10 章把案例彻底剖开，给出三种修复方案与各自的代价。
...
> 📌 **本篇定位**：这是整个 C 语言专栏的**地基篇**。第 02-20 篇讲的指针运算、栈帧原理、内存对齐、动态内存管理，本质都是"在这张地址空间地图上的某一段里发生的事"。读完本篇后，再看后面任何一个 C 语言内存话题，都能立刻回答："**它住在哪一段**"。
...
2. **加载时机不同**——text/rodata/data 在 ELF 文件里就有，加载器把文件**直接 mmap** 进虚拟地址；bss 不占文件空间（节省磁盘），运行时才映射到零页；heap/stack 完全运行时动态分配。**段对应"何时存在"的差异**。

3. **共享与独占不同**——同一份 libc.so 被 100 个进程加载时，text 段的**物理内存只有一份**（多进程共享同一物理页）；bss/data/heap/stack 每个进程独占一份（写时复制）；**段对应"是否共享"的差异**。
...
```
堆向上长、栈向下长，留中间一大片"待分配区域"给两者动态争抢——这是 1970 年代 Unix 设计者的**空间复用智慧**。如果两者同向，很快就会撞在一起。

5. 反向验证：如果不分段会怎样？参考早期 DOS 的"实模式"——任何指针都能改任何地址，包括代码段。结果就是蓝屏家常便饭，一个野指针就能干掉整个操作系统。

**结论**：分段不是为了"好看"，而是把**权限、加载时机、共享性、生长方向**这四个独立维度同时编码进地址空间——一段空间一种语义，崩溃定位、安全防护、内存共享、动态扩展全都得益于此。这是 Unix/Linux 内存管理的根基哲学，**C 语言是它最直接的受益者——也是最大的责任者**。
...
2. 进程间的 VA 完全独立——进程 A 的 `0x400000` 和进程 B 的 `0x400000` 指向**不同的物理页**。
...
**结论**：虚拟内存把"地址"和"物理存储"解耦——这是 1960 年代 Multics 留给现代操作系统最珍贵的遗产。C 语言对"地址"的直接操作（`&a`、`*p`、`void *`），操作的**全是虚拟地址，不是物理地址**——程序永远不需要也不可能知道数据在物理内存的哪个芯片上。
...
1. 早期 PDP-11 等 CPU 内存小，约定"代码在低地址、栈在高地址"，让两者**从两端向中间生长**，最大化共用空间。
...
这就是 Linux "少占物理内存" 的另一个关键技巧——所有进程共享 glibc 几 MB 的代码，物理内存里**只算一次**。
...
Linux 内核在 `/proc/PID/maps` 暴露一个进程的**完整地址空间映射**——这是排查的利器：
...
```
代价：代码量增加，但**栈深度恒定 1 层**，目录嵌套百万层都不会崩。`stack` 在堆上分配，深度由堆大小而非 `ulimit -s` 限制。
...
栈溢出可以被静默继续吗？技术上可以（让程序"踩进堆里"）。但 Linux 选择**守护页让它立刻崩**，让"问题立刻可见"。这与 C 语言的"fail fast"理念一致——bug 越早发现，修复成本越低。**信号量比静默错误珍贵一千倍**。
...
> **下一篇**：我们已经知道了"数据住在地址空间的哪一段"，下一步进入 [02.栈与堆底层对决](https://yccoding.com/pages/cc002/)——把栈帧的完整生命周期、堆的分配机制，从寄存器级别掰开揉碎。
