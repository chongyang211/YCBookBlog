...
把需求滚动一轮：1.加一种折扣策略 → 改 `main` 的拼装顺序；2.同一个订单要既导出 PDF 又发短信 → 又得新增两组函数；3.多人协作时，数组下标含义全靠注释维护，调用方写错下标 → 静默 bug。
...
"装大象进冰箱"是个好比喻，但比喻只能帮你理解**方向**。这一节用一段真实可跑的代码，让你亲眼看到同一条业务规则在两种范式下落成代码的**质感差异**。
...
| 维度 | 过程式 | 对象式 |
|------|-------|-------|
| 参数传递 | 每个函数传 3-4 个参数，重复 6 次 | 构造函数一次性注入，之后**零参数** |
...
- 把 `price` 包成 `Money` 而不是 `double`，预告了 [11 篇 DDD 的值对象](https://yccoding.com/pages/d522e9/)；
- 把 `Activity` 设成接口，预告了 [04 篇接口编程](https://yccoding.com/pages/d080c5/)；
- 让 `Order.total()` 是**唯一入口**，预告了 [02 篇封装](https://yccoding.com/pages/f9fcfa/) 与 [11 篇聚合根](https://yccoding.com/pages/d522e9/)。
...
下一篇 [02.面向对象的特性](https://yccoding.com/pages/f9fcfa/) 会从一次"钱包余额对不上"的真实事故切入，把四大特性逐一拆穿，并且，**会回答本篇遗留的三道思考题**。
