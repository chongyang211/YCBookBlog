...
- [4.封装特性](#4封装特性)
  ...
  - [4.5 封装的层次](#45-封装的层次)
  - [4.6 实现原理](#46-实现原理)
  - [4.7 好坏封装对比](#47-好坏封装对比)
- [5.抽象特性](#5抽象特性)
  - [5.1 抽象的动机](#51-抽象的动机)
  - [5.2 抽象的定义](#52-抽象的定义)
  - [5.3 接口与抽象类](#53-接口与抽象类)
  - [5.4 抽象的层级](#54-抽象的层级)
  - [5.5 命名即抽象](#55-命名即抽象)
- [6.继承特性](#6继承特性)
  - [6.1 继承的动机](#61-继承的动机)
  - [6.2 继承的定义](#62-继承的定义)
  - [6.3 类型层级与里氏替换](#63-类型层级与里氏替换)
  - [6.4 实现原理](#64-实现原理)
  - [6.5 继承的代价](#65-继承的代价)
- [7.多态特性](#7多态特性)
  - [7.1 多态的动机](#71-多态的动机)
...
- [9.综合实战案例](#9综合实战案例)
  ...
  - [9.3 四大特性逐层加固](#93-四大特性逐层加固)
  - [9.4 类图与不变量清单](#94-类图与不变量清单)
...
上一篇 [01.面向对象设计思想](https://yccoding.com/pages/a81c3d/) 末尾留下了三道题：
...
- 🟢 `items` 改成 `public` → 外部可以 `order.items.add(newItem)` 绕过 `Order.total()`，**总价不变量失效**。根因：缺乏**封装**，没有守门人。
- 🟡 不改 `Order` 一行，只改 `BuyTwoGetOne` → 因为 `Activity` 是 `interface`，新规则只需加一个实现类。根因：**抽象+多态**同时在场。
...
**一句话定义**：封装 = **把数据和对数据的操作捆绑成一个整体，对外隐藏内部实现细节，只暴露一个受控的访问接口**。它让你"管好自己的内部，只给别人一个面板"。

### 4.3 钱包封装版

类的封装包含两层含义：**一是把数据和操作数据的行为绑定在一起**（数据和逻辑不散落各处）；**二是用访问权限控制外部对内部成员的访问**——用 `private` 把字段和内部逻辑藏起来，只通过 `public` 方法暴露有限的、安全的面板。数据不能直接被外部篡改，只能走类提供的入口，类内部的修改也不影响外部调用方。
...
**一句话定义**：抽象 = **只保留"能做什么"，隐藏"怎么做"，把稳定的"是什么"从易变的"怎么实现"中剥离出来**。它让调用方面对一个简化的契约，而非一堆实现细节。
...
**一句话定义**：继承 = **子类复用父类的状态与行为，并只覆写差异的部分，从而建立"是一个（is-a）"的子类型关系**。它解决的是"一群相似对象里，如何消除重复、只保留差异"的问题。
...
这就是**里氏替换原则（LSP）** 的数学本质：子类必须能替换父类，不破坏程序正确性。
...
C++/Java 的继承落到底层就是**虚函数表（vtable）**。以 `CreditWallet` 为例：
...
**一句话定义**：多态 = **同一个方法调用，随接收对象的实际类型而表现出不同的行为**。它让调用方只面向一个抽象接口，而无需关心具体是哪一个子类在响应，从而把"类型差异"从调用方里彻底剥离出去。
...
- **何时用接口、何时用抽象类？** → [03.接口vs抽象类比较](https://yccoding.com/pages/eceb4b/)
- **如何让代码不锁死在某个实现上？** → [04.接口而非实现编程](https://yccoding.com/pages/d080c5/)
- **继承怎么用才不翻车？** → [05.多用组合和少继承](https://yccoding.com/pages/3b28c3/)
...
下一篇 [03.接口vs抽象类比较](https://yccoding.com/pages/eceb4b/) 将从一次"日志器被改了 47 次"的事故出发，回答**抽象到底该用接口还是抽象类**，并揭晓本篇 §8.5 的三道思考题。
