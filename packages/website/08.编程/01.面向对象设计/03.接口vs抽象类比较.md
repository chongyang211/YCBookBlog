...
上一篇 [02.面向对象的特性](https://yccoding.com/pages/f9fcfa/) 末尾留下了三道题：
...
三道题同时指向**本篇的核心重点**：抽象类与接口到底要怎么选？

### 1.2 日志器被改了47次

让这个问题从抽象变成血肉，来看一段真实代码史。某中台项目 `MyLogger.java` 的 git 记录：
...
回到日志器。§3 用抽象类给了「is-a」的答案，FileLogger is-a Logger。但如果需求不是"一类东西"，而是"一个可插拔的能力"呢？比如：**输出目的地本身应该是可替换的**，文件、Kafka、ES 之间没有 is-a 关系，只是都能「追加日志」。
...
public class FileAppender  implements LogAppender { /* 写文件 */ }
public class KafkaAppender implements LogAppender { /* 写 Kafka */ }
public class ESAppender    implements LogAppender { /* 写 ES */ }
```

对照 §3 的抽象类 `Logger`（侧重复用骨架），这里的接口 `LogAppender`（侧重能力契约）。**同一个 Logger 主题，两种抽象工具，解决两个不同的问题**。
...
```java
public class Logger {
    private List<LogAppender> appenders;   // 只依赖接口，不关心是文件还是 Kafka
    public void log(Level level, String msg) {
        LogEntry entry = new LogEntry(level, msg);
        for (LogAppender a : appenders) a.append(entry);
    }
}
```

- 新增一种输出（如写 Redis）→ **只加新类**，`Logger` 零修改；
- 测试时可以注入假 `LogAppender`（写到内存，断言输出内容）；
- 不同输出目的地各自演进，互不踩脚。
...
| 场景 | 推荐 | 理由 |
|------|------|------|
| 日志骨架（限流/级别校验） | 抽象类 | 公共流程复用，子类只填 `doWrite` |
| 日志输出目的地（文件/Kafka/ES） | 接口 | can-do 能力，多种实现并存 |
| 支付渠道（支付宝/微信/银联） | 接口+抽象类 | 接口定义契约，抽象类提供校验/重试/记账骨架 |
| Wallet 的持久化（MySQL/Redis/内存） | 接口 | 仅约束行为，底层随意换 |
| 实体基类 `id/createTime` 字段 | 抽象类 | 共享状态字段
...
public class AlipayGateway   extends AbstractPaymentGateway {
    @Override protected PayResult doPay(Order order) { /* 调支付宝 */ }
}
```
...
下一篇 [04.接口而非实现编程](https://yccoding.com/pages/d080c5/) 会用图片存储与支付渠道两个真实案例，详细拆解这条原则的应用与边界。
...
public class AlipayGateway   extends AbstractPaymentGateway {
    @Override protected PayResult doPay(Order order) { /* 30 行 · 调支付宝 */ }
}
public class WechatPayGateway extends AbstractPaymentGateway {
    @Override protected PayResult doPay(Order order) { /* 28 行 · 调微信支付 */ }
}
public class UnionPayGateway  extends AbstractPaymentGateway {
    @Override protected PayResult doPay(Order order) { /* 32 行 · 调银联 */ }
}
```

三个子类加起来不到 100 行，而骨架里的公共逻辑（校验/限流/记账/重试/事件）一处都没重复。

代码量从三个类各 ~200 行（600 行）→ 一个骨架 ~160 行 + 三个 `doPay` ~90 行（250 行）。
...
一句话：**接口回答「能干什么」，抽象类回答「如何干」。两者不是二选一，是两个不同问题的答案。**

下一篇 [04.接口而非实现编程](https://yccoding.com/pages/d080c5/)，你这里已经学会了「该怎么设计抽象」，但还有一个更哲学的问题：**调用方**为什么要看接口而不是实现？下篇就从一次【云迁移 6 万行代码】的虐漆事故说起。
