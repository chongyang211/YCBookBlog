---
title: 7.SOLID原则案例汇
date: 2022-04-19 21:14:36
permalink: /pages/f7d8c6/
categories:
  - 通用技术的提升
  - 重学设计模式
  - 面向对象设计
...
#### 目录介绍
- [1.一次SOLID争吵](#1一次solid争吵)
  - [1.1 团队三年之痛](#11-团队三年之痛)
  - [1.2 五次反转排查](#12-五次反转排查)
  - [1.3 灵魂五连问](#13-灵魂五连问)
- [2.SRP真相揭秘](#2srp真相揭秘)
  - [2.1 一个职责的边界](#21-一个职责的边界)
  - [2.2 滥用现场扫描](#22-滥用现场扫描)
  - [2.3 重构前后对比](#23-重构前后对比)
  - [2.4 误用反模式](#24-误用反模式)
- [3.OCP扩展真意](#3ocp扩展真意)
  - [3.1 修改与扩展之分](#31-修改与扩展之分)
  - [3.2 真实演化案例](#32-真实演化案例)
  - [3.3 扩展点设计](#33-扩展点设计)
  - [3.4 过度设计陷阱](#34-过度设计陷阱)
- [4.LSP契约深挖](#4lsp契约深挖)
  - [4.1 子类型规则](#41-子类型规则)
  - [4.2 矩形正方形悖论](#42-矩形正方形悖论)
  - [4.3 契约前后置](#43-契约前后置)
  - [4.4 真实事故复盘](#44-真实事故复盘)
- [5.ISP接口隔离](#5isp接口隔离)
  - [5.1 胖接口的成本](#51-胖接口的成本)
  - [5.2 拆分的判据](#52-拆分的判据)
  - [5.3 与SRP差异](#53-与srp差异)
  - [5.4 拆过头的代价](#54-拆过头的代价)
- [6.DIP依赖倒置](#6dip依赖倒置)
  - [6.1 倒置的方向](#61-倒置的方向)
  - [6.2 IoC容器原理](#62-ioc容器原理)
  - [6.3 字节码织入](#63-字节码织入)
  - [6.4 何时不要倒置](#64-何时不要倒置)
- [7.SOLID关系图](#7solid关系图)
  - [7.1 五原则相互依赖](#71-五原则相互依赖)
  - [7.2 与设计模式的映射](#72-与设计模式的映射)
- [8.综合案例实战](#8综合案例实战)
  - [8.1 风控接入需求](#81-风控接入需求)
  - [8.2 不用SOLID翻车](#82-不用solid翻车)
  - [8.3 SOLID演化版](#83-solid演化版)
  - [8.4 类图与时序](#84-类图与时序)
...
**【A 段】**：表面有 3 个 if，**实际上只有 1 个 actor——业务规则方**。**不违反**。
...
**【B 段】**：看起来都是「用户操作」。**实际**：`uploadAvatar` 改动频率属于「图片处理团队」，与「身份团队」是两个 actor。**违反**。
...
**【C 段】**：三个动词分别属于：**报表统计团队、运营团队、运维团队**。三个 actor，**严重违反**。
...
**一句话总结 SOLID**：**SOLID 不是写好代码的"五条规则"，而是面对烂代码时"五种诊断工具"**——SRP 看 actor、ISP 看 caller、OCP 看 diff、LSP 看契约、DIP 看抽象层归属。
...
下一篇 [08.反模式与坏味道](https://yccoding.com/pages/69d9e3/)——把 30+ 种代码坏味道映射到 SOLID，让你的"代码嗅觉"成为肌肉记忆。本篇 §7.5 的三道思考题答案，也将在下一篇开头揭晓。
