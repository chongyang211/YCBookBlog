---
title: 2.银行账户管理系统
date: 2016-05-25 10:28:03
permalink: /pages/6a2180/
categories:
  - Cpp入门到精通
  - 综合案例
tags:
...
```bash
cd 项目目录下
g++ -std=c++17 main.cpp Account.cpp NormalAccount.cpp VipAccount.cpp SavingAccount.cpp Bank.cpp FileManager.cpp -o bank_system
./bank_system
```
...
新建一个 `BankSystem/` 目录，进入目录后创建空的源文件——**注意现在只是空文件，下面我们会逐个文件、按阶段往里填内容**：

```bash
mkdir BankSystem && cd BankSystem
...
**VipAccount.h**，这个类需要注意，存款是有利息的，并且取钱可以进行透支1000元。接口设计如下：
...
**SavingAccount.h**，存钱可以支持定期，会有到期时间，提前取钱会有违约金：
...
✋ **暂停！这里有个新问题冒出来了**：**怎么根据账号找到对应账户？** 这就是工程师的**真实开发节奏**——**新需求往往催生新工具**。我们需要一个**辅助函数**：
...
🧪 第三次编译运行。操作：选 `1` 开 N001 张三存 1000 → 选 `2` 给 N001 存 500
...
然后在`main.cpp` 接上 `case 4: bank.queryBalance(); break;`
...
紧接着`main.cpp` 接上 `case 5: bank.transfer(); break;`
