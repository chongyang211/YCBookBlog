...
模板就是建立通用的模具，大大提高复用性。比如一个 `swap` 函数模板，`int`、`double`、`string` 都能用同一份代码，无需为每种类型重写一遍。

模板的特点：

1. 模板不可以直接使用，它只是一个框架，必须指定具体类型（编译器推导或显式指定）后才能生成可用函数
2. 模板的通用并不是万能的，只有该类型支持函数体内的操作才能正常使用（例如自定义类型未重载 `<`，就不能用于 `max` 模板）

C++另一种编程思想称为 ==泛型编程== ，主要利用的技术就是模板：

1. C++提供两种模板机制: 函数模板和类模板
2. 模板由编译器在**编译期**根据实际使用的类型实例化出对应的代码，不是运行时机制
...
原理：每个类型参数在编译期都会按调用处的实际类型各实例化一份函数。上面两次调用分别生成 `printPair<int, double>` 和 `printPair<const char*, int>` 两个函数，参数类型不同、返回类型也能不同。编译器按实参顺序逐个推导 `T1`、`T2`，互不影响。

### 16.1.5 显式指定模板参数

使用模板需注意两点：

1. 自动类型推导时，所有实参必须能推导出**一致**的数据类型 `T`
2. 模板必须**确定出 `T` 的类型**后才能真正使用
...
int main() {
    int a = 10, b = 20;
    char c = 'c';
    mySwap(a, b);   // 正确，推导出一致的 T
    // mySwap(a, c);   // 错误，推导不出 int 和 char 一致的 T
    // func();         // 错误，无实参无法推导 T
    func<int>();    // 显式指定类型，才能实例化
    return 0;
}
```
...
**显式指定的核心原理**：模板在编译期才实例化，而实例化必须知道每个模板参数的具体类型。类型来源有两条路径——① 从实参自动推导（`mySwap(a,b)` 由 `int` 实参推出 `T=int`）；② 显式指定（`func<int>()` 直接在尖括号里给定）。当参数列表不含 `T`（如 `func()` 无参）时无法推导，只能显式指定；当多个实参推导出的 `T` 冲突时（如 `mySwap(a,c)` 推出 `int` 又推出 `char`）也是编译错误。显式指定与部分实参组合时，显式指定的在前、可推导的在后（如 `f<int>(x, 1.5)`）。

**`class` 与 `typename` 的区别**：声明模板参数时 `template<class T>` 和 `template<typename T>` 完全等价，可互换（早期模板复用了 `class` 关键字，后来才补充 `typename` 让语义更准确，因为 `T` 不一定是类）。两者唯一区别是 `typename` 还能用于标明「依赖类型」——当某个嵌套名字依赖模板参数时，必须用 `typename` 告诉编译器它是类型：

```cpp
template<class T>
void print(const T& container) {
    typename T::const_iterator it = container.begin(); // 必须用 typename，不能用 class
}
```

这里 `T::const_iterator` 依赖模板参数 `T`，编译器默认不知道它是「类型」还是「静态成员」，需要 `typename` 显式声明，此时 `class` 无法替代。

### 16.1.6 函数模板的特化

函数模板的特化，是为**特定类型**提供一份专门的实现，让它走定制逻辑，其余类型仍走通用模板。
...
原理：当调用处的实参类型能匹配特化版本时，编译器会**优先选择特化版**，而不是用通用模板再实例化一份。特化语法是 `template <>`（空的模板参数列表）+ 函数名后用尖括号写明具体类型。`const char*` 的普通 `print` 只会输出指针地址，特化后能正确打印字符串，这就是特化的典型用途——弥补通用模板对某种类型不适用的情况。

### 16.1.7 函数模板与重载

函数模板可以与普通函数重载，编译器会优先选择**最匹配**的那个。
...
int main() {
    print(10);    // 调用普通函数（int 精确匹配）
    print(10.5);  // 调用函数模板（double 没有普通重载）
    return 0;
}
```

原理：重载决议时，若一个普通函数和模板实例化出的函数**匹配度完全相同**，编译器优先选普通函数（普通函数优先于模板）。上面 `print(10)` 两者都匹配，但普通函数胜出；`print(10.5)` 只有模板能通过 `T=double` 实例化，所以走模板。规则可记为：**精确匹配的普通函数 > 模板实例化 > 需要隐式转换的普通函数**。
...
`typename T`：表示一个类型参数，`T` 可以是任意类型（如 `int`、`double`、`std::string` 等）。在类中，`T` 可以像普通类型一样使用。


### 16.2.3 类模板示例

**核心原理**：类模板本身不是类，而是一个「类的生成蓝图」。只有当你用具体类型实例化（如 `Box<int>`）时，编译器才会按需生成一份真正的类代码。因此 `Box<int>` 和 `Box<std::string>` 是**两个完全不同的类型**，各自拥有独立的成员函数副本。
...
private:
    T value;
public:
    Box(T v) : value(v) {}   // 类内定义构造函数
    void print();            // 类内只声明
};

// 成员函数类外定义：必须重新写 template，并用 Box<T>:: 限定
template <typename T>
void Box<T>::print() {
...
int main() {
    Box<int> intBox(10);            // 实例化出 Box<int>
    intBox.print();                 // 输出: Value: 10

    Box<std::string> strBox("hi");  // 实例化出 Box<std::string>
    strBox.print();                 // 输出: Value: hi
    return 0;
}
```
...
1. **类外定义为什么要重写 `template`**：类内定义时模板参数 `T` 在作用域内，可直接使用；但类外定义时已「离开」模板作用域，必须再次用 `template <typename T>` 重新声明参数，并用 `Box<T>::` 告诉编译器「这是哪个模板类的成员」。
2. **惰性实例化**：类模板的成员函数只有在**真正被调用**时才实例化。上例中 `print` 被 `int` 和 `string` 两种类型调用，编译器才生成两份代码；从未被调用的成员函数不会生成任何代码。
3. **类模板实参推导（C++17）**：C++17 之前必须显式写 `Box<int>`；C++17 起引入 CTAD，`Box box(10);` 可从构造参数自动推导为 `Box<int>`。

**思考题**：

1. `Box<int>` 和 `Box<double>` 是同一个类吗？它们之间能直接互相赋值吗？
2. 如果 `print()` 从未被调用，编译产物里还会有 `Box<int>::print` 的代码吗？
3. 类外定义成员函数时，`Box<T>::` 中的 `<T>` 为什么不能省略？
