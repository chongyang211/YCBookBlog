...
**综合案例：复合类型概览**，演示了C++中的四种复合数据类型：数组、结构体、枚举、联合体。
...
**为什么要指定数组大小？** 因为数组是一块**连续的内存**，编译器必须在**编译期**就确定它占多少字节：

1. **好分配空间**：局部数组在栈上，而函数栈帧的大小是编译期就固定的，编译器得先知道数组多大，才能安排每个变量的位置（如 `[rbp-20]`）；
2. **好找元素**：访问 `arr[i]` 要换算成 `首地址 + i × sizeof(元素)`，只有总大小已知，`sizeof(arr)` 才算得出。

**不设置会怎样？** 分三种情况：

| 写法 | 结果 |
|------|------|
| `int arr[];` | ❌ 编译报错：不完整类型，编译器不知道该分配多少空间 |
| `int arr[] = {1, 2, 3};` | ✅ 合法：编译器按初始化列表**自动推断**大小为 3 |
| `int n; cin >> n; int arr[n];` | ❌ 标准 C++ 报错：n 是变量不是常量（部分编译器放过，但不可移植） |

💡 **大小要运行时才知道怎么办？** 用 `new int[n]` 在堆上动态分配（用完 `delete[]`），或直接用 `std::vector<int>`（推荐，自动管理内存、可扩容）。
...
C++ 使用 `new` 和 `delete` 运算符动态分配和释放数组。示例如下：
...
结构体作为函数参数主要有三种传递方式：**值传递**、**引用传递**和**地址传递**。
...
// 2. 引用传递：直接操作原数据，避免拷贝开销
void birthday(Student &stu) {
    stu.age += 1;
}

// 3. 地址传递：通过指针修改原数据
void changeScore(Student *stu, int newScore) {
    stu->score = newScore;
}
...
int main() {
    ...
    birthday(stu);             // 引用传递：stu.age 变为 19
    changeScore(&stu, 90);     // 地址传递：stu.score 变为 90
    printStudent(stu);
    return 0;
}
```

| 传递方式 | 是否修改原数据 | 是否产生拷贝 | 推荐场景 |
|---|---|---|---|
| 值传递 `func(Student s)` | 否 | 是 | 结构体较小 |
| 引用传递 `func(Student &s)` | 是 | 否 | 需要修改原数据 |
| 地址传递 `func(Student *s)` | 是 | 否 | 指针操作 |
| const引用 `func(const Student &s)` | 否 | 否 | 只读且高效 |
...
打印学生成绩：学校正在做毕设项目，每名老师带领5个学生，总共有3名老师，需求如下：

1.设计学生和老师的结构体，其中在老师的结构体中，有老师姓名和一个存放5名学生的数组作为成员

2.学生的成员有姓名、考试分数，创建数组存放3名老师，通过函数给每个老师及所带的学生赋值，最终打印出老师数据以及老师所带的学生数据。
...
void allocateSpace(Teacher tArray[], int len) {
	...
	for (int i = 0; i < len; i++) {
		tArray[i].name = "教师" + string(1, nameSeed[i]);
		for (int j = 0; j < 5; j++) {
			tArray[i].sArray[j].name = "学生" + string(1, nameSeed[j]);
