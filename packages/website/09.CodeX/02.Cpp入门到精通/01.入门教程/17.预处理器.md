---
title: 预处理器
date: 2026-05-07 20:23:20
permalink: /pages/52fb59/
categories:
  - book
  - Cpp入门到精通
  - 入门教程
...
## 目录介绍
- [第 17 章 C++ 预处理器](#第-17-章-c-预处理器)
- [目录介绍](#目录介绍)
- [17.1 头文件](#171-头文件)
  - [17.1.1 头文件作用](#1711-头文件作用)
  - [17.1.2 基本结构](#1712-基本结构)
  - [17.1.3 头文件包含规则](#1713-头文件包含规则)
  - [17.1.4 综合案例与思考](#1714-综合案例与思考)
- [17.2 宏定义](#172-宏定义)
  - [17.2.1 什么是宏](#1721-什么是宏)
  - [17.2.2 基本用法](#1722-基本用法)
  - [17.2.3 带参宏与陷阱](#1723-带参宏与陷阱)
  - [17.2.4 宏的作用域](#1724-宏的作用域)
  - [17.2.5 宏与函数的区别](#1725-宏与函数的区别)
  - [17.2.6 综合案例与思考](#1726-综合案例与思考)
- [17.3 头文件保护](#173-头文件保护)
  - [17.3.1 理解头文件保护](#1731-理解头文件保护)
  - [17.3.2 头文件保护作用](#1732-头文件保护作用)
  - [17.3.3 示例说明](#1733-示例说明)
  - [17.3.4 不用头文件保护](#1734-不用头文件保护)
  - [17.3.5 使用场景](#1735-使用场景)
  - [17.3.6 头文件保护原理](#1736-头文件保护原理)
  - [17.3.7 头文件保护总结](#1737-头文件保护总结)
  - [17.3.8 综合案例与思考](#1738-综合案例与思考)
- [17.4 预定义宏](#174-预定义宏)
  - [17.4.1 标准预定义宏](#1741-标准预定义宏)
  - [17.4.2 编译器相关宏](#1742-编译器相关宏)
  - [17.4.3 操作系统相关宏](#1743-操作系统相关宏)
  - [17.4.4 标准库相关宏](#1744-标准库相关宏)
  - [17.4.5 其他常用宏](#1745-其他常用宏)
  - [17.4.6 预定义宏案例](#1746-预定义宏案例)
  - [17.4.7 综合案例与思考](#1747-综合案例与思考)
- [17.5 条件编译](#175-条件编译)
  - [17.5.1 基本语法](#1751-基本语法)
  - [17.5.2 常用指令](#1752-常用指令)
  - [17.5.3 示例代码案例](#1753-示例代码案例)
- [17.6 错误/警告指令](#176-错误警告指令)
  - [17.6.1 错误指令](#1761-错误指令)
  - [17.6.2 警告指令](#1762-警告指令)
  - [17.6.3 使用的场景](#1763-使用的场景)
  - [17.6.4 注意事项](#1764-注意事项)
  - [17.6.5 综合案例与思考](#1765-综合案例与思考)
- [17.9 综合宏案例](#179-综合宏案例)
  - [17.9.1 日志记录宏](#1791-日志记录宏)
- [17.10 预处理器底层原理](#1710-预处理器底层原理)
  - [17.10.1 预处理的编译阶段](#17101-预处理的编译阶段)
  - [17.10.2 宏展开的规则](#17102-宏展开的规则)
  - [17.10.3 头文件包含的实现](#17103-头文件包含的实现)
- [17.11 预处理器训练题](#1711-预处理器训练题)
- [17.12 综合思考题](#1712-综合思考题)
...
**避免循环包含**，如果两个头文件相互包含，会导致编译错误。可以通过前置声明（Forward Declaration）解决。
...
宏的本质是**纯文本替换**：预处理阶段，编译器把源码中出现的宏名逐字替换为定义的内容，不做任何类型检查，也不参与后续编译阶段的语法分析。因此宏不具备作用域，只要定义之后、在 `#undef` 之前，全文件皆可使用。

按是否带参数，宏通常分为两类：

- **对象宏（常量宏）**：如 `#define PI 3.14159`，用于替换常量或字符串。
- **函数宏（带参宏）**：如 `#define SQUARE(x) ((x) * (x))`，用于替换代码片段。
...
- `MACRO_NAME`：宏名，习惯使用大写字母。
- `value`：替换体，可以是常量、表达式或代码片段。
...
调用 `SQUARE(5)` 会展开为 `((5) * (5))`。

**原理：纯文本替换**。如果忘记加括号，会因运算符优先级而出错：

```cpp
#define SQUARE(x) x * x        // 错误写法

SQUARE(1 + 2);   // 展开为 1 + 2 * 1 + 2 = 5，而不是期望的 9
```

正确写法必须给**每个参数**和**整体**都加括号：
...
**副作用陷阱**：参数若带副作用（如 `i++`），会被求值多次：

```cpp
int i = 3;
SQUARE(i++);   // 展开为 ((i++) * (i++))，i 被自增两次，结果未定义
```

因此带参宏的参数应避免传入 `++`、`--`、函数调用等有副作用的表达式。

### 17.2.4 宏的作用域

宏没有作用域，从定义处起、到文件结束或 `#undef` 之前全局有效：
...
| 对比项   | 宏                         | 函数                             |
|----------|----------------------------|----------------------------------|
| 处理阶段 | 预处理期文本替换           | 编译期 + 运行期调用              |
| 类型检查 | 无                         | 有，更安全                       |
| 参数求值 | 可能被求值多次             | 只求值一次                       |
| 调试     | 难（代码已被替换）         | 易                               |
| 运行开销 | 无调用开销，但代码膨胀     | 有调用开销（`inline` 可消除）    |

**优缺点小结**：

- **优点**：简单直接、无函数调用开销、可用于条件编译与生成代码（`#`、`##`）。
- **缺点**：无类型检查、难调试、易出现优先级与副作用问题、降低可读性。

现代 C++ 中，能用 `constexpr`、`inline` 函数或模板替代的，应优先使用它们而非宏。

### 17.2.6 综合案例与思考

下面通过一个"调试工具宏"案例，综合演示宏的**用法**、**原理**与**注意事项**：
...
- **用法**：常量宏 `VERSION` 替代魔法数字提升可读性；带参宏 `MAX/MIN/CLAMP` 封装表达式；`PRINT_VAR` 用 `do-while-0` 包裹多语句；`#`（字符串化）与 `##`（连接）生成字符串和唯一标识符。
- **原理**：宏在预处理期做纯文本替换——`STRINGIFY(x)` 展开为 `"x"`，`CONCAT(a, b)` 展开为 `ab`，`__LINE__` 替换为调用处的行号。
- **注意事项**：带参宏每个参数都要加括号防优先级错误；参数避免副作用（`MAX(a++, 3)` 中 `a` 被求值两次，而 `safeMax(b++, 3)` 只求值一次）；多语句宏必须用 `do-while-0` 包裹才能在 `if-else` 中安全使用；`#undef` 用于提前取消宏定义。
...
1. **删除注释**：`//`和`/* */`被替换为空格
2. **处理`#include`**：将头文件的内容原封不动地插入到当前位置
3. **展开宏**：将所有`#define`定义的宏替换为对应的文本
4. **处理条件编译**：根据`#if/#ifdef`等决定保留或删除代码段
5. **处理`#line`和`#pragma`**
...
**`#include "file"`和`#include <file>`的区别**：`"file"`：先在当前文件所在目录搜索，再搜索系统路径。`<file>`：只在系统路径（`-I`指定的路径和标准库路径）搜索

**`#pragma once` vs `#ifndef`保护的底层差异**：
...
- `#ifndef`：每次`#include`都要打开文件、读取内容、检查宏。但如果宏已定义，预处理器可以快速跳过
- `#pragma once`：编译器记录文件的唯一标识（路径或inode），同一文件只打开一次。更快但不标准（主流编译器都支持）

**头文件展开导致的编译时间问题**：

一个简单的`#include <iostream>`会展开为约2万行代码。如果100个`.cpp`文件都包含它，预处理器要处理200万行。这就是C++编译慢的主要原因之一。
...
1. **宏 vs constexpr vs inline**：C++11之后，很多宏的使用场景可以被`constexpr`函数和`inline`函数替代。请对比三者在类型安全、调试支持、作用域控制方面的差异。哪些场景仍然必须使用宏？（提示：`__FILE__`、`__LINE__`、字符串化`#`）

2. **头文件地狱与编译时间**：一个大型C++项目（如Chromium）有数千个头文件，包含关系错综复杂。前向声明（forward declaration）、Pimpl模式、预编译头（PCH）分别如何减少编译时间？C++20的模块（Modules）是否能彻底解决这个问题？

3. **`#pragma`的跨平台问题**：`#pragma once`、`#pragma pack`、`#pragma comment`等在不同编译器上的行为可能不同。如何编写真正可移植的代码？GCC的`__attribute__`和MSVC的`__declspec`如何统一？
