...
**扩容策略**：`StringBuilder` 默认初始容量 16 字符。当 `append()` 导致容量不足时，新容量 = **旧容量 × 2 + 2**，然后 `Arrays.copyOf()` 复制数据到新数组。每次扩容是 O(n) 的，但均摊到 n 次 append 操作上，平均复杂度为 O(1)——这就是**均摊分析（Amortized Analysis）**。如果预知字符串长度，推荐用 `new StringBuilder(预估长度)` 避免多次扩容。
...
├── String（不可变）        数组
│   ├── 常量池共享          ├── 声明 int[] arr;
│   ├── 线程安全            ├── 初始化 new int[5] / {1,2,3}
│   ├── hashCode 缓存       ├── 遍历 for / for-each / Arrays.toString
│   └── 少量拼接用 +        ├── 多维 int[][] matrix
│                           └── 工具 Arrays.sort/copy/fill/equals
├── StringBuilder（可变、快、非线程安全）
│   ├── 循环拼接首选
│   ├── 初始容量 16，扩容 2n+2
│   └── 链式调用 append().insert().reverse()
│
└── StringBuffer（可变、慢、线程安全）
    └── 多线程场景
```

**三个核心要点**：

1. **String 不可变性是设计基石**：安全、可共享、可缓存 hashCode。拼接操作 `+` 底层编译为 `StringBuilder.append()`，循环中直接用 `StringBuilder` 避免重复创建临时对象
2. **`==` 和 `equals()` 的区别**：`==` 比较引用地址，`equals()` 比较内容。字符串常量池让相同字面量共享同一个对象，所以 `"a" == "a"` 为 true，但 `new String("a") == "a"` 为 false
3. **数组的特点**：固定长度、连续内存、O(1) 随机访问。适合已知大小的场景；动态场景用 `ArrayList`
