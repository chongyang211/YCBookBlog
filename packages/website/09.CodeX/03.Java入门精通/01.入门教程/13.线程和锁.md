...
并发编程是 Java 高级开发的必修课——线程和锁是基础，理解它们才能驾驭更高层的并发工具。

**并发三要素**：**可见性**（一个线程的修改对其他线程立即可见）、**原子性**（操作不可分割）、**有序性**（指令重排不影响单线程结果但影响多线程）

**三个核心要点**：

1. **synchronized 底层**：JDK 1.6 后做了大量优化——偏向锁 → 轻量级锁（CAS） → 重量级锁（OS mutex），**锁升级**是单向的。现在 synchronized 的性能已经不输 ReentrantLock
2. **Lock vs synchronized**：`ReentrantLock` 支持公平锁、可中断锁、超时获取、多条件变量——功能更丰富。synchronized 自动释放锁（代码块结束或异常时），更安全。**优先用 synchronized，需要高级特性再用 Lock**
3. **线程池是标配**：不要手动 new Thread()，用 `Executors` 工厂方法或自定义 `ThreadPoolExecutor`。关键参数：核心线程数、最大线程数、存活时间、工作队列类型（有界/无界）、拒绝策略
