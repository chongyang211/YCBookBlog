...
方法核心体系
├── 定义                    参数传递
│   ├── [修饰符] 返回值 方法名(参数)    ├── 基本类型：传值副本，方法内修改不影响外部
│   └── 必须在类中                    └── 引用类型：传引用的副本
│                                          ├── 通过 .set 修改对象内容 ✅ 有效
│                                          └── acc = new... 改变指向  ❌ 无效
├── 重载（Overload）
│   ├── 同方法名 + 不同参数列表          可变参数（Varargs）
│   ├── 返回值不同不算重载              ├── int... nums → 编译后等价于 int[]
│   └── 静态分派：编译期根据参数类型决定    └── 必须是最后一个参数，只能有一个
│
├── 递归
│   ├── 必须要有终止条件
│   ├── 栈溢出风险（每次调用消耗栈帧）
...
1. **Java 只有值传递**：没有 C++ 的引用传递。基本类型传值副本，引用类型传"引用的值"副本——可以修改对象内容，但不能改变引用本身
2. **重载 = 静态分派**：编译器根据参数类型在编译期选定方法，返回类型不参与分派。可变参数优先级最低
3. **递归解决"同结构子问题"**：凡是能用循环的都能用递归；递归代码更简洁，但有栈溢出风险，深层递归优先考虑迭代
