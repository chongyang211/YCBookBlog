---
title: 基础语法
date: 2026-04-07 11:13:30
permalink: /pages/d62067/
categories:
  - book
  - Java高级进阶课
  - 入门教程
...
- [1.2 Java开发环境](#12-java开发环境)
  ...
  - [1.2.6 综合案例：JDK环境检测工具](#126-综合案例jdk环境检测工具)
  - [1.2.7 训练题](#127-训练题)
...
- [1.5 多种注释使用](#15-多种注释使用)
  ...
  - [1.5.2 多行注释](#152-多行注释)
  - [1.5.3 文档注释](#153-文档注释)
  - [1.5.4 综合案例：API文档生成器](#154-综合案例api文档生成器)
  - [1.5.5 训练题](#155-训练题)
...
- [1.8 标识符](#18-标识符)
  ...
  - [1.8.2 命名规范](#182-命名规范)
  - [1.8.3 综合案例：命名规范检查器](#183-综合案例命名规范检查器)
  - [1.8.4 训练题](#184-训练题)
- [1.9 关键字](#19-关键字)
...
Java 源文件 → `javac` → 字节码 `.class` → `java` → JVM 执行。**一次编译，到处运行**的基石是 JVM 的不同平台实现。

**三个核心要点**：

1. **标识符与关键字**：标识符区分大小写、不能以数字开头。Java 有 50+ 关键字，`goto`/`const` 是保留但未使用的
2. **编译流程**：`.java` 经 javac 编译为平台无关的 `.class` 字节码，由 JVM 解释执行。JDK 包含 JRE + 开发工具，JRE 包含 JVM + 核心类库
3. **规范高于技巧**：类名大驼峰、方法名小驼峰、常量全大写下划线——这些不是炫技，是团队协作的通行证
