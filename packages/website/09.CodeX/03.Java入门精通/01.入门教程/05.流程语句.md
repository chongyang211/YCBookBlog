...
流程控制是程序的骨架。Java 的流程语句与 C/C++ 语义基本一致，但做了安全增强。

**三个核心要点**：

1. **switch 演进**：JDK 7 支持 String、JDK 12+ 支持箭头语法（`case x ->`）、JDK 14+ 支持 switch 表达式返回值。现代 Java 的 switch 已不逊于 Kotlin 的 when
2. **增强 for 循环**：`for (Type var : iterable)` 内部使用 Iterator，遍历时修改集合会抛 `ConcurrentModificationException`。需要索引或修改元素时用传统 for
3. **标签 break/continue**：Java 支持带标签的跳转语句跳出多层循环（`break label`），虽然 goto 是保留字，但标签机制提供了等价的安全跳转能力
