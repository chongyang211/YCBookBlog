...
泛型是 Java 5 引入的最重要特性之一——它把类型检查从运行时提前到编译期，消除了大量的 `ClassCastException`。

**三个核心要点**：

1. **类型擦除是核心机制**：`List<String>` 和 `List<Integer>` 编译后都是 `List`（原生类型）——泛型信息只存在于编译期。这不是 Bug，是为了兼容 JDK 5 之前的代码。要绕过擦除看类型信息，可以用反射读 `ParameterizedType`
2. **通配符的本质是限制**：`? extends T`（上界）只读不写（生产者），`? super T`（下界）只写不读（消费者）。记住 PECS 原则（Producer Extends, Consumer Super），泛型设计就不会出错
3. **泛型擦除带来的坑**：不能 `new T()`（类型参数无法实例化）、不能用 `instanceof` 检查泛型类型、静态成员不能引用类的类型参数。这些限制都源自"类型信息在运行时不存在"这一事实
