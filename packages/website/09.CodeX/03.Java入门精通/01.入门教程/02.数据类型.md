---
title: 数据类型
date: 2026-04-07 11:15:08
permalink: /pages/0dceae/
categories:
  - book
  - Java高级进阶课
  - 入门教程
...
- [2.8 包装类](#28-包装类)
  ...
  - [2.8.6 训练题](#286-训练题)
  - [2.8.7 类型转换案例](#287-类型转换案例)
  - [2.8.8 键盘数据输入](#288-键盘数据输入)
...
Java 的 8 种基本类型（byte/short/int/long 整型 + float/double 浮点 + char 字符 + boolean 布尔）是值类型，存在栈上或内嵌于对象中，**不是对象**——这是和 C# 等"一切皆对象"语言的关键区别。

**三个核心要点**：

1. **自动装箱拆箱**：`Integer i = 10` 编译为 `Integer.valueOf(10)`，-128~127 有缓存池。`==` 比较包装类型时极容易踩坑——超出缓存范围的装箱对象 `==` 返回 false
2. **类型转换优先级**：`byte → short → int → long → float → double`，小转大自动、大转小需强转+可能丢精度。三元运算符 `? :` 两边类型不一致时会做二进制数值提升
3. **引用类型 ≠ 基本类型**：引用类型的 `==` 比较地址，`equals()` 比较内容；基本类型的 `==` 比较值。这是 Java 面试最高频的陷阱之一
