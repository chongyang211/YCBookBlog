...
> 🔗 **延伸阅读**：← [11.线程前世今生探索](./11.线程前世今生探索.md) · → [13.线程异常设计原理](./13.线程异常设计原理.md) · → [15.并发编程设计思想](./15.并发编程设计思想.md) · → [18.锁核心设计和思想](./18.锁核心设计和思想.md) · → [19.AQS核心思想揭秘](./19.AQS核心思想揭秘.md) · → [23.协程核心设计思想](./23.协程核心设计思想.md) · → [24.Actor与CSP并发模型](./24.Actor与CSP并发模型.md)
...
- [2.通信设计哲学](#2通信设计哲学)
  ...
  - [2.3 共享内存与消息传递的边界](#23-共享内存与消息传递的边界)
  - [2.4 同步与异步的取舍](#24-同步与异步的取舍)
  - [2.5 设计决策树](#25-设计决策树)
- [3.通信原理深析](#3通信原理深析)
  - [3.1 数学本质：Happens-Before偏序](#31-数学本质happens-before偏序)
  - [3.2 三件硬件武器](#32-三件硬件武器)
  - [3.3 同步原语金字塔](#33-同步原语金字塔)
  - [3.4 四种通信语义](#34-四种通信语义)
  - [3.5 设计权衡](#35-设计权衡)
- [4.共享内存模型深度剖析](#4共享内存模型深度剖析)
  - [4.1 互斥的本质与 Mutex 实现](#41-互斥的本质与-mutex-实现)
  - [4.2 等待与通知Condition Variable](#42-等待与通知condition-variable)
  - [4.3 信号量及其变种](#43-信号量及其变种)
  - [4.4 内存屏障与原子操作](#44-内存屏障与原子操作)
  - [4.5 读写分离与乐观锁](#45-读写分离与乐观锁)
- [5.消息传递模型深度剖析](#5消息传递模型深度剖析)
  - [5.1 Channel 的设计本质](#51-channel-的设计本质)
  - [5.2 Go channel 源码级剖析](#52-go-channel-源码级剖析)
  - [5.3 Actor 模型与 Erlang](#53-actor-模型与-erlang)
  - [5.4 CSP vs Actor 对比](#54-csp-vs-actor-对比)
  - [5.5 异步消息与背压](#55-异步消息与背压)
- [6.Java 线程通信全景](#6java-线程通信全景)
  - [6.1 wait/notify与Object Monitor](#61-waitnotify与object-monitor)
  - [6.2 Lock + Condition](#62-lock--condition)
  - [6.3 BlockingQueue](#63-blockingqueue)
  - [6.4 CountDownLatch/Barrier/Phaser](#64-countdownlatchbarrierphaser)
  - [6.5 Semaphore](#65-semaphore)
  - [6.6 Future与CompletableFuture](#66-future与completablefuture)
  - [6.7 通信场景全表](#67-通信场景全表)
- [7.C++/Go/Rust 通信对比](#7cgorust-通信对比)
  - [7.1 C++：贴着 OS 的细粒度控制](#71-c贴着-os-的细粒度控制)
  - [7.2 Go：channel 一统天下](#72-gochannel-一统天下)
  - [7.3 Rust：所有权防数据竞争](#73-rust所有权防数据竞争)
  - [7.4 横向对比总表](#74-横向对比总表)
- [8.经典陷阱与反模式](#8经典陷阱与反模式)
  - [8.1 锁陷阱](#81-锁陷阱)
  - [8.2 条件变量陷阱](#82-条件变量陷阱)
  - [8.3 channel 陷阱](#83-channel-陷阱)
  - [8.4 调试与定位](#84-调试与定位)
  - [8.5 跨语言通信陷阱速查表](#85-跨语言通信陷阱速查表)
- [9.一句话总结](#9一句话总结)
- [10.案例驱动的设计深入](#10案例驱动的设计深入)
  - [10.1 案例：生产事故看选型错误](#101-案例生产事故看选型错误)
  - [10.2 案例：BlockingQueue vs Disruptor](#102-案例blockingqueue-vs-disruptor)
  - [10.3 案例：Go channel关闭模式](#103-案例go-channel关闭模式)
- [11.通信演化史与一句话总结](#11通信演化史与一句话总结)
  - [11.1 通信范式的螺旋上升](#111-通信范式的螺旋上升)
  - [11.2 设计哲学三大流派](#112-设计哲学三大流派)
  - [11.3 七字真言 × 七语言映射](#113-七字真言--七语言映射)
...
- ← [11.线程前世今生探索](./11.线程前世今生探索.md)：通信前置——线程是什么
- → [13.线程异常设计原理](./13.线程异常设计原理.md)：通信失败时如何安全终止
- → [14.多线程并发经典案例](./14.多线程并发经典案例.md)：把通信原理落到生产实战
...
- → [24.Actor与CSP并发模型](./24.Actor与CSP并发模型.md)：消息传递的两大流派
...
### 10.2 案例：BlockingQueue vs Disruptor
