...
#### 目录介绍
- [01.真实事故引入](#01真实事故引入)
  - [1.1 一次幽灵任务账户错乱事故](#11-一次幽灵任务账户错乱事故)
  - [1.2 一个故意泄漏1秒的bug](#12-一个故意泄漏1秒的bug)
  - [1.3 灵魂三问](#13-灵魂三问)
  - [1.4 五个层层递进的追问](#14-五个层层递进的追问)
  - [1.5 探索路径](#15-探索路径)
  - [1.6 为什么这个问题值得讲透](#16-为什么这个问题值得讲透)
- [02.根本矛盾：并发为什么难](#02根本矛盾并发为什么难)
  - [2.1 顺序代码vs并发代码根本差异](#21-顺序代码vs并发代码根本差异)
  - [2.2 三大深坑](#22-三大深坑)
  - [2.3 错误处理的灾难](#23-错误处理的灾难)
- [03.结构化并发的诞生：Goto十年回响](#03结构化并发的诞生goto十年回响)
  - [3.1 1968 年的"Goto 大辩论"](#31-1968-年的goto-大辩论)
  - [3.2 2018年Nathaniel Smith重磅炸弹](#32-2018年nathaniel-smith重磅炸弹)
  - [3.3 结构化并发的"反向革命"](#33-结构化并发的反向革命)
  - [3.4 演进时间线](#34-演进时间线)
- [04.设计灵魂：词法作用域 = 生命周期](#04设计灵魂词法作用域--生命周期)
  - [4.1 一行定义](#41-一行定义)
  - [4.2 三条不可违反的铁律](#42-三条不可违反的铁律)
  - [4.3 顺序代码vs并发代码完美对照](#43-顺序代码vs并发代码完美对照)
  - [4.4 为什么这个设计如此强大](#44-为什么这个设计如此强大)
- [05.跨语言实现对照](#05跨语言实现对照)
  - [5.1 Kotlin：coroutineScope标杆实现](#51-kotlincoroutinescope标杆实现)
  - [5.2 Swift：async let与TaskGroup](#52-swiftasync-let与taskgroup)
  - [5.3 Java 21：StructuredTaskScope预览](#53-java-21structuredtaskscope预览)
  - [5.4 Python Trio：始祖（2018）](#54-python-trio始祖2018)
  - [5.5 半结构化：Promise.all与Task.WhenAll](#55-半结构化promiseall与taskwhenall)
  - [5.6 Go：context.Context另一条路](#56-gocontextcontext另一条路)
- [06.核心机制三要素](#06核心机制三要素)
  - [6.1 等待合并](#61-等待合并)
  - [6.2 取消传播](#62-取消传播)
  - [6.3 异常聚合](#63-异常聚合)
- [07.Scope的三种人格：合并策略](#07scope的三种人格合并策略)
  - [7.1 ShutdownOnFailure：全员皆输](#71-shutdownonfailure全员皆输)
  - [7.2 ShutdownOnSuccess：一胜即返](#72-shutdownonsuccess一胜即返)
  - [7.3 Supervisor：各自独立（Kotlin）](#73-supervisor各自独立kotlin)
  - [7.4 三种策略对照](#74-三种策略对照)
- [08.经典陷阱与生产级反模式](#08经典陷阱与生产级反模式)
  - [8.1 陷阱一：GlobalScope启动协程](#81-陷阱一globalscope启动协程)
  - [8.2 陷阱二：launch当fire-and-forget](#82-陷阱二launch当fire-and-forget)
  - [8.3 陷阱三：CPU密集任务忽略取消](#83-陷阱三cpu密集任务忽略取消)
  - [8.4 陷阱四：runBlocking在suspend里](#84-陷阱四runblocking在suspend里)
  - [8.5 陷阱五：忘记结构化导致泄露](#85-陷阱五忘记结构化导致泄露)
  - [8.6 陷阱六：异常处理时序](#86-陷阱六异常处理时序)
  - [8.7 陷阱七：跨 scope 的依赖](#87-陷阱七跨-scope-的依赖)
- [09.跨语言结构化并发陷阱速查表](#09跨语言结构化并发陷阱速查表)
- [10.七字真言 × 多语言映射表](#10七字真言--多语言映射表)
  - [10.1 真言 ⇨ 跨语言原语映射](#101-真言--跨语言原语映射)
  - [10.2 三大支柱（七字真言压缩版）](#102-三大支柱七字真言压缩版)
- [11.一句话总结](#11一句话总结)
  - [11.1 三层认知阶梯](#111-三层认知阶梯)
  - [11.2 选型决策树](#112-选型决策树)
  - [11.3 七字真言](#113-七字真言)
  - [11.4 第 3 卷的收束](#114-第-3-卷的收束)
