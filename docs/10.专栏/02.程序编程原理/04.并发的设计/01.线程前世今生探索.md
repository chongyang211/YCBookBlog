...
#### 目录介绍
- [1.案例引入](#1案例引入)
  - [1.1 Web 服务器场景](#11-web-服务器场景)
  - [1.2 单线程串行的代价](#12-单线程串行的代价)
  - [1.3 多进程方案的代价](#13-多进程方案的代价)
  - [1.4 多线程方案的价值](#14-多线程方案的价值)
  - [1.5 引出核心矛盾](#15-引出核心矛盾)
- [2.线程设计哲学](#2线程设计哲学)
  - [2.1 核心设计原则](#21-核心设计原则)
  - [2.2 线程演进时间线](#22-线程演进时间线)
  - [2.3 共享与隔离的边界](#23-共享与隔离的边界)
  - [2.4 抢占式调度的必然性](#24-抢占式调度的必然性)
  - [2.5 设计决策树](#25-设计决策树)
- [3.三大线程模型](#3三大线程模型)
  - [3.1 1:1 内核级线程模型](#31-11-内核级线程模型)
  - [3.2 N:1 用户级线程模型](#32-n1-用户级线程模型)
  - [3.3 M:N 混合线程模型](#33-mn-混合线程模型)
  - [3.4 GMP 调度模型剖析](#34-gmp-调度模型剖析)
  - [3.5 模型选择决策树](#35-模型选择决策树)
- [4.线程架构设计](#4线程架构设计)
  - [4.1 task_struct 深度解析](#41-task_struct-深度解析)
  - [4.2 fork 与 clone 的演进](#42-fork-与-clone-的演进)
  - [4.3 上下文切换三段式](#43-上下文切换三段式)
  - [4.4 线程栈架构](#44-线程栈架构)
  - [4.5 缓存一致性与内存屏障](#45-缓存一致性与内存屏障)
- [5.线程生命周期](#5线程生命周期)
  - [5.1 五态模型与状态机](#51-五态模型与状态机)
  - [5.2 跨语言状态对比](#52-跨语言状态对比)
  - [5.3 关键状态转换的内核视角](#53-关键状态转换的内核视角)
  - [5.4 线程状态观察工具](#54-线程状态观察工具)
- [6.线程核心 API](#6线程核心-api)
  - [6.1 生命周期 API 与 interrupt 哲学](#61-生命周期-api-与-interrupt-哲学)
  - [6.2 调度控制 API](#62-调度控制-api)
  - [6.3 同步原语层级](#63-同步原语层级)
  - [6.4 AQS 深度剖析](#64-aqs-深度剖析)
- [7.跨语言线程对比](#7跨语言线程对比)
  - [7.1 Java：从平台线程到虚拟线程](#71-java从平台线程到虚拟线程)
  - [7.2 C/C++：贴着内核的线程](#72-cc贴着内核的线程)
  - [7.3 Go：GMP 与 goroutine 的胜利](#73-gogmp-与-goroutine-的胜利)
  - [7.4 JavaScript：单线程的逆袭](#74-javascript单线程的逆袭)
  - [7.5 Kotlin：协程的状态机魔法](#75-kotlin协程的状态机魔法)
  - [7.6 横向对比总表](#76-横向对比总表)
  - [7.7 线程技术演进趋势](#77-线程技术演进趋势)
- [8.经典陷阱与反模式](#8经典陷阱与反模式)
  - [8.1 创建陷阱](#81-创建陷阱)
  - [8.2 中断陷阱](#82-中断陷阱)
  - [8.3 内存可见性陷阱](#83-内存可见性陷阱)
  - [8.4 调试与定位](#84-调试与定位)
  - [8.5 跨语言线程陷阱速查表](#85-跨语言线程陷阱速查表)
- [9.一句话总结](#9一句话总结)
- [10.案例驱动的深度剖析](#10案例驱动的深度剖析)
  - [10.1 案例：12306抢票脚本压垮Tomcat](#101-案例12306抢票脚本压垮tomcat)
  - [10.2 案例：Netty用1024个线程跑100万连接](#102-案例netty用1024个线程跑100万连接)
  - [10.3 案例：jstack 看死锁的标准操作](#103-案例jstack-看死锁的标准操作)
- [11.线程未来：虚拟线程与结构化并发](#11线程未来虚拟线程与结构化并发)
  - [11.1 虚拟线程（Project Loom）的颠覆](#111-虚拟线程project-loom的颠覆)
  - [11.2 结构化并发Structured Concurrency](#112-结构化并发structured-concurrency)
  - [11.3 结构化并发的设计哲学](#113-结构化并发的设计哲学)
- [12.七字真言 × 七语言映射](#12七字真言--七语言映射)
...
- → [13.线程异常设计原理](./13.线程异常设计原理.md)：线程内异常为何不会传染外部，怎么向外传递
- → [14.多线程并发经典案例](./14.多线程并发经典案例.md)：把上述原理落到生产实战的 N 个经典案例
...
### 11.2 结构化并发Structured Concurrency
