...
> 🔗 **延伸阅读**：← [3.11 异步和同步的设计](./3.11.异步和同步的设计.md) · ← [3.12 单线程模型的思想](./3.12.单线程模型的思想.md) · → [3.14 Actor 与 CSP 并发模型](./3.14.Actor与CSP并发模型.md) · → [3.18 结构化并发设计思想](./3.18.结构化并发设计思想.md) · → [2.4 函数调用栈与栈帧设计](../02.运行时模型/2.4.函数调用栈与栈帧设计.md)
...
#### 目录介绍
- [01.真实事故引入](#01真实事故引入)
  - [1.1 一次线程池打满CPU闲置雪崩](#11-一次线程池打满cpu闲置雪崩)
  - [1.2 一段Java 21看似未改的革命](#12-一段java-21看似未改的革命)
  - [1.3 灵魂三问](#13-灵魂三问)
  - [1.4 本篇的探索路径](#14-本篇的探索路径)
  - [1.5 为什么这个问题值得讲透](#15-为什么这个问题值得讲透)
- [02.为什么需要协程：线程与回调的困境](#02为什么需要协程线程与回调的困境)
  - [2.1 线程模型的三大天花板](#21-线程模型的三大天花板)
  - [2.2 异步回调的"反人类"本质](#22-异步回调的反人类本质)
  - [2.3 协程要解决的根本矛盾](#23-协程要解决的根本矛盾)
  - [2.4 协程史前史：60年的等待](#24-协程史前史60年的等待)
- [03.协程的本质：可挂起的函数](#03协程的本质可挂起的函数)
  - [3.1 从"子程序"到"协程"的飞跃](#31-从子程序到协程的飞跃)
  - [3.2 挂起与恢复的机器视角](#32-挂起与恢复的机器视角)
  - [3.3 与线程、回调的关键区别](#33-与线程回调的关键区别)
  - [3.4 协程是语法糖还是机器特性](#34-协程是语法糖还是机器特性)
- [04.两大流派：有栈 vs 无栈](#04两大流派有栈-vs-无栈)
  - [4.1 有栈协程Stackful：Goroutine设计](#41-有栈协程stackfulgoroutine设计)
  - [4.2 无栈协程Stackless：async/await](#42-无栈协程stacklessasyncawait)
  - [4.3 两派对比](#43-两派对比)
  - [4.4 为什么现代语言都转向无栈](#44-为什么现代语言都转向无栈)
- [05.协程实现三件套：挂起点/状态机/调度器](#05协程实现三件套挂起点状态机调度器)
  - [5.1 挂起点（Suspend Point）](#51-挂起点suspend-point)
  - [5.2 状态机：CPS 变换](#52-状态机cps-变换)
  - [5.3 调度器：谁决定何时恢复](#53-调度器谁决定何时恢复)
- [06.跨语言对比与设计哲学](#06跨语言对比与设计哲学)
  - [6.1 Kotlin协程：状态机+Continuation](#61-kotlin协程状态机continuation)
  - [6.2 Go goroutine：有栈协程+GMP](#62-go-goroutine有栈协程gmp)
  - [6.3 Rust async/await：零成本抽象](#63-rust-asyncawait零成本抽象)
  - [6.4 JavaScript：Generator到async/await](#64-javascriptgenerator到asyncawait)
  - [6.5 Python asyncio](#65-python-asyncio)
  - [6.6 Java虚拟线程Project Loom](#66-java虚拟线程project-loom)
  - [6.7 跨语言横评](#67-跨语言横评)
- [07.协程的经典陷阱](#07协程的经典陷阱)
  - [7.1 陷阱一：async函数写阻塞调用](#71-陷阱一async函数写阻塞调用)
  - [7.2 陷阱二：忘await让协程消失](#72-陷阱二忘await让协程消失)
  - [7.3 陷阱三：协程泄漏](#73-陷阱三协程泄漏)
  - [7.4 陷阱四：上下文丢失ThreadLocal失效](#74-陷阱四上下文丢失threadlocal失效)
  - [7.5 陷阱五：细粒度切换反而慢](#75-陷阱五细粒度切换反而慢)
  - [7.6 陷阱六：异常吞没](#76-陷阱六异常吞没)
  - [7.7 陷阱七：CPU密集任务错用Dispatcher](#77-陷阱七cpu密集任务错用dispatcher)
- [08.共性抽象与未来演进](#08共性抽象与未来演进)
  - [8.1 去掉语法糖后的共同骨架](#81-去掉语法糖后的共同骨架)
  - [8.2 协程的面向对象时刻结构化并发](#82-协程的面向对象时刻结构化并发)
  - [8.3 历史的回望：50 年的循环](#83-历史的回望50-年的循环)
  - [8.4 未来：协程会怎么演进](#84-未来协程会怎么演进)
- [09.跨语言协程陷阱速查表](#09跨语言协程陷阱速查表)
- [10.七字真言 × 多语言映射表](#10七字真言--多语言映射表)
- [11.一句话总结](#11一句话总结)
  - [11.1 三层认知阶梯](#111-三层认知阶梯)
  - [11.2 协程选型决策树](#112-协程选型决策树)
  - [11.3 七字真言](#113-七字真言)
...
**为什么 Go 能做到，C 做不到？**（详见 [2.4 函数调用栈与栈帧设计](../02.运行时模型/2.4.函数调用栈与栈帧设计.md)）
...
## 05.协程实现三件套：挂起点/状态机/调度器

### 5.1 挂起点（Suspend Point）
...
### 6.4 JavaScript：Generator到async/await
...
**修复**：使用结构化并发（详见 [3.18 结构化并发](./3.18.结构化并发设计思想.md)）：
...
**结构化并发（Structured Concurrency）** 是协程的"面向对象时刻"——**让协程的生命周期跟随作用域**，详见 [3.18 结构化并发设计思想](./3.18.结构化并发设计思想.md)。
...
下一篇 [3.14 Actor 与 CSP 并发模型](./3.14.Actor与CSP并发模型.md) 我们会看到——**让协程通过消息传递协作**，这是 Erlang 和 Go 共同的答卷。

---

## 🔗 延伸阅读

- 同卷上篇：[3.11 异步和同步的设计](./3.11.异步和同步的设计.md) ｜ [3.12 单线程模型的思想](./3.12.单线程模型的思想.md)
- 同卷下篇：[3.14 Actor 与 CSP 并发模型](./3.14.Actor与CSP并发模型.md) ｜ [3.18 结构化并发设计思想](./3.18.结构化并发设计思想.md)
- 第 2 卷视角：[2.4 函数调用栈与栈帧设计](../02.运行时模型/2.4.函数调用栈与栈帧设计.md)（理解协程栈的物理基础）
- 经典文献：
