...
> 🔗 **延伸阅读**：← [3.15 线程池的设计思想](./3.15.线程池的设计思想.md) · → [3.17 线程池使用技巧](./3.17.线程池使用技巧.md) · → [3.18 结构化并发设计思想](./3.18.结构化并发设计思想.md) · → [3.13 协程核心设计思想](./3.13.协程核心设计思想.md)
...
#### 目录介绍
- [01.真实事故引入](#01真实事故引入)
  - [1.1 凌晨3点：execute卡了30秒](#11-凌晨3点execute卡了30秒)
  - [1.2 灵魂三问](#12-灵魂三问)
  - [1.3 五个层层递进的追问](#13-五个层层递进的追问)
  - [1.4 探索路径](#14-探索路径)
  - [1.5 为什么这个问题值得讲透](#15-为什么这个问题值得讲透)
- [02.Executor 框架的设计哲学](#02executor-框架的设计哲学)
  - [2.1 设计由来：Thread到Executor](#21-设计由来thread到executor)
  - [2.2 框架层次结构](#22-框架层次结构)
  - [2.3 接口设计的精妙](#23-接口设计的精妙)
  - [2.4 ThreadPoolExecutor的7个参数](#24-threadpoolexecutor的7个参数)
- [03.ThreadPoolExecutor源码级解剖](#03threadpoolexecutor源码级解剖)
  - [3.1 ctl变量：状态加线程数合一个int](#31-ctl变量状态加线程数合一个int)
  - [3.2 5 种状态的物理含义](#32-5-种状态的物理含义)
  - [3.3 状态转换的精妙](#33-状态转换的精妙)
- [04.execute流程的三段论设计](#04execute流程的三段论设计)
  - [4.1 三段论的算法](#41-三段论的算法)
  - [4.2 三段论的设计哲学](#42-三段论的设计哲学)
  - [4.3 队列饱和的"三段论"反作用](#43-队列饱和的三段论反作用)
- [05.Worker内部类：复用与不可重入](#05worker内部类复用与不可重入)
  - [5.1 Worker 是什么](#51-worker-是什么)
  - [5.2 runWorker：复用线程的核心](#52-runworker复用线程的核心)
  - [5.3 Worker为什么用不可重入锁](#53-worker为什么用不可重入锁)
  - [5.4 Worker隐藏陷阱：状态-1](#54-worker隐藏陷阱状态-1)
- [06.状态流转：5状态4转换状态机](#06状态流转5状态4转换状态机)
  - [6.1 完整状态图](#61-完整状态图)
  - [6.2 shutdown vs shutdownNow](#62-shutdown-vs-shutdownnow)
  - [6.3 awaitTermination等待真正终止](#63-awaittermination等待真正终止)
  - [6.4 拒绝策略4种内置+自定义](#64-拒绝策略4种内置自定义)
- [07.跨语言线程池实现对照](#07跨语言线程池实现对照)
  - [7.1 Netty EventLoopGroup](#71-netty-eventloopgroup)
  - [7.2 Tomcat 线程池防饱和](#72-tomcat-线程池防饱和)
  - [7.3 Go runtime：隐式线程池](#73-go-runtime隐式线程池)
  - [7.4 Python concurrent.futures](#74-python-concurrentfutures)
  - [7.5 .NET ThreadPool](#75-net-threadpool)
- [08.源码级经典陷阱](#08源码级经典陷阱)
  - [8.1 陷阱一：Executors工厂内存炸弹](#81-陷阱一executors工厂内存炸弹1)
  - [8.2 陷阱二：core=max让keepAlive无意义](#82-陷阱二coremax让keepalive无意义)
  - [8.3 陷阱三：异常吞没](#83-陷阱三异常吞没)
  - [8.4 陷阱四：prestartCoreThread意义](#84-陷阱四prestartcorethread意义)
  - [8.5 陷阱五：Worker 不可重入锁踩坑](#85-陷阱五worker-不可重入锁踩坑)
  - [8.6 陷阱六：拒绝策略选错](#86-陷阱六拒绝策略选错)
  - [8.7 陷阱七：监控盲区](#87-陷阱七监控盲区)
- [09.一句话总结](#09一句话总结)
  - [9.1 三层认知阶梯](#91-三层认知阶梯)
  - [9.2 线程池决策树](#92-线程池决策树)
  - [9.3 七字真言](#93-七字真言)
  - [9.3.1 七字真言 × 多语言映射表](#931-七字真言--多语言映射表)
...
## 03.ThreadPoolExecutor源码级解剖

### 3.1 ctl变量：状态加线程数合一个int
...
下一篇 [3.17 线程池使用技巧](./3.17.线程池使用技巧.md) 我们会进入**实战调优**——如何根据业务设计线程池参数、如何避免雪崩、如何做监控。再下一篇 [3.18 结构化并发设计思想](./3.18.结构化并发设计思想.md) 会回答"任务关系"这个本质问题。

---

## 🔗 延伸阅读

- 同卷上篇：[3.15 线程池的设计思想](./3.15.线程池的设计思想.md)
- 同卷下篇：[3.17 线程池使用技巧](./3.17.线程池使用技巧.md) ｜ [3.18 结构化并发设计思想](./3.18.结构化并发设计思想.md)
- 同卷相关：[3.13 协程核心设计思想](./3.13.协程核心设计思想.md)（"线程池的协程版"）
- 经典文献：
