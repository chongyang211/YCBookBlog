...
## 00.为什么所有支持线程的语言都要重新发明线程池

> 🧭 **本节定位**：把"线程池"从 Java `ThreadPoolExecutor` 的具体实现上抬到**语言无关的池化哲学**。读完三问，你将明白：**线程池不是一种 API，而是一种被多核 + 高频请求逼出来的资源管理范式**——任何能创建线程的语言都会重新发明它，只是形态各异。

### 0.1 章首三问（带着问题读完整篇）

1. **Go 的 goroutine 又便宜又轻（2KB 起步），为什么 Go 没有"goroutine pool"概念？** 那为什么 Go 项目里又频繁出现 `worker pool` 模式？
2. **Java/C++/C# 都有线程池，但 Python 的 GIL 让多线程并行打折——Python 为什么还保留 `ThreadPoolExecutor`？**
3. **Rust 的 `tokio Runtime` 算线程池吗？它和 Java `ThreadPoolExecutor` 有什么本质区别？**

### 0.2 通用本质：线程池是"资源池化"的一种特化
...
不管你用哪种语言、哪种池化方案，下面四个问题**永远绕不开**：

```mermaid
flowchart TD
    Q1[Q1: 多少个线程?<br/>core / max] --> Q2[Q2: 任务怎么排队?<br/>有界 / 无界 / Direct]
    Q2 --> Q3[Q3: 爆掉怎么办?<br/>拒绝 / 阻塞 / 丢弃]
    Q3 --> Q4[Q4: 怎么观测?<br/>active / queue / 拒绝率]
...
| 问题 | Java | Go (worker pool) | C++ | Rust (tokio) |
|---|---|---|---|---|
| **多少线程** | `corePoolSize` / `maximumPoolSize` | `semaphore.Weighted(N)` / 自建 N 个 goroutine | 自己定 `vector<thread>` 大小 | `tokio::runtime::Builder.worker_threads(N)` |
| **任务排队** | `BlockingQueue`（**有界！**） | `chan Task`（buffered） | `queue<Task> + mutex + condvar` | task queue 由 runtime 管 |
| **爆掉怎么办** | `RejectedExecutionHandler` | channel 满则发送阻塞 / `select default` 丢弃 | 用户自己 if 检查 | `try_send` 失败 / 自定义策略 |
| **怎么观测** | `getActiveCount` / `getQueueSize` | 自己埋 `metrics.Gauge` | 自己埋 | `tokio-metrics` |

→ **学会了这四问，看任何语言的线程池实现都是变形题**。
...
### 0.7 本篇通用收益（与你用什么语言无关）

读完本篇你将获得三个语言无关的能力：

1. **看到任何"池"都能识别它的预创/复用/限闸/拒压四要素**——不论 ThreadPool、ConnectionPool、ObjectPool；
2. **能在 Go / Rust / C++ / Python 里写出符合"建池四问"的池化代码**——不被 Java API 绑架；
3. **能解释为什么 Go 不建线程池但建 worker pool**——前者由 runtime 管，后者控的是业务并发度。
