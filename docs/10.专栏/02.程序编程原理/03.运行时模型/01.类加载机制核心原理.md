...
- [1.真实事故引入](#1真实事故引入)
  - [1.1 五种语言的"同一个事故"](#11-五种语言的同一个事故同名却不同身份)
  - [1.1.1 一个统一的故事：同名实体被加载了两次](#111-一个统一的故事同名实体被加载了两次)
  - [1.1.2 一句话穿透五个事故的共同骨架](#112-一句话穿透五个事故的共同骨架)
  - [1.1.3 不止双加载——一连串"看似无关"的奇怪现象](#113-不止双加载一连串看似无关的奇怪现象)
  - [1.2 灵魂的五问](#12-灵魂的五问)
  - [1.3 本篇探索路径](#13-本篇探索路径)
  - [1.4 本章学习价值](#14-本章学习价值)
- [2.加载的本质矛盾](#2加载的本质矛盾)
  - [2.1 字节与对象鸿沟](#21-字节与对象鸿沟)
  - [2.2 三大根本约束](#22-三大根本约束)
  - [2.3 五种语言的答卷](#23-五种语言的答卷)
  - [2.4 加载阶段拆解](#24-加载阶段拆解)
  - [2.5 设计哲学共识](#25-设计哲学共识)
- [3.Java加载机制](#3java加载机制)
  - [3.0 所有语言都要回答的"加载三问"](#30-所有语言都要回答的加载三问)
  - [3.0.1 三个绕不开的物理问题](#301-三个绕不开的物理问题)
  - [3.0.2 各语言的"加载三问答卷"](#302-各语言的加载三问答卷)
  - [3.0.3 翻译速查表：JVM 加载 ↔ 其他语言](#303-翻译速查表jvm-加载--其他语言)
  - [3.0.4 一个对比性洞察：为什么 Java 把"链接"拆成三步](#304-一个对比性洞察为什么-java-把链接拆成三步)
  - [3.1 三阶段全景图](#31-三阶段全景图)
  - [3.2 双亲委派推导](#32-双亲委派推导)
  - [3.3 命名空间隔离](#33-命名空间隔离)
  - [3.4 类的唯一性等价](#34-类的唯一性等价)
  - [3.5 打破委派的场景](#35-打破委派的场景)
  - [3.6 初始化触发时机](#36-初始化触发时机)
- [4.CC加载机制](#4cc加载机制)
  - [4.1 静态链接的瞬间](#41-静态链接的瞬间)
  - [4.2 动态库三件事](#42-动态库三件事)
  - [4.3 符号解析与重定位](#43-符号解析与重定位)
  - [4.4 dlopen运行时加载](#44-dlopen运行时加载)
  - [4.5 与Java的本质差异](#45-与java的本质差异)
- [5.脚本语言加载](#5脚本语言加载)
  - [5.1 JS模块加载演进](#51-js模块加载演进)
  - [5.2 Python import机制](#52-python-import机制)
  - [5.3 Go静态链接哲学](#53-go静态链接哲学)
  - [5.4 解释vs编译的取舍](#54-解释vs编译的取舍)
- [6.经典陷阱反模式](#6经典陷阱反模式)
  - [6.1 类版本冲突地狱](#61-类版本冲突地狱)
  - [6.2 静态初始化死锁](#62-静态初始化死锁)
  - [6.2.1 其他语言里的"同根陷阱"](#621-其他语言里的同根陷阱)
  - [6.2.2 三语言"初始化死锁/顺序"对照表](#622-三语言初始化死锁顺序对照表)
  - [6.3 同名类不同加载器](#63-同名类不同加载器)
  - [6.4 动态库符号污染](#64-动态库符号污染)
  - [6.5 循环依赖陷阱](#65-循环依赖陷阱)
- [7.综合案例串讲](#7综合案例串讲)
  - [7.1 案例背景](#71-案例背景)
  - [7.2 链路全景](#72-链路全景)
  - [7.3 启动期加载](#73-启动期加载)
  - [7.4 插件热加载](#74-插件热加载)
  - [7.4.1 同样的"插件热加载"在其他语言里怎么做](#741-同样的插件热加载在其他语言里怎么做)
  - [7.5 跨容器调用](#75-跨容器调用)
  - [7.6 类卸载与GC](#76-类卸载与gc)
  - [7.7 案例知识点回归](#77-案例知识点回归)
- [8.一句话总结](#8一句话总结)
...
不是不需要懂，而是**每种语言把"加载"放在了不同的时间点**。看下面这张对比图：
...
我们用整整一章讲完了"字节如何变成对象"——这是运行时模型的**第一个核心矛盾**。下一篇 [《2.对象创建核心流程》](./02.对象创建核心流程.md) 将进入第二个矛盾：
