...
> 🔗 **延伸阅读**：← [2.4 调用栈与栈帧设计](./04.调用栈与栈帧设计.md) · → [2.6 JIT 与运行时优化](./06.JIT与运行时优化.md) · → [2.7 反射元编程核心设计](./07.反射元编程核心设计.md) · → [2.1 类加载机制核心原理](./01.类加载机制核心原理.md)
...
**HotSpot 启动后**：模板解释器立刻可用 → C1 编译热点 → C2 进一步优化。这是一个分层加速过程，下一篇 [2.6 JIT 与运行时优化](./06.JIT与运行时优化.md) 会深入。
...
**反射调用比直接调用慢 100 倍**——这就是为什么 Java 8 引入 `MethodHandle`（编译为字节码）和 Java 7 引入 `invokedynamic`，让动态调用接近直接调用速度。详见 [2.7 反射元编程核心设计](./07.反射元编程核心设计.md)。
...
下一篇 [2.6 JIT 与运行时优化](./06.JIT与运行时优化.md) 我们将深入这个"性能黑魔法"——**热点检测、分层编译、内联、逃逸分析、去优化**，理解为什么"代码越跑越快"是真实存在的。

---

## 🔗 延伸阅读

- 同卷上篇：[2.4 调用栈与栈帧设计](./04.调用栈与栈帧设计.md)
- 同卷下篇：[2.6 JIT 与运行时优化](./06.JIT与运行时优化.md) ｜ [2.7 反射元编程核心设计](./07.反射元编程核心设计.md) ｜ [2.8 异常机制核心设计](./08.异常机制核心设计.md)
- 同卷相关：[2.1 类加载机制核心原理](./01.类加载机制核心原理.md)（详细的 .class 文件加载和类初始化）
- 经典文献：
