...
> 🔗 **延伸阅读**：← [2.7 反射元编程核心设计](./07.反射元编程核心设计.md) · ← [2.4 调用栈与栈帧设计](./04.调用栈与栈帧设计.md) · → [3.x 并发错误处理](../04.并发的设计/) · → [5.x RPC 错误传播](../06.交互和系统/)
...
- [2.1](./01.类加载机制核心原理.md) 类加载——程序如何从字节码变成可执行实体
- [2.2](./02.对象创建核心流程.md) 对象创建——new 关键字背后的全部工作
- [2.3](./03.对象和函数访问原理.md) 对象与函数访问——成员访问和方法调用的物理实现
- [2.4](./04.调用栈与栈帧设计.md) 调用栈——函数调用的物理痕迹
- [2.5](./05.字节码虚拟机执行.md) 字节码与 VM——逻辑 CPU 的本质
- [2.6](./06.JIT与运行时优化.md) JIT——运行时的性能魔法
- [2.7](./07.反射元编程核心设计.md) 反射与元编程——程序操纵自身
...
- 同卷上篇：[2.7 反射元编程核心设计](./07.反射元编程核心设计.md) ｜ [2.4 调用栈与栈帧设计](./04.调用栈与栈帧设计.md)
