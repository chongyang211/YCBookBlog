...
我们用整整一章讲完了"模板如何变成实例"——这是运行时模型的**第二个核心矛盾**。上一篇《1.类加载机制核心原理》解决了"字节如何变成模板"，本篇解决了"模板如何变成实例"。下一篇 [《3.对象和函数访问原理》](./03.对象和函数访问原理.md) 将进入第三个矛盾：
...
- ← [01.类加载机制核心原理](./01.类加载机制核心原理.md)：对象创建的前提条件——类要先存在
- → [03.对象和函数访问原理](./03.对象和函数访问原理.md)：对象创建后的访问机制
- → [07.反射元编程核心设计](./07.反射元编程核心设计.md)：动态对象创建的高级技术
- → [04.内存模型技术设计](../05.内存的真相/02.内存模型技术设计.md)：对象内存布局的底层原理
