...
> ➡ 卷三第 2 章 [指针与逃逸分析](../03.专栏博客/02.指针与逃逸分析.md) 会完整讲 6 条转换规则、`uintptr` GC 风险、`reflect.SliceHeader` 在 1.20 后被弃用的历史。
...
### 卷内
- 第 5 章 [复合类型](./05.复合类型.md)（slice/map/struct 内存布局基础）
- 第 7 章 [函数](./07.函数.md)（闭包逃逸、defer 与命名返回）
- 第 9 章 [方法与接收者](./09.方法与接收者.md)（接收者选型）
- 第 10 章 [接口与多态](./10.接口与多态.md)（接口值导致的逃逸）
