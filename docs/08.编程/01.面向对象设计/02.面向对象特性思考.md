...
上一篇 [01.面向对象设计思想](https://yccoding.com/pages/oop/) 末尾留下了三道题：
...
> 所以换一个最小的样本：**一只钱包**，只有 `balance` 和 `balanceLastModifiedTime` 两个字段。
> 在这个玩具级的例子上，四个特性的"有没有"差别会像放大镜一样清楚。
...
> 回到钱包：`Wallet` 对外暴露 `increase / decrease / balance`，调用方不知道余额是写到 MySQL 还是 Redis——这正是抽象在业务模型中的落地。
...
```java
// 调用方被迫写满 if-else
void consume(Wallet w, BigDecimal amt) {
    if (w instanceof NormalWallet && w.balance().compareTo(amt) < 0)
        throw new InsufficientBalanceException();
    else if (w instanceof CreditWallet && w.balance().subtract(amt).compareTo(CREDIT_LIMIT) < 0)
        throw new InsufficientBalanceException();
    // 每加一种钱包，这里都得改 ← 灾难
}
```
...
```java
void consume(Wallet w, BigDecimal amt) { w.consume(amt); }   // 永远不用改
```

**多态消除条件分支**，把"类型判断"从代码里彻底拿掉。

> 现实中文付渠道（Alipay / Wechat / UnionPay）、日志器（FileLogger / ConsoleLogger）、存储引擎（OSS / S3）
> 才是多态最高频的战场——它们会在第 04 篇展开。
