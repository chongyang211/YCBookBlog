...
> 2. **第 02 节 · 模式介绍** — 建造者要解决什么、和工厂有何不同
> 3. **第 03 节 · 实现演变** — 从 "new 12 个参数" → "setter" → "Builder" 三步演化
> 4. **第 04 节 · 模式分析** — 4 个角色、时序图、结构图一次拆透
> 5. **第 05 节 · 盖房子实战** — 普通方式 → Builder 方式对比开发
> 6. **第 06 节 · 模式拓展** — Builder 何时可以省略 Director / 抽象建造者
> 7. **第 07~08 节 · 优缺点 + 联动** — 真实开源案例、踩坑、决策口诀
>
