...
> 📌 **本篇定位**：CQRS 是从「分层架构」「六边形架构」走向「事件驱动架构」之间的**关键过渡篇**。读完本篇后，再看 [04.事件驱动架构设计](04.事件驱动架构设计.md)，你会发现 CQRS 就是事件驱动在"读写视角"下的必然产物。
...
- `order_events` 是事件出箱（详见 [04.事件驱动架构设计](04.事件驱动架构设计.md) 第 6.2 节）
...
3. **事件契约不能变**——事件已存就不能改格式（详见 [04.事件驱动架构设计](04.事件驱动架构设计.md) 第 8.1 节）
...
详见 [04.事件驱动架构设计](04.事件驱动架构设计.md) 第 6.3 节 Saga 编排。
...
> **下一篇**：我们已经知道了"读写如何分离"，下一步进入 [04.事件驱动架构设计](04.事件驱动架构设计.md)——把"服务之间如何通过事件解耦"剖到投递语义级别。
