...
2. YUV422 采样: 每2个Y分量对应一组UV分量，一个像素共占用(8+8*1/2 + 8*1/2)bit = 2byte 
3. YUV420 采样: 每4个Y分量对应一组UV分量，一个像素共占用(8+8*1/4+8*1/4) = 1.5byte
...
一张 1920 * 1280 大小的图片，如果按照YUV444采样，其存储占用共：(1920*1280*8 + 1920*1280*8*1/2 + 1920*1280*8*1/2)/8/1024/1024 = 4.68M
