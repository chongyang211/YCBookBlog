---
title: USB开发设计实践
date: 2025-06-30 10:05:53
permalink: /pages/96e32a/
categories:
  - book
  - Android提升进阶
  - 智能硬件
...
通过`UsbManager`这个系统提供的类，我们可以枚举出当前连接的所有usb设备，我们主要需要的是`UsbDevice`对象。

`UsbDevice`，这个类就代表了Android所连接的usb设备。通过`getDeviceList()`获取已连接的USB设备列表。
...
请注意，为了使用`UsbManager`，你需要在`AndroidManifest.xml`文件中声明USB权限：
