...
> **下一篇**：list 的节点级容器说清了。下一篇进入 [35.关联容器红黑树](35.关联容器红黑树.md)——`map/set` 的底层红黑树五性质、节点重用 `extract/insert(node_type)`、heterogeneous lookup 的 O(1) 免临时对象查找——为什么 `map.find("key")` 不需要构造 `std::string` 了。
