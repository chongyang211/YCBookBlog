...
> 📌 **本篇定位**：[02.对象内存布局原理](02.对象内存布局原理.md) 看的是**单一对象**的字段排列；[05.虚函数表深度剖析](05.虚函数表深度剖析.md) 拆开了 vtable 这台引擎本身；**本篇站在"完整对象"视角，看多个子对象如何拼接、转型时指针如何调整、菱形与虚继承如何工作**。下游 [07.内存对齐与缓存行](07.内存对齐与缓存行.md) 会回到对齐与 cache 的视角。
...
> **下一篇**：本篇拆开了多继承的完整对象拓扑，但还停留在"对象内部如何排布"。下一步进入 [07.内存对齐与缓存行](07.内存对齐与缓存行.md)——**对齐的硬件原因、cache line 与假共享、alignas/alignof 实战、std::hardware_destructive_interference_size、SIMD 对齐分配、perf c2c 检测假共享**。本篇说"对象内部怎么拼"，下一篇拆"对象之间在 cache 上如何相安无事"。
