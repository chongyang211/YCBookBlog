...
> **下一篇**：vector 的连续内存扩容说清了。下一篇进入 [33.deque 分段连续设计](33.deque分段连续设计.md)——deque 的 map+chunk 两级结构、为什么 deque 有 O(1) 随机访问却不是真正的连续内存、front/back O(1) 插入的妙处、为什么 stack/queue 默认用 deque 而不是 vector。
