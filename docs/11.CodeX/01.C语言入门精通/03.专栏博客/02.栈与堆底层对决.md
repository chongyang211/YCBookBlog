...
- [4. 栈帧结构全拆解](#4-栈帧结构全拆解)
  - [4.1 函数调用的栈帧完整生命周期](#41-函数调用的栈帧完整生命周期)
  - [4.2 返回地址的存储与劫持](#42-返回地址的存储与劫持)
  - [4.3 局部变量在栈上的排列](#43-局部变量在栈上的排列)
  - [4.4 栈变量为何不需要手动管理](#44-栈变量为何不需要手动管理)
- [5. 堆的扩展与回收](#5-堆的扩展与回收)
  - [5.1 从brk到mmap两条通道](#51-从brk到mmap两条通道)
  - [5.2 malloc/free的chunk模型](#52-mallocfree的chunk模型)
...
- [9. 常见线上事故](#9-常见线上事故)
  ...
  - [9.3 返回局部变量地址的幽灵](#93-返回局部变量地址的幽灵)
- [10. 综合案例串讲](#10-综合案例串讲)
  - [10.1 案例真相揭晓](#101-案例真相揭晓)
  - [10.2 一段代码的栈与堆双重生涯](#102-一段代码的栈与堆双重生涯)
...
> **下一篇**：我们已经知道数据是放在栈上还是堆上了，下一步进入 [03.指针本质与多级解引](03.指针本质与多级解引.md)——`&x` 返回的地址到底是什么？为什么编译器说 `int*` 和 `char*` 是两种不同的东西？从汇编级别看透指针。
