...
## 目录
- [1. 案例引入](#1-案例引入)
  - [1.1 一个切换编译器就变异的循环](#11-一个切换编译器就变异的循环)
  - [1.2 顺藤摸到根因](#12-顺藤摸到根因)
  - [1.3 我们要回答什么](#13-我们要回答什么)
- [2. 架构概览](#2-架构概览)
  - [2.1 三种行为的定义与边界](#21-三种行为的定义与边界)
  - [2.2 UB 是编译器优化的燃料](#22-ub-是编译器优化的燃料)
- [3. UB 三级分类](#3-ub-三级分类)
  - [3.1 未定义行为: 一切皆可能](#31-未定义行为-一切皆可能)
  - [3.2 实现定义行为: 编译器说了算](#32-实现定义行为-编译器说了算)
  - [3.3 未指定行为: 标准不管顺序](#33-未指定行为-标准不管顺序)
  - [3.4 三者关系图谱与面试陷阱](#34-三者关系图谱与面试陷阱)
- [4. 整数溢出优化陷阱](#4-整数溢出优化陷阱)
  - [4.1 INT_MAX + 1 的三个面孔](#41-int_max--1-的三个面孔)
  - [4.2 编译器如何基于"溢出=UB"做激进优化](#42-编译器如何基于溢出ub做激进优化)
  - [4.3 安全检查被删除的完整过程](#43-安全检查被删除的完整过程)
  - [4.4 防御: -fwrapv 与无符号回绕](#44-防御--fwrapv-与无符号回绕)
- [5. 空指针解引用连锁反应](#5-空指针解引用连锁反应)
  - [5.1 解引用那一刻就已经 UB](#51-解引用那一刻就已经-ub)
  - [5.2 后续 null 检查被优化删除的证据](#52-后续-null-检查被优化删除的证据)
  - [5.3 Linux 内核的经典 null-check 删除事故](#53-linux-内核的经典-null-check-删除事故)
  - [5.4 防御: 先检查再用 vs 编译器屏障](#54-防御-先检查再用-vs-编译器屏障)
- [6. strict aliasing 规则](#6-strict-aliasing-规则)
  - [6.1 类型的"别名"约束是什么](#61-类型的别名约束是什么)
  - [6.2 为什么 `char*` 是唯一的万能别名](#62-为什么-char-是唯一的万能别名)
...
- [7. Sanitizer 三件套实战](#7-sanitizer-三件套实战)
  - [7.1 UBSan: 捕捉整数溢出/除零/非法类型双关](#71-ubsan-捕捉整数溢出除零非法类型双关)
  - [7.2 ASan: 捕捉越界/use-after-free/栈溢出](#72-asan-捕捉越界use-after-free栈溢出)
  - [7.3 TSan: 捕捉数据竞争和死锁](#73-tsan-捕捉数据竞争和死锁)
  - [7.4 三件套的 CI 集成最佳实践](#74-三件套的-ci-集成最佳实践)
- [8. 综合案例串讲](#8-综合案例串讲)
...
### 7.2 ASan: 捕捉越界/use-after-free/栈溢出
