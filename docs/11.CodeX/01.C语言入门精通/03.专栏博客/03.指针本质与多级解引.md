...
- [3. 地址与类型的双重身份](#3-地址与类型的双重身份)
  - [3.1 指针即数字——但数字有类型](#31-指针即数字但数字有类型)
  - [3.2 类型决定解引用后的宽度](#32-类型决定解引用后的宽度)
  - [3.3 类型决定指针运算的步长](#33-类型决定指针运算的步长)
...
- [5. & 与 * 的对称世界](#5--与--的对称世界)
  ...
  - [5.3 指针作为"可修改的门牌号"](#53-指针作为可修改的门牌号)
...
- [7. void* 类型擦除的艺术](#7-void-类型擦除的艺术)
  ...
  - [7.2 不可对void*做算术运算](#72-不可对void做算术运算)
  - [7.3 void* 的设计哲学与使用场景](#73-void-的设计哲学与使用场景)
- [8. 指针的安全边界](#8-指针的安全边界)
  - [8.1 野指针三宗罪](#81-野指针三宗罪)
  - [8.2 空指针解引用的电路级后果](#82-空指针解引用的电路级后果)
  - [8.3 安全指针编程规范](#83-安全指针编程规范)
- [9. 指针在数据结构的实际应用](#9-指针在数据结构的实际应用)
  - [9.1 链表：指针连接离散节点](#91-链表指针连接离散节点)
  - [9.2 函数指针表：用数据驱动逻辑](#92-函数指针表用数据驱动逻辑)
- [10. 综合案例串讲](#10-综合案例串讲)
  - [10.1 案例真相揭晓](#101-案例真相揭晓)
  - [10.2 指针对决——同一数据三种访问方式](#102-指针对决同一数据三种访问方式)
...
> **下一篇**：我们已经理解了指针对单个数据的间接访问，下一步进入 [04.指针运算底层真相](04.指针运算底层真相.md)——把 `p + 1`、`arr[i]`、`*(arr + i)` 从汇编层面统一，证明它们为什么是等价的。
