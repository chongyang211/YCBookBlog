...
- [2. 架构概览](#2-架构概览)
  - [2.1 函数指针的三层抽象全景](#21-函数指针的三层抽象全景)
  - [2.2 为什么编译期不知道要调用哪个函数](#22-为什么编译期不知道要调用哪个函数)
- [3. 函数名即地址](#3-函数名即地址)
  - [3.1 函数名在表达式中退化为函数指针](#31-函数名在表达式中退化为函数指针)
  - [3.2 call 指令的汇编级跳转](#32-call-指令的汇编级跳转)
  - [3.3 直接调用 vs 函数指针调用的汇编差异](#33-直接调用-vs-函数指针调用的汇编差异)
- [4. 声明与typedef 技巧](#4-声明与typedef-技巧)
  - [4.1 从简单到复杂的声明阶梯](#41-从简单到复杂的声明阶梯)
  - [4.2 typedef 把函数指针变成可读的类型名](#42-typedef-把函数指针变成可读的类型名)
  - [4.3 返回函数指针的函数声明](#43-返回函数指针的函数声明)
- [5. qsort源码逐行拆解](#5-qsort源码逐行拆解)
  - [5.1 qsort 的函数签名与调用契约](#51-qsort-的函数签名与调用契约)
  - [5.2 比较函数指针在泛型排序中的角色](#52-比较函数指针在泛型排序中的角色)
  - [5.3 base + i × size——函数指针与指针运算的完美合体](#53-base--i--size函数指针与指针运算的完美合体)
- [6. 回调模式与上下文传递](#6-回调模式与上下文传递)
  - [6.1 无上下文回调的局限](#61-无上下文回调的局限)
  - [6.2 void* user_data 传递上下文的经典模式](#62-void-user_data-传递上下文的经典模式)
  - [6.3 用回调注册实现观察者模式](#63-用回调注册实现观察者模式)
- [7. 函数指针数组与状态机](#7-函数指针数组与状态机)
  - [7.1 用函数指针数组替代 switch-case](#71-用函数指针数组替代-switch-case)
  - [7.2 状态机——每个状态一个处理函数](#72-状态机每个状态一个处理函数)
  - [7.3 跳转表——编译后的函数指针表在 rodata 段](#73-跳转表编译后的函数指针表在-rodata-段)
- [8. 信号处理与系统级回调](#8-信号处理与系统级回调)
  - [8.1 signal() 的函数签名与信号处理模型](#81-signal-的函数签名与信号处理模型)
  - [8.2 atexit——退出时回调](#82-atexit退出时回调)
  - [8.3 信号处理函数中的限制——异步信号安全](#83-信号处理函数中的限制异步信号安全)
- [9. 函数指针的安全陷阱](#9-函数指针的安全陷阱)
  - [9.1 把数据强转成函数指针的反模式](#91-把数据强转成函数指针的反模式)
  - [9.2 函数指针与数据指针的不可互转](#92-函数指针与数据指针的不可互转)
  - [9.3 回调中的悬空函数指针](#93-回调中的悬空函数指针)
- [10. 综合案例串讲](#10-综合案例串讲)
  - [10.1 案例真相揭晓](#101-案例真相揭晓)
  - [10.2 一个插件系统的完整生命周期](#102-一个插件系统的完整生命周期)
...
### 5.3 base + i × size——函数指针与指针运算的完美合体
...
> **下一篇**：函数指针让行为变成了可传递的值，下一步进入 [06.限定符与指针语义](06.限定符与指针语义.md)——`const`/`volatile`/`restrict` 如何与指针组合，让编译器在优化、安全、硬件交互三个维度上精准发力。
