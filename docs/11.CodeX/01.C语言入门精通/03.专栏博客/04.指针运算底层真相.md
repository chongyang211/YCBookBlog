...
- [2. 架构概览](#2-架构概览)
  ...
  - [2.2 为什么指针需要类型化运算](#22-为什么指针需要类型化运算)
- [3. 类型决定步长](#3-类型决定步长)
  - [3.1 p+1 的汇编等价形式](#31-p1-的汇编等价形式)
  - [3.2 不同类型指针的步长对比](#32-不同类型指针的步长对比)
  - [3.3 步长在结构体指针上的体现](#33-步长在结构体指针上的体现)
  - [3.4 p++ 与 ++p 的语义与汇编差异](#34-p-与-p-的语义与汇编差异)
- [4. 下标即指针运算](#4-下标即指针运算)
  - [4.1 arr[i] 的完整翻译链](#41-arri-的完整翻译链)
  - [4.2 为什么数组形参丢失大小信息](#42-为什么数组形参丢失大小信息)
  - [4.3 sizeof(arr)/sizeof(arr[0]) 的失效场景](#43-sizeofarrsizeofarr0-的失效场景)
- [5. 三等价终极证明](#5-三等价终极证明)
  - [5.1 arr[i] ↔ *(arr+i) 汇编证明](#51-arri---arri-汇编证明)
  - [5.2 i[arr] 的反直觉等价](#52-iarr-的反直觉等价)
  - [5.3 三等价对可读性的影响](#53-三等价对可读性的影响)
- [6. 指针减法与差值的语义](#6-指针减法与差值的语义)
  - [6.1 指针减法的元素个数语义](#61-指针减法的元素个数语义)
  - [6.2 无符号减法的陷阱](#62-无符号减法的陷阱)
  - [6.3 ptrdiff_t 的存在理由](#63-ptrdiff_t-的存在理由)
- [7. 指针比较的合法边界](#7-指针比较的合法边界)
  - [7.1 同数组内的比较总是合法](#71-同数组内的比较总是合法)
  - [7.2 跨对象比较的危险地带](#72-跨对象比较的危险地带)
  - [7.3 与 NULL 的比较是唯一跨对象安全操作](#73-与-null-的比较是唯一跨对象安全操作)
- [8. 越界指针与UB](#8-越界指针与ub)
  - [8.1 只可指向数组末尾+1](#81-只可指向数组末尾1)
  - [8.2 编造指针值的完全UB](#82-编造指针值的完全ub)
  - [8.3 编译器的激进优化——空指针检查被删除](#83-编译器的激进优化空指针检查被删除)
- [9. void* 与不完整类型的算术禁令](#9-void-与不完整类型的算术禁令)
...
- [10. 综合案例串讲](#10-综合案例串讲)
  ...
  - [10.2 一段指针运算的完整生涯](#102-一段指针运算的完整生涯)
...
> **下一篇**：指针运算让我们可以在数组上自由跳转，下一步进入 [05.函数指针与回调机制](05.函数指针与回调机制.md)——把函数也当成数据，用指针指向它、传递它、调用它。qsort 的源码里，`base + i × size` 这一行就是本章指针运算+下一篇函数指针的完美合体。
