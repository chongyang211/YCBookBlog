...
| 本章概念 | 卷三对应篇 |
|---|---|
| `sync.Mutex` 的自旋 + 信号量实现 | [10.sync 同步原语剖析](https://yccoding.com/pages/4b9b33/) |
| `RWMutex` 的读锁计数与写锁等待队列 | [10.sync 同步原语剖析](https://yccoding.com/pages/4b9b33/) |
| `atomic` 的 CPU 指令（LOCK 前缀） | [12.Go 内存模型一致性](https://yccoding.com/pages/946899/) |
| `sync.Pool` 的 local pool + victim cache | [10.sync 同步原语剖析](https://yccoding.com/pages/4b9b33/) |
| `sync.Map` 的 read/dirty 双 map 结构 | [11.map 并发安全与哈希](https://yccoding.com/pages/db2460/) |
...
- 入门卷：[第 12 章 goroutine](https://yccoding.com/pages/f7872e/)——goroutine 起步、WaitGroup、context
- 入门卷：[第 13 章 channel](https://yccoding.com/pages/8dba21/)——"通过通信共享内存"
- 卷三：[10.sync 同步原语剖析](https://yccoding.com/pages/4b9b33/)——Mutex/RWMutex/Once/Pool 源码级拆解
- 卷三：[12.Go 内存模型一致性](https://yccoding.com/pages/946899/)——happens-before、atomic 内存序
- 卷三：[11.map 并发安全与哈希](https://yccoding.com/pages/db2460/)——sync.Map 的 read/dirty 结构
