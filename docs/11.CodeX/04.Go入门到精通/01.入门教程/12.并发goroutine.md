...
> 卷三第 08 篇 [GMP 协程调度器机制](../03.专栏博客/08.GMP协程调度器机制.md) 会深入 GMP 的调度循环、工作窃取、抢占式调度。
...
| 本章概念 | 卷三对应篇 |
|---|---|
| `go f()` 如何被调度 | [08.GMP 协程调度器机制](../03.专栏博客/08.GMP协程调度器机制.md) |
| G、M、P 的生命周期 | [08.GMP 协程调度器机制](../03.专栏博客/08.GMP协程调度器机制.md) |
| 连续栈扩容（2KB→1GB） | [01.内存模型与栈堆布局](../03.专栏博客/01.内存模型与栈堆布局.md) |
| `context` 取消传播实现 | [23.上下文取消与传播](../03.专栏博客/23.上下文取消与传播.md) |
| goroutine 泄漏排查 | [15.协程泄漏排查与修复](../03.专栏博客/15.协程泄漏排查与修复.md) |
| `sync.WaitGroup` 源码 | [10.sync 同步原语剖析](../03.专栏博客/10.sync同步原语剖析.md) |
| goroutine 抢占式调度 | [21.抢占式调度器原理](../03.专栏博客/21.抢占式调度器原理.md) |
...
- 入门卷：[第 13 章 channel](./13.通道channel.md)——channel 是 goroutine 间的通信工具
- 入门卷：[第 14 章 sync 包](./14.同步sync包.md)——Mutex、RWMutex、Once、Pool
- 卷三：[08.GMP 协程调度器机制](../03.专栏博客/08.GMP协程调度器机制.md)——GMP 三者的协作全流程
- 卷三：[23.上下文取消与传播](../03.专栏博客/23.上下文取消与传播.md)——context 的底层实现
- 卷三：[15.协程泄漏排查与修复](../03.专栏博客/15.协程泄漏排查与修复.md)——pprof 分析泄漏 + 5 种修复模式
