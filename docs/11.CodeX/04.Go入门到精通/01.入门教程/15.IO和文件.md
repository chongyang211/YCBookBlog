...
| 本章概念 | 卷三对应篇 |
|---|---|
| `io.Copy` 零拷贝（sendfile/splice） | [32.文件 IO 与零拷贝](../03.专栏博客/32.文件IO与零拷贝.md) |
| `bufio` 批量 syscall 加速 | [32.文件 IO 与零拷贝](../03.专栏博客/32.文件IO与零拷贝.md) |
| `os.File` 底层文件描述符与 poller | [32.文件 IO 与零拷贝](../03.专栏博客/32.文件IO与零拷贝.md) |
...
- 入门卷：[第 13 章 channel](./13.通道channel.md)——goroutine 间通信
- 入门卷：[第 14 章 sync 包](./14.同步sync包.md)——并发安全
- 卷三：[32.文件 IO 与零拷贝](../03.专栏博客/32.文件IO与零拷贝.md)——`io.Copy` 零拷贝、`bufio` 批量 syscall 源码级拆解
