...
> **下一篇**：我们已经掌握了 syscall 和 cgo 的边界机制——`entersyscall` 释放 P、`cgocall` 三层栈切换、`LockOSThread` 线程绑定。下一步进入卷三的终章 [36.编译链接与PGO优化](36.编译链接与PGO优化.md)——看看 `go build` 的编译流程到底做了什么、`-ldflags` 怎么注入版本信息、以及 PGO（Profile-Guided Optimization）如何让编译器基于生产 profile 做内联和分支优化。
