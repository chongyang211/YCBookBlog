...
> **下一篇**：我们已经看清了 channel 的 `hchan` 结构和 send/recv/close/select 的全流程，下一步进入 [10.sync同步原语剖析](10.sync同步原语剖析.md)——把 `sync.Mutex`、`sync.RWMutex`、`sync.WaitGroup`、`sync.Once` 的源码和锁竞争优化剖开。
