---
title: 链接器工作原理
date: 2026-06-06 12:33:20
permalink: /pages/a9c637/
categories:
  - book
  - Cpp入门到精通
  - 专栏博客
...
#### 目录介绍
- [1. 案例引入](#1-案例引入)
  - [1.1 链接顺序灾](#11-链接顺序灾libb-在-liba-前面undefined-reference)
  - [1.2 强弱符号重叠](#12-强弱符号重叠)
  - [1.3 七个待解疑问](#13-七个待解疑问)
- [2. 架构概览](#2-架构概览)
  - [2.1 输入与输出](#21-输入与输出)
  - [2.2 为何这么切](#22-为何这么切)
- [3. 符号解析](#3-符号解析链接器的核心搜索算法)
  - [3.1 符号表的结构](#31-符号表的结构)
  - [3.2 未定义符号的搜索路径](#32-未定义符号的搜索路径)
  - [3.3 增量提取](#33-增量提取)
  - [3.4 为什么链接顺序至关重要](#34-为什么链接顺序至关重要)
- [4. 重定位](#4-重定位填上所有空洞的地址)
  - [4.1 重定位表](#41-重定位表)
  - [4.2 两种最常见重定位](#42-r_x86_64_pc32-与-r_x86_64_plt32两种最常见-x86-64-重定位)
  - [4.3 数据段的重定位](#43-数据段的重定位)
  - [4.4 汇编层视角](#44-汇编层视角)
- [5. 强符号与弱符号](#5-强符号与弱符号链接器的冲突仲裁)
  - [5.1 强弱符号规则](#51-强弱符号规则初始化-vs-未初始化-vs-__attribute__weak)
  - [5.2 弱符号的典型用例](#52-弱符号的典型用例)
  - [5.3 COMDAT 与模板](#53-comdat-与模板)
- [6. 静态库 .a](#6-静态库-a归档文件的内在机制)
  - [6.1 .a 文件的结构](#61-a-文件的结构)
  - [6.2 对 .a 的处理方式](#62-对-a-的处理方式)
  - [6.3 循环依赖的解决方案](#63-循环依赖的解决方案---与---start-group)
- [7. --gc-sections](#7--gc-sections垃圾回收未使用的段)
  - [7.1 段级可达性分析](#71-段级可达性分析)
  - [7.2 前置条件](#72--ffunction-sections-与--fdata-sections-的前置条件)
  - [7.3 实际收益](#73-实际收益)
- [8. 常见陷阱与反模式](#8-常见陷阱与反模式)
  - [8.1 静态库顺序陷阱](#81-静态库顺序陷阱)
  - [8.2 同名的全局变量](#82-同名的全局变量)
  - [8.3 inline 函数地址不唯一](#83-inline-函数地址不唯一)
  - [8.4 静态初始化顺序](#84-静态初始化顺序)
- [9. 综合案例串讲](#9-综合案例串讲)
  - [9.1 案例真相揭晓](#91-案例真相揭晓)
  - [9.2 一次完整的链接过程](#92-一次完整的链接过程)
...
> **下一篇**：链接器把符号和段缝合好了——但 ODR 规则可能在缝合过程中埋下定时炸弹。下一篇进入 [51.ODR规则与陷阱](https://yccoding.com/pages/073d0c/)——一次定义规则、inline 变量 C++17、模板与 ODR、跨 TU 的 static 与匿名命名空间——链接器缝合时的 UB 典型场景。
