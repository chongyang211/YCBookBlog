...
> **下一篇**：Modules 把「编译过程」重新设计了。下一篇我们进入卷四 [25.RAII 的设计哲学](https://yccoding.com/pages/4bc5fe/)——从编译期回到运行时，看 C++ 最优雅的设计 RAII 如何用「构造函数获取资源、析构函数释放资源」这一对对称操作，替代 Java 的 `try-finally`、C 的 `goto cleanup`、Go 的 `defer`，成为资源管理的第一性原理。
