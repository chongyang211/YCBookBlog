...
> **下一篇**：[17.模板实例化机制](https://yccoding.com/pages/afb0a5/) 将打开 C++ 编译期的黑箱——**当写下 `vector<int>` 的瞬间，编译器内部发生了什么？两阶段名称查找如何决定一个名字能否通过编译？为什么模板代码要写在头文件里？显式实例化 `extern template` 到底能不能解决二进制膨胀？** 模板——C++ 图灵完备的编译期引擎，从下一篇正式启航。
