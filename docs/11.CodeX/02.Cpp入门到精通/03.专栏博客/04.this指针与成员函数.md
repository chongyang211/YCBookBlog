...
> 📌 **本篇定位**：[03.引用与指针本质](https://yccoding.com/pages/3f4361/) 解决"指向对象的另一个名字"，本篇解决"**对象自己怎么知道自己是谁**"——这是后续 05（虚函数表）、06（多继承）、27（拷贝控制）、30（enable_shared_from_this）的**钥匙**。
...
> **下一篇**：本篇剖开了 this 的真相和成员函数的翻译机制。下一步进入 [05.虚函数表深度剖析](https://yccoding.com/pages/627ad6/)——**vptr 为什么放在对象头部、vtable 在哪一段、单继承 vs 多继承 vs 虚继承下 vtable 的不同布局、构造析构期间 vptr 的切换时机**。本篇说"this 是多态的桥"，下一篇拆桥本身。
