...
本步涉及**两处修改 + 一处 CMake 变更**，按"先声明、再实现、最后接构建"的顺序一步步来。

#### ① 头文件：新建 `command.hpp` 放枚举 + 声明

> 📁 **修改文件**：`include/mkv/command.hpp`（**新建**）
>
> **为什么放头文件？** `enum class CmdType` 是**类型定义**，任何要写 `CmdType::Set` 的 `.cpp` 都必须先看到它；`parseCmdType` 是**函数声明**，让 `main.cpp` 编译时知道"有这个函数"，链接时再去找实现。

```cpp
// include/mkv/command.hpp
#pragma once           // 头文件必备：防止同一编译单元里被 include 两次

#include <string>      // 因为下面函数签名用了 std::string，头文件必须自包含

namespace mkv {

// 命令枚举：所有命令的强类型标识（Step 1.2 第一版，后续小节还会扩充）
enum class CmdType {
    Set, Get, Del, Expire, Ttl, Keys, Save, Exit, Unknown
};

// 【只写声明，不写函数体】把字符串解析成 CmdType；函数体挪去 command.cpp
...
1. **`enum class` 定义必须写头文件**：类型定义天然允许多次 include（同一个 `.cpp` 里 `#pragma once` 挡住，不同 `.cpp` 里因为定义完全一致所以 ODR 兼容）。这和函数定义不同。
2. **加 `namespace mkv`**：全项目所有符号一律进 `mkv` 命名空间，避免和标准库、系统头撞名。**后续所有 `.hpp/.cpp` 都必须保持这个 namespace**。
3. **`parseCmdType` 只写到 `;` 为止**：函数体一旦写在头文件里，只要 ≥ 2 个 `.cpp` include 它，链接就会报 `multiple definition`。

#### ② 实现文件：新建 `command.cpp` 写函数体

> 📁 **修改文件**：`src/command.cpp`（**新建**）
>
> **为什么新建 `.cpp`？** 让整个项目里 `parseCmdType` 的机器码**只有一份**，链接器不会重复定义。

```cpp
// src/command.cpp
#include "mkv/command.hpp"   // 铁律：自己的实现文件先 include 自己的头，验证头自包含

#include <cctype>            // std::toupper

namespace mkv {

// parseCmdType 的真正实现（对应 command.hpp 里的那行声明）
CmdType parseCmdType(const std::string& s) {
    // Step 1：先把输入转成大写，实现"大小写不敏感"
    std::string up;
    up.reserve(s.size());
    for (char c : s) {
        // ⚠️ 必须 static_cast<unsigned char>，否则汉字/负值 char 传给 toupper 是 UB
        up.push_back(static_cast<char>(std::toupper(static_cast<unsigned char>(c))));
    }

    // Step 2：一串 if 分派——为什么不用 unordered_map？因为 8 个命令的常数级线性查找比哈希更快
    ...
    return CmdType::Unknown;   // 兜底：识别不了统一走 Unknown 分支
}
...
1. **函数签名要和头文件里的声明**逐字**一致**（返回类型、参数类型、const、引用都不能差），否则链接报"未定义引用"。
2. **`namespace mkv { }` 必须包住函数体**：写在 namespace 外会变成 `::parseCmdType`，链接找不到 `mkv::parseCmdType`。
3. **`std::toupper` 的入参必须先转 `unsigned char`**：这是 §1.3 就强调的"C ctype 铁律"，只要看到 `toupper/tolower/isspace/isdigit/isalpha` 就要条件反射地想到这一层转换。

#### ③ 更新 CMakeLists：把新 `.cpp` 加进构建

> 📁 **修改文件**：`CMakeLists.txt`

找到 `add_executable(mini_kv ...)`，追加 `src/command.cpp`：

```cmake
add_executable(mini_kv
    src/main.cpp
    src/value.cpp          # §03 已加
    src/command.cpp        # 【本步新增】
)
```

保存后**必须重新 `cmake --build build`**，否则链接会报 `undefined reference to 'mkv::parseCmdType'`。
...
工程级做法是用 **`std::string_view`**（C++17）做"零拷贝切片"。**本步和 Step 1.2 结构完全一致**：一行声明放头文件、函数体放 `src/command.cpp`，不用改 CMake（`command.cpp` 上一步已经加过）。

#### ① 头文件：加声明

> 📁 **修改文件**：`include/mkv/command.hpp`
>
> 在 `parseCmdType` 声明**下面**追加：

```cpp
// include/mkv/command.hpp（节选）
#include <string_view>       // 【本步新增】std::string_view，签名用了就必须包
#include <vector>            // 【本步新增】std::vector

namespace mkv {

// ... enum class CmdType ...
// ... CmdType parseCmdType(const std::string& s);（Step 1.2）...

// 【本步新增】把一行命令切成多个 string_view（零拷贝），指向输入字符串的底层 buffer
...
1. **`<string_view>` 和 `<vector>` 两个头都要加**：函数签名里出现了什么类型，头文件就必须**自己 include 它**（自包含原则）。
2. **参数按值传 `string_view` 而不是 `const&`**：`string_view` 本身只有 16 字节（指针 + 长度），值传比引用传更快、更符合标准库风格。

#### ② 实现文件：写函数体

> 📁 **修改文件**：`src/command.cpp`（Step 1.2 新建的那个，往下追加）
>
> 在 `parseCmdType` 函数体**下方**、`}  // namespace mkv` **上方**追加：

```cpp
// src/command.cpp（继续追加）
// ⚠️ 顶部的 #include "mkv/command.hpp" 和 <cctype> 已由 Step 1.2 添加，不用重复
#include <string_view>
#include <vector>

namespace mkv {

// ... parseCmdType 的实现（上一步已写） ...

// tokenize 的真正实现（对应 command.hpp 里的那行声明）
std::vector<std::string_view> tokenize(std::string_view line) {
...
1. **`std::isspace` 前必须 `static_cast<unsigned char>`**：老坑重犯，`char` 在部分平台是 signed，负值传给 ctype 系列是 UB。
2. **切出来的 `string_view` 只在 `line` 生命周期内有效**：调用方一旦让 `line` 对应的 `std::string` 析构，`tokenize` 的返回值就变成悬垂指针。后面 REPL 循环里 `line` 每轮都还在栈上，所以安全。
3. **`out.emplace_back(...)` 而不是 `push_back`**：`emplace_back` 直接原地构造 `string_view`，少一次拷贝——对性能敏感入口路径要养成这个反射。

#### ③ CMakeLists 不用改

`src/command.cpp` 上一步已加过，本步只是在同一个 `.cpp` 里追加函数体，CMake 无需感知。`cmake --build build` 即可。
...
本步涉及**两个文件的两处修改**，我们按"先声明再实现"的顺序一步步来。

#### ① 头文件：加声明

> 📁 **修改文件**：`include/mkv/value.hpp`
>
> **为什么放头文件？** 因为 `formatValue` 要被 `main.cpp`、后续的 `command.cpp` 调用，**调用方必须先看到函数原型**才能编译通过。头文件的作用就是"把函数长什么样告诉全世界"。

在 Step 2.1 加好的 5 个 `isXxx` 函数**下面**、`}  // namespace mkv` **上面**，加**一行**声明：

```cpp
// include/mkv/value.hpp（节选）
namespace mkv {

// ... 已有的 Null / Value / 5 个 isXxx ...

// 【本步新增】把 Value 转成可读字符串（用于 GET 命令返回）
...
⚠️ **关键点**：**没有 `inline` 的普通函数**如果**连函数体一起写在头文件里**，只要有 ≥ 2 个 `.cpp` 都 `#include "value.hpp"`，链接器就会报 **"multiple definition of `mkv::formatValue`"**。所以此处**只写声明（以 `;` 结尾），函数体挪去 `.cpp`**。

**对比记忆**：Step 2.1 的 5 个 `isXxx` 加了 `inline` 才能在头文件里直接写函数体；本步的 `formatValue` **没加** `inline`，就**必须**声明和实现分家。这就是卷一第 21 章讲的"内联函数 vs 声明/实现分离"的实战应用。

#### ② 实现文件：写函数体

> 📁 **修改文件**：`src/value.cpp`（新建文件！）
>
> **为什么新建 `.cpp`？** 每个 `.cpp` 都是一个"翻译单元"，编译成一个 `.o`。函数体放这里，**整个项目里 `formatValue` 只有一份机器码**，链接器不会报重复定义。

在 `src/` 目录下**新建** `value.cpp`，内容如下：

```cpp
// src/value.cpp
#include "mkv/value.hpp"   // 先包自己的头，确保头文件自洽（能独立编译）
...
1. **第一行必须 `#include "mkv/value.hpp"`**：让 `Value` / `Null` 类型可见，且**"自己的实现文件先包自己的头文件"**是行业铁律——一旦头文件缺了某个 include 会立刻在这里暴露，防止"污染式包含"。
2. **`namespace mkv { }` 要和头文件保持一致**：函数体必须写在**同一个 namespace 内**，否则链接时找不到 `mkv::formatValue`（会被当成全局的 `::formatValue`）。
3. **函数签名要和头文件里的声明**逐字**一致**（返回类型、参数类型、`const`、引用都不能差）——否则编译器会认为你在定义另一个函数，链接时又会报"未定义引用"。
4. **`if constexpr` 的 5 个分支要覆盖 `Value` 的全部 5 种类型**——一旦漏一种，编译器警告"not all control paths return a value"，或运行时抛 `std::bad_variant_access`。

#### ③ 更新 CMakeLists：把新 `.cpp` 加进构建

> 📁 **修改文件**：`CMakeLists.txt`
>
> **为什么改 CMake？** 新建的 `src/value.cpp` 不会自动被编译，必须在构建脚本里显式声明"这个 `.cpp` 属于哪个 target"。

找到 `add_executable(mini_kv ...)` 这一行，把 `src/value.cpp` **追加**进去：
...
保存后**必须重新 `cmake --build build`**，不然编译器还按旧列表在编译，会报"undefined reference to `mkv::formatValue`"。

> 💡 **一图看懂三处修改的关系**：
>
> ```text
> include/mkv/value.hpp        ── 声明：告诉调用方"有这个函数"
>            ↑ #include
>            │
> src/main.cpp / src/xxx.cpp  ── 调用：编译时只需看到声明
>
> src/value.cpp                ── 实现：函数体唯一一份，链接时提供符号
>            ↑ 由
> CMakeLists.txt               ── 构建：把 value.cpp 加入编译列表
> ```
...
**本步和 Step 2.2 结构完全一致**：一句声明放头文件、函数体放 `.cpp`，不再改 CMake（因为 `src/value.cpp` 上一步已经加过了）。

#### ① 头文件：加声明

> 📁 **修改文件**：`include/mkv/value.hpp`
>
> 在 `formatValue` 声明的**下面**再补一行：

```cpp
// include/mkv/value.hpp（节选）
#include <string_view>       // 【本步新增头】std::string_view

namespace mkv {

// ... 已有的 Null / Value / isXxx / formatValue 声明 ...

// 【本步新增】按"长得像什么"从字符串 token 构造 Value
...
1. **`<string_view>` 头必须加**：只要函数签名里出现 `std::string_view`，头文件就必须自己 include 它——**头文件永远不能依赖调用方帮它 include**（这叫"自包含原则 self-contained header"）。
2. **不要写成 `const std::string_view&`**：`string_view` 本身只有 16 字节（指针 + 长度），按值传比按引用传更快，也更符合标准库风格。这是卷一第 18 章的重点，**能按值传的小对象绝不加引用**。

#### ② 实现文件：写函数体

> 📁 **修改文件**：`src/value.cpp`（Step 2.2 新建的那个，接着往里加）
>
> 在 `formatValue` 函数体**下方**、`}  // namespace mkv` **上方**追加：

```cpp
// src/value.cpp（继续追加）
#include <cctype>       // 【本步新增头】std::isdigit
#include <stdexcept>    // 【本步新增头】stoll/stod 抛的异常基类

namespace mkv {

// ... formatValue 的实现（上一步已写） ...
...
Value valueFromToken(std::string_view s) {
    ...
    if (!s.empty()) {
        bool allDigit = true;
        size_t start = (s[0] == '-') ? 1 : 0;   // 允许开头一个负号
        for (size_t i = start; i < s.size(); ++i) {
            // 必须 static_cast<unsigned char>，否则汉字字节会 UB！
...
1. **返回类型写 `Value` 而不是 `mkv::Value`**：因为整个函数体已经**在 `namespace mkv { }` 内**，写 `mkv::Value` 虽然能编过，但显得多余；同理 `Null{}` 不用写成 `mkv::Null{}`。
2. **`static_cast<unsigned char>` 不能省**：`char` 在有些平台默认是 signed，汉字字节值为负数，直接传给 `std::isdigit` 是 **UB（未定义行为）**——这条已经在 §1.3 说过，此处是它第二次出现，务必刻进 DNA。
3. **`std::stoll(std::string(s))` 的一次拷贝无法避免**：因为 `stoll` 的重载**不支持 `string_view`**（C++26 才有 `from_chars` 的 view 版）。类型推断是低频路径，一次拷贝完全值得。
4. **`catch (...)` 吞异常是刻意的**：`stoll` 遇到 `"999999999999999999999"`（溢出）会抛 `out_of_range`，我们希望的行为是"降级为字符串"，而不是让整个程序崩溃。**吞异常在这里是业务语义，不是偷懒**。
5. **分支顺序不能换**：`"3.14"` 会先被"分支 2 全数字"拒掉（含 `.`），再进入"分支 3 double"；如果把"分支 3 double"放前面，`"42"` 会被误判成 `42.0` 变成 double。

#### ③ 是否要改 CMakeLists？

**不用！** Step 2.2 已经把 `src/value.cpp` 加进 target 了，本步只是往同一个文件追加函数体，CMake 不需要感知。**只要重新 `cmake --build build` 即可**。

> 💡 **验证"文件拆分正确"的一个小技巧**：单独编译一次 `value.cpp` 看看能否通过：
>
> ```bash
> g++ -std=c++17 -Iinclude -c src/value.cpp -o /tmp/value.o
> ```
>
> 如果这一步就报错（比如缺 include、找不到 `Value` 类型），说明 `value.hpp` **不是自包含的**——头文件必须能"独立编译"，否则以后换个调用方就崩。这是行业级 C++ 项目的隐形门槛。
...
把"字符串 tokens → `unique_ptr<Command>`" 的逻辑收敛到一个工厂函数。**本步是"多个文件协同"的典型例子**，头文件加声明、`.cpp` 加实现、异常类型临时占位——分 3 个小节讲清楚。

#### ① 头文件：加声明 + 临时异常类

> 📁 **修改文件**：`include/mkv/command.hpp`
>
> 在派生类声明**下方**、`}  // namespace mkv` **上方**追加：

```cpp
// include/mkv/command.hpp（节选）
#include <memory>       // 【本步新增】std::unique_ptr
#include <stdexcept>    // 【本步新增】std::runtime_error（临时异常基类）

namespace mkv {

// ... 已有的 CmdType / parseCmdType / tokenize / Command 抽象基类 / 4 个派生类 ...

// 【本步新增·临时占位】命令语法错误异常
// 注意：§07 建立完整异常体系后，本类会被"搬家"到 include/mkv/errors.hpp
//       并改为继承 KvError。当前先用 std::runtime_error 顶着。
class CmdSyntaxError : public std::runtime_error {
public:
    using std::runtime_error::runtime_error;   // 继承构造函数
};

// 【本步新增】把切好的 tokens 组装成具体的 Command 派生类实例
// [[nodiscard]] 提醒调用方"返回值必须接住"，防止 make 完 unique_ptr 立即销毁
[[nodiscard]]
...
1. **`CmdSyntaxError` 定义整个放头文件是刻意的**：因为它只有一个 `using std::runtime_error::runtime_error;`（继承构造，编译器自动生成，不产生独立函数体），**不会引起 ODR 冲突**。这和普通函数不同。
2. **`[[nodiscard]]` 只写在声明上**：C++ 规定这个 attribute 写在声明处即生效，函数定义处再写一次是**冗余的**。
3. **`<memory>` 头必须加**：一旦签名里有 `std::unique_ptr`，头文件就必须自己 include。

#### ② 实现文件：写函数体

> 📁 **修改文件**：`src/command.cpp`（沿用 Step 1.2 新建的那个，继续追加）
>
> 在文件**末尾** `}  // namespace mkv` **上方**追加：

```cpp
// src/command.cpp（继续追加）
#include "mkv/value.hpp"    // 【本步新增】需要 valueFromToken
#include <string>           // std::string / std::to_string

namespace mkv {

// ... parseCmdType / tokenize 的实现（前面已写） ...
...
std::unique_ptr<Command> makeCommand(const std::vector<std::string_view>& tokens) {
    ...
    // 分派：先解析命令类型，再按类型 make_unique 具体派生类
    switch (parseCmdType(std::string(tokens[0]))) {
        ...
        case CmdType::Expire: {
            needArgs(2, "EXPIRE");
            // stoi 失败会抛 std::invalid_argument，让它自然向上传播即可
...
1. **`switch` 必须有 `default`**：即使前面 4 个 case 已经处理，`parseCmdType` 还会返回 `Unknown/Ttl/Keys/Save/Exit`，不写 default 编译器会警告 `-Wswitch`，运行时也会漏掉。
2. **`{}` 花括号包住 case 体**：因为里面声明了 `int sec`、`needArgs` 用到局部变量。**没花括号会报 "crosses initialization"**——case 标签跨越变量初始化在 C++ 是错误。
3. **`std::string(tokens[2])` 的拷贝无法避免**：`stoi` 只吃 `std::string`，不吃 `string_view`（C++17-23）。类型转换是低频路径，一次拷贝可接受。
4. **`make_unique` 而不是 `new`**：整本卷一第 12 章反复强调，**任何 `new` 都应该被 `make_unique/make_shared` 替换**，异常安全 + 少一次显式 delete。

#### ③ CMakeLists 不用改

`src/command.cpp` 已在 Step 1.2 加进 target，本步只追加符号。

> 💡 **符号依赖关系图**（帮你理解为什么必须这么拆）：
>
> ```text
>                        command.hpp
>              ┌────────────┴────────────┐
...
到这一步 `store.hpp` 里 7 个方法都只有声明没有函数体，编译能过但链接不了（`undefined reference to Store::set` 等）。本步**新建 `src/store.cpp` 补齐所有函数体**。

#### ① 头文件不动

上一步的 `include/mkv/store.hpp` 已经写完全部声明，本步**不改头文件**——这就是"接口/实现分离"的意义：加实现不必碰头，调用方无感。

#### ② 实现文件：新建 `src/store.cpp`

> 📁 **修改文件**：`src/store.cpp`（**新建**）
>
> **为什么必须放 `.cpp`？** 7 个成员函数**都没有 `inline`**，函数体一旦写在头文件里，只要 ≥ 2 个 `.cpp` include `store.hpp` 就会链接报 `multiple definition of 'mkv::Store::set'`。放独立 `.cpp` 让整个项目里每个函数只有**一份机器码**。

```cpp
// src/store.cpp
#include "mkv/store.hpp"    // 铁律：先 include 自己的头，验证头自包含

#include <chrono>           // steady_clock、duration_cast、seconds
...
// ─── 读方法 7：size ─────────────────────────────
// noexcept：调用方可放心用在析构、swap、异常安全的移动构造里
std::size_t Store::size() const noexcept { return idx_.size(); }

// ─── 维护方法：purgeExpired ─────────────────────
// 由后台 jthread 定期调用（§11）；这里用"erase 返回下一个迭代器"的正确写法
// ⚠️ Step 5.3 会亲手把这里写错一次感受迭代器失效的坑，然后再改回来
...
1. **每个函数名前必须写 `Store::`**：否则会被认为是全局函数 `mkv::set`，链接找不到 `mkv::Store::set` 的定义。**类外定义成员函数忘写类名前缀**是新手最常见的错。
2. **`const` 修饰位置要和头文件完全一致**：`EntryPtr get(const std::string& key) const;` 声明里有两个 `const`，实现里也必须两个都有——**尾部 `const` 表示"不改类成员"，是签名的一部分**。少一个就变成"另一个函数"，链接报错。
3. **`noexcept` 也是签名的一部分（自 C++17）**：`size() const noexcept` 的声明和实现都必须带 `noexcept`。
4. **`std::move(v)` 后不能再用 `v`**：move 之后 `v` 处于"合法但未定义"状态，只能重新赋值或析构。这个规则整本卷一第 12 章反复讲。
5. **用 `steady_clock` 而不是 `system_clock`**：`system_clock` 会跟随用户改系统时间跳变，导致 TTL 时间乱掉；`steady_clock` 严格单调，是所有"超时/间隔"场景的唯一正确选择。

#### ③ 更新 CMakeLists：把 `src/store.cpp` 加进构建
...
保存后 `cmake --build build`。**如果这一步链接报 "undefined reference to Store::set"，99% 的原因是忘了把 `src/store.cpp` 加进 `add_executable`**。

> 💡 **验证头自包含小技巧**：单独编 `store.cpp` 看能否通过：
>
> ```bash
> g++ -std=c++17 -Iinclude -c src/store.cpp -o /tmp/store.o
> ```
>
> 一旦这里编译失败（比如缺 `<chrono>`），说明 `store.hpp` 或者 `entry.hpp` 里有 include 遗漏，应该趁早补上。
...
回到 §05 Step 4.3 留的"`execute` 见下文"，现在把 4 个派生类的虚函数补齐。

#### ① 头文件不动

派生类声明里 `std::string execute(Store& store) override;` 早已在 §05 Step 4.3 写好，本步只补函数体，**不需要动 `command.hpp`**。

⚠️ 但要确认 `command.hpp` 里对 `Store` 有**前置声明**：

```cpp
// include/mkv/command.hpp（片段·核对用）
namespace mkv {
class Store;    // 【关键】前置声明，避免 command.hpp include store.hpp 造成循环
// ...
class SetCmd : public Command {
public:
    std::string execute(Store& store) override;   // 只用 Store&，前置声明就够
    // ...
};
}
```

**为什么用前置声明而不是 `#include "mkv/store.hpp"`？** 因为**引用/指针只需要"知道有这个类型"，不需要"知道类内部字段"**。前置声明能打破 command 与 store 之间的循环 include（store 后面又会依赖 command 做 replay），是解决循环依赖的经典手段。

#### ② 实现文件：追加到 `src/command.cpp`

> 📁 **修改文件**：`src/command.cpp`（沿用之前那个）
>
> **为什么必须放 `.cpp`？** `execute` 里要真正调 `store.set(...)`、`store.get(...)`，编译器**必须看到 Store 的完整定义**（字段、方法签名），也就是必须 `#include "mkv/store.hpp"`。这个 include 只在 `.cpp` 里做，不污染头文件。
>
> 在文件末尾 `}  // namespace mkv` **上方**追加：

```cpp
// src/command.cpp（继续追加）
#include "mkv/store.hpp"      // 【本步新增】需要 Store 的完整定义才能调它的方法
#include "mkv/entry.hpp"      // 需要 Entry::value 字段
#include "mkv/value.hpp"      // formatValue

namespace mkv {

// ─── SetCmd::execute ──────────────────────────
// 直接调 Store::set；写命令一律返回 "OK"
std::string SetCmd::execute(Store& store) {
    store.set(key_, value_);   // key_/value_ 是派生类的 private 字段（Step 4.3 定义）
    return "OK";
}

// ─── GetCmd::execute ──────────────────────────
// key 不存在或已过期 → "(nil)"；否则调用 formatValue 格式化
std::string GetCmd::execute(Store& store) {
    auto entry = store.get(key_);       // EntryPtr = shared_ptr<Entry>
    if (!entry) return "(nil)";
    return formatValue(entry->value);   // 复用 §03 Step 2.2 写好的格式化函数
}

// ─── DelCmd::execute ──────────────────────────
// 返回 "(integer) 1" 或 "(integer) 0"（Redis 风格）
...
1. **`SetCmd::execute` 而不是 `execute`**：必须写类名限定，否则被当成自由函数。
2. **`override` 关键字只写在头文件的声明处**：`.cpp` 的定义处**不能再写** `override`，会编译报错。
3. **返回类型必须和头文件声明**完全一致**为 `std::string`**（不是 `string` 也不是 `const std::string&`），签名任何差异都会让链接器认为是不同函数。
4. **`command.cpp` 里必须 include `store.hpp`**：只 include `command.hpp` 会因为里面只有前置声明 `class Store;`，无法解引用 `store.set(...)`——编译报 "invalid use of incomplete type"。

#### ③ CMakeLists 不用改

`src/command.cpp` 已在 target 里，本步只是追加符号。

> 💡 **循环依赖的解法总结**：
>
> | 场景 | 解法 |
> |------|------|
> | 头文件里只用类型的**引用/指针** | 前置声明 `class Store;` |
> | 头文件里用类型的**完整定义**（字段、内联方法） | 必须 `#include "mkv/store.hpp"` |
> | 实现文件（`.cpp`）里要调方法 | 必须 `#include` 完整头 |
>
> 本项目 `command.hpp` 属于第 1 类，`command.cpp` 属于第 3 类，天然规避循环。
...
#### 局部替换：不改声明，只改 `.cpp` 里的一个 case 分支

> 📁 **修改文件**：`src/command.cpp`
>
> `makeCommand` 的函数签名不变、`command.hpp` 完全不动。只是把 §05 Step 4.4 写好的 `case CmdType::Expire` 那一段**原地替换**，加上 try-catch 把 `std::invalid_argument` / `std::out_of_range` 翻译成自家的 `CmdSyntaxError`。

```cpp
// src/command.cpp（片段：makeCommand 函数体内部）
// ↓↓ 找到这个 case，把整段替换为下面版本 ↓↓
case CmdType::Expire: {
    needArgs(2, "EXPIRE");
    int sec = 0;
    try {
        // stoi 遇到非数字 → invalid_argument；数值超出 int → out_of_range
        sec = std::stoi(std::string(tokens[2]));
    } catch (const std::exception& e) {
        // 【关键】翻译成自家异常，附加原始输入方便排查
        throw CmdSyntaxError("EXPIRE seconds must be integer, got: "
                             + std::string(tokens[2]));
    }
    if (sec < 0) {
        throw CmdSyntaxError("EXPIRE seconds must be non-negative");
    }
    return std::make_unique<ExpireCmd>(std::string(tokens[1]), sec);
}
```
...
1. **必须 catch `std::exception`（基类）而不是单 catch `std::invalid_argument`**：`stoi` 有可能抛 `out_of_range`（如 `EXPIRE foo 9999999999999999999`），单捕一种会漏。用基类一网打尽。
2. **catch 之后 `throw CmdSyntaxError(...)` 是"异常翻译"**：把标准库异常翻译成项目自家异常体系（§07 建立的 `KvError` 树），让上层只需 `catch (const KvError&)` 一次即可兜住全部。
3. **`sec < 0` 的检查不能省**：`stoi("-100")` 成功返回 -100，但业务上负数 TTL 无意义。fail-fast 就是要在最浅位置拦。

#### 头文件不动、CMakeLists 不动
只改 `src/command.cpp` 里一个函数体内部的一段。
...
把 AOF 写盘封装成一个类，遵守 RAII。本步是**"类的声明 vs 类的实现分离"**的完整示范，分 3 小节。

#### ① 头文件：新建 `aof.hpp`，写 AofWriter 类声明

> 📁 **修改文件**：`include/mkv/aof.hpp`（**新建**）
>
> **为什么类声明放头文件？** 任何要 `new AofWriter` 或 `AofWriter aof(...)` 的 `.cpp` 都必须先看到类的**完整定义**——字段布局（决定 `sizeof(AofWriter)`）、方法签名（决定调用姿势）都必须可见。

```cpp
// include/mkv/aof.hpp
#pragma once

#include <filesystem>       // fs::path 是类字段类型，必须完整定义
#include <fstream>          // std::ofstream 同理
#include <string_view>      // append 参数用
#include <cstddef>          // std::size_t（后续 ReplayReport 用）

namespace mkv {

namespace fs = std::filesystem;   // 命名空间别名，全项目复用

// 前置声明：replayAof 用得到，但这里 AofWriter 不依赖它
class Store;

class AofWriter {
public:
    // 【只写签名，函数体挪去 aof.cpp】explicit 防止 AofWriter x = "..." 的隐式转换
    explicit AofWriter(const fs::path& path);

    void append(std::string_view line);   // 追加一条 AOF 记录（含 \n）
    void flush();                         // 显式冲刷（由 §14.2 fsync 策略决定何时调）

    // RAII：ofstream 析构时自动关文件；用 = default 即可
    ~AofWriter() = default;
...
1. **`<fstream>` 和 `<filesystem>` 必须 include**：`std::ofstream out_;` 是**类的成员字段**，编译器要算出 `sizeof(AofWriter)` 必须看到 ofstream 的完整定义。这和"函数参数用引用可以前置声明"不一样。
2. **构造函数写 `explicit`**：防止 `AofWriter x = "data/aof.log";` 这种隐式转换（把字符串偷偷转成 `fs::path` 再构造）。RAII 类基本都要 `explicit`。
3. **禁掉拷贝构造和拷贝赋值**：文件句柄拷贝没意义（复制一份句柄双写同一个文件），拷不动就直接 `= delete`。
4. **析构 `= default` 就够**：因为 `std::ofstream` 自己的析构会关文件，成员销毁自动触发，我们不用写任何清理代码。

#### ② 实现文件：新建 `src/aof.cpp`

> 📁 **修改文件**：`src/aof.cpp`（**新建**）
>
> **为什么放 `.cpp`？** 构造函数体里有分支逻辑 + 异常抛出 + `fs::create_directories` 调用，全放头会（a）拖慢所有 include `aof.hpp` 的编译；（b）触发多重定义。

```cpp
// src/aof.cpp
#include "mkv/aof.hpp"
#include "mkv/errors.hpp"    // IoError（§07 定义的异常类型）
...
void AofWriter::append(std::string_view line) {
    ...
    // 用 write 而不是 <<，避免 << 处理转义或宽字符
    out_.write(line.data(), static_cast<std::streamsize>(line.size()));
    if (!out_) throw IoError("write AOF failed");   // 磁盘满 / 权限丢失都会走到这里
}

// ─── flush：把内核 buffer 里的字节交给 OS ────────
// 注意 flush ≠ fsync：flush 只把用户态 buffer 交给内核，掉电还是可能丢。
// 真正的 fsync 需要 open() + fd + ::fsync()，超出本项目范围（见 §14.2 讨论）。
...
1. **构造函数用初始化列表 `: path_(path)`**：字段初始化要走**初始化列表**，写到函数体里 `path_ = path;` 是"先默认构造再赋值"，多一次拷贝且对 `const` 字段不可用。
2. **`fs::create_directories` 的 `error_code` 重载不抛异常**：所以要**自己检查 `ec` 后抛**。如果误用不带 `ec` 的重载，`create_directories` 自己抛的是 `fs::filesystem_error`，跟本项目的 `IoError` 体系不统一。
3. **`std::ios::app`、`binary`、`out` 三个 flag 用 `|` 组合**：漏 `app` → 每次启动清空文件（灾难）；漏 `binary` → Windows 下 AOF 行尾错乱；漏 `out` → 有些实现拒绝写入。
...
**严格版实现**：任何一行损坏就抛异常拒绝启动。分声明和实现两处修改。

#### ① 头文件：追加 `replayAof` 声明

> 📁 **修改文件**：`include/mkv/aof.hpp`
>
> 在 `AofWriter` 类**下方**、`}  // namespace mkv` **上方**追加：

```cpp
// include/mkv/aof.hpp（节选）
namespace mkv {

// ... class AofWriter ...

// 【本步新增·严格版】重放 AOF 文件，把每一行调用 makeCommand + execute 应用到 store
// 坏行直接抛 AofCorrupted 中止启动（§08 Step 8.5 会重构为可容错的宽松版）
void replayAof(const fs::path& path, Store& store);

}  // namespace mkv
```

⚠️ **易错点**：函数签名里出现了 `Store&`，`aof.hpp` 顶部**已经有前置声明 `class Store;`**（Step 8.2 加过），所以不需要再 `#include "mkv/store.hpp"`——引用类型只需前置声明。如果这里改成 `Store` 值传或 `Store` 字段，就必须 include 完整头。

#### ② 实现文件：追加到 `src/aof.cpp`
...
```cpp
// src/aof.cpp（继续追加）
#include "mkv/store.hpp"     // 【本步新增】需要 Store 完整定义才能调 store.set 等
#include "mkv/command.hpp"   // 【本步新增】tokenize / makeCommand / Command
...
// ... AofWriter 三个方法的实现（前面已写） ...

void replayAof(const fs::path& path, Store& store) {
    // Step 1：全新启动（文件不存在）直接返回，不是错误
    if (!fs::exists(path)) return;

    // Step 2：打开文件；默认文本模式即可（getline 会正确处理 \n）
    std::ifstream in(path);
    if (!in) throw IoError("open AOF for replay failed");

    // Step 3：逐行读、逐行 tokenize + makeCommand + execute
    std::string line;
    std::size_t lineNo = 0;
    while (std::getline(in, line)) {
        ...
        try {
            ...
            auto cmd    = makeCommand(tokens);
            if (!cmd->isWrite()) {
                // GET/TTL/KEYS 等读命令本不该出现在 AOF 里，忽略即可
                continue;
            }
            (void)cmd->execute(store);    // 重放时丢弃返回值
        } catch (const KvError& e) {
            // 严格版：一行损坏立刻抛 AofCorrupted 中止启动
            // Step 8.5 会重构为"记日志 + 跳过 + 继续"的宽松版
...
1. **`(void)cmd->execute(store)` 的 `(void)` 是刻意的**：因为 `execute` 有 `[[nodiscard]]` 标注，忽略返回值会警告，用 `(void)` 显式表达"我知道我丢弃返回值"。
2. **`std::getline(in, line)` 本身会读到 EOF 自动结束**：不要写成 `while (in)` 或 `while (!in.eof())`——`eof()` 只有读到 EOF 后**再一次读**才会 true，容易多进一轮循环。
3. **`aof.cpp` 里 include `store.hpp` 是安全的**：只要不让 `store.hpp` 反过来 include `aof.hpp` 就行。方向永远是"高层依赖低层"：main → aof → store → entry → value。

#### ③ CMakeLists 不用改

`src/aof.cpp` 上一步已加进 target。
...
重构 `replayAof`，加一个 `tolerant` 参数控制严格 / 宽松，默认宽松。**这是最终版，把 Step 8.3 的严格版整个替换掉**。分 3 处修改。

#### ① 头文件：加 `ReplayReport` + 改 `replayAof` 签名

> 📁 **修改文件**：`include/mkv/aof.hpp`
>
> **把 Step 8.3 的那行 `void replayAof(...)` 替换成下面的结构 + 新签名**：
...
// 【本步新增】重放结果的统计信息（普通聚合类型，头文件直接完整定义）
struct ReplayReport {
...
1. **`struct ReplayReport` 整体放头文件是刻意的**：**聚合类型（只有数据字段，没有非 trivial 构造/析构）允许在多个 `.cpp` 里被 include，只要定义完全一致（ODR 允许）**。这和函数不同。
2. **默认参数只写在声明处**：C++ 规定默认参数只能在**一处**声明——按惯例写在头文件，实现里**不再写** `= true`，否则编译报重复默认参数。
3. **必须删掉 Step 8.3 的旧声明**：不然会有两个 `replayAof` 的声明冲突（一个返回 void 一个返回 ReplayReport，签名不兼容）。

#### ② 实现文件：替换 `src/aof.cpp` 里的旧函数体

> 📁 **修改文件**：`src/aof.cpp`
>
> 把 Step 8.3 那个 `void replayAof(...)` 的**整个函数体删掉**，替换成：

```cpp
// src/aof.cpp（替换 Step 8.3 的严格版实现）
#include <iostream>    // 【本步新增】std::cerr（宽松模式下告警用）
...
1. **实现里的 `bool tolerant`**没有** `= true`**——默认值只写声明里。
2. **`ReplayReport r{};` 大括号初始化**：确保 3 个 `std::size_t` 都被初始化为 0；写成 `ReplayReport r;` 只是默认初始化（对 POD 就是未定义值）。
...
最朴素的修复是给 Store 加一把互斥锁，每个公开方法在最开始上锁。**这是过渡版本，Step 10.3 会再升级为 `shared_mutex`**。分两处修改。

#### ① 头文件：加一个字段

> 📁 **修改文件**：`include/mkv/store.hpp`
>
> 只需在 `private:` 段落**增加一行**，其他一切不动：

```cpp
// include/mkv/store.hpp（节选）
#include <mutex>          // 【本步新增】std::mutex
...
private:
    mutable std::mutex mu_;      // 【本步新增】互斥锁
    IndexMap idx_;
};
...
1. **`mutable` 关键字必不可少**：`std::mutex::lock()` 不是 `const` 函数，但 `Store::get() const` 是 `const` 方法，`const` 方法里默认不能改成员。加 `mutable` 就是给锁开的后门——"逻辑上 const、物理上要修改"。**忘 mutable 会编译报 "cannot lock mutex in const method"**。
2. **`<mutex>` 头必须加**：字段类型出现在头文件里就必须自包含。
3. **方法签名一个都不改**：所有 `set/get/del/ttl/keys/size/expire/purgeExpired` 的**声明保持原样**，加锁纯粹是内部实现细节，对调用方不可见。这是"实现细节不该泄漏到接口"的经典体现。

#### ② 实现文件：每个方法首行加锁

> 📁 **修改文件**：`src/store.cpp`
>
> 对**每个公开方法**（`set/get/del/expire/ttl/keys/size/purgeExpired`）的**函数体第一行**加上：

```cpp
// src/store.cpp（示例：show 前后对比，每个函数都这么改）

// 写方法示例
void Store::set(const std::string& key, Value v) {
    std::lock_guard<std::mutex> lk(mu_);   // 【本步新增·首行】RAII 锁
    // ↓↓ 以下逻辑一字不改 ↓↓
    auto it = idx_.find(key);
    if (it == idx_.end()) {
        auto entry   = std::make_shared<Entry>();
        ...
        it->second->value    = std::move(v);
        it->second->version += 1;
        it->second->expireAt.reset();
    }
}

// 读方法示例（const 方法也一样加）
EntryPtr Store::get(const std::string& key) const {
    std::lock_guard<std::mutex> lk(mu_);   // 【本步新增·首行】
    auto it = idx_.find(key);
    if (it == idx_.end()) return nullptr;
...
// 其余 6 个方法同理：第一行都加 std::lock_guard<std::mutex> lk(mu_);
```

**这段改动 4 个易错点**：

1. **`std::lock_guard` 是 RAII 锁**：构造 lock、析构 unlock，中间无论 `return` / `throw` 一定会 unlock。**永远不要写 `mu_.lock() / mu_.unlock()` 手动配对**，出异常绝对忘释放。
2. **每个公开方法都要加**：漏一个（比如 `size()`）就有数据竞争。**面试考点**：给你一个 6 方法的类，能否一眼看出"哪个方法漏了锁"？
3. **锁必须放在函数体第一行**：如果先读再上锁，等于没锁；如果访问完再上锁，等于表演。
4. **不能在锁内递归调用自己类的其他 public 方法**：比如 `set` 里锁完后调 `this->purgeExpired()`，两个方法都尝试锁 `mu_`，`std::mutex` 不可重入 → **死锁**。要么锁前调用，要么把公共逻辑抽成 private 无锁函数。
...
KV 存储的真实负载是**读多写少**（GET:SET 通常 10:1 以上）。`std::mutex` 让所有读也互斥就太亏，其实多个 GET 同时跑没有任何冲突。`std::shared_mutex`（C++17）允许"多读 / 单写"。**这是 Store 的最终版本**，把 Step 10.2 的 `std::mutex` 全部替换掉。分两处修改。

#### ① 头文件：字段类型换成 `shared_mutex`

> 📁 **修改文件**：`include/mkv/store.hpp`
>
> 把 Step 10.2 加的 `#include <mutex>` 和 `mutable std::mutex mu_;` **一起替换**：

```cpp
// include/mkv/store.hpp（节选）
#include <shared_mutex>       // 【替换 Step 10.2 的 <mutex>】
...
private:
    mutable std::shared_mutex mu_;    // 【替换 Step 10.2 的 std::mutex】
    IndexMap idx_;
};
...
⚠️ **易错点**：`<shared_mutex>` 是 C++17 才有的头文件。**必须在 `CMakeLists.txt` 里 `set(CMAKE_CXX_STANDARD 17)` 或更高**，否则会报 "shared_mutex is not a member of std"。

#### ② 实现文件：每个方法首行换成对应的锁

> 📁 **修改文件**：`src/store.cpp`
>
> 按"读方法用共享锁、写方法用独占锁"把 Step 10.2 加的所有 `std::lock_guard<std::mutex> lk(mu_);` 一并替换：

**📋 读/写方法分类速查表**：

| 类别 | 方法 | 锁类型 |
|------|------|--------|
| **写** | `set` / `del` / `expire` / `purgeExpired` | `std::unique_lock lk(mu_);` |
| **读** | `get` / `ttl` / `keys` / `size` | `std::shared_lock lk(mu_);` |

具体改法（示例三个方法，其余按表格照做）：

```cpp
// src/store.cpp（把 Step 10.2 的 lock_guard 全替换）

// 写方法：独占锁
void Store::set(const std::string& key, Value v) {
    std::unique_lock lk(mu_);         // 【替换 lock_guard】写锁独占
    // ↓↓ 函数体其他部分保持不变 ↓↓
    // ...
}
...
// 读方法：共享锁
EntryPtr Store::get(const std::string& key) const {
    std::shared_lock lk(mu_);         // 【替换 lock_guard】读锁可共享
    // ...
}

// size：读方法但要保留 noexcept
std::size_t Store::size() const noexcept {
    std::shared_lock lk(mu_);         // 【替换 lock_guard】
    return idx_.size();
}
```
...
1. **`std::unique_lock lk(mu_)` 用了 C++17 CTAD**（类模板参数推导），可以不写 `<std::shared_mutex>`；如果你的编译器不支持 CTAD，就得写 `std::unique_lock<std::shared_mutex> lk(mu_);`。
2. **千万别把写方法写成 `std::shared_lock`**：写锁写成读锁 = 完全没锁，因为多个"写者"能同时拿到 shared 锁一起改数据——**数据竞争，UB**。
3. **读方法必须是 `shared_lock`**：写成 `unique_lock` 能跑但**性能被打回 mutex 版本**，读并发失效。
4. **`size() const noexcept` 里加 `shared_lock` 会破坏 noexcept 承诺**：因为 lock 有可能抛异常（比如资源耗尽）。**严格讲要么摘掉 noexcept，要么用 `try_lock` + 兜底**。本案例简化保留 noexcept，学术上不严谨但工程上极少触发，卷三网络化重构时再改。
5. **`purgeExpired` 用 unique_lock**：虽然它扫描时"看似只读"，但 `erase` 会修改容器，属于写操作。

#### ③ 验证读并发的加速效果

上面表格里"读方法"只要参与并发都能受益。写个小 benchmark：

```bash
# mutex 版
$ ./bench_mutex          → QPS = 1.2M ops/s
# shared_mutex 版
$ ./bench_shared_mutex   → QPS = 4.8M ops/s  ⚡ 读并发上去了
```

4 倍吞吐差距**只来自锁类型的一个字替换**——这就是"选对同步原语"的价值。
...
定义一个守护线程。**这里推荐"声明放头文件、构造函数体放 `.cpp`"的标准工程写法**，虽然 Lambda 短小，但一旦函数体里 include 了 `store.hpp` / `log.hpp`，头文件的传染面就会变大。分 3 处修改。

#### ① 头文件：写 `Server` 的类声明（只留签名）

> 📁 **修改文件**：`include/mkv/server.hpp`（**新建**）
>
> **为什么类声明放头文件？** `main.cpp` 里要 `mkv::Server server(store);`，编译器必须看到 `Server` 的字段布局和构造函数签名。

```cpp
// include/mkv/server.hpp
#pragma once

#include <thread>          // 【必须】std::jthread 是字段类型，要完整定义
...
class Server {
public:
    // 【只写签名，函数体挪去 server.cpp】
    // explicit 防止 Server s = someStore; 这种隐式转换（Store 得先加 conversion 才能命中）
...
1. **`<thread>` 头必须包**：`std::jthread` 是**字段**，编译器要算 `sizeof(Server)` 必须看到 jthread 的完整定义。前置声明不够。
2. **`Store` 只用引用则前置声明足够**：因为编译期不需要知道 Store 的字段布局，只需要知道"有这么个类型"。这样 `server.hpp` 不会 include `store.hpp`，避免头文件依赖膨胀。
3. **引用成员 `Store& store_` 必须初始化列表绑定**：C++ 引用一旦绑定不可改绑，也不允许"默认构造"。**只能在初始化列表里写 `: store_(store)`**，写到函数体里 `store_ = store;` 是错的（那是赋值）。
4. **`std::jthread purger_` 声明顺序放在 `store_` 之后**：因为**成员按声明顺序初始化**，`purger_` 的 Lambda 里会捕获 `this` 后访问 `store_`，`store_` 必须先绑定完。声明顺序颠倒 = 未初始化引用被访问 = UB。

#### ② 实现文件：新建 `src/server.cpp` 写构造函数体

> 📁 **修改文件**：`src/server.cpp`（**新建**）
>
> **为什么构造函数体放 `.cpp`？** 一旦函数体里用到 `store_.purgeExpired()`，就必须 `#include "mkv/store.hpp"`；如果放在 header 里，任何 include `server.hpp` 的 `.cpp` 都会被拖着 include `store.hpp` + `entry.hpp` + `value.hpp` 全套，编译时间飙升。放 `.cpp` 只让 `server.cpp` 一个文件承受。
...
Server::Server(Store& store)
    ...
    , purger_(std::jthread([this](std::stop_token st) {
        ...
        while (!st.stop_requested()) {                                          // 协作式停止
            std::this_thread::sleep_for(std::chrono::milliseconds(100));         // 每 100ms 巡一次
            try {
                store_.purgeExpired();
            } catch (const std::exception& e) {
...
1. **`this` 捕获后必须保证 `Server` 对象生命周期覆盖线程运行期**：本项目里 `Server` 是 main 的栈对象，`~Server()` 触发 jthread 析构会先 stop + join，正好在对象销毁前把线程收尾干净。**如果 `Server` 是堆对象且被提前 delete，`this` 就是野指针**。
2. **后台线程里必须 `try-catch` 兜底**：C++ 规定线程函数抛异常且没捕获 → `std::terminate` 进程直接死。`purgeExpired` 内部可能因锁失败抛异常，这里必须兜住。
3. **`stop_token st` 参数是 jthread 特殊照顾**：只要 Lambda 的**第一个参数**类型是 `std::stop_token`，jthread 就会自动传入；写不带 stop_token 的 Lambda 也能编译，但**无法响应 request_stop**，析构时线程可能永远不退出。
4. **`sleep_for` 用的是 `std::this_thread`，不是 `store_`**：新手常写成 `this->sleep_for(...)`。
5. **初始化列表逗号顺序 = 成员声明顺序**：`store_` 在前、`purger_` 在后，这**必须**和头文件里字段声明顺序一致，编译器警告 `-Wreorder` 会提示但不强制。
...
⚠️ **关键点**：`std::jthread` 是 C++20 起的类型。若编译器提示"'jthread' is not a member of 'std'"，检查 `CMAKE_CXX_STANDARD` 是否 ≥ 20；GCC < 10、Clang < 12 不完整支持，建议 GCC 11+ 或 Clang 14+。
