...
> 📌 **本篇定位**：这是排查篇的**第四篇**。前三篇分别讲了"信号定位"（[01.信号崩溃快速排查](01.信号崩溃快速排查.md)）、"内存类 bug 第一现场暴露"（[02.ASan内存诊断](02.ASan内存诊断.md)）、"现场调试器钉死它"（[03.GDB命令速查表](03.GDB命令速查表.md)）。本篇专攻的是**最后一种调试时态**：**事后**——程序已经死了，只剩一个文件，要把全部现场还原回来。读完本篇，再看任何 core，都能立刻回答："**符号齐不齐、信号是哪类、寄存器和栈讲了什么故事、根因在哪个时间点产生**"。
...
崩溃捕获框架（[01.信号崩溃快速排查](01.信号崩溃快速排查.md) 第 8 节、Breakpad、sentry-native）解决的是"崩溃自动上报"——它们生成的是 **minidump**：只保存关键线程栈 + 寄存器 + 模块列表，几百 KB。
...
读法对照（详见 [01.信号崩溃快速排查](01.信号崩溃快速排查.md) 第 5 章）：
...
**对 ASan 编译的 core**——有更省事的方法：[ASan 篇](02.ASan内存诊断.md) 提到的 `__asan_describe_address` 可以反向给出"这个地址当前是什么状态、之前是什么对象、谁分配的"。
...
**修法**：智能指针 + 显式所有权（[01.信号崩溃快速排查](01.信号崩溃快速排查.md) 第 13.1 节）。
...
生产环境不能交互、不能挂 gdb、运维醒不来——所以现场必须**自动留下来**。`ulimit -c` + `core_pattern` + 调试符号分离 = 任何凌晨 3 点的崩溃，第二天上班都能回到那一刻。**没有 core 的生产环境是裸奔**。这条与 [01.信号崩溃快速排查](01.信号崩溃快速排查.md) 第 13.3 节同源——本篇把它具体化为"工程上怎么留下、怎么取出、怎么读懂"。
...
崩溃位置只是"第一个绷不住的地方"。`Channel::status` 是 crash site；`reconcile` 读到 `0x18` 是近因；`channel_map_` 的并发写撕裂才是 bug site。**调试就是从 crash 沿栈反向爬，每一帧都问"这一层的输入是不是已经错了"**——直到找到自己代码里的某个错误假设。这条和 [01.信号崩溃快速排查](01.信号崩溃快速排查.md) 第 10.4 节互为表里：信号篇讲"概念"，core 篇讲"如何在 gdb 里一帧一帧实操"。
...
> **下一篇**：本篇讲了"事后从一个崩溃文件还原全部现场"，至此排查篇四件套（[01.信号崩溃快速排查](01.信号崩溃快速排查.md) → [02.ASan内存诊断](02.ASan内存诊断.md) → [03.GDB命令速查表](03.GDB命令速查表.md) → 04.CoreDump破案实录）已经形成完整闭环：**事前阻断 → 现场调试 → 事后破案**。配套阅读：[01.进程地址空间布局](../03.专栏博客/01.进程地址空间布局.md)（栈溢出、堆段、mmap 在 core 里的物理对照基础）。
