...
> **下一篇**：本篇讲了"如何让内存 bug 在第一现场暴露"，下一步进入 [03.GDB高级调试](03.GDB高级调试.md)——把"现场已经被抓住后如何把 bug 钉死"这一面讲透。配套阅读：[01.信号崩溃快速排查](01.信号崩溃快速排查.md)（崩溃定位的姊妹篇）、[01.进程地址空间布局](../03.专栏博客/01.进程地址空间布局.md)（理解 shadow memory 必备的内存模型基础）。
