...
> **下一篇**：本篇讲了"现场抓住怎么钉死它"，下一步进入 [04.CoreDump破案实录](04.CoreDump破案实录.md)——把"事后从一个崩溃文件还原全部现场"的实战流程讲透。配套阅读：[01.信号崩溃快速排查](01.信号崩溃快速排查.md)（崩溃信号定位）、[02.ASan内存诊断](02.ASan内存诊断.md)（让内存类 bug 在第一现场暴露）、[01.进程地址空间布局](../03.专栏博客/01.进程地址空间布局.md)（多线程栈与寄存器的内存模型基础）。
