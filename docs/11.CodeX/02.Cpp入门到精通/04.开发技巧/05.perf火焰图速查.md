...
- 分片锁（hash 到 N 把锁）；
- RCU 风格快照（详见 [04.CoreDump破案实录](04.CoreDump破案实录.md) 第 13.1 节）。
...
性能直觉常常是错的——主线一谁能想到 "string copy 占 42% CPU"？主线二谁能想到 "改个结构体大小 QPS 跌一半"？**好的工程师不靠猜，靠测量**。perf 让"瓶颈在哪"从猜测变成数据。这条与 [01.信号崩溃快速排查](01.信号崩溃快速排查.md) 第 13.3 节哲学 2 同源——"先看错误信息再开调试器"，本篇翻译成"先看 perf stat 再上火焰图"。
...
普通 perf 只看 "CPU 上发生了什么"。但生产里**最难调的问题是 "CPU 上没在跑，但用户在等"**——锁竞争、IO 阻塞、调度延迟。off-CPU 火焰图把这些"看不见的等待"也画出来。**on-CPU 看竞争 CPU 的代码，off-CPU 看竞争资源的代码——两者合一才是完整画像**。这条对应 [04.CoreDump破案实录](04.CoreDump破案实录.md) 哲学 3 的"crash site ≠ bug site"——表面现象不是真相。
...
> **下一篇**：本篇讲了"代码不崩但慢怎么办"，至此排查篇五件套（[01.信号崩溃快速排查](01.信号崩溃快速排查.md) → [02.ASan内存诊断](02.ASan内存诊断.md) → [03.GDB命令速查表](03.GDB命令速查表.md) → [04.CoreDump破案实录](04.CoreDump破案实录.md) → 05.perf火焰图实战）已经形成完整闭环：**事前阻断 → 现场调试 → 事后破案 → 持续优化**。配套阅读：[01.进程地址空间布局](../03.专栏博客/01.进程地址空间布局.md)（理解 cache line / TLB / 内存层级是 perf annotate 的物理基础）。
