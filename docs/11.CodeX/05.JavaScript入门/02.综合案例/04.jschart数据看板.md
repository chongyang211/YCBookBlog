...
❓ **既然有 Vue 为什么自己写模板引擎？** 答：**就是为了看清"Vue 模板编译做了什么"**——`{{ expr }}` 替换其实是**正则扫描 + new Function** 两件事。Vue 模板编译比这复杂 20 倍（生成 render 函数、vnode、diff），但起点是同一行 new Function。
...
- ⬅ 上一案例：[03.jsplayer H5 视频播放器](./03.jsplayer视频播放器.md)
