...
| # | 案例 | 难度 | 时长 | 字数 | 主导领域 |
|---|------|------|------|------|---------|
| 01 | [`jstodo` 原生待办清单](./01.jstodo待办清单.md) | ⭐⭐ | 5 h | ~1.5 万 | DOM / 事件委托 / ESM / 命令模式 |
| 02 | [`jsfeed` 异步 RSS 阅读器](./02.jsfeed异步阅读器.md) | ⭐⭐⭐⭐ | 10 h | ~2 万 | Promise / fetch / 生成器 / 流式 |
| 03 | [`jsplayer` H5 视频播放器](./03.jsplayer视频播放器.md) | ⭐⭐⭐⭐ | 12 h | ~2 万 | MSE / 视频流 / Canvas / Web API |
| 04 | [`jschart` 响应式数据看板](./04.jschart数据看板.md) 🎓 | ⭐⭐⭐⭐⭐ | 16 h | ~2.5 万 | Proxy 响应式 / Canvas / WebSocket / Worker（毕业设计） |
...
➡ 下一章：[案例 01 · `jstodo` 原生待办清单](./01.jstodo待办清单.md)
