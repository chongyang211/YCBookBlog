...
### 3.7 Light DOM vs Shadow DOM 速记
...
### 4.5 EventTarget vs Node.js EventEmitter
...
### 5.6 Step 2：用 fetch 拉 fmp4 喂 MSE
...
### 5.8 Step 4：故意造 bug——同步 append 双发
...
### 6.3 Step 2：HlsLoader——串起解析 + 下载 + MSE
...
### 6.7 故意造 bug：忘记 cancel 旧 HlsLoader
...
### 7.1 灵魂三问：HTMLVideoElement 的 24 个事件
...
### 7.4 Step 3：把 Controls 接到 JsPlayerApp
...
### 7.5 灵魂解析：rVFC vs rAF vs timeupdate
...
### 7.6 故意造 bug：rVFC 没在 destroy 里取消
...
### 8.4 Step 3：把 Engine 接进 Controls
...
### 9.3 Step 2：Media Session（系统级控制）
...
>
> ✅ **三大综合案例总成绩单**：
> - `01.jstodo` ：JS 语法 + DOM + LocalStorage
> - `02.jsfeed` ：异步 + 流式 + 限流 + 缓存
> - `03.jsplayer`：Web Component + MSE + Canvas + 平台 API
>
