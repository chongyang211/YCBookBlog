...
#### 目录介绍
- [1. 案例引入](#1-案例引入)
  - [1.1 一个事件监听器让 200MB 的数据永远不被回收——闭包泄漏的典型现场](#11-一个事件监听器让-200mb-的数据永远不被回收闭包泄漏的典型现场)
  - [1.2 顺藤摸到根因](#12-顺藤摸到根因)
  - [1.3 我们要回答什么](#13-我们要回答什么)
- [2. 架构概览](#2-架构概览)
  - [2.1 执行上下文 + 作用域链总图——V8 在函数调用时建立的物理结构](#21-执行上下文--作用域链总图v8-在函数调用时建立的物理结构)
  - [2.2 为什么需要三种执行上下文——全局/函数/eval 的架构分界线](#22-为什么需要三种执行上下文全局函数eval-的架构分界线)
- [3. 三种执行上下文与执行上下文栈——ECS 的压栈与出栈](#3-三种执行上下文与执行上下文栈ecs-的压栈与出栈)
  - [3.1 全局执行上下文（GEC）——混合型环境记录的根](#31-全局执行上下文gec混合型环境记录的根)
  - [3.2 函数执行上下文（FEC）——每次调用都新建一个宇宙](#32-函数执行上下文fec每次调用都新建一个宇宙)
  - [3.3 eval 执行上下文——V8 为什么拒绝优化包含 eval 的函数](#33-eval-执行上下文v8-为什么拒绝优化包含-eval-的函数)
  - [3.4 执行上下文栈的压栈与出栈——LIFO 调度与 Context 对象的堆栈解耦](#34-执行上下文栈的压栈与出栈lifo-调度与-context-对象的堆栈解耦)
- [4. 词法环境与变量环境——为什么 ES6 把变量拆成两类存储](#4-词法环境与变量环境为什么-es6-把变量拆成两类存储)
  - [4.1 let/const 存在词法环境，var 存在变量环境——同一函数内的两套储存体系](#41-letconst-存在词法环境var-存在变量环境同一函数内的两套储存体系)
  - [4.2 为什么分两个环境——不是历史包袱，而是设计必然](#42-为什么分两个环境不是历史包袱而是设计必然)
- [5. TDZ 暂时性死区——V8 用 TheHole 哨兵值锁住变量](#5-tdz-暂时性死区v8-用-thehole-哨兵值锁住变量)
  - [5.1 `TheHole` sentinel 值与 `LdaTheHoleCheck` 字节码——TDZ 的硬件级实现](#51-thehole-sentinel-值与-ldatheholecheck-字节码tdz-的硬件级实现)
  - [5.2 var 提升 vs let/const 提升——两者都提升，差异在初始化](#52-var-提升-vs-letconst-提升两者都提升差异在初始化)
  - [5.3 TDZ 的隐蔽触发——class 声明、函数参数默认值、super 调用](#53-tdz-的隐蔽触发class-声明函数参数默认值super-调用)
- [6. 作用域链的形成机制——[[OuterEnv]] 指针链的物理结构](#6-作用域链的形成机制outerenv-指针链的物理结构)
  - [6.1 `[[OuterEnv]]` 在定义时锁定——词法作用域的物理根基](#61-outerenv-在定义时锁定词法作用域的物理根基)
  - [6.2 RHS 查找 vs LHS 查找——读变量和写变量走的是同一条链吗](#62-rhs-查找-vs-lhs-查找读变量和写变量走的是同一条链吗)
  - [6.3 `with` 和 `eval` 对作用域链的破坏——为什么 TurboFan 直接放弃优化](#63-with-和-eval-对作用域链的破坏为什么-turbofan-直接放弃优化)
- [7. 闭包的 V8 物理实现——从堆上 Context 到 GC 存活判定的完整链路](#7-闭包的-v8-物理实现从堆上-context-到-gc-存活判定的完整链路)
  - [7.1 V8 的 Context 对象链——堆上分配的变量槽位数组](#71-v8-的-context-对象链堆上分配的变量槽位数组)
  - [7.2 `JSFunction::context` 字段——连接"函数体"和"外层变量"的指针](#72-jsfunctioncontext-字段连接函数体和外层变量的指针)
  - [7.3 闭包如何让本该释放的变量「活下来」——GC 可达性分析在闭包中的具体应用](#73-闭包如何让本该释放的变量活下来gc-可达性分析在闭包中的具体应用)
- [8. 闭包的六大陷阱——从内存泄漏到 forEach+async](#8-闭包的六大陷阱从内存泄漏到-foreachasync)
  - [8.1 循环 + var 经典陷阱及四种解法](#81-循环--var-经典陷阱及四种解法)
  - [8.2 闭包内 this 的陷阱](#82-闭包内-this-的陷阱)
  - [8.3 forEach 里写 await 为什么不行](#83-foreach-里写-await-为什么不行)
  - [8.4 闭包导致的内存泄漏——DevTools 堆快照定位实战](#84-闭包导致的内存泄漏devtools-堆快照定位实战)
- [9. 闭包的六大实战模式——从模块模式到防抖节流](#9-闭包的六大实战模式从模块模式到防抖节流)
  - [9.1 模块模式 + 惰性初始化——用闭包封装"只有创建者知道"的状态](#91-模块模式--惰性初始化用闭包封装只有创建者知道的状态)
  - [9.2 柯里化 + partial + 数据缓存——函数工厂的三种形态](#92-柯里化--partial--数据缓存函数工厂的三种形态)
  - [9.3 回调工厂 + 防抖节流——闭包保存 timer 和 lastTime](#93-回调工厂--防抖节流闭包保存-timer-和-lasttime)
  - [9.4 模块作用域——IIFE 模拟 vs ESM 天然隔离](#94-模块作用域iife-模拟-vs-esm-天然隔离)
- [10. 综合案例串讲](#10-综合案例串讲)
  - [10.1 案例真相揭晓——六个疑问逐条解答](#101-案例真相揭晓六个疑问逐条解答)
  - [10.2 一个闭包的诞生与消亡——从 `function outer()` 到 GC 回收的完整生命周期](#102-一个闭包的诞生与消亡从-function-outer-到-gc-回收的完整生命周期)
  - [10.3 设计哲学回扣](#103-设计哲学回扣)
  - [10.4 速查表：闭包六陷阱 / 六模式 / TDZ 触发条件 / 执行上下文三类型](#104-速查表闭包六陷阱--六模式--tdz-触发条件--执行上下文三类型)

---

## 1. 案例引入

### 1.1 一个事件监听器让 200MB 的数据永远不被回收——闭包泄漏的典型现场
...
### 2.1 执行上下文 + 作用域链总图——V8 在函数调用时建立的物理结构
...
### 2.2 为什么需要三种执行上下文——全局/函数/eval 的架构分界线
...
## 3. 三种执行上下文与执行上下文栈——ECS 的压栈与出栈

### 3.1 全局执行上下文（GEC）——混合型环境记录的根
...
### 3.3 eval 执行上下文——V8 为什么拒绝优化包含 eval 的函数
...
### 3.4 执行上下文栈的压栈与出栈——LIFO 调度与 Context 对象的堆栈解耦
...
## 4. 词法环境与变量环境——为什么 ES6 把变量拆成两类存储

### 4.1 let/const 存在词法环境，var 存在变量环境——同一函数内的两套储存体系
...
## 5. TDZ 暂时性死区——V8 用 TheHole 哨兵值锁住变量

### 5.1 `TheHole` sentinel 值与 `LdaTheHoleCheck` 字节码——TDZ 的硬件级实现
...
### 5.2 var 提升 vs let/const 提升——两者都提升，差异在初始化
...
### 5.3 TDZ 的隐蔽触发——class 声明、函数参数默认值、super 调用
...
## 6. 作用域链的形成机制——[[OuterEnv]] 指针链的物理结构

### 6.1 `[[OuterEnv]]` 在定义时锁定——词法作用域的物理根基
...
### 6.2 RHS 查找 vs LHS 查找——读变量和写变量走的是同一条链吗
...
### 6.3 `with` 和 `eval` 对作用域链的破坏——为什么 TurboFan 直接放弃优化
...
## 7. 闭包的 V8 物理实现——从堆上 Context 到 GC 存活判定的完整链路

### 7.1 V8 的 Context 对象链——堆上分配的变量槽位数组
...
### 7.2 `JSFunction::context` 字段——连接"函数体"和"外层变量"的指针
...
### 7.3 闭包如何让本该释放的变量「活下来」——GC 可达性分析在闭包中的具体应用
...
## 8. 闭包的六大陷阱——从内存泄漏到 forEach+async

### 8.1 循环 + var 经典陷阱及四种解法
...
### 8.4 闭包导致的内存泄漏——DevTools 堆快照定位实战
...
## 9. 闭包的六大实战模式——从模块模式到防抖节流

### 9.1 模块模式 + 惰性初始化——用闭包封装"只有创建者知道"的状态
...
### 9.2 柯里化 + partial + 数据缓存——函数工厂的三种形态
...
### 9.3 回调工厂 + 防抖节流——闭包保存 timer 和 lastTime
...
### 10.2 一个闭包的诞生与消亡——从 `function outer()` 到 GC 回收的完整生命周期
...
**下一步**：闭包让函数记住了外部环境。但函数内部还有一个更诡的话题——**this**。为什么 `obj.method()` 和 `const fn = obj.method; fn()` 的 this 不一样？进入 [05.this 四规则与函数组合](05.this四规则与函数组合.md)。
