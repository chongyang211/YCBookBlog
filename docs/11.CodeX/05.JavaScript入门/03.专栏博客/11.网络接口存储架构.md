...
**下一步**：浏览器端的存储搞定了。JS 的另一主场——服务端 Node.js，它的运行模型完全不同。进入 [12.Node.js 运行时与流式编程](https://yccoding.com/pages/js-core-12/)。
