...
#### 目录介绍
- [1. 案例引入](#1-案例引入)
  - [1.1 一个 500 行的项目，构建出来 2MB——bundle 分析暴露了什么](#11-一个-500-行的项目构建出来-2mbbundle-分析暴露了什么)
  - [1.2 顺藤摸到根因——五条假设推演](#12-顺藤摸到根因五条假设推演)
  - [1.3 我们要回答什么](#13-我们要回答什么)
- [2. 架构概览](#2-架构概览)
  - [2.1 开发 → 构建 → 测试 → 部署的现代前端工程流水线](#21-开发--构建--测试--部署的现代前端工程流水线)
  - [2.2 为什么 Vite 的出现改变了打包哲学——从「全量打包」到「按需编译」](#22-为什么-vite-的出现改变了打包哲学从全量打包到按需编译)
...
- [5. 生产构建：Rollup + Tree Shaking](#5-生产构建rollup--tree-shaking)
  - [5.1 ESM 静态分析 + usedExports + sideEffects: false——三管齐下](#51-esm-静态分析--usedexports--sideeffects-false三管齐下)
...
- [6. 测试金字塔：从单元到 E2E](#6-测试金字塔从单元到-e2e)
  - [6.1 静态分析（TypeScript / ESLint）——免费的第一道防线](#61-静态分析typescript--eslint免费的第一道防线)
  - [6.2 单元测试（Vitest）：mocking/spy——与 Jest 的本质差异](#62-单元测试vitestmockingspy与-jest-的本质差异)
  - [6.3 组件测试（Testing Library）：jsdom vs happy-dom 差异——以用户视角测试](#63-组件测试testing-libraryjsdom-vs-happy-dom-差异以用户视角测试)
  - [6.4 E2E（Playwright）：三浏览器自动录制 + 网络拦截——为什么贵但不可替代](#64-e2eplaywright三浏览器自动录制--网络拦截为什么贵但不可替代)
...
- [8. Performance 面板火焰图读法](#8-performance-面板火焰图读法)
  ...
  - [8.2 火焰图中的布局/绘制/合成阶段识别——颜色编码与优化策略](#82-火焰图中的布局绘制合成阶段识别颜色编码与优化策略)
  - [8.3 长任务（Long Task）的定位与拆分策略——从 200ms 到 4×50ms](#83-长任务long-task的定位与拆分策略从-200ms-到-4×50ms)
- [9. Memory 面板与 Lighthouse](#9-memory-面板与-lighthouse)
  ...
  - [9.2 堆快照对比（Heap Snapshot Diff）：定位内存泄漏——三步定位法](#92-堆快照对比heap-snapshot-diff定位内存泄漏三步定位法)
...
- [10. 综合案例串讲](#10-综合案例串讲)
  ...
  - [10.2 对一个组件做完整性能诊断：Lighthouse → 火焰图 → Memory Snapshot → 优化 → 再测](#102-对一个组件做完整性能诊断lighthouse--火焰图--memory-snapshot--优化--再测)
...
### 1.1 一个 500 行的项目，构建出来 2MB——bundle 分析暴露了什么
...
### 2.2 为什么 Vite 的出现改变了打包哲学——从「全量打包」到「按需编译」
...
### 5.1 ESM 静态分析 + usedExports + sideEffects: false——三管齐下
...
### 6.1 静态分析（TypeScript / ESLint）——免费的第一道防线
...
### 6.2 单元测试（Vitest）：mocking/spy——与 Jest 的本质差异
...
### 6.3 组件测试（Testing Library）：jsdom vs happy-dom 差异——以用户视角测试
...
### 6.4 E2E（Playwright）：三浏览器自动录制 + 网络拦截——为什么贵但不可替代
...
### 8.2 火焰图中的布局/绘制/合成阶段识别——颜色编码与优化策略
...
### 8.3 长任务（Long Task）的定位与拆分策略——从 200ms 到 4×50ms
...
### 9.2 堆快照对比（Heap Snapshot Diff）：定位内存泄漏——三步定位法
...
### 10.2 对一个组件做完整性能诊断：Lighthouse → 火焰图 → Memory Snapshot → 优化 → 再测
