...
#### 目录介绍
- [1. 案例引入](#1-案例引入)
  - [1.1 console.log 被劫持了——一个未改源码的前端监控如何实现](#11-consolelog-被劫持了一个未改源码的前端监控如何实现)
  - [1.2 顺藤摸到根因——Proxy 拦截的不是属性，是引擎内部方法](#12-顺藤摸到根因proxy-拦截的不是属性是引擎内部方法)
  - [1.3 我们要回答什么](#13-我们要回答什么)
- [2. 架构概览](#2-架构概览)
  - [2.1 Proxy 13 种 trap ↔ Reflect 13 种方法的对偶关系](#21-proxy-13-种-trap--reflect-13-种方法的对偶关系)
  - [2.2 为什么 Proxy 能覆盖 defineProperty 做不到的事——从 ECMA-262 内部方法谈起](#22-为什么-proxy-能覆盖-defineproperty-做不到的事从-ecma-262-内部方法谈起)
- [3. 对象操作类 trap](#3-对象操作类-trap)
  - [3.1 get / set / has / deleteProperty 四大常用 trap](#31-get--set--has--deleteproperty-四大常用-trap)
  - [3.2 Reflect 的默认行为保证不变性——为什么它返回 true/false 而不是抛异常](#32-reflect-的默认行为保证不变性为什么它返回-truefalse-而不是抛异常)
  - [3.3 为什么 get trap 总是和 Reflect.get 成对出现——receiver 的 this 传递链](#33-为什么-get-trap-总是和-reflectget-成对出现receiver-的-this-传递链)
- [4. 函数操作类 trap](#4-函数操作类-trap)
  - [4.1 apply：拦截函数调用——AOP 的 JS 原生实现](#41-apply拦截函数调用aop-的-js-原生实现)
  - [4.2 construct：拦截 new 操作——单例、工厂、依赖注入的基石](#42-construct拦截-new-操作单例工厂依赖注入的基石)
- [5. 属性枚举与描述符 trap](#5-属性枚举与描述符-trap)
  ...
  - [5.2 getOwnPropertyDescriptor 的联动陷阱——ownKeys 报告了 key 但描述符消失的后果](#52-getownpropertydescriptor-的联动陷阱ownkeys-报告了-key-但描述符消失的后果)
...
- [6. 可撤销 Proxy 与 this 穿透](#6-可撤销-proxy-与-this-穿透)
  - [6.1 Proxy.revocable 的安全场景——一次性数据借出](#61-proxyrevocable-的安全场景一次性数据借出)
  - [6.2 Proxy 里的 this 指向谁——原生方法调用中的 this 穿透乱象](#62-proxy-里的-this-指向谁原生方法调用中的-this-穿透乱象)
  - [6.3 用 bind + Reflect 修复 this 穿透——三行代码的修复与它的边界](#63-用-bind--reflect-修复-this-穿透三行代码的修复与它的边界)
- [7. Proxy 性能实测](#7-proxy-性能实测)
  - [7.1 10 万次增删改查：Proxy vs 原始对象 vs defineProperty benchmark](#71-10-万次增删改查proxy-vs-原始对象-vs-defineproperty-benchmark)
  - [7.2 为什么 Proxy 不能用于高性能热路径——V8 内部对 Proxy 的优化限制](#72-为什么-proxy-不能用于高性能热路径v8-内部对-proxy-的优化限制)
  - [7.3 Vue 3 的响应式优化策略——如何在 Proxy 的性能代价上做减法](#73-vue-3-的响应式优化策略如何在-proxy-的性能代价上做减法)
- [8. 迭代器协议](#8-迭代器协议)
  - [8.1 Symbol.iterator + next() 返回 `{ value, done }`——不只是语法，是 ECMA-262 定义的两种协议](#81-symboliterator--next-返回--value-done不只是语法是-ecma-262-定义的两种协议)
  - [8.2 为什么迭代器不是「指针」而是「协议」——JS 的 Duck Typing 哲学在遍历上的极致体现](#82-为什么迭代器不是指针而是协议js-的-duck-typing-哲学在遍历上的极致体现)
  - [8.3 可迭代协议 vs 迭代器协议的实际区别——为什么 NodeList 可以用 for…of 但 Object 不行](#83-可迭代协议-vs-迭代器协议的实际区别为什么-nodelist-可以用-forof-但-object-不行)
- [9. 生成器与异步迭代](#9-生成器与异步迭代)
  - [9.1 Generator 状态机的 V8 实现：`SuspendGenerator` / `ResumeGenerator` 字节码全析](#91-generator-状态机的-v8-实现suspendgenerator--resumegenerator-字节码全析)
  - [9.2 yield 的双向通信——生成器不只是「返回多次」，而是「每次返回时都能收一笔新钱」](#92-yield-的双向通信生成器不只是返回多次而是每次返回时都能收一笔新钱)
  - [9.3 异步迭代：Symbol.asyncIterator + for await…of + ReadableStream 三方配合的生产实战](#93-异步迭代symbolasynciterator--for-awaitof--readablestream-三方配合的生产实战)
- [10. 综合案例串讲](#10-综合案例串讲)
  - [10.1 案例真相揭晓——六问逐条作答](#101-案例真相揭晓六问逐条作答)
  - [10.2 用 Proxy 实现带校验和变更日志的 ORM 风格 model 层](#102-用-proxy-实现带校验和变更日志的-orm-风格-model-层)
  - [10.3 设计哲学回扣](#103-设计哲学回扣)
  - [10.4 速查表：13 种 trap 速查 + 迭代器协议全景 + 60 秒诊断清单](#104-速查表13-种-trap-速查--迭代器协议全景--60-秒诊断清单)

---

## 1. 案例引入

### 1.1 console.log 被劫持了——一个未改源码的前端监控如何实现
...
### 1.2 顺藤摸到根因——Proxy 拦截的不是属性，是引擎内部方法
...
### 2.2 为什么 Proxy 能覆盖 defineProperty 做不到的事——从 ECMA-262 内部方法谈起
...
### 3.2 Reflect 的默认行为保证不变性——为什么它返回 true/false 而不是抛异常
...
### 3.3 为什么 get trap 总是和 Reflect.get 成对出现——receiver 的 this 传递链
...
### 4.1 apply：拦截函数调用——AOP 的 JS 原生实现
...
### 4.2 construct：拦截 new 操作——单例、工厂、依赖注入的基石
...
### 5.2 getOwnPropertyDescriptor 的联动陷阱——ownKeys 报告了 key 但描述符消失的后果
...
### 6.1 Proxy.revocable 的安全场景——一次性数据借出
...
### 6.2 Proxy 里的 this 指向谁——原生方法调用中的 this 穿透乱象
...
### 6.3 用 bind + Reflect 修复 this 穿透——三行代码的修复与它的边界
...
### 7.1 10 万次增删改查：Proxy vs 原始对象 vs defineProperty benchmark
...
### 7.2 为什么 Proxy 不能用于高性能热路径——V8 内部对 Proxy 的优化限制
...
### 7.3 Vue 3 的响应式优化策略——如何在 Proxy 的性能代价上做减法
...
### 8.1 Symbol.iterator + next() 返回 `{ value, done }`——不只是语法，是 ECMA-262 定义的两种协议
...
### 8.2 为什么迭代器不是「指针」而是「协议」——JS 的 Duck Typing 哲学在遍历上的极致体现
...
### 8.3 可迭代协议 vs 迭代器协议的实际区别——为什么 NodeList 可以用 for…of 但 Object 不行
...
### 9.1 Generator 状态机的 V8 实现：`SuspendGenerator` / `ResumeGenerator` 字节码全析
...
### 9.2 yield 的双向通信——生成器不只是「返回多次」，而是「每次返回时都能收一笔新钱」
...
### 9.3 异步迭代：Symbol.asyncIterator + for await…of + ReadableStream 三方配合的生产实战
...
### 10.4 速查表：13 种 trap 速查 + 迭代器协议全景 + 60 秒诊断清单
...
**下一步**：元编程和迭代器都是**同步世界**的工具。JS 的真正主角——**异步**——该登场了。Promise 的状态机如何实现？`async/await` 和生成器是什么关系？微任务和宏任务到底谁先执行？进入 [08.事件循环 Promise 一体论](08.事件循环Promise一体论.md)。
