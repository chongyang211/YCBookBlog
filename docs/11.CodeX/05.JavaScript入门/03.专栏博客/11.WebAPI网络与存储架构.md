...
#### 目录介绍
- [1. 案例引入](#1-案例引入)
  - [1.1 PWA 离线打开白屏——缓存策略的锅](#11-pwa-离线打开白屏缓存策略的锅)
  - [1.2 顺藤摸到根因](#12-顺藤摸到根因)
  - [1.3 我们要回答什么](#13-我们要回答什么)
- [2. 架构概览](#2-架构概览)
  - [2.1 网络层（fetch / WS / SSE）+ 存储层（Storage / IDB / Cache）的双塔架构](#21-网络层fetch--ws--sse--存储层storage--idb--cache的双塔架构)
  - [2.2 数据生命周期：网络获取 → 内存处理 → 持久化存储 → 离线读取](#22-数据生命周期网络获取--内存处理--持久化存储--离线读取)
  - [2.3 为什么浏览器的存储方案分这么多种](#23-为什么浏览器的存储方案分这么多种)
- [3. fetch 深度](#3-fetch-深度)
  - [3.1 Request / Response 对象结构剖析](#31-request--response-对象结构剖析)
  - [3.2 流式读取：`getReader()` 实现下载进度——为什么 fetch 原生不支持进度回调](#32-流式读取getreader-实现下载进度为什么-fetch-原生不支持进度回调)
  - [3.3 AbortController + signal：中断正在进行的请求](#33-abortcontroller--signal中断正在进行的请求)
  - [3.4 fetch 的反模式与防遗忘清单](#34-fetch-的反模式与防遗忘清单)
- [4. WebSocket 帧协议深度](#4-websocket-帧协议深度)
  - [4.1 帧格式：opcode / mask / payload length / mask-key](#41-帧格式opcode--mask--payload-length--mask-key)
  - [4.2 心跳机制 + 自动重连 + 指数退避 + jitter](#42-心跳机制--自动重连--指数退避--jitter)
  - [4.3 对比 SSE：单工 vs 双工的选择依据](#43-对比-sse单工-vs-双工的选择依据)
- [5. SSE（Server-Sent Events）](#5-sseserver-sent-events)
  - [5.1 单工推送 + 自动重连 + EventSource API](#51-单工推送--自动重连--eventsource-api)
  - [5.2 什么时候 SSE 比 WebSocket 更合适](#52-什么时候-sse-比-websocket-更合适)
  - [5.3 SSE 的事件 ID 与重连恢复](#53-sse-的事件-id-与重连恢复)
- [6. Web Storage 三件套](#6-web-storage-三件套)
  - [6.1 Cookie：自动附带 + 5KB 上限 + HttpOnly/Secure/SameSite](#61-cookie自动附带--5kb-上限--httponlysecuresamesite)
  - [6.2 localStorage vs sessionStorage：容量/过期/跨域/同步阻塞 四维对比](#62-localstorage-vs-sessionstorage容量过期跨域同步阻塞-四维对比)
  - [6.3 为什么 localStorage 是同步 API——对渲染的影响](#63-为什么-localstorage-是同步-api对渲染的影响)
- [7. IndexedDB 深度](#7-indexeddb-深度)
  - [7.1 对象仓库 + 索引 + 游标 + 事务模型](#71-对象仓库--索引--游标--事务模型)
  - [7.2 为什么 IDB 的 API 这么难用——事件驱动 vs Promise 化的历史原因](#72-为什么-idb-的-api-这么难用事件驱动-vs-promise-化的历史原因)
  - [7.3 复合键 + IDBKeyRange.bound + 游标范围查询](#73-复合键--idbkeyrangebound--游标范围查询)
  - [7.4 IDB 事务的自动提交与 VACUUM 机制](#74-idb-事务的自动提交与-vacuum-机制)
- [8. Cache API + Service Worker 缓存策略](#8-cache-api--service-worker-缓存策略)
  - [8.1 Cache-First / Network-First / Stale-While-Revalidate / Cache-Only / Network-Only](#81-cache-first--network-first--stale-while-revalidate--cache-only--network-only)
  - [8.2 每种策略的适用场景与失效风险](#82-每种策略的适用场景与失效风险)
  - [8.3 Workbox 简化方案对比手写略](#83-workbox-简化方案对比手写略)
- [9. 存储选型决策矩阵](#9-存储选型决策矩阵)
  - [9.1 Cookie / localStorage / sessionStorage / IDB / Cache 五维对比](#91-cookie--localstorage--sessionstorage--idb--cache-五维对比)
  - [9.2 大数据量 / 高频写入 / 离线场景 / 安全敏感 四种场景的最佳选择](#92-大数据量--高频写入--离线场景--安全敏感-四种场景的最佳选择)
- [10. 综合案例串讲](#10-综合案例串讲)
  - [10.1 案例真相揭晓](#101-案例真相揭晓)
  - [10.2 实现「离线优先」新闻阅读器：在线 fetch→存 IDB→渲染；离线从 IDB 读](#102-实现离线优先新闻阅读器在线-fetch存-idb渲染离线从-idb-读)
  - [10.3 设计哲学回扣](#103-设计哲学回扣)
  - [10.4 速查表：存储方案五维对比 / 缓存策略五模式 / fetch 反模式防遗忘](#104-速查表存储方案五维对比--缓存策略五模式--fetch-反模式防遗忘)

---

## 1. 案例引入

### 1.1 PWA 离线打开白屏——缓存策略的锅
...
### 2.2 数据生命周期：网络获取 → 内存处理 → 持久化存储 → 离线读取
...
### 3.3 AbortController + signal：中断正在进行的请求
...
### 6.1 Cookie：自动附带 + 5KB 上限 + HttpOnly/Secure/SameSite
...
### 6.2 localStorage vs sessionStorage
...
### 6.3 为什么 localStorage 是同步 API——对渲染的影响
...
### 7.3 复合键 + IDBKeyRange.bound + 游标范围查询
...
## 8. Cache API + Service Worker 缓存策略
...
**下一步**：浏览器端的存储搞定了。JS 的另一主场——服务端 Node.js，它的运行模型完全不同。进入 [12.Node.js 运行时与流式编程](12.Node.js运行时与流式编程.md)。
