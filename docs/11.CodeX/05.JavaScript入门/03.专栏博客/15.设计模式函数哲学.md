...
#### 目录介绍
- [1. 案例引入](#1-案例引入)
  - [1.1 「点击按钮→请求数据→更新 UI」：10 行 vs 50 行的差异从哪来](#11-点击按钮请求数据更新-ui10-行-vs-50-行的差异从哪来)
  - [1.2 顺藤摸到根因——两次需求变更暴露了什么](#12-顺藤摸到根因两次需求变更暴露了什么)
...
- [3. 单例与工厂](#3-单例与工厂)
  ...
  - [3.2 工厂模式：一个函数替代 if/else 丛林——从条件分支到策略表](#32-工厂模式一个函数替代-ifelse-丛林从条件分支到策略表)
- [4. 观察者 vs 发布订阅](#4-观察者-vs-发布订阅)
  - [4.1 EventEmitter 手写实现（on/off/once/emit）——完整版](#41-eventemitter-手写实现onoffonceemit完整版)
  - [4.2 观察者（Subject 直接通知 Observer）vs 发布订阅（通过 EventBus 解耦）——架构级区别](#42-观察者subject-直接通知-observervs-发布订阅通过-eventbus-解耦架构级区别)
  - [4.3 与 Vue 的 watch、RxJS 的 Observable 三方对比](#43-与-vue-的-watchrxjs-的-observable-三方对比)
- [5. 策略与命令模式](#5-策略与命令模式)
  - [5.1 策略模式：表单校验器的生产级实现——一个对象替代 if/else 丛林](#51-策略模式表单校验器的生产级实现一个对象替代-ifelse-丛林)
  - [5.2 命令模式：撤销/重做——用命令栈实现富文本编辑器的 undo/redo](#52-命令模式撤销重做用命令栈实现富文本编辑器的-undoredo)
- [6. 适配器 vs 代理 vs 装饰器](#6-适配器-vs-代理-vs-装饰器)
  - [6.1 三者长得像但有本质区别——用业务案例逐行区分](#61-三者长得像但有本质区别用业务案例逐行区分)
  - [6.2 TC39 Stage 3 装饰器提案——与 Java 注解的本质差异](#62-tc39-stage-3-装饰器提案与-java-注解的本质差异)
- [7. 纯函数与副作用隔离](#7-纯函数与副作用隔离)
  - [7.1 相同的输入 → 相同的输出 + 无外部修改——推理边界的意义](#71-相同的输入--相同的输出--无外部修改推理边界的意义)
...
- [9. pipe / compose 与 Maybe 模式](#9-pipe--compose-与-maybe-模式)
  ...
  - [9.2 Maybe / Either 模式在 JS 中的轻量实现——用链式调用消除 null 防御代码](#92-maybe--either-模式在-js-中的轻量实现用链式调用消除-null-防御代码)
  - [9.3 TC39 Stage 2 pipe 操作符 (`|>`) 展望](#93-tc39-stage-2-pipe-操作符---展望)
- [10. 综合案例串讲](#10-综合案例串讲)
  - [10.1 案例真相揭晓——五问逐条作答](#101-案例真相揭晓五问逐条作答)
  - [10.2 一个「订单处理管线」从 30 行 if/else → 策略+pipe 的完整重构过程](#102-一个订单处理管线从-30-行-ifelse--策略pipe-的完整重构过程)
  - [10.3 设计哲学回扣](#103-设计哲学回扣)
  - [10.4 速查表：七种设计模式 + 框架对应 / pipe vs compose / 不可变方案对比 / 60 秒诊断清单](#104-速查表七种设计模式--框架对应--pipe-vs-compose--不可变方案对比--60-秒诊断清单)

---

## 1. 案例引入

### 1.1 「点击按钮→请求数据→更新 UI」：10 行 vs 50 行的差异从哪来
...
### 3.2 工厂模式：一个函数替代 if/else 丛林——从条件分支到策略表
...
### 4.1 EventEmitter 手写实现（on/off/once/emit）——完整版
...
### 4.2 观察者（Subject 直接通知 Observer）vs 发布订阅（通过 EventBus 解耦）——架构级区别
...
### 5.1 策略模式：表单校验器的生产级实现——一个对象替代 if/else 丛林
...
### 9.2 Maybe / Either 模式在 JS 中的轻量实现——用链式调用消除 null 防御代码
...
### 10.2 一个「订单处理管线」从 30 行 if/else → 策略+pipe 的完整重构过程
...
### 10.4 速查表：七种设计模式 + 框架对应 / pipe vs compose / 不可变方案对比 / 60 秒诊断清单
...
**下一步**：代码优雅了。但用户不止在浏览器里——跨端才是终极战场。进入 [16.跨端架构终局](16.跨端架构终局总结.md)。
