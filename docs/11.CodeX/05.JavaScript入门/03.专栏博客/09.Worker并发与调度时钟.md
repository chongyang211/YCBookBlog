...
#### 目录介绍
- [1. 案例引入](#1-案例引入)
  - [1.1 一个实时图像滤镜工具——同时处理 5 张图片导致页面死锁 3 秒](#11-一个实时图像滤镜工具同时处理-5-张图片导致页面死锁-3-秒)
  - [1.2 顺藤摸到根因](#12-顺藤摸到根因)
  - [1.3 我们要回答什么](#13-我们要回答什么)
- [2. 架构概览](#2-架构概览)
  - [2.1 DedicatedWorker / SharedWorker / ServiceWorker 三件套的能力边界](#21-dedicatedworker--sharedworker--serviceworker-三件套的能力边界)
  - [2.2 为什么浏览器要把这三种 Worker 分开设计](#22-为什么浏览器要把这三种-worker-分开设计)
- [3. DedicatedWorker 深度——从创建到销毁的完整生命周期](#3-dedicatedworker-深度从创建到销毁的完整生命周期)
  - [3.1 Worker 线程的底层创建——Blink 是怎么"借"一个线程给 JS 的](#31-worker-线程的底层创建blink-是怎么借一个线程给-js-的)
  - [3.2 postMessage 的两种传输模式——结构化克隆 vs Transferable 的内部实现](#32-postmessage-的两种传输模式结构化克隆-vs-transferable-的内部实现)
  - [3.3 Worker 中能访问哪些 API——一张能力矩阵的深层原因](#33-worker-中能访问哪些-api一张能力矩阵的深层原因)
- [4. SharedArrayBuffer + Atomics：真正的共享内存](#4-sharedarraybuffer--atomics真正的共享内存)
  - [4.1 SAB 的内存模型——为什么 postMessage(SAB) 不拷贝而是共享同一块物理页](#41-sab-的内存模型为什么-postmessagesab-不拷贝而是共享同一块物理页)
  - [4.2 Atomics 的线程同步原语——wait/notify 是怎么实现的](#42-atomics-的线程同步原语waitnotify-是怎么实现的)
  - [4.3 SAB 的安全前提——Spectre 漏洞是怎么逼出 COOP+COEP 的](#43-sab-的安全前提spectre-漏洞是怎么逼出-coopcoep-的)
- [5. ServiceWorker 架构——独立于页面的网络代理](#5-serviceworker-架构独立于页面的网络代理)
  - [5.1 生命周期：install → waiting → activate → fetch 的精确时序](#51-生命周期install--waiting--activate--fetch-的精确时序)
  - [5.2 为什么 ServiceWorker 不能直接操作 DOM——架构隔离的深层理由](#52-为什么-serviceworker-不能直接操作-dom架构隔离的深层理由)
  - [5.3 ServiceWorker 的线程独立性——它能活多久？谁管理它的生命周期？](#53-serviceworker-的线程独立性它能活多久谁管理它的生命周期)
- [6. 并发模式演进——从回调到 Observable 的六次范式跃迁](#6-并发模式演进从回调到-observable-的六次范式跃迁)
  - [6.1 回调 → Thunk → Promise → Generator+co → async/await → Observable 的精确解题链](#61-回调--thunk--promise--generatorco--asyncawait--observable-的精确解题链)
  - [6.2 每一步解决的是上一步的什么问题——不可逆的因果链](#62-每一步解决的是上一步的什么问题不可逆的因果链)
- [7. RxJS 的冷与热——Observable 订阅时机的精确含义](#7-rxjs-的冷与热observable-订阅时机的精确含义)
  - [7.1 Cold Observable——每次 subscribe 都是一次独立的生产流程](#71-cold-observable每次-subscribe-都是一次独立的生产流程)
  - [7.2 Hot Observable——生产者独立于订阅者而存在](#72-hot-observable生产者独立于订阅者而存在)
  - [7.3 Subject / BehaviorSubject / ReplaySubject 的生产者-消费者角色转换](#73-subject--behaviorsubject--replaysubject-的生产者-消费者角色转换)
- [8. setTimeout / setInterval 深度剖析——4ms 夹持的规范根源与实现边界](#8-settimeout--setinterval-深度剖析4ms-夹持的规范根源与实现边界)
  - [8.1 4ms 夹持的精确触发条件——HTML §8.6 的字面含义与实际执行](#81-4ms-夹持的精确触发条件html-86-的字面含义与实际执行)
  - [8.2 setTimeout(fn, 0) 在浏览器和 Node.js 中的真实延迟差异及根因](#82-settimeoutfn-0-在浏览器和-nodejs-中的真实延迟差异及根因)
  - [8.3 setInterval 的累积误差——为什么它不可能精准，以及如何修复](#83-setinterval-的累积误差为什么它不可能精准以及如何修复)
- [9. requestAnimationFrame 与 requestIdleCallback：帧内的时间预算管理](#9-requestanimationframe-与-requestidlecallback帧内的时间预算管理)
  - [9.1 rAF 与 Vsync 的对齐机制——为什么 rAF 在 60Hz 屏幕上每 16.67ms 触发一次](#91-raf-与-vsync-的对齐机制为什么-raf-在-60hz-屏幕上每-1667ms-触发一次)
  - [9.2 requestIdleCallback——浏览器在帧内空闲时执行低优先级任务](#92-requestidlecallback浏览器在帧内空闲时执行低优先级任务)
  - [9.3 React Scheduler 的 Time Slicing 如何借鉴 rIC 但不直接用 rIC](#93-react-scheduler-的-time-slicing-如何借鉴-ric-但不直接用-ric)
- [10. 综合案例串讲](#10-综合案例串讲)
  - [10.1 案例真相揭晓——七个疑问逐条解答](#101-案例真相揭晓七个疑问逐条解答)
  - [10.2 Worker 排序 + rAF 渲染——实现不卡主线程的实时监控看板](#102-worker-排序--raf-渲染实现不卡主线程的实时监控看板)
  - [10.3 设计哲学回扣](#103-设计哲学回扣)
  - [10.4 速查表：Worker 三件套能力矩阵 / 定时器对比 / 调度时钟优先级 / RxJS 冷热](#104-速查表worker-三件套能力矩阵--定时器对比--调度时钟优先级--rxjs-冷热)

---

## 1. 案例引入

### 1.1 一个实时图像滤镜工具——同时处理 5 张图片导致页面死锁 3 秒
...
### 2.1 DedicatedWorker / SharedWorker / ServiceWorker 三件套的能力边界
...
## 3. DedicatedWorker 深度——从创建到销毁的完整生命周期

### 3.1 Worker 线程的底层创建——Blink 是怎么"借"一个线程给 JS 的
...
### 3.2 postMessage 的两种传输模式——结构化克隆 vs Transferable 的内部实现
...
### 3.3 Worker 中能访问哪些 API——一张能力矩阵的深层原因
...
### 4.1 SAB 的内存模型——为什么 postMessage(SAB) 不拷贝而是共享同一块物理页
...
### 4.2 Atomics 的线程同步原语——wait/notify 是怎么实现的
...
### 4.3 SAB 的安全前提——Spectre 漏洞是怎么逼出 COOP+COEP 的
...
## 5. ServiceWorker 架构——独立于页面的网络代理

### 5.1 生命周期：install → waiting → activate → fetch 的精确时序
...
### 5.2 为什么 ServiceWorker 不能直接操作 DOM——架构隔离的深层理由
...
### 5.3 ServiceWorker 的线程独立性——它能活多久？谁管理它的生命周期？
...
## 6. 并发模式演进——从回调到 Observable 的六次范式跃迁

### 6.1 回调 → Thunk → Promise → Generator+co → async/await → Observable 的精确解题链
...
## 7. RxJS 的冷与热——Observable 订阅时机的精确含义

### 7.1 Cold Observable——每次 subscribe 都是一次独立的生产流程
...
### 7.2 Hot Observable——生产者独立于订阅者而存在
...
### 7.3 Subject / BehaviorSubject / ReplaySubject 的生产者-消费者角色转换
...
## 8. setTimeout / setInterval 深度剖析——4ms 夹持的规范根源与实现边界

### 8.1 4ms 夹持的精确触发条件——HTML §8.6 的字面含义与实际执行
...
### 8.2 setTimeout(fn, 0) 在浏览器和 Node.js 中的真实延迟差异及根因
...
### 8.3 setInterval 的累积误差——为什么它不可能精准，以及如何修复
...
## 9. requestAnimationFrame 与 requestIdleCallback：帧内的时间预算管理

### 9.1 rAF 与 Vsync 的对齐机制——为什么 rAF 在 60Hz 屏幕上每 16.67ms 触发一次
...
### 9.2 requestIdleCallback——浏览器在帧内空闲时执行低优先级任务
...
### 9.3 React Scheduler 的 Time Slicing 如何借鉴 rIC 但不直接用 rIC
...
### 10.2 Worker 排序 + rAF 渲染——实现不卡主线程的实时监控看板
...
**下一步**：Worker 让排序不卡主线程了。但 DOM 操作本身也会卡——浏览器怎么把 HTML/CSS 变成屏幕像素？进入 [10.浏览器渲染像素之路](10.浏览器渲染像素之路.md)。
