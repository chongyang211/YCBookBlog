...
- [1. 案例引入](#1-案例引入)
  ...
  - [1.2 顺藤摸到根因——四条假设推演](#12-顺藤摸到根因四条假设推演)
  - [1.3 我们要回答什么](#13-我们要回答什么)
- [2. 架构概览](#2-架构概览)
  - [2.1 模块化五世演进：全局污染 → IIFE → CJS → AMD/UMD → ESM](#21-模块化五世演进全局污染--iife--cjs--amdumd--esm)
  - [2.2 每一步的「为什么需要变」——每一世在解决上一世的什么问题](#22-每一步的为什么需要变每一世在解决上一世的什么问题)
- [3. CJS 的 require 实现](#3-cjs-的-require-实现)
  - [3.1 手写 30 行的 require 函数——Module._load 全过程逐行拆解](#31-手写-30-行的-require-函数module_load-全过程逐行拆解)
  - [3.2 module.exports vs exports 的关系与陷阱——为什么 exports = {} 会失联](#32-moduleexports-vs-exports-的关系与陷阱为什么-exports---会失联)
  - [3.3 require 的缓存机制与缓存失效——键是什么、何时失效、如何强制刷新](#33-require-的缓存机制与缓存失效键是什么何时失效如何强制刷新)
- [4. ESM 三阶段加载](#4-esm-三阶段加载)
  - [4.1 Parse（解析）：静态分析 import/export 语句——不做任何执行](#41-parse解析静态分析-importexport-语句不做任何执行)
  - [4.2 Instantiate（实例化）：分配模块命名空间 + 绑定 Live Binding——内存槽的创建](#42-instantiate实例化分配模块命名空间--绑定-live-binding内存槽的创建)
  - [4.3 Evaluate（求值）：按依赖后序执行模块体——顶层 await 的阻塞行为](#43-evaluate求值按依赖后序执行模块体顶层-await-的阻塞行为)
- [5. Live Binding vs 值拷贝](#5-live-binding-vs-值拷贝)
  - [5.1 ESM 的导出是「活的引用」——导出方变了，导入方也变](#51-esm-的导出是活的引用导出方变了导入方也变)
  - [5.2 CJS 的导出是值拷贝——导入方拿到的是赋值瞬间的快照](#52-cjs-的导出是值拷贝导入方拿到的是赋值瞬间的快照)
  - [5.3 为什么 ESM 选择 Live Binding——Tree Shaking 与静态分析的前提](#53-为什么-esm-选择-live-bindingtree-shaking-与静态分析的前提)
- [6. 循环依赖：两种策略谁更安全](#6-循环依赖两种策略谁更安全)
  - [6.1 CJS 返回未完成对象的策略——为什么可能拿到 undefined 属性](#61-cjs-返回未完成对象的策略为什么可能拿到-undefined-属性)
  - [6.2 ESM 保证引用可用的策略——Instantiate 阶段就分配好命名空间](#62-esm-保证引用可用的策略instantiate-阶段就分配好命名空间)
  - [6.3 循环依赖的常见场景与三种规避方案](#63-循环依赖的常见场景与三种规避方案)
- [7. CJS/ESM 双轨互操作核心陷阱](#7-cjsesm-双轨互操作核心陷阱)
  - [7.1 __dirname / __filename 缺失——ESM 里怎么拿到文件路径](#71-__dirname--__filename-缺失esm-里怎么拿到文件路径)
  - [7.2 default 导出的双重嵌套 `.default` 地狱——谁在中间多包了一层](#72-default-导出的双重嵌套-default-地狱谁在中间多包了一层)
  - [7.3 双包 hazard：同时存在 `.mjs` `.cjs` 导致的重复加载——两份实例两份状态](#73-双包-hazard同时存在-mjs-cjs-导致的重复加载两份实例两份状态)
  - [7.4 require ESM / import CJS 的限制与绕过——为什么 Node 禁了 require(ESM)](#74-require-esm--import-cjs-的限制与绕过为什么-node-禁了-requireesm)
- [8. import() 动态加载与顶层 await](#8-import-动态加载与顶层-await)
  - [8.1 代码分割的基石——Vite 怎么用 import() 实现按需加载](#81-代码分割的基石vite-怎么用-import-实现按需加载)
  - [8.2 顶层 await 对下游模块的阻塞行为——谁被暂停、为什么暂停](#82-顶层-await-对下游模块的阻塞行为谁被暂停为什么暂停)
  - [8.3 动态 import 的缓存与重载——同样的 URL 第二次 import() 会发生什么](#83-动态-import-的缓存与重载同样的-url-第二次-import-会发生什么)
- [9. npm 包的双模发布实践](#9-npm-包的双模发布实践)
  - [9.1 conditional exports 的 exports 字段配置——Node 的自动路由机制](#91-conditional-exports-的-exports-字段配置node-的自动路由机制)
  - [9.2 同时支持 CJS 和 ESM 消费者的四种方案——选择矩阵](#92-同时支持-cjs-和-esm-消费者的四种方案选择矩阵)
  - [9.3 迁移旧 CJS 包到 ESM 的四步策略——从 require 到 import 的平稳过渡](#93-迁移旧-cjs-包到-esm-的四步策略从-require-到-import-的平稳过渡)
- [10. 综合案例串讲](#10-综合案例串讲)
  - [10.1 案例真相揭晓——五问逐条作答](#101-案例真相揭晓五问逐条作答)
  - [10.2 发布一个同时支持 CJS 和 ESM 的 npm 包——用 conditional exports 实现](#102-发布一个同时支持-cjs-和-esm-的-npm-包用-conditional-exports-实现)
  - [10.3 设计哲学回扣](#103-设计哲学回扣)
  - [10.4 速查表：CJS vs ESM 九维对比 / 双轨互操作全景 / 循环依赖策略 / 60 秒诊断清单](#104-速查表cjs-vs-esm-九维对比--双轨互操作全景--循环依赖策略--60-秒诊断清单)
...
### 2.2 每一步的「为什么需要变」——每一世在解决上一世的什么问题
...
### 3.1 手写 30 行的 require 函数——Module._load 全过程逐行拆解
...
### 3.2 module.exports vs exports 的关系与陷阱——为什么 exports = {} 会失联
...
### 3.3 require 的缓存机制与缓存失效——键是什么、何时失效、如何强制刷新
...
### 4.1 Parse（解析）：静态分析 import/export 语句——不做任何执行
...
### 4.2 Instantiate（实例化）：分配模块命名空间 + 绑定 Live Binding——内存槽的创建
...
### 4.3 Evaluate（求值）：按依赖后序执行模块体——顶层 await 的阻塞行为
...
### 5.2 CJS 的导出是值拷贝——导入方拿到的是赋值瞬间的快照
...
### 5.3 为什么 ESM 选择 Live Binding——Tree Shaking 与静态分析的前提
...
### 6.2 ESM 保证引用可用的策略——Instantiate 阶段就分配好命名空间
...
### 7.1 __dirname / __filename 缺失——ESM 里怎么拿到文件路径
...
### 7.2 default 导出的双重嵌套 `.default` 地狱——谁在中间多包了一层
...
### 7.3 双包 hazard：同时存在 `.mjs` `.cjs` 导致的重复加载——两份实例两份状态
...
### 7.4 require ESM / import CJS 的限制与绕过——为什么 Node 禁了 require(ESM)
...
### 8.2 顶层 await 对下游模块的阻塞行为——谁被暂停、为什么暂停
...
### 8.3 动态 import 的缓存与重载——同样的 URL 第二次 import() 会发生什么
...
### 9.1 conditional exports 的 exports 字段配置——Node 的自动路由机制
...
### 9.2 同时支持 CJS 和 ESM 消费者的四种方案——选择矩阵
