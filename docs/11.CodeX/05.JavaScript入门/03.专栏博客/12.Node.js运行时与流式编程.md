...
#### 目录介绍
- [1. 案例引入](#1-案例引入)
  - [1.1 一个静默 OOM 的服务](#11-一个静默-oom-的服务)
  - [1.2 顺藤摸到根因](#12-顺藤摸到根因)
  - [1.3 我们要回答什么](#13-我们要回答什么)
- [2. 架构概览](#2-架构概览)
  - [2.1 Node.js = V8 + libuv + OpenSSL + zlib + http-parser 的层叠结构](#21-nodejs--v8--libuv--openssl--zlib--http-parser-的层叠结构)
  - [2.2 单线程 JS + 多线程 C++ 的混合模型](#22-单线程-js--多线程-c-的混合模型)
  - [2.3 为什么不是"浏览器事件循环的简化版"](#23-为什么不是浏览器事件循环的简化版)
- [3. libuv 事件循环六阶段](#3-libuv-事件循环六阶段)
  - [3.1 timers → pending → idle/prepare → poll → check → close 六阶段时序](#31-timers--pending--idleprepare--poll--check--close-六阶段时序)
  - [3.2 每阶段的职责与回调执行条件](#32-每阶段的职责与回调执行条件)
  - [3.3 poll 阶段的「饿死」保护机制](#33-poll-阶段的饿死保护机制)
  - [3.4 浏览器 vs Node 事件循环——为什么 4ms 夹持只在浏览器](#34-浏览器-vs-node-事件循环为什么-4ms-夹持只在浏览器)
- [4. process.nextTick vs Promise](#4-processnexttick-vs-promise)
  - [4.1 nextTick 队列在每阶段之间执行](#41-nexttick-队列在每阶段之间执行)
  - [4.2 Promise.then 在本阶段的微任务清空后执行](#42-promisethen-在本阶段的微任务清空后执行)
  - [4.3 nextTick 递归调用导致的事件循环饿死](#43-nexttick-递归调用导致的事件循环饿死)
  - [4.4 完整优先级图谱](#44-完整优先级图谱)
- [5. Stream 四类型](#5-stream-四类型)
  - [5.1 Readable：流动模式 vs 暂停模式 + push/pull 策略](#51-readable流动模式-vs-暂停模式--pushpull-策略)
  - [5.2 Writable：内部缓冲 + highWaterMark](#52-writable内部缓冲--highwatermark)
  - [5.3 Duplex / Transform：双向流与转换流](#53-duplex--transform双向流与转换流)
  - [5.4 objectMode：流式传输 JS 对象](#54-objectmode流式传输-js-对象)
- [6. 背压（Backpressure）](#6-背压backpressure)
  - [6.1 write() 返回 false 的信号机制](#61-write-返回-false-的信号机制)
  - [6.2 drain 事件恢复写入——为什么必须处理](#62-drain-事件恢复写入为什么必须处理)
  - [6.3 pipe() 自动处理背压——但我们还是要知道原理](#63-pipe-自动处理背压但我们还是要知道原理)
  - [6.4 pipeline() vs pipe() 的错误传播差异](#64-pipeline-vs-pipe-的错误传播差异)
- [7. Buffer 的本质](#7-buffer-的本质)
  - [7.1 Uint8Array 的子类 + slab 预分配 + 内存池](#71-uint8array-的子类--slab-预分配--内存池)
  - [7.2 Buffer.alloc vs Buffer.allocUnsafe 的安全差异](#72-bufferalloc-vs-bufferallocunsafe-的安全差异)
  - [7.3 与浏览器 TypedArray 的区别：为什么 Node 需要独立的 Buffer 类](#73-与浏览器-typedarray-的区别为什么-node-需要独立的-buffer-类)
- [8. Cluster 多进程与 Worker Threads](#8-cluster-多进程与-worker-threads)
  - [8.1 master-worker 通信 + 端口共享 + 平滑重启——PM2 的底层原理](#81-master-worker-通信--端口共享--平滑重启pm2-的底层原理)
  - [8.2 Worker Threads：线程共享内存 vs 进程独立内存的优缺点矩阵](#82-worker-threads线程共享内存-vs-进程独立内存的优缺点矩阵)
  - [8.3 child_process.exec vs spawn vs fork 三兄弟的区别](#83-child_processexec-vs-spawn-vs-fork-三兄弟的区别)
- [9. 同构代码设计与双端差异](#9-同构代码设计与双端差异)
  - [9.1 Node 18+ 的 Web API 对齐：fetch / URL / AbortController / crypto](#91-node-18-的-web-api-对齐fetch--url--abortcontroller--crypto)
  - [9.2 SSR/SSG 的双端渲染路径——同一个组件如何跑在两边](#92-ssrssg-的双端渲染路径同一个组件如何跑在两边)
  - [9.3 最常见的 5 个浏览器/Node 行为差异](#93-最常见的-5-个浏览器node-行为差异)
- [10. 综合案例串讲](#10-综合案例串讲)
  - [10.1 案例真相揭晓](#101-案例真相揭晓)
  - [10.2 用 Stream 管道处理 GB 级日志文件——不炸内存的流式分析](#102-用-stream-管道处理-gb-级日志文件不炸内存的流式分析)
  - [10.3 设计哲学回扣](#103-设计哲学回扣)
  - [10.4 速查表：libuv 六阶段 / Stream 四类型 / Buffer 安全分配 / 同构差异](#104-速查表libuv-六阶段--stream-四类型--buffer-安全分配--同构差异)
...
### 2.1 Node.js = V8 + libuv + OpenSSL + zlib + http-parser 的层叠结构
...
### 3.1 timers → pending → idle/prepare → poll → check → close
...
### 3.4 浏览器 vs Node 事件循环——为什么 4ms 夹持只在浏览器
...
### 4.2 Promise.then 在本阶段的微任务清空后执行
...
### 5.2 Writable：内部缓冲 + highWaterMark
...
### 7.1 Uint8Array 的子类 + slab 预分配 + 内存池
...
### 7.2 Buffer.alloc vs Buffer.allocUnsafe 的安全差异
...
### 7.3 与浏览器 TypedArray 的区别：为什么 Node 需要独立的 Buffer 类
...
## 8. Cluster + Worker Threads + child_process

### 8.1 Cluster 多进程——PM2 的底层原理
...
### 10.2 用 Stream 管道处理 GB 级日志文件——不炸内存的流式分析
...
**下一步**：浏览器和 Node 都跑通了。但一个产品不是单文件——模块系统怎么组织？进入 [13.模块系统双轨互操作](13.模块系统双轨互操作.md)。
