...
#### 目录介绍
- [1. 案例引入](#1-案例引入)
  - [1.1 一个点击后 3 秒才响应的按钮——微任务泄漏的典型现场](#11-一个点击后-3-秒才响应的按钮微任务泄漏的典型现场)
  - [1.2 顺藤摸到根因](#12-顺藤摸到根因)
  - [1.3 我们要回答什么](#13-我们要回答什么)
- [2. 架构概览](#2-架构概览)
  - [2.1 浏览器事件循环完整模型——一个 tick 的精确解剖](#21-浏览器事件循环完整模型一个-tick-的精确解剖)
  - [2.2 为什么微任务必须在宏任务之间全部清空——而不是一次只执行一个](#22-为什么微任务必须在宏任务之间全部清空而不是一次只执行一个)
- [3. 宏任务队列——浏览器的多源任务调度](#3-宏任务队列浏览器的多源任务调度)
  - [3.1 script / setTimeout / setInterval / I/O / UI 事件 / postMessage 六种来源的精确差异](#31-script--settimeout--setinterval--io--ui-事件--postmessage-六种来源的精确差异)
  - [3.2 不同宏任务源的优先级差异——postMessage 为什么比 setTimeout 快](#32-不同宏任务源的优先级差异postmessage-为什么比-setTimeout-快)
  - [3.3 浏览器 vs Node.js 的宏任务调度差异——为什么它们是两套完全不同的体系](#33-浏览器-vs-nodejs-的宏任务调度差异为什么它们是两套完全不同的体系)
- [4. 微任务队列——V8 在每段 JS 执行后的检查点](#4-微任务队列v8-在每段-js-执行后的检查点)
  - [4.1 Promise.then/catch/finally——ECMA-262 §27.2 定义的微任务注册机制](#41-promisethencatchfinallyecma-262-272-定义的微任务注册机制)
  - [4.2 MutationObserver 的微任务触发时机——为什么 DOM 变更被"批量"通知](#42-mutationobserver-的微任务触发时机为什么-dom-变更被批量通知)
  - [4.3 queueMicrotask vs Promise.resolve().then——同一个队列，不同的创建开销](#43-queuemicrotask-vs-promiseresolvethen同一个队列不同的创建开销)
- [5. 手写 Promise/A+ 规范逐条实现（上）——状态机与核心链路](#5-手写-promisea-规范逐条实现上状态机与核心链路)
  - [5.1 构造函数 + 三种状态：pending/fulfilled/rejected——为什么状态只能变一次](#51-构造函数--三种状态pendingfulfilledrejected为什么状态只能变一次)
  - [5.2 then 的链式调用——返回值自动包装为 Promise + 回调必须异步执行](#52-then-的链式调用返回值自动包装为-promise--回调必须异步执行)
  - [5.3 递归解析 thenable——Promise/A+ §2.3 的完整解析算法](#53-递归解析-thenablepromisea-23-的完整解析算法)
- [6. 手写 Promise/A+ 规范逐条实现（下）——静态方法与测试](#6-手写-promisea-规范逐条实现下静态方法与测试)
  - [6.1 catch / finally 的实现——finally 为什么不能"吞掉"前一个 Promise 的值](#61-catch--finally-的实现finally-为什么不能吞掉前一个-promise-的值)
  - [6.2 all / race / any / allSettled / resolve / reject——短路与不短路的语义差异](#62-all--race--any--allsettled--resolve--reject短路与不短路的语义差异)
  - [6.3 A+ 测试套件跑通完整实现——872 条用例覆盖了什么边界条件](#63-a-测试套件跑通完整实现872-条用例覆盖了什么边界条件)
- [7. Promise 的六大反模式——生产代码中的常见陷阱](#7-promise-的六大反模式生产代码中的常见陷阱)
  - [7.1 构造器反模式 + 忘记 return 的链断裂](#71-构造器反模式--忘记-return-的链断裂)
  - [7.2 嵌套地狱 vs 扁平链 + 并行不用 all + 不处理 rejection + forEach+async](#72-嵌套地狱-vs-扁平链--并行不用-all--不处理-rejection--foreachasync)
- [8. async/await 的本质——Generator + Promise 的语法封装](#8-asyncawait-的本质generator--promise-的语法封装)
  - [8.1 Generator + Promise 的脱糖结果——Babel playground 逐行对照](#81-generator--promise-的脱糖结果babel-playground-逐行对照)
  - [8.2 await 的微任务插入时机——为什么 await null 后面的代码变成了微任务](#82-await-的微任务插入时机为什么-await-null-后面的代码变成了微任务)
  - [8.3 for…of + await vs forEach + async——事件循环视角的根因分析](#83-forof--await-vs-foreach--async事件循环视角的根因分析)
- [9. 错误处理与并发控制——Promise 的正确使用范式](#9-错误处理与并发控制Promise-的正确使用范式)
  - [9.1 try/catch 能捕获 reject 吗——async 与非 async 上下文的差异](#91-trycatch-能捕获-reject-吗async-与非-async-上下文的差异)
  - [9.2 五个并发方法的精确语义矩阵——all / race / allSettled / any 的区别](#92-五个并发方法的精确语义矩阵all--race--allsettled--any-的区别)
  - [9.3 三层错误防护策略——从单链到全局兜底的纵深防御](#93-三层错误防护策略从单链到全局兜底的纵深防御)
- [10. 综合案例串讲](#10-综合案例串讲)
  - [10.1 案例真相揭晓——七个疑问逐条解答](#101-案例真相揭晓七个疑问逐条解答)
  - [10.2 实现一个支持重试/超时/并发限制的 requestManager](#102-实现一个支持重试超时并发限制的-requestmanager)
  - [10.3 设计哲学回扣](#103-设计哲学回扣)
  - [10.4 速查表：事件循环四阶段 / Promise 六反模式 / 五种并发方法 / async 脱糖对照](#104-速查表事件循环四阶段--promise-六反模式--五种并发方法--async-脱糖对照)

---

## 1. 案例引入

### 1.1 一个点击后 3 秒才响应的按钮——微任务泄漏的典型现场
...
### 2.2 为什么微任务必须在宏任务之间全部清空——而不是一次只执行一个
...
### 3.1 script / setTimeout / setInterval / I/O / UI 事件 / postMessage 六种来源的精确差异
...
### 3.2 不同宏任务源的优先级差异——postMessage 为什么比 setTimeout 快
...
### 3.3 浏览器 vs Node.js 的宏任务调度差异——为什么它们是两套完全不同的体系
...
## 4. 微任务队列——V8 在每段 JS 执行后的检查点

### 4.1 Promise.then/catch/finally——ECMA-262 §27.2 定义的微任务注册机制
...
### 4.2 MutationObserver 的微任务触发时机——为什么 DOM 变更被"批量"通知
...
### 4.3 queueMicrotask vs Promise.resolve().then——同一个队列，不同的创建开销
...
## 5. 手写 Promise/A+ 规范逐条实现（上）——状态机与核心链路

### 5.1 构造函数 + 三种状态：pending/fulfilled/rejected——为什么状态只能变一次
...
### 5.2 then 的链式调用——返回值自动包装为 Promise + 回调必须异步执行
...
### 5.3 递归解析 thenable——Promise/A+ §2.3 的完整解析算法
...
## 6. 手写 Promise/A+ 规范逐条实现（下）——静态方法与测试

### 6.1 catch / finally 的实现——finally 为什么不能"吞掉"前一个 Promise 的值
...
### 6.2 all / race / any / allSettled / resolve / reject——短路与不短路的语义差异
...
### 6.3 A+ 测试套件跑通完整实现——872 条用例覆盖了什么边界条件
...
## 7. Promise 的六大反模式——生产代码中的常见陷阱

### 7.1 构造器反模式 + 忘记 return 的链断裂
...
### 7.2 嵌套地狱 vs 扁平链 + 并行不用 all + 不处理 rejection + forEach+async
...
## 8. async/await 的本质——Generator + Promise 的语法封装

### 8.1 Generator + Promise 的脱糖结果——Babel playground 逐行对照
...
### 8.2 await 的微任务插入时机——为什么 await null 后面的代码变成了微任务
...
### 8.3 for…of + await vs forEach + async——事件循环视角的根因分析
...
## 9. 错误处理与并发控制——Promise 的正确使用范式

### 9.1 try/catch 能捕获 reject 吗——async 与非 async 上下文的差异
...
### 9.2 五个并发方法的精确语义矩阵——all / race / allSettled / any 的区别
...
### 10.2 实现一个支持重试/超时/并发限制的 requestManager
...
**下一步**：事件循环统一了主线程的调度。但 JS 不止一个线程——Worker 和并发即将展开。进入 [09.Worker 并发与调度时钟](09.Worker并发与调度时钟.md)。
