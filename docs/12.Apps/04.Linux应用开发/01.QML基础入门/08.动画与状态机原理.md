...
- [8.2 引擎原理](#82-引擎原理)
    - [8.2.1 插值引擎](#821-插值引擎)
    - [8.2.2 缓动曲线](#822-缓动曲线)
    - [8.2.3 动画绑定协作](#823-动画绑定协作)
- [8.3 属性动画体系](#83-属性动画体系)
    - [8.3.1 三种基础动画](#831-三种基础动画)
    - [8.3.2 串行与并行](#832-串行与并行)
    - [8.3.3 命令式控制](#833-命令式控制)
- [8.4 Behavior 动画](#84-behavior-动画)
    - [8.4.1 拦截机制](#841-拦截机制)
    - [8.4.2 多属性组合](#842-多属性组合)
- [8.5 状态机](#85-状态机)
    - [8.5.1 声明式状态](#851-声明式状态)
    - [8.5.2 过渡动画](#852-过渡动画)
    - [8.5.3 折叠面板](#853-折叠面板)
- [8.6 GPU 与 CPU](#86-gpu-与-cpu)
    - [8.6.1 属性路径分类](#861-属性路径分类)
    - [8.6.2 弹性动画](#862-弹性动画)
    - [8.6.3 Shader 动画](#863-shader-动画)
- [8.7 仪表盘动效](#87-仪表盘动效)
- [8.8 性能与陷阱](#88-性能与陷阱)
- [8.9 新手陷阱](#89-新手陷阱)
- [8.10 训练题](#810-训练题)
...
1. JS 是**单线程**——一次 GC 卡 200ms → 动画停 200ms
2. JS 到 C++ 的每次调用都有开销——60fps × 每帧插值 → 每秒 60 次 JS→C++ 跳转
3. `setInterval` 的时钟精度依赖 event loop——GUI 线程忙时定时器会漂移
