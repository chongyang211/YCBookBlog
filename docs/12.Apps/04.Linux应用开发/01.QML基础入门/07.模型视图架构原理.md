...
#### 目录介绍
- [7.1 案例引入](#71-案例引入)
  - [7.1.1 5000 条 CAN 报文列表的离奇卡顿](#711-5000-条-can-报文列表的离奇卡顿)
  - [7.1.2 揭开谜底：ListModel 在 JS 堆里创建了 5000 个 QObject](#712-揭开谜底listmodel-在-js-堆里创建了-5000-个-qobject)
  - [7.1.3 本章要回答的三个问题](#713-本章要回答的三个问题)
- [7.2 Model / View / Delegate 三角色](#72-model--view--delegate-三角色)
  - [7.2.1 三角色职责边界](#721-三角色职责边界)
  - [7.2.2 数据流的完整路径](#722-数据流的完整路径)
  - [7.2.3 内置数据模型全景](#723-内置数据模型全景)
- [7.3 ListView 视口渲染原理](#73-listview-视口渲染原理)
  - [7.3.1 delegate 复用池机制](#731-delegate-复用池机制)
  - [7.3.2 cacheBuffer 与预渲染](#732-cachebuffer-与预渲染)
  - [7.3.3 综合案例：5000 条 CAN 报文实时列表](#733-综合案例5000-条-can-报文实时列表)
- [7.4 GridView / PathView / TableView](#74-gridview--pathview--tableview)
- [7.5 C++ 自定义 QAbstractListModel](#75-c-自定义-qabstractlistmodel)
  - [7.5.1 roleNames 与数据角色映射](#751-rolenames-与数据角色映射)
  - [7.5.2 分批插入与性能优化](#752-分批插入与性能优化)
  - [7.5.3 线程安全的 Model 更新](#753-线程安全的-model-更新)
- [7.6 综合实战：万级 CAN 报文监控面板](#76-综合实战万级-can-报文监控面板)
- [7.7 性能与陷阱](#77-性能与陷阱)
- [7.8 本章新手陷阱 Top 5](#78-本章新手陷阱-top-5)
- [7.9 训练题](#79-训练题)
...
### 7.1.2 揭开谜底：ListModel 在 JS 堆里创建了 5000 个 QObject
...
## 7.4 GridView / PathView / TableView
...
**案例知识融合**：本案例整合了整章的核心——①`QAbstractListModel` 线程安全接收 CAN 数据；②`reuseItems` 确保万级数据只创建 ~20 个 delegate；③`TextField` 过滤 `canModel.setIdFilter()` 调用 C++ 侧过滤，不重建 delegate；④工具栏"暂停/播放/清空"直接操作 C++ model 方法。
