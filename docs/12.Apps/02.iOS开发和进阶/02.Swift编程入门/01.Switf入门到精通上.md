---
title: Switf入门到精通上
date: 2025-02-28 15:12:31
permalink: /pages/ios-swift/
categories:
  - book
  - iOS开发和进阶
  - Swift编程入门
...
`guard` 不仅仅是非空判断，它在 Swift 项目中有三个高频使用场景：

**场景一：函数入口参数校验。** 这是 guard 最常见的用法，提前过滤无效输入。
...
**场景二：可选值安全解包。** 相比于 `if let` 的嵌套地狱，`guard let` 让主线代码更扁平。
...
**场景三：前置条件断言。** 确保方法在正确的状态下执行，比 `if` 更清晰地表达"不满足条件就不该到这里"的语义。

## 3.5 闭包的捕获列表与内存管理

闭包是引用类型，会强引用所有捕获的外部变量。当闭包被对象长期持有时（如 completion handler、回调属性），极容易造成循环引用。

**捕获列表的语法**：在闭包体开头用 `[weak self]` 或 `[unowned self]` 声明弱引用。
...
**weak vs unowned 的选择原则**：
| 修饰符 | 可选性 | 适用场景 |
|--------|:--:|------|
| `weak` | Optional，自动置 nil | 闭包执行期间 self 可能已被释放 |
| `unowned` | 非 Optional，不置 nil | 闭包和 self 生命周期相同，self 必存在 |

**实用口诀**：99% 的情况下用 `[weak self]`，只有当你 100% 确定 self 不会被提前释放时才用 `unowned`。用错 `unowned` 会直接 crash。

## 3.6 集合的高阶函数

Swift 为 Array、Set、Dictionary 提供了一套强大的函数式操作方法，熟练掌握能大幅减少代码量。
...
**reduce**：将集合聚合成一个值（累加、拼接等）。
```
let sum = numbers.reduce(0) { $0 + $1 } // 10
let joined = ["Hello", "World"].reduce("") { $0 + " " + $1 } // " Hello World"
```

**compactMap**：过滤 nil 并解包。
```
let strings = ["1", "2", "abc", "3"]
let validNumbers = strings.compactMap { Int($0) } // [1, 2, 3]
```

**链式调用**：组合多个高阶函数一行完成复杂操作。
```
