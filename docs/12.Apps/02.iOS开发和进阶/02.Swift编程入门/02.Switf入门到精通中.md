---
title: Switf入门到精通中
date: 2025-04-10 14:22:06
permalink: /pages/ea6054/
categories:
  - book
  - iOS开发和进阶
  - Swift编程入门
...
结构体和类是 Swift 中最常需要做选择的两个类型。它们的关键区别不是语法，是**值语义 vs 引用语义**。

| 特性 | 结构体 | 类 |
|------|--------|-----|
| 类型 | 值类型（拷贝传递） | 引用类型（引用传递） |
| 继承 | 不可继承 | 可继承 |
| 内存 | 栈上分配 | 堆上分配，ARC 管理 |
| 线程安全 | 天然线程安全（每次拷贝独立） | 需额外同步 |
...
**实用原则**：优先用结构体，除非你明确的理由需要类。Swift 标准库中 String、Array、Dictionary 全是结构体，这是 Apple 的最佳实践示范。

## 6.7 属性观察器的使用场景

属性观察器 `willSet` 和 `didSet` 让你在属性值改变前后执行自定义代码，是替代 KVO 的轻量方案。

```
class TemperatureController {
    var temperature: Double = 0 {
        willSet(newTemp) {
            print("温度即将变为 \(newTemp)°C")
        }
        didSet {
            if temperature > 100 {
                print("⚠️ 温度过高！已从 \(oldValue)°C 升至 \(temperature)°C")
            }
        }
    }
}
...
controller.temperature = 120
// 输出：温度即将变为 120.0°C
// 输出：⚠️ 温度过高！已从 0.0°C 升至 120.0°C
```

**常见使用场景**：UI 数据源变化后自动 reload、模型数据变化后自动保存到缓存、播放进度条自动更新。

**注意**：构造器中设置属性不会触发 willSet/didSet，要在初始化后单独赋值才会触发。

## 6.8 枚举的高级用法

**关联值（Associated Values）**：每个 case 可以携带不同类型和数量的值，非常适合表示状态机。
...
func handle(result: NetworkResult) {
    ...
    case .success(let data, let code):
        print("成功: code \(code), size \(data.count) bytes")
...
**原始值（Raw Values）**：每个 case 绑定固定的值，适合映射常量。
