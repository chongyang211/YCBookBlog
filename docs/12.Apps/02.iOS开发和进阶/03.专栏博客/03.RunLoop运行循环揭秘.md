...
> **下一篇**：你已经知道 RunLoop 如何在每个周期里调度触摸、Timer、动画和绘图，下一章进入 [04.UIView生命周期与渲染](04.UIView生命周期与渲染.md)——把 "一次 `setNeedsDisplay` 从调用到像素亮起来的完整链路" 从 CoreAnimation → Render Server → GPU 级别彻底剖开。
