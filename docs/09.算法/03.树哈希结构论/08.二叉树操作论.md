...
线性结构详解逼不出"查找、插入、删除全都 O(log N)"这种均衡；**树通过分层** 把这三件事捏在一起。

> **底层原因**：线性结构详解只能"线性扫描或有序对半"——前者插入快、查找慢；后者反过来。**树把"有序"沿层次摊开**，让任何一次操作都只走一条 log N 的路径，从而同时拿到三方面的均衡。
...
- 《数据库系统实现》（Garcia-Molina, Ullman, Widom）第 13 章：B+ 树哈希结构论索引
