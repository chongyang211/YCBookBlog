...
> 🔗 **延伸阅读**：← [3.13 协程核心设计思想](https://yccoding.com/pages/5ebc69/) · ← [3.9 锁核心设计和思想](https://yccoding.com/pages/db7194/) · → [3.15 线程池的设计思想](https://yccoding.com/pages/2b1ae7/) · → [3.18 结构化并发设计思想](https://yccoding.com/pages/218459/) · → [5.x 消息机制设计思想](..../06.交互和系统/)
...
- [02.共享内存尽头](#02共享内存尽头)
  - [2.1 多核诅咒](#21-多核诅咒)
  - [2.2 四大不可能](#22-四大不可能)
  - [2.3 不共享行不行](#23-不共享行不行)
  - [2.4 五十年后赢](#24-五十年后赢)
- [03.历史演进](#03历史演进)
  - [3.1 历史时间线](#31-历史时间线)
  - [3.2 同源异流](#32-同源异流)
  - [3.3 身份vs通道](#33-身份vs通道)
  - [3.4 背压机制](#34-背压机制)
- [04.Actor模型](#04actor模型)
  - [4.1 三条公理](#41-三条公理)
  - [4.2 Erlang实现](#42-erlang实现)
  - [4.3 Akka实现](#43-akka实现)
  - [4.4 监督树](#44-监督树)
  - [4.5 分布式透明](#45-分布式透明)
- [05.CSP模型](#05csp模型)
  - [5.1 会合同步](#51-会合同步)
  - [5.2 Go实现](#52-go实现)
  - [5.3 Select多路](#53-select多路)
  - [5.4 Pipeline](#54-pipeline)
  - [5.5 Fan-out/Fan-in](#55-fan-outfan-in)
- [06.六语言对比](#06六语言对比)
  - [6.1 横评表](#61-横评表)
  - [6.2 同题五写法](#62-同题五写法)
  - [6.3 决策树](#63-决策树)
...
- [07.物理实现](#07物理实现)
  - [7.1 邮箱实现](#71-邮箱实现)
  - [7.2 Channel实现](#72-channel实现)
  - [7.3 调度器对比](#73-调度器对比)
  - [7.4 有界vs无界](#74-有界vs无界)
- [08.陷阱与反模式](#08陷阱与反模式)
  - [8.1 邮箱堆积](#81-邮箱堆积)
  - [8.2 Channel死锁](#82-channel死锁)
  - [8.3 消息次序](#83-消息次序)
  - [8.4 Actor内阻塞](#84-actor内阻塞)
  - [8.5 锁用Channel](#85-锁用channel)
  - [8.6 循环死锁](#86-循环死锁)
  - [8.7 Goroutine泄漏](#87-goroutine泄漏)
- [09.跨语言陷阱速查](#09跨语言陷阱速查)
- [10.真言映射](#10真言映射)
- [11.总结](#11总结)
  - [11.1 认知阶梯](#111-认知阶梯)
  - [11.2 消息派四件套](#112-消息派四件套)
  - [11.3 七字真言](#113-七字真言)
  - [11.4 下篇承接](#114-下篇承接)
  - [11.5 设计哲学](#115-设计哲学)
...
根因是**缓存一致性协议（MESI）的代价**——详见 [3.6 并发 Bug 源头由来](https://yccoding.com/pages/57fd8a/)。每次 `count++`：
...
**疑惑**：Actor 和 CSP 都讲"发消息"，但生产者和消费者速度不匹配时——谁负责减速？

**论证**，背压（Backpressure）在两种模型中的实现完全不同：

**Actor 的背压——邮箱有界 + 封锁发送方**：

```scala
// Akka 有界邮箱：满了就阻止发送
bounded-mailbox { mailbox-capacity = 10000; mailbox-push-timeout-time = 100ms }
// 超时后 → 发送方收到超时异常 → 自行决定重试/降级
```

**CSP 的背压——channel 天然同步**：

```go
ch := make(chan int, 100)  // 有缓冲
ch <- val   // 缓冲满 → 发送阻塞 → 背压自动传播到上游
```

**两种模型的背压对比**：

```
Actor：异步默认 → 背压需要"后装"（有界邮箱 + 超时策略）
CSP：  同步默认 → 背压是"出厂自带"（channel 满了自然阻塞）
```

**背压是分布式系统韧性的核心**——没有背压的消息系统，峰值流量会无衰减地传导到最底层，直到 OOM 崩溃。§1.1 的事故就是"无背压 Actor"的典型案例。
...
**Go 的 GMP 调度器**（详见 [3.13 协程核心设计思想](https://yccoding.com/pages/5ebc69/)）：
...
**疑惑**：为什么 Erlang 默认无界，现代 Akka 推荐有界？

**论证**，两种设计的背景不同。**电信场景**（Erlang 诞生地）：Actor 处理微秒级、密度可预测 → 无界安全。**互联网场景**（Akka 的现实）：下游响应不可预测 → 无界 = OOM 定时炸弹（§1.1 的 16GB 事故）。**Go channel** 则用容量参数显式声明"最多容忍多少积压"。**结论**：有界 vs 无界不是技术选择，是工程信任模型——可控场景无界，不可控场景必须有界。
...
下一篇 [3.15 线程池的设计思想](https://yccoding.com/pages/2b1ae7/) 我们会深入这个主题——**为什么池化是工程最古老也最有效的优化模式之一**。

### 11.5 设计哲学回扣

**哲学 1：不共享是解决并发问题的根源**

锁体系的四大不可能根源于**共享状态**。Actor 和 CSP 走的是同一条路——**把并发从"状态管理"问题转化为"通信设计"问题**。这不是技术升级，是问题域的转换。

**哲学 2：消息传递让分布式成为"天然"而非"附加"**

共享内存的锁在跨机器时完全失效；消息却天然可序列化、可跨网络。**Actor/CSP 在设计之初就没有"单机假设"**——这是它们在云原生时代爆发的根本原因。

**哲学 3：监督树重新定义可靠性**

"Let it crash"不是放弃错误处理，是把**错误处理从应用逻辑中剥离**——Worker 只管崩溃，Supervisor 只管恢复。这种职责分离让系统韧性提升了数个量级。

**哲学 4：背压是消息系统的韧性基石**

§1.1 的事故和 §3.4 的分析共同指向一个结论——**没有背压的消息系统在高负载下必然雪崩**。Actor 需要"后装"有界邮箱，CSP 的同步 channel 是"出厂自带"背压。
...
- 同卷上篇：[3.13 协程核心设计思想](https://yccoding.com/pages/5ebc69/) ｜ [3.9 锁核心设计和思想](https://yccoding.com/pages/db7194/)
- 同卷下篇：[3.15 线程池的设计思想](https://yccoding.com/pages/2b1ae7/) ｜ [3.18 结构化并发设计思想](https://yccoding.com/pages/218459/)
- 第 5 卷视角：[5.x 消息机制设计思想](..../06.交互和系统/)（事件循环 + 消息队列的实战）
- 经典文献：
