...
> 🔗 **延伸阅读**：← [18.锁核心设计和思想](https://yccoding.com/pages/db7194/) · → [23.协程核心设计思想](https://yccoding.com/pages/5ebc69/) · → [25.线程池的设计思想](https://yccoding.com/pages/2b1ae7/)
> 📣 **语言无关声明**：本篇是**跨语言通用篇**——上下文切换的"为什么 / 切什么 / 多贵 / 如何省"都是 OS / CPU 层面的物理事实，对 **Java / C / C++ / C# / Go / Rust / Kotlin / Python / JavaScript / Swift** 全部成立。各语言的差异只是**调度发生在哪一层**（内核 / Runtime / 编译期），切换的"动作清单"几乎一样——保存寄存器、切栈、刷新缓存、恢复执行。读完本篇，你看任何语言的并发性能问题都会有 X 光视角。

### 🧭 通用三问

**Q1：为什么必须切换？**——物理 CPU 数永远 < 逻辑任务数。要在 N 个核上跑 M 个任务（M >> N），就必须**时分复用**——一个任务跑一会儿，保存状态，让另一个任务跑。"切换"这件事，本质是用**时间换空间**：用 μs 级的状态保存代价，换 N 个任务"看起来同时在跑"的假象。

**Q2：切换到底要做什么？**——三件事，缺一不可：①**保存**当前执行流的状态（PC / 寄存器 / 栈指针 / 浮点状态）；②**切换**地址空间（如果跨进程，要换 CR3，TLB 全部失效）；③**恢复**目标执行流的状态。这三步在所有语言、所有 OS 上都一样——只是"状态"的颗粒度不同（OS 线程切几十个寄存器，goroutine 切几个 callee-saved，async 状态机只动一个枚举字段）。

**Q3：切换的真实代价是什么？**——表面上是几 μs 的指令开销，**真实代价 90% 来自"切换之后"**：L1/L2/L3 缓存全冷、TLB 全空、分支预测器记录失效，IPC（每周期指令数）会瞬间掉到平时的 1/10。这就是为什么"切换次数"比"切换时长"更值得关注——**一次切换的余波长达数百微秒**。

🎯 **七字真言**：**触发 / 保存 / 换栈 / 跨空间 / 失效 / 恢复 / 余波**——切换的全部内幕都在这七个字里。本篇末尾会把它映射到 5 种主流语言，你会发现"切换"的骨架是同一个。
...
- [2.两类切换](#2两类切换)
  - [2.1 进程级切换](#21-进程级切换)
  - [2.2 线程级切换](#22-线程级切换)
  - [2.3 切换完整流程](#23-切换完整流程)
- [3.切换设计思想](#3切换设计思想)
  - [3.1 设计演进](#31-设计演进)
  - [3.2 抢占vs协作](#32-抢占vs协作)
  - [3.3 调度算法演进](#33-调度算法演进)
- [4.切换核心原理](#4切换核心原理)
  - [4.0 通用三段式](#40-通用三段式)
  - [4.1 切换时机](#41-切换时机)
  - [4.2 切换硬件支持](#42-切换硬件支持)
  - [4.3 内核切换伪码](#43-内核切换伪码)
- [5.各语言差异](#5各语言差异)
  - [5.1 C/C++实现](#51-cc实现)
...
- [6.切换优化实践](#6切换优化实践)
  - [6.1 减少切换策略](#61-减少切换策略)
  - [6.2 降低开销技术](#62-降低开销技术)
  - [6.3 各平台监控法](#63-各平台监控法)
- [7.协程与无栈](#7协程与无栈)
  - [7.1 有栈vs无栈](#71-有栈vs无栈)
  - [7.2 各语言协程对比](#72-各语言协程对比)
  - [7.3 协程切换伪码](#73-协程切换伪码)
- [8.观测与调优](#8观测与调优)
  - [8.1 真实成本测算](#81-真实成本测算)
  - [8.2 切换风暴工具](#82-切换风暴工具)
  - [8.3 切换风暴模式](#83-切换风暴模式)
  - [8.4 容器化特殊问题](#84-容器化特殊问题)
  - [8.5 调优三原则](#85-调优三原则)
  - [8.6 切换不可避免](#86-切换不可避免)
- [9.生产事故看切换](#9生产事故看切换)
  - [9.1 5000线程反降](#91-5000线程反降)
  - [9.2 Redis单线程胜](#92-redis单线程胜)
  - [9.3 goroutine百万化](#93-goroutine百万化)
- [10.切换成本全景](#10切换成本全景)
- [11.五语言切换全景](#11五语言切换全景)
- [12.一句话总结](#12一句话总结)
...
> 这一节是 §4 的母图。看清这张"三段式"，你再读任何 OS / 语言运行时的源码（Linux `__switch_to`、Go `runtime.gogo`、Kotlin 协程 `resumeWith`），都会发现"骨架完全一致，只是颗粒度不同"。

```mermaid
flowchart TD
    A[切换触发<br/>时钟中断 / 阻塞调用 / 主动让出] --> B[第 1 段: 保存现场]
    B --> B1[把寄存器组写入 TCB/G/状态机]
    B1 --> B2[把当前 SP 写入控制块]
    B2 --> C[第 2 段: 切换执行流]
    C --> C1{跨进程?}
    C1 -->|是| D1[换 CR3 → TLB 全部失效]
    C1 -->|否| D2[同进程仅换栈指针]
    D1 & D2 --> E[第 3 段: 恢复目标现场]
    E --> E1[从新 TCB 加载寄存器]
    E1 --> E2[跳转到新 PC 继续执行]
    E2 --> F[隐形余波<br/>L1/L2 全冷 · 分支预测失效<br/>IPC 暴跌 100-1000 μs]
    style F fill:#f8d7da
    style E fill:#d4edda
```
...
| 切换类型 | 第 1 段（保存） | 第 2 段（切换） | 第 3 段（恢复） | 总成本 | 典型代表 |
|---|---|---|---|---|---|
| **进程切换** | 全部寄存器 + FPU/SSE + 内核栈 | 换 CR3、TLB 全空、L1 D-cache 几乎全失效 | 全部寄存器 + 重新分页 | 5-10 μs + 长余波 | Linux `__switch_to` 跨 mm |
| **线程切换（同进程）** | 全部寄存器 + FPU | 换 SP，不换 CR3，TLB 部分保留 | 全部寄存器恢复 | 1-3 μs + 中余波 | pthread / Java 平台线程 |
| **协程切换（用户态）** | callee-saved 寄存器（约 8 个） | 仅换栈指针，**完全不进内核** | callee-saved 恢复 | 100-300 ns + 短余波 | Go goroutine / Loom / Kotlin |
| **async 状态机** | 把局部变量写入堆上的 Future/Promise | 函数返回，控制权交回 Runtime | Future 被 poll → 跳进 match 分支 | 10-50 ns + 几乎无余波 | Rust async / JS Promise / C# Task |

**核心洞察**：

1. **三段式不可逾越**——再轻量的切换也要做"保存 / 换流 / 恢复"。`async` 之所以快，是因为它**把"全部寄存器"压缩到了"几个状态字段"**，而不是省略了保存。
2. **跨进程切换比跨线程贵 3-5 倍**——主要不在指令开销，而在 **TLB 全部失效后页表 walk 的代价**和 **L1 cache 重建**。这就是"线程比进程轻"的物理根源。
3. **"切换之后"比"切换本身"更值得关注**——很多线上事故的真正诱因不是"切了多少 μs"，而是"切完之后缓存全冷 50-100 μs 内 IPC 暴跌 90%"。优化思路：**把切换集中到少数线程上**（Reactor 模式 / Worker Pool），让大多数线程"切完之后能持续跑很久"。

> 💡 **设计哲学**：上下文切换是**操作系统给"并发"这个抽象付的税**——你逃不掉，只能选择"什么档位的税"。从 OS 进程切换（最贵）到 Rust async（几乎免费），整个并发演化史就是**"如何把切换的颗粒度做小"**的历史。
...
> 开篇的七字真言——**触发 / 保存 / 换栈 / 跨空间 / 失效 / 恢复 / 余波**——是切换的"骨架"。下面把它映射到 5 种主流语言，你会发现"骨架完全一致，只是颗粒度不同"。

| 七字 | 通用本质 | C / POSIX 线程 | Java 平台线程 | Go goroutine | Kotlin 协程 | Rust async |
|---|---|---|---|---|---|---|
| **触发** | 何时发生切换 | 时钟中断 / 阻塞 syscall / `pthread_yield` | 时钟中断 / 阻塞 IO / `Thread.yield` | sysmon 抢占 / channel 阻塞 / `runtime.Gosched()` | `suspendCancellableCoroutine` / `delay` | `.await`（永远显式） |
| **保存** | 写哪些寄存器 | 全部通用 + FPU + 内核栈 | 同 C（=OS 线程） | 仅 callee-saved（约 8 个） | 编译期生成：把局部变量写状态机字段 | 同 Kotlin（编译期状态机） |
| **换栈** | 改 SP 指向哪 | 内核 `__switch_to` 改 RSP | 走 OS 切换 | `runtime.gogo` 改 G 栈 SP | 没"栈"概念，换 continuation 引用 | 同 Kotlin |
| **跨空间** | 是否换 CR3 | 跨进程才换，同进程不换 | 同 C | 永远不换（在 OS 线程内调度） | 永远不换 | 永远不换 |
| **失效** | 哪些缓存被冲刷 | TLB（跨进程）+ L1/L2 | 同 C | 几乎不冲（仍在同 OS 线程） | 不冲 | 不冲 |
| **恢复** | 跳到哪继续 | 内核栈返回用户态 | 同 C | `JMP` 到 G 保存的 PC | 调用 continuation.resumeWith() | 同 Kotlin（match arm） |
| **余波** | 切完之后 IPC 多久回升 | 100-500 μs（缓存重建） | 同 C | < 10 μs（缓存大概率仍热） | 几乎无 | 几乎无 |

**横向看一行——例如"保存"**：你会发现五种语言只在做**同一件事**（把"恢复执行所需的状态"存下来），区别只是"状态有多大"——从全部寄存器到几个状态字段，**颗粒度小了 100 倍，性能就提升 100 倍**。

**纵向看一列——例如 Go**：你会发现 goroutine 不是魔法，它只是把 OS 线程切换的"七字流程"**搬到了用户态，且永远不跨地址空间**——所以省掉了"跨空间 / 失效"两段最贵的代价，从 μs 级降到 ns 级。
