...
- [11.演化史与总结](#11演化史与总结)
  ...
  - [11.5 现代融合趋势](#115-现代融合趋势)
  - [11.6 通信设计回扣](#116-通信设计回扣)
...
**疑惑**：既然共享内存派"性能高但易翻"、消息传递派"易写但有拷贝"，那未来是二选一，还是会融合？

**论证**：过去 10 年，主流语言实际走的是**"上层消息化 + 底层共享化"** 的融合路线：

```mermaid
flowchart TD
    A[写法层<br/>看起来像消息传递] --> A1[Java Virtual Thread + StructuredTaskScope]
    A --> A2[Kotlin Flow / Channel]
    A --> A3[Rust async + Tokio channel]
    A --> A4[Swift Task / AsyncSequence]

    B[执行层<br/>跑起来像共享内存] --> B1[同 JVM 堆 / 同地址空间]
    B --> B2[零拷贝 / 引用传递]
    B --> B3[缓存友好 / 亲和调度]
...
1. **写法上不见锁**：你几乎不需要写 `synchronized`、`Mutex`，只需要 `send`/`receive`、`await`/`emit`；
2. **执行上不真拷贝**：channel 内部传的是引用/句柄，Java 虚拟线程之间共享同一个堆；
3. **正确性靠"结构"而非"纪律"**：结构化并发、类型系统、作用域绑定，把"通信正确性"下沉到编译器和运行时。

**结论**：**未来通信不是"选择共享还是消息"，而是"写法用消息，执行用共享"**。这就是"**Do not communicate by sharing memory; instead, share memory by communicating**"这句 Go 名言的真正含义 —— 它并没有排斥共享内存，只是把"共享"藏到了 channel 的实现里。

### 11.6 通信设计回扣

**疑惑**：写完这一整篇，业务代码里到底该怎么选通信原语？

**论证**：**只需要问自己三个问题**：

1. **两个执行流的"生命周期归属"是否明确？**
   - 是 → 用**结构化并发**（scope/TaskGroup），异常和取消自动传播；
   - 否 → 用**外置通道**（BlockingQueue/channel/MQ），把生命周期解耦。

2. **数据的"所有权归属"是否清晰？**
   - 清晰（只有一个写者）→ 用**无锁数据结构 / atomic / SPSC 队列**；
   - 不清晰（多写者）→ 用**Mutex + Condition 或 MPMC channel**；
   - 想彻底消除竞争 → 用**Actor / 独立进程 / 分片单写**。

3. **对"实时性 vs 一致性"的偏好是什么？**
   - 强一致（订单、账户）→ 走**锁 + 事务 + 强内存序**；
   - 弱一致、最终一致（推送、缓存）→ 走**channel + 幂等 + 重试**；
   - 极致吞吐（日志、指标）→ 走**Disruptor / lock-free ring buffer + 批量**。

**结论**：**通信选型从来不是"哪种更好"，而是"哪种更贴问题"**。把上面 3 个问题过一遍，90% 的场景选择自然浮现；剩下 10% 的场景，是本篇 §3 到 §7 讲的"三件武器 + 四种语义"该出场的地方。**记住这一句 —— 选原语先选边界，再选档位，最后才谈性能**。
