...
> 🔗 **延伸阅读**：← [3.13 协程核心设计思想](https://yccoding.com/pages/5ebc69/) · ← [3.14 Actor 与 CSP 并发模型](https://yccoding.com/pages/ff832c/) · ← [3.16 线程池设计核心原理](https://yccoding.com/pages/61229d/) · → [4.x 内存模型技术设计](..../05.内存的真相/)
...
#### 目录介绍
- [01.真实事故引入](#01真实事故引入)
  - [1.1 幽灵任务事故](#11-幽灵任务事故)
  - [1.2 泄漏1秒的bug](#12-泄漏1秒的bug)
  - [1.3 灵魂三问](#13-灵魂三问)
  - [1.4 五个层层递进的追问](#14-五个层层递进的追问)
  - [1.5 探索路径](#15-探索路径)
  - [1.6 为何值得讲透](#16-为何值得讲透)
- [02.并发的根本矛盾](#02并发的根本矛盾)
  - [2.1 顺序与并发差异](#21-顺序与并发差异)
  - [2.2 三大深坑](#22-三大深坑)
  - [2.3 错误处理的灾难](#23-错误处理的灾难)
- [03.诞生的历史回响](#03诞生的历史回响)
  - [3.1 Goto大辩论](#31-goto大辩论)
  - [3.2 Smith的宣言](#32-smith的宣言)
  - [3.3 反向革命](#33-反向革命)
  - [3.4 演进时间线](#34-演进时间线)
- [04.设计灵魂](#04设计灵魂)
  - [4.1 一行定义](#41-一行定义)
  - [4.2 三条铁律](#42-三条铁律)
  - [4.3 顺序与并发对照](#43-顺序与并发对照)
  - [4.4 设计的力量](#44-设计的力量)
- [05.跨语言实现对照](#05跨语言实现对照)
  - [5.1 Kotlin标杆](#51-kotlin标杆)
  - [5.2 Swift实现](#52-swift实现)
  - [5.3 Java 21实现](#53-java-21实现)
  - [5.4 Python Trio](#54-python-trio)
  - [5.5 半结构化方案](#55-半结构化方案)
  - [5.6 Go的路径](#56-go的路径)
- [06.核心机制三要素](#06核心机制三要素)
  - [6.1 等待合并](#61-等待合并)
  - [6.2 取消传播](#62-取消传播)
  - [6.3 异常聚合](#63-异常聚合)
- [07.Scope三种人格](#07scope三种人格)
  - [7.1 失败即停](#71-失败即停)
  - [7.2 一胜即返](#72-一胜即返)
  - [7.3 各自独立](#73-各自独立)
  - [7.4 三种策略对照](#74-三种策略对照)
- [08.经典陷阱与反模式](#08经典陷阱与反模式)
  - [8.1 GlobalScope陷阱](#81-globalscope陷阱)
  - [8.2 fire-and-forget](#82-fire-and-forget)
  - [8.3 忽略取消信号](#83-忽略取消信号)
  - [8.4 runBlocking误用](#84-runblocking误用)
  - [8.5 结构化泄露](#85-结构化泄露)
  - [8.6 异常处理时序](#86-异常处理时序)
  - [8.7 跨scope依赖](#87-跨scope依赖)
- [09.跨语言陷阱速查](#09跨语言陷阱速查)
- [10.七字真言映射](#10七字真言映射)
  - [10.1 真言原语映射](#101-真言原语映射)
  - [10.2 三大支柱](#102-三大支柱)
- [11.总结与收束](#11总结与收束)
...
**Promise.all 部分缓解了这个问题**——它能"全部失败一起报"。但它仍然**不能取消其他任务**——这就是 §1.3 第二题的答案。

### 2.4 泄漏的数学模型

**疑惑**：§1.2 那个"每天泄漏 100MB"的故事——泄漏量到底怎么算出来的？

**论证**，任务泄漏可以用一个简单的积分公式量化：

设每次泄漏的资源量 $R$（平均每个幽灵任务占据的内存 + 线程），泄漏频率 $f$（每秒发生多少次泄漏事件），程序运行时间 $T$，则总泄漏量：
...
```
R = 每个 GlobalScope.launch 泄漏的闭包 ≈ 2KB（协程对象）
f = 高峰期每秒 10 次 fetchUserData 调用，其中 5% 触发泄漏路径
  → f = 10 × 0.05 = 0.5 次/秒
  
T = 1 天 = 86400 秒

L = 2KB × 0.5 × 86400 ≈ 86MB/天
```

**为什么监控看到的是 100MB/天而非 86MB？** 因为每个协程的 closure 还间接引用了日志框架的 MDC、TraceContext、用户 token，实际对象图约 2.5KB——所以 $L_{实测} = 2.5 \times 0.5 \times 86400 \approx 108MB/天$。

**非结构化泄漏的"死亡螺旋"特性**：

```
第 1 天：泄漏 100MB → 堆变大 → GC 暂停变长
第 2 天：GC 暂停变长 → 请求响应变慢 → 并发请求堆积 → f 增大
第 3 天：f 进一步增大 → 泄漏加速 → OOM 风险逼近
      ...
第 7 天：JVM 必须重启
...
**结论**：**任务泄漏的问题不是"泄漏了多大"，而是"泄漏会自我加速"**——泄漏占用的内存挤占了 GC 效率，GC 效率下降又导致更多并发请求积压，进一步增大泄漏频率 $f$。这是一个正反馈循环。结构化并发打破这个循环的方式非常粗暴——**根本不允许 $f > 0$**。
...
**疑惑**：§1.1 那个银行账户错乱事故——`CompletableFuture.get(5, SECONDS)` 超时后，内部的 `supplyAsync` 为什么还在继续跑？结构化并发怎么把它"彻底杀死"？

**论证**，非结构化系统中"超时 ≠ 终止"，结构化系统中"超时 = 终止"：

**非结构化（CompletableFuture）的时间线**：

```
T=0:  supplyAsync 启动，开始执行 balanceService.get()
T=5:  get(5, SECONDS) 超时 → TimeoutException → 主线程返回失败
T=5:  ⚠️ balanceService.get() 还在跑！没有被任何机制中断！
T=8:  balanceService.get() 终于返回 → supplyAsync 完成
       → 回调写入 ThreadLocal → 污染了另一个请求的上下文
```

**结构化并发（coroutineScope / StructuredTaskScope）的时间线**：

```
T=0:  scope.fork { balanceService.get() }
T=5:  scope.joinUntil(5s) 超时
       → scope 自动调用所有子任务 cancel()
       → 每个子协程在下一个挂起点检查 isActive
       → 如果 balanceService.get() 内部有挂起点 → 抛 CancellationException
       → 如果 balanceService.get() 没挂起点 → 需要 yield() 检查
T=5:  ✅ 所有子任务已结束或正在结束
       → scope 离开时保证没有幽灵任务逃逸
       → 不会污染任何上下文
```

**关键区别——"取消是协作的"**。结构化并发的 cancel 不是 `thread.stop()` 那种物理毁灭，而是"礼貌请求 + 挂起点协作"：

```
cancel() 做了什么：
  1. 设置 Job 状态为 Canceled
  2. 向下递归 cancel 所有 children
  3. 如果当前正在挂起 → 抛出 CancellationException
  4. 如果当前在 CPU 计算 → 等下一个 yield() / await() 挂起点检查
```

**只有结构化才能做到"彻底终结"**——因为 scope 持有所有子任务的引用。非结构化系统中产生一个 `CompletableFuture` 后，创建者就失去了对它的"控制权"——除非显式保存并手动 cancel 每一个。

**结论**：**幽灵任务不是"偶然泄漏"，是"非结构化天然的必然"**——因为任务的生命周期和调用者脱钩后，没有任何机制保证"调用者退出 → 任务退出"。结构化并发把"撒出去不管"变成"代码块即边界"，从设计上消灭了幽灵任务的存在空间。
...
**§1.6 第二题的答案**：Go 不是"设计倒退"——它选择了**显式 + 库实现**而不是**语言原生 scope**。两者都能达成结构化并发的目标，只是哲学不同：
...
**疑惑**：6.2 节讲取消是"自顶向下传播"的——但具体怎么传？Kotlin 协程内部用了什么数据结构来跟踪父子关系？

**论证**，**所有协程的 Job 构成一棵树**，这是结构化并发唯一的数据结构基础：

```
CoroutineScope (root)
    │
    ├── Job: launch { user }     ← 父 job
    │       │                        持有 children list
    │       ├── Job: delay(100)       ← 子 job
    │       └── Job: fetch()          ← 子 job
    │
...
```
每个 Job 维护三个关键引用：
  ├── parent: Job?          ← 指向上一个 Job（用于异常传播向上）
  ├── children: List<Job>   ← 当前 Job 的所有直接子 Job
  └── state: Enum           ← Active / Cancelling / Completed / Cancelled

取消传播的算法（自顶向下）：
  cancel(job):
    1. job.state = Cancelling
    2. for each child in job.children:
         cancel(child)       ← 递归取消
    3. 所有 children 完成后，job.state = Cancelled

异常传播的算法（自底向上）：
  child 抛出未捕获异常：
    1. child.state = Cancelled（因为异常而结束）
    2. 异常传播给 parent
    3. parent 收到异常后：
       a. 取消所有其他 children（兄弟）
       b. 等所有 children 结束
       c. 自身以异常结束 → 继续向祖父传播
```

**多级嵌套的实际执行过程**（以 3 级 scope 为例）：

```kotlin
coroutineScope {                          // scope-A (root)
    launch {                              // job-B (scope-A 的子)
        coroutineScope {                  // scope-C (job-B 的子 scope)
            launch {                      // job-D (scope-C 的子)
                delay(100)
                throw IOException()       // ★ 这里出异常
            }
            launch { delay(500) }         // job-E → 被取消
        }
        launch { delay(1000) }            // job-F → 被取消
    }
    launch { delay(2000) }                // job-G → 被取消
}
```
...
job-D 抛 IOException
  → parent = scope-C
    → scope-C 收到异常
      → cancel 所有 children：job-E 被 cancel
      → scope-C 以 IOException 结束
  → parent = job-B
    → job-B 收到异常
      → cancel 所有 children：job-F 被 cancel
      → job-B 以 IOException 结束
  → parent = scope-A
    → scope-A 收到异常
      → cancel 所有 children：job-G 被 cancel
      → scope-A 以 IOException 结束（抛给调用者）

从 D 到 A 的传播路径：D → C → B → A（4 跳）
取消从 A 向下传播的路径：A → G/B → F/C → E（树形广播）
```

**为什么需要 Job 树？** 如果没有这棵树，就回到了"非结构化"——每个任务独立飘浮，取消一个任务时无法找到它的"子任务"，也不能在父任务失败时递归取消子孙。**Job 树就是结构化并发的"物理骨骼"**，scope 只是它的"语法糖"。
...
**疑惑**：scope 的"全员等待"语义看起来完美——但如果两个嵌套 scope 互相依赖呢？

**论证**，这是结构化并发最隐蔽的陷阱：

```kotlin
// ❌ 嵌套 scope 循环等待 → 死锁
suspend fun deadlock() = coroutineScope {   // scope-A
    val deferred = async {                  // job-B
        coroutineScope {                    // scope-C
            launch {
                process()                   // 需要 scope-D 的结果
            }
        }
    }
    
    coroutineScope {                        // scope-D
        launch {
            val r = deferred.await()        // ★ 等 job-B 结束
            // 但 job-B 在 scope-C 里，scope-C 在等所有子任务结束
            // 而 scope-D 的子任务又在等 deferred
            // → 死锁！
        }
    }
}
...
```
scope-D 的子任务 在等 deferred（job-B）
    ↓
deferred（job-B）在 scope-A 内
    ↓
job-B 内部有 scope-C，scope-C 在等所有子任务
    ↓
scope-C 的子任务 process() 依赖 scope-D 的结果
    ↓
scope-D 在等自己的子任务完成
    ↓
→ 循环依赖：scope-D ↔ deferred ↔ scope-C → 死锁
```

**触发条件（三个必要条件）**：

```
1. 两个 scope 有数据依赖（scope-D 的 launch 依赖 scope-C 的结果）
2. 依赖的方向和对"等待完成"的顺序冲突
3. 没有"惰性求值"中间件打破循环
```
...
suspend fun fixed() = coroutineScope {
    ...
    coroutineScope {
        launch {
            val r = channel.receive()   // 不直接 await deferred
            process(r)
        }
    }
}

// ✅ 方案 2：提升 scope 层级，消除嵌套
suspend fun fixed2() = coroutineScope {
    val deferred = async { compute() }
    launch { process(deferred.await()) }   // 同一个 scope，无嵌套
}
```

**结论**：**结构化并发的"全员等待"语义在 99% 的场景是安全的，但在嵌套 scope + 交叉依赖的场景下会产生死锁**。解决方案的核心不是放弃结构化，是**识别并消除循环依赖**——这在任何并发模型下都是死锁的根因。结构化只是让这个根因"更容易被看到"。
...
> **同一个坑、不同方言**——下表每一行是一种语言在结构化并发模型下"最容易翻车"的反模式，按七字真言**scope 是边界 / 取消会传染 / 异常要聚合**三象限分类。
...
> 本篇 §11.3 给出了**七条**真言（scope=生命周期 / 不要 GlobalScope / 取消是协作 / 异常会聚合 / try 包住 scope / 三种人格分清楚 / 不跨 scope 引用）。下表把这七条**核心理念**映射到主流语言的具体原语，看每条真言在每种语言里是哪一个 API。
...
**反例识别**：
- 看到 "GlobalScope.launch" / "裸 go fn()" / "Task { }" → 缺**边界**；
- 看到 "errgroup 不传 ctx" / "Promise.all 后想取消但没办法" → 缺**传染**；
- 看到 "Promise.allSettled 后不看结果" / "gather 没 try" → 缺**聚合**。
...
下一卷 [第 4 卷 内存的真相](..../05.内存的真相/) 我们将进入并发问题的"地基层"——**内存模型**。所有锁、CAS、volatile 的正确性最终都奠基于此。

---

## 🔗 延伸阅读

- 同卷上篇：[3.13 协程核心设计思想](https://yccoding.com/pages/5ebc69/) ｜ [3.14 Actor 与 CSP 并发模型](https://yccoding.com/pages/ff832c/) ｜ [3.16 线程池设计核心原理](https://yccoding.com/pages/61229d/)
- 下卷开篇：[4.x 内存模型技术设计](..../05.内存的真相/)
- 经典文献：
