...
- [5.CAS 内部实现机制](#5cas-内部实现机制)
  - [5.1 strong vs weak：服务LL/SC架构](#51-strong-vs-weak服务llsc架构)
  - [5.2 内存序：CAS最微妙的6个参数](#52-内存序cas最微妙的6个参数)
  - [5.3 Java AtomicInteger三层调用栈](#53-java-atomicinteger三层调用栈)
  - [5.4 LongAdder：分段解决CAS风暴](#54-longadder分段解决cas风暴)
  - [5.5 Java 实现层的反响与联动](#55-java-实现层的反响与联动)
- [6.不同语言实现](#6不同语言实现)
...
- [8.CAS技术总结](#8cas-技术总结)
  ...
  - [8.3 局限性剖析](#83-局限性剖析)
- [9.总结：CAS设计的核心价值](#9总结cas设计的核心价值)
  - [9.1 CAS应用场景总结](#91-cas应用场景总结)
  - [9.2 CAS与锁的选择指南](#92-cas与锁的选择指南)
  - [9.3 CAS在各语言中的实现](#93-cas在各语言中的实现)
- [10.案例驱动：CAS 的实战陷阱](#10案例驱动cas-的实战陷阱)
  - [10.1 案例：AtomicInteger双11热点](#101-案例atomicinteger双11热点)
  - [10.2 案例：ABA事故与无锁栈崩溃](#102-案例aba事故与无锁栈崩溃)
  - [10.3 案例：C++ memory_order选错导致race](#103-案例c-memory_order选错导致race)
- [11.CAS 与现代并发原语的关系](#11cas-与现代并发原语的关系)
  - [11.1 CAS 是无锁世界的基石](#111-cas-是无锁世界的基石)
  - [11.2 与未来：CAS-2 与多字 CAS](#112-与未来cas-2-与多字-cas)
- [12.设计哲学：CAS教会我们三件事](#12设计哲学cas教会我们三件事)
  - [12.1 乐观主义在工程中的胜利](#121-乐观主义在工程中的胜利)
  - [12.2 硬件抽象的下沉](#122-硬件抽象的下沉)
  - [12.3 简单接口背后的精密配合](#123-简单接口背后的精密配合)
- [13.跨语言 CAS 陷阱速查表](#13跨语言-cas-陷阱速查表)
...
### 10.3 案例：C++ memory_order选错导致race
