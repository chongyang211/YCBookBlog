...
- [08.优化与最佳实践](#08优化与最佳实践)
  ...
  - [8.5 性能监控调优](#85-性能监控调优)
- [09.阶段性总结](#09阶段性总结)
  - [9.1 核心要点总结](#91-核心要点总结)
  - [9.2 设计原则](#92-设计原则)
  - [9.3 技术选型建议](#93-技术选型建议)
- [10.案例驱动哲学](#10案例驱动哲学)
  - [10.1 电商秒杀演进](#101-电商秒杀演进)
  - [10.2 日志无锁演进](#102-日志无锁演进)
  - [10.3 Go并发哲学](#103-go并发哲学)
- [11.跨语言深度对比](#11跨语言深度对比)
  - [11.1 原语三层抽象](#111-原语三层抽象)
  - [11.2 三支柱五语言](#112-三支柱五语言)
  - [11.3 哲学三支谱系](#113-哲学三支谱系)
  - [11.4 实战取舍清单](#114-实战取舍清单)
  - [11.5 反模式警觉](#115-反模式警觉)
  - [11.6 跨语言陷阱速查](#116-跨语言陷阱速查)
  - [11.7 七字真言映射](#117-七字真言映射)
  - [11.8 一句话总结](#118-一句话总结)
  - [11.9 并发选型回扣](#119-并发选型回扣)
...
**疑惑**：三大支柱（分工、同步、互斥）讲完了，但落到具体项目里，我到底该按什么顺序做决定？

**论证**：把并发选型拆成"**5 步决策链**"，绝大多数项目都能沿这条链走完：

```mermaid
flowchart TD
    S1[Step 1 · 定负载画像<br/>CPU密集 / IO密集 / 混合]
    S2[Step 2 · 定并发单位<br/>线程 / 协程 / Actor]
    S3[Step 3 · 定通信路线<br/>共享内存 / 消息传递]
    S4[Step 4 · 定同步档位<br/>原子/锁/信号量/屏障]
    S5[Step 5 · 定可观测手段<br/>指标/tracing/race detector]
    S1 --> S2 --> S3 --> S4 --> S5
    style S1 fill:#f8d7da
    style S5 fill:#d4edda
```

- **Step 1**：CPU 密集 → 线程数 = 核数、亲和绑核；IO 密集 → 走协程/异步；混合 → 双池；
- **Step 2**：延迟敏感 + 少量任务 → OS 线程；海量连接 + 短任务 → 协程 / VT；强隔离 + 崩不影响 → Actor / 子进程；
- **Step 3**：共享数据结构小、访问密集 → 共享内存 + Mutex/atomic；数据结构大、跨模块 → channel/MQ；跨节点 → 消息队列；
- **Step 4**：单变量、无复合操作 → atomic；复合临界区 → Mutex；一次性会合 → Latch/Barrier；限流/池化 → Semaphore；
- **Step 5**：所有并发代码必须配套 **QPS / P99 / 线程状态 / 等锁时间 / 竞态告警** 5 类观测。**没有 observability，并发就是黑箱**。

**结论**：**并发工程不是拍脑袋选一个原语，而是走完这条决策链**。**跳过 Step 1（负载画像）** 是最常见的翻车根因 —— 明明是 IO 密集非要开百线程池；**跳过 Step 5（可观测）** 是最常见的暴雷根因 —— 生产出问题前你根本不知道锁在哪、等多久。**把这 5 步写进设计文档，就是团队并发能力的 baseline**。
