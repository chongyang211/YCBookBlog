...
> 🔗 **延伸阅读**：← [5.1 窗口核心设计思想](https://yccoding.com/pages/2f009c/) · ← [5.2 视图加载渲染设计](https://yccoding.com/pages/070bb8/) · ← [5.7 组件生命周期管理](https://yccoding.com/pages/aaaaaa/) · → [5.9 响应式数据绑定设计](https://yccoding.com/pages/bbbbbb/) · ↔ [6.x 架构思想](../../07.架构演进/) · ↔ [4.x 内存的真相](../04.内存的真相/)
...
- [00.从一个路由事故开始](#00从一个路由事故开始)
- [01.为什么需要路由系统](#01为什么需要路由系统)
  - [1.1 没有路由的世界：手动跳转的混乱](#11-没有路由的世界手动跳转的混乱)
  - [1.2 三大根因：解耦 / 栈管理 / 深链](#12-三大根因解耦--栈管理--深链)
  - [1.3 路由系统的设计目标](#13-路由系统的设计目标)
- [02.路由通用五件套契约](#02路由通用五件套契约)
  - [2.1 URI（统一资源标识）](#21-uri统一资源标识)
  - [2.2 路由栈（Stack）](#22-路由栈stack)
  - [2.3 参数传递（Params）](#23-参数传递params)
  - [2.4 拦截器（Interceptor / Guard）](#24-拦截器interceptor--guard)
  - [2.5 转场动画（Transition）](#25-转场动画transition)
  - [2.6 跨端名词矩阵（七端对照）](#26-跨端名词矩阵七端对照)
- [03.Web 路由设计](#03web-路由设计)
  - [3.1 Hash 路由 vs History 路由](#31-hash-路由-vs-history-路由)
  - [3.2 React Router：声明式路由 + 嵌套路由](#32-react-router声明式路由--嵌套路由)
  - [3.3 Vue Router：路由守卫的设计](#33-vue-router路由守卫的设计)
  - [3.4 Next.js / Nuxt：基于文件系统的路由](#34-nextjs--nuxt基于文件系统的路由)
- [04.Android 导航体系](#04android-导航体系)
  - [4.1 Intent 显式/隐式跳转](#41-intent-显式隐式跳转)
  - [4.2 Activity TaskAffinity 与 launchMode](#42-activity-taskaffinity-与-launchmode)
  - [4.3 Jetpack Navigation：单 Activity + 多 Fragment](#43-jetpack-navigation单-activity--多-fragment)
  - [4.4 ARouter / TheRouter：组件化路由的工业实践](#44-arouter--therouter组件化路由的工业实践)
- [05.iOS 导航体系](#05ios-导航体系)
  - [5.1 UINavigationController 的栈模型](#51-uinavigationcontroller-的栈模型)
  - [5.2 Modal / Push / Present 的三种范式](#52-modal--push--present-的三种范式)
  - [5.3 URLRouter / Coordinator 模式](#53-urlrouter--coordinator-模式)
  - [5.4 SwiftUI NavigationStack 与 NavigationPath](#54-swiftui-navigationstack-与-navigationpath)
- [06.跨端框架的路由设计](#06跨端框架的路由设计)
  - [6.1 Flutter Navigator 1.0 vs 2.0](#61-flutter-navigator-10-vs-20)
  - [6.2 go_router：声明式路由的胜利](#62-go_router声明式路由的胜利)
  - [6.3 React Native / Compose Navigation](#63-react-native--compose-navigation)
  - [6.4 小程序四种导航 API 的辨析](#64-小程序四种导航-api-的辨析)
- [07.横向对比矩阵（七端 × 八维度）](#07横向对比矩阵七端--八维度)
  - [7.1 路由能力对齐表](#71-路由能力对齐表)
  - [7.2 设计哲学站位图](#72-设计哲学站位图)
- [08.路由反模式与陷阱](#08路由反模式与陷阱)
  - [8.1 反模式 1：硬编码跳转散落全工程](#81-反模式-1硬编码跳转散落全工程)
  - [8.2 反模式 2：通过 Intent/Bundle 传大对象](#82-反模式-2通过-intentbundle-传大对象)
  - [8.3 反模式 3：栈污染（重复入栈 / 死循环跳转）](#83-反模式-3栈污染重复入栈--死循环跳转)
  - [8.4 反模式 4：深链直达未做权限校验](#84-反模式-4深链直达未做权限校验)
  - [8.5 反模式 5：跨模块强依赖导致编译墙](#85-反模式-5跨模块强依赖导致编译墙)
- [09.现代演进：类型安全与声明式路由](#09现代演进类型安全与声明式路由)
  - [9.1 类型安全：从 String → Sealed Class / TypeScript](#91-类型安全从-string--sealed-class--typescript)
  - [9.2 声明式路由：URL 作为单一事实源](#92-声明式路由url-作为单一事实源)
  - [9.3 深链 + Universal Link + App Links 三件套](#93-深链--universal-link--app-links-三件套)
  - [9.4 路由 + 状态恢复：进程死亡后的 URI 重建](#94-路由--状态恢复进程死亡后的-uri-重建)
...
https://yccoding.com/pages/aaaaaa/
...
### 4.2 Activity TaskAffinity 与 launchMode
...
### 4.3 Jetpack Navigation：单 Activity + 多 Fragment
...
### 4.4 ARouter / TheRouter：组件化路由的工业实践
...
### 5.1 UINavigationController 的栈模型
...
### 5.4 SwiftUI NavigationStack 与 NavigationPath
...
### 6.1 Flutter Navigator 1.0 vs 2.0
...
### 6.3 React Native / Compose Navigation
...
### 8.2 反模式 2：通过 Intent/Bundle 传大对象
...
### 9.1 类型安全：从 String → Sealed Class / TypeScript
...
### 9.3 深链 + Universal Link + App Links 三件套
...
**本篇结**——下一篇 [5.9 响应式数据绑定设计](https://yccoding.com/pages/bbbbbb/) 我们将看到，当"位置"变成数据之后，"数据"本身又如何反过来驱动 UI——这是 SwiftUI/React/Vue/Compose 共同走向的"声明式终局"。
