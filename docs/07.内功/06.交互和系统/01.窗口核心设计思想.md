...
> 📍 **本篇位置**：第 5 卷 · 交互与系统 · 第 1 篇（屏幕呈现四部曲之首）
> 🎯 **核心矛盾**：**像素矩阵的同质 vs UI 元素的层次** —— 窗口系统用"树 + 分层 + 合成"把二维平面切成可交互的区域
> 🧭 **设计灵魂**：所有 GUI 框架都是**视图树 + 布局算法 + 绘制管线**三件套——区别只在"渲染怎么合成"（CPU 软光栅 vs GPU 硬合成）和"管理放哪个进程"（应用内 vs 系统服务）
> 🌐 **跨平台覆盖**：Android(View / ViewGroup / Surface / WMS) · iOS(UIView / CALayer / Core Animation / UIWindowScene) · Web(window / Document / Stacking Context / Compositor) · Flutter(Widget / Element / RenderObject 三棵树 + FlutterView) · Qt(QWidget / QPainter / QWindow) · 嵌入式(LVGL lv_obj / Wayland Surface / X Window) · 桌面(Win32 HWND / macOS NSWindow)
> 🔗 **延伸阅读**：→ [5.2 视图加载渲染设计](https://yccoding.com/pages/070bb8/) · → [5.3 图形渲染管线原理](https://yccoding.com/pages/318066/) · → [5.4 手势事件设计灵魂](https://yccoding.com/pages/5b96db/) · → [5.5 消息机制设计思想](https://yccoding.com/pages/0aed31/) · → [5.6 跨进程通信设计](https://yccoding.com/pages/e09fe7/)
> 💡 **通用心智**：忘掉具体平台的 API，记住一句话——**窗口 = 「矩形坐标系 + Z 序栈 + 输入路由表 + 渲染表面」四元组**。任何平台的窗口系统都只在这四个维度上做取舍。
...
#### 目录介绍
- [0.窗口泄漏事故](#0窗口泄漏事故)
  - [0.1 凌晨崩溃日志](#01-凌晨崩溃日志)
  - [0.2 三个反直觉现象](#02-三个反直觉现象)
  - [0.3 窗口视角拆解](#03-窗口视角拆解)
  - [0.4 事故揭示](#04-事故揭示)
  - [0.5 五个递进追问](#05-五个递进追问)
- [1.概述与背景](#1概述与背景)
  - [1.1 由来与背景](#11-由来与背景)
  - [1.2 窗口演进史](#12-窗口演进史)
  - [1.3 为何需要窗口](#13-为何需要窗口)
  - [1.4 主要解决问题](#14-主要解决问题)
  - [1.5 警醒的资源数字](#15-警醒的资源数字)
  - [1.6 窗口设计目标](#16-窗口设计目标)
  - [1.7 设计哲学思想](#17-设计哲学思想)
- [2.各平台窗口机制](#2各平台窗口机制)
  - [2.1 Android 窗口](#21-android-窗口)
  - [2.2 iOS 窗口](#22-ios-窗口)
  - [2.3 Web 窗口](#23-web-窗口)
  - [2.4 Flutter 窗口](#24-flutter-窗口)
  - [2.5 Qt 桌面窗口](#25-qt-桌面窗口)
  - [2.6 嵌入式窗口](#26-嵌入式窗口)
  - [2.7 平台对比分析](#27-平台对比分析)
  - [2.8 四元组抽象模型](#28-四元组抽象模型)
  - [2.9 一句话窗口本质](#29-一句话窗口本质)
- [3.窗口架构设计](#3窗口架构设计)
  - [3.1 整体架构图](#31-整体架构图)
  - [3.2 为何架构复杂](#32-为何架构复杂)
  - [3.3 分层架构思想](#33-分层架构思想)
  - [3.4 核心组件协作](#34-核心组件协作)
  - [3.5 窗口与显示系统](#35-窗口与显示系统)
- [4.窗口类型设计](#4窗口类型设计)
  - [4.1 为何区分窗口类型](#41-为何区分窗口类型)
  - [4.2 应用窗口](#42-应用窗口)
  - [4.3 子窗口](#43-子窗口)
  - [4.4 系统窗口](#44-系统窗口)
  - [4.5 类型与 Z 序](#45-类型与-z-序)
  - [4.6 类型设计权衡](#46-类型设计权衡)
  - [4.7 跨平台层级对照](#47-跨平台层级对照)
- [5.窗口属性设计](#5窗口属性设计)
  - [5.1 窗口属性全貌](#51-窗口属性全貌)
  - [5.2 布局属性设计](#52-布局属性设计)
  - [5.3 标志位设计](#53-标志位设计)
  - [5.4 透明度与动画](#54-透明度与动画)
  - [5.5 属性继承与覆盖](#55-属性继承与覆盖)
- [6.窗口管理器设计](#6窗口管理器设计)
  - [6.1 为何需要管理器](#61-为何需要管理器)
  - [6.2 管理器核心职责](#62-管理器核心职责)
  - [6.3 窗口增删改查](#63-窗口增删改查)
  - [6.4 焦点与事件分发](#64-焦点与事件分发)
  - [6.5 动画与过渡](#65-动画与过渡)
  - [6.6 多窗口与分屏](#66-多窗口与分屏)
- [7.窗口与View关系](#7窗口与view关系)
  - [7.1 窗口是 View 容器](#71-窗口是-view-容器)
  - [7.2 ViewRootImpl 桥接](#72-viewrootimpl-桥接)
  - [7.3 DecorView 职责](#73-decorview-职责)
  - [7.4 Surface 与 Canvas](#74-surface-与-canvas)
- [8.窗口加载 View](#8窗口加载-view)
  - [8.1 从 setContentView 起](#81-从-setcontentview-起)
  - [8.2 布局加载流程](#82-布局加载流程)
  - [8.3 测量布局绘制](#83-测量布局绘制)
  - [8.4 窗口首帧渲染](#84-窗口首帧渲染)
  - [8.5 懒加载按需渲染](#85-懒加载按需渲染)
- [9.窗口生命周期](#9窗口生命周期)
  - [9.1 创建与销毁](#91-创建与销毁)
  - [9.2 可见性管理](#92-可见性管理)
  - [9.3 状态机设计](#93-状态机设计)
  - [9.4 各端生命周期对照](#94-各端生命周期对照)
  - [9.5 泄漏与防护](#95-泄漏与防护)
- [10.设计总结与演进](#10设计总结与演进)
  - [10.1 核心模式提炼](#101-核心模式提炼)
  - [10.2 哲学本质](#102-哲学本质)
  - [10.3 GUI 架构演进](#103-gui-架构演进)
  - [10.4 跨领域思想迁移](#104-跨领域思想迁移)
- [11.经典陷阱与反模式](#11经典陷阱与反模式)
  - [11.1 AppContext 建窗](#111-appcontext-建窗)
  - [11.2 父销毁后 dismiss](#112-父销毁后-dismiss)
  - [11.3 滥用 OVERLAY](#113-滥用-overlay)
  - [11.4 onCreate 取尺寸](#114-oncreate-取尺寸)
  - [11.5 依赖设备 Z 序](#115-依赖设备-z-序)
  - [11.6 输入法遮挡](#116-输入法遮挡)
  - [11.7 SurfaceView 销毁绘制](#117-surfaceview-销毁绘制)
  - [11.8 多 Scene keyWindow](#118-多-scene-keywindow)
  - [11.9 iframe z-index](#119-iframe-z-index)
  - [11.10 dispose 后 setState](#1110-dispose-后-setstate)
- [12.认知阶梯总结](#12认知阶梯总结)
  - [12.1 三层认知](#121-三层认知)
  - [12.2 设计哲学](#122-设计哲学)
  - [12.3 跨端术语对照](#123-跨端术语对照)
  - [12.4 本卷章节呼应](#124-本卷章节呼应)
  - [12.5 延伸阅读](#125-延伸阅读)
...
| 追问 | 答案章节 |
|---|---|
| 窗口为什么必须独立于 View 存在？历史动机是什么？ | §1 |
| Android/iOS/Web 三个平台的窗口实现差在哪里？ | §2 |
| 窗口的层级、属性、管理器是怎么协同工作的？ | §3~§6 |
| 窗口和 View 的边界在哪里？为什么 Dialog 是窗口而不是 View？ | §7~§8 |
| 窗口的生命周期为什么会泄漏？怎么从架构上根治？ | §9 |
...
窗口系统的设计体现了多个深刻的哲学思想，这些思想不仅指导了窗口系统的设计，也是理解所有GUI架构的钥匙——**无论你做 Android / iOS / Web / Flutter / 嵌入式 / 桌面 / 游戏 / AR**，下面这五条都适用。
...
Flutter 走的是**第四条路线**——它**不依赖原生窗口系统的视图**，自己实现一套完整的渲染管线，只向宿主平台借一个"裸窗口"。这种"自绘引擎"思路本质上是窗口设计的极简哲学：**只要给我一块 Surface，剩下的我自己来**。

**核心组件**：

| 组件 | 职责 | 类比 |
|------|------|------|
| **FlutterView / FlutterWindow** | 宿主平台提供的"窗口容器" | Android 的 SurfaceView / iOS 的 UIView |
| **Embedder** | 平台适配层（窗口创建 / 事件接入 / VSync 接入） | Android 的 FlutterActivity / iOS 的 FlutterViewController |
| **Flutter Engine（C++）** | 调度 / 任务 / 平台通道 | 操作系统的"框架进程" |
| **Skia / Impeller** | 跨平台 2D 渲染后端 | SurfaceFlinger + GPU 合成器 |
| **三棵树（Widget / Element / RenderObject）** | 应用层视图描述 → 实例 → 渲染节点 | Android 的 View 树 / Web 的 DOM 树 |
| **OverlayEntry** | Flutter 自己的"子窗口"机制 | Android 的 PopupWindow / iOS 的 Modal |

**Flutter 设计思想的深层分析**：

Flutter 的窗口选择是**只用宿主一个窗口，自己分层**。原生的"子窗口"（Dialog/PopupWindow）在 Flutter 中**根本不存在**——所有的弹窗、菜单、Tooltip 都是 `OverlayEntry`，本质上是在同一棵 RenderObject 树里多挂一个分支。

```
传统 Android：
  Activity 窗口 + Dialog 子窗口 + PopupWindow 子窗口 = 3 个独立 Surface
  → 每个有独立 Token / 输入通道 / 合成层

Flutter：
  FlutterView 一个 Surface
  ├─ 主页内容 (RenderObject 树)
  ├─ Overlay 层 (RenderObject 子树) ← Dialog / SnackBar / Tooltip 都在这
  └─ 自绘合成 → 提交 Skia → 提交宿主 Surface
```

**这是窗口设计的"反向革命"**：以前我们把"子窗口"当作系统原生能力来理解，Flutter 告诉我们：**子窗口本质就是"分层叠加的渲染节点"**。原生窗口系统多出的那一层（Token / IPC / 独立 Surface），是为了"跨应用隔离"而存在，应用内部根本不需要。

**这点对学习窗口系统至关重要**：当你用 Flutter / Compose / SwiftUI 这类声明式框架时，"窗口"被收敛到只有**应用级 1 个**，而所有原生中的"子窗口"全部退化为应用内的分层 widget。**理解了这个，你就理解了为什么 Flutter 没有"WindowLeaked"——根本就没有第二个窗口可以泄漏。**

### 2.5 Qt 桌面窗口

Qt 是跨桌面操作系统（Linux X11/Wayland、Windows、macOS）的窗口框架，它的设计是**多窗口系统的"通用适配器"**——把不同 OS 的窗口 API 统一为一套 QWindow / QWidget 接口。

**核心组件**：

| 组件 | 职责 |
|------|------|
| **QWindow** | 顶层窗口（对应 Linux 的 X Window、Windows 的 HWND、macOS 的 NSWindow） |
| **QWidget** | 应用级控件（既能当窗口，也能当 View） |
| **QPainter / QBackingStore** | 软件绘制 + GPU 加速可选 |
| **QPlatformWindow** | 平台适配抽象（PIMPL 模式） |
| **QGuiApplication** | 全局窗口管理器（每进程一个） |

**Qt 设计的关键洞察**：QWidget 是"窗口与 View 的合体"——一个 QWidget 既可以是顶层窗口（独立的操作系统级窗口），也可以是另一个 QWidget 的子部件（共享父窗口）。**当且仅当 `setWindowFlags()` 设置了 Qt::Window 时，它才会向 OS 申请独立窗口**。

这与 Android（窗口与 View 严格分离）、iOS（UIWindow 继承 UIView）形成有趣的三种范式：

```
Android：窗口 ≠ View          严格两层（系统级窗口 + 应用级 View）
iOS：    UIWindow extends UIView 单层（UIWindow 是特殊的 View）
Qt：     QWidget 可上可下       动态（同一个类，看 flags 决定）
Web：    document 内嵌在 window 半分离（window 容器 + DOM 内容）
Flutter：一个 FlutterView 多分支 单窗口（所有"子窗口"都是节点）
```

**这五种范式覆盖了几乎所有 UI 框架的窗口设计选择**——任何应用开发者都能在其中找到自己框架的位置。

**Win32 / macOS 原生**作为 Qt 的下层，简要补充：

| 系统 | 窗口标识 | 管理方式 | 备注 |
|------|---------|---------|------|
| **Win32** | HWND (句柄) | 单进程内 USER32.dll 管理；DWM 跨进程合成 | 每个控件（按钮、文本框）都是 HWND——"万物皆窗口" |
| **macOS** | NSWindow / CGWindowID | App 内 NSApplication 管理；WindowServer 跨进程合成 | 重 OO，NSWindow 包含 NSView |
| **X11** | Window (XID) | X Server 集中管理 | C/S 架构，窗口管理器是独立进程（如 i3 / GNOME Shell） |
| **Wayland** | wl_surface | Compositor 集中管理（混合 X11 + 合成器） | 现代 Linux 桌面，安全模型更强 |

### 2.6 嵌入式窗口

嵌入式场景——智能手表、车机仪表盘、家电触屏、工业 HMI——是窗口设计的**极简版本**。这里资源极度稀缺：可能只有 64KB SRAM、单核 200MHz CPU、没有 GPU。

**LVGL（Light and Versatile Graphics Library）** 是嵌入式 GUI 的事实标准：

```c
/* LVGL 的"窗口"：lv_obj_t */
lv_obj_t *scr = lv_scr_act();           // 当前活动屏幕（= 全屏窗口）
lv_obj_t *btn = lv_btn_create(scr);     // 子控件
lv_obj_t *win = lv_win_create(scr, 40); // "窗口"控件（其实是带标题栏的容器）
```
...
|------|----------|------|
| 多窗口 | 支持 | **不支持**（只有一个屏幕） |
| 独立 Surface | 每个窗口一个 | **全局一个 framebuffer** |
| GPU 合成 | 通常有 | **CPU 软光栅** |
| 双缓冲 | 默认 | 可选（看内存够不够） |
| 输入路由 | 复杂 hit test | 全局指针位置直接比对 |
| 进程隔离 | 必须 | **单进程，无隔离** |

**为什么嵌入式根本"不需要窗口系统"**：

```
嵌入式设备只有一个应用（智能手表只显示表盘 + 几个 App）
   → 不需要进程隔离
   → 不需要跨应用 Z 序仲裁
   → 不需要"应用窗口 / 系统窗口 / 子窗口"的类型设计
   → 窗口退化为"控件容器"——和 Flutter 的 OverlayEntry 几乎一样
```

**这又一次印证了通用心智模型**：

> **窗口系统的复杂度，正比于"互不信任的实体"数量**。
> 桌面/手机有几百个应用互相不信任 → WMS 是仲裁者；
> 嵌入式只有一个应用 → 根本不需要仲裁者。

**Wayland / X 在嵌入式场景的妥协**：高端嵌入式（车载 IVI、工业平板）会用 Wayland 替代裸 framebuffer——为了支持多进程应用（导航 + 多媒体 + 仪表盘 三个独立进程合到一块屏幕）。这时窗口系统又"重生"了——**因为又有了"互不信任的实体"**。

### 2.7 平台对比分析

| 维度 | Android | iOS | Web | Flutter | Qt | 嵌入式(LVGL) |
|------|---------|-----|-----|---------|-----|-------------|
| **窗口基类** | Window（抽象类） | UIWindow（继承UIView） | window（全局对象） | FlutterView（只 1 个） | QWindow / QWidget | lv_obj_t（无独立窗口） |
| **窗口数量** | 一 Activity 数个 | 通常 1 个 UIWindow | 一标签页 1 个 | 1 个 + N 个 Overlay | 多个 | 1 个屏幕 |
| **管理方式** | 跨进程 WMS | 应用内自管理 | 浏览器引擎管理 | 应用内 + 自绘 | 应用 + OS 适配 | 应用直接画 framebuffer |
| **层级控制** | type+Z 序 | windowLevel | z-index+ 层叠上下文 | OverlayEntry 顺序 | stayOnTop flags | z_index 字段 |
| **独立 Surface** | 每窗口一个 | 共享 CALayer | 浏览器分配 layer | 1 个总 Surface | 每 QWindow 一个 | 单 framebuffer |
| **隔离粒度** | 进程级 | 进程级 | 进程/沙箱级 | **无**（单应用） | 进程级 | **无** |
| **多窗口支持** | 分屏/悬浮窗 | iOS13+ Scene | 多标签页 | 单窗口为主 | 强（桌面应用） | 通常不支持 |
| **典型代表** | App 后台 | Mac/iPad App | 浏览器 | 跨端 App | KDE / 工业软件 | 手表/家电 |

### 2.8 四元组抽象模型

**这是本篇的"灵魂章节"**——剥离所有平台细节，**窗口的本质就是四个东西的合体**：

```mermaid
flowchart TB
    W[窗口 Window] --> P1[几何坐标系<br/>x, y, width, height, transform]
    W --> P2[Z 序栈位置<br/>layer index / type / order]
    W --> P3[渲染表面<br/>Surface / Layer / Framebuffer]
    W --> P4[输入路由表项<br/>InputChannel / EventTarget / hit test 项]
...
**通用 API 契约（抽象出"窗口"应该具备的最小接口）**：

```
interface Window {
    // 维度 1：几何
    Rect getBounds();              // 当前位置+大小
    void setBounds(Rect);          // 改位置或大小
    Matrix getTransform();         // 旋转/缩放变换（可选，现代窗口才有）
    ...
    // 维度 3：渲染表面
    Surface getSurface();          // 拿到画布
    void requestRedraw();          // 请求重绘
    
    // 维度 4：输入路由
    bool hitTest(Point);           // 这个点是不是落在我身上
    void dispatchInput(InputEvent);// 处理输入
...
| 通用接口 | Android | iOS | Web | Flutter | Qt | LVGL |
|---------|---------|-----|-----|---------|-----|------|
| `getBounds()` | `LayoutParams.x/y/width/height` | `UIWindow.frame` | `Element.getBoundingClientRect()` | `OverlayEntry.position` | `QWindow.geometry()` | `lv_obj_get_coords()` |
| `getZOrder()` | `LayoutParams.type` + 创建时间 | `UIWindow.windowLevel` | `z-index` + stacking ctx | OverlayEntry 列表 idx | `Qt::WindowStaysOnTop` | `lv_obj_move_foreground()` |
| `getSurface()` | `Window.getSurface()` | `CALayer.contents` | Compositor Layer | Engine 内部 Skia | `QBackingStore` | 全局 `lv_disp_buf_t` |
| `hitTest()` | `WMS.findTouchedWindow()` | `UIView.hitTest:` | DOM 命中 + pointer-events | `RenderBox.hitTest()` | `QWidget::childAt()` | `lv_indev_search_obj()` |
| `onDestroyed()` | `Activity.onDestroy()` | `dealloc` | `unload` 事件 | `dispose()` | `QObject` 析构 | `lv_obj_del()` |

**这五个要素的设计思想无处不在**：Photoshop 的图层系统、视频编辑软件的轨道叠放、地图应用的图层（底图+POI+UI）、AR 应用的 World Anchor、游戏引擎的 Scene Graph、3D 软件的 Viewport——**它们都在做同一件事：在二维或三维空间里圈定一块区域 + 决定渲染顺序 + 路由输入**。
...
- **Android**：三者明确拆开——Token（安全）、Surface（渲染）、InputChannel（事件），由 WMS 跨进程仲裁
- **iOS**：三者与 UIView 合为一体（UIWindow extends UIView），ARC 自动管理生命周期
- **Web**：三者隐藏在 BrowsingContext + Compositor Layer + Event Target 里，浏览器引擎做仲裁
- **Flutter**：三者在引擎内退化为"一个总 Surface + Overlay 节点 + RenderObject hit test"——**几乎不暴露窗口概念**
- **Qt**：QWidget 通过 `Qt::Window` flag 决定是否升格为系统窗口——窗口与控件本一体
- **嵌入式（LVGL）**：三者退化为同一个 `lv_obj_t`——**整个系统只有一个总窗口**

**给所有应用开发者的总记忆**：

> 不管你在做什么应用，请记住——**屏幕上每一块"独立的、可被覆盖的、能接收输入的矩形"，都是一个窗口**。它叫 Activity 还是 UIWindow 还是 iframe 还是 OverlayEntry 不重要，它的"窗口属性"——几何 / Z 序 / 表面 / 输入——才重要。
...
不同平台用不同方式表达"哪个在上、哪个在下"——但**抽象语义完全一致：分段 + 段内排序**。

```mermaid
flowchart TB
    subgraph 通用模型[" 通用窗口层级模型 "]
        Z3["系统级<br/>不可被应用覆盖"]
        Z2["浮层级<br/>应用临时弹层"]
        Z1["应用级<br/>普通内容"]
        Z3 --> Z2 --> Z1
...
| 通用层级 | Android | iOS | Web | Flutter | Qt | LVGL |
|---------|---------|-----|-----|---------|-----|------|
| **系统级** | type 2000-2999 | `UIWindow.Level.alert` (2000+) | 浏览器 UI（无法被网页 cover） | — (不支持) | `Qt::WindowStaysOnTopHint` | — |
| **浮层级** | type 1000-1999 子窗口 | `UIWindow.Level.statusBar` (1000+) | `position:fixed` + 高 z-index | `Overlay.of(context).insert()` | `Qt::Tool / Qt::Popup` | `lv_obj_move_foreground()` |
| **应用级** | type 1-99 主窗口 | `UIWindow.Level.normal` (0) | 默认 stacking ctx | 主 Widget 树 | 普通 QWidget | 默认 `lv_scr_act()` |

**关键洞察**：

```
六个平台、六套不同命名，但都在做同一件事：
  把"屏幕的有限层数"切成 3 大段
   ↓
  每段内允许自由排序，跨段不可越界
   ↓
...
**为什么所有平台都这么设计？** 因为这是「**最小权限原则**」在 UI 层的必然体现——**应用不应该能盖住系统 UI，临时浮层不应该能盖住对话框**。任何打破这个原则的设计都会成为安全漏洞（比如 Android 早期的悬浮窗钓鱼）。

**给应用开发者的实战心法**：

> **绝对不要尝试跨段越界**——不要试图用 Web 的 `z-index: 999999`、Android 的 `TYPE_SYSTEM_OVERLAY` 或 iOS 的 `windowLevel = .alert + 1` 来"压住"高层级。这要么被系统拒绝，要么被未来版本封堵。**正确做法是回到"自己的段内做事"**——应用级问题用应用级解（DialogFragment / OverlayEntry / position:absolute），浮层问题用平台官方浮层 API（Toast / SnackBar / Snackbar / Notification）。
...
Android、iOS、Web、Flutter 的"窗口生命周期"看似各异，**抽象后是同一个状态机**——只是触发回调的命名不同：

```mermaid
stateDiagram-v2
    [*] --> Created: 构造完成
    Created --> Attached: 加到窗口管理器
    Attached --> Visible: 屏幕可见
    Visible --> Active: 获得焦点
    Active --> Visible: 失焦
    Visible --> Hidden: 不可见
    Hidden --> Visible: 重新可见
    Hidden --> Destroyed: 销毁
    Visible --> Destroyed: 强制销毁
    Destroyed --> [*]
```

**五个通用状态在各端的对应回调**：

| 通用状态 | Android (Activity) | iOS (UIWindow + ViewController) | Web (window) | Flutter (FlutterView) |
|---------|--------------------|--------------------------------|--------------|----------------------|
| **Created** | `onCreate` | `init` / `loadView` | DOMContentLoaded | `initState` |
| **Attached** | `onAttachedToWindow` | `viewDidLoad` + `makeKey` | `load` 事件 | `didChangeDependencies` |
| **Visible** | `onStart` + `onWindowFocusChanged(true)` | `viewWillAppear` → `viewDidAppear` | `visibilitychange` (visible) | `WidgetsBindingObserver.didChangeAppLifecycleState(resumed)` |
| **Active** | `onResume` | `windowDidBecomeKey` | `focus` | `AppLifecycleState.resumed` |
| **Hidden** | `onStop` | `viewDidDisappear` + `resignKey` | `visibilitychange` (hidden) | `AppLifecycleState.paused` |
| **Destroyed** | `onDestroy` + `removeView` | `dealloc` / Scene disconnect | `unload` / `beforeunload` | `dispose` |

**关键差异 1：销毁触发机制**

```
Android：WMS 主动通知应用 onDestroy → 系统驱动
iOS：    引用计数归零 → ARC 触发 dealloc → 应用内存驱动
Web：    导航离开页面 → 浏览器触发 unload → 浏览器驱动
Flutter：父 Widget 不再 build 此节点 → dispose → 框架驱动
```

**这四种触发机制对应了四种"窗口归谁管"的哲学**：
- Android：**操作系统是窗口的主人**（你只是借用）
- iOS：**应用内存是窗口的主人**（你持有引用就活着）
- Web：**浏览器是窗口的主人**（页面跳走窗口就死）
- Flutter：**Widget 树是窗口的主人**（不在树里就死）

**关键差异 2：能否对应"Activity 被系统杀掉"这种异常**

```
Android onSaveInstanceState：系统会强杀进程，必须存状态
iOS UIStateRestoration：相同概念，但极少触发（iOS 较少杀进程）
Web 页面：浏览器随时可能卸载 Tab，sessionStorage 可救
Flutter：跟随宿主平台
```

**这是任何应用开发者都必须理解的"窗口生命周期不变量"**：

> **窗口的存活时长 ≤ 你以为的存活时长**——OS 随时可以杀掉它，你的状态保存代码必须假设"下一秒就被杀"。这条原则在 Android / iOS / Web / Flutter 上**完全一致**。
...
**6. AR/VR 中的"World Anchor / Spatial Anchor"**。AR 应用中漂浮在桌面上的 UI 卡片（如 Apple Vision Pro 的 Window）本质上是"三维空间中的窗口"——它有位置（3D 坐标）、Z 序（视线遮挡）、渲染表面（一块贴在虚拟平面的纹理）、输入路由（视线 + 手势 hit test）。**四元组完整成立**，只是从 2D 升到了 3D。

**7. 游戏引擎的 Viewport / Camera Frustum**。Unity / Unreal 的 Viewport 决定"哪部分游戏世界被渲染到屏幕"，多人分屏游戏的多个 Viewport 与桌面分屏窗口在数学模型上完全一致——**都是"屏幕的子矩形 + 独立渲染目标 + 独立输入裁剪"**。

**8. 车载 IVI 多屏镜像**。现代汽车有中控屏 + 仪表盘 + HUD + 后排娱乐屏，每块屏幕都是一个独立的"显示设备"，每个设备上可能有多个应用窗口。Android Automotive / QNX 的窗口系统是手机 WMS 的"多 Display 扩展"——**核心抽象未变，只是窗口管理器变成了多屏调度器**。

**9. 终端模拟器的 tmux / screen**。`tmux` 把一个终端切成多个"窗格（pane）"，每个 pane 有独立的 PTY、独立的滚动缓冲区、独立的输入路由。**这是字符模式下的窗口系统**——证明窗口设计完全独立于"图形 vs 字符"。

**10. 微前端框架的 iframe / Web Components 沙箱**。qiankun、single-spa 等微前端把一个浏览器页面切成多个独立子应用，每个子应用有自己的 DOM 子树、独立的事件总线、独立的样式作用域——**这是 Web 内部对 OS 窗口隔离思想的二次实现**。

**这十个领域的共同点告诉我们一条普适规律**：

> **凡是"需要在共享资源上隔离多个互不信任 / 互不感知的内容单元"的场景，最终都会演化出窗口系统**。
> 资源可以是屏幕、3D 空间、终端字符面、CPU 时间、网络带宽——抽象都一样。
>
> 学会了窗口设计，你就学会了一种**通用的"资源仲裁器"建模能力**——这远比"会用 PhoneWindow"重要。
...
### 11.8 多 Scene keyWindow

```swift
// ❌ iOS 13+ 多 Scene 时，UIApplication.shared.keyWindow 在 iPad 上可能为 nil
let kw = UIApplication.shared.keyWindow   // Deprecated + nil
let root = kw?.rootViewController         // 拿到的可能是非当前 Scene
```

**原因**：iOS 13 引入 UIWindowScene 后，应用可能同时有多个 Scene（iPad 多任务、Mac Catalyst），全局 `keyWindow` 概念失效。

**修复**：从当前 Scene 取 key window。
...
**心智模型**：iOS 13 之后，**"全局唯一 keyWindow" 这个假设已死**——和 Android 多 Display、Web 多 Tab 一样，**多窗口时代必须用"当前作用域"取代"全局单例"**。

### 11.9 iframe z-index

```html
<!-- 宿主页面 -->
<div style="position:fixed; z-index:1000">宿主 Header</div>
<iframe src="child.html"></iframe>

<!-- iframe child.html 内 -->
<div style="position:fixed; z-index:99999999">我以为我最高</div>
<!-- 实际：被宿主 Header 完全盖住 -->
```

**原因**：iframe 是独立的 BrowsingContext，其内部的 z-index 只能在 iframe 的 stacking context 内排序。**iframe 本身在父页面里只有一个 z-index 值**——子页面再高也只能撑到 iframe 元素这一层。

**这与 Android 的 type 分段完全是同一思想**：
- Android：子窗口 type 不能超越系统窗口 type
- Web：iframe 内部 z-index 不能超越 iframe 自身在父文档中的 z-index

**修复**：弹层必须由宿主页面渲染（postMessage 通知 iframe → iframe 通过消息让宿主弹），或者使用 `<dialog>` 元素的 top layer。

```javascript
// iframe 内
window.parent.postMessage({ type: 'showModal', content: '...' }, '*');

// 宿主页面
window.addEventListener('message', e => {
    if (e.data.type === 'showModal') {
        showHostModal(e.data.content);   // 在宿主 stacking 体系里弹
    }
});
```

### 11.10 dispose 后 setState

```dart
// ❌ Flutter 经典 setState() called after dispose() 崩溃
class _MyWidgetState extends State<MyWidget> {
  @override
  void initState() {
    super.initState();
    Future.delayed(Duration(seconds: 5), () {
      setState(() {});   // 5 秒后 widget 可能已经被 dispose
    });
  }
}
```

**原因**：Flutter 中没有原生子窗口概念，但 Widget 的"挂载状态"等价于"窗口的 Attached 状态"——dispose 后任何更新都会报错。
...
// ✅ 修复 2：用 StreamSubscription / Timer，dispose 时取消
...
**心智模型**：Flutter 的 `mounted` 字段 = Android 的 `isFinishing()` = iOS 的 weak self = Web 的 `AbortController`——**所有平台都要求"异步操作必须感知宿主是否还在"**。这是窗口生命周期一致性原则的直接体现。
...
**任何平台开发者必备的「同名异姓」字典**——下次看陌生平台的窗口文档，先来查这张表：

| 通用概念 | Android | iOS | Web | Flutter | Qt | LVGL |
|---------|---------|-----|-----|---------|-----|------|
| **顶层窗口** | Activity 的 PhoneWindow | UIWindow | window 对象 | FlutterView | QWindow / 顶层 QWidget | `lv_scr_act()` |
| **窗口管理器** | WindowManagerService | UIApplication + WindowScene | 浏览器内核 Compositor | OverlayState | QGuiApplication | `lv_disp_t` |
| **窗口身份证** | WindowToken (IBinder) | UIWindow 指针 | BrowsingContext id | OverlayEntry key | QWindow 指针 | `lv_obj_t*` |
| **渲染表面** | Surface (GraphicBuffer) | CALayer.contents | Compositor Layer | Engine 内部 Skia Surface | QBackingStore | 全局 framebuffer |
| **合成器** | SurfaceFlinger | Render Server | Chromium Compositor | Impeller / Skia | KWin / DWM / Quartz | 软件 blit |
| **输入通道** | InputChannel | UIEvent 系统 | DOM Event Path | RenderBox.hitTest | QEvent 派发 | `lv_indev_t` |
| **子窗口** | Dialog / PopupWindow | Modal VC | iframe / `<dialog>` | OverlayEntry | Qt::Popup QWidget | 子 `lv_obj` |
| **层级控制** | type + Z 序 | windowLevel | z-index + stacking | OverlayEntry 顺序 | WindowFlags | 添加顺序 |
| **生命周期回调** | onCreate/Resume/Pause/Destroy | viewDidLoad/Appear/Disappear | load/visibilitychange/unload | initState/dispose | showEvent/closeEvent | event callback |

**把这张表贴在 IDE 旁边**——切换平台时不再需要"重新学窗口"，只需要查表"在新平台里它叫什么"。

### 12.4 本卷章节呼应

```
5.2 视图加载渲染设计       ─→ 窗口是"画框"，View 是"画作"——本篇定义画框，5.2 定义画作
5.3 图形渲染管线原理       ─→ SurfaceFlinger 跨进程 BufferQueue 是零拷贝合成的典范
5.4 手势事件设计灵魂       ─→ InputDispatcher → InputChannel → ViewRootImpl 在本章 §6.4 详述
5.5 消息机制设计思想       ─→ Choreographer 与 VSync 依赖主线程 Looper，窗口刷新依赖消息循环
5.6 跨进程通信设计         ─→ WMS 与应用进程靠 Binder，本章是 IPC 的"舞台"
5.7 组件生命周期管理       ─→ 窗口生命周期是 Activity / ViewController / FlutterView 生命周期的"地基"
5.8 页面导航与路由设计     ─→ 路由切换 = 窗口出栈入栈
5.9 响应式数据绑定设计     ─→ Surface 重绘 = 数据驱动 UI 的最终归宿

跨卷呼应：
- 第 4 卷·内存：§0 窗口泄漏 → Activity 无法 GC；BufferQueue → 零拷贝
- 第 3 卷·并发：主线程 Looper + VSync 是窗口刷新的核心调度
- 第 2 卷·编程基础：Dialog 持 Activity 强 / 弱引用的取舍
```
...
- 调试：`adb shell dumpsys window` / `adb shell dumpsys SurfaceFlinger`

**iOS 方向**：

- Apple：[UIWindow Documentation](https://developer.apple.com/documentation/uikit/uiwindow)
- Apple：[UIScene & Multi-Window Support](https://developer.apple.com/documentation/uikit/app_and_environment/scenes/)
- WWDC：Adopting Scenes for Multiple Windows
- 调试：Xcode View Hierarchy Debugger

**Web 方向**：

- W3C：[HTML Living Standard - Browsing Contexts](https://html.spec.whatwg.org/multipage/browsers.html)
...
- Flutter Engine：`engine/shell/platform/*` 各平台窗口适配层
- 文档：[Inside Flutter](https://docs.flutter.dev/resources/inside-flutter)
- 调试：DevTools Widget Inspector

**Qt / 桌面**：

- Qt 官方文档：[QWindow](https://doc.qt.io/qt-6/qwindow.html)、[QPlatformWindow](https://doc.qt.io/qt-6/qplatformwindow.html)
- Wayland 协议规范：[wayland.freedesktop.org](https://wayland.freedesktop.org/)
- 书：《Programming Windows》Charles Petzold（Win32 窗口圣经）

**嵌入式**：

- LVGL 官方文档：[lvgl.io/docs](https://docs.lvgl.io/)
- 书：《Embedded Graphics Programming with LVGL》

**跨平台原理书单**：

- 《计算机图形学：原理与实践》（Foley & van Dam）—— 窗口系统的数学基础
- 《Inside Microsoft Windows》—— Win32 窗口管理 + DWM 合成原理
