...
> 🔗 **延伸阅读**：← [5.1 窗口核心设计思想](https://yccoding.com/pages/2f009c/) · ← [5.2 视图加载渲染设计](https://yccoding.com/pages/070bb8/) · ← [5.5 消息机制设计思想](https://yccoding.com/pages/0aed31/) · ← [5.6 跨进程通信设计](https://yccoding.com/pages/c93f5a/) · → [5.8 页面导航与路由设计](https://yccoding.com/pages/aaaaaa/) · ↔ [3.x 并发之道](../03.并发的设计/) · ↔ [4.x 内存的真相](../04.内存的真相/)
...
- [00.从一个生命周期事故开始](#00从一个生命周期事故开始)
- [01.为什么需要生命周期](#01为什么需要生命周期)
  - [1.1 没有生命周期的世界：纯函数 vs 长生命组件](#11-没有生命周期的世界纯函数-vs-长生命组件)
  - [1.2 三大根因：所有权 / 资源 / 可见性](#12-三大根因所有权--资源--可见性)
  - [1.3 生命周期的设计目标](#13-生命周期的设计目标)
- [02.生命周期通用四件套契约](#02生命周期通用四件套契约)
  - [2.1 状态机（States）](#21-状态机states)
  - [2.2 钩子（Hooks）](#22-钩子hooks)
  - [2.3 所有权（Ownership）](#23-所有权ownership)
  - [2.4 可见性（Visibility）](#24-可见性visibility)
  - [2.5 跨端名词矩阵（七端对照）](#25-跨端名词矩阵七端对照)
- [03.Android 生命周期深度解析](#03android-生命周期深度解析)
  - [3.1 Activity 七大回调 + 状态机](#31-activity-七大回调--状态机)
  - [3.2 Fragment 13 个回调的设计](#32-fragment-13-个回调的设计)
  - [3.3 配置变更与进程死亡恢复](#33-配置变更与进程死亡恢复)
  - [3.4 ViewModel：解耦生命周期的银弹](#34-viewmodel解耦生命周期的银弹)
- [04.iOS / SwiftUI 生命周期](#04ios--swiftui-生命周期)
  - [4.1 UIViewController 五大回调](#41-uiviewcontroller-五大回调)
  - [4.2 ARC 与 deinit 时机](#42-arc-与-deinit-时机)
  - [4.3 SwiftUI View 的"声明式生命周期"](#43-swiftui-view-的声明式生命周期)
- [05.前端组件生命周期](#05前端组件生命周期)
  - [5.1 React Class 组件 16 个回调](#51-react-class-组件-16-个回调)
  - [5.2 React Hooks：useEffect 替代一切](#52-react-hooksuseeffect-替代一切)
  - [5.3 Vue Options API 八大钩子](#53-vue-options-api-八大钩子)
  - [5.4 Vue Composition API 与 onMounted 系列](#54-vue-composition-api-与-onmounted-系列)
- [06.跨端框架的"函数式生命周期"](#06跨端框架的函数式生命周期)
  - [6.1 Flutter StatefulWidget 11 阶段](#61-flutter-statefulwidget-11-阶段)
  - [6.2 Jetpack Compose 的 remember / DisposableEffect](#62-jetpack-compose-的-remember--disposableeffect)
  - [6.3 函数式生命周期 vs 命令式生命周期](#63-函数式生命周期-vs-命令式生命周期)
- [07.横向对比矩阵（七端 × 八维度）](#07横向对比矩阵七端--八维度)
  - [7.1 生命周期对齐表](#71-生命周期对齐表)
  - [7.2 设计哲学站位图](#72-设计哲学站位图)
- [08.生命周期反模式与陷阱](#08生命周期反模式与陷阱)
  - [8.1 反模式 1：在 onPause 做耗时操作](#81-反模式-1在-onpause-做耗时操作)
  - [8.2 反模式 2：异步回调引用销毁的组件](#82-反模式-2异步回调引用销毁的组件)
  - [8.3 反模式 3：Fragment 持有 Activity 强引用](#83-反模式-3fragment-持有-activity-强引用)
  - [8.4 反模式 4：闭包捕获 self 引发循环引用](#84-反模式-4闭包捕获-self-引发循环引用)
  - [8.5 反模式 5：忘记反注册监听器](#85-反模式-5忘记反注册监听器)
- [09.现代演进：响应式生命周期](#09现代演进响应式生命周期)
  - [9.1 LifecycleOwner + LiveData/Flow 自动绑定](#91-lifecycleowner--livedataflow-自动绑定)
  - [9.2 协程作用域：lifecycleScope / viewModelScope](#92-协程作用域lifecyclescope--viewmodelscope)
  - [9.3 Combine + SwiftUI 的"自动取消"](#93-combine--swiftui-的自动取消)
  - [9.4 函数式范式：副作用与生命周期统一](#94-函数式范式副作用与生命周期统一)
...
事故是「果」，生命周期设计是「因」。本篇将沿着这条线展开：
  ...
  §00 这个事故 ── 看懂"生命周期事故"的 5 个埋雷点（你现在在这）
        ↓
  §01 为什么需要生命周期 ── 理解 OS 为什么要发明这套机制（三大根因）
        ↓
  §02 通用四件套契约 ── 看清"任何平台都遵循"的生命周期骨架（跨端原理）
        ↓
  §03-§06 各端实现 ── Android / iOS / React / Vue / Flutter / Compose（具体差异）
        ↓
  §07 横向对比矩阵 ── 不同平台生命周期的「翻译表」
        ↓
  §08 反模式 ── §0 事故只是冰山一角，还有 5 个常见埋雷
        ↓
  §09 现代演进 ── 响应式 + 协程怎么让生命周期"自动管理"
        ↓
  §10 收口 ── 跨端心智 + 与全卷呼应
```
...
### 5.2 React Hooks：useEffect 替代一切
...
### 5.4 Vue Composition API 与 onMounted 系列
...
### 6.1 Flutter StatefulWidget 11 阶段
...
### 6.2 Jetpack Compose 的 remember / DisposableEffect
...
### 8.3 反模式 3：Fragment 持有 Activity 强引用
...
### 9.1 LifecycleOwner + LiveData/Flow 自动绑定
...
### 9.2 协程作用域：lifecycleScope / viewModelScope
