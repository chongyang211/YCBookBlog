...
> 🔗 **延伸阅读**：← [5.2 视图加载渲染设计](https://yccoding.com/pages/070bb8/) · → [5.4 手势事件设计灵魂](https://yccoding.com/pages/5b96db/) · → [5.5 消息机制设计思想](https://yccoding.com/pages/0aed31/) · → [5.6 跨进程通信设计](https://yccoding.com/pages/aaaaaa/)
...
- [01.像素的旅程：8阶段流水线](#01像素的旅程8阶段流水线)
    - [1.1 从Text(Hello)到屏幕发光](#11-从texthello到屏幕发光)
    - [1.2 16ms 是怎么来的](#12-16ms-是怎么来的)
    - [1.3 双缓冲为何必须](#13-04第二题双缓冲为何必须)
    - [1.4 GPU 是个流水线工厂](#14-gpu-是个流水线工厂)
    - [1.5 管线通用契约与跨端阶段矩阵](#15-管线通用契约与跨端阶段矩阵)
- [02.双缓冲与VSync：撕裂卡顿权衡](#02双缓冲与vsync撕裂卡顿权衡)
    - [2.1 VSync 信号的物理本质](#21-vsync-信号的物理本质)
    - [2.2 双缓冲 vs 三缓冲](#22-双缓冲-vs-三缓冲)
    - [2.3 VSync 的副作用：输入延迟](#23-vsync-的副作用输入延迟)
    - [2.4 帧调度的三段式](#24-帧调度的三段式)
    - [2.5 缓冲策略全谱](#25-缓冲策略全谱单缓冲到-mailbox-到-adaptive-sync)
- [03.合成器Compositor：分层渲染革命](#03合成器compositor分层渲染革命)
    - [3.1 为何需要分层](#31-04第四题为何需要分层)
    - [3.2 硬件合成 vs 软件合成](#32-硬件合成-vs-软件合成)
    - [3.3 过度绘制（Overdraw）](#33-过度绘制overdraw)
    - [3.4 Flutter 为何自绘](#34-06第二题flutter为何自绘)
    - [3.5 合成器跨端对照](#35-合成器跨端对照同一职责的五种实现)
- [04.OpenGL到Vulkan：图形API代际更替](#04opengl到vulkan图形api代际更替)
    - [4.1 OpenGL 的设计与局限](#41-opengl-的设计与局限)
    - [4.2 Vulkan/Metal/DX12 革命](#42-vulkanmetaldx12革命)
    - [4.3 Apple Metal 与移动端特殊性](#43-apple-metal与移动端特殊性)
    - [4.4 Flutter Impeller 为何诞生](#44-flutter-impeller为何诞生)
    - [4.5 图形 API 全景矩阵](#45-图形-api-全景矩阵覆盖-webgpu--directx--gles)
- [05.Skia：跨平台2D渲染事实标准](#05skia跨平台2d渲染事实标准)
- [5B.渲染后端选型矩阵](#5b渲染后端选型矩阵skiaimpellerwebrenderfilament)
- [06.跨平台渲染架构对照](#06跨平台渲染架构对照)
- [07.经典陷阱与生产级反模式](#07经典陷阱与生产级反模式)
    - [7.1 (Android) onDraw里new对象](#71-陷阱一ondraw里new对象)
    - [7.2 复杂 shader](#72-陷阱二复杂-shader)
    - [7.3 纹理过大或过多](#73-陷阱三纹理过大或过多)
    - [7.4 图层过多](#74-陷阱四图层过多)
    - [7.5 忽略 RenderThread 卡死](#75-陷阱五忽略renderthread卡死)
    - [7.6 动画用 setLayoutParams](#76-陷阱六动画用setlayoutparams)
    - [7.7 (iOS) 不设置 opaque](#77-陷阱七ios-不设置-opaque)
    - [7.8 (iOS) 阴影没设 shadowPath](#78-陷阱八ios阴影没设-shadowpath)
    - [7.9 (Web) 触发 Layout/Paint 的动画](#79-陷阱九web触发-layout--paint-的动画)
    - [7.10 (Flutter) 忘了升级到 Impeller](#710-陷阱十flutter忘了升级到-impeller)
    - [7.11 (嵌入式) framebuffer 撕裂](#711-陷阱十一嵌入式-lvgl-framebuffer-撕裂)
- [08.一句话总结](#08一句话总结)
    - [8.1 三层认知阶梯](#81-三层认知阶梯)
    - [8.2 渲染瓶颈定位决策树](#82-渲染瓶颈定位决策树)
    - [8.3 七字真言](#83-七字真言)
    - [8.4 跨端通用术语对照](#84-跨端通用术语对照一图带走)
    - [8.5 与本卷其它章节呼应](#85-与本卷其它章节呼应)
...
### 2.5 缓冲策略全谱（单缓冲到 Mailbox 到 Adaptive Sync）★
...
### 4.5 图形 API 全景矩阵（覆盖 WebGPU / DirectX / GLES） ★
...
## 5B.渲染后端选型矩阵（Skia/Impeller/WebRender/Filament）

§05 讲透了 Skia——但 Skia 不是唯一选择。**当你做"自绘引擎"或"高性能渲染"时，业界存在 5 个主流后端**，每个都代表一种工程哲学：
...
### 5B.2 Skia vs Impeller：为什么 Flutter 重写
...
### 7.9 陷阱九（Web）：触发 Layout / Paint 的动画
...
### 7.10 陷阱十（Flutter）：忘了升级到 Impeller
...
### 7.11 陷阱十一（嵌入式 LVGL）：framebuffer 撕裂
...
5.2 视图加载渲染设计       ─→ 本篇是 5.2 §03 三阶段之"draw"的下游延展
