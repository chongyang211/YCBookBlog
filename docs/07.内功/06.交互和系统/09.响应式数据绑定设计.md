...
> 🔗 **延伸阅读**：← [5.2 视图加载渲染设计](https://yccoding.com/pages/070bb8/) · ← [5.3 图形渲染管线原理](https://yccoding.com/pages/render-pipeline/) · ← [5.5 消息机制设计思想](https://yccoding.com/pages/message/) · ← [5.7 组件生命周期管理](https://yccoding.com/pages/lifecycle/) · ← [5.8 页面导航与路由设计](https://yccoding.com/pages/navigation/) · → [5.10 数据加密和解密](https://yccoding.com/pages/encryption/) · ↔ [6.x 架构思想 MVVM/MVI](../../07.架构演进/)
...
- [00.从一个数据不同步事故开始](#00从一个数据不同步事故开始)
- [01.为什么需要响应式](#01为什么需要响应式)
  - [1.1 命令式更新的三宗罪](#11-命令式更新的三宗罪)
  - [1.2 从观察者模式到响应式编程的演化](#12-从观察者模式到响应式编程的演化)
  - [1.3 响应式系统的设计目标](#13-响应式系统的设计目标)
- [02.响应式通用六件套契约](#02响应式通用六件套契约)
  - [2.1 可观察数据源（Observable）](#21-可观察数据源observable)
  - [2.2 依赖收集（Dependency Tracking）](#22-依赖收集dependency-tracking)
  - [2.3 变更通知（Notification）](#23-变更通知notification)
  - [2.4 视图调度（Scheduling / Batching）](#24-视图调度scheduling--batching)
  - [2.5 差异比对（Diffing / Reconciliation）](#25-差异比对diffing--reconciliation)
  - [2.6 生命周期绑定（Lifecycle-Aware）](#26-生命周期绑定lifecycle-aware)
  - [2.7 跨端名词矩阵（八端对照）](#27-跨端名词矩阵八端对照)
- [03.Web 响应式设计](#03web-响应式设计)
  - [3.1 Vue 2：Object.defineProperty 的依赖收集](#31-vue-2objectdefineproperty-的依赖收集)
  - [3.2 Vue 3：Proxy + Effect 的现代实现](#32-vue-3proxy--effect-的现代实现)
  - [3.3 React：useState + Fiber 的差异化哲学](#33-reactusestate--fiber-的差异化哲学)
  - [3.4 MobX / Solid / Svelte：细粒度响应式的三种路线](#34-mobx--solid--svelte细粒度响应式的三种路线)
- [04.Android 响应式体系](#04android-响应式体系)
  - [4.1 DataBinding：编译期生成的双向绑定](#41-databinding编译期生成的双向绑定)
  - [4.2 LiveData：生命周期感知的观察者](#42-livedata生命周期感知的观察者)
  - [4.3 Flow / StateFlow / SharedFlow：协程时代的响应式](#43-flow--stateflow--sharedflow协程时代的响应式)
  - [4.4 Jetpack Compose：Snapshot 状态系统](#44-jetpack-composesnapshot-状态系统)
- [05.iOS 响应式体系](#05ios-响应式体系)
  - [5.1 KVO：Objective-C Runtime 的运行时魔法](#51-kvoobjective-c-runtime-的运行时魔法)
  - [5.2 RxSwift / ReactiveCocoa：Rx 流派的胜利](#52-rxswift--reactivecocoarx-流派的胜利)
  - [5.3 Combine：Apple 官方的 Publisher/Subscriber](#53-combineapple-官方的-publishersubscriber)
  - [5.4 SwiftUI：@State / @Observable 的声明式革命](#54-swiftuistate--observable-的声明式革命)
- [06.跨端框架的响应式设计](#06跨端框架的响应式设计)
  - [6.1 Flutter：setState / ValueNotifier / Riverpod 三层](#61-fluttersetstate--valuenotifier--riverpod-三层)
  - [6.2 React Native：与 React 一脉相承](#62-react-native与-react-一脉相承)
  - [6.3 小程序 setData：脏检查 + 跨进程同步](#63-小程序-setdata脏检查--跨进程同步)
  - [6.4 鸿蒙 ArkTS：@State / @Link / @Provide 装饰器体系](#64-鸿蒙-arkts@state--@link--@provide-装饰器体系)
- [07.横向对比矩阵（八端 × 九维度）](#07横向对比矩阵八端--九维度)
  - [7.1 响应式能力对齐表](#71-响应式能力对齐表)
  - [7.2 设计哲学站位图](#72-设计哲学站位图)
- [08.响应式反模式与陷阱](#08响应式反模式与陷阱)
  - [8.1 反模式 1：在 render 中产生副作用](#81-反模式-1在-render-中产生副作用)
  - [8.2 反模式 2：直接修改不可变数据](#82-反模式-2直接修改不可变数据)
  - [8.3 反模式 3：依赖循环与无限渲染](#83-反模式-3依赖循环与无限渲染)
  - [8.4 反模式 4：内存泄漏（订阅未解除）](#84-反模式-4内存泄漏订阅未解除)
  - [8.5 反模式 5：滥用全局响应式状态](#85-反模式-5滥用全局响应式状态)
- [09.现代演进：细粒度响应式与编译时优化](#09现代演进细粒度响应式与编译时优化)
  - [9.1 Signal 派 vs VDom 派：两条技术路线的对决](#91-signal-派-vs-vdom-派两条技术路线的对决)
  - [9.2 编译时响应式：Svelte / Solid / Vue Vapor](#92-编译时响应式svelte--solid--vue-vapor)
  - [9.3 时间旅行调试与状态机化](#93-时间旅行调试与状态机化)
  - [9.4 服务端响应式与边缘渲染](#94-服务端响应式与边缘渲染)
- [10.收口：跨端心智与设计哲学](#10收口跨端心智与设计哲学)
  - [10.1 跨端通用术语对照表](#101-跨端通用术语对照表)
  - [10.2 与本卷其它章节的呼应](#102-与本卷其它章节的呼应)
  - [10.3 通用心智总结（七句口诀）](#103-通用心智总结七句口诀)
...
#### 1.1.1 第一宗罪：**散弹枪修改（Shotgun Surgery）**
...
#### 1.1.2 第二宗罪：**派生数据不新鲜（Stale Derived State）**
...
#### 1.1.3 第三宗罪：**多数据源打架（Multiple Sources of Truth）**
...
### 2.5 差异比对（Diffing / Reconciliation）
...
**这张表是本章最重要的"罗塞塔石碑"**——后面 §03~§06 的所有内容，都是在这张表的某一列里展开细节。

---

## 03.Web 响应式设计

### 3.1 Vue 2：Object.defineProperty 的依赖收集
...
### 3.3 React：useState + Fiber 的差异化哲学
...
### 3.4 MobX / Solid / Svelte：细粒度响应式的三种路线
...
### 4.3 Flow / StateFlow / SharedFlow：协程时代的响应式
...
### 4.4 Jetpack Compose：Snapshot 状态系统
...
### 5.1 KVO：Objective-C Runtime 的运行时魔法
...
### 5.2 RxSwift / ReactiveCocoa：Rx 流派的胜利
...
### 5.3 Combine：Apple 官方的 Publisher/Subscriber
...
### 5.4 SwiftUI：@State / @Observable 的声明式革命
...
### 6.1 Flutter：setState / ValueNotifier / Riverpod 三层
...
### 6.4 鸿蒙 ArkTS：@State / @Link / @Provide 装饰器体系
...
## 09.现代演进：细粒度响应式与编译时优化

### 9.1 Signal 派 vs VDom 派：两条技术路线的对决
...
### 9.2 编译时响应式：Svelte / Solid / Vue Vapor
...
| 本章 §X | 呼应章节 | 关系 |
|---|---|---|
| §02 六件套 | [5.2 视图加载渲染](https://yccoding.com/pages/070bb8/) | 响应式触发的最终是视图重绘——measure/layout/draw 是被响应式调用的执行层 |
| §02.4 调度 | [5.3 图形渲染管线](https://yccoding.com/pages/render-pipeline/) | 调度合并多次更新到一帧，配合 VSync 保证 60FPS |
| §02.6 生命周期 | [5.7 组件生命周期](https://yccoding.com/pages/lifecycle/) | LifecycleOwner 是响应式自动解绑的依赖 |
| §05.4 SwiftUI | [5.8 页面导航](https://yccoding.com/pages/navigation/) | NavigationPath 本质就是响应式数组 |
| §02.3 通知 | [5.5 消息机制设计](https://yccoding.com/pages/message/) | 通知最终走 Handler/RunLoop/Microtask 队列 |
| §08 反模式 | [4.x 内存的真相](../04.内存的真相/) | 订阅未解 = 强引用链 = OOM 根源 |
| §09 状态机 | [6.x 架构思想](../../07.架构演进/) | 响应式是 MVVM/MVI 的运行时基石 |
...
下一篇 [5.10 数据加密和解密](https://yccoding.com/pages/encryption/) 会讨论数据从内存离开到磁盘/网络的最后一道防线——**对称加密 / 非对称加密 / 哈希 / 数字签名 / TLS / 端到端加密**。响应式解决了"数据怎么在 UI 里流动"，加密解决了"数据怎么在世界里安全地流动"。两者结合，才是完整的"数据生命周期"。
