...
> 🔗 **延伸阅读**：← [5.1 窗口核心设计思想](https://yccoding.com/pages/cdba23/) · → [5.3 图形渲染管线原理](https://yccoding.com/pages/318066/) · → [5.4 手势事件设计灵魂](https://yccoding.com/pages/5b96db/) · → [5.7 组件生命周期管理](https://yccoding.com/pages/aaaaaa/)
...
#### 目录介绍
- [00.一次60帧变或6帧事故说起](#00一次60帧变或6帧事故说起)
...
- [02.视图加载的完整生命周期](#02视图加载的完整生命周期)
    ...
    - [2.5 通用『视图四态』状态机](#25-通用视图四态状态机跨端心智模型)
- [03.三大渲染阶段：Measure、Layout、Draw](#03三大渲染阶段measurelayoutdraw)
    ...
    - [3.6 三阶段通用算法签名](#36-三阶段通用算法签名跨端契约)
- [04.GPU 与硬件加速](#04gpu-与硬件加速)
    ...
    - [4.5 渲染线程模型跨端对照](#45-渲染线程模型跨端对照通用心智)
...
- [7B.声明式 vs 命令式渲染](#7b声明式-vs-命令式渲染范式之争)
    - [7B.1 命令式渲染的内在矛盾](#7b1-命令式渲染的内在矛盾)
    - [7B.2 声明式渲染的核心思想](#7b2-声明式渲染的核心思想)
    - [7B.3 五大声明式框架对比](#7b3-五大声明式框架对比)
    - [7B.4 声明式的代价](#7b4-声明式的代价)
- [08.跨平台渲染机制对比](#08跨平台渲染机制对比)
    ...
    - [8.6 嵌入式 LVGL：软光栅极简版](#86-嵌入式-lvgl软光栅极简版)
    - [8.7 跨端渲染阶段矩阵](#87-跨端渲染阶段矩阵一图对照)
- [09.经典陷阱与生产级反模式](#09经典陷阱与生产级反模式)
    - [9.1 陷阱一（Android）：onDraw 里 new 对象](#91-陷阱一androidondraw里new对象)
    - [9.2 陷阱二（Android）：过度绘制的隐形杀手](#92-陷阱二android过度绘制隐形杀手)
    - [9.3 陷阱三（Android）：嵌套布局的指数爆炸](#93-陷阱三android嵌套布局指数爆炸)
    - [9.4 陷阱四（Android）：主线程加载大图导致掉帧](#94-陷阱四android主线程加大图掉帧)
    - [9.5 陷阱五（Android）：动画期间触发 requestLayout](#95-陷阱五android动画中requestlayout)
    - [9.6 陷阱六（iOS）：cornerRadius 触发离屏渲染](#96-陷阱六ioscornerradius-触发离屏渲染)
    - [9.7 陷阱七（Web）：强制同步布局 layout thrashing](#97-陷阱七web强制同步布局-layout-thrashing)
    - [9.8 陷阱八（Flutter）：build 方法里做耗时操作](#98-陷阱八flutterbuild方法里做耗时操作)
    - [9.9 陷阱九（Compose/React）：过度 recompose 与闭包陷阱](#99-陷阱九composereact过度-recompose-与闭包陷阱)
- [10.一句话总结：渲染的设计哲学](#10一句话总结渲染设计哲学)
    - [10.1 渲染的三层认知阶梯](#101-渲染的三层认知阶梯)
    - [10.2 优化决策清单](#102-优化决策清单贴工位旁边)
    - [10.3 设计哲学一句话](#103-设计哲学一句话)
    - [10.4 跨端通用术语对照](#104-跨端通用术语对照一图带走)
    - [10.5 与本卷其它章节的呼应](#105-与本卷其它章节的呼应)
    - [10.6 延伸阅读](#106-延伸阅读)
...
### 9.5 陷阱五（Android）：动画中requestLayout
...
### 9.6 陷阱六（iOS）：cornerRadius 触发离屏渲染
...
### 9.7 陷阱七（Web）：强制同步布局 layout thrashing
...
### 9.8 陷阱八（Flutter）：build 方法里做耗时操作
...
### 9.9 陷阱九（Compose/React）：过度 recompose 与闭包陷阱
...
5.3 图形渲染管线原理       ─→ 本篇 §04 GPU/合成 在 5.3 详细展开 + Vulkan/Metal 演进
