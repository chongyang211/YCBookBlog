...
> 5.2 讲了视图渲染的"画什么"、5.3 讲了 GPU 管线的"怎么画"——但**画出来的字长啥样、按什么方向排、按什么规则大小写**，才是用户真正看到的东西。
>
> 大多数应用层程序员对国际化的认知停在"翻译字符串"——直到 App 出海遇到「阿拉伯语从右到左」「德语单词长度爆表」「土耳其语 i 大写不是 I」「印度数字系统不是 0-9」——才明白国际化是一整条**产品/工程/文化**的系统工程。本篇要把「英文写死 → 全球化产品」这条完整路径走一遍，揭开 Locale、Unicode、ICU、CLDR、RTL、复数规则的所有底层秘密。

#### 目录介绍
- [0.真实事故引入](#0真实事故引入)
  - [0.1 阿拉伯语用户投诉](#01-阿拉伯语用户投诉)
  - [0.2 德语按钮撑爆](#02-德语按钮撑爆)
  - [0.3 灵魂三问](#03-灵魂三问)
  - [0.4 五个递进追问](#04-五个递进追问)
  - [0.5 探索路径](#05-探索路径)
  - [0.6 为何值得讲透](#06-为何值得讲透)
- [1.国际化本质](#1国际化本质)
  - [1.1 i18n 与 l10n 之别](#11-i18n-与-l10n-之别)
  - [1.2 Locale 三要素](#12-locale-三要素)
  - [1.3 Unicode 的救赎](#13-unicode-的救赎)
  - [1.4 CLDR 数据源](#14-cldr-数据源)
  - [1.5 五层协议模型](#15-五层协议模型)
- [2.多语言资源体系](#2多语言资源体系)
  - [2.1 键值分离思想](#21-键值分离思想)
  - [2.2 资源查找回退链](#22-资源查找回退链)
  - [2.3 复数与性别规则](#23-复数与性别规则)
  - [2.4 占位符与插值](#24-占位符与插值)
  - [2.5 资源打包与热更](#25-资源打包与热更)
- [3.格式化 API 全景](#3格式化-api-全景)
  - [3.1 数字格式化](#31-数字格式化)
  - [3.2 货币格式化](#32-货币格式化)
  - [3.3 日期时间格式化](#33-日期时间格式化)
  - [3.4 相对时间与列表](#34-相对时间与列表)
  - [3.5 姓名地址格式化](#35-姓名地址格式化)
- [4.RTL 与视觉适配](#4rtl-与视觉适配)
  - [4.1 RTL 语言清单](#41-rtl-语言清单)
  - [4.2 镜像与不镜像](#42-镜像与不镜像)
  - [4.3 双向文本 Bidi](#43-双向文本-bidi)
  - [4.4 字体与字重回退](#44-字体与字重回退)
  - [4.5 布局伸缩策略](#45-布局伸缩策略)
- [5.时区与日历](#5时区与日历)
  - [5.1 UTC/本地/时区 ID](#51-utc本地时区-id)
  - [5.2 夏令时陷阱](#52-夏令时陷阱)
  - [5.3 非公历日历](#53-非公历日历)
  - [5.4 存储与展示分离](#54-存储与展示分离)
- [6.输入法与文本处理](#6输入法与文本处理)
  - [6.1 IME 输入协商](#61-ime-输入协商)
  - [6.2 大小写与折叠](#62-大小写与折叠)
  - [6.3 排序 Collation](#63-排序-collation)
  - [6.4 分词与断行](#64-分词与断行)
- [7.跨平台落地矩阵](#7跨平台落地矩阵)
  - [7.1 Android 资源体系](#71-android-资源体系)
  - [7.2 iOS Localizable](#72-ios-localizable)
  - [7.3 Web Intl 与 i18n 库](#73-web-intl-与-i18n-库)
  - [7.4 Flutter 与 ARB](#74-flutter-与-arb)
  - [7.5 嵌入式精简方案](#75-嵌入式精简方案)
  - [7.6 服务端语言协商](#76-服务端语言协商)
- [8.经典陷阱与反模式](#8经典陷阱与反模式)
  - [8.1 拼接字符串](#81-拼接字符串)
  - [8.2 硬编码货币符号](#82-硬编码货币符号)
  - [8.3 用字符串排序数字](#83-用字符串排序数字)
  - [8.4 忽略复数规则](#84-忽略复数规则)
  - [8.5 硬编码日期格式](#85-硬编码日期格式)
  - [8.6 图片里写死文字](#86-图片里写死文字)
  - [8.7 忘记 RTL 镜像](#87-忘记-rtl-镜像)
  - [8.8 土耳其语 I 大写](#88-土耳其语-i-大写)
  - [8.9 时区当 UTC 存](#89-时区当-utc-存)
  - [8.10 字体不含目标语言](#810-字体不含目标语言)
- [9.总结](#9总结)
  - [9.1 三层认知阶梯](#91-三层认知阶梯)
  - [9.2 出海项目决策清单](#92-出海项目决策清单)
  - [9.3 八字真言](#93-八字真言)
  - [9.4 跨端术语对照](#94-跨端术语对照)
  - [9.5 本卷章节呼应](#95-本卷章节呼应)
  - [9.6 与本卷承接](#96-与本卷承接)
...
**§0.3 第三题答案**——Unicode 联盟做的不只是"字符编码"，还有 **CLDR（Common Locale Data Repository）**：
...
**§0.4 第二题答案**——找不到翻译时怎么办？**回退链**（Fallback Chain）。
...
**§0.3 第二题答案**——为什么同样是 1234.56，各地写法完全不同？因为**数字格式**本身就是文化的一部分。
...
**§0.4 第四题答案**——RTL 不只是"从右往左写字"，是**整个视觉系统的镜像**。
...
**§0.4 第五题答案**——为什么时间是国际化"最容易翻车"的部分？因为**时间 = UTC 时刻 + 时区偏移 + 日历系统**——三层容易混淆。
...
5.2 视图加载渲染设计       ─→ 布局伸缩策略是 §4.5 的实现基础
