---
title: 7.内存泄漏与诊断原理
date: 2026-05-14 19:38:29
permalink: /pages/7c795f/
categories:
  - book
  - 通用技术的提升
  - 程序编程原理
...
> 🔗 **延伸阅读**：← [4.5 内存回收机制设计](https://yccoding.com/pages/8327cf/) · ← [4.6 多种引用技术设计](https://yccoding.com/pages/9ff724/) · → [4.8 数据拷贝设计原理](https://yccoding.com/pages/3d7b39/)
...
#### 目录介绍
- [00.真实事故引入](#00真实事故引入)
  - [0.1 午夜OOM难找](#01-午夜oom难找)
  - [0.2 100MB/天泄漏](#02-100mb天泄漏)
  - [0.3 灵魂三问](#03-灵魂三问)
  - [0.4 五个递进追问](#04-五个递进追问)
  - [0.5 探索路径](#05-探索路径)
  - [0.6 为何值得讲透](#06-为何值得讲透)
  - [0.7 泄漏五语言对照](#07-泄漏五语言对照)
- [01.内存泄漏本质](#01内存泄漏本质)
  - [1.1 GC活与业务活](#11-gc活与业务活)
  - [1.2 泄漏四种范式](#12-泄漏四种范式)
  - [1.3 四范式共同本质](#13-四范式共同本质)
  - [1.4 资源vs内存泄漏](#14-资源vs内存泄漏)
- [02.引用链分析](#02引用链分析)
  - [2.1 GC Roots起点](#21-gc-roots起点)
  - [2.2 Shallow与Retained](#22-shallow与retained)
  - [2.3 支配树](#23-支配树)
  - [2.4 GC Root路径](#24-gc-root路径)
- [03.Heap Dump原理](#03heap-dump原理)
  - [3.1 触发时机](#31-触发时机)
  - [3.2 STW 代价](#32-stw-代价)
  - [3.3 HPROF 格式](#33-hprof-格式)
  - [3.4 增量流式dump](#34-增量流式dump)
- [04.Profiler原理](#04profiler原理)
  - [4.1 采样与精确](#41-采样与精确)
  - [4.2 AllocationTracking](#42-allocationtracking)
  - [4.3 火焰图设计](#43-火焰图设计)
  - [4.4 Async Profiler](#44-async-profiler)
- [05.移动端的特殊难题](#05移动端的特殊难题)
  - [5.1 LeakCanary革命](#51-leakcanary革命)
  - [5.2 iOS MLeaksFinder](#52-ios-mleaksfinder)
  - [5.3 Valgrind与ASan](#53-valgrind与asan)
- [06.跨平台诊断工具对照](#06跨平台诊断工具对照)
  - [6.1 工具矩阵](#61-工具矩阵)
  - [6.2 pprof vs MAT](#62-pprof-vs-mat)
  - [6.3 Node heap](#63-node-heap)
  - [6.4 Python追踪](#64-python追踪)
  - [6.5 采样vs快照](#65-采样vs快照)
- [07.经典陷阱反模式](#07经典陷阱反模式)
  - [7.1 监听器忘反注册](#71-监听器忘反注册)
  - [7.2 缓存无界增长](#72-缓存无界增长)
  - [7.3 ThreadLocal未清](#73-threadlocal未清)
  - [7.4 单例持Activity](#74-单例持activity)
  - [7.5 内部类隐式持有](#75-内部类隐式持有)
  - [7.6 监控指标缺失](#76-监控指标缺失)
  - [7.7 生产dump大堆](#77-生产dump大堆)
- [08.综合案例串讲](#08综合案例串讲)
  - [8.1 时间线总览](#81-时间线总览)
  - [8.2 电商搜索背景](#82-电商搜索背景)
  - [8.3 T+0告警大盘](#83-t0告警大盘)
  - [8.4 T+1抓Heap Dump](#84-t1抓heap-dump)
  - [8.5 T+2 MAT支配树](#85-t2-mat支配树)
  - [8.6 T+3.5锁定真凶](#86-t35锁定真凶)
  - [8.7 T+5 Fix提交](#87-t5-fix提交)
  - [8.8 T+12 JFR监控](#88-t12-jfr监控)
  - [8.9 知识点回归](#89-知识点回归)
...
§00 已经铺好了"凌晨告警"的故事开头，前面 7 节把"本质 / 引用链 / Heap Dump / Profiler / 移动端 / 工具 / 陷阱"逐项拆开。这一节把它们串成一条**真实可操作的 24 小时排查时间线**——一个工程师从接到告警到提交修复 PR 的全过程。

### 8.1 时间线总览

```mermaid
gantt
    title 内存泄漏排查 24 小时全链路
    dateFormat HH:mm
    axisFormat %H:%M
    section 告警阶段
    P99 OOM 告警触发     :crit, t1, 02:00, 30m
    section 诊断阶段
    jstat 快速判断    :t2, 02:30, 30m
    Heap Dump 8GB     :t3, 03:00, 60m
    section 分析阶段
    MAT Dominator Tree   :t4, 04:00, 90m
    Path to GC Root      :t5, 05:30, 60m
    section 定位阶段
    锁定 ThreadLocalMap  :crit, t6, 06:30, 60m
    Code Review          :t7, 07:30, 60m
    section 修复阶段
    本地复现 + Fix       :t8, 09:00, 180m
    回归测试             :t9, 12:00, 120m
    section 验证阶段
    JFR 持续监控         :t10, 14:00, 600m
    确认无泄漏           :crit, t11, 24:00, 30m
```
...
业务诉求：**先恢复线上**（重启），但**必须找到根因**（团队连续 3 周受困）。
...
**11.8 GB 全在 1024 个 Thread 里**——这就是著名的 **`ThreadLocal` 泄漏 + 线程池**组合拳。
...
2. `Tomcat` 线程池复用线程 → 每个线程的 ThreadLocalMap 累计**所有它处理过的请求**的 SearchContext
...
修复部署后不能马上下班——必须**用 JFR 持续录制 12 小时**确认无泄漏：
...
下一篇 [4.8 数据拷贝设计原理](https://yccoding.com/pages/3d7b39/) 是本卷的收束——讨论"内存搬运"的工程艺术：从浅拷贝/深拷贝、到 zero-copy、再到 mmap 和 sendfile。**所有数据传输的性能优化，最终都归结为"少搬一次"**。

---

## 🔗 延伸阅读

- 同卷上篇：[4.5 内存回收机制设计](https://yccoding.com/pages/8327cf/) ｜ [4.6 多种引用技术设计](https://yccoding.com/pages/9ff724/)
- 同卷下篇：[4.8 数据拷贝设计原理](https://yccoding.com/pages/3d7b39/)
- 经典文献：
