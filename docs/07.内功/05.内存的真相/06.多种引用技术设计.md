...
> 🔗 **延伸阅读**：← [33.内存回收机制设计](https://yccoding.com/pages/8327cf/) · → [35.数据拷贝设计原理](https://yccoding.com/pages/3d7b39/)
...
#### 目录介绍
- [00.Activity泄漏事故](#00activity泄漏事故)
  - [0.1 用户投诉与日志](#01-用户投诉与日志)
  - [0.2 LeakCanary看链](#02-leakcanary看链)
  - [0.3 老司机灵魂三问](#03-老司机灵魂三问)
  - [0.4 事故揭示了什么](#04-事故揭示了什么)
  - [0.5 五个递进追问](#05-五个递进追问)
  - [0.6 循环引用五语言](#06-循环引用五语言)
- [01.单一引用的困境](#01单一引用的困境)
  - [1.1 朴素引用世界](#11-朴素引用世界)
  - [1.2 循环引用泄漏](#12-循环引用泄漏)
  - [1.3 缓存膨胀OOM](#13-缓存膨胀oom)
  - [1.4 观察者隐式泄漏](#14-观察者隐式泄漏)
  - [1.5 临时关联难表达](#15-临时关联难表达)
- [02.引用强度分级](#02引用强度分级)
  - [2.1 现实世界的类比](#21-现实世界的类比)
  - [2.2 强度偏序关系](#22-强度偏序关系)
  - [2.3 三条核心原则](#23-三条核心原则)
- [03.可达性模型](#03可达性模型)
  - [3.1 可达性层次定义](#31-可达性层次定义)
  - [3.2 最强路径决定一切](#32-最强路径决定一切)
  - [3.3 计数vs追踪](#33-计数vs追踪)
- [04.强引用](#04强引用)
  - [4.1 强引用为何默认](#41-强引用为何默认)
  - [4.2 shared_ptr控制块](#42-shared_ptr控制块)
  - [4.3 追踪式可达性](#43-追踪式可达性)
  - [4.4 强引用致命陷阱](#44-强引用致命陷阱)
- [05.软引用](#05软引用)
  - [5.1 图片缓存演化](#51-图片缓存演化)
  - [5.2 LRU时间戳机制](#52-lru时间戳机制)
  - [5.3 Java独有软引用](#53-java独有软引用)
  - [5.4 软引用真实陷阱](#54-软引用真实陷阱)
- [06.弱引用](#06弱引用)
  - [6.1 弱引用根本矛盾](#61-弱引用根本矛盾)
  - [6.2 weak_ptr打破循环](#62-weak_ptr打破循环)
  - [6.3 weak拿手监听器](#63-weak拿手监听器)
  - [6.4 weak与unowned](#64-weak与unowned)
  - [6.5 弱引用访问模式](#65-弱引用访问模式)
- [07.虚引用](#07虚引用)
  - [7.1 finalize反模式](#71-finalize反模式)
  - [7.2 Phantom设计巧思](#72-phantom设计巧思)
  - [7.3 JS Finalization](#73-js-finalization)
- [08.跨语言引用全景](#08跨语言引用全景)
  - [8.1 全景对比表](#81-全景对比表)
  - [8.2 C++四级智能指针](#82-c四级智能指针)
  - [8.3 Java四种引用](#83-java四种引用)
  - [8.4 ObjC/Swift表](#84-objcswift表)
  - [8.5 五语言API速查](#85-五语言api速查)
- [09.经典陷阱与反模式](#09经典陷阱与反模式)
  - [9.1 Activity泄漏修法](#91-activity泄漏修法)
  - [9.2 LRU缓存最佳实践](#92-lru缓存最佳实践)
  - [9.3 EventBus自动清理](#93-eventbus自动清理)
  - [9.4 ThreadLocal+弱](#94-threadlocal弱)
- [10.综合案例串讲](#10综合案例串讲)
  - [10.1 日活千万背景](#101-日活千万背景)
  - [10.2 四级缓存梯度](#102-四级缓存梯度)
  - [10.3 L1强引用Bitmap](#103-l1强引用bitmap)
  - [10.4 L2 LRU内存缓存](#104-l2-lru内存缓存)
  - [10.5 L3软引用缓存](#105-l3软引用缓存)
  - [10.6 L4弱引用退场](#106-l4弱引用退场)
  - [10.7 L5虚引用兜底](#107-l5虚引用兜底)
  - [10.8 综合演练流转](#108-综合演练流转)
  - [10.9 多语言对照](#109-多语言对照)
  - [10.10 知识点回归](#1010-知识点回归)
  - [10.11 一句话提炼](#1011-一句话提炼)
- [11.一句话总结](#11一句话总结)
  - [11.1 三层认知阶梯](#111-三层认知阶梯)
  - [11.2 引用选择决策](#112-引用选择决策)
  - [11.3 设计哲学一句](#113-设计哲学一句)
  - [11.4 七字真言映射](#114-七字真言映射)
  - [11.5 与本卷的呼应](#115-与本卷的呼应)
  - [11.6 延伸阅读](#116-延伸阅读)
...
前面 9 节把"强 / 软 / 弱 / 虚 / 跨语言 / 陷阱"逐项拆开。这一节用一个**真实的图片浏览 App**——类似 Glide / Picasso 的设计——把全章 4 种引用类型在一个完整业务里**全部用上**，看看它们如何配合形成"内存自适应"的全景方案。
...
```mermaid
flowchart TB
    subgraph L1[L1 当前显示<br/>强引用]
        VW[ImageView ── ▶ Bitmap]
    end
    subgraph L2[L2 LRU 内存缓存<br/>强引用 + 容量限制]
        LRU[LruCache size=heap/8<br/>最近 50 张图]
    end
    subgraph L3[L3 软引用二级缓存<br/>SoftReference]
        SR[SoftReferenceMap<br/>能保留就保留<br/>OOM 前自动放]
    end
    subgraph L4[L4 弱引用监听器<br/>WeakReference]
        WR[Activity → 注册回调<br/>EventBus → WeakRef]
    end
    subgraph L5[L5 虚引用清理<br/>PhantomReference]
        PR[Bitmap.recycle 兜底<br/>Cleaner 释放 native]
    end
    Net[网络 / 磁盘] --> L3
    L3 -.压力大时.-> L2
    L2 -.访问命中.-> L1
    L4 -.Activity 销毁.-> Cleanup[自动清理]
    L1 --> L5
    style L1 fill:#f8d7da
    style L2 fill:#ffeaa7
    style L3 fill:#d4edda
    style L4 fill:#a8e6cf
    style L5 fill:#b6e0fe
```
...
**关键设计**：被踢出 LRU 不是直接释放，而是**降级到 §10.5 的软引用层**——这就是工程上"分层退化"的精妙。
...
**软引用的真正贡献**：**OOM 之前 GC 会先回收所有软引用**——这意味着**只要还有内存，缓存就保留**。换句话说：有内存时 = LRU 命中；内存吃紧 = 软引用降级；快 OOM 时 = 软引用全部释放。
...
**关键收益**：业务代码**完全不需要写 unregister()** —— Activity 一关闭，weak ref 自动失效，监听器自动从列表里被清掉。**这就是 §06.3 弱引用的拿手戏**。
...
sequenceDiagram
    ...
    participant A as Activity A<br/>(瀑布流)
    participant Loader as ImageLoader
    participant L1 as L1 ImageView
    participant L2 as L2 LRU
    participant L3 as L3 SoftRef
    participant GC as GC
    User->>A: 滑动浏览 100 张图
    A->>Loader: load(url1..100)
    Loader->>L1: 强引用每张到 ImageView
    L1->>L2: 离屏后进入 LRU
    L2->>L3: 50 张超出 LRU 容量，降级软引用
    User->>A: 关闭 Activity A
    A->>GC: 不再被强持
    GC->>GC: A 不可达 → 回收 Activity 自身
    GC->>L1: ImageView 也不可达 → 回收
    Note over GC,L3: L2 LRU 被 ImageLoader 持有（应用级）<br/>L3 软引用根据内存压力存活
    User->>User: 内存足够 → L3 全部命中
    User->>User: 切到耗内存的视频应用 → 系统压力<br/>GC 回收所有 SoftRef
    User->>User: 切回 App → L3 全失效，重新下载
```
