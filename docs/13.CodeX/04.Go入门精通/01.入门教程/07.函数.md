...
**业务代码内禁止 `recover`**——它会把"程序员该修的 bug"伪装成"运行时错误"。详见 [11. 错误处理](./11.错误处理.md)。
...
**根因**：`error` 是 interface，由（类型，值）二元组构成。`(*MyErr, nil)` 这个 interface 不是 nil interface（`(nil, nil)` 才是）。详见 [10. 接口与多态](./10.接口与多态.md)。
...
### 卷内
- 第 6 章 [流程语句](./06.流程语句.md)（控制流基础）
- 第 8 章 [指针与逃逸](./08.指针与逃逸.md)（闭包变量逃逸）
- 第 10 章 [接口与多态](./10.接口与多态.md)（nil interface 陷阱）
- 第 11 章 [错误处理](./11.错误处理.md)（error 模式 + recover 边界）
