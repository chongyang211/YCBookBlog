---
title: LVGL设计原理
date: 2025-08-06 16:35:51
permalink: /pages/8e6bc7/
categories:
  - book
  - Linux应用开发
  - Linux实践开发
...
- `DisplayService` 只管**底层驱动**，不关心具体 UI 内容
- `BaseUi` 管理**页面级**生命周期（一个 screen 对象 = 一个完整页面）
- `BaseWidget` 管理**组件级**生命周期（一个 Widget = 页面内的一个独立功能块）
- `OperationUi` 通过**组合模式**持有多个 Widget，按状态机切换显示
