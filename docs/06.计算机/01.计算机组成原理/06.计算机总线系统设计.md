---
title: 计算机总线系统设计
date: 2021-06-26 14:21:17
permalink: /pages/93f155/
categories:
  - book
  - 通用技术的提升
  - 计算机组成原理
...
#### 目录介绍
- [01.工作案例引入](#01工作案例引入)
  - [1.1 双实例性能鸿沟](#11-双实例性能鸿沟)
  - [1.2 排查思路与结论](#12-排查思路与结论)
  - [1.3 本文要回答的问题](#13-本文要回答的问题)
- [02.总线的基本概念](#02总线的基本概念)
  - [2.1 什么是总线](#21-什么是总线)
  - [2.2 为什么需要总线](#22-为什么需要总线)
  - [2.3 总线的基本特征](#23-总线的基本特征)
- [03.总线的分类](#03总线的分类)
  - [3.1 按功能分类](#31-按功能分类)
  - [3.2 按层次分类](#32-按层次分类)
  - [3.3 按传输方式分类](#33-按传输方式分类)
- [04.总线的关键指标](#04总线的关键指标)
  - [4.1 总线宽度](#41-总线宽度)
  - [4.2 总线频率](#42-总线频率)
  - [4.3 总线带宽](#43-总线带宽)
  - [4.4 总线性能瓶颈](#44-总线性能瓶颈)
- [05.总线仲裁](#05总线仲裁)
  - [5.1 为什么需要仲裁](#51-为什么需要仲裁)
  - [5.2 集中仲裁](#52-集中仲裁)
  - [5.3 分布仲裁](#53-分布仲裁)
- [06.总线标准演进](#06总线标准演进)
  - [6.1 ISA到PCI](#61-isa到pci)
  - [6.2 PCI到PCIe](#62-pci到pcie)
  - [6.3 PCIe核心设计](#63-pcie核心设计)
  - [6.4 总线标准对比](#64-总线标准对比)
- [07.现代总线架构](#07现代总线架构)
  - [7.1 传统南北桥架构](#71-传统南北桥架构)
  - [7.2 现代集成架构](#72-现代集成架构)
  - [7.3 NUMA架构](#73-numa架构)
  - [7.4 对编程的影响](#74-对编程的影响)
- [08.综合案例NVMe读取](#08综合案例nvme读取)
  - [8.1 全景路径图](#81-全景路径图)
  - [8.2 10个步骤拆解](#82-10个步骤拆解)
  - [8.3 知识对号入座](#83-知识对号入座)
...
```mermaid
flowchart TB
    A[💥 mysql-B p99 延迟 +30%<br/>CPU 在 node1 内存却在 node0] --> B[Q1: 为什么跨节点慢？]
    B --> C["答：QPI/UPI 总线距离远<br/>本地 80ns vs 远程 150ns"]
    C --> D[Q2: 为什么内存距离不一样？]
    D --> E["答：NUMA 不是共享总线<br/>每节点有专属内存"]
    E --> F[Q3: 总线有多复杂？]
    F --> G["答：多层互联网络<br/>片内→DDR→PCIe→QPI"]
    G --> H[Q4: 写代码要操心这个吗？]
    H --> I["答：NUMA感知编程<br/>线程和数据<br/>放同一节点"]
    I --> J[✅ 学会：总线是<br/>分层的、有远有近的<br/>不是一条铜线]
...
flowchart LR
    subgraph P2P["❌ 点对点全连接"]
        ...
        P_CPU <--> P_NIC[网卡]
        P_MEM <--> P_GPU
        P_NOTE["N 设备 = N×(N-1)/2 连线<br/>5 设备 = 10 组连线！"]
    end
    subgraph BUS["✅ 共享总线"]
        direction LR
        B_BUS[公共总线]
        ...
        B_NIC[网卡] --- B_BUS
        B_HDD[硬盘] --- B_BUS
        B_NOTE2["N 设备 = 1 组连线<br/>但同一时刻只能一对通信"]
    end
    
    style P2P fill:#FFE4E1
    style BUS fill:#E0FFE0
```
...
flowchart TB
    BUS["🔌 总线 Bus"]
    BUS --> DATA["📦 数据总线 Data Bus<br/>宽度=一次传输位数<br/>32位/64位"]
    BUS --> ADDR["📍 地址总线 Address Bus<br/>宽度=最大寻址空间<br/>32位→4GB，64位→16EB"]
    BUS --> CTRL["🎛️ 控制总线 Control Bus<br/>读/写/中断/总线请求"]
    
    style DATA fill:#87CEEB,color:#000
    style ADDR fill:#FFD700,color:#000
...
sequenceDiagram
    ...
    participant AB as 地址总线
    participant CB as 控制总线
    participant DB as 数据总线
    participant SLAVE as 从设备(内存)
    
    MASTER->>AB: ① 发送地址 0x0000_1000
    MASTER->>CB: ② 发送 READ 信号
    AB-->>SLAVE: 地址到达
    CB-->>SLAVE: 读命令到达
    SLAVE->>SLAVE: ③ 根据地址取出数据
    SLAVE->>DB: ④ 数据 0x12345678
    DB-->>MASTER: ⑤ CPU 读取数据
```
...
**三层功能的直观关系**：片内总线是"芯片里的走廊"，系统总线是"主楼到配楼的大马路"，I/O 总线是"配楼里的小巷子"——越往里越快、越窄；越往外越慢、越宽。
...
```mermaid
flowchart TB
    subgraph PAR["并行总线 如旧式 PCI"]
        direction LR
        P1["线1: 0 1 0 1"]
        P2["线2: 1 0 1 0"]
        P3["线3: 0 0 1 1"]
        P4["线4: 1 1 0 0"]
        P_NOTE["❌ 多线间串扰<br/>❌ 时序偏移 skew<br/>→ 频率无法超过几百 MHz"]
    end
    subgraph SER["串行总线 如 PCIe"]
        direction LR
        S1["线1: 0101010101010101"]
        S_NOTE["✅ 无串扰<br/>✅ 频率可达 GHz 级<br/>✅ 多 Lane 并行 = 带宽 ×N"]
    end
    
    style PAR fill:#FFE4E1
    style SER fill:#E0FFE0
```
...
```mermaid
flowchart LR
    LANE["PCIe Lane<br/>差分信号对"]
    LANE --> X1["×1: ~2 GB/s<br/>网卡/声卡"]
    LANE --> X4["×4: ~8 GB/s<br/>NVMe SSD"]
    LANE --> X8["×8: ~16 GB/s<br/>RAID 卡"]
    LANE --> X16["×16: ~32 GB/s<br/>高端显卡"]
    
    style X1 fill:#E0FFE0,color:#000
    style X4 fill:#FFE4B5,color:#000
    style X8 fill:#D1ECF1,color:#000
    style X16 fill:#FFB6C1,color:#000
```
...
flowchart TB
    PERF["总线性能"]
    PERF --> BW["📊 带宽 Bandwidth<br/>单位时间传多少数据<br/>→ 影响大文件传输"]
    PERF --> LAT["⏱️ 延迟 Latency<br/>请求到响应的时间<br/>→ 影响随机小数据访问"]
    BW -.-> BIG[大文件快]
    LAT -.-> SMALL[小文件也快]
    
    style BW fill:#87CEEB,color:#000
    style LAT fill:#FFD700,color:#000
```
...
```mermaid
flowchart LR
    CPU["⚙️ CPU<br/>我要读内存！"] -->|请求| ARB["🎛️ 总线仲裁器<br/>同一时刻<br/>只能一个主设备"]
    DMA["📦 DMA<br/>我也要写内存！"] -->|请求| ARB
    ARB -->|"授权<br/>(DMA 优先<br/>因为它不能等)"| DMA
    ARB -->|"等待"| CPU
    
    style ARB fill:#FFD700,color:#000
```
...
```mermaid
flowchart TB
    TX["📦 事务层 Transaction Layer<br/>数据包组装/拆解 TLP"]
    DL["🔗 数据链路层 Data Link Layer<br/>流控 · 纠错 · 重传 · ACK/NAK"]
    PHY["⚡ 物理层 Physical Layer<br/>串行 SerDes · 差分信号 · GT/s"]
    
    APP["应用程序数据"] --> TX
    TX --> DL
    DL --> PHY
    PHY --> WIRE["PCIe 物理链路<br/>差分信号线对"]
    
    style TX fill:#87CEEB,color:#000
    style DL fill:#FFD700,color:#000
    style PHY fill:#90EE90,color:#000
```
...
```mermaid
flowchart TB
    subgraph CPU_CHIP["CPU 芯片（集成北桥功能）"]
        CORE["CPU 核心"]
        IMC["集成内存控制器 IMC<br/>（原北桥功能）"]
        PCIE_ROOT["PCIe Root Complex<br/>（原北桥功能）"]
        GPU_INT["核显 GPU（可选）"]
    end
    subgraph PCH["PCH 平台控制集线器<br/>（相当于南桥）"]
        USB["USB 控制器"]
        SATA["SATA 控制器"]
        PCIE_SLOW["PCIe 低速"]
        NET["网络/音频"]
    ...
    CORE --> IMC
    IMC <-->|"DDR 总线"| MEM["📝 DRAM 内存"]
    PCIE_ROOT <-->|"PCIe x16"| GPU_EXT["显卡"]
    PCIE_ROOT <-->|"PCIe x4"| NVME["NVMe SSD"]
    PCIE_ROOT -->|"DMI 4.0"| PCH
    PCH --> USB
    PCH --> SATA
    PCH --> PCIE_SLOW
    
    style CPU_CHIP fill:#90EE90,color:#000
    style PCH fill:#FFD700,color:#000
```

优势：
  - CPU 直接与内存通信，不再经过北桥，延迟大幅降低
  - PCIe 设备直连 CPU，带宽更高
  - **代价**：DMI 是 CPU ↔ PCH 的单一通道，所有低速设备都走这条路——但有专有协议，不会像旧式共享总线那样严 重竞争
...
```mermaid
flowchart LR
    subgraph A["mysql-A ✅ 本地"]
        A_CPU["node 0 CPU"] <-->|"DDR ~80ns"| A_MEM["node 0 内存"]
    end
    subgraph B["mysql-B ❌ 跨节点"]
        B_CPU["node 1 CPU"] <-.->|"UPI ~150ns"| B_CPU0["node 0 IMC"]
        B_CPU0 <-->|"DDR"| B_MEM["node 0 内存"]
    end
