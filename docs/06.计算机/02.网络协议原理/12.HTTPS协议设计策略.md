...
#### 目录介绍
- [01.工作案例引入](#01工作案例引入)
  - [1.1 一个小程序上线的HTTPS五连击](#11-一个小程序上线的HTTPS五连击)
  - [1.2 事故背后的HTTPS安全知识图谱](#12-事故背后的HTTPS安全知识图谱)
- [02.为何会有Https](#02为何会有Https)
  - [2.1 Http的缺点](#21-Http的缺点)
  - [2.2 Http缺点解决方案](#22-Http缺点解决方案)
  - [2.3 Http的风险](#23-Http的风险)
  - [2.4 如何避免风险](#24-如何避免风险)
- [03.解决方案分析](#03解决方案分析)
  - [3.1 Https加密方式](#31-Https加密方式)
  - [3.2 SSL是什么](#32-SSL是什么)
  - [3.3 SSL原理是什么](#33-SSL原理是什么)
- [04.SSL存在隐患](#04SSL存在隐患)
  - [4.1 RSA验证的隐患](#41-RSA验证的隐患)
  - [4.2 中间方伪造公钥和私钥](#42-中间方伪造公钥和私钥)
  - [4.3 存在两类问题](#43-存在两类问题)
- [05.CA证书解决SSL隐患](#05CA证书解决SSL隐患)
  - [5.1 如何解决SSL隐患](#51-如何解决SSL隐患)
  - [5.2 CA证书流程原理](#52-CA证书流程原理)
- [06.Https工作流程](#06Https工作流程)
  - [6.1 工作流程图](#61-工作流程图)
  - [6.2 详细流程说明](#62-详细流程说明)
- [07.Https真安全吗](#07Https真安全吗)
  - [7.1 Https代理了解](#71-Https代理了解)
  - [7.2 charles抓包流程](#72-charles抓包流程)
- [08.Https性能分析](#08Https性能分析)
  - [8.1 HTTPS性能损耗](#81-HTTPS性能损耗)
  - [8.2 HTTPS接入优化](#82-HTTPS接入优化)
- [09.TLS协议深度解析](#09TLS协议深度解析)
  - [9.1 TLS记录协议](#91-TLS记录协议)
  - [9.2 TLS握手协议详解](#92-TLS握手协议详解)
  - [9.3 密钥导出过程](#93-密钥导出过程)
  - [9.4 TLS 1.3的改进](#94-TLS-13的改进)
- [10.HTTPS安全攻防](#10HTTPS安全攻防)
  - [10.1 SSL剥离攻击](#101-SSL剥离攻击)
  - [10.2 降级攻击防护](#102-降级攻击防护)
  - [10.3 证书固定(Pinning)](#103-证书固定(Pinning))
  - [10.4 HPKP的兴衰](#104-HPKP的兴衰)
- [11.HTTPS部署实践](#11HTTPS部署实践)
  - [11.1 证书申请与管理](#111-证书申请与管理)
  - [11.2 服务器配置最佳实践](#112-服务器配置最佳实践)
  - [11.3 安全评估与测试](#113-安全评估与测试)
  - [11.4 常见问题排查](#114-常见问题排查)
- [12.综合案例：从0到1部署生产级HTTPS服务](#12综合案例：从0到1部署生产级HTTPS服务)
  - [12.1 案例背景与目标](#121-案例背景与目标)
  - [12.2 第一站：HTTP裸奔（零安全基线）](#122-第一站：HTTP裸奔（零安全基线）)
  - [12.3 第二站：自签名证书（有加密没信任）](#123-第二站：自签名证书（有加密没信任）)
  - [12.4 第三站：Let's Encrypt（信任但不完美）](#124-第三站：Let's-Encrypt（信任但不完美）)
  - [12.5 第四站：生产级加固（A+评级）](#125-第四站：生产级加固（A+评级）)
  - [12.6 四种方案横向对比](#126-四种方案横向对比)
  - [12.7 案例升华：HTTPS的攻防哲学](#127-案例升华：HTTPS的攻防哲学)
  - [12.8 全文知识图谱回顾](#128-全文知识图谱回顾)
- [13.思考题与作业](#13思考题与作业)
  - [13.1 基础思考题](#131-基础思考题)
  - [13.2 进阶思考题](#132-进阶思考题)
  - [13.3 动手作业](#133-动手作业)
## 01.工作案例引入
...
TLS 1.2的完整握手需要**2个RTT**（客户端→服务器→客户端→服务器），即两次网络往返。在100ms RTT的移动网络下，仅握手就消耗200ms。TLS 1.3将握手优化到**1个RTT**，甚至推出了**0-RTT**模式（利用之前会话的PSK密钥，第一个包就携带加密数据）。但0-RTT有重放攻击风险——攻击者可以录下0-RTT数据包并重新发送。这就是TLS设计的永恒困境：**安全性和性能永远在博弈**。理解这个博弈，你就理解了为什么HTTPS的部署优化是一个持续的过程。
