...
#### 目录介绍
- [01.工作案例引入](#01工作案例引入)
  - [1.1 一次"服务挂了"的排查全记录](#11-一次"服务挂了"的排查全记录)
  - [1.2 故障背后的排查知识图谱](#12-故障背后的排查知识图谱)
...
- [09.CURL网络请求工具](#09CURL网络请求工具)
  ...
  - [9.2 CURL访问网络](#92-CURL访问网络)
  - [9.3 CURL的GET请求](#93-CURL的GET请求)
  - [9.4 CURL的POST请求](#94-CURL的POST请求)
  - [9.5 CURL文件上传](#95-CURL文件上传)
  - [9.6 CURL文件下载](#96-CURL文件下载)
  - [9.7 CURL添加请求头](#97-CURL添加请求头)
  - [9.8 CURL通信过程](#98-CURL通信过程)
  - [9.9 CURL其他的实践](#99-CURL其他的实践)
- [10.综合案例：一个完整故障的逐层排查实战](#10综合案例：一个完整故障的逐层排查实战)
  - [10.1 案例背景与目标](#101-案例背景与目标)
  - [10.2 第一层：用户侧验证（能访问吗？）](#102-第一层：用户侧验证（能访问吗？）)
  - [10.3 第二层：应用层排查（HTTP返回什么？）](#103-第二层：应用层排查（HTTP返回什么？）)
  - [10.4 第三层：传输层排查（端口通不通？）](#104-第三层：传输层排查（端口通不通？）)
  - [10.5 第四层：网络层排查（路径上卡在哪？）](#105-第四层：网络层排查（路径上卡在哪？）)
  - [10.6 第五层：链路层排查（网线/交换机/ARP）](#106-第五层：链路层排查（网线/交换机/ARP）)
