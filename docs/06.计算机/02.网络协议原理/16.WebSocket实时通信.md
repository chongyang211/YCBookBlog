...
#### 目录介绍
- [01.工作案例引入](#01工作案例引入)
  - [1.1 一个"消息已读"功能的技术选型之路](#11-一个"消息已读"功能的技术选型之路)
  - [1.2 选型背后的实时通信知识图谱](#12-选型背后的实时通信知识图谱)
- [02.为什么需要WebSocket](#02为什么需要WebSocket)
  - [2.1 HTTP的单向限制](#21-HTTP的单向限制)
  - [2.2 短轮询的代价](#22-短轮询的代价)
  - [2.3 长轮询的改进](#23-长轮询的改进)
  - [2.4 SSE单向推送](#24-SSE单向推送)
  - [2.5 四种方案对比](#25-四种方案对比)
- [03.WebSocket握手与协议升级](#03WebSocket握手与协议升级)
  - [3.1 HTTP升级机制](#31-HTTP升级机制)
  - [3.2 握手的完整流程](#32-握手的完整流程)
  - [3.3 Sec-WebSocket-Key/Accept的计算](#33-Sec-WebSocket-Key/Accept的计算)
  - [3.4 扩展协商](#34-扩展协商)
- [04.WebSocket帧格式深度解析](#04WebSocket帧格式深度解析)
  - [4.1 帧结构概览](#41-帧结构概览)
  - [4.2 FIN/RSV/Opcode/MASK/Payload](#42-FIN/RSV/Opcode/MASK/Payload)
  - [4.3 控制帧：Close/Ping/Pong](#43-控制帧：Close/Ping/Pong)
  - [4.4 掩码设计的深意](#44-掩码设计的深意)
  - [4.5 分片传输](#45-分片传输)
- [05.心跳与保活机制](#05心跳与保活机制)
  - [5.1 为什么需要心跳](#51-为什么需要心跳)
  - [5.2 Ping/Pong帧的使用](#52-Ping/Pong帧的使用)
  - [5.3 中间代理的超时](#53-中间代理的超时)
  - [5.4 重连策略设计](#54-重连策略设计)
- [06.服务端架构设计](#06服务端架构设计)
  - [6.1 连接管理](#61-连接管理)
  - [6.2 消息广播](#62-消息广播)
  - [6.3 水平扩展](#63-水平扩展)
  - [6.4 背压处理](#64-背压处理)
- [07.安全与优化](#07安全与优化)
  - [7.1 WebSocket安全风险](#71-WebSocket安全风险)
  - [7.2 消息压缩](#72-消息压缩)
  - [7.3 二进制传输](#73-二进制传输)
  - [7.4 连接数优化](#74-连接数优化)
- [08.综合案例：从0到1搭建实时聊天系统](#08综合案例：从0到1搭建实时聊天系统)
  - [8.1 案例背景与目标](#81-案例背景与目标)
  - [8.2 第一代：短轮询（能跑就行）](#82-第一代：短轮询（能跑就行）)
  - [8.3 第二代：长轮询（假装实时）](#83-第二代：长轮询（假装实时）)
  - [8.4 第三代：WebSocket（真正的全双工）](#84-第三代：WebSocket（真正的全双工）)
  - [8.5 第四代：生产级WebSocket（心跳+重连+分布式）](#85-第四代：生产级WebSocket（心跳+重连+分布式）)
  - [8.6 四种方案横向对比](#86-四种方案横向对比)
  - [8.7 案例升华：WebSocket的设计哲学](#87-案例升华：WebSocket的设计哲学)
...
### 1.5 核心原理：WebSocket为什么不是普通Socket

WebSocket虽然名字里有Socket，但它和TCP Socket是完全不同的抽象层。TCP Socket在传输层，WebSocket在应用层。WebSocket连接经历三个阶段：**HTTP Upgrade握手**（客户端发`Upgrade: websocket`，服务器回`101 Switching Protocols`，复用HTTP 80/443端口穿透防火墙）→ **帧协议通信**（每条消息封装成帧，带opcode标识文本/二进制/关闭/心跳）→ **双向全双工**（客户端和服务器可以随时互发，不需要请求-响应对称）。理解WebSocket的本质是"HTTP握手+自定义帧协议"而非"HTTP之上"，你就能避开把WebSocket当HTTP用的常见陷阱。
