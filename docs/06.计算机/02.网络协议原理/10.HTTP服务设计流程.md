...
#### 目录介绍
- [01.工作案例引入](#01工作案例引入)
  - [1.1 一个HTTP服务的性能攻坚之路](#11-一个HTTP服务的性能攻坚之路)
  - [1.2 问题背后的HTTP服务知识图谱](#12-问题背后的HTTP服务知识图谱)
- [02.HTTP服务架构](#02HTTP服务架构)
  - [2.1 Web服务器的角色](#21-Web服务器的角色)
  - [2.2 服务端处理模型](#22-服务端处理模型)
  - [2.3 经典服务器架构](#23-经典服务器架构)
- [03.请求处理流水线](#03请求处理流水线)
  - [3.1 接收连接](#31-接收连接)
  - [3.2 解析请求](#32-解析请求)
  - [3.3 路由分发](#33-路由分发)
  - [3.4 处理业务逻辑](#34-处理业务逻辑)
  - [3.5 构建响应](#35-构建响应)
  - [3.6 发送响应](#36-发送响应)
- [04.并发模型设计](#04并发模型设计)
  - [4.1 单线程阻塞模型](#41-单线程阻塞模型)
  - [4.2 多进程/多线程模型](#42-多进程/多线程模型)
  - [4.3 IO多路复用模型](#43-IO多路复用模型)
  - [4.4 异步IO模型](#44-异步IO模型)
  - [4.5 各模型对比](#45-各模型对比)
- [05.HTTP服务优化](#05HTTP服务优化)
  - [5.1 Keep-Alive连接复用](#51-Keep-Alive连接复用)
  - [5.2 Pipeline管线化](#52-Pipeline管线化)
  - [5.3 压缩传输](#53-压缩传输)
  - [5.4 分块传输编码](#54-分块传输编码)
- [06.Web服务器深度剖析](#06Web服务器深度剖析)
  - [6.1 Nginx架构设计](#61-Nginx架构设计)
  - [6.2 事件驱动的本质](#62-事件驱动的本质)
  - [6.3 Worker进程模型](#63-Worker进程模型)
  - [6.4 Nginx与Apache对比](#64-Nginx与Apache对比)
- [07.请求解析的细节](#07请求解析的细节)
  - [7.1 HTTP解析状态机](#71-HTTP解析状态机)
  - [7.2 请求体处理策略](#72-请求体处理策略)
  - [7.3 慢速攻击与防御](#73-慢速攻击与防御)
  - [7.4 请求大小限制](#74-请求大小限制)
- [08.负载均衡策略](#08负载均衡策略)
  - [8.1 负载均衡算法](#81-负载均衡算法)
  - [8.2 健康检查机制](#82-健康检查机制)
  - [8.3 会话保持方案](#83-会话保持方案)
  - [8.4 灰度发布设计](#84-灰度发布设计)
- [09.综合案例：HTTP服务性能的四次进化](#09综合案例：HTTP服务性能的四次进化)
  - [9.1 案例背景与目标](#91-案例背景与目标)
  - [9.2 第一代：单线程原型（能跑就行）](#92-第一代：单线程原型（能跑就行）)
  - [9.3 第二代：多线程+Keep-Alive（略有改进）](#93-第二代：多线程+Keep-Alive（略有改进）)
  - [9.4 第三代：Nginx反向代理（架构分层）](#94-第三代：Nginx反向代理（架构分层）)
  - [9.5 第四代：全链路优化（逼近极限）](#95-第四代：全链路优化（逼近极限）)
  - [9.6 四种方案横向对比](#96-四种方案横向对比)
  - [9.7 案例升华：现代Web架构的本质](#97-案例升华：现代Web架构的本质)
...
### 1.5 核心原理：HTTP服务的并发模型

HTTP服务处理并发请求有三种经典模型：**多进程**（Apache prefork，每个连接一个进程，最稳定但内存开销大）、**多线程**（Tomcat，线程池复用减少创建开销）、**事件驱动**（Nginx/Node.js，单线程+事件循环，用epoll监控成千上万个连接）。三种模型的效率天花板是C10K问题的经典答案：多进程/线程受限于内存（每个连接消耗几MB），事件驱动模型受限于CPU（单线程epoll处理10K连接CPU不到10%）。选择哪种模型取决于你的场景——CPU密集用多线程，I/O密集用事件驱动。
