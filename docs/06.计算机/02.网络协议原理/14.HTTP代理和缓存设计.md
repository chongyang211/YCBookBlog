...
#### 目录介绍
- [01.工作案例引入](#01工作案例引入)
  - [1.1 缓存代理连环事故](#11-缓存代理连环事故)
  - [1.2 知识图谱](#12-知识图谱)
  - [1.5 核心原理](#15-核心原理)
- [02.HTTP代理设计](#02HTTP代理设计)
  - [2.1 什么是代理](#21-什么是代理)
  - [2.2 正向代理](#22-正向代理)
  - [2.3 反向代理](#23-反向代理)
  - [2.4 正向与反向代理对比](#24-正向与反向代理对比)
  - [2.5 代理的典型应用](#25-代理的典型应用)
- [03.HTTP缓存设计](#03HTTP缓存设计)
  - [3.1 为什么需要缓存](#31-为什么需要缓存)
  - [3.2 缓存的层级](#32-缓存的层级)
  - [3.3 强缓存机制](#33-强缓存机制)
  - [3.4 协商缓存机制](#34-协商缓存机制)
  - [3.5 缓存决策流程](#35-缓存决策流程)
- [04.CDN加速原理](#04CDN加速原理)
  - [4.1 CDN是什么](#41-CDN是什么)
  - [4.2 CDN的工作流程](#42-CDN的工作流程)
  - [4.3 CDN回源机制](#43-CDN回源机制)
- [05.代理协议与头部](#05代理协议与头部)
  - [5.1 Via头部追踪](#51-Via头部追踪)
  - [5.2 X-Forwarded头部族](#52-X-Forwarded头部族)
  - [5.3 PROXY Protocol](#53-PROXY-Protocol)
  - [5.4 透明代理设计](#54-透明代理设计)
- [06.缓存的深度设计](#06缓存的深度设计)
  - [6.1 Vary头部与缓存Key](#61-Vary头部与缓存Key)
  - [6.2 缓存失效策略](#62-缓存失效策略)
  - [6.3 缓存穿透与雪崩](#63-缓存穿透与雪崩)
  - [6.4 边缘计算与ESI](#64-边缘计算与ESI)
- [07.CDN深度架构](#07CDN深度架构)
  - [7.1 CDN多级缓存架构](#71-CDN多级缓存架构)
  - [7.2 CDN智能调度](#72-CDN智能调度)
  - [7.3 CDN安全防护](#73-CDN安全防护)
  - [7.4 CDN与HTTPS](#74-CDN与HTTPS)
- [08.综合案例](#08综合案例)
...
HTTP代理扮演三个角色：**正向代理**（替客户端访问互联网，如公司内网代理）、**反向代理**（替服务器接收请求，如Nginx）、**透明代理**（客户端和服务端都无感知，如运营商缓存节点）。三者的区别在于"谁部署、谁受益"：正向代理保护客户端，反向代理保护服务端，透明代理保护网络基础设施。CDN本质上是一个全球分布的反向代理网络——把静态内容推送到离用户最近的节点，减少传输距离带来的延迟。
