...
#### 目录介绍
- [01.工作案例引入](#01工作案例引入)
  - [1.1 一次死锁事故——秒杀系统的瘫痪](#11-一次死锁事故秒杀系统的瘫痪)
  - [1.2 为什么要深入学锁原理](#12-为什么要深入学锁原理)
- [02.锁的基本概念](#02锁的基本概念)
  - [2.1 什么是锁](#21-什么是锁)
  - [2.2 锁与事务隔离级别的关系](#22-锁与事务隔离级别的关系)
  - [2.3 快照读 vs 当前读——谁用锁](#23-快照读-vs-当前读谁用锁)
- [03.锁的粒度——全局/表/页/行](#03锁的粒度全局表页行)
  - [3.1 全局锁](#31-全局锁)
  - [3.2 表级锁](#32-表级锁)
  - [3.3 页级锁](#33-页级锁)
  - [3.4 行级锁](#34-行级锁)
- [04.共享锁与排他锁](#04共享锁与排他锁)
  - [4.1 S锁与X锁的兼容矩阵](#41-s锁与x锁的兼容矩阵)
  - [4.2 加锁方式](#42-加锁方式)
  - [4.3 索引与锁——没有索引会怎样](#43-索引与锁没有索引会怎样)
- [05.InnoDB的三种行锁](#05innodb的三种行锁)
  - [5.1 Record Lock——记录锁](#51-record-lock记录锁)
  - [5.2 Gap Lock——间隙锁](#52-gap-lock间隙锁)
  - [5.3 Next-Key Lock——临键锁](#53-next-key-lock临键锁)
  - [5.4 为什么RR下默认用Next-Key Lock](#54-为什么rr下默认用next-key-lock)
- [06.意向锁——表锁与行锁的桥梁](#06意向锁表锁与行锁的桥梁)
  - [6.1 为什么需要意向锁](#61-为什么需要意向锁)
  - [6.2 IS/IX锁的兼容性](#62-isix锁的兼容性)
- [07.自增锁Auto-Inc Lock](#07自增锁auto-inc-lock)
  - [7.1 AUTO_INCREMENT的并发问题](#71-auto_increment的并发问题)
  - [7.2 三种自增锁模式](#72-三种自增锁模式)
- [08.一条UPDATE到底加了哪些锁](#08一条update到底加了哪些锁)
  - [8.1 等值查询——唯一索引命中](#81-等值查询唯一索引命中)
  - [8.2 等值查询——普通索引命中](#82-等值查询普通索引命中)
  - [8.3 范围查询](#83-范围查询)
  - [8.4 无索引的情况](#84-无索引的情况)
- [09.死锁](#09死锁)
  - [9.1 死锁的四个必要条件](#91-死锁的四个必要条件)
  - [9.2 死锁复现——两个事务互相等待](#92-死锁复现两个事务互相等待)
  - [9.3 死锁检测与解除](#93-死锁检测与解除)
  - [9.4 死锁日志解读](#94-死锁日志解读)
  - [9.5 如何避免死锁](#95-如何避免死锁)
- [10.综合案例秒杀系统的锁优化](#10综合案例秒杀系统的锁优化)
  - [10.1 场景分析](#101-场景分析)
  - [10.2 方案对比与知识图谱](#102-方案对比与知识图谱)
- [11.思考题与作业](#11思考题与作业)
