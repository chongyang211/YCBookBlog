...
### 5.1 Story 太大怎么办——INVEST 原则

| 字母 | 含义 | 反例 | 正例 |
|---|---|---|---|
| I (Independent) | 独立于其他 Story | "完成用户系统后" | 拆成独立的注册和登录 |
| N (Negotiable) | 可协商 | 规定死了"必须用 Redis" | 只说"需要缓存"，技术选型是团队的 |
| V (Valuable) | 对用户有价值 | "重构 DAO 层"——用户无感 | 要翻译成"搜索速度提升 50%" |
| E (Estimable) | 可估算 | 太模糊，谁也不敢估 | 拆分后有明确范围 |
| S (Small) | 足够小 | 8 点以上 | 拆到 3-5 点 |
| T (Testable) | 可测试 | "用户体验要好" | "搜索后 2 秒内出结果" |

### 5.2 DoD（完成定义）的详细模板

DoD 不是"代码写完了"——是**"可以交付给用户了"**。一个可操作的 DoD 至少包含：
...
注意：**DoD 是团队级别的约定，不是每个 Story 各写各的**。
...
对工程师最关键的：
1. Planning 前看 Product Backlog——提前想好你对哪些 story 有发言权
2. 站会上暴露阻碍——"卡住了"不丢人
3. 每个 story 写好 DoD——"做完了"的标准和 PO 提前对齐
4. 大 story 按 INVEST 原则拆——拆到 3-5 点才算可执行
```

**行动清单**：
- [ ] 找 SM 确认：团队每个 story 的 DoD 写了吗？
- [ ] 下次 Planning 找一个 8 点以上的 story，拆成两个 3-5 点的
- [ ] Retro 上提一条具体的改进建议——与其抱怨流程，不如提一个可操作的建议
- [ ] 用 INVEST 原则检查当前 Sprint 的 Story——有几个不满足？
