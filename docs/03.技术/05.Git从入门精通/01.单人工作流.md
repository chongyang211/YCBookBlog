...
## 二、追根溯源：Git 里「看」和「回」的本质

在深入命令之前，先建立两个关键认知：

### 认知 1：`git log` 不是日志，是一张「时间地图」
...
## 五、撤回工作区：`git restore`（还没 add，想反悔）
...
## 六、撤回暂存区：`git restore --staged`（add 错了，后悔了）
...
## 七、回退版本：`git reset` 的三个模式（已 commit，想反悔）
