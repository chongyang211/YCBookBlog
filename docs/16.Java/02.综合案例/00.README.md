---
title: README
date: 2026-04-07 11:47:14
permalink: /pages/6cc1aa/
categories:
  - book
  - Java高级进阶课
  - 综合案例
...
| 02 | [银行账户管理系统](https://blog.csdn.net/m0_37700275/article/details/103463225) | ★★★☆☆ | 8 h | OOP 进化：`class` 封装 + **多态**（普通 / VIP / 储蓄账户）+ 异常体系 + CSV 持久化 | **第 7-10、12 章** |
| 03 | [校园身份预约系统](https://blog.csdn.net/m0_37700275/article/details/103627302) | ★★★★☆ | 10 h | 抽象身份 + **集合框架全家桶**（List/Map/Set/Queue）+ 多模块包结构 + Lambda + Comparator | **第 9-11、12 章** |
| 04 | [JSON 序列化器与内存数据库](./04.Json与内存数据库) | ★★★★☆ | 12 h | 现代 Java：**泛型** + 递归下降解析 + **反射 + 注解** 自动映射 + IO 流 + 自定义异常 | **第 10-12、14-15 章** |
| 05 | [多线程订单系统与线程池](./05.订单票务购买系统) | ★★★★★ | 14 h | 并发栈：`Thread` / `synchronized` / `ReentrantLock` / `BlockingQueue` / `ExecutorService` / `CompletableFuture` + 策略模式 | **第 11、13 章** |
| 06 | [迷你 KV 存储引擎](./06.迷你KV存储引擎器) 🎓 | ★★★★★ | 18 h | **毕业设计**：命令模式 + AOF 持久化 + 读写锁 + 守护线程 TTL + 注解扫描命令 + 自定义线程池 + 内置单测 | **第 1-15 全章** |
