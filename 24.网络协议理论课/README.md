...
- 2.2 理解四元组：用于唯一标识一个网络连接的四个要素，包括源IP地址、源端口号、目标IP地址和目标端口号。
- 2.3 服务端的编写：1.创建ServerSocket对象，绑定监听的端口；2.调用accept()方法监听客户端的请求；3.连接建立后发数据；……
- 2.4 客户端的编写：
...
- 8.1 如何理解CURL工具：CURL（Client URL） 是一个强大的命令行工具和库，用于在终端或脚本中与服务器进行数据传输。
