...
使用条件语句、循环和 console 方法实现一个支持加减乘除的简易计算器：
...
function calculator(expression) {
  ...
  switch (operator) {
    ...
    case '/':
      ...
      break;
    default:
      console.error(`不支持的运算符：${operator}`);
      return;
  }
...
// 批量计算
const expressions = ['10 + 5', '20 - 8', '6 * 7', '100 / 0', '15 / 3'];
console.time('批量计算耗时');
expressions.forEach(expr => calculator(expr));
console.timeEnd('批量计算耗时');
```

## 1.8 思考题

1. `var`、`let`、`const` 三者在变量提升、作用域和重复声明方面有什么区别？请用代码示例说明。
2. 为什么 `typeof null` 返回 `"object"` 而不是 `"null"`？这是设计如此还是历史遗留 bug？
3. 在 `switch` 语句中，如果故意省略某个 `case` 的 `break`，这种"贯穿（fall-through）"行为在什么场景下是有用的？
4. `console.time()` 和 `console.timeEnd()` 测量的时间精度如何？在生产环境中应该用什么方式来测量性能？
5. JavaScript 是解释型语言还是编译型语言？请结合 V8 引擎的 JIT 编译机制说明。
