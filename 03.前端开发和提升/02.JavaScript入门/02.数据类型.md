...
实现一个通用的类型检测函数，支持精确判断所有 JavaScript 数据类型：
...
console.log('原对象 hobbies:', original.hobbies);   // ['reading', 'coding', 'gaming'] — 被影响了！
console.log('深拷贝 hobbies:', deepCopy.hobbies);    // ['reading', 'coding'] — 不受影响
```

## 2.8 思考题

1. 为什么 `typeof null === "object"`？如果要精确判断 `null`，应该使用什么方法？
2. `0.1 + 0.2 !== 0.3` 的原因是什么？在实际开发中如何处理浮点数精度问题？
3. `Symbol` 类型有哪些实际应用场景？为什么说它适合用作对象的属性键？
4. `Object.freeze()` 和 `Object.seal()` 的区别是什么？`Object.freeze()` 能实现深冻结吗？
5. `==` 和 `===` 的类型转换规则分别是什么？在工程实践中，哪些场景下可以使用 `==`？
