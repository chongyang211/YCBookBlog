...
1. 箭头函数和普通函数在 `this` 绑定、`arguments` 对象、`new` 调用方面有什么区别？
2. 闭包会导致内存泄漏吗？在什么情况下会发生？如何避免？
3. `call`、`apply`、`bind` 三者的区别是什么？如果对一个已经 `bind` 的函数再次 `bind`，`this` 会改变吗？
4. 请手写实现一个简单的 `Function.prototype.bind` 方法。
5. 为什么说"JavaScript 中函数是一等公民"？这给编程带来了哪些便利和陷阱？
