...
- 1.1 项目重构的背景是什么：项目的代码往往牵一发而动全身，业务逻辑耦合严重；当初设计的架构让项目的依赖关系越来越复杂，维护成本也越来越高。
- 1.2 重构项目有哪些要求：1.洞察代码不足并且用合理方案解决；2.提高代码的质量；3.一定要多问自己为什么这样设计
- 1.3 项目重构遇到的问题：1.要明确项目痛点是什么；2.重构什么和如何重构；3.对重构是想到哪里改哪里
- 1.4 重构的目的是什么：重构是一种对软件内部结构的改善，目的是在不改变软件的可见行为的情况下，使其更易理解，修改成本更低。
- 1.5 重构设计目标是什么：主要是3个目标，需要解耦合（划分模块角色，明确代码层级），增强拓展性（方便拓展，保持开闭原则等），提高代码可维护性（代码可读性）
- 2.2 什么时候重构比较好：找到代码中的问题，遇到问题再重构，维护代码的过程中，真正遇到问题的时候，再对代码进行重构，能有效避免前期投入太多时间做过度的设计
- 2.3.1 如何进行大型重构操作：包括：系统、模块、代码结构、类与类之间的关系等的重构，重构的手段有：分层、模块化、解耦、抽象可复用组件等等。
- 2.3.2 如何进行小型重构操作：主要是针对类、函数、变量等代码级别的重构，比如规范命名、规范注释、消除超大类或函数、提取重复代码等等。
- 3.0 重构技术手段有哪些：需要熟练掌握各种设计原则、思想、模式，还需要对所重构的业务和代码有足够的了解。
- 3.1 大型业务重构工作事项：1.根据业务抽取抽象接口；2.为三个层次：底层、组件层和应用层；3.通过接口实现模块间的通讯；4.抽离功能模块，抽离公共视图模块，抽离公共业务模块。
- 3.6 优化编码方式有哪些：
