...
- 1.1 计算机基础组成：由 CPU（做各种复杂的计算）、内存（各种程序加载的存储空间）、主板这些设备组成的硬件。还有输入和输出设备。
- 1.2 到底应该怎么学：主要是广，深，还很难学以致用。组成原理中的概念非常多，组成原理中的很多概念特别深入，无法和自己日常的开发工作联系起来。
- 存储程序计算机如何理解：包含两个概念，一个是“可编程”计算机，一个是“存储”计算机。可编程是指硬件固定后可以做程序的逻辑运算，可存储是指
- 3.冯·诺依曼描述计算机：不管是那种计算机，都可以归纳为，运算器、控制器、存储器、输入设备和输出设备。
...
- 4.计算机基本组成有哪些：运算器、控制器、存储器、输入设备和输出设备五大基本组件。性能和功耗是重要考虑因素！
- 程序是如何在计算机上运行：
- 控制器是如何理解：控制器由程序计数器PC、指令寄存器IR和控制单元CU组成。
- 
...
- 6.7 数据大小端设计：指的是在计算机系统中存储多字节数据（如整数、浮点数）时，字节的存储顺序。


## 09.计算机内存设计原理

- 1.1 看一个实际场景：我们写的程序是怎么装载到内存里面来的呢？
- 1.2 程序装载到内存：多个文件合并成一个最终可执行文件，执行指令的时候，程序计数器是顺序地一条一条指令执行下去。这一条条指令需要连续地存储在一起。
- 1.3 虚拟和物理内存：虚拟内存是指令里用到的内存地址，物理内存是在内存硬件里面的空间地址。
- 1.4 什么是物理内存：由一组存储单元组成，每个存储单元都有一个唯一的地址。这些存储单元以字节为单位进行编址，可以存储和读取数据。
