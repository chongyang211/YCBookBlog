...
**Base64** 是一种将二进制数据编码为 ASCII 字符的编码方式。它主要用于在文本协议（如电子邮件、URL、JSON 等）中安全地传输二进制数据。理解 Base64 的编解码原理及其应用场景对于处理数据传输和存储非常重要。
...
- 编码过程将每 3 个字节分为 4 个 6 位单元，并映射为 Base64 字符。
- 解码过程将 Base64 字符转换回二进制数据。
- Base64 广泛应用于文本协议中传输二进制数据。
- 通过实现 Base64 编解码函数，可以更好地理解其原理和应用。
