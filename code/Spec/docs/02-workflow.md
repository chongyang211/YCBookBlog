...
需求录入（`/spec-intake`）· 这个是可选项

// TODO 可以将下面内容做成表格的方式呈现出来
...
技术方案（`/spec-design`）· 大需求必须，小需求可以忽略
...
大需求直接起草 spec 容易因技术方向未定而反复返工。

先做技术方案 → 团队评审 → 敲定"用什么方案、拆几个 spec" → 再基于"需求 + 方案"起草 spec，spec 会更准确、边界更清晰。
...
实施中发现 spec 与现实不符是**常态**——处理方式决定流程是否扎实。核心原则：**不沉默、不私改**（先记到 tasks「偏离记录」，再按严重度分流）。

| 严重度 | 典型场景 | 处理 |
|--------|----------|------|
| 🟢 小（minor） | 措辞 / 命名不符、补充细节（如 spec 写 `user_id` 实际字段是 `userId`） | **不暂停**：tasks 记录 → 完成当前 task → Phase 末由 Dev Owner 直接修 spec 措辞（不改 FR 边界与验收标准），修订记录追加一行 |
| 🟡 中（major） | 流程多 / 少一步、参数类型变更、验收标准漏了容错场景 | **暂停当前 task**：tasks 记录 → 找 Spec Author 15 分钟短会 → 三选一：修订 spec / 拒绝偏离按原 spec / 拆独立 spec |
| 🔴 大（escalate） | FR 边界变化、新增 / 删除功能、FR 在现有架构下无法实现 | **停止整个 spec**：tasks 记录 → 重走阶段 1 评审 → 三选一：大改当前 spec（回 draft、重生成 plan/tasks）/ 拆多个 spec / 暂停（deprecated） |

**决策树**：

```txt
我发现 spec 与现实不符
   ├─ 只是措辞 / 命名 / 补充细节？→ 🟢 继续实施 + tasks 记录 + Phase 末改 spec
   ├─ 流程 / 参数变化？          → 🟡 暂停 task + 找 Spec Author + 三选一决策
   └─ 功能边界 / FR 本身要改？   → 🔴 停止 spec + 重走评审 + 重新生成 plan/tasks
```

**回流四步**：① tasks「偏离记录」立刻记一行（日期 / 偏离点 / 原因 / 严重度）→ ② 按上表评估分流 → ③ 决策为"修订 spec"时：改 spec（修订记录追加）→ 评估 plan 是否联动改 → tasks 状态改 `spec-updated` → ④ 恢复实施，完成后 `/spec-sync` 验证 spec / plan / tasks 三方一致。

**MR 里的偏离说明**：此时 MR 描述的「偏离说明」段不能写"无"——逐条列偏离点 + 严重度 + 处理结果（已回流 / 已拒绝 / 已拆分）。
