...
完整命令链：`intake → design → draft → 评审 → plan → tasks → implement → test → review → push → sync`，共 10 个命令（其中 3 个可选），唯一强制卡口是「人工评审」。

逐命令的输入 / 输出 / AI 行为详解见 [02-workflow.md](./02-workflow.md)。
