...
- AI 的对话上下文有窗口限制，会随着对话变长而遗忘早期信息；
- 同一个 spec 可能跨会话推进，可能换人接手，可能停几天后再回来；
- 只有磁盘文件能跨"会话 / 时间 / 人"这三个边界持续存在。

### 2.1 可选扩展：Design + Intake
...
**Design 与 Plan 的边界**（最易混淆的一对）：

> **Design 决定"用什么方案、拆几个 spec"；Plan 决定"改哪些文件、按什么步骤"。**

Design 出现在 spec **之前**（方案级、可选）；Plan 出现在 spec ready **之后**（执行级、复杂改动必须）。
