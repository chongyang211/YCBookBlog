...
> 适用仓：`proto` 及你团队的协议 / IDL 仓库；以及任意端改到 `*.proto`、gRPC/HTTP 接口、错误码、序列化结构。
...
两套 proto 与命名空间一一对应（改接口先改 proto；下表为示例结构，按你团队实际替换）：

| proto 仓 | module | 服务对象 |
|----------|--------|----------|
| `proto`（主协议仓） | `<GIT-HOST>/<ORG>/proto` | 后端各服务 + 算法侧 + 设备网关 |
| `<协议仓库>`（扩展协议仓） | `<GIT-HOST>/<ORG>/<协议仓库>` | 后端业务服务 |
...
- [ ] `go_package` 前缀与 proto 仓一一对应：改接口前先定位到**正确的 proto 仓**。⚠️ 有归并历史的 proto 可能残留旧 `go_package` 前缀——改接口前先 grep 确认实际前缀，别脑补。
...
- [ ] **枚举值业务分段**要延续：状态机按 10 递增（`instruction.proto` `DISTRIBUTING=10…CANCELED=90`），操作符/业务状态按 100 分段（`deviceaccess.proto` `SearchDeviceOperator`、`<协议仓库>/comm` 业务状态）——新增值插进正确段位，别打乱。
...
- [ ] 设备网关（`<设备网关>`/`devicegateway`/`legacy-devicegateway`）的 message 变更是**老设备兼容重灾区**：字段号只加不改、老字段默认值兼容、错误码只加不改（强制走 [`device.md`](./device.md) §2.1 + SKILL.md 4.3）。
...
- [ ] 🔴 **stub 由 proto 仓流水线 `protoc` 生成回提、业务仓 `go get -u` 更新**，本地不生成——评审确认 **proto MR 先行合入**，业务代码别抢跑用还没生成的字段（否则编译不过）。protoc 版本以 proto 仓流水线配置为准。
