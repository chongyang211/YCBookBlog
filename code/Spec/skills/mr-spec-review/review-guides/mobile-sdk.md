...
> 适用：被宿主 App / 集成方集成的业务 SDK 与移动应用。
> - `mobile-sdk`：`android`（Kotlin）、`ios`（Swift）、`web`（JS/TS SDK）—— 对外发布的接入 SDK。
> - 业务 Android App（Java）（如你团队有 App 仓）。
...
- 与**设备端**：若 SDK 与设备协同（如终端联动业务设备），协议对齐见 [`device.md`](./device.md)。
