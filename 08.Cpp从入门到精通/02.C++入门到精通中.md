...
引用是一个变量的别名，它允许你通过不同的名称访问同一个变量。引用是 C++ 中非常强大的特性，常用于函数参数传递、返回值优化等场景。
...
void print(const LargeObject& obj) { // 使用 const 引用避免拷贝
    std::cout << obj.data[0];
}
```
...
* 引用必须初始化：引用在声明时必须初始化，否则会编译错误。
* 不可重新绑定：引用在初始化后，不可以改变。
...
没有空引用：引用必须绑定到一个有效的变量，不能绑定到 nullptr 或未初始化的变量。

```cpp
int* ptr = nullptr;
int& ref = *ptr; // 未定义行为，可能导致崩溃
```
