...
- 3.3.1 隐式类型转换：隐式类型转换由编译器自动完成，通常发生在赋值、表达式计算或函数调用时。
- 3.3.2 显式类型转换：C风格转换是强制转化，比如将double转化为int类型。C++风格是提供了四种显式转换运算符，更加安全和明确。
- 3.3.2.1 static_cast：用于基本数据类型之间的转换，以及具有继承关系的类之间的转换。
- 3.3.2.2 dynamic_cast：用于具有多态性的类之间的转换（通常用于向下转型），转换失败时返回 `nullptr`。
- 3.3.2.3 const_cast：用于移除或添加 `const` 或 `volatile` 修饰符。
