...
- 1.2 遇到OOM问题的疑惑：1.没办法确定具体引发OOM的地方；2.在OOM的时候如果去获取内存信息基本上是获取不到呢？原因是已经内存不足了，无法在创建数据。
- 1.3 内存分析工具有哪些：Memory Profiler：查看内存分配、对象创建和销毁的详细信息。检测内存泄漏和内存抖动。


## 14.FPS检测设计实践

- 1.1 流畅度衡量指标：FPS越大则滑动时的体验越流畅。
- 1.2 遇到问题说明：如何理解`FPS`，`FPS`是怎么计算的。
...
**FPS（Frames Per Second，帧率）** 是衡量 Android 应用或游戏流畅度的重要指标。在 Android 系统中，FPS 的设计思想和原理主要围绕 **渲染性能优化** 和 **用户体验提升** 展开。以下是 Android FPS 的设计思想和原理的详细解析：
...
### **3. FPS 的设计思想**
#### **3.1 流畅性**
- 目标：尽可能接近设备的刷新率（如 60 FPS）。
- 实现：优化 UI 线程和渲染线程的性能，减少帧丢失。

#### **3.2 稳定性**
- 目标：避免帧率波动，确保帧时间一致。
- 实现：通过 VSync 和缓冲机制，保证帧的稳定输出。

#### **3.3 响应性**
- 目标：减少用户操作的延迟。
- 实现：优化事件处理逻辑，确保用户操作能及时反映到屏幕上。
...
- 复杂的动画会导致帧率下降。
- **优化方法**：
    - 使用 `Property Animation` 代替 `View Animation`。
    - 使用 `Hardware Layer` 加速动画。
...
#### **5.4 设备性能**
- 低端设备的 GPU 和 CPU 性能有限。
- **优化方法**：
    - 根据设备性能动态调整渲染质量。
    - 使用 `RenderScript` 或 `Vulkan` 提高图形性能。
...
#### **6.2 渲染优化**
- 使用 `SurfaceView` 或 `TextureView` 进行复杂渲染。
- 减少过度绘制（Overdraw）。

#### **6.3 动画优化**
- 使用 `Lottie` 实现复杂动画。
- 使用 `Hardware Layer` 加速视图动画。
...
### **7. 总结**
Android FPS 的设计思想和原理主要围绕 **流畅性**、**稳定性** 和 **响应性** 展开，通过以下机制实现：
- **VSync** 和 **双缓冲/三缓冲** 确保帧的稳定输出。
- **UI 线程** 和 **渲染线程** 的优化减少帧丢失。
- **帧率监控工具** 帮助开发者分析和解决性能问题。
...
通过以上方法，可以显著提升 Android 应用的流畅度和用户体验。
