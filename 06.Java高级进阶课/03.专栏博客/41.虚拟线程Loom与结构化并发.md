...
🎯 **下一篇预告：第 42 篇《NIO 之 ByteBuffer 与堆外内存》**——卷五并发深水区收官，下一篇正式启动**卷六《IO、网络与序列化》**。卷六开篇直接深入 11 篇泛讲过的 NIO 三大件之一——**Buffer**：ByteBuffer 的 position/limit/capacity/mark 四指针状态机 + flip/compact/rewind/clear 四件套源码 + **HeapByteBuffer vs DirectByteBuffer** 真实拷贝路径揭秘（不止"少一次拷贝"那么简单）+ DirectByteBuffer 通过 **Cleaner + PhantomReference** 回收 native 内存的完整链路 + **Bits 类全局账簿**与 `-XX:MaxDirectMemorySize` 的真实关系 + **堆外内存泄漏 4 种典型现场 + NMT/pmap/jcmd 三件套排查实战** + Netty PooledByteBufAllocator 为什么不用 JDK 的 ByteBuffer + **零拷贝 mmap / sendfile / transferTo 在 Java API 里的对应**。**ByteBuffer 与堆外内存一次讲透。**
