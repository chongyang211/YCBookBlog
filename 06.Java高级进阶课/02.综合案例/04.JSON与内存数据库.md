...
本章是综合案例的**第四关·现代 Java 大考**——从 [03.校园身份预约](./03.校园身份预约系统.md) 的"`fromCsv` 一字段一字段 set" 跃迁到工业级框架基础设施：**反射 + 注解 + 泛型 + 设计模式**四件套，手写一个零依赖的 mini Jackson + mini MyBatis 雏形。
...
下一案例 [05.多线程订单与线程池](./05.多线程订单与线程池.md) 会做 **5 件升级**：
...
- ⬅ 上一案例：[03.校园身份预约系统](./03.校园身份预约系统.md) —— 8 种集合 + 双索引 + Comparable + Stream
- ➡ 下一案例：[05.多线程订单与线程池](./05.多线程订单与线程池.md) —— 并发容器 + ExecutorService + 生产者-消费者 + 协调工具