...
本章是综合案例的**第二关·OOP 大考**——从 [01.学生成绩](./01.学生成绩管理系统.md) 的"三个并行数组 + 静态方法"跃迁到工业级 OOP：`class` 封装 + 抽象类 + **多态三态**（普通 / VIP / 储蓄）+ **自定义异常体系** + CSV 文件持久化 + 分包架构。
...
下一案例 [03.校园身份预约系统](./03.校园身份预约系统.md) 会做 **5 件升级**：
...
- ⬅ 上一案例：[01.学生成绩管理系统](./01.学生成绩管理系统.md) —— 三个并行数组 + 静态方法 + 内存数据
- ➡ 下一案例：[03.校园身份预约系统](./03.校园身份预约系统.md) —— 集合框架登场（List/Map/Set），泛型与 Comparator，Stream + Lambda 函数式风格首秀
