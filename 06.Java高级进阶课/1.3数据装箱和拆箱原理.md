...
装箱就是：自动将基本数据类型转换为包装器类型；如下所示

```java
int a = 10;
Integer b = Integer.valueOf(a); // 显式装箱
Integer c = a;                  // 自动装箱（Java 5+）
```

### 2.2 什么是拆箱

拆箱就是：自动将包装器类型转换为基本数据类型。如下所示
...
从反编译得到的字节码内容可以看出，在装箱的时候自动调用的是`Integer`的`valueOf(int)`方法。而在拆箱的时候自动调用的是Integer的`intValue`方法。
