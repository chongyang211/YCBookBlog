...
- [15.6 综合案例：简易依赖注入框架](#156-综合案例简易依赖注入框架)
- [15.7 思考题](#157-思考题)
...
6. **反射的 Inflation 机制是什么？为什么前 15 次反射调用使用 Native 实现，之后切换为动态生成字节码？这个阈值可以配置吗？（提示：`-Dsun.reflect.inflationThreshold`）**

7. **Java 的注解处理器（Annotation Processor，APT）和反射有什么区别？Lombok 的 `@Data`、`@Getter` 是如何在编译期生成代码的？为什么它不需要 RUNTIME 保留策略？**

8. **注解和 XML 配置都能实现框架配置（如 Spring 早期用 XML，后来用注解）。两者各有什么优缺点？为什么现在主流框架都倾向于使用注解？**
