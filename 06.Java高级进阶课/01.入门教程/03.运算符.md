...
6. **以下代码中哪些表达式会导致编译错误？哪些会导致运行时错误？**
   ```java
...
7. **Java 的整数除法截断规则是"向零取整"（即 `-7/2 = -3`），而 Python 的整数除法是"向负无穷取整"（即 `-7//2 = -4`）。请分析两种方式的优缺点，以及 `Math.floorDiv()` 方法的作用。**

8. **为什么 `0.1 + 0.2 != 0.3` 在 Java（以及几乎所有编程语言）中成立？这和运算符无关，而是浮点数的表示问题。请解释 IEEE 754 浮点数标准的基本原理，以及在 Java 中如何正确比较浮点数。**

**延伸知识：运算符在不同 JDK 版本中的演进**

| 版本 | 新增运算符特性 |
|------|-------------|
| JDK 1.0 | 基础运算符完整体系 |
| JDK 5 | 自动装箱/拆箱使运算符可用于包装类型 |
| JDK 7 | switch 支持 String 类型 |
| JDK 14 | switch 表达式（`->` 语法） |
| JDK 16 | instanceof 模式匹配 |
| JDK 17 | switch 模式匹配（预览） |
| JDK 21 | switch 模式匹配（正式） |

理解运算符的底层原理和演进历史，有助于写出更高效、更安全的代码。运算符虽然是最基础的语法，但其中蕴含的设计思想值得深入思考。
