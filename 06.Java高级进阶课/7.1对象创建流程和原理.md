...
1. **类加载**：
  - 在对象创建之前，JVM 需要确保 `Student` 类和 `Person` 类都已加载。类加载过程包括加载、验证、准备、解析和初始化。

2. **内存分配**：
  - JVM 在堆内存中为 `Student` 对象分配空间。分配的大小取决于对象的字段和对齐填充。

3. **初始化零值**：
  - 在调用构造方法之前，JVM 会将对象的字段初始化为默认值，确保对象的状态是确定的。

4. **构造方法调用**：
  - 构造方法用于初始化对象的字段。子类的构造方法会先调用父类的构造方法，确保父类的字段被正确初始化。

5. **对象引用**：
  - 对象创建完成后，JVM 将对象的引用返回给调用者。调用者可以通过引用访问对象的字段和方法。
