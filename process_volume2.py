...
def extract_info(filepath):
    ...
    for line in lines:
        if '核心诗词：' in line:
            ...
            # 清理格式
            core_poem = core_poem.replace('## ', '').replace('> ', '').replace('核心诗词：', '').strip()
            break
    ...
    for line in lines:
        if '精神痛点：' in line:
            ...
            pain_point = pain_point.replace('## ', '').replace('> ', '').replace('精神痛点：', '').strip()
            break
    
    # 如果没有找到核心诗词，尝试从文件名或内容推断
    if not core_poem:
        # 根据文件名猜测
        if '走踏清秋' in filepath:
            core_poem = '李贺《南园》「男儿何不带吴钩，收取关山五十州」'
        elif '不破楼兰' in filepath:
            core_poem = '王昌龄《从军行》「黄沙百战穿金甲，不破楼兰终不还」'
        elif '生当人杰' in filepath:
            core_poem = '李清照《夏日绝句》「生当作人杰，死亦为鬼雄」'
        elif '落红护花' in filepath:
            core_poem = '龚自珍《己亥杂诗》「落红不是无情物，化作春泥更护花」'
        elif '家祭无忘' in filepath:
            core_poem = '陆游《示儿》「王师北定中原日，家祭无忘告乃翁」'
        elif '铁马冰河' in filepath:
            core_poem = '陆游《十一月四日风雨大作》「铁马冰河入梦来」'
        elif '留取丹心' in filepath:
            core_poem = '文天祥《过零丁洋》「人生自古谁无死？留取丹心照汗青」'
...
def main():
    volume_dir = '/Users/yc/YCBookBlog/09.出版作品的书籍/06.品读唐诗宋词/02.少年立志'
    ...
    for filename in files:
        ...
        title, core_poem, pain_point = extract_info(filepath)
        print(f"  标题: {title}")
