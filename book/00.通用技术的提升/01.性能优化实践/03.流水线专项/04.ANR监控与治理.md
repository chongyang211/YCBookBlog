...
| 视图层级太深 | 重构布局（[`卷三·05`](05.页面UI与布局优化.md)）|
| onClick 中做 IO | 异步化 |
...
[`卷四·01 §14.3`](../04.业务专项篇/01.App冷启动优化.md) 已述。**核心思想**：
...
| 场景 | 应对 |
|---|---|
| 启动 hang | 优化启动期（[`卷四·01`](../04.业务专项篇/01.App冷启动优化.md)）|
...
**收益**：详见 [`卷四·01 §13`](../04.业务专项篇/01.App冷启动优化.md)。冷启动篇的金融 App 案例 4.2s→1.4s 直接降低 D 类 ANR。
...
**做法**：用 LooperPrinter 详见 [`卷三·03 §3.1`](03.卡顿捕获与归因.md)。
...
提出**统一假设**："SDK 分级 init + 主线程减负"（同 [`卷四·01 §18.2`](../04.业务专项篇/01.App冷启动优化.md) 案例）。
